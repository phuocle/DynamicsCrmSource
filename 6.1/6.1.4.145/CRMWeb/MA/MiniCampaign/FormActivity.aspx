...
var queryString = Mscrm.CrmUri.create("/MA/MiniCampaign/iframes/activityForm.aspx?objectTypeCode=" + WizardGetProperty("SelectedItem"));
...
var callbackFunction = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterBulkEmail", this,false);
...
{
frameBehavior.GetBehavior(idArray[i]).RemoveItem(frameBehavior.GetBehavior(idArray[i]).get_dataValue()[0].id);
}
...
$addHandler(frameBehavior.GetBehavior(idArray[i]).get_frame(), 'load', callPopulateEmailBody);
}
...
WizardSetProperty("ActivityXml", _activityFormFrame.document.getElementById('crmFormSubmit').crmFormSubmitXml.value);
}
...
{
_activityFormFrame.$find("crmForm").BuildXml(false, false);
var formXml = XUI.Xml.LoadXml(_activityFormFrame.document.getElementById('crmFormSubmit').crmFormSubmitXml.value);
...
if (typeof (_activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(firstNode.nodeName).Items) != "undefined") {
dataArray[i] = _activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(firstNode.nodeName).Items(false, true, undefined, undefined, true);
}
else if (_activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(firstNode.nodeName).get_dataValueAsDate) {
dataArray[i] = _activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(firstNode.nodeName).get_dataValueAsDate().toString();
}
else {
dataArray[i] = _activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(firstNode.nodeName).get_dataValue();
}
}
...
try {
$removeHandler(_activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior('idDescription').get_frame(), 'load', callPopulateEmailBody);
}
catch (e)
{ }
}
...
<img id="insertTemplateImg" src="/_imgs/htmlbar/cmd-insertField.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Dialog_BulkEmail_Template_SubTitle' runat='server'/>" /></a>
</div>
</td>
</tr>
</table>
