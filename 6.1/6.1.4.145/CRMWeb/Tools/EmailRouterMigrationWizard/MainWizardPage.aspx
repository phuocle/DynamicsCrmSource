...
&& ((profileSelectionPageVisible || summaryPageVisible || migrationInProcess) && !confirm(LOCID_CONFIRM_CANCEL_MIGRATION))) {
return;
}
RemoveEventListenersFromFileControls();
cancelClicked = true;
...
if (profileSelectionPageVisible) {
...
if (shortName == LOCID_ROUTER_ENCRYPTION_XML.toLowerCase() && ((fileBitMapper & EncryptionFileBitLocation) == 0)) {
fileBitMapper = fileBitMapper | EncryptionFileBitLocation;
totalFileCount++;
}
else if (shortName == LOCID_ROUTER_SYSTEMSTATE_XML.toLowerCase() && ((fileBitMapper & SystemStateFileBitLocation) == 0)) {
fileBitMapper = fileBitMapper | SystemStateFileBitLocation;
totalFileCount++;
}
else if (shortName == LOCID_ROUTER_EMAILAGENT_XML.toLowerCase() && ((fileBitMapper & EmailAgentFileBitLocation) == 0)) {
fileBitMapper = fileBitMapper | EmailAgentFileBitLocation;
totalFileCount++;
} else { fileNotSupported = true;}
}
}
if (fileNotSupported || (totalFileCount > 0 && totalFileCount < 3)) {
alert(LOCID_INCOMPLETE_ROUTER_FILES);
...
<li id="serverprofilestep" class="mscrm-iw-orderedListItem"><loc:Text ResourceId="MigrationWizard.CreatingEmailServerProfiles" runat="server"/></li>
<li id="forwardmailboxstep" class="mscrm-iw-orderedListItem"><loc:Text ResourceId="MigrationWizard.CreatingForwardMailboxes" runat="server"/></li>
<li id="userqueueupdatestep" class="mscrm-iw-orderedListItem"><loc:Text ResourceId="MigrationWizard.UpdatingUserQueueMailboxes" runat="server"/></li>
</ol>
...
<ul class="mscrm-iw-unorderlist">
<li><loc:Text ResourceId="MigrationWizard.Step_1_ToFollow.Description" runat="server"/></li>
<li><loc:Text ResourceId="MigrationWizard.Step_2_ToFollow.Description" runat="server"/></li>
</ul>
</div>
</div>
