...
if(XUI.Html.GetComputedStyle(subject, "display") != "none" && XUI.Html.GetComputedStyle(subject, "visibility") != "hidden" && !subject.disabled)
{
subject.focus();
}
...
function OnLoad()
{
try{window.opener.auto($get('crmFormSubmit').crmFormSubmitObjectType.value);}catch(e){};
}
...
<div class="ms-crm-Dialog-Header-Desc" id="DlgHdDesc"><loc:Text ResourceId="Attachment_Form_Description" runat="server"/></div>
</div>
<% } else { %>
<div height="28">
...
<div <%	if(_workflowNote) { %> class="ms-crm-FormBodyContainer ms-crm-FormBodyRIExpanded" <% } else { %> class="ms-crm-FormBodyContainer" style="left:0px;right:0px" <% } %> id="formBodyContainer">
...
<div id="tr_desc" style="height:100%;display:<%if (_hideDescription){%>none<%}else{%>block<%}%>;">
...
<col width="80"><col>
<tr>
<td class="ms-crm-Field-Required" id="subject_c"><label for="subject"><loc:Text ResourceId="Web.Notes.edit.aspx_97" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/></label></td>
<td><sdk:TextBoxControl id="subject" Required="true" MaxLength="255" runat="server"/></td>
</tr>
<tr height="100%">
<td colspan="2"><sdk:TextAreaControl id="notetext" runat="server" TitleResource="Web.Notes.textarea.title" Height="135px"/></td>
</tr>
<tr>
<td colspan="2" nowrap>
<table cellpadding="0" cellspacing="0"  width="100%"><col width="80"><col>
<tr><td class="ms-crm-Field-Normal" id="regarding_c"><label for="crmOwnerLookup"><loc:Text ResourceId="Notes_Parent_Entity" runat="server"/></label></td>
<frm:FormCell runat="server" ShowLabel="True"><sdk:LookupControl id="crmOwnerLookup" AccessibilityLabelResourceId="Notes_Parent_Entity" LookupStyle="single" runat="server" /><div style="display:none"><sdk:LookupControl id="ownerid" LookupStyle="single" runat="server" /></div></frm:FormCell>
</tr>
</table>
</td>
</tr>
