...
cur.html = "<NOBR>" + CrmEncodeDecode.CrmHtmlEncode(XUI.Html.GetText(item)) + "</NOBR>";

cur.name = _isInlineMultiLookup ? XUI.Html.GetText(item).substr(0, XUI.Html.GetText(item).length - 1) : XUI.Html.GetText(item);
...
multiLookupControlsDiv.style.height = (window.innerHeight - 333) + 'px';
...
function selectItem() {
Mscrm.Utilities.setReturnValue(buildReturnValue($find("crmGrid").get_innerGrid().get_selectedRecords(), true, $find("crmGrid").get_entityTypeName()));
closeWindow(true);
}
...
<div id="tblFindGridContainer" class="tblFindGridContainer" onclick="if(typeof(onGridSelection) == 'function'){onGridSelection();}" onkeyup="if(typeof(onGridSelection) == 'function'){onGridSelection();}">
