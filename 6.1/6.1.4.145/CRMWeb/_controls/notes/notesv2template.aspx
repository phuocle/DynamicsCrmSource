...
<div class="deleteActionGradientMask"/>
...
<div class="ms-crm-Inline-GradientMask notesTitleGradient"/></input>
...
title="{{if ($data.text != null && $data.text != '') }}${CrmEncodeDecode.CrmHtmlAttributeEncode(text)}{{else}}<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.NewNoteHintText")) %>{{/if}}"
alt="{{if ($data.text != null && $data.text != '') }}${CrmEncodeDecode.CrmHtmlAttributeEncode(text)}{{else}}<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.NewNoteHintText")) %>{{/if}}"
aria-label="<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.NewNoteHintText")) %>"
WatermarkText="<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.NewNoteHintText")) %>"
{{else}}
title="{{if ($data.text != null && $data.text != '') }}${CrmEncodeDecode.CrmHtmlAttributeEncode(text)}{{else}}<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.DescriptionHintText")) %>{{/if}}"
alt="{{if ($data.text != null && $data.text != '') }}${CrmEncodeDecode.CrmHtmlAttributeEncode(text)}{{else}}<%= CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("NotesV2.DescriptionHintText")) %>{{/if}}"
...
onkeydown="if (event.keyCode==46 || event.keyCode==127){ var domEvent=new Sys.UI.DomEvent(event); domEvent.stopPropagation();} else if (event.keyCode==13 && event.shiftKey ){ dispatchNoteCommand(this, 'post', '${postId}'); return false;} else if(event.keyCode==9){dispatchNoteCommand(this, 'onblur', '${postId}');}"
onfocus="dispatchNoteCommand(this, 'edit', '${postId}');return false;"
>{{if ($data.text == null || $data.text == '') }}{{if $data.postId == 'createNote'}}<%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("NotesV2.NewNoteHintText")) %>{{else}}<%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("NotesV2.DescriptionHintText")) %>{{/if}}{{else}}{{html text}}{{/if}}</textarea>
</div>
<div class="footer">
...
<img width="16" height="16" class="imageAction attachImage" src="/_imgs/ico_16_attach.gif"
...
<iframe id="${postId}_attachFileIframe" class='notesAttachmentIframe' frameBorder="0" style="display:none" />
{{/if}}
...
{{/if}}
{{if $data.postId != 'createNote' && $data.postId.substring(0,4) != 'temp' }}
href=${window.self._notesV2DownloadUrl + "&AttachmentId=" + postId +  window.self._notesV2DownloadToken} target="_blank" onclick="var domEvent=new Sys.UI.DomEvent(event); domEvent.stopPropagation();"
...
{{/if}}
>{{if ($data.fileName != null && $data.fileName != '') }}${CrmEncodeDecode.CrmHtmlEncode(fileName)}{{/if}}
...
title="<%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("NotesV2.Loading")) %>"
alt="<%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("NotesV2.Loading")) %>">
