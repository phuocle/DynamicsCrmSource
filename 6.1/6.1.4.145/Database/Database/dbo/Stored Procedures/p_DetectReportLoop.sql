...
Declare @TempParentId uniqueidentifier
   ...
   IF not exists(select ReportId from Report where ReportId = @ParentId)
     BEGIN 
   ...
   IF not exists(select ReportId from Report where ReportId = @ChildId)
     BEGIN 
...
WHILE ( 1 = 1 )
 ...
 BEGIN
   ...
   select @TempParentId = ParentReportId from Report  where ReportId = @TempParentId	
