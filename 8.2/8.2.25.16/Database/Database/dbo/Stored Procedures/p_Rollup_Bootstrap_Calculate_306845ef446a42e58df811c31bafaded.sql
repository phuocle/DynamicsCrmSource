CREATE PROCEDURE [dbo].[p_Rollup_Bootstrap_Calculate_306845ef446a42e58df811c31bafaded]
...
BEGIN
		 ...
		 SELECT TOP (1) @maxRecord=[Id] FROM [dbo].[Rollup_Bootstrap_Target_306845ef446a42e58df811c31bafaded] ORDER BY [Id] DESC
		 SET @maxRecord = ISNULL(@maxRecord, 0)
		 SELECT @start = [BootstrapTargetPointer] FROM [RollupPropertiesBase]		 WHERE [RollupPropertiesId] = '306845ef-446a-42e5-8df8-11c31bafaded'
		 ...
		 FROM [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] [RBT] WITH (TABLOCK) 
		 JOIN
			 (
				 SELECT [T].[ParentEntityId], SUM([T].[OutputValue]) AS [OutputValue] 
				 FROM [dbo].[Rollup_Bootstrap_Target_306845ef446a42e58df811c31bafaded] [T] WITH (TABLOCK) 
...
BEGIN
		 ...
		 FROM [RollupPropertiesBase] WHERE [RollupPropertiesId] = '306845ef-446a-42e5-8df8-11c31bafaded'
-- Get parent depth markers
		...
		WHERE [Id] = @currDepth
		 ...
		 BEGIN
				...
				UPDATE [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] WITH (TABLOCK) 
				SET [Status] =2, [OutputValue]=NULL
				WHERE [Id] BETWEEN @start AND @end 
				AND [OutputValue] IS NOT NULL
				AND ([OutputValue] NOT BETWEEN -922337203685477.5808 AND 922337203685477.5807				 OR [OutputValue] * ExchangeRate NOT BETWEEN -922337203685477.5808 AND 922337203685477.5807)
				 --  Mark for Currency Field Missing Error state on current set


				UPDATE [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] WITH (TABLOCK) 
				SET [Status] =7, [OutputValue]=NULL
				WHERE [Id] BETWEEN @start AND @end
			 ...
			 BEGIN
						...
						UPDATE [parent]
						 ...
						 FROM [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] [parent] WITH (TABLOCK) 
						 JOIN (

							SELECT [c].[ParentEntityId], SUM(ROUND([c].[OutputValue],4)) AS [OutputValue] 
							FROM [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] [c] WITH (TABLOCK) 
				 ...
				 -- Bubble up Errors for Overflow,Hierarchy TO other Error
						...
						UPDATE [parent]
						 ...
						 FROM [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] [parent] WITH (TABLOCK) 
						 WHERE EXISTS( SELECT 1 FROM  [dbo].[Rollup_Bootstrap_306845ef446a42e58df811c31bafaded] [c] WITH (TABLOCK) 
										WHERE [c].[Id] BETWEEN @start AND @end
			 ...
			 BEGIN -- Termination condition
				 ...
				 TRUNCATE TABLE [dbo].[Rollup_Bootstrap_Markers_306845ef446a42e58df811c31bafaded]
				 SET @rowCount = 0
			 END
