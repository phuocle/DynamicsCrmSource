...
-- Checking for Exchange Rate Is Null or Not
		 ...
		 BEGIN
				...
				SELECT @result=SUM(ROUND([IQ].[EstimatedValue_Base],4)), @maxLimitAggregate=MAX(__AggLimitExceededFlag__)
...
-- Leaf Value calculation
				IF (@state = 1) BEGIN 
						 SELECT @leafValue=SUM(ROUND([aggregateEntity].[EstimatedValue_Base],4))
						 FROM [Opportunity] [aggregateEntity] 
...
-- Sum of Hierarchy and Leaf Values
	...
	SET @result = ROUND(@result,4)
		IF (@state = 1) BEGIN 
			 SET @result_money= ROUND( @result * @exchRate , 4)
		END
	 END TRY
	 BEGIN CATCH
