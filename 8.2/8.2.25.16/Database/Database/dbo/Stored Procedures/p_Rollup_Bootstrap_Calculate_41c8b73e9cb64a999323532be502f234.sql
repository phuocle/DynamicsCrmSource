CREATE PROCEDURE [dbo].[p_Rollup_Bootstrap_Calculate_41c8b73e9cb64a999323532be502f234]
...
BEGIN
		 ...
		 SELECT TOP (1) @maxRecord=[Id] FROM [dbo].[Rollup_Bootstrap_Target_41c8b73e9cb64a999323532be502f234] ORDER BY [Id] DESC
		 SET @maxRecord = ISNULL(@maxRecord, 0)
		 SELECT @start = [BootstrapTargetPointer] FROM [RollupPropertiesBase]		 WHERE [RollupPropertiesId] = '41c8b73e-9cb6-4a99-9323-532be502f234'
		 SET @start= ISNULL(@start,0)
		 SET @end = @start + @batchSize
		 IF @end > @maxRecord  SET @end = @maxRecord

		 UPDATE [RBT] SET [RBT].[OutputValue] = ISNULL([RBT].[OutputValue],0) + ISNULL(result.[OutputValue],0), [RBT].[CountValue] = ISNULL([RBT].[CountValue],0) + ISNULL(result.[CountValue],0)

		 FROM [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] [RBT] WITH (TABLOCK) 
		 JOIN
			 (
				 SELECT [T].[ParentEntityId], SUM([T].[OutputValue]) AS [OutputValue] , COUNT([T].[Id]) AS [CountValue]
				 FROM [dbo].[Rollup_Bootstrap_Target_41c8b73e9cb64a999323532be502f234] [T] WITH (TABLOCK) 
		 ...
		 BEGIN
					...
					WHERE [RollupPropertiesId]='41c8b73e-9cb6-4a99-9323-532be502f234'
		 END
		 ELSE
		 BEGIN
...
BEGIN
		 ...
		 FROM [RollupPropertiesBase] WHERE [RollupPropertiesId] = '41c8b73e-9cb6-4a99-9323-532be502f234'
-- Get parent depth markers

					SELECT @startParentId = [Start], @endParentId =[End]
					FROM [dbo].[Rollup_Bootstrap_Markers_41c8b73e9cb64a999323532be502f234] 
		...
		WHERE [Id] = @currDepth
		 ...
		 BEGIN
				...
				UPDATE [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] WITH (TABLOCK) 
				SET [Status] =2, [OutputValue]=NULL
				WHERE [Id] BETWEEN @start AND @end 
				AND [OutputValue] IS NOT NULL
				AND ([OutputValue] NOT BETWEEN -100000000000 AND 100000000000)
...
[parent].[OutputValue] = CASE WHEN ([parent].[Status] IS NOT NULL AND [parent].[Status] != 6 ) THEN NULL ELSE ISNULL([parent].[OutputValue],0) + ISNULL(result.[OutputValue],0) END, 
					  [parent].[CountValue] = CASE WHEN ([parent].[Status] IS NOT NULL AND [parent].[Status] != 6 ) THEN NULL ELSE ISNULL([parent].[CountValue],0) + ISNULL(result.[CountValue],0) END
						 FROM [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] [parent] WITH (TABLOCK) 
						 JOIN (

							SELECT [c].[ParentEntityId], SUM(ROUND([c].[OutputValue],10)) AS [OutputValue] , SUM([c].[CountValue]) AS [CountValue]
							FROM [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] [c] WITH (TABLOCK) 
				 ...
				 -- Bubble up Errors for Overflow,Hierarchy TO other Error
						...
						UPDATE [parent]
						 SET [parent].[Status] = 3,
						 [parent].[OutputValue] = NULL , [parent].[CountValue] = NULL
						 FROM [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] [parent] WITH (TABLOCK) 
						 WHERE EXISTS( SELECT 1 FROM  [dbo].[Rollup_Bootstrap_41c8b73e9cb64a999323532be502f234] [c] WITH (TABLOCK) 
										WHERE [c].[Id] BETWEEN @start AND @end
			 ...
			 BEGIN -- Termination condition
				 ...
				 TRUNCATE TABLE [dbo].[Rollup_Bootstrap_Markers_41c8b73e9cb64a999323532be502f234]
				 SET @rowCount = 0
			 END
