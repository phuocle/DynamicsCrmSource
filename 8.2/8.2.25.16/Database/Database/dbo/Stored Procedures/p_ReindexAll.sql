...
(  
	...
	@LockTimeout			int = 180000 	--defaulting to 3 minutes to reduce the locking behaviour of the job
)  
as
/****************************************************************************************************************************************/  
/*Usage																																	*/
/*	1. ONLY REORGANIZE for frag 10% - 40%	:	exec p_reindexAll @FragRebuildPctLow=10, @MaxFragPctToRebuild=40,  @DefragType=1		*/
/*	2. ONLY REBUILD for frag 40% - 100%		:	exec p_reindexAll @FragRebuildPctLow=40, @MaxFragPctToRebuild=100, @DefragType=2		*/
...
begin
	...
	-- set LOCK_TIMEOUT to value in param @LockTimeout
	declare @sqlLockTimeout char(100)
	set @sqlLockTimeout = 'SET LOCK_TIMEOUT ' + convert (char,@LockTimeout);
	exec (@sqlLockTimeout) ;
	...
	if (
		...
		(@Maxdop not in (0,1)) OR
		(@SortInTempDB not in (0,1)) OR
		(@ScanMode not in (0,1)) OR
		(@StatisticsNoRecompute not in (0,1)) OR
		(@LockTimeout <= 0)	
		)
	begin  
	...
	begin
		...
		print '  @LockTimeout: ' + CAST(@LockTimeout as varchar)
		print '  '
	end
	...
	begin
		...
		select @sqlReorg = N'ALTER INDEX [' + @IndexName + N'] ON [' + @SchemaName + N'].[' + @TableName + N'] REORGANIZE WITH ( LOB_COMPACTION = ON ); '	-- default is ON
		...
		begin
			select @reorganize = 0
			select @sqlReBuildTemplate = N'ALTER INDEX ['+ @IndexName + N'] ON [' + @SchemaName + N'].[' + @TableName + N'] REBUILD WITH '+
					N'( PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = '+ @STATISTICS_NORECOMPUTE +', ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, SORT_IN_TEMPDB = '+ @SORT_IN_TEMPDB +', MAXDOP = '+ CAST(@Maxdop as nvarchar(10)) +', ONLINE = '	  
