...
--Owner Bucket
	...
	select distinct BaseTableName 
		from EntityView e 
		where e.OwnershipTypeMask = 1 AND e.EntityId IN (select EntityId from AttributeView a  where a.EntityId = e.EntityId AND a.Name = 'OwnerId' and a.IsLogical = 0);
...
--OwningBusinessUnit 
	...
	for
		select distinct BaseTableName 
			from EntityView e 
			where e.OwnershipTypeMask = 1 AND e.EntityId IN (select EntityId from AttributeView a  where a.EntityId = e.EntityId AND a.Name = 'OwningBusinessUnit' and a.IsLogical = 0);
