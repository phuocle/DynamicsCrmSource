CREATE TABLE [dbo].[ActivityPointerBase] (
    ...
    CONSTRAINT [email_email_parentactivityid] FOREIGN KEY ([ParentActivityId]) REFERENCES [dbo].[ActivityPointerBase] ([ActivityId]),
...
CREATE UNIQUE NONCLUSTERED INDEX [fndx_Email_MessageId]
    ON [dbo].[ActivityPointerBase]([MessageId] ASC, [EmailDirectionCode] ASC, [MessageIdDupCheck] ASC) WHERE ([ActivityTypeCode]=(4202) AND [MessageId] IS NOT NULL AND [EmailDirectionCode] IS NOT NULL) WITH (FILLFACTOR = 80);
...
CREATE NONCLUSTERED INDEX [fndx_Email_EmailConnectorDequeue]
    ...
    INCLUDE([DeliveryPriorityCode], [ActualEnd], [SenderMailboxId]) WHERE ([StateCode]=(1) AND [ActivityTypeCode]=(4202) AND [PostponeActivityProcessingUntil] IS NOT NULL) WITH (FILLFACTOR = 80);
...
CREATE NONCLUSTERED INDEX [ndx_QF_BulkOperationNumber]
    ON [dbo].[ActivityPointerBase]([BulkOperationNumber] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [ndx_SendersAccount]
    ON [dbo].[ActivityPointerBase]([SendersAccount] ASC) WHERE ([ActivityTypeCode]=(4202)) WITH (FILLFACTOR = 80);
