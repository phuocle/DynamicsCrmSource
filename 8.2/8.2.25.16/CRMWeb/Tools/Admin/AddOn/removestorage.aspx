...
XUI.Html.SetText($get('LicenseToRemoveTableHeader'),  <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.RemoveStorageLicenseInputPage.LicenseToRemoveHeaderAfterChange"))%>);
$get('UserInputTable').setAttribute('summary', <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.RemoveStorageLicenseInputPage.LicenseToRemoveHeaderAfterChange"))%>);
...
setStorageLicenseUsageInfoText(STORAGE_LICENSES_TO_REMOVE_USAGE_INFO_CONFIRM_PAGE, (CURRENT_MONTHLY_CHARGES_STORAGE_QUANTITY * 1 - storage * 1 + STORAGE_LICENSES_INCLUDED * 1).toFixed(NUMBER_OF_DIGITS_AFTER_DECIMAL_POINT_FOR_STORAGE), (STORAGE_LICENSES_AVAILABLE * 1 - storage * 1).toFixed(NUMBER_OF_DIGITS_AFTER_DECIMAL_POINT_FOR_STORAGE));
}
else if(PageId == "ConfirmPage")
{

WizardSetProperty("ProcessingUrl", "/tools/admin/addon/removestoragecomplete.aspx");
var postData = CrmEncodeDecode.CrmXmlEncode($get('NumberOfStorageLicensesToRemove').value) + ";" + CURRENT_MONTHLY_CHARGES_USER_LICENSES_QUANTITY + ";" + CURRENT_MONTHLY_CHARGES_STORAGE_QUANTITY;
...
XUI.Html.SetText($get('LicenseToRemoveTableHeader'), <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.RemoveStorageLicenseInputPage.LicenseToRemoveHeader"))%>);
$get('UserInputTable').setAttribute('summary', <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.RemoveStorageLicenseInputPage.LicenseToRemoveHeader"))%>);
...
setStorageLicenseUsageInfoText(STORAGE_LICENSES_TO_REMOVE_USAGE_INFO_PURCHASE_PAGE,(CURRENT_MONTHLY_CHARGES_STORAGE_QUANTITY * 1 + STORAGE_LICENSES_INCLUDED * 1).toFixed(NUMBER_OF_DIGITS_AFTER_DECIMAL_POINT_FOR_STORAGE),STORAGE_LICENSES_AVAILABLE);
XUI.Html.SetText($get('StorageLicensesToRemoveUsageInfo'), XUI.Html.GetText($get('StorageLicensesToRemoveUsageInfo')) + " " + STORAGE_REMOVE_INFO_TEXT);
}
...
{
XUI.Html.SetText($get('InvalidLicenseCountErrorMessage'), <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.LicenseInputPage.OutOfRangeRemoveStorageCountErrorMessage"))%>);
}
else
{
XUI.Html.SetText($get('InvalidLicenseCountErrorMessage'), <%=Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.LicenseInputPage.InvalidRemoveStorageCountErrorMessage"))%>);
}
}
else
{
...
var updatedTotalMonthlyChargesPrice = (CURRENT_MONTHLY_CHARGES_TOTAL_PRICE * 1 - (CURRENT_MONTHLY_CHARGES_STORAGE_UNIT_PRICE * storage));
...
FormatToCurrencyAsync('StorageLicensesToRemovePrice', storageLicensesToRemovePrice.toFixed(NUMBER_OF_DIGITS_AFTER_DECIMAL_POINT));
...
setStorageLicenseUsageInfoText(STORAGE_LICENSES_TO_REMOVE_USAGE_INFO_PURCHASE_PAGE,CURRENT_MONTHLY_CHARGES_STORAGE_QUANTITY * 1 + STORAGE_LICENSES_INCLUDED * 1, STORAGE_LICENSES_AVAILABLE);
XUI.Html.SetText($get('StorageLicensesToRemoveUsageInfo'), XUI.Html.GetText($get('StorageLicensesToRemoveUsageInfo')) + " " + STORAGE_REMOVE_INFO_TEXT);
...
FormatToCurrencyAsync('StorageLicensesToRemoveUnitPriceBeforePromotion', CURRENT_MONTHLY_CHARGES_STORAGE_UNIT_PRICE);
