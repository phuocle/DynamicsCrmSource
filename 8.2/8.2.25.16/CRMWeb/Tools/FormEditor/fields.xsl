<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="exslt msxsl" version="2.0">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="$sortOrder='ascending'">
							<xsl:for-each select="field">
								<xsl:sort order="ascending" select="@displayname[$sortColumnName='displayname'] |
								@name[$sortColumnName='name'] |
								@datatype[$sortColumnName='datatype']" lang="$languageName"/>
								<xsl:call-template name="renderRow"/>
							</xsl:for-each>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template name="renderRow">
		<xsl:if test="not(starts-with(@datatype, 'bad-news-here'))">
			<tr height="20" style="cursor: pointer;" onmouseover="on(new Sys.UI.DomEvent(event));" onmouseout="off(new Sys.UI.DomEvent(event));" onclick="chkSwitch(this.cells[0].children[0]);">
