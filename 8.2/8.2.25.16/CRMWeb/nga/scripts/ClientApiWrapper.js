...
window.onerror = function (message, url, line, coloumnNo, errorObj) {
	Xrm.XrmInternalWrapper.displayError(message, null, url, null, errorObj.stack);
};
...
Mscrm.handleErrorDisplay = function (message, eventDetails, fileName, functionName, stack) {
	Xrm.XrmInternalWrapper.displayError(message, eventDetails, fileName, functionName, stack);
}

Mscrm.RunHandlerWithHandlerName = function (method, eventDetails, parameterArray, methodObj, srcName) {
	try {
		...
		// Adding additional information with custom script fie name in which error occured.
		var fileName = Mscrm.InternalUtilities._Script.isNullOrUndefined(methodObj.libraryName) ? srcName : methodObj.libraryName;
		Xrm.XrmInternalWrapper.displayError(e.message, eventDetails, fileName, methodObj.functionName, e.stack);
	}
};

Mscrm.RunHandlerWithHandlerObject = function (method, eventDetails, parameter, methodObj, executionContext, srcName) {
	try {
