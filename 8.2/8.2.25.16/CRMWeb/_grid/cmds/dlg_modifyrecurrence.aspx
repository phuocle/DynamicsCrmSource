...
if (!(numDaysValue > 0 && numDaysValue <= 365)) {
numberOfDaysInputControl.value = parseInt(numberOfDaysInputControl.getAttribute("defaultValue"), 10);
}
else {
...
if (!(numHoursValue > 0 && numHoursValue <= 24)) {
numberOfHoursInputControl.value = parseInt(numberOfHoursInputControl.getAttribute("defaultValue"), 10);
}
else {
...
<% if (IsRollupJob) { %>
if (onChangeNumberOfHours() == false) { return; }
else if(onChangeNumberOfDaysForRollup() == false) { return; }
<% } else { %>
...
<tr>
<td colspan="2"></td>
<td><input type="radio" class="radio" id="rdHourly" name="rdRecurrenceOptions" value="3" onclick="OnCheckedChanged();" <% if (OptionSelected == RecurrenceFrequency.Hourly.ToString()) { %>checked<% } %>></td>
<td><label class="radioLabel" for="rdHourly"><loc:Text ResourceId="Freq_Hourly" runat="server"/></label></td>
</tr>
<tr>
<td colspan="2"></td>
<td><input type="radio" class="radio" id="rdDaily" name="rdRecurrenceOptions" value="4" onclick="OnCheckedChanged();" <% if (OptionSelected == RecurrenceFrequency.Daily.ToString()) { %>checked<% } %>></td>
<td><label class="radioLabel" for="rdDaily"><loc:Text ResourceId="Freq_Daily" runat="server"/></label></td>
</tr>
<% } %>
<tr id="recurrencecontrol" class= " <% if (OptionSelected == RecurrenceFrequency.Daily.ToString()) { %> daily <% } else { %> hourly <% } %>" >
<td colspan="3"></td>
<td><label class="radioLabel" for="rdSchedule"><loc:Text ResourceId="BulkDeleteSettings.aspx_ReRunOptionLabel" runat="server"/></td>
<%  if (IsRollupJob) { %>
<td class="hourly"><ui:EditableSelect id="numberOfHours" onchange="onChangeNumberOfHours();" onfocusout="onChangeNumberOfHours();" runat="server"/>
</td>
<td class="hourly"><label class="radioLabel" for="rdHourly"><loc:Text ResourceId="BulkDeleteSettings.aspx_ReRunOptionHoursLabel" runat="server"/>
</td>
<td class="daily"><ui:EditableSelect id="numberOfDaysForRollup" onchange="onChangeNumberOfDaysForRollup();" onfocusout="onChangeNumberOfDaysForRollup();" runat="server"/>
</td>
<td class="daily"><label class="radioLabel" for="rdDaily"><loc:Text ResourceId="BulkDeleteSettings.aspx_ReRunOptionDaysLabel" runat="server"/>
</td>
