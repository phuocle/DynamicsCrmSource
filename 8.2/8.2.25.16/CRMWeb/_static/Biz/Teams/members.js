...
function ModifyMembership(oInputGrid,refreshGridId,aTargetIds,action,oTypeToAdd,additionalParams,dataProvider)
{
    ...
    if(!Xrm.Internal.isTurboForm())
    {
        ...
        var callbackRef = Mscrm.Utilities.createCallbackFunctionObject("modifyMemberShipAction",this,parameters,false);
        if(typeof dataProvider != "undefined" && !isNullOrEmptyString(dataProvider))
            LookupObjectsWithCallback(callbackRef,null,lookupSelectionStatus,oTypeToAdd == AppSalesPerson ? SystemUser : oTypeToAdd,0,null,additionalParams,0,0,1,"","",dataProvider);
        else
            LookupObjectsWithCallback(callbackRef,null,lookupSelectionStatus,oTypeToAdd == AppSalesPerson ? SystemUser : oTypeToAdd,0,null,additionalParams)
    }
    else
    {
        var lookupOptions = new Xrm.LookupOptions;
        lookupOptions.lookupTypes = [Xrm.Internal.getEntityName(oTypeToAdd == AppSalesPersonTypeCode ? SystemUser : oTypeToAdd)];
        ...
        if(!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(additionalParams) && additionalParams.indexOf("filterdefault=1"))
        {
        ...
        Xrm.Internal.lookupObjects(lookupOptions,function(lookupItems)
        {
...
function modifyMemberShipAction(retval,targetids,oTypeToAdd,action,refreshGridId)
{
    ...
    else
    {
        ...
        !Mscrm.InternalUtilities.JSTypes.isNull(gridControl) && handleXMLErr(oXml) && 
            gridControl.Refresh()
    }
}
