  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	    ...
	    init : function(options) {
		    ...
		    var containerDiv = $('<div id="' + zoomContainerId + '" class="' + "canvas zoomCanvasWrapper" + '"></div>');
		    ...
		    targetElement.data('smartZoomData', {
               ...
               originalSize: {width:targetElement.get(0).scrollWidth, height:targetElement.get(0).scrollHeight}, // plugin target size at the beginning
    ...
    function animate(target, globalLeft, globalTop, width, height, duration, callback){
		...
		if (GenericWorkflowDesigner.Settings.renderRTL)
			...
			cssObject[smartData.transitionObject.transform + '-origin'] = '0 0';
		  	...
		  	cssObject[smartData.transitionObject.transition] = 'all '+0 / 1000+'s ease-out'; // set effect duration
    ...
    function getBrowserTransitionObject(){
	   ...
	   for(var i=0; i<length; i++){ // for all kind of css transition we make a test
	   		if(bodyStyle[transitionTestArr[i]] != null){
			    ...
			    css3dSupported = (Math.round(div.offset().left - $('body').offset().left) === 20); // if translate3d(20px,0,0) via transformArr[i] == 20px the transformation is valid for this browser
			    div.empty().remove();
	 ...
	 */	 
   	function getRect(jqObject) {
	    ...
	    return {'x':formX, 'y':formY, 'width':jqObject.get(0).scrollWidth, 'height':jqObject.get(0).scrollHeight};
	}
