...
function loadScriptAdv(sScriptSrc,sId,preloaderOutlookCookies,onLoad)
{
    ...
    if(!_isFirefox || getFirefoxVersion() < 28 || sScriptSrc.indexOf("jquery-2.1.1.min.js") == -1 && sScriptSrc.indexOf("jquery.tmpl.min.js") == -1)
        oScript.async = true;
...
function waitForContentToStartLoading()
{
    ...
    (!contentIframeElement || contentIframeElement.contentWindow.document.readyState != "complete" && contentIframeElement.contentWindow.document.readyState != "loaded" || !contentIframeElement.contentWindow.location.pathname.endsWith("edit.aspx") || !contentIframeElement.contentWindow.actionQueueInitializationComplete) && 
        getInitActionQueue().unshift(arguments.callee)
}
