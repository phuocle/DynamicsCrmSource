...
function DisableDescriptionAndOtherwiseText()
{
    ...
    if(tdElements != null && tdElements.length > 0)
        for(var tdElement = tdElements[0],
            i = 0; i < tdElement.childNodes.length; i++)
        {
    ...
    var descElements = Mscrm.Utilities.getChildElementsByClassName(container,"ms-crm-Input-Container wfStepDescription",false);
...
function LoadWorkflowEventNotification()
{
    ...
    !IsNull(errMessage) && errMessage.length != 0 && 
        ...
        if(pos == controlId.length - expectedSuffix.length)
        {
            ...
            if(!IsNull(control))
                if(controlId.indexOf("Assign") == 0)
                {
                    ...
                    if(controlId.indexOf("ChildWorkflow") == 0)
                    {
...
function PopulateAssignStepSlugs()
{
        ...
        i = 0; i < len; i++)
    {
        var controlId = lookups[i].id,
            pos = controlId.lastIndexOf(expectedSuffix);
        pos == controlId.length - expectedSuffix.length && controlId.substr(0,expectedPrefix.length) == expectedPrefix && 
            AddAttributeTypeMapping(controlId,"Owner")
    }
    PopulateSlugControlsWithCallback(CrmEncodeDecode.CrmXmlDecode($get2("hidSlugInfo").value),"deleteslugbody",OnAssigneeSlugDelete)
}
function OnAssigneeSlugDelete(sender,args)
{
    var el = getLookUpElementWithEvent(sender);
...
function showWorkStepMenu()
{
    ...
    var isFeatureEnabled = Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.AnyChanelToAnyEntityRecordCreation);
    if(isFeatureEnabled)
    {
        var container = $Id("gridDefinitionContainer");
        if(container == "gridDefinitionContainer_CtrlPrimaryActionStep" && rendererTypeCode == Mscrm.EntityTypeCode.ConvertRuleItem.toString())
        {
            var stepContainer = $get($get2("hidBaseStepId").value + "DIV");
            if(!IsNull(stepContainer) && !IsNull(stepContainer.children))
                if(stepContainer.children.length == 1 && (stepContainer.children[0].getAttribute("id") == "NoStep" || stepContainer.children[0].getAttribute("id").startsWith("UpdateStep")))
                    isCreateRecordMenuDisabled = false;
                else
    ...
    IsStepPresent($Id("mnu_AddStep_CreateStep")) && 
        addStepMenuCtrl.setItemProperty($Id("mnu_AddStep_CreateStep"),"disabled",isCreateRecordMenuDisabled.toString());
    IsStepPresent($Id("mnu_AddStep_UpdateStep")) && 
        addStepMenuCtrl.setItemProperty($Id("mnu_AddStep_UpdateStep"),"disabled",isUpdateRecordMenuDisabled.toString());
    IsStepPresent($Id("mnu_AddStep_AssignStep")) && 
        addStepMenuCtrl.setItemProperty($Id("mnu_AddStep_AssignStep"),"disabled",isAssignEntityMenuDisabled.toString());
    IsStepPresent($Id("mnu_AddStep_ChangeStatusStep")) && 
        addStepMenuCtrl.setItemProperty($Id("mnu_AddStep_ChangeStatusStep"),"disabled",isSetStateMenuDisabled.toString());
    IsStepPresent($Id("mnu_AddStep_SendEmailStep")) && 
        addStepMenuCtrl.setItemProperty($Id("mnu_AddStep_SendEmailStep"),"disabled",isSendEmailMenuDisabled.toString());
