...
function GetValidForConvertToOpportunityWithLead(sUrlElement,iObjType)
{
    ...
    if(!IsNull(regardingCtrl))
    {
        ...
        if(!IsNull(regardingItems) && regardingItems.length == 1)
            if(regardingItems[0].type.toString() == Mscrm.InternalUtilities.EntityTypeCode.CampaignActivity.toString() || regardingItems[0].type.toString() == Mscrm.InternalUtilities.EntityTypeCode.BulkOperation.toString() || regardingItems[0].type.toString() == Mscrm.InternalUtilities.EntityTypeCode.Campaign.toString())
