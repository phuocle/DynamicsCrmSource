        ...
        Xrm.Internal.messages.followEmailAttachment(activityMimeAttachmentId).then(function(response)
        {
            if(response != null && response.response != null)
            {
                ...
                if(followStopWatch != null)
                {
                    ...
                    console.log("Attachment upload completed in " + followStopWatch.get_elapsedMilliseconds() + " milliseconds")
                }
            ...
            else
            {
                ...
                {
                    followStopWatch.stop();
                    console.log("Attachment upload failed in " + followStopWatch.get_elapsedMilliseconds() + " milliseconds")
                }
            }
        ...
        Xrm.Internal.messages.unfollowEmailAttachment(activityMimeAttachmentId).then(function(response)
        {
            if(response != null)
            {
                ...
                if(unfollowStopWatch != null)
                {
                    ...
                    console.log("Unfollow attachment completed in " + unfollowStopWatch.get_elapsedMilliseconds() + " milliseconds")
                }
                window.location = $get("Attachments").SuccessURL.value
            }
            else
            {
                ...
                {
                    unfollowStopWatch.stop();
                    console.log("Unfollow attachment failed in " + unfollowStopWatch.get_elapsedMilliseconds() + " milliseconds")
                }
            }
        },function(errorResponse)
        {
...
function attachForLegacy(sUrl,sNode,notesv2)
{
    ...
    if(spUpload != "true" && sFileName.length > 185)
        ...
        if(spUpload == "true" && sFileName.length > 165)
    ...
    if(notesv2)
        ...
        if(attachements.AttachmentId.value == "")
        {
            var subject = $get("subject");
            !IsNull(subject) && subject.value === "" && 
    ...
    if(bIsValid)
    {
        ...
        try
        {
            ...
            {
                var parentPage = window.opener;
                !IsNull(parentPage) && (typeof parentPage.OnAttach == "function" || typeof parentPage.OnAttach == "object") && 
                    parentPage.OnAttach()
            }
        }
        catch(e)
...
function downloadAttachment(eventObject)
{
        ...
        if(parentHeadNode != null)
        {
            ...
            if(metaElements != null)
                for(var i = 0; i < metaElements.length; i++)
                    if(metaElements[i].getAttribute("name") != "undefined" && metaElements[i].getAttribute("name") == "DownloadOptions")
                    {
                        if(metaElements[i].getAttribute("content") != "undefined" && metaElements[i].getAttribute("content") == "noopen")
                            bFoundDownLoadOptionsWithNoOpen = true;
                        (hideOpenButton == "undefined" || hideOpenButton == "0" || metaElements[i].getAttribute("content") != "undefined" && metaElements[i].getAttribute("content") != "noopen") && 
    ...
    var downloadUrl = element.getAttribute("url") + "?AttachmentType=" + element.getAttribute("attachmentType") + "&AttachmentId=" + element.getAttribute("attachmentId") + "&IsNotesTabAttachment=" + element.getAttribute("IsNotesTabAttachment") + element.getAttribute("WRPCTokenUrl");
