...
function ResolveUnkownParty(domEventObject,sPartyName,sFieldName)
{
    ...
    var oUrl = Mscrm.CrmUri.create("/_controls/partylist/resolve.aspx?address=" + CrmEncodeDecode.CrmUrlEncode(sPartyName)),
        parameters = [lookupItem,sPartyName,sFieldName],
        callbackFunctionObject = Mscrm.Utilities.createCallbackFunctionObject("resolvePartyFromLookUp",this,parameters),
...
function resolvePartyFromLookUp(returnObject,lookupItem,sPartyName,sFieldName)
{
    if(returnObject)
    {
        ...
        if(inlineLookupItem)
        {
            partyListControlId = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetNextSibling(XUI.Html.DomUtils.GetNextSibling(lookupItem.parentNode.parentNode.parentNode.parentNode))).id;
            XUI.Html.DomUtils.GetFirstChild(lookupItem).innerHTML = '<SPAN class=ms-crm-LookupItem-Name wrapper="true">' + CrmEncodeDecode.CrmHtmlEncode(returnObject.name) + "</SPAN>";
            ...
            {
                partyListControlId = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetNextSibling(lookupItem.parentNode.parentNode.parentNode.parentNode)).id;
                XUI.Html.DomUtils.GetFirstChild(lookupItem).innerHTML = "<img alt='' class='ms-crm-Lookup-Item' src='/_imgs/ico_16_" + returnObject.type + '.gif\'><SPAN class=ms-crm-LookupItem-Name wrapper="true">' + CrmEncodeDecode.CrmHtmlEncode(returnObject.name) + "</SPAN>";
        ...
        if(!lookupDisabled)
        {
            ...
            if(inlineLookupItem)
            {
                ...
                partyListControl.normalizeLookupItem(spanElement,Mscrm.FormInputControl.LookupUIBehavior.unresolvedLookupItemClass);
...
function ResolvePartyFromDisabledLookup(returnObject,sPartyName,sFieldName)
{
    for(var oAttribute = Xrm.Page.data.entity.attributes.get(sFieldName),
        ...
        oNewLookupItems = [],
        i = 0; i < iCount; i++)
        if(parseInt(oOldLookupItems[i].type,10) == UnresolvedEmailParty && Mscrm.LookupAttribute.undefinedAndEmptyToNull(oOldLookupItems[i].name) == Mscrm.LookupAttribute.undefinedAndEmptyToNull(sPartyName))
        {
...
function OpenItemForceSubmit(eventObject)
{
    ...
    var lookupItem = eventObject.target,
        lookupButtonTD = XUI.Html.DomUtils.GetNextSibling(lookupItem.parentNode.parentNode.parentNode.parentNode.parentNode);
    if(!IsNull(lookupButtonTD))
    {
        var formControlInputBehavior = Mscrm.FormControlInputBehavior.GetBehavior(XUI.Html.DomUtils.GetFirstChild(lookupButtonTD).id);
