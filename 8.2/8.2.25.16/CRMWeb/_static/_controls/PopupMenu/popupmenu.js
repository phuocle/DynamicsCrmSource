...
function PopupMenu()
{
    ...
    function _addMenuGeneric(sName,fnCallback,sTdClass,sIconUrl,bShowLink,bannerColumn)
    {
        ...
        {
            if(!IsNull(sIconUrl))
                _sMenuItemsHtml += '<img alt="" class="popupMenuIcon" src="' + CrmEncodeDecode.CrmHtmlAttributeEncode(sIconUrl) + '"/>';
            else
        ...
        if(bShowLink)
            _sMenuItemsHtml += "<a tabindex=-1 class='atLink' id='atmnu" + _iMenuID + "' onclick='return false;' href='javascript:onclick();' target='_self' title=\"" + CrmEncodeDecode.CrmHtmlAttributeEncode(sName) + '"></a>';
    ...
    {
        _sMenuItemsHtml += '<tr height=1></td><td name="menuItem" menuID="' + _iMenuID + '" id="mnu' + _iMenuID + '" class="seperator" nowrap></td></tr>';
    ...
    function _show(iX,iY,iWidth,iHeight,oElement,bIsAT)
    {
        ...
        _maxRows = _iMenuID;
        !IsNull(_oPopup) && _oPopup.isOpen && (iX != 0 || iY != 0) && 
            popupInit()
    }
    function _hide()
    {
