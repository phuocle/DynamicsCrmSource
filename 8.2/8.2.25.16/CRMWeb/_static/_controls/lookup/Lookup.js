...
function LookupControlItem(sId,iType,sName,sOnclick,sDisplayClass,sData,sTypeName,iCategory,sAmbiguousRecordsXml,bSelected)
{
    this.id = sId;
    this.type = iType;
    this.name = sName;
...
function LookupObjects(lookupField,lookupStyle,lookupTypes,lookupBrowse,bindingColumns,additionalParams,showNew,showProp,bPopulateLookup,defaultType,searchString,dataProviderOverride,defaultViewId,customViews,filterRelationshipId,rId,rType,rDependAttr,allowFilterOff,disableQuickFind,disableViewPicker,viewsIds,crmAttributeId,callbackReference,additionalFetchFilter,additionalFetchFilterTypes)
{
    ...
    var isRegardingFieldPopulated = false;
    if(lookupField != null)
    {
        args.items = [];
        for(var a = lookupField.getElementsByTagName("SPAN"),
            ...
            {
                var item = a[i];
                if(!IsNull(item.getAttribute("otype")) && item.getAttribute("otype") !== "0" || !IsNull(item.getAttribute("otypename")) && item.getAttribute("otypename") !== "" || !IsNull(item.getAttribute("activitypartyid")))
                {
                    !IsNull(item.getAttribute(Mscrm.BusinessProcessConstants.ProcessId)) && 
                        item.setAttribute(Mscrm.BusinessProcessConstants.ProcessId,item.getAttribute(Mscrm.BusinessProcessConstants.ProcessId));
                    !IsNull(item.getAttribute(Mscrm.BusinessProcessConstants.ProcessTimestamp)) && 
                        item.setAttribute(Mscrm.BusinessProcessConstants.ProcessTimestamp,item.getAttribute(Mscrm.BusinessProcessConstants.ProcessTimestamp));
                    args.items.push(item);
                    if(item.getAttribute("otype") == defaultType)
                        isRegardingFieldPopulated = true;
                    if(!IsNull(item.getAttribute("otype")))
    ...
    var bIsSubjectLookup = lookupStyle.toUpperCase() === "SUBJECT",
        oUrl = bIsSubjectLookup ? Mscrm.CrmUri.create("/_controls/lookup/lookupsubject.aspx") : Mscrm.CrmUri.create("/_controls/lookup/lookupinfo.aspx");
    ...
    oUrl.get_query()["isRegardingFieldPopulated"] = isRegardingFieldPopulated;
    setMobilePopupMode();
    ...
    var parameters = [lookupField,bPopulateLookup,callbackReference],
        callbackFunctionObject = Mscrm.Utilities.createCallbackFunctionObject("buildAndReturnLookupItems",this,parameters,false),
...
{
    !Mscrm.InternalUtilities.Utilities.isTurboForm() && lookupItems != null && lookupField != null && bPopulateLookup && 
...
function openlui(domEventObject,o,isGridClick)
{
    ...
    if(o.getAttribute("otype") == UnresolvedAddress)
        ...
        if(o.getAttribute("category") == LookupItemCategories.AMBIGUOUS)
        {
            ...
            listItem.innerHTML = String.format('<div class="Notifications" style="text-align:center;height:18px;padding-top:2px">{0}</div>',LOCID_LOOKUPAMBIGUOUSTITLE);
            ...
            for(var oLookupCtrl = getLookupControlFromItemSpan(o),
                oRecords = XUI.Xml.SelectNodes(oXmlDoc,"/records/record",null),
                i = 0; i < oRecords.length; i++)
            {
                var item = Mscrm.MenuItem.createMenuItem(oRecords[i].getAttribute("oname"));
                item.set_iconPath(oLookupCtrl.GetLookupTypeIcon(parseInt(oRecords[i].getAttribute("otype"),10)).get_source());
                ...
                var sCallBack = Function.createDelegate(this,function(menuItem)
                    {
            ...
            if(!IsNull(XUI.DomUtilities.GetFirstChild(XUI.DomUtilities.GetChildElementAt(anchorItem.get_itemContents(),1))))
            {
                var rootSpan = XUI.DomUtilities.GetFirstChild(XUI.DomUtilities.GetChildElementAt(anchorItem.get_itemContents(),1));
            ...
            if(o.getAttribute("category") == LookupItemCategories.UNKNOWN)
            {
                var oLookupCtrl = getLookupControlFromItemSpan(o);
                oLookupCtrl.Lookup(true,false,XUI.Html.GetText(o))
            }
            else
                if(o.getAttribute("category") != LookupItemCategories.UNKNOWN_TYPE)
                {
                    if(window.UseTabletExperience && !IsNull(window.parent) && !IsNull(window.parent.$get("crmFormSubmitId")) && window.parent.$get("crmFormSubmitId").value == o.getAttribute("oid"))
                    {
                        o.parentNode.parentNode.style.textDecoration = "none";
                        return
                    }
                    if(Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(o.getAttribute("otype")) && Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(o.getAttribute("oid")))
                        return;
                    else
                        if(!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(o.getAttribute(Mscrm.BusinessProcessConstants.ProcessTimestamp)) && !Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(o.getAttribute(Mscrm.BusinessProcessConstants.ProcessId)))
                        {
                            var entityParams = "?";
                            entityParams += Mscrm.BusinessProcessConstants.Process + "=" + CrmEncodeDecode.CrmUrlEncode(o.getAttribute(Mscrm.BusinessProcessConstants.ProcessId));
                            entityParams += "&" + Mscrm.BusinessProcessConstants.ProcessTimestamp + "=" + CrmEncodeDecode.CrmUrlEncode(o.getAttribute(Mscrm.BusinessProcessConstants.ProcessTimestamp));
                            entityParams += "&" + Mscrm.BusinessProcessConstants.Opened + "=" + CrmEncodeDecode.CrmUrlEncode(Mscrm.BusinessProcessConstants.Lookup);
...
function resolveAmbiguousItem(oItem,oChangeToRecord)
{
    ...
    function _execute()
    {
        ...
        for(var aoValues = [],
            ...
            oLookupItem = new LookupControlItem(oChangeToRecord.getAttribute("oid"),oChangeToRecord.getAttribute("otype"),oChangeToRecord.getAttribute("oname"),null,"ms-crm-Lookup-Item",null,null,false),
            i = 0,
            len = oChangeToRecord.childNodes.length; i < len; i++)
        {
...
function subjectJstree(treeDiv,jsonTree_Data,control_id)
{
    ...
    treeDiv.jstree({json_data:jsonTree_Data,themes:{dots:false,icons:false,url:urltheme},core:{animation:200,rtl:is_rtl},plugins:["themes","json_data","ui","wholerow","hotkeys"],hotkeys:{"return":function()
    {
        ...
        if(this.data.ui.hovered)
        {
            ...
            else
            {
                ...
                if(!IsNull(Mscrm.Utilities) && Mscrm.Utilities.isTurboForm() && !IsNull(Mscrm.ServiceRefreshUtilities))
                    isDataUnboundControl = Mscrm.ServiceRefreshUtilities.triggerSearchOnSubjectLookupChange(lookupReference[0],e.target.id);
    ...
    treeDiv.bind("select_node.jstree",function(e,data)
    {
        if(!IsNull(treeBehavior))
        {
            treeBehavior.selectInlineSubjectItem(data.inst.get_json()[0].data,data.inst.get_json()[0].metadata.subjectid);
            treeBehavior.closeTreeDialog()
        }
        else
        {
            ...
            var isDataUnboundControl = false;
            if(!IsNull(Mscrm.Utilities) && Mscrm.Utilities.isTurboForm() && !IsNull(Mscrm.ServiceRefreshUtilities))
                isDataUnboundControl = Mscrm.ServiceRefreshUtilities.triggerSearchOnSubjectLookupChange(lookupReference[0],e.target.parentNode.id);
            if(!isDataUnboundControl)
            {
                var subjectAttribute = Xrm.Page.data.entity.attributes.getByName(control_id);
                subjectAttribute.setValue(lookupReference);
    ...
    treeDiv.bind("loaded.jstree open_node.jstree close_node.jstree",function()
    {
...
{
    entityRecord != null && 
        updateParentLookupExtension(entityRecord.Id,entityRecord.TypeCode,entityRecord.Name,entityRecord.TypeName,lookupParentControlId,true)
}
function updateParentLookupExtension(entityRecordId,entityRecordTypeCode,entityRecordName,entityRecordTypeName,lookupParentControlId,needAssociate)
{
    var oLookupItem = new LookupControlItem(entityRecordId,entityRecordTypeCode,entityRecordName,"openlui(new Sys.UI.DomEvent(event)); return false;","ms-crm-Lookup-Item","",entityRecordTypeName,false);
    ...
    if(!lookupParentControlId.endsWith("_i"))
    {
        ...
        existingItems = lookupControl.getAttribute().getValue();
        if(lookupControl.getAttribute().getLookupDataAttribute().getLookupStyle() == "multi" && existingItems != null && existingItems.length > 0)
            newItems = addNewItemsToLookup(existingItems,oLookupItem);
        else
            newItems = new Array(oLookupItem);
        lookupControl.getAttribute().getLookupDataAttribute().setValue(newItems,lookupParentControlId)
    }
    else
    {
        ...
        if(oLookupCtrl != null)
        {
            existingItems = oLookupCtrl.get_dataValue();
            if(oLookupCtrl.get_isInlineMultiLookup() && existingItems != null && existingItems.length > 0)
                newItems = addNewItemsToLookup(existingItems,oLookupItem);
            else
                newItems = new Array(oLookupItem);
            oLookupCtrl.set_dataValue(newItems);
            oLookupCtrl.resolveLookupMruItem(oLookupItem,needAssociate)
        }
    }
}
function addNewItemsToLookup(existingItems,oLookupItem)
{
    for(var newItems,
        indexOfExistingItem = -1,
        i = 0; i < existingItems.length; i++)
    {
        var existingItem = existingItems[i];
        if(existingItem.id == oLookupItem.id)
        {
    ...
    if(indexOfExistingItem < 0)
        newItems = existingItems.concat(oLookupItem);
    else
    {
        existingItems[indexOfExistingItem] = oLookupItem;
