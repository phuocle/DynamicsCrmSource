...
var _sLoadingMessageHtml = "<TABLE width='100%' height='100%'><TR><TD ID='loadingMessage' align='center'><IMG id='imgLoading' alt='{0}' src='/_imgs/AdvFind/progress.gif'><br>{0}</TD></TR></TABLE>";
function InPageHelp_ExpandCollapse(e)
{
    ...
    else
    {
        ...
        {
            inPageHelpIframe.contentWindow.document.body.innerHTML = formatString(_sLoadingMessageHtml,CrmEncodeDecode.CrmHtmlEncode(LOCID_PAGELOADING_MSG));
...
function InPageHelp_MouseOverOut(e,over)
{
    ...
    if(divInnAppContent.getAttribute("expanded") == 1)
        helpCollapseImageInfo = Mscrm.ImageStrip.getImageInfo(Mscrm.CrmUri.create(over ? "/_imgs/helpvisor/visoropenhover.png" : "/_imgs/helpvisor/visoropen.png"));
    else
        helpCollapseImageInfo = Mscrm.ImageStrip.getImageInfo(Mscrm.CrmUri.create(over ? "/_imgs/helpvisor/visorclosehover.png" : "/_imgs/helpvisor/visorclose.png"));
                ...
                if(!IsNull(columnRows))
                    for(var i = 1; i <= columnRows.length; i++)
                        try
                        {
                            ...
                            if(!IsNull(defaultOnKeyPressString) && defaultOnKeyPressString.indexOf("ExecuteAction") != -1)
                            {
