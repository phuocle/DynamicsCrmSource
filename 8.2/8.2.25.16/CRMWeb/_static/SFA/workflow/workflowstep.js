...
function GetConditionStepChildren(conditionStep)
{
        ...
        index = 0; index < length; index++)
    {
        ...
        if(oNode.getAttribute("stepname") == "ElseIfCondition")
            ...
            if(oNode.getAttribute("stepname") == "ElseCondition")
                children[2]++;
            else
                if(oNode.getAttribute("stepname") == "IfCondition")
                    children[0]++
    }
    return children
}
...
function ConfigureFormEntity(domEvent,workflowId,stepId,primaryEntityName,mode)
{
    ...
    else
    {
        var parameters = [arrTokens[0],entityNameValue,mode],
            callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("PerformActionAfterConfigureFormEntity",this,parameters,false),
            sRetVal = openStdDlgWithCallback(Mscrm.CrmUri.create("/SFA/Workflow/entityform.aspx?" + param),{},iDialogX,iDialogY + 20,callbackFunc,true,false,"maximize:yes;minimize:yes");
...
function ConfigureAssignTarget(domEvent,workflowId,customStepCategory)
{
    ...
    var parameters = new Array(arrTokens[0]),
        callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("PerformActionAfterConfigureAssignTarget",this,parameters,false),
        sRetVal = openStdDlgWithCallback(Mscrm.CrmUri.create("/SFA/Workflow/assignactivityform.aspx?" + param),{},850,600,callbackFunc);
    isOutlookHostedWindow() && 
...
function ConfigureStopWorkflowMessage(domEvent,workflowId,isReadOnly)
{
    ...
    var parameters = [el,arrTokens[0]],
        callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("PerformActionAfterConfigureStopWorkflowMessage",this,parameters,false),
        sRetVal = openStdDlgWithCallback(Mscrm.CrmUri.create("/SFA/Workflow/statusMessageForm.aspx?" + param),{},850,600,callbackFunc);
    isOutlookHostedWindow() && 
...
function ConfigureCustomActivity(domEvent,workflowId,customStepCategory)
{
    ...
    var parameters = [el.value,arrTokens[0],customStepCategory],
        callBackRef = Mscrm.Utilities.createCallbackFunctionObject("ConfigureCustomActivityCallBack",this,parameters,false),
        sRetVal = openStdDlgWithCallback(Mscrm.CrmUri.create("/SFA/Workflow/customactivityform.aspx?" + param),{},850,600,callBackRef);
    isOutlookHostedWindow() && 
...
function OnInvokeSdkMessageOptionChange(domEvent)
{
    ...
    command.SetParameter("sdkMessageId",Mscrm.FormControlInputBehavior.GetElementBehavior(sdkMessageList).get_dataValue());
...
function OnInvokeEntityOptionChange(domEvent)
{
    ...
    command.SetParameter("activityId",arrTokens[0]);
    command.SetParameter("sdkMessageId",Mscrm.FormControlInputBehavior.GetElementBehavior(sdkMessageList).get_dataValue());
    command.SetParameter("sdkMessageFilterId",Mscrm.FormControlInputBehavior.GetElementBehavior(entityLst).get_dataValue());
    ExecuteCommandAndRefreshWorkflowArea(arrTokens[0],command);
    return null
}
function OnChangeStatusStepStateChanged(domEvent)
{
    ...
    sb.Append('<column id="colParameter" value="');
    sb.Append(CrmEncodeDecode.CrmXmlEncode(Mscrm.FormControlInputBehavior.GetElementBehavior(elLst).get_dataValue()) + '"></column>');
    sb.Append('<column id="colStatus" value="');
    sb.Append(CrmEncodeDecode.CrmXmlEncode(Mscrm.FormControlInputBehavior.GetElementBehavior(stLst).get_dataValue()) + '"></column>');
...
function StatusToState(stLst)
{
    var stLstCtrl = Mscrm.FormControlInputBehavior.GetBehavior(stLst.id),
        state = null;
    XUI.Html.DomUtils.ForEachChild(stLst,Function.createDelegate(this,function(oGroup)
    {
        XUI.Html.DomUtils.ForEachChild(oGroup,Function.createDelegate(this,function(child)
        {
...
function OnChangeStatusStepEntityLstChanged(domEvent)
{
    ...
    XUI.Html.DomUtils.ForEachChild(elLst,Function.createDelegate(this,function(oGroup)
    {
        XUI.Html.DomUtils.ForEachChild(oGroup,Function.createDelegate(this,function(child)
        {
            if(endLoop)
        ...
        i = 0; i < iSize; i++)
    {
            ...
            j = 0; j < len; j++)
        {
            ...
            stLstComponent.AddOption(CrmEncodeDecode.CrmXmlDecode(statusNode.getAttribute("label")),statusNode.getAttribute("value"),groupId)
        }
    }
...
function SetFieldFocusAtOnLoad(activeElement)
{
    if(!IsNull(activeElement))
    {
        ...
        if(Sys.UI.DomElement.containsCssClass(activeElement,"ms-crm-Lookup"))
        {
            ...
            !IsNull(element) && element.length > 0 && 
                SetFieldFocus(element[0])
        }
        else
            if(activeElement.className == "ms-crm-DateTime")
            {
...
{
    if(ele.nodeName != "#document" && XUI.Html.GetComputedStyle(ele,"display") != "none" && XUI.Html.GetComputedStyle(ele,"visibility") != "hidden" && !ele.isDisabled)
    {
...
function CanSetFocus(ele)
{
    while(!IsNull(ele))
    {
        if(ele.nodeName != "#document" && (XUI.Html.GetComputedStyle(ele,"display") == "none" || XUI.Html.GetComputedStyle(ele,"visibility") == "hidden" || ele.getAttribute("disabled") || ele.isDisabled))
            return false;
        ele = ele.parentNode
    }
    return true
...
function CheckParameters()
{
    ...
    while(!IsNull(parameter))
    {
        ...
        if(!IsNull(slug) && slug.IsDataSlug)
        {
            ...
            if(parameter.IsDirty || IsSlugDeleted(parameter))
                return false;
        parameterIndex++;
        parameter = $get("ParameterValue" + parameterIndex)
    }
...
function addHyperlink(textControlId)
{
    ...
    var parameters = new Array(textControlId),
        callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterInsertHyperlink",this,parameters,false),
