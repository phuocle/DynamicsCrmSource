...
function GetPublishXml(onlySelected)
{
    for(var sXml = "<publish><entities>",
        ...
        i = 0; i < oRowCount; i++)
        if(!onlySelected || XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(oList.childNodes[i])).checked)
        {
...
function SelectionChanged()
{
    ...
    if(!IsNull(PublishButton))
        PublishButton.disabled = SelectedRowsCount(XUI.Html.DomUtils.GetFirstChild(SelectedEntities)) == 0 && SelectedRowsCount(XUI.Html.DomUtils.GetFirstChild(AvailableEntities)) == 0
}
function BuildHomePageXML(useOnlySelected)
{
    for(var sXml = "<Entities><AvailableEntities>",
        ...
        oRowCount = oList.childNodes.length,
        i = 0; i < oRowCount; i++)
        if(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(oList.childNodes[i])).checked || !useOnlySelected)
...
function PublishEntities()
{
    if(SelectedRowsCount(XUI.Html.DomUtils.GetFirstChild(SelectedEntities)) == 0 && SelectedRowsCount(XUI.Html.DomUtils.GetFirstChild(AvailableEntities)) == 0)
        return false;
