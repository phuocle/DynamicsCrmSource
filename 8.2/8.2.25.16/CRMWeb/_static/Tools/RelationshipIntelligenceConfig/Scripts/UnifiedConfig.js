...
var internalUtilities = "",
    ...
    crmContentPanel = currentwindow.top.$("#crmContentPanel"),
    crmTopBar = currentwindow.top.$("#crmTopBar");
...
function tabUnSavedChangesConfirmDialog()
{
    ...
    isEmailEnagagementEnabled && isRIEmailEngagementPreviewEnabled && isRIEmailEngagementOrgSettingEnabled != isEmailInteractionAllowed && 
        openUnsavedChangesDialog("tabEmailInteractionAnalysis");
    isADCEnabled && isAutodataCapturePreviewEnabled && isADCOrgSettingEnabled != isAutoCaptureChecked && 
...
function getOrgSettings()
{
    try
    {
        var columnNames = ["organizationid","isactioncardenabled","ispreviewenabledforactioncard","isemailmonitoringallowed","ispreviewforemailmonitoringallowed","isautodatacaptureenabled","isactivityanalysisenabled","ispreviewforautocaptureenabled","isrelationshipinsightsenabled"];
        xrmInternal.messages.retrieve(internalUtilities.EntityNames.Organization,organizationId,columnNames).then(function(responseOrganization)
        {
            ...
            else
            {
                ...
                isRIActionCardPreviewEnabled = previewActionCardsObj !== undefined && previewActionCardsObj !== null && previewActionCardsObj.get_ValueString() == "1" ? true : false;
                var previewEmailEngagementObj = responseOrganization.entity.fields["ispreviewforemailmonitoringallowed"];
                isRIEmailEngagementPreviewEnabled = previewEmailEngagementObj !== undefined && previewEmailEngagementObj !== null && previewEmailEngagementObj.get_ValueString() == "1" ? true : false;
                ...
                isAutodataCapturePreviewEnabled = previewAutoCapture !== undefined && previewAutoCapture !== null && previewAutoCapture.get_ValueString() == "1" ? true : false;
                hidePageLoadingIcon();
...
function updateDisplayOfErrorText(controlid,hide)
{
    if(!hide)
    {
        $("#" + controlid).attr({role:"alert"});
        $("#" + controlid).show()
    }
    else
        $("#" + controlid).hide()
...
function validateIfEmpty(currenttextbox)
{
    if(currenttextbox.id == "huburlValue")
        currenttextbox.value == "" ? updateDisplayOfErrorText("hubUriEmtpyTextErrorId",false) : updateDisplayOfErrorText("hubUriEmtpyTextErrorId",true);
    validateProvisioningSection()
}
function validateIfEmtpyAndCompareAccessKeys(currenttextbox)
{
    if(currenttextbox.id == "accessKey")
        currenttextbox.value == "" ? updateDisplayOfErrorText("accessKeyEmtpyTextErrorId",false) : updateDisplayOfErrorText("accessKeyEmtpyTextErrorId",true);
    if(currenttextbox.id == "reEnterAccessKey")
        if(currenttextbox.value == "")
        {
            updateDisplayOfErrorText("reenterAccessKeyEmtpyTextErrorId",false);
            updateDisplayOfErrorText("reenterAccessKeyCompareTextErrorId",true)
        }
        else
            updateDisplayOfErrorText("reenterAccessKeyEmtpyTextErrorId",true);
    if((currenttextbox.id == "accessKey" || currenttextbox.id == "reEnterAccessKey") && $("#accessKey").val() != "" && $("#reEnterAccessKey").val() != "")
        $("#accessKey").val() != $("#reEnterAccessKey").val() ? updateDisplayOfErrorText("reenterAccessKeyCompareTextErrorId",false) : updateDisplayOfErrorText("reenterAccessKeyCompareTextErrorId",true);
    validateProvisioningSection()
}
...
function loadProvisionResourceString()
{
    ...
    var createTenantLinkText = getLocaleString("RI_Provisioning_CustomerInsightshub_Link"),
        ...
        createtenantinfostring = HtmlEncodeString(internalUtilities._String.format(HtmlDecodeString(getLocaleString("RI_Provisioning_DontHaveHub_Lable")),"<a href='" + HtmlDecodeString(createTenantAzureUrl) + "' target='_blank' title='" + HtmlDecodeString(createTenantLinkText) + "' tabIndex='2'>" + HtmlDecodeString(createTenantLinkText) + "</a>"));
...
var onloadEvent = function()
    {
        if(crmContentPanel && crmTopBar)
        {
            var param = {topOfContentPanel:crmContentPanel.offset().top,topOfTopBar:crmTopBar.offset().top};
            crmContentPanel.css("top",param.topOfTopBar);
            $(crmTopBar).hide()
        }
        $(window).on("unload",param,unloadEvent)
    },
    unloadEvent = function(event)
    {
        if(crmContentPanel && crmTopBar)
        {
            crmContentPanel.css("top",event.data.topOfContentPanel);
            $(crmTopBar).show()
        }
        $(window).off("load");
        $(window).off("unload")
