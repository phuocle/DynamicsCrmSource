...
function Close(oEvent)
{
    ...
    if(isChanged != false)
        if(isManager.checked != isManagerSelected || isPosition.checked != isPositionSelected || hierarchyDepthValue != hierarchyDepthControl.value || hsmControl.checked != hsmConfigurationSelected || excludeEntityChanged == true)
        {
...
function SaveForm(closePage)
{
    var oList = GetList("SelectedEntities"),
        ...
        oCommand = new RemoteCommand("HierarchySecurityConfiguration","SaveHierarchySecurityModelingEntityConfiguration");
    oCommand.SetParameter("entityList",GetSelectedEntities());
    if(!IsNull(document.getElementById("ckEnableHSMConfiguration")) && $get("ckEnableHSMConfiguration").checked == true)
