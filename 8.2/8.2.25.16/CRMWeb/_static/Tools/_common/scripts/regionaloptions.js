...
function LoadLocaleDefaultDataXml(bAsync,oCallback)
{
    ...
    else
    {
        ...
        if(oResult.Success == ERROR_NONE)
            return !IsNull(oCallback) && typeof oCallback == "function" ? oCallback(oResult) : XUI.Xml.GetText(oResult.Xml);
        return null
    }
}
...
function onRegionalFormatChange(o)
{
    ...
    for(var elementId in _sampleDataNodeArray)
    {
        var element = document.getElementById(elementId);
        if(!IsNull(element))
            element.value = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(localeDefaultData,_sampleDataNodeArray[elementId],null))
    }
}
function CustomizeRegionalOptionsClicked(oButton,sUserSettings)
{
    ...
    var parameters = [];
    parameters[0] = lcid;
    var callbackFunctionObject = Mscrm.Utilities.createCallbackFunctionForXrmDialog(regionalOptionsCallback,parameters),
    ...
    if(!IsNull(retval))
    {
        ...
        if(!IsNull(_regionalOptionsUpdatedSampleStandard) && _regionalOptionsUpdatedSampleStandard != "")
        {
            ...
            for(var elementId in _sampleDataNodeArray)
            {
                var element = document.getElementById(elementId);
                if(!IsNull(element))
                    element.value = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oXmlDoc,selectPrefix + _sampleDataNodeArray[elementId],null))
            }
        }
    }
}
