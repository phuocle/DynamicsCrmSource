...
function AddTab(tabTemplateNumber)
{
    ...
    if(_iTabs < _iMaxTabs)
    {
        ...
        var tabId = createGuid(),
            ...
            oTab = new TabObj(tabId,new Array(new LocalizedObj(LOCID_FORMEDITOR_TAB_LABEL,_langCode)),false,true,iTabCols,tabCols,true,Mscrm.FormEditorVariables.formXml,true,true,tabName);
...
function UpdateTab(oActive)
{
    ...
    var oTab = new TabObj(sTabId,oLabels,bLockTab,bAutoExpand,itabCols,icolWidths,bShowLabel,Mscrm.FormEditorVariables.formXml,bVisible,bAvailable,sTabName),
        oArgs = {};
    oArgs.Tab = oTab;
    ...
    if(Mscrm.FormEditorVariables.FormType != "card")
    {
        ...
        var dialogUrl = Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/tab.aspx?title=" + CrmEncodeDecode.CrmUrlEncode(LOCID_FORMED_TABPROPS) + "&description=" + CrmEncodeDecode.CrmUrlEncode(LOCID_FORMED_MODIFYTABPROPS) + "&editorType=" + CrmEncodeDecode.CrmUrlEncode(Mscrm.FormEditorVariables.editorType) + "&formType=" + CrmEncodeDecode.CrmUrlEncode(Mscrm.FormEditorVariables.FormType)),
...
function tabDoubleClickedCallBack(oTab,oActive,oUpdatedTab)
{
    if(oUpdatedTab)
    {
        ...
        if(!IsNull(oUpdatedTab.FormXml) && XUI.Xml.XMLSerializer.serializeToString(oUpdatedTab.FormXml) != XUI.Xml.XMLSerializer.serializeToString(Mscrm.FormEditorVariables.formXml))
        {
...
{
    if(oTabOriginal.TabName != oUpdatedTab.TabName || oTabOriginal.Visible != oUpdatedTab.Visible || oTabOriginal.AvailableForPhone != oUpdatedTab.AvailableForPhone || oTabOriginal.ShowLabel != oUpdatedTab.ShowLabel || oTabOriginal.ColWidths[2] != oUpdatedTab.ColWidths[2] || oTabOriginal.ColWidths[1] != oUpdatedTab.ColWidths[1] || oTabOriginal.ColWidths[0] != oUpdatedTab.ColWidths[0] || oTabOriginal.ExpandByDefault != oUpdatedTab.ExpandByDefault || oTabOriginal.LockTab != oUpdatedTab.LockTab || oTabOriginal.Columns != oUpdatedTab.Columns || GetDescByLangCode(oTabOriginal.Labels) != GetDescByLangCode(oUpdatedTab.Labels))
        return true;
    else
        return false
}
function AddTabXml(oTabObj,iTabCols)
{
    ...
    PersistLocalizedNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + oTabObj.TabId + "']/labels/label",oTabObj.Labels);
    return true
}
function UpdateTabXml(oTabObj,oUpdatedTabObj)
{
    ...
    if(iUpdatedTabCols < iOriginalTabCols)
    {
        for(var columnsToRemove = iOriginalTabCols - iUpdatedTabCols,
            i = columnsToRemove - 1; i >= 0; i--)
        {
            var tabColumnToUpdate = i + iUpdatedTabCols;
            XUI.Xml.SelectSingleNode(oTab,"columns",null).removeChild(XUI.Xml.SelectNodes(oTab,"columns/column",null)[tabColumnToUpdate])
        }
        UpdateTabColumnWidths(oTab,oUpdatedTabObj)
    }
    iUpdatedTabCols == iOriginalTabCols && IsTabModified(oTabObj,oUpdatedTabObj) && 
        UpdateTabColumnWidths(oTab,oUpdatedTabObj);
    PersistLocalizedNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + oUpdatedTabObj.TabId + "']/labels/label",oUpdatedTabObj.Labels)
}
function UpdateTabColumnWidths(oTab,oUpdatedTabObj)
{
    for(var i = 0; i < oUpdatedTabObj.Columns; i++)
        XUI.Xml.SelectNodes(oTab,"columns/column",null)[i].setAttribute("width",oUpdatedTabObj.ColWidths[i].toString() + "%")
}
function AddTabHtml(oTabObj,iTabCols,childNum)
{
    ...
    insertHtml.append("<div><table valign='top' style='table-layout: fixed;' class='ms-crm-Tab' tabID='" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabId) + "' id='" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabId) + "' name='" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabName) + "' onmouseover ='RegisterDragObject(Mscrm.Utilities.eventToDomEvent(event));' onclick='SetActiveObject(Mscrm.Utilities.eventToDomEvent(event));' onmousedown='SetActiveObject(Mscrm.Utilities.eventToDomEvent(event));' ondblclick='ViewPropertiesNow(Mscrm.Utilities.eventToDomEvent(event));' width='100%'>" + (oTabObj.LockTab == true ? "<img alt='" + CrmEncodeDecode.CrmHtmlAttributeEncode(LOCID_FORMEDITOR_TAB_LOCKED) + "' class='imgLock' src='/_imgs/tools/ico_lock.gif' />" : "") + "<tr><td name='tabData'><a name='tabHeader' expanded='true' id='tabHeader_" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabId) + "' class='ms-crm-Menu-Label' onclick='Mscrm.TabUtils.expandCollapseTab(this)' ><IMG style='" + imgStyle + "' name='tabImage' class='ms-crm-Menu-ButtonFirst' alt='' id='tabImage_" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabId) + "' src='/_imgs/tab_section_down.png' /><SPAN style='cursor:pointer;' name='tabText' class='ms-crm-InlineTabHeaderText' style='font-size:12px;font-weight:bold'>" + CrmEncodeDecode.CrmHtmlEncode(GetDescByLangCode(oTabObj.Labels)) + "</SPAN></a><table cellspacing='0' cellpadding='0' class='stdTable' id='tabBody_" + CrmEncodeDecode.CrmHtmlAttributeEncode(oTabObj.TabId) + "'>");
    for(var i = 0; i < iTabCols; i++)
        insertHtml.append("<col width='" + CrmEncodeDecode.CrmHtmlEncode(oTabObj.ColWidths[i].toString()) + "%'/>");
    insertHtml.append("<tr>");
    for(var i = 0; i < iTabCols; i++)
        insertHtml.append("<td><table cellspacing='0' cellpadding='0' class='stdTable'><tr><td></td></tr></table></td>");
...
function UpdateTabHtml(oActive,oTabObj,oUpdatedTabObj)
{
    ...
    var tabTableContainer = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(oActive)));
    if(oUpdatedTabObj.LockTab == true)
        XUI.Html.DomUtils.GetFirstChild(tabTableContainer).innerHTML = "<IMG name='tabImage' id='tabImage_" + CrmEncodeDecode.CrmHtmlAttributeEncode(oActive.id) + "' class='" + imgClass + " ms-crm-Menu-ButtonFirst' style = '" + imgStyle + "'  alt='' src='" + imageSrc + "'/><img alt='" + CrmEncodeDecode.CrmHtmlAttributeEncode(LOCID_FORMEDITOR_TAB_LOCKED) + "' class='imgLock' src='/_imgs/tools/ico_lock.gif' /><SPAN name='tabText' class='ms-crm-InlineTabHeaderText' style=" + style + ">" + CrmEncodeDecode.CrmHtmlEncode(GetDescByLangCode(oUpdatedTabObj.Labels)) + "</SPAN>";
    else
        XUI.Html.DomUtils.GetFirstChild(tabTableContainer).innerHTML = "<IMG name='tabImage' id='tabImage_" + CrmEncodeDecode.CrmHtmlAttributeEncode(oActive.id) + "' class='" + imgClass + "' style = '" + imgStyle + "'  alt='' src='" + imageSrc + "'/><SPAN name='tabText' class='ms-crm-InlineTabHeaderText' style=" + style + ">" + CrmEncodeDecode.CrmHtmlEncode(GetDescByLangCode(oUpdatedTabObj.Labels)) + "</SPAN>";
    ...
    if(iUpdatedTabCols > iOriginalTabCols)
    {
        ...
        if(oUpdatedTabObj.Columns < oTabObj.Columns)
        {
            for(var columnsToRemove = iOriginalTabCols - iUpdatedTabCols,
                i = columnsToRemove - 1; i >= 0; i--)
            {
            ...
            oUpdatedTabObj.Columns === oTabObj.Columns && IsTabModified(oTabObj,oUpdatedTabObj) && 
                UpdateTabColumnWidthsHtml(tabColGroup,oUpdatedTabObj)
}
...
function RemoveTab(oActive)
{
    if(_iTabs > 1)
    {
        var oTab = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + GetCurrentTabName(oActive) + "']",null);
        ...
        if(Mscrm.FormEditorVariables.editorType != Mscrm.EditorType.dashboardEditor && !IsNull(oTab))
        {
            ...
            Mscrm.FieldPropertiesUtils.removeHandlersFromFields(XUI.Xml.SelectNodes(oTab,"columns/column/sections/section/rows/row/cell",null))
        }
