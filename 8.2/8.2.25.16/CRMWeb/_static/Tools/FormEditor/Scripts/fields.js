...
function UpdateField(oActive)
{
    ...
    var oParameters = GetParameterBag(XUI.Xml.SelectNodes(oFormCell,"control/parameters/*",null)),
        oEntityField = XUI.Xml.SelectSingleNode(oFieldsXml,"/entity/fields/field[@name = '" + sActualName + "']",null),
    ...
    var oFieldObj = new FieldObj(sLabel,bShowLabel,bDisabled,sTabName,sSectionName,bLocked,iColSpan,iRowSpan,bAutoExpanding,sControlClassId,sDataType,sDataTypeFormat,sAttributeDisplayName,sAttributeSchemaName,sAttributeDescription,oParameters,false,null,null,Mscrm.FormEditorVariables.formXml,bVisible,bAvailable,sCustomControlUniqueId);
    ...
    if(!(oActive.getAttribute("name") === Mscrm.DragDropUtils.NOTES_CONTROL_ID || controlFor === Mscrm.DragDropUtils.HEADER || controlFor === Mscrm.DragDropUtils.FOOTER))
...
function BindUpdatedFieldProperties(oUpdatedFieldObj,oFieldObj,oActive)
{
    if(oUpdatedFieldObj)
    {
        ...
        {
            var xmlAdjusted = Mscrm.FieldPropertiesUtils.adjustFormXmlForColSpanChange(oCell,oFieldObj.iColSpan,oUpdatedFieldObj.iColSpan);
            if(xmlAdjusted)
            {
        ...
        {
            Mscrm.FieldPropertiesUtils.adjustFormXmlForRowSpanChange(oCell,oFieldObj.iRowSpan,oUpdatedFieldObj.iRowSpan);
...
function AddInteractionWall(oActive)
{
    ...
    var oInteractionWall = new InteractionWallObj(_interactionWallControlId,new Array(new LocalizedObj("Interaction Wall",_langCode)));
...
function AddInteractionWallXml(oInteractionWall)
{
    ...
    PersistLocalizedNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell[control/@id = '" + oInteractionWall.Id + "']/labels/label",oInteractionWall.Labels);
    return cellId
}
...
function AddSubGrid(oActive,isChart,useComponentGallery)
{
    ...
    oArgs.Field = new SubGridObj("","","",rowSpan,rowsPerPage,secCols,false,false,"",false,"",false,false,gridMode,"",true,null,"",false,false,true,null,null,isReferencePanelType(oActive),null);
...
function AddDelve(oActive,useComponentGallery)
{
    ...
    var gridMode = "Grid",
        ...
        oSubGrid = new SubGridObj(subGridId,"",Mscrm.FormEditorVariables.officeGraphDocument,rowSpan,rowsPerPage,secCols,false,true,Mscrm.FormEditorVariables.delveViewId,false,"",false,false,gridMode,"",false,labels,"",false,false,true,gridUIMode);
...
function AddIFrame(oActive,type,useComponentGallery)
{
    ...
    switch(type)
    {
        ...
        case "powerbitile":
            ...
            createInlineDlg(Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/powerbitile.aspx?mode=add&editorType=" + CrmEncodeDecode.CrmUrlEncode(oArgs.EditorType)),oArgs,_subGridDefaults.iDialogHeight + 32,_subGridDefaults.iDialogWidth,BindFormDataOnCallback,[type,sTabName,sSectionName,oArgs.SecCols],null);
            return;
        case "bingmap":
        ...
        case "timercontrol":
            ...
            return;
        case "searchwidget":
            oArgs.Field = new SearchWidgetObj("","","","","","","","","","","","",0,secCols,sTabName,sSectionName,null,false,Mscrm.FormEditorVariables.formXml,true,"",isReferencePanelType(oActive),null,false,"");
...
function UpdateSubGrid(oActive,isChart,useComponentGallery)
{
    ...
    var oSubGrid = new SubGridObj(oControl.getAttribute("id"),XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"RelationshipName",null)),XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"TargetEntityType",null)),parseInt(oFormCell.getAttribute("rowspan"),10),recPerPage,parseInt(oFormCell.getAttribute("colspan"),10),oFormCell.getAttribute("auto") == "true",oFormCell.getAttribute("showlabel") == "true",viewId,enViewPicker,viewIds,enQuickFind,enJumpBar,chartGridMode,visualizationId,enChartPicker,GetLocalizedObjsArray(XUI.Xml.SelectNodes(oFormCell,"labels/label",null)),relOrdinal,isUserView,isUserChart,oFormCell.getAttribute("availableforphone") !== "false",null,null,isReferencePanelSubGrid,referencePanelSubGridIconUrl);
...
function UpdateComponentGallery(oUpdatedGrid,oSubGrid,sTabName,sSectionName,oActive,control,oFormCell,recPerPage)
{
    if(oUpdatedGrid)
    {
        ...
        var bChangeClassId = AddSubgridCustomControlSnippet(oParameters,oUpdatedGrid.CustomControlSnippet,oUpdatedGrid.CustomControlUniqueId);
...
function UpdateIFrame(oActive,useComponentGallery,mode)
{
    ...
    switch(oActive.className)
    {
        ...
        case "aci":
            ...
            break;
        case "powerbitile":
            oIframe = new PowerBITileObj(oControl.getAttribute("id"),parseInt(oFormCell.getAttribute("rowspan"),10),parseInt(oFormCell.getAttribute("colspan"),10),false,sTabName,sSectionName,GetLocalizedObjsArray(XUI.Xml.SelectNodes(oFormCell,"labels/label",null)),oFormCell.getAttribute("showlabel") == "true",XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"PowerBIDashboardId",null)),XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"TileId",null)),XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"TileUrl",null)),0,XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oParameters,"EnableInMobile",null)) == "true",bVisible);
            oArgs.CellId = oActive.id;
            break;
        case "webresource":
    ...
    switch(oActive.className)
    {
        ...
        case "aci":
            ...
            return;
        case "powerbitile":
            createInlineDlg(Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/powerbitile.aspx?mode=" + mode + "&editorType=" + CrmEncodeDecode.CrmUrlEncode(oArgs.EditorType)),oArgs,Math.min(window.parent.innerHeight,_subGridDefaults.iDialogHeight + 32),_subGridDefaults.iDialogWidth,UpdateFormXml,[sTabName,sSectionName,mode,oActive,oIframe],null);
            return;
        case "webresource":
...
function AddSubgridCustomControlSnippet(oParameters,oCustomControlSnippet,sCustomControlUniqueId)
{
    ...
    if(Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) && oCustomControlSnippet != null)
    {
        ...
        if(controlDescriptionInSnippet != null)
        {
            ...
            var defaultControlNodes = XUI.Xml.SelectNodes(formXMLDoc,"entity/form/controlDescriptions/controlDescription[@forControl='" + sCustomControlUniqueId + "']/customControl[@id='" + _ControlClassIds.subgrid + "']",null);
