...
(
	...
	[List].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 4300))
...
(
	...
	(
		...
		[List].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 4300)
	) 
	or
	(
		-- global security
	...
	or 
	[List].[ListId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 4300))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredList] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredList] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
