...
(
	...
	[Goal].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 9600))
...
(
	...
	(
		...
		[Goal].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 9600)
	) 
	or
	(
		-- global security
	...
	or 
	[Goal].[GoalId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 9600))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredGoal] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredGoal] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
