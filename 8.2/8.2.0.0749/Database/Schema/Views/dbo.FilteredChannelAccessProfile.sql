...
(
	...
	[ChannelAccessProfile].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 3005))
...
(
	...
	(
		...
		[ChannelAccessProfile].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 3005)
	) 
	or
	(
		-- global security
	...
	[ChannelAccessProfile].[ChannelAccessProfileId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 3005))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredChannelAccessProfile] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredChannelAccessProfile] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
