...
select
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([Invoice].[DueDate],
			us.TimeZoneCode),
        [Invoice].[DueDate],
...
(
	...
	[Invoice].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 1090))
...
(
	...
	(
		...
		[Invoice].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 1090)
	) 
	or
	(
		-- global security
	...
	[Invoice].[InvoiceId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 1090))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredInvoice] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredInvoice] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
