...
create procedure [dbo].[p_DefragIndexes]
as;
begin;
set nocount on;
...
select @sqlstr = N'alter index @index_name_value ON @table_name_value rebuild';
...
declare indexCursor cursor local FORWARD_ONLY READ_ONLY for
	select IndexId, EntityId, Name,	IsClustered, IsPrimaryKey
	from EntityIndex;
...
open indexCursor;
	...
	begin;
		...
		if (@hasBaseIndex <> 0)
		begin;
			exec dbo.sp_executesql @sqlstr, @parameters, @table_name_value = @baseTableName, @index_name_value = @indexname;
		end;
	
		if (@hasExtensionIndex <> 0)
		begin;
			exec dbo.sp_executesql @sqlstr, @parameters, @table_name_value = @extensionTableName, @index_name_value = @indexname;
		end;
