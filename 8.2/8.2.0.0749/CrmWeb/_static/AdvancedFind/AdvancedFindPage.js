...
function ToggleSearchResultsContextualRibbon() {
    ...
    if (_mainWindow._currentPage == ResultsPage) {
        ...
        parameters["initialTabId"] =
            formatString("EntityTemplateTab.{0}.NoRelationship.SubGridStandard.Mscrm.SubGrid.{0}.MainTab",
                _mainWindow.$find("advFind").get_entityName())
    } else {
...
function ExecuteQuery() {
    ...
    var tmpSearchingMsg =
            "<table height='100%' width='100%' style='cursor:wait' class='ms-crm-Loading'><tr><td valign='middle' align='center' id='ExecutingSearchMessage'><img alt='' src='/_imgs/AdvFind/progress.gif'/><br>" + _mainWindow.LOCID_AF_EXECUTINGSEARCH + "</td></tr></table>",
        fetchResults = _mainWindow.resultFrame.document.getElementById("fetchResults");
    if (!IsNull(fetchResults)) {
        _mainWindow.resultFrame.XUI.Html.DomUtils.ForEachChild(fetchResults,
            _mainWindow.resultFrame.Function.createDelegate(this,
                function(child) {
