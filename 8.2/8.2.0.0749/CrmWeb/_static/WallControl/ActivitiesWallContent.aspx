...
<div class="userThumbnailArea">
    <div class="userThumbnail">
        {{if objectTypeCode == Mscrm.EntityTypeCode.UntrackedEmail}}
        <div class="iconThumbnailUntrackEmail" exchangeweblink="{{html exchangeweblink}}" objecttypecode="{{html objectTypeCode}}" postid="{{html postId}}">
            <a href="#" tabindex="-1">
                {{html imageSource}}
            </a>
        </div>
...
<div id="${postId}_expandedaccordion" class="collapsedaccordion">
    ...
    {{if scheduledEnd}}
    <div class="clear"></div>
    <div class="expandedAttributeName">{{html CrmEncodeDecode.CrmHtmlEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.DUEDATE_LOC_STRING) }}</div>
    <div class="expandedAttributeValue">{{html scheduledEnd}}</div>
    {{/if}}
</div>
...
<%
    if (Util.IsEmailConversationViewFeatureAvailable(Microsoft.Crm.Application.Security.UserInformation.Current))
    {
%>
    {{tmpl "#emailTemplate"}}
<%
...
%>
    ...
    <div id="${postId}_expandedaccordion" class="emailexpandedaccordion">
        ...
        <div style="display: table">
            <div style="display: table-row">
                ...
                {{if to}}
                <div class="expandedAttributeValue" style="display: table-cell">
                    {{each(i, value) to}}
    ...
    <div id="${postId}_lastDIV" class="lastDiv">
        <%
            if (IsEmailEngagementFeatureEnabled())
            {
        %>
            ...
            <div class="emailSentOrOpenParentContianer">
                <span class="emailSentOrOpenTextContianer">
                    <img alt="Follow Email" src="/_imgs/inlineedit/FollowedEmail_16.png" class="emailFollowImageContianer"/>
                </span>
...
<div id="${postId}_expandedaccordion" class="collapsedaccordion">
    ...
    {{if priority}}
    <div class="clear"></div>
    <div class="expandedAttributeName">{{html CrmEncodeDecode.CrmHtmlEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.PRIORITYCODE_LOC_STRING) }}</div>
    <div class="expandedAttributeValue">{{html priority}}</div>
    {{/if}}
</div>
{{tmpl "#lastDivTemplate"}}
...
%>
    <script id="emailCollapse" type="text/html">
        <div id="collapseIcon_${postId}" style="display: inline-flex; width: 100%; padding-bottom: 10px;" class="emailCollapseIcon" title="{{html CrmEncodeDecode.CrmHtmlAttributeEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.EXPAND_CONVERSATION_TOOL_TIP)}}" BaseConversationIndexHash="{{html baseconversationindexhash}}" postid="{{html postId}}">
            <a href="#" title="{{html CrmEncodeDecode.CrmHtmlAttributeEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.EXPAND_CONVERSATION_TOOL_TIP)}}" class="imageExpandCollapse" style="float: left; padding-right: 10px; padding-top: 5px;">
            ...
            <div id="${postId}" class="post" title="{{html text}}" style="cursor: pointer; width: calc(100% - 41px); display: inline; padding-top: 0px; padding-bottom: 0px;" createdby="{{html createdBy}}" createdon="{{html createdOn}}" objecttypecode="{{html objectTypeCode}}" statecode="{{html state}}" entitylogicalname="{{html entityLogicalName}}">
                <div class="userThumbnailArea" style="padding-top: 15px;">
                    <div class="userThumbnail">
                        <div class="iconThumbnail" objecttypecode="{{html objectTypeCode}}" postid="{{html postId}}">
                            <a href="#" tabindex="-1">
                                {{html imageSource}}
                ...
                <div id="${postId}_firstDIV" class="wallbody" style="display: inline;">
                    <a href="#" title="{{html text}}" onclick="Mscrm.Utilities.click(previousSibling);"></a>
                    <div id="${postId}_userName" class="userName"></div>
                    <div id="${postId}_participantsList" class="acTitle">
                        {{each(i, value) participants}} {{html value.partyName}};{{/each}}
                    </div>
                    <div id="${postId}_text" class="acSubject" style="display: inline;">{{html text}}</div>
                    <div id="${postId}_lastDIV" class="lastDivEmail">
                        <div class="acDetailsEmail">{{html modifiedOn}}</div>
                    </div>
                    <div class="emailbodyContentDiv">{{html jQuery("
                        <div/>").html(descriptiontext).text()}}
                    </div>
                </div>
            </div>
        </div>
    ...
    <script id="emailExpand" type="text/html">
        <div id="expandIcon_${postId}" style="display: none; border: 0.1em solid lightgrey" class="emailExpandIcon" title="{{html CrmEncodeDecode.CrmHtmlAttributeEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.COLLAPSE_CONVERSATION_TOOL_TIP)}}" BaseConversationIndexHash="{{html baseconversationindexhash}}" postid="{{html postId}}">
    ...
    <script id="emailTemplate" type="text/html">
        ...
        <div id="${postId}_firstDIV" class="wallbody">
            ...
            <div id="${postId}_userName" class="userName"></div>
            {{if from}}
            <div id="${postId}_Sendername" class="acTitle">
                {{html from[0].partyName}}
            </div>
            {{else}}
            <div class="acTitle">{{html CrmEncodeDecode.CrmHtmlEncode(Mscrm.FormInputControl.ActivitiesWallPage.localizedStrings.SENDER_NOT_FOUND_MESSAGE_LOC_STRING) }}</div>
            {{/if}}
