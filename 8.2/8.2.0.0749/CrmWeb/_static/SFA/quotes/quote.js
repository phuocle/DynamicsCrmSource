...
function CreateOrder() {
    ...
    Xrm.Internal.messages.createOrder(new Microsoft.Crm.Client.Core.Framework.Guid(Xrm.Page.data.entity.getId()), $v_0)
        .then(function($p1_0) {
                var $v_1 = ($p1_0).entity;
...
function $1() {
    ...
    $v_4[2] = $v_5;
    $v_2[$v_5] = 6;
    $v_3[$v_5] = new Xrm.Objects.EntityReference('quote',
    ...
    $v_4[4] = $v_5;
    $v_2[$v_5] = 14;
    $v_3[$v_5] = Xrm.Page.data.entity.attributes.get('description').getValue();
    $v_5 = 'quotenumber';
    $v_4[5] = $v_5;
    $v_2[$v_5] = 14;
    ...
    $v_4[7] = $v_5;
    $v_2[$v_5] = 6;
    var $v_7 = (Xrm.Page.data.entity.attributes.get('ownerid').getValue())[0];
    $v_3[$v_5] = new Xrm.Objects.EntityReference($v_7
        .entityType,
        new Microsoft.Crm.Client.Core.Framework.Guid($v_7.id));
    var $v_8 = new Xrm.Objects.EntityReference('quoteclose', Microsoft.Crm.Client.Core.Framework.Guid.get_empty());
    ...
    $v_9.get_changedFieldNames().addRange($v_4);
    Xrm.Internal.messages.retrieveDefaultStatusForState('quote', 3).then(function($p1_0) {
            ...
            if (!Mscrm.InternalUtilities.JSTypes.isNull($p1_0)) {
                $v_A = ($p1_0).status;
                Xrm.Internal.messages.closeQuote($v_9, $v_A).then(function($p2_0) {
                        $0($v_1);
                    },
                    Mscrm.InternalUtilities.ClientApiUtility.actionFailedCallback);
            }
...
function $0($p0) {
    Xrm.Internal.messages.reviseQuote(new Microsoft.Crm.Client.Core.Framework.Guid(Xrm.Page.data.entity.getId()), $p0)
        .then(function($p1_0) {
                var $v_0 = ($p1_0).entity;
...
function closeQuote() {
    ...
    if (Xrm.Page.context.client.getClient() !== 'Mobile' || !Mscrm.InternalUtilities.DialogUtility.isMDDEnabled()) {
        ...
        $v_3.height = 475;
        $v_3.width = 350;
        var $v_4 = Mscrm.GlobalImported.CrmUri.create('/sfa/quotes/dlg_close.aspx?QuoteId=' +
        ...
        if (Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString($v_1)) {
            ...
            var $v_5 = 2;
            Xrm.Internal.messages.canCloseOpportunity(new Microsoft.Crm.Client.Core.Framework.Guid($v_1),
                new Microsoft.Crm.Client.Core.Framework.Guid(Xrm.Page.data.entity.getId()),
                $v_5).then(function($p1_0) {
...
function closeOpportunityDialogCloseCallback(dialogParams, callbackParams) {
    ...
    Mscrm.CommandBarActions.closeQuoteAndOpportunity(callbackParams['closequotestatusreason_id'],
        $v_1['reason'],
        $v_1['state'],
        callbackParams['description_id'],
        callbackParams['closequotedate_id'],
        null,
        $v_1,
        callbackParams['closequotecreaterevisedquote_id'],
        true);
}
    ...
    if (!IsNull(oResult)) {
        ...
        Xrm.Page.data.save().then(function() {
                Mscrm.CommandBarActions
                    .closeQuoteAndOpportunity(oResult.newStatus,
                        oResult.OpportunityStatus,
                        oResult.OpportunityState,
                        oResult.description,
                        oResult.closeDate,
                        oResult.ActivityXml,
                        oResult.OpportunityInfo,
                        oResult.createRevision,
                        oResult.CloseOpportunity);
            },
            function($p1_0) {
                return;
            });
...
function printQuote() {
    ...
    try {
        var $v_0 = Mscrm.GlobalImported.CrmUri.create('/_grid/cmds/dlg_webmailmerge.aspx?mergetype=3&objectTypeCode=' +
            CrmEncodeDecode.CrmUrlEncode('1084') +
            '&objectId=' +
            CrmEncodeDecode.CrmUrlEncode(Xrm.Page.data.entity.getId()));
        var $v_1 = new Xrm.DialogOptions();
        $v_1.height = 600;
...
function acceptQuote() {
    ...
    if (Xrm.Page.context.client.getClient() !== 'Mobile' || !Mscrm.InternalUtilities.DialogUtility.isMDDEnabled()) {
        ...
        $v_3.height = 500;
        $v_3.width = 475;
        var $v_4 = Mscrm.GlobalImported.CrmUri.create('/sfa/quotes/dlg_accept.aspx?QuoteId=' +
            CrmEncodeDecode.CrmUrlEncode(Xrm.Page.data.entity.getId()) +
            '&opportunityid=' +
            CrmEncodeDecode.CrmUrlEncode($v_1));
...
function performActionAfterAcceptQuote(oResult) {
    ...
    if (Mscrm.InternalUtilities.JSTypes.isNull($v_3)) {
        Xrm.Internal.messages.getActualDate(oResult['closeDate']).then(function($p1_0) {
                ...
                Mscrm.CommandBarActions
                    .performActionAfterAcceptQuote(oResult['newStatus'],
                        oResult['newStatusMsg'],
                        $v_3,
                        oResult['description'],
                        $v_0,
                        $v_1,
