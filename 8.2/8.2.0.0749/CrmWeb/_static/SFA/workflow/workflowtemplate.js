...
var workflowTemplate = function() {
    ...
    function AutoPopulateUniqueName() {
        ...
        if (processName != null &&
            ...
            oSchemaName.set_dataValue(processName.replace(/[^a-zA-Z0-9_-]/g, "")
    ...
    function RefreshTemplatesGrid() {
        ...
        else if (categoryControl.get_dataValue() == LOCID_CUSTOM_OPERATION_WORKFLOW)
            crmGrid.SetParameter("ViewId", LOCID_CUSTOMOPSWORKFLOW_VIEW_ID);
        else if (IsBusinessProcessFlow())
            setEmptyView = true;
        else
    ...
    function GetPrimaryEntityNameFromCurrentSelector() {
        ...
        else if ($get("PrimaryProcessEntity").style.display != "none")
            return Mscrm.FormControlInputBehavior.GetBehavior("PrimaryProcessEntity").get_dataValue();
        else if ($get("PrimaryTbfEntity").style.display != "none")
    ...
    function AdjustRadiosForIE() {
        ...
        } else if (Mscrm.Utilities.isFirefox()) {
        ...
        if (!IsNull(categoryControl)) {
            ...
            for (var option in options)
                if (options[option].DataValue == "4") {
                    bpfOption = new Mscrm.FormInputControl
                        .PicklistOption(options[option].Text, options[option].DataValue);
                    break
                }
    ...
    return {
        ...
        CategoryChanged: function() {
            ...
            !IsNull(entityElement.options[1]) &&
                entityElement.options[1].value == GENERIC_ENTITY_OPTIONVALUE &&
        ...
        HandleKeyDown: function(eventObject) {
            if (eventObject.keyCode == KEY_ENTER) {
                ...
                if (el.tagName != "BUTTON") {
                    if (el.id == "PrimaryEntity" ||
                        el.id == "PrimaryProcessEntity" ||
                        IsTaskFlowEnabled() && el.id == "PrimaryTbfEntity") {
                        el.blur();
                        XUI.Html.DispatchDomEvent(el, XUI.Html.CreateDomEvent("change"));
                        window.setTimeout("workflowTemplate.BtnOk()", 1);
                        return true
                    } else if (el.id == "workflowMode") {
                        if ($get("workflowMode").checked)
