...
function treeClose(oEvent) {
    ...
    if (oTreeFrame != null) {
        ...
        if (crmFormCtrl.BuildXml(false, true) != 3) {
            ...
            return msg
        } else if (oTreeFrame != null &&
            typeof oTreeFrame.contentWindow.IsDirty != "undefined" &&
            oTreeFrame.contentWindow.IsDirty()) {
            oEvent.returnValue = msg;
            return msg
        }
    }
...
function SetNotification(sNotification) {
    ...
    if (IsNull(sNotification) || sNotification.length == 0)
        ...
        var oaNot = new Array(crmNotifications
            .CreateNotification("NoResources", 2, sNotificationSource, sNotification));
