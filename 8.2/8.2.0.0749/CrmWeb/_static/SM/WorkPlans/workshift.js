...
function newWorkShift(parentCalendarId, resourceId, oType) {
    openStdWin(Mscrm.CrmUri.create("/SM/workplans/edit.aspx?id=&calendarId=" +
            CrmEncodeDecode.CrmUrlEncode(parentCalendarId) +
            "&resourceId=" +
            CrmEncodeDecode.CrmUrlEncode(resourceId) +
            "&oType=" +
            CrmEncodeDecode.CrmUrlEncode(oType) +
            "&selecteddates=" +
            CrmEncodeDecode.CrmUrlEncode(window.frames["areaMonthlyCalendarFrame"].frames["MonthCalendar"]
                .$get("frmGrid")
                .selectedDates.value) +
            "&name=Working Hours&mode=New"),
        "Calendar",
        550,
        540)
}
...
function newWorkDay(parentCalendarId, resourceId, oType) {
    var callbackFunc = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performGridRefreshAction, [this]),
        dialogUrl = Mscrm.CrmUri.create("/SM/workplans/Dialogs/timesheet.aspx?id=&calendarId=" +
            CrmEncodeDecode.CrmUrlEncode(parentCalendarId) +
            "&resourceId=" +
            CrmEncodeDecode.CrmUrlEncode(resourceId) +
            "&oType=" +
            ...
            "&selecteddates=" +
            CrmEncodeDecode.CrmUrlEncode(window.frames["areaMonthlyCalendarFrame"].frames["MonthCalendar"]
                .$get("frmGrid").selectedDates.value));
    inlineDialogHelper(dialogUrl, 660, 500, null, null, callbackFunc)
}

function newTimeOff(parentCalendarId, resourceId) {
    var callbackFunc = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performGridRefreshAction, [this]),
        dialogUrl = Mscrm.CrmUri.create("/SM/workplans/Dialogs/timeoff.aspx?id=&calendarId=" +
            CrmEncodeDecode.CrmUrlEncode(parentCalendarId) +
            "&resourceId=" +
            CrmEncodeDecode.CrmUrlEncode(resourceId) +
            "&oType=" +
