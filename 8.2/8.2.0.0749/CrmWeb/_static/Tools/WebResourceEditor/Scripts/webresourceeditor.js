...
function CheckPathName() {
    if (validate) {
        ...
        if (pathValue.toLowerCase().endsWith(".aspx") ||
            pathValue.toLowerCase().endsWith(".ashx") ||
            pathValue.toLowerCase().endsWith(".ascx")) {
    ...
    if (CanCustomizeWebResource()) {
        ...
        if (url.length > 0) {
            if (_crmFormSubmit.webResourceType.value == typeMapping["xap"])
                url = previewUrl +
                    "?Name=" +
                    CrmEncodeDecode.CrmUrlEncode(XUI.Html.GetText($get("solPrefix")) +
                        _crmFormSubmit.pathAndFileName.value) +
                    "&SilverLightVersion=" +
                    CrmEncodeDecode.CrmUrlEncode(_crmFormSubmit.silverlightVersion.value);
            else
                url = url + "?preview=1";
...
function setResourceDependencies() {
    _crmFormSubmit.webResourceDependencyXML.value = XUI.Xml.XMLSerializer
        .serializeToString(XUI.Xml.SelectSingleNode(Mscrm.FormLibraryAndEventHandlerUtils.dependencyXml,
            "/Dependencies"))
}
