...
function TemplateProperties(mode) {
    var oTemplateObj = new TemplateObj(titleComponent.get_element().value,
            descriptionComponent.get_element().value,
            languageCodeComponent.get_element().value),
    ...
    var url = Mscrm.CrmUri.create("/Tools/KBTemplateEditor/Dialogs/kbtemplateproperties.aspx?mode=" + mode),
        ...
        callbackObject = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(templatePropertiesCallbackFunction, parameters);
...
function MoveUp() {
    ...
    if (oParentDomNode.rowIndex > 2) {
        ...
        oParentXmlNode.insertBefore(oParentXmlNode.removeChild(XUI.Html.DomUtils.GetPrevSibling(oNode)),
            XUI.Html.DomUtils.GetNextSibling(oNode));
        oParentDomNode.parentNode.insertBefore(oParentDomNode.parentNode
            .removeChild(XUI.Html.DomUtils.GetPrevSibling(oParentDomNode)),
            XUI.Html.DomUtils.GetNextSibling(oParentDomNode));
        UpdateFormData()
    }
}
...
function MoveDown() {
    ...
    if (oParentDomNode.rowIndex < oParentDomNode.parentNode.parentNode.rows.length - 1) {
        ...
        oParentDomNode.parentNode.insertBefore(oParentDomNode.parentNode
            .removeChild(XUI.Html.DomUtils.GetNextSibling(oParentDomNode)),
            oParentDomNode);
        UpdateFormData()
    }
}
...
function GetSectionNode() {
    return XUI.Xml.SelectSingleNode(oStructureXml,
        "kbarticle/sections/section[@id = '" + _oActive.getAttribute("sectionId") + "']",
        null)
}