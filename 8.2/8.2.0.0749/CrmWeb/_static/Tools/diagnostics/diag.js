...
var tests = {
        latency: { func: runLatencyTest, desc: "Latency Test" },
        bandwidth: { func: runBandwidthTest, desc: "Bandwidth Test" },
        browserInfo: { func: runBrowserInfoTest, desc: "Browser Info" },
        ipAddress: { func: runIpAddressTest, desc: "IP Address" },
        jsArrayBenchmark: { func: runJsArrayBenchmark, desc: "JavaScript Array Benchmark" },
        jsMorphBenchmark: { func: runJsMorphBenchmark, desc: "JavaScript Morph Benchmark" },
        jsBase64Benchmark: { func: runJsBase64Benchmark, desc: "JavaScript Base64 Benchmark" },
        jsDomBenchmark: { func: runJsDomBenchmark, desc: "JavaScript Dom Benchmark" },
        orgInfo: { func: runOrgInfoTest, desc: "Organization Info" },
        all: { func: runAllTests, desc: "All Tests" }
    },
    testQueue = [];
...
function buildTestMatrix() {
    for (var testId in tests) {
        ...
        actionCell.innerHTML = window.location.href.indexOf("debug=1") > 0 || testId == "all"
            ? "       <input type='button' id='runBtn_" +
            testId +
            "' value='Run' class='runButton' onclick=\"runTest('" +
            testId +
            "')\" />"
            : "";
...
function emailResults() {
    var subject = escape("CRM Client Performance Report" +
            (ORG_UNIQUE_NAME !== "" ? " from [" + ORG_UNIQUE_NAME + "]" : "")),
...
function runBandwidthTest(testId) {
    for (var trialsToRun = 10,
        adaptionSchedule = [
            { speed: 0, url: "/_static/Tools/Diagnostics/random100x100.jpg" },
            { speed: .5, url: "/_static/Tools/Diagnostics/random350x350.jpg" },
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (clIdx >= 0) {
                ...
                if (clEndIdx > 0)
                    downloadedContentLength = headers
                        .substr(clIdx + clString.length, clEndIdx - clIdx - clString.length).replace("\r", "")
            }
        }
    };
