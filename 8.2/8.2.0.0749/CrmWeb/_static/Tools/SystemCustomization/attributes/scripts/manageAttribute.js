...
var _oMessages = { sCloseAlert: LOCID_FORMS_SAVE_CONFIRM_TITLE },
    ...
    _oConst = {
        ...
        sCreateAttributeUrlFormat: "/tools/systemcustomization/attributes/manageAttribute.aspx?entityId={0}",
        sUpdateAttributeUrlFormat:
            "/tools/systemcustomization/attributes/manageAttribute.aspx?attributeId={0}&entityId={1}&_bEdit={2}",
        sInformationPageId: "divInformation",
        sBusinessRulesPageId: "divBusinessRules",
        sLookupRelationshipEditorUrlFormat:
            "/tools/systemcustomization/relationships/managerelationship.aspx?attributeId={0}&entityId={1}",
        sRelationshipEditorUrlFormat:
            "/tools/systemcustomization/relationships/managerelationship.aspx?attributeId={0}&entityId={1}&referencedEntity={2}",
        sPicklistEditorUrlFormat: "/tools/systemcustomization/optionset/optionset.aspx?id={0}",
        sPblEditorUrlFormat:
            "/tools/systemcustomization/calculatedfields/manageCalculatedFields.aspx?BRlaunchpoint=AttributeEditor&attributeId={0}&entityId={1}",
        sRollUpEditorUrlFormat:
            "/tools/systemcustomization/RollupFields/manageRollupFields.aspx?BRlaunchpoint=AttributeEditor&attributeId={0}&entityId={1}",
        sLookupSchemaNameFormat: "{0}",
        sDynamic: "1",
        sDateOnly: "2",
        sStaticTimeInvariant: "3"
    },
    _bSaving = false,
    _sInputXml = "",
    _iOldPrecision = null,
...
function ManageAttributeJsWindowOnLoad() {
    ...
    _bEdit &&
        !isNullOrEmptyString(_sAttributeId) &&
        OpenFormulaEditor();
...
function remoteAttributeCommandCallback(oResult, oCallbackParams) {
    if (oResult.Success) {
        ...
        if (oCallbackParams.bReload) {
            var oUrl = Mscrm.CrmUri.create(formatString(_oConst.sUpdateAttributeUrlFormat,
                _sAttributeId,
                _sEntityId,
                _bEdit));
...
function saveAttribute(bClose, bNew) {
    if (!_bSaving && validateParameters()) {
        ...
        if (_bCreate)
            ...
            frmReload.submit();
            return
        } else if (_bEdit && !isNullOrEmptyString(_sAttributeId))
            OpenFormulaEditor();
        else
            _bSaving = false
    } else
        !isFormDirty() &&
            ...
            !isNullOrEmptyString(_sAttributeId) &&
            OpenFormulaEditor();
    !isNullOrEmptyString(_sAttributType) &&
        _sAttributType == "status" &&
...
function getAttributeXml() {
    ...
    _linkedAttributeRequired &&
        _bCanCreateLinkedAttribute &&
        addLinkedAttributeXml(oDataXml);
    return convertXmlDocToString(oDataXml)
}

function getStatesXml() {
    return convertXmlDocToString(XUI.Xml.SelectSingleNode(XUI.Xml.LoadXml(getAttributeXml()),
        "attribute/type/states",
        null))
}
...
function addSchemaXml(oDataXml) {
    ...
    addControlDataValue(oDataXml,
        _oTags
        .isfieldsecurityenabled,
        $get("rdIsFieldSecurityEnabled"),
        bAddEvenIfDisabled());
    ...
    typeof selectIsGlobalFilterEnabled != "undefined" &&
        addControlDataValue(oDataXml,
            _oTags.isglobalfilterenabled,
            $get("selectIsGlobalFilterEnabled"),
            bAddEvenIfDisabled());
...
function addLinkedAttributeXml(oDataXml) {
    ...
    if (_selectLinkedAttribute.selectedIndex > 0 &&
        (_isCurrentSelectedLookup || _selectLinkedAttribute.selectedIndex < _selectLinkedAttribute.length - 1))
        addControlDataValue(oDataXml, "linkedattributeid", _selectLinkedAttribute);
    else if (_txtNewAttributeName.get_dataValue() != null) {
        var regExp = new RegExp(_oConst.sInvalidSchemaNameChars, "g"),
            linkedattributeschemaname = _txtNewAttributeName.get_dataValue().replace(regExp, "")
                .substr(0, _txtSchemaName.get_element().maxLength);
...
function addLookupXml(oTypeXml) {
    if (!_selectTarget.get_element().disabled && _selectTarget.get_element().selectedIndex >= 0) {
        ...
        addTextXmlNode(oRelationshipXml,
            _oTags.attributeschemaname,
            formatString(_oConst.sLookupSchemaNameFormat,
                _txtSchemaName.get_dataValue() == null ? "" : _txtSchemaName.get_dataValue()));
        ...
        addControlDataValue(oRelationshipXml,
            _oTags.isfieldsecurityenabled,
            $get("rdIsFieldSecurityEnabled"),
            bAddEvenIfDisabled());
...
function addCustomerXml(oTypeXml) {
    ...
    if (!IsNull(txtPrefix)) {
        addTextXmlNode(oRelationshipXml,
            _oTags.schemaname,
            Trim(XUI.Html.GetText(txtPrefix)) + $get("txtAccountRelationshipName").value);
        addTextXmlNode(oRelationshipXml,
            _oTags.schemaname2,
            Trim(XUI.Html.GetText(txtPrefix)) + $get("txtContactRelationshipName").value)
    } else {
...
function bAddEvenIfDisabled() {
    ...
    if (_selDataSource.get_dataValue() == _oDataSources.sCalculated ||
        _selDataSource.get_dataValue() == _oDataSources.sRollup ||
        _linkedAttributeRequired &&
        _bIsCustomAttribute != "False" &&
        _selectLinkedAttribute.selectedIndex == _selectLinkedAttribute.length - 1 &&
        !_isCurrentSelectedLookup)
        return true;
...
function checkForDateTimeBehaviorChange(bClose, bNew) {
    ...
    if (!isNull(_newBehavior) && _dateTimeBehavior != _newBehavior && !_bCreate) {
        var _newBehaviorString = document.getElementById("selDatetimeBehavior")[document
                .getElementById("selDatetimeBehavior").selectedIndex].innerHTML,
...
function addStatusXml(oTypeXml) {
    ...
    for (iState = 0; iState < _aStates.length; iState++)
        if (_aStates[iState] != undefined)
            sStatusXml += formatString('<state value="{0}" label="{1}">{2}</state>',
                CrmEncodeDecode.CrmXmlAttributeEncode(iState),
                CrmEncodeDecode.CrmXmlAttributeEncode(_aStateLabels[iState]),
                _aStates[iState]);
    sStatusXml += "</states>";
...
function validateParameters() {
    ...
    if (bValid)
        ...
        else if (_linkedAttributeRequired && !_bCreate && typeof LOCID_UPDATE != "undefined") {
            if (!confirm(LOCID_UPDATE))
                return false
        } else if (typeof LOCID_UPDATELINKTOATTRIBUTE != "undefined")
...
function validateMoneyParams() {
    ...
    return validateRequiredValue(_selMoneyPrecision.get_element()) &&
...
function validateIntegerParams() {
    ...
    return Mscrm.FormControlInputBehavior.GetBehavior("selIntFormat").get_dataValue() != null ||
...
function validateLookupParams() {
    ...
    return validateRequiredValue(_selectTarget.get_element()) &&
        validateRequiredValue(txtRelationshipName) &&
        validateSchemaName(txtRelationshipName)
}
...
function validateCustomerParams() {
    ...
    return validateRequiredValue(txtAccountRelationshipName) &&
        validateRequiredValue(txtContactRelationshipName) &&
        validateSchemaName(txtAccountRelationshipName) &&
        validateSchemaName(txtContactRelationshipName)
}
...
function validateLinkedAttributeParams(selectLinkedAttribute) {
    ...
    if (selectLinkedAttribute.selectedIndex != 0 && !_bCreate)
        if (XUI.Html.GetText(selectLinkedAttribute) !=
            Mscrm.FormControlInputBehavior.GetBehavior("txtLinkedAttributeName").get_dataValue())
            if (typeof LOCID_UPDATELINKEDATTRIBUTE == "undefined")
                return true;
            else if (!confirm(LOCID_UPDATELINKEDATTRIBUTE))
                return false;
            else
                return true;
    if (selectLinkedAttribute.selectedIndex == 0) {
        ...
        return false
    } else if (selectLinkedAttribute.selectedIndex == selectLinkedAttribute.length - 1 && !_isCurrentSelectedLookup)
        return validateRequiredValue(_txtNewAttributeName.get_element());
    else if (selectLinkedAttribute.selectedIndex != 1)
...
function onDisplayNameChange() {
    if (_bCreate) {
        if (_txtDisplayName.get_dataValue() != null &&
            _txtDisplayName.get_dataValue().length > 0 &&
            (_txtSchemaName.get_dataValue() == null || _txtSchemaName.get_dataValue().length == 0)) {
            var regExp = new RegExp(_oConst.sInvalidSchemaNameChars, "g");
            _txtSchemaName.set_dataValue(_txtDisplayName.get_dataValue().replace(regExp, "")
                .substr(0, _txtSchemaName.get_maxLength()))
        }
        var selectTarget = Mscrm.FormControlInputBehavior.GetBehavior("selectTarget");
        if (!IsNull(selectTarget.get_dataValue()))
            $get("txtRelationshipName").value = selectTarget.get_dataValue() +
                "_" +
                _CurrentEntityName +
                "_" +
                _txtSchemaName.get_dataValue();
        $get("txtAccountRelationshipName").value = "account_" +
            _CurrentEntityName +
            "_" +
            _txtSchemaName.get_dataValue();
        $get("txtContactRelationshipName").value = "contact_" +
            _CurrentEntityName +
            "_" +
            _txtSchemaName.get_dataValue()
    }
...
function onTypeChange() {
    ...
    if (selDataValue == _oTypes.sBoolean ||
        selDataValue == _oTypes.sPicklist ||
        selDataValue == _oTypes.sLookup ||
        selDataValue == _oTypes.sOwner ||
        selDataValue == _oTypes.sCustomer ||
        selDataValue == _oTypes.sPrimaryKey ||
        selDataValue == _oTypes.sStatus ||
        selDataValue == _oTypes.sImage) {
    ...
    if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CalculatedFields) ||
        Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.Rollups))
        if (selDataValue == _oTypes.sMemo ||
            selDataValue == _oTypes.sFloat ||
            selDataValue == _oTypes.sLookup ||
            selDataValue == _oTypes.sOwner ||
            selDataValue == _oTypes.sCustomer ||
            selDataValue == _oTypes.sPrimaryKey ||
    ...
    if (selDataValue == _oTypes.sImage) {
        ...
        if (_selDataSource.get_dataValue() == _oDataSources.sCalculated ||
            ...
            selReqLevel.value = "None";
            selReqLevel.disabled = true
        } else if (!IsNull(selReqLevel) && selReqLevel.disabled)
    ...
    if (!IsNull(picklist_section) && !IsNull(picklist_control)) {
        picklist_section.style.display = selDataValue == _oTypes.sPicklist &&
            Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 0
            ? ""
            : "none";
        picklist_control.style.display = selDataValue == _oTypes.sPicklist &&
            Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 0
            ? ""
            : "none";
    ...
    if (_linkedAttributeRequired)
        if (selDataValue == _oTypes.sLookup && !_isCurrentSelectedLookup) {
            ...
            removeCreateNew()
        } else if (_isCurrentSelectedLookup && selDataValue != _oTypes.sLookup) {
            _isCurrentSelectedLookup = false;
            addCreateNew()
        }
    ...
    if (!isNull(_selDataSource) &&
        !isNull(_selDataSource.get_dataValue()) &&
        (_selDataSource.get_dataValue() == _oDataSources.sCalculated ||
            _selDataSource.get_dataValue() == _oDataSources.sRollup))
...
function filterIntFormatOnChange() {
    ...
    if (!isNull(_oVisibleIntParams) && _oVisibleIntParams.length > 0)
        ...
        else if (!IsNull(oVisibleParams) && !IsNull(oVisibleParams.length))
            for (var i = 0; i < oVisibleParams.length; i++)
    ...
    if (!isNull(_selDataSource)) {
        ...
        if (!isNull(dataSourceOptions)) {
            if (selDataValue == _oTypes.sInteger ||
                selDataValue == _oTypes.sDecimal ||
                selDataValue == _oTypes.sMoney ||
                selDataValue == _oTypes.sDateTime)
                _selDataSource.set_options(dataSourceOptions);
            else if (selDataValue == _oTypes.sString ||
                selDataValue == _oTypes.sBoolean ||
                selDataValue == _oTypes.sPicklist ||
                selDataValue == _oTypes.sStatus ||
                selDataValue == _oTypes.sOwner ||
                selDataValue == _oTypes.sCustomer ||
...
function displayUpdateTooltipForDateTimeBehavior() {
    ...
    } else if (GetDataFromControl("selDatetimeBehavior") == _oConst.sStaticTimeInvariant) {
        $get("dynamicToolTip").style.display = "none";
        $get("dateOnlyToolTip").style.display = "none";
        $get("dateOnlyToolTipForUpdate").style.display = "none";
        if ($get("selDatetimeBehavior").disabled == false) {
    ...
    } else if (GetDataFromControl("selDatetimeBehavior") == _oConst.sDynamic) {
        $get("dynamicToolTip").style.display = "";
        $get("dateOnlyToolTipForUpdate").style.display = "none";
        $get("dateOnlyToolTip").style.display = "none";
        $get("staticTimeZoneInvariantToolTip").style.display = "none";
...
function LinkedAttributeDataChanges() {
    ...
    } else if (selDataValue == _oTypes.sInteger) {
    ...
    } else if (selDataValue == _oTypes.sFloat) {
    ...
    } else if (selDataValue == _oTypes.sDecimal) {
    ...
    } else if (selDataValue == _oTypes.sDateTime) {
    ...
    for (var xmlNodes = XUI.Xml.SelectNodes(oXmlDoc, xpath, null),
        i = _isCurrentSelectedLookup ? _selectLinkedAttribute.length - 1 : _selectLinkedAttribute.length - 2;
        i > 1;
        i--)
    ...
    if (_sLinkedAttributeId != null &&
        _sLinkedAttributeId != "" &&
        _sLinkedAttributeId != "{00000000-0000-0000-0000-000000000000}") {
        var node = document.createElement("option");
        if (_isCurrentSelectedLookup) {
            _selectLinkedAttribute.options.add(node, _selectLinkedAttribute.length);
            _selectLinkedAttribute.selectedIndex = _selectLinkedAttribute.length - 1
...
function onValidateClick() {
    ...
    if (!IsNull(oNewAttribute.get_dataValue())) {
        ...
        var severity = Mscrm.Severity.SEVERITYINFORMATION,
            ...
            linkedattributeschemaname = oNewAttribute.get_dataValue().replace(regExp, "")
                .substr(0, $get("txtSchemaName").maxLength),
...
function showFormatSpecificParams(sFormat, sParamGroupSuffix) {
    for (var oVisibleParams = Mscrm.Utilities
                 .getChildElementsByClassName($get("sectionBodyTable"), sFormat + sParamGroupSuffix),
        i = 0;
        i < oVisibleParams.length;
        i++)
        oVisibleParams[i].style.display = "";
...
function OnOptionSetChange() {
    if (Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 1 &&
        !IsNull(_selectOptionSet.get_dataValue()) &&
        _selectOptionSet.get_dataValue().length > 1) {
...
function ReloadDefault() {
    ...
    if (ledtPicklistDefault != null) {
        ...
        XUI.Html.SetOuterHTML(ledtPicklistDefault,
            _ledtPicklistValues.createDefaultControl("ledtPicklistDefault", defaultValue));
