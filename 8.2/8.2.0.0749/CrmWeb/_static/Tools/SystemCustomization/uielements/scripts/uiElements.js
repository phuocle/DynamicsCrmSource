...
var _oErrors = {
        ...
        sCannotDeactivateNonPublicView: LOCID_MANUIE_CANTDEANONPUBVIEW,
        sCannotDeactivateDefaultView: LOCID_MANUIE_CANTDEACDEFVIEW,
        sCannotDefaultInactiveView: LOCID_MANUIE_CANTDEFINACTVIEW,
    ...
    _oConst = {
        ...
        sShowDependencyAction: "showDependency",
        sActivateAction: "activate",
        sDeactivateAction: "deactivate",
        sManagedPropertyAction: "managedProperty",
        ...
        iViewEditorHeight: 510,
        sActivePublicView: "ActivePublic",
        sInactivePublicView: "InactivePublic",
        sDefaultPublicView: "DefaultPublic",
        sViewStateActive: "Active",
        sViewStateInactive: "Inactive",
...
function dispatchAction(sUIElementOid, sAction) {
    if (sUIElementOid == null)
        ...
        case _oConst.sManagedPropertyAction:
            Mscrm.ManagedPropertyUtil
                .launchManagedPropertyDlg(Mscrm.EntityTypeCode.SavedQuery, getViewId(sUIElementOid));
            break;
        default:
...
function createView() {
    var oUrl = Mscrm.CrmUri.create(formatString(_oConst.sCreateViewUrlFormat,
        _entityOtc,
        CrmEncodeDecode.CrmUrlEncode(GetEntityIdFromQuery())));
...
function editView(sUIElementOid) {
    var viewId = getViewId(sUIElementOid),
        oUrl = Mscrm.CrmUri.create(formatString(_oConst.sEditViewUrlFormat,
            viewId,
            CrmEncodeDecode.CrmUrlEncode(GetEntityIdFromQuery())));
...
function setDefaultView(sUIElementOid) {
    ...
    case _oConst.sActivePublicView:
        var sUrl = Mscrm.CrmUri.create(formatString(_oConst.sSetDefaultUrlFormat,
                getViewId(sUIElementOid),
                getDefaultViewId(),
                GetTokenAsQS(Mscrm.CrmUri.create("/tools/ViewEditor/cmds/cmd_setdefaultview.aspx")))).toString(),
...
function onFilterChange() {
    var gridUIElements = $find("gridUIElements");
    gridUIElements.SetParameter(_oConst.uiElementsFilter,
