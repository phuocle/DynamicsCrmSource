...
var _oSCVConst = { regexpSchemaNameChars: RegExp("[A-Za-z0-9_]", "g") },
    _oSCVErrors = {
        sCannotBeBlank: LOCID_SYSCUST_CANTBEBLANK,
        sCannotHaveInvalidChars: LOCID_SYSCUST_INVALCHARS,
        sCanOnlyHaveAlphaNumerics: LOCID_SYSCUST_ALPHANUMSONLY,
        sInvalidColorValue: LOCID_SYSCUST_INVALIDCOLORVALUE,
        sCollectionNameShouldBeDifferent: LOCID_SYSCUST_COLLECTIONAME,
        sCannotSetOneNoteInteg: LOCID_SYSCUST_CANTSETONENOTEINT,
        sDaysSinceRecordLastModifiedDefault: LOCID_SYSCUST_DAYS_DEFVAL_ALERT,
        sDaysSinceRecordLastModifiedChanged: LOCID_SYSCUST_DAYS_CHANGED_ALERT,
        sDaysSinceRecordLastModifiedIncreased: LOCID_SYSCUST_DAYS_INCR_ALERT,
        sDaysSinceRecordLastModifiedDecreased: LOCID_SYSCUST_DAYS_DECR_ALERT,
        sMobileOfflineFiltersDefault: LOCID_SYSCUST_NOFILTERS_ALERT,
        sMobileOfflineFiltersChanged: LOCID_SYSCUST_FILTERS_CHG_ALERT
    };
...
function validateName(txtName, oValidCharsRegExp, sError_InvalidCharsFormat, sError_InvalidChars) {
    ...
    else if (sName.replace(oValidCharsRegExp, "") != "")
...
function validateDaysSinceRecordLastModifiedValue(currentValue, oldValue, enablingMobileOffline) {
    ...
    else if (!enablingMobileOffline && currentValue != oldValue)
