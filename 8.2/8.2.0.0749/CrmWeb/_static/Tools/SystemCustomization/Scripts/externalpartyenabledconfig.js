...
function SelectionChanged() {
    ...
    if (!IsNull(RemoveButton)) {
        ...
        RemoveButton.disabled = SelectedRowsCount(selectedAttributesFirstChild) != 1 ||
            _externalPartyEnabledEntityDisplayNames.indexOf(selectedRow.attributes.getNamedItem("name").value) > -1
    }
}
...
function DoubleClick(event) {
    if (IsNull(_oRow) ||
        _externalPartyEnabledEntityDisplayNames.indexOf(_oRow.attributes.getNamedItem("name").value) > -1)
        return;
    var oList = GetList(GetListName());
    UnselectRows(oList);
    _oRowWithoutShift = _oRow;
