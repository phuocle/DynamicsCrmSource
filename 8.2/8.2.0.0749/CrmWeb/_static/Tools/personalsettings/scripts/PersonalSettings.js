    ...
    if (!IsNull(checkBoxAllowCredentials))
        if (checkBoxAllowCredentials.checked) {
            if (!IsNull(textBoxEmailUserName) && !IsNull(textBoxEmailPassword) && !IsNull(textBoxEmailPasswordRetype))
                if (textBoxEmailUserName.value.trim() == "" ||
                    textBoxEmailPassword.value == "" ||
                    textBoxEmailPasswordRetype.value == "") {
                    retValue = false;
                    alert(LOCID_MSG_EMPTY_CREDENTIALS);
                    return
                }
            if (Mscrm.FormUtility.isControlDirty(textBoxEmailPassword) ||
                Mscrm.FormUtility.isControlDirty(textBoxEmailPasswordRetype))
...
function updateUserSettings() {
    ...
    if (!IsOutlookClient()) {
        ...
        for (var sEntityNames = ["Appointment", "Task", "Contact", "Email"],
            i = 0;
            i < sEntityNames.length;
            i++)
            retval.Append(getCheckBoxUserSetting("ckUseCrmFormFor" + sEntityNames[i],
                "usecrmformfor" + sEntityNames[i].toLowerCase()));
        retval.Append(getCheckBoxUserSetting("ckSyncContactCompany", "synccontactcompany"));
        retval.Append(getTextBoxUserSetting("numPeriodBetweenABPScheduledSync",
        ...
        retval.Append(getCheckBoxUserSetting("ckDuplicateDetectionWhenGoingOnline",
...
function updateAutoCreateSettings() {
    ...
    if (!IsNull(checkAutoCreateEnabled) &&
    (Mscrm.FormUtility.isControlDirty(checkAutoCreateEnabled) ||
        Mscrm.FormUtility.isControlDirty(selectAutoCreateEntity))) {
...
function getEmailSettingsXml() {
    ...
    if (!IsNull(checkBoxAllowCredentials))
        ...
        else if (Mscrm.FormUtility.isControlDirty(checkBoxAllowCredentials)) {
...
function GetEnableDefaultCountryCodeFlag() {
    ...
    if (document.getElementById("ckEnableDefaultCountryCode") != null)
        Mscrm.FormUtility.isControlDirty($get("ckEnableDefaultCountryCode")) &&
            retval.Append("<isdefaultcountrycodecheckenabled>" +
...
function GetEnableDefaultCountryCodeValue() {
    ...
    if (document.getElementById("txtDefaultCountryCode") != null)
        if (Mscrm.FormUtility.isControlDirty($get("txtDefaultCountryCode"))) {
            var txtDefaultCountryCodeControl = Mscrm.FormControlInputBehavior.GetBehavior("txtDefaultCountryCode"),
                dataValue = txtDefaultCountryCodeControl.get_dataValue()
...
function updateFilterSettings() {
    ...
    if (filterXml != "") {
        ...
        xmlhttpObj.open("POST",
            Mscrm.CrmUri.create("/Tools/PersonalSettings/cmds/cmd_updateOutlookSyncFilters.aspx?" +
                GetTokenAsQS(Mscrm.CrmUri.create("/Tools/PersonalSettings/cmds/cmd_updateOutlookSyncFilters.aspx")))
            .toString(),
            false);
...
function getABPSyncFilterSetting() {
    ...
    null != contactAbpRadio &&
        contactAbpRadio.value != iInitialContactAbpOption &&
...
function updatePrimaryClient() {
    ...
    if (null != ckSetSynchronizingClient && ckSetSynchronizingClient.checked != ckSetSynchronizingClient.defaultChecked
        ...
        xmlhttpPrimaryClient.open("POST",
            Mscrm.CrmUri.create("/Tools/PersonalSettings/cmds/cmd_updatePrimaryClient.aspx?" +
                GetTokenAsQS(Mscrm.CrmUri.create("/Tools/PersonalSettings/cmds/cmd_updatePrimaryClient.aspx")))
            .toString(),
            false);
...
function updateHomeSubarea() {
    ...
    if (homearea == "<Default>") {
        SelectSubArea.AddOptionHtml('<option selected="selected" value="<Default>">' +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_HOME_PAGE_SUB_AREA_DEFAULT) +
            "</option>");
...
function loadRegKeySettings() {
    ...
    if (null != runOutlookScheduledSync && !runOutlookScheduledSync.get_disabled()) {
        ...
        Mscrm.FormControlInputBehavior.GetBehavior("numPeriodBetweenOutlookScheduledSync")
            .set_disabled(!runOutlookScheduledSync.get_dataValue())
    }
    var runOfflineScheduledSync = Mscrm.FormControlInputBehavior.GetBehavior("ckRunOfflineScheduledSync");
    if (null != runOfflineScheduledSync && !runOfflineScheduledSync.get_disabled()) {
        runOfflineScheduledSync.set_dataValue(crmRcUtil.RunOfflineScheduledSync);
        Mscrm.FormControlInputBehavior.GetBehavior("numPeriodBetweenOfflineScheduledSync")
            .set_disabled(!runOfflineScheduledSync.get_dataValue())
    }
    var runABPScheduledSync = Mscrm.FormControlInputBehavior.GetBehavior("ckRunABPScheduledSync");
    if (null != runABPScheduledSync && !runABPScheduledSync.get_disabled()) {
        runABPScheduledSync.set_dataValue(crmRcUtil.RunABPScheduledSync);
        Mscrm.FormControlInputBehavior.GetBehavior("numPeriodBetweenABPScheduledSync")
            .set_disabled(!runABPScheduledSync.get_dataValue())
    }
    var sqmAccept = document.getElementById("ckSQMAccept");
    if (null != sqmAccept)
...
function CreateNewPersonalTemplate() {
    ...
    Xrm.Internal.openDialog(Mscrm.CrmUri
        .create("/Tools/EmailTemplateEditor/Dialogs/emailtemplateproperties.aspx?isPersonal=1").toString(),
        options,
        window,
        null,
        null)
...
function openRIConfiguUi() {
    window.open("/_static/tools/RelationshipIntelligenceConfig/UnifiedConfig.html?ispersonal=true",
        "_blank",
        "toolbar=0,location=0,menubar=0,resizable=1,width=966,height=628")
}
