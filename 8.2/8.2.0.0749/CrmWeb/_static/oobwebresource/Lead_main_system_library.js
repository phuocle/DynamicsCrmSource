...
Mscrm.LeadCommandActions.qualifyLeadQuick = function() { Mscrm.LeadCommandActions.qualifyLeadAs(-1) };
Mscrm.LeadCommandActions.qualifyLeadAs = function(qualifyStatus) {
    ...
    if (Xrm.Utility.isMocaOffline())
        if (Xrm.Page.data.entity.getIsDirty())
            Xrm.Page.data.save().then(function() { Mscrm.LeadCommandActions.$7(qualifyStatus) },
                function($p1_0) {
                    Xrm.Internal.openErrorDialog($p1_0.errorCode, Xrm.Internal.getErrorMessage($p1_0.errorCode))
                });
        else Mscrm.LeadCommandActions.$7(qualifyStatus);
    else
        Xrm.Page.data.save().then(function() { Mscrm.LeadCommandActions.$7(qualifyStatus) },
            function($p1_0) {
                Xrm.Internal.openErrorDialog($p1_0.errorCode, Xrm.Internal.getErrorMessage($p1_0.errorCode))
...
Mscrm.LeadCommandActions.disqualifyLeadQuick = function() { Mscrm.LeadCommandActions.disqualifyLeadAs(-1) };
...
Mscrm.LeadCommandActions.$9 = function($p0, $p1, $p2, $p3, $p4, $p5) {
    ...
    if (Xrm.Page.data.entity.getIsDirty())
        Xrm.Page.data.save().then(function() {
            ...
            function($p1_0) {
                Xrm.Internal.openErrorDialog($p1_0.errorCode,
                    Xrm.Internal.getErrorMessage($p1_0
                        .errorCode))
            });
    else Mscrm.LeadCommandActions.$0($p0, $p1, true)
};
Mscrm.LeadCommandActions.getTransactionCurrencyId = function(succeedCallback, errorCallback) {
    ...
    var $v_1 = Xrm.Page.context.getUserId(), $v_2 = ["transactioncurrencyid", "organizationid"];
...
Mscrm.LeadCommandActions.$4 = function($p0) {
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0)) {
        ...
        if (!Mscrm.InternalUtilities.JSTypes.isNull($v_1) && $v_1.length > 0) return $v_1[0].name
    }
    return ""
};
Mscrm.LeadCommandActions.dupWarningOnLoadHandler = function() {
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull(Mscrm.InternalUtilities.DialogUtility
        .getAttributeValue("parentAccountId"))) {
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull(Mscrm.InternalUtilities.DialogUtility
        .getAttributeValue("parentContactId"))) {
...
Mscrm.LeadCommandActions.dupWarningOnOkClickHandler = function() {
    ...
    var $v_0, $v_1 = Xrm.Page.data.entity.attributes.get("parentContactLookup_id");
    $v_0 = $v_1.getValue();
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0)) {
        Mscrm.InternalUtilities.DialogUtility.setAttributeValue("parentContactId", $v_0[0].id);
        Mscrm.InternalUtilities.DialogUtility.setAttributeValue("parentContactName", $v_0[0].name)
    }
    var $v_2 = Xrm.Page.data.entity.attributes.get("parentAccountLookup_id");
    $v_0 = $v_2.getValue();
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0)) {
        Mscrm.InternalUtilities.DialogUtility.setAttributeValue("parentAccountId", $v_0[0].id);
        Mscrm.InternalUtilities.DialogUtility.setAttributeValue("parentAccountName", $v_0[0].name)
    }
    Xrm.Page.ui.close()
};
Mscrm.LeadCommandActions.closeDupWarningDialogCallback = function(dialogParams, callbackParams) {
    if (!Mscrm.InternalUtilities.JSTypes.isNull(dialogParams) && dialogParams["lastButtonClicked"] === "ok_id") {
        var $v_0 = parseInt(callbackParams["qualifyStatus"].toString(), 10),
            ...
            $v_2 = !Mscrm.InternalUtilities.JSTypes.isNull(dialogParams["parentAccountId"])
                ? dialogParams["parentAccountId"].toString()
                : null,
            $v_3 = !Mscrm.InternalUtilities.JSTypes.isNull(dialogParams["parentContactId"])
                ? dialogParams["parentContactId"].toString()
                : null,
            $v_4 = !Mscrm.InternalUtilities.JSTypes.isNull(dialogParams["parentAccountName"])
                ? dialogParams["parentAccountName"].toString()
                : null,
            $v_5 = !Mscrm.InternalUtilities.JSTypes.isNull(dialogParams["parentContactName"])
                ? dialogParams["parentContactName"].toString()
                : null;
        Mscrm.LeadCommandActions.$9($v_0, $v_1, $v_2, $v_3, $v_4, $v_5)
    }
};
Mscrm.LeadCommandActions.$0 = function($p0, $p1, $p2) {
    ...
    if ($v_5 && $v_4) {
        $v_7 = false;
        $v_8 = false;
        $v_B = new Xrm.Objects.EntityReference("account",
            new Microsoft.Crm.Client.Core.Framework.Guid($v_0),
            Mscrm.LeadCommandActions.$4("parentAccountId"))
    } else if ($v_5) {
        $v_7 = false;
        $v_8 = true
    } else if ($v_4)
        if (!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString($v_6)) {
            ...
            $v_7 = false;
            $v_8 = false;
            $v_B = new Xrm.Objects.EntityReference("contact",
                new Microsoft.Crm.Client.Core.Framework.Guid($v_1),
                Mscrm.LeadCommandActions.$4("parentContactId"))
        }
    var $v_F = new Xrm.Objects.EntityReference("lead",
            new Microsoft.Crm.Client.Core.Framework.Guid(Xrm.Page.data.entity.getId())),
        $v_G = Mscrm.InternalUtilities._String.isNullOrWhiteSpace($p1)
            ? null
            : new Xrm.Objects.EntityReference("lead",
                new Microsoft.Crm.Client.Core.Framework.Guid($p1),
                Mscrm.LeadCommandActions.$4("transactioncurrencyid")),
        $v_H = Mscrm.InternalUtilities._String.isNullOrWhiteSpace($v_A)
            ? null
            : new Xrm.Objects.EntityReference("campaign", new Microsoft.Crm.Client.Core.Framework.Guid($v_A));
    if (Xrm.Utility
        .isMocaOffline() &&
        Mscrm.InternalUtilities.JSTypes.isNull($v_G)) $v_G = Xrm.Utility.getDefaultTransactionCurrency();
    var $v_I = function($p1_0) {
        var $v_K = -2147220685;
        if ($p1_0
            .get_organizationServiceFault() &&
            $p1_0.get_organizationServiceFault()
            .get_errorCode() ===
            $v_K) Mscrm.LeadCommandActions.$A($v_0, $v_1, $v_2, $v_3, $p0, $p1);
        else Mscrm.InternalUtilities.ClientApiUtility.actionFailedCallback($p1_0)
    };
    if (Xrm.Utility.isMocaOffline()) {
        var $v_L = null, $v_M = null, $v_N = null;
        try {
        ...
        if (!Mscrm.InternalUtilities.JSTypes.isNull($v_M) && !Mscrm.InternalUtilities.JSTypes.isNull($v_N)) {
            var $v_W = $v_M.getId();
            $v_O = $v_N.getId();
            if (!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString($v_Q)) $v_P = $v_W + $v_Q;
            else $v_P = $v_W + "," + $v_O
        }
        $p0 = 3;
        $p2 = true;
        ...
        var $v_S = function($p1_0) {
            ...
            if ($v_X) {
                ...
                Xrm.Utility.openEntityForm("opportunity".toLowerCase(),
                    $v_X["opportunityId".toLowerCase()].toString(),
                    {})
            }
        };
...
Mscrm.LeadCommandActions.$7 = function($p0) {
    Mscrm.LeadCommandActions
        .getTransactionCurrencyId(function($p1_0) { Mscrm.LeadCommandActions.$0($p0, $p1_0, false) },
...
Mscrm.LeadCommandActions.$6 = function($p0) {
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0)) {
        ...
        if (!Mscrm.InternalUtilities.JSTypes.isNull($v_2)) return $v_2[0]
    }
...
Mscrm.LeadCommandActions.$5 = function($p0) {
    var $v_0 = null;
    if (!Mscrm.InternalUtilities.JSTypes.isNull($p0)) $v_0 = $p0.id;
    return $v_0
};
Mscrm.LeadCommandActions.$2 = function($p0, $p1, $p2, $p3, $p4) {
    var $v_0 = Xrm.Page.getAttribute($p0);
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0) && !Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString($p2)) {
        if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0.getValue())) if ($v_0.getValue()[0].id === $p2) return;
        var $v_1 = new Xrm.LookupObject;
        $v_1.entityType = $p3;
        $v_1.id = $p2;
        $v_1.name = $p1;
...
Mscrm.LeadGridCommandActions.qualifyLeadAs = function(gridControl, records, entityTypeCode, statusCode) {
        ...
        if (Xrm.Utility.isMocaOffline())
            for (var $$arr_6 = records, $$len_7 = $$arr_6.length, $$idx_8 = 0; $$idx_8 < $$len_7; ++$$idx_8) {
                var $v_2$9 = $$arr_6[$$idx_8],
                    $v_3 = new Xrm.Objects.EntityReference("lead",
                        new Microsoft.Crm.Client.Core.Framework.Guid($v_2$9.Id)),
                    $v_4 = function($p1_0) {
                        $v_0++;
                        Mscrm.LeadGridCommandActions.$0($p1_0, statusCode, $v_2$9, $v_0, gridControl, records)
                    };
                Xrm.Utility.retrieveEntityRecord($v_3,
                    $v_1,
                    $v_4,
            ...
            for (var $$arr_D = records, $$len_E = $$arr_D.length, $$idx_F = 0; $$idx_F < $$len_E; ++$$idx_F) {
                ...
                Xrm.Internal.messages.retrieve("lead", $v_5.Id, $v_1).then(function($p1_0) {
                        $v_0++;
                        var $v_6 = $p1_0.entity, $v_7 = new Mscrm.InternalUtilities.EntityReference;
    ...
    } else Mscrm.LeadGridCommandActions.$8(gridControl, records, entityTypeCode, true, statusCode)
};
Mscrm.LeadGridCommandActions.$0 = function($p0, $p1, $p2, $p3, $p4, $p5) {
    ...
    if (Xrm.Utility
        .isMocaOffline() &&
        Mscrm.InternalUtilities.JSTypes.isNull($v_4)) $v_4 = Xrm.Utility.getDefaultTransactionCurrency();
    if ($p0.hasValue("companyname") && $p0.getValue("companyname")) $v_9 = $p0.getValue("companyname").toString();
    if (!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString($v_9)) $v_0 = true;
    ...
    if (Xrm.Utility.isMocaOffline()) {
        var $v_A = null, $v_B = null, $v_C = null;
        ...
        Xrm.Utility.executeNonCudCommand("QualifyLead",
            "lead",
            $v_F,
            $v_G,
            Mscrm.InternalUtilities.DialogUtility.actionFailedCallbackForMoca)
...
Mscrm.LeadGridCommandActions.disqualifyLeadAs = function(gridControl, records, entityTypeCode, statusCode) {
    if (Mscrm.InternalUtilities.DialogUtility.isMDDEnabled()) {
        ...
        if (Xrm.Utility.isMocaOffline())
            for (var $$arr_5 = records, $$len_6 = $$arr_5.length, $$idx_7 = 0; $$idx_7 < $$len_6; ++$$idx_7) {
                var $v_1 = $$arr_5[$$idx_7],
                    $v_2 = new Xrm.Objects.EntityReference("lead",
                        new Microsoft.Crm.Client.Core.Framework.Guid($v_1.Id)),
    ...
    } else Mscrm.LeadGridCommandActions.$8(gridControl, records, entityTypeCode, false, statusCode)
};
Mscrm.LeadGridCommandActions.$8 = function($p0, $p1, $p2, $p3, $p4) {
    ...
    Mscrm.InternalUtilities.GridUtilities
        .executeStandardAction($v_0, $p1, 400, 200, Mscrm.GridCommandActions.createRefreshGridCallback($p0))
};
Mscrm.LeadMainSystemLibraryWebResource = function() {};
Mscrm.LeadMainSystemLibraryWebResource.formOnload = function() { Mscrm.LeadMainSystemLibraryWebResource.$D() };
Mscrm.LeadMainSystemLibraryWebResource.form_OnSave = function() {
    window.setTimeout(function() { Mscrm.LeadMainSystemLibraryWebResource.$C() }, 10)
};
Mscrm.LeadMainSystemLibraryWebResource.$D = function() {
    var $v_0 = Xrm.Page.getAttribute("parentcontactid");
    if (!Mscrm.InternalUtilities.JSTypes.isNull($v_0)) {
        Mscrm.LeadMainSystemLibraryWebResource.$3 = function($p1_0) { Mscrm.LeadMainSystemLibraryWebResource.$B() };
        $v_0.addOnChange(Mscrm.LeadMainSystemLibraryWebResource.$3)
    }
};
Mscrm.LeadMainSystemLibraryWebResource.$B = function() { Mscrm.LeadMainSystemLibraryWebResource.$1 = true };
