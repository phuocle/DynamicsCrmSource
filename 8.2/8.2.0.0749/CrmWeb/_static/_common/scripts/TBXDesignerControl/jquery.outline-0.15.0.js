...
(function() {
    ...
    function Outline(canvas, options, viewport) {
        ...
        var find = settings.zoomOutDiv === window
            ? function(selector) { return $(selector); }
            : function(selector) { return $zoomport.find(selector); };
        ...
        function drawRect(rect, strokeWidth, strokeStyle, fillStyle, invert) {
            ...
            var viewPortLeft = ((rect.left * settings.miniMapTileWidthRatio) / zoomTransformRatio) +
                settings.miniMapViewPortLeftPadding;
            ...
            if (fillStyle) {
                ...
                if (invert) {
                    ...
                    if (drawViewportFlag) {
                        ...
                        context.rect(rect.left / zoomTransformRatio,
                            rect.top / zoomTransformRatio,
                            rect.width,
                            rect.height);
                    } else {
            ...
            if (strokeStyle) {
                ...
                context
                    .lineWidth = zoomOutcurrentScale ? Math.max(strokeWidth, 0.2 / zoomOutcurrentScale) : strokeWidth;
        ...
        function drawElement(element, strokeWidth, strokeStyle, fillStyle) {
            ...
            if ($element[0].tagName == "SPAN") {
                ...
                $.each(processComponentsArray,
                    function(i) {
                        if ($element[0].innerHTML.indexOf(processComponentsArray[i]) > -1) {
                            var selectorvalue = "." + processComponentsArray[i];
                            var SymbolFontIconValue = window
                                .getComputedStyle(document.querySelector(selectorvalue), ':before')
                                .getPropertyValue('content').replace(/"/g, '');

                            // Set the left value of the tile icon depending on the Org language direction
                            if (window.LOCID_UI_DIR == "RTL")
                                context.fillText(SymbolFontIconValue,
                                    (rect.left / zoomTransformRatio) + (rect.width / 2) - (fontIconSize / 2) + 15,
                                    (rect.top / zoomTransformRatio) + (rect.height / 2) + (fontIconSize / 2));
                            else
                                context.fillText(SymbolFontIconValue,
                                    (rect.left / zoomTransformRatio) + (rect.width / 2) - (fontIconSize / 2),
                                    (rect.top / zoomTransformRatio) + (rect.height / 2) + (fontIconSize / 2));
                        }
                    });
            }
        ...
        function drawViewport() {
            ...
            drawRect(currentViewportRect,
                style.strokeWidth,
                style.strokeStyle,
                style.fillStyle,
                settings.invertViewport);
        ...
        function draw() {
            ...
            if (settings.crop) {
                $canvas.attr('width', currentContentRect.width * currentScale)
                    .attr('height', currentContentRect.height * currentScale);
            }
        ...
        function onDragStart(event) {
            ...
            if (settings.autoFocus) {
                ...
                focusWidth = (((event.pageX - r.left) / currentScale) - currentViewportRect.left) /
                    currentViewportRect.width;
                focusHeight = (((event.pageY - r.top) / currentScale) - currentViewportRect.top) /
