...
function OpenPerformanceUI(refresh) {
    ...
    if (refresh) {
        ...
        if (!mainWnd.FinishDataBindingTimestamp ||
            typeof _IsRefreshForm != "undefined" &&
            _IsRefreshForm == "1" &&
            !mainWnd.FinishInlineEditInitializerTimestamp)
            waitForMarkers = true;
        ...
        if (AfterWindowOpenTimestamp &&
            BeforeWindowOpenTimestamp &&
            (typeof _IsRefreshForm == "undefined" ||
                _IsRefreshForm != "1" ||
                mainWnd.FinishInlineEditInitializerTimestamp)) {
            addToPerfContent("--------------------------------------------------------------------------------<br/>");
            ...
            openWindowTime > 0 &&
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to open the window:\t" + openWindowTime + " ms<br/>");
            ...
            if (mainWnd.REQ_ID) {
                var dataAspxResults = getJson("/Tools/Diagnostics/diag.aspx/GetPerformanceMarkers",
                    "{'markersCacheId':'" + mainWnd.REQ_ID + "'}");
                addToPerfContent("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Page.aspx server queuing time:\t" +
                    (dataAspxResults.HttpContextTimestamp - dataAspxResults.RequestArrivalTime) +
                    " ms<br/>");
                addToPerfContent("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Page.aspx server exec time:\t" +
                    (dataAspxResults.OnEndRequest - dataAspxResults.HttpContextTimestamp) +
                    " ms<br/>")
            }
            if (mainWnd.dataReqId) {
                var dataAspxResults = getJson("/Tools/Diagnostics/diag.aspx/GetPerformanceMarkers",
                    "{'markersCacheId':'" + mainWnd.dataReqId + "'}");
                addToPerfContent("&nbsp;&nbsp;&nbsp; data.aspx server queuing time:\t" +
                    (dataAspxResults.HttpContextTimestamp - dataAspxResults.RequestArrivalTime) +
                    " ms<br/>");
                addToPerfContent("&nbsp;&nbsp;&nbsp; data.aspx server exec time:\t" +
                    (dataAspxResults.OnEndRequest - dataAspxResults.HttpContextTimestamp) +
                    " ms<br/>")
            }
            ...
            if (typeof _IsRefreshForm != "undefined" && _IsRefreshForm == "1") {
                ...
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to bind form data using layout template:\t" +
                    dataBindingTime +
                    " ms<br/>");
                var ScriptBlocksTime = mainWnd.FinishLoadingScriptBlocksTimestamp -
                    mainWnd.StartLoadingScriptBlocksTimestamp;
                ...
                var BrowserScriptProcessingTime = mainWnd.StartRefreshInitializationTimestamp -
                    mainWnd.FinishLoadFormLayoutTimestamp;
                BrowserScriptProcessingTime > 0 &&
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time taken by browser to call Refresh Page Initialize:\t" +
                        BrowserScriptProcessingTime +
                        " ms<br/>");
                var LoadScriptTime = mainWnd
                    .StartRefreshInitializationTimestamp -
                    mainWnd.GlobalInitializeReadFormBegin;
                LoadScriptTime > 0 &&
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time taken by browser to load page Scripts:\t" +
                        LoadScriptTime +
                        " ms<br/>");
                var TimeTakenByBrowserForCallback = mainWnd.GlobalInitializeReadFormBegin -
                    mainWnd.HideLoadingScreenTimestamp;
                TimeTakenByBrowserForCallback > 0 &&
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time taken by browser for browser callback\t" +
                        TimeTakenByBrowserForCallback +
                        " ms<br/>");
                ...
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to load Dynamic scripts:\t" +
                    DynamicScriptsTime +
                    " ms<br/>");
                var ScriptBlocksTime = mainWnd.FinishLoadingScriptBlocksTimestamp -
                    mainWnd.StartLoadingScriptBlocksTimestamp;
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to load script blocks/application init statements:\t" +
                    ScriptBlocksTime +
                    " ms<br/>");
                ...
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to initialize link control:\t" +
                    LinkControlTime +
                    " ms<br/>");
                var dataBindingTime2 = mainWnd.FinishDataBindingTimestamp - mainWnd.StartDataBindingTimestamp;
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to bind activity Feeds and ProcessControl:\t" +
                    dataBindingTime2 +
                    " ms<br/>");
                var ProcessControlInitializerTime = mainWnd.FinishProcessControlInitializerTimestamp -
                    mainWnd.StartProcessControlInitializerTimestamp;
                ProcessControlInitializerTime > 0 &&
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to initialize process control:\t" +
                        ProcessControlInitializerTime +
                        " ms<br/>");
                var FinalizeProcessControlTime = mainWnd.FinishFinalizeProcessControlTimestamp -
                    mainWnd.StartFinalizeProcessControlTimestamp;
                addToPerfContent("&nbsp;&nbsp;&nbsp; Time to finalize Process Control:\t" +
                    FinalizeProcessControlTime +
                    " ms<br/>");
                mainWnd.MediaQueryInitializationTime &&
                    addToPerfContent("mediaQuery initialization time: \t" +
                        mainWnd.MediaQueryInitializationTime +
                        "ms<br/>");
                mainWnd.MediaQueryExecutionTime &&
                    addToPerfContent("mediaQuery execution time: \t" +
                        mainWnd.MediaQueryExecutionTime.join(", ") +
                        "ms<br/>");
                mainWnd.MediaQueryApplyTime &&
                    addToPerfContent("mediaQuery apply time: \t" + mainWnd.MediaQueryApplyTime.join(", ") + "ms<br/>");
                ...
                var WallLoadFirstTime = mainWnd.FinishWallHtmlLoadTimestamp - mainWnd.StartWallHtmlLoadTimestamp,
                    WallLoadRefreshTime = mainWnd
                        .FinishWallRefreshLoadTimestamp -
                        mainWnd.StartWallRefreshLoadTimestamp,
                    TotalWallLoadTime = WallLoadFirstTime + WallLoadRefreshTime;
                TotalWallLoadTime &&
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to load Activity Wall:\t" +
                        TotalWallLoadTime +
                        " ms<br/>");
                ...
                if (mainWnd.FinishSubjectLookupRetrieveTimestamp && mainWnd.StartSubjectLookupRetrieveTimestamp) {
                    var SubjectLookupTime = mainWnd.FinishSubjectLookupRetrieveTimestamp -
                        mainWnd.StartSubjectLookupRetrieveTimestamp;
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to retrieve subject lookup:\t" +
                        SubjectLookupTime +
                        " ms<br/>")
                }
                if (mainWnd.FinishSubjectLookupShowTimestamp && mainWnd.StartSubjectLookupShowTimestamp) {
                    var SubjectLookupShowTime = mainWnd.FinishSubjectLookupShowTimestamp -
                        mainWnd.StartSubjectLookupShowTimestamp;
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to show subject lookup:\t" +
                        SubjectLookupShowTime +
                        " ms<br/>")
                }
                ...
                if (mainWnd.FinishLoadingNewProcessStageTimestamp && mainWnd.StartLoadingNewProcessStageTimestamp) {
                    var ProcessStageChangeTime = mainWnd.FinishLoadingNewProcessStageTimestamp -
                        mainWnd.StartLoadingNewProcessStageTimestamp;
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to change process stage:\t" +
                        ProcessStageChangeTime +
                        " ms<br/>")
                }
                if (mainWnd.FinishLoadingProcessStageBackTimestamp && mainWnd.StartLoadingProcessStageBackTimestamp) {
                    var ProcessBackChangeTime = mainWnd.FinishLoadingProcessStageBackTimestamp -
                        mainWnd.StartLoadingProcessStageBackTimestamp;
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to go back process stage:\t" +
                        ProcessBackChangeTime +
                        " ms<br/>")
                }
                if (mainWnd.FinishLoadingProcessStageAdvanceTimestamp &&
                    mainWnd.StartLoadingProcessStageAdvanceTimestamp) {
                    var ProcessAdvanceChangeTime = mainWnd.FinishLoadingProcessStageAdvanceTimestamp -
                        mainWnd.StartLoadingProcessStageAdvanceTimestamp;
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to advance process stage:\t" +
                        ProcessAdvanceChangeTime +
                        " ms<br/>")
                }
                if (!isNavQuickCreate() && !window._globalQuickCreate) {
                    var CommandBarLayoutLoadTime = Mscrm.AggregatePerformanceMarkers
                        .getMarkerValue("CommandBarLayoutLoadTime", Mscrm.PerformanceMarkerAggregateType.sum);
                    addToPerfContent("&nbsp;&nbsp;&nbsp; Time to load commandbar layout:\t" +
                        CommandBarLayoutLoadTime +
                        " ms<br/>");
                    if (mainWnd.top.CommandBarUnloadEndTime && mainWnd.top.CommandBarUnloadStartTime) {
                        var CommandBarUnLoadTime = mainWnd.top.CommandBarUnloadEndTime -
                            mainWnd.top.CommandBarUnloadStartTime;
                        if (CommandBarUnLoadTime < 1)
                            addToPerfContent("&nbsp;&nbsp;&nbsp; Time to unload commandbar:\t< 1 ms<br/>");
                        else
                            addToPerfContent("&nbsp;&nbsp;&nbsp; Time to unload commandbar:\t" +
                                CommandBarUnLoadTime +
                                " ms<br/>")
                    }
                    if (mainWnd.top.CommandBarLoadEndTime && mainWnd.top.CommandBarLoadStartTime) {
                        var unloadTimeToSubtract = 0;
                        if (mainWnd.top.CommandBarUnloadStartTime >= mainWnd.top.CommandBarLoadStartTime &&
                            mainWnd.top.CommandBarUnloadEndTime <= mainWnd.top.CommandBarLoadEndTime)
                            unloadTimeToSubtract = mainWnd.top.CommandBarUnloadEndTime -
                                mainWnd.top.CommandBarUnloadStartTime;
                        var CommandBarLoadTime = mainWnd.top.CommandBarLoadEndTime -
                            mainWnd.top.CommandBarLoadStartTime -
                            unloadTimeToSubtract;
                        addToPerfContent("&nbsp;&nbsp;&nbsp; Time to Load commandbar:\t" +
                            CommandBarLoadTime +
                            " ms<br/>")
                    }
                }
        ...
        if (!isGridHomePage) {
            ...
            if (!innerFrame ||
                !innerFrame.contentWindow.PostRenderTime ||
                fullyLoadTimeExpected && !mainWnd.FullyLoadedTime ||
                !mainWnd.LoadStartTime) {
            ...
            var indent = _perfDivIndent,
                ...
                getPerformanceMarkersUri = Mscrm.CrmUri.create("/Tools/Diagnostics/diag.aspx").toString() +
                    "/GetPerformanceMarkers";
            ...
            addToPerfContent(indent +
                "Main.aspx server queuing time:\t" +
                (_perfMainAspxResults.HttpContextTimestamp - _perfMainAspxResults.RequestArrivalTime) +
                " ms<br/>");
            addToPerfContent(indent +
                "Main.aspx server exec time:\t" +
                (_perfMainAspxResults.OnEndRequest - _perfMainAspxResults.HttpContextTimestamp) +
                " ms<br/>");
            addToPerfContent(indent +
                "Edit.aspx server queuing time:\t" +
                (_perfInnerAspxResults.HttpContextTimestamp - _perfInnerAspxResults.RequestArrivalTime) +
                " ms<br/>");
            addToPerfContent(indent +
                "Edit.aspx server exec time:\t" +
                (_perfInnerAspxResults.OnEndRequest - _perfInnerAspxResults.HttpContextTimestamp) +
                " ms<br/>");
            ...
            hasOpenerWnd &&
                openerWnd.PreloadStartTime &&
                openerWnd.PreloadEndTime &&
                addToPerfContent("Time to preload Edit.aspx:\t" +
                    (openerWnd.PreloadEndTime - openerWnd.PreloadStartTime) +
                    " ms<br/>");
            ...
            pltError > 0 &&
                addToPerfContent(indent + "Potential calculation error:\t&plusmn; " + pltError + " ms<br/>");
            ...
            if (innerWnd.PostRenderTime < loadStartTime)
                ...
                if (_perfInnerAspxResults) {
                    var queuingTime = _perfInnerAspxResults.HttpContextTimestamp -
                        _perfInnerAspxResults.RequestArrivalTime;
                    queuingTime > 0 &&
                        addToPerfContent(indent + "Grid request server queuing time:\t" + queuingTime + " ms<br/>");
                    addToPerfContent(indent +
                        "Grid request server exec time:\t" +
                        (_perfInnerAspxResults.OnEndRequest - _perfInnerAspxResults.HttpContextTimestamp) +
                        " ms<br/>")
                }
            }
...
function retrieveMarkersFromOpener() {
    ...
    if (mainWnd.opener && mainWnd.opener.getWindowOpenTimeJson)
        try {
            var windowLoadTime = Sys.Serialization.JavaScriptSerializer
                .deserialize(mainWnd.opener.getWindowOpenTimeJson(mainWnd.name, true));
...
function AddActionQueuePerfInfoToContent(wnd, loadStartTime) {
    var actionQueuePerfMarkers = wnd._actionQueuePerfMarkers,
        actionQueueInfoHtml = "";
    actionQueueInfoHtml +=
        "<div style='height:100px;overflow-y:scroll;border-style:solid;border-width:1px;background-color:#ddd;'>";
    ...
    if (wnd.InitActionQueueCompletedTime > loadStartTime)
        wnd.InitActionQueueCompletedTime &&
            addToPerfContent("Total Init Queue Time:\t" +
                (wnd.InitActionQueueCompletedTime - loadStartTime) +
                " ms<br/>");
    else
...
function ShowPerfReport(show) {
    ...
    if (show) {
        ...
        var closeAction = "window.top.document.getElementById('" +
            currentContentId +
            "').contentWindow.RemovePerfReportDiv()";
        if (isRefresh)
            perfUpperActions.innerHTML = "<font color='blue'><a href='#' onclick=" +
                closeAction +
                "><u>close</u></a></font>";
        else if (window.location.href.indexOf("/homepage.aspx") != -1)
            perfUpperActions.innerHTML = "<font color='blue'><a href='#' onclick=" +
                closeAction +
                "><u>close</u></a></font>";
        else
            perfUpperActions.innerHTML = "<font color='blue'><a href='#' onclick='window." +
                currentContentId +
                ".RemovePerfReportDiv()'><u>close</u></a></font>";
        if (window.clipboardData) {
            perfInnerActions.innerHTML = "<font color='blue'>";
            if (!isRefresh)
                if (window.location.href.indexOf("/homepage.aspx") != -1) {
                    perfInnerActions.innerHTML += "<a href='#' onclick='window." +
                        currentContentId +
                        ".copyPerfResults()'><u>copy</u></a>";
                    perfInnerActions.innerHTML += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                    perfInnerActions.innerHTML += "<a href='#' onclick='window." +
                        currentContentId +
                        ".copyPerfReport()'><u>copy report</u></a>"
                } else {
                    perfInnerActions.innerHTML += "<a href='#' onclick='window." +
                        currentContentId +
                        ".copyPerfResults()'><u>copy</u></a>";
        ...
        if (isRefresh)
            ...
            perfUpperActions.innerHTML = "Perf: <font color='blue'><a href='#' onclick='window." +
                currentContentId +
                ".ShowPerfReport(true)'><u>show</u></a>"
    }
}
...
function getContentIframe() {
    if (window.top.document.getElementById("crmContentPanel").getAttribute("currentContentId"))
        return window.top.document.getElementById(window.top.document.getElementById("crmContentPanel")
