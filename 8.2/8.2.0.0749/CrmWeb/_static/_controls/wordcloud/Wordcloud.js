...
var Wordcloud = function() {
    ...
    Wordcloud.prototype.drawOneWord = function(index, word, data) {
        ...
        if (word.offsetLeft < 10 ||
            word.offsetTop < 20 ||
            word.offsetLeft + word.offsetWidth > data.container.width - 10 ||
            word.offsetTop + word.offsetHeight > data.container.height - 20) {
    ...
    Wordcloud.prototype.processWords = function(data) {
        return this.resultArray.map(function(val, ind) {
            return {
                ...
                trend: val.trend,
                opacity: val.opacity,
                processWeight: val.processWeight,
                color: val.color,
                width: WordcloudWord.getPercentage(val.offsetWidth, data.container.width),
                height: WordcloudWord.getPercentage(val.offsetHeight, data.container.height),
                left: WordcloudWord.getPercentage(val.offsetLeft, data.container.width),
                top: WordcloudWord.getPercentage(val.offsetTop, data.container.height),
                orig: val.orig || val,
        ...
        this.wordsArray.forEach(function(val, index) {
            ...
            if (_this.colors)
                if (_this.colors.length > 1)
                    val.color = val.processWeight === 5
                        ? _this.colors[0]
                        : _this.colors[index % _this.colors.length - 1];
                else
                    val.color = _this.colors[0]
        });
        return {
            series: this.processWords(data),
            data: { data: data, wordsArray: this.wordsArray, resultsArray: this.resultArray }
        }
    };
    return Wordcloud
}()