...
var WordcloudWord = function() {
    ...
    WordcloudWord.determineWordWeight = function(word, arr) {
        if (arr[0].weight > arr[arr.length - 1].weight)
            return Math.round((word.weight - arr[arr.length - 1].weight) /
                    (arr[0].weight - arr[arr.length - 1].weight) *
                    9) +
                1;
        return 10
    };
    WordcloudWord.areOverlapping = function(el1, el2) {
        var isWidthOverlapping = Math.abs(el1.offsetLeft * 2 + el1.offsetWidth - el2.offsetLeft * 2 - el2.offsetWidth) <
                el1.offsetWidth + el2.offsetWidth + 10
                ? true
                : false,
            isHeightOverlapping = Math.abs(el1
                    .offsetTop *
                    2 +
                    el1.offsetHeight -
                    el2.offsetTop * 2 -
                    el2.offsetHeight) <
                el1.offsetHeight + el2.offsetHeight + 5
                ? true
                : false;
