...
function isClientApiAvailable() {
    return !Mscrm.InternalUtilities.JSTypes.isNull(Xrm.Page.data) &&
...
function initDates() {
    ...
    if (getAttributeValue(_scheduledstart) == null || getAttributeValue(_scheduledend) == null)
        typeof IS_SCHEDULABLE_ACTIVITY == "undefined" &&
            durationChanged();
    else if (!IsNull(_scheduleddurationminutes) && !IsNull(getAttributeValue(_scheduleddurationminutes)))
        endTimeChanged();
    else
        _minutesInDuration = 30;
    isAllDayEvent() &&
    ...
    window.skipInitialXMLValue &&
        window.setInitialFormXMLValue &&
    ...
    try {
        if (isAllDayEvent()) {
            ...
            if (_oldNonAllDayStartTime == null && _oldNonAllDayEndTime == null) {
                ...
                _oldNonAllDayEndTime = new Date(_oldNonAllDayStartTime.valueOf() +
                    _minutesInDuration * _millisecondsPerMinute)
            }
            if (_oldNonAllDayStartTime == null)
                _oldNonAllDayStartTime = new Date(_oldNonAllDayEndTime.valueOf() -
                    _minutesInDuration * _millisecondsPerMinute);
            ...
            if (getIsAllDay(_scheduledendCtrl)) {
                ...
                if (_oldNonAllDayEndTime.getHours() != 0 ||
                    _oldNonAllDayEndTime.getMinutes() != 0 ||
                    _oldNonAllDayEndTime.valueOf() == oStartTime.valueOf())
            ...
            oEndTime < oStartTime &&
                setAttributeValue(_scheduledend,
                    new Date(getAttributeValue(_scheduledstart)
                        .valueOf() +
                        _minutesInDuration * _millisecondsPerMinute))
        }
...
function setDuration(oDuration, bIsAllDay) {
    ...
    if (bIsAllDay) {
        ...
        if (_workdayStart < _workdayEnd) {
            ...
            var dayStart = new Date(_workdayStart.getFullYear(),
                    _workdayStart.getMonth(),
                    _workdayStart.getDate(),
                    0,
                    0,
                    0,
                    0),
        ...
        if (IsNull(timeEnd) || IsNull(timeStart))
            ...
            var tempDuration = Math.round((timeEnd - timeStart) / _millisecondsPerDay * minutesPerWorkday -
                (timeEnd.getTimezoneOffset() - timeStart.getTimezoneOffset()));
            if (!IsNull(oDuration))
                setAttributeValue(oDuration, tempDuration);
            else
                _minutesInDuration = tempDuration
        }
    } else if (IsNull(timeEnd) || IsNull(timeStart))
        if (!IsNull(oDuration))
            setAttributeValue(oDuration, 0);
        else
            _minutesInDuration = 0;
    else {
        var tempDuration = Math.round((timeEnd - timeStart) / 60 / 1e3 -
            (timeEnd.getTimezoneOffset() - timeStart.getTimezoneOffset()));
        if (!IsNull(oDuration))
            setAttributeValue(oDuration, tempDuration);
        else
...
function startTimeChanged(eventObject) {
    if (isControlDisabled(_scheduledstartCtrl) ||
        isControlDisabled(_scheduledendCtrl) ||
        !IsNull(_scheduleddurationminutesCtrl) && isControlDisabled(_scheduleddurationminutesCtrl))
        return;
    if (_bSuppressEvents)
        return;
    if (getAttributeValue(_scheduledstart) == null || getAttributeValue(_scheduledend) == null)
...
function endTimeChanged(eventObject) {
    if (!IsNull(_scheduledstartCtrl) && isControlDisabled(_scheduledstartCtrl) ||
        !IsNull(_scheduledendCtrl) && isControlDisabled(_scheduledendCtrl) ||
        !IsNull(_scheduleddurationminutesCtrl) && isControlDisabled(_scheduleddurationminutesCtrl))
        return;
    if (_bSuppressEvents)
        return;
    ...
    suppressEvents(true);
    try {
        if (GetScheduledEndValue() < getAttributeValue(_scheduledstart) ||
            isAllDayEvent() && GetScheduledEndValue().toString() == getAttributeValue(_scheduledstart).toString()) {
            if (_isendtimecleared)
                _isendtimecleared = false;
            else
                alert(LOCID_SPECIFY_END_TIME);
            setAttributeValue(_scheduledend,
                new Date(getAttributeValue(_scheduledstart).valueOf() + _minutesInDuration * _millisecondsPerMinute))
        }
        setDuration(_scheduleddurationminutes, isAllDayEvent())
    } catch (e) {
    } finally {
    ...
    try {
        if (getAttributeValue(_scheduledstart) == null)
            if (getAttributeValue(_scheduledend) == null)
                ...
                setAttributeValue(_scheduledstart,
                    new Date(GetScheduledEndValue().valueOf() - _minutesInDuration * _millisecondsPerMinute));
        if (_minutesInDuration < _minutesPerDay && isAllDayEvent()) {
            ...
            _oldNonAllDayEndTime = new Date(getAttributeValue(_scheduledstart).valueOf() +
                _minutesInDuration * _millisecondsPerMinute);
