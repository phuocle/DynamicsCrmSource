...
var _oCalPopUp,
    ...
    _sCalMonths = window.LOCID_ARRAY_SHORT_MONTHS
        ? LOCID_ARRAY_SHORT_MONTHS.split(",")
        : _oDateTimeFormat["AbbreviatedMonthNames"],
    ...
    _sTimeFormats = [
        "h:mm tt", "hh:mm tt", "H:mm", "HH:mm", "h:mm:ss tt", "hh:mm:ss tt", "H:mm:ss", "HH:mm:ss", "tt h:mm",
        "tt hh:mm"
    ],
    ...
    _iCalWidth = typeof USER_SHOW_WEEK_NUMBER != "undefined" && USER_SHOW_WEEK_NUMBER
        ? iShowWeekCalWidth
        : iNoWeekCalWidth,
    _iTodayBarHeight = 18,
    _bDragging = false,
...
function DrawCalendar(dInitDate, iNumMonths, bSelectRange, sOnClick, hiliteCode, dHiliteDay, bPopUp, yearOnly) {
    ...
    if (!IsNull(yearOnly) && yearOnly == true)
        ...
        for (var i = 1; i < iNumMonths - 1; i++) {
            ...
            calendarHTML += DrawMonth(dNextMonthDate,
                sOnClick,
                hiliteCode,
                dHiliteDay,
                false,
...
function DrawPopUpCalendar(dInitDate, iNumMonths, bSelectRange, sOnClick, hiliteCode, dHiliteDay, yearOnly, callback) {
    ...
    temp
        .innerHTML =
        DrawCalendar(dInitDate, iNumMonths, bSelectRange, sOnClick, hiliteCode, dHiliteDay, true, yearOnly);
...
function DrawInlineCalendar(dInitDate, iNumMonths, bSelectRange, sOnClick, oInlineContainer, hiliteCode, dHiliteDay) {
    if (oInlineContainer) {
        ...
        oInlineContainer.innerHTML = DrawCalendar(dInitDate,
            iNumMonths,
            bSelectRange,
            sOnClick,
            hiliteCode,
...
function InitVars(oInputElem, dInitDate, iNumMonths, bSelectRange, oEndDate, sOnClick, hiliteCode, dHiliteDay) {
    ...
    if (_oCalPopUp) {
        var calendarContainer = _oCalPopUp.get_body() && _oCalPopUp.get_body().parentNode
            ? _oCalPopUp.get_body().parentNode
            : null;
        _oCalPopUp.dispose();
        _oCalPopUp = null;
        calendarContainer &&
            calendarContainer.parentNode &&
...
function LaunchCalendar(oContainer,
    oInputElem,
    dInitDate,
    iNumMonths,
    bSelectRange,
...
function LaunchCalendar(oContainer,
    ...
    if (Mscrm.Utilities.isGlobalQuickCreateForm()) {
        var formBodyContainer = document.getElementById("formBodyContainer"),
            calHeight = typeof iCalHeight == "number" ? iCalHeight : !isNaN(iCalHeight) ? parseInt(iCalHeight) : 0,
            renderHeight = (formBodyContainer.scrollHeight > calHeight ? formBodyContainer.scrollHeight : calHeight) +
                "px";
        if (!IsNull(Mscrm.GlobalQuickCreate) && !IsNull(Mscrm.GlobalQuickCreate.GlobalQuickCreateBehavior)) {
            var qcForms = Mscrm.GlobalQuickCreate.GlobalQuickCreateBehavior.allQuickForms();
            for (i = 0; i < qcForms.length; i++) {
                var quickForm = qcForms[i];
                if (!IsNull(oContainer) && !IsNull(quickForm) && oContainer.ownerDocument == quickForm.contentDocument
                ) {
                    quickForm.style.height = renderHeight;
                    break
                }
            }
        }
...
function LaunchInlineCalendar(oInputElem,
    dInitDate,
    iNumMonths,
    bSelectRange,
    oEndDate,
...
function HiliteDateRange(sBeginUTC, sEndUTC) {
    ...
    if (_oCalPopUp)
        DrawPopUpCalendar(ParseUtcDate(ZeroUtcTime(sBeginUTC)),
            _iNumMonths,
            _bSelectRange,
            _sOnClick,
            _hiliteCode,
        ...
        DrawInlineCalendar(ParseUtcDate(ZeroUtcTime(sBeginUTC)),
            _iNumMonths,
            _bSelectRange,
            _sOnClick,
            _oInlineContainer,
    ...
    try {
        for (var iBeginDate = parseInt(sBeginDate, 10),
            ...
            if (isNullOrEmptyString(oTbl.getAttribute("month")))
                continue;
            for (var j = 0,
                iNumRows = oTbl.rows.length;
                j < iNumRows;
                j++)
                for (var k = 0,
                    iNumCells = oTbl.rows[j].cells.length;
                    k < iNumCells;
                    k++) {
...
function DrawMonth(D, sOnClick, hiliteCode, hiliteDay, bShowToday, bShowMonthNav, bSelectRange, bPopUp) {
    ...
    s += formatString(LOCID_CALENDAR_SUMMARY_MASK,
        CrmEncodeDecode.CrmHtmlEncode(Mscrm.DateTimeUtility.formatDate(dInitDate, "y")),
        "");
    ...
    if (typeof USER_SHOW_WEEK_NUMBER != "undefined" && USER_SHOW_WEEK_NUMBER)
        ...
        s += "7";
    s +=
        "'><table cellpadding='0' cellspacing='0' style='table-layout:fixed; width:100%;'><tr class='ms-crm-MiniCal-Nav'>";
    var iValue = dInitDate.valueOf(),
        tabIndexAttribute = isMobile() ? "" : "tabindex='0'";
    if (tmpDate < _dCalMinDate || bShowMonthNav == false)
        s += "<td id='tdPrv' onclick='" +
            sOnClick +
            "' align='center' width='22' style='cursor: default' nav='0' " +
            tabIndexAttribute +
            " onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_PREVIOUS_MONTH) +
            "'>&nbsp;</td>";
    else
        s += "<td id='tdPrv' onclick='" +
            sOnClick +
            "' align='center' width='22' d='" +
            iValue +
            "' nav='1' onmouseover='this.className = \"ms-crm-MiniCal-Nav-Button-Hovered\"' onmouseout='this.className = \"\"' " +
            tabIndexAttribute +
            " onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_PREVIOUS_MONTH) +
            "'><img src=\"/_imgs/ribbon/arrow_left.png\" class='ms-crm-MiniCal'/></td>";
    s += "<td id='tdCaption' onclick='" +
        sOnClick +
        "' align='center' d='" +
        iValue +
        "' nav='2' onmouseover='this.className = \"ms-crm-MiniCal-Nav-Button-Hovered\"' onmouseout='this.className = \"\"' " +
        tabIndexAttribute +
        " onkeydown='HandleKeysForCal(event);'>";
    ...
    if (dLastDay > _dCalMaxDate || bShowMonthNav == false)
        s += "<td id='tdNxt' onclick='" +
            sOnClick +
            "' align='center' width='22' style='cursor: default;' nav='0' " +
            tabIndexAttribute +
            "onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_NEXT_MONTH) +
            "'>&nbsp;</td>";
    else
        s += "<td id='tdNxt' onclick='" +
            sOnClick +
            ...
            "onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_NEXT_MONTH) +
            "'><img src=\"/_imgs/ribbon/arrow_right.png\" class='ms-crm-MiniCal'/></td>";
    s += "</tr></table></td></tr>";
    s += "<tr>";
    var i = 0,
        ii = _iCalStartDay;
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (ii = 0; ii < 7; ii++) {
            ...
            if ((hiliteCode == HILITE_DAY || hiliteCode == HILITE_WEEK || hiliteCode == HILITE_MONTH) &&
                tmpDate.getTime() >= dHiliteStart.getTime() &&
                tmpDate.getTime() <= dHiliteEnd.getTime())
                sClass += dayIsInThisMonth
                    ? " ms-crm-MiniCal-Day-Hilited-Current"
                    : " ms-crm-MiniCal-Day-Hilited-NotCurrent";
    ...
    if (bShowToday == true) {
        var showWeekNumbers = typeof USER_SHOW_WEEK_NUMBER != "undefined" && USER_SHOW_WEEK_NUMBER
            ? USER_SHOW_WEEK_NUMBER
            : false;
...
function newDate(year, month, day, hour, minute, sec) {
    ...
    else if (dayDiff(oDay, oCorrectDay) <= -1)
...
function getTodayBarHtml(iColSpan, sOnClick) {
    var dToday = LocalDateTimeNow(),
        s =
            "<tr class='ms-crm-MiniCal-TodayBar'><td id='tdToday' class='ms-crm-MiniCal-TodayBar' cls='ms-crm-MiniCal-TodayBar' colspan='",
        tabIndexAttribute = isMobile() ? "" : "tabindex='0'";
    s += iColSpan;
    s += "' onclick='" +
        sOnClick +
        "' d='" +
        dToday.valueOf() +
        "' onmouseover='this.className = \"ms-crm-MiniCal-TodayBar-Hovered\"' onmouseout='this.className = \"ms-crm-MiniCal-TodayBar\"' " +
        tabIndexAttribute +
        " onkeydown='HandleKeysForCal(event);'><div class='ms-crm-MiniCal-TodayBar'>";
...
function getWeekNumber(oDate) {
    ...
    else if (oDate.valueOf() < oFirstDayNextYear.valueOf())
...
function DrawYear(D, sOnClick) {
    ...
    s += "'>";
    s +=
        "<tr><td class='ms-crm-MiniCal-Header' colspan='3'><table cellpadding='0' cellspacing='0' style='table-layout:fixed;width:100%'><tr class='ms-crm-MiniCal-Nav'>";
    ...
    if (iYear <= _dCalMinDate.getFullYear())
        ...
        s += "<td id='tdPrv' onclick='" +
            ...
            tabIndexAttribute +
            " onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_PREVIOUS_YEAR) +
            "'><img src=\"/_imgs/ribbon/arrow_left.png\" class='ms-crm-MiniCal'/></td>";
    s += "<td id='tdCaption' align='center' style='cursor:default;' nav='0' " +
        tabIndexAttribute +
        ">" +
        CrmEncodeDecode.CrmHtmlEncode(Mscrm.DateTimeUtility.getCalendarYear(tmpDate)) +
        "</td>";
    if (iYear >= _dCalMaxDate.getFullYear())
        ...
        s += "<td id='tdNxt' onclick='" +
            ...
            tabIndexAttribute +
            " onkeydown='HandleKeysForCal(event);' title ='" +
            CrmEncodeDecode.CrmHtmlEncode(LOCID_CALENDAR_NEXT_YEAR) +
            "'><img src=\"/_imgs/ribbon/arrow_right.png\" class='ms-crm-MiniCal'/></td>";
    s += "</tr></table></td></tr>";
    var iMonth = 0;
    for (i = 0; i < 4; i++) {
        s += "<tr style='height:" +
            (31 + i % 2) +
            "px;' onmouseover='var domEventObject = new Sys.UI.DomEvent(event); domEventObject.target.className += \" ms-crm-MiniCal-Month-Hovered\"' onmouseout='var domEventObject = new Sys.UI.DomEvent(event); domEventObject.target.className = \"ms-crm-MiniCal-Month\"'>";
        for (ii = 0; ii < 3; ii++) {
            iMonth = tmpDate.getMonth();
            iValue = tmpDate.valueOf();
            s += "<td class='ms-crm-MiniCal-Month' onclick='" +
                sOnClick +
                "' d='" +
                iValue +
                "' nav='1' tabindex='0' onkeydown='HandleKeysForCal(event);'>" +
                CrmEncodeDecode.CrmHtmlAttributeEncode(_sCalMonths[iMonth]) +
                "</td>";
...
function ReturnDate(o, yearOnly) {
    ...
    if (o.getAttribute("nav"))
        ...
        if (_oCalPopUp)
            ...
            _oEndDate &&
                _oEndDate.getAttribute("returnValue") &&
...
function FormatUtcDate(D) {
    ...
    return FormatDateTime(dateTimeNoSeconds)
}
