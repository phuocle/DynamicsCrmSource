...
Mscrm.DocumentTemplate.CreateDialogControl.prototype = {
    ...
    $T_0: function($p0) {
        this.$5_0.append(String.format('<option value=\'{0}\'>{1}</option>',
            CrmEncodeDecode.CrmHtmlAttributeEncode(JSON.stringify($p0.ViewReference)),
            CrmEncodeDecode.CrmHtmlEncode($p0.ViewName)));
    },
    ...
    $13_0: function($p0) {
        if (Mscrm.InternalUtilities._String.isNullOrEmpty(this.$8_0) ||
            Mscrm.InternalUtilities._String.isNullOrEmpty(this.$9_0)) {
            var $v_0 = this.$5_0.find(':selected').val();
            if (isNullOrEmptyString($v_0)) {
                return;
            }
    ...
    $14_0: function($p0, $p1) {
        ...
        this.$8_0 = CrmEncodeDecode.CrmXmlDecode(XUI.Xml.XMLSerializer
            .serializeToString($v_0.getElementsByTagName('fetchxml')[0].firstChild));
        this.$9_0 = CrmEncodeDecode.CrmXmlDecode(XUI.Xml.XMLSerializer
            .serializeToString($v_0.getElementsByTagName('layoutxml')[0].firstChild));
        this.$X_0();
    },
    ...
    $X_0: function() {
        ...
        var $v_2 = new Mscrm.CrmDialog(Mscrm.CrmUri.create(String
                .format('/AdvancedFind/dialogs/dlg_editview.aspx?EntityName={0}&FromTemplate=1', $v_1)),
            $v_0,
            800,
            450,
            null);
...
Mscrm.DocumentTemplate.UploadDialogControl.prototype = {
    ...
    $e_0: function() {
        ...
        var $v_3 = (Mscrm.InternalUtilities.Utilities.isHighContrastEnabled())
            ? window.CDNURL + '/_imgs/office/helpdialog_hi_contrast_white_on_black_24x24.png'
            : window.CDNURL + '/_imgs/office/helpdialog_black_24x24.png';
        var $v_4 = String
            .format('<a target=\"_blank\" href=\"http://go.microsoft.com/fwlink/?LinkID=624118\" id=\"helpAnchor\" class=\"helpContainer {2}\" title=\"{1}\">\r\n\t\t\t\t\t<img src=\"{0}\" alt=\"{1}\"></a>', $v_3, $v_1, $v_2);
        this.$1_0.before($v_4);
    },
    ...
    $16_0: function($p0) {
        ...
        if ($v_0 < $p0[0].size) {
            alert(String.format(Xrm.Internal
                .getResourceString('ImportWizard.UploadFilePage.UploadIFrame.Error.FileSize'),
                this.$L_0));
            return false;
        }
        return true;
    ...
    $m_0: function($p0) {
        ...
        this.$K_0++;
        Xrm.Internal.trace.logT(Mscrm.DocumentTemplate.UploadDialogControl,
            'Enter:\ntarget: {0}\ncurr: {1}\nrel: {2}',
            Mscrm.DocumentTemplate.UploadDialogControl.$C($p0.target),
            Mscrm.DocumentTemplate.UploadDialogControl.$C($p0.currentTarget),
            Mscrm.DocumentTemplate.UploadDialogControl.$C($p0.relatedTarget));
        $p0.stopPropagation();
        $p0.preventDefault();
...
Mscrm.DocumentTemplate.SelectDialogControl.SelectedEntities
    .registerClass('Mscrm.DocumentTemplate.SelectDialogControl.SelectedEntities');
