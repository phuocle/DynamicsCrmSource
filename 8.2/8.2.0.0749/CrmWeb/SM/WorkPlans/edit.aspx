...
<title>
    <loc:Text ResourceId="SM_Weekly_Schedule" runat="server"/>
</title>
...
<script language="javascript">
    ...
    Sys.Application.add_load(function() {
        ...
        var tab0 = $get("tab0");
        crmTabBarWrapper.style.top = (IsNull(notificationWrapper) ? 0 : notificationWrapper.offsetHeight).toString() +
            "px";
        tab0.style.top = ((IsNull(notificationWrapper) ? 0 : notificationWrapper.offsetHeight) +
                crmTabBarWrapper.offsetHeight).toString() +
            "px";
    ...
    function refreshOpener() {
        try {
            if (!IsNull(_windowOpener))
                _windowOpener.auto($get("calendarType").value == "<%= CustomerService %>"
                    ? Mscrm.EntityTypeCode.Calendar
                    : Mscrm.EntityTypeCode.CalendarRule);
    ...
    function Save(sender, args) {
        ...
        if ($get("calendarType").value == "<%= CustomerService %>" &&
            ...
            $get("closurelinkmode").value == "observe" &&
            _crmCalendarLookup.get_dataValue() == null) {
    ...
    function SaveCalendar(sender, args) {
        ...
        if (($get("rad2").checked) || ($get("submitEnable").value == "true")) {
            ...
            if (dataValue != null) {
                ...
                command.SetParameter("holidayCalendarId", "");
            }
    ...
    function editworkinghours(params) {
        var url = Mscrm.CrmUri.create('/SM/workplans/Dialogs/TimeSheet.aspx?id=' +
            (!IsNull(params) ? params : "") +
            "&resourceid=<%= Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(ResourceId) %>&oType=<%= Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(ParentType) %>&calendarid=<%= Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(CalendarId) %>&calendarType=<%= Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(CalendarType) %>");

        var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("editworkinghourscallback", this, null, false);
        var workshiftid = openStdDlgWithCallback(url,
            null,
            640,
            460,
            callbackFunc,
        ...
        for (i = 0; i < oEndDates.length - 1; i++) {
            ...
            if (oEndDate > currentEndDate) {
                $get("endDateDescription")
                    .innerHTML = formatString(LOCID_UNTIL_DATE, Mscrm.DateTimeUtility.formatDate(oEndDate));
...
<table id="customerServiceTable" width="70%" cellspacing="0" cellpadding="0">
    <tr height="28">
        <td id="nameLabel" class="ms-crm-Field-Required" width="30%">
            <label for="name">
                <loc:text resourceid="SM_Name" runat="server"/>
    ...
    <tr height="28">
        <td id="descriptionLabel">
            <label for="description">
                <loc:text resourceid="SM_Description" runat="server"/>
            </label>
        </td>
        <td>
            <ui:textarea id="description" runat="server"/>
        </td>
    </tr>
</table>
...
<table id="weeklyScheduleTable" width="100%" cellspacing="0" cellpadding="2">
    ...
    <tr height="24">
        <td class="ms-crm-Form-Section ms-crm-Form-SectionBar" colspan="4" valign="bottom">
            <loc:Text ResourceId="SM_Edit_Weekly_Recurring_Schedule" runat="server"/>
        </td>
    </tr>
    <tr>
        <td nowrap valign="center">
            <loc:Text ResourceId="Web.SM.WorkPlans.edit.aspx_86" runat="server"/>
        </td>
        <td nowrap>
        ...
        <td nowrap class="workplan_edit_td_same" colspan="2">
            <label for="rad0">
                <loc:Text ResourceId="SM_Are_The_Same" runat="server"/>
            </label>
    ...
    <tr>
        ...
        <td nowrap class="workplan_edit_td_caption" colspan="2">
            <label for="rad1">
                <loc:Text ResourceId="SM_Vary_By_Day" runat="server"/>
            </label>
        </td>
    </tr>
    <tr>
        ...
        <td nowrap class="workplan_edit_td_caption" colspan="2">
            <label for="resourceNotWorking">
                <% if (CalendarType == CustomerService)
                   { %><loc:Text ResourceId="Label_24x7Support" runat="server"/>
                </label>
            <% }
                   else
                   { %>
    ...
    <tr id="sameworkplandays_row2" style="<%= ((IsAllTheSameRules() || Is247OrNotWorkingRules()) ? "" : "display:none") %>">
        <td colspan="2">&nbsp;</td>
        <td nowrap colspan="2">
            <img alt="" src="/_imgs/clear.gif" class="changeschedule_img_clear" width="20px">
    ...
    <tr>
        ...
        <label for="businessClosure">
            <% if (CalendarType == CustomerService)
               { %><loc:Text ResourceId="SM_Holiday_Schedule_Label" runat="server"/>
            <% }
               else
               { %>
        ...
        <td nowrap class="workplan_edit_td_caption">
            <label for="rad3">
                <loc:Text ResourceId="SM_Closure_Observe" runat="server"/>
            </label>
        </td>
        <td>&nbsp;</td>
    </tr>
    <tr>
        ...
        <td nowrap class="workplan_edit_td_caption">
            <label for="rad4">
                <loc:Text ResourceId="SM_Closure_Do_Not_Observe" runat="server"/>
            </label>
        </td>
        <td>&nbsp;</td>
    </tr>
...
<table width="100%" cellspacing="0" cellpadding="2">
    ...
    <tr id="dateTimeTr">
        <td>
            <label for="start">
                <loc:Text ResourceId="SM_Starting_On" runat="server"/>
            </label>
        </td>
        <td>
            <ui:DateTimeUI id="start" runat="server" AllowBlankDate="False"/>
        </td>
    ...
    <tr>
        <td>
            <label for="timeZone">
                <loc:Text ResourceId="SM_Timezone_Label" runat="server"/>
            </label>
        </td>
        <td colspan="2">
            <cnt:AppTimeZoneControl id="timeZone" AutoBinding="false" runat="server"/>
        </td>
        <td>&nbsp;</td>
    </tr>
</table>
