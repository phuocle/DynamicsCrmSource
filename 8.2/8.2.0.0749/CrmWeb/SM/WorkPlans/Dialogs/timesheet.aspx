...
<head>
    ...
    <style type="text/css">
        .RefreshDialog-Button_Override { margin-top: 0px !important; }
    </style>
</head>
<script language="javascript">
    ...
    function DeleteServiceRestrictions(sGrid, sTargetId) {
        ...
        if (target.length > 0) {
            ...
            var callbackObject = Mscrm.Utilities
                .createCallbackFunctionForXrmDialog(DeleteServiceRestrictionsCallbackFunction, parameters);
    ...
    function ServiceRestriction(serviceRestrictionParams) {
        if (!IsNull(serviceRestrictionParams)) {
            ...
            sRuleXml += "<serviceid name=\"" +
                CrmEncodeDecode.CrmXmlAttributeEncode(serviceName) +
                "\" dsc=\"0\">" +
                CrmEncodeDecode.CrmXmlEncode(serviceId) +
                "</serviceid>";
            ...
            if (calRuleId != "") {
                ...
                var oOldRule = XUI.Xml.SelectSingleNode(oRestrictionXml,
                    "/calendarrules/calendarrule[calendarruleid='" + calRuleId + "']",
                    null);
                XUI.Xml.SelectSingleNode(oRestrictionXml, "/calendarrules", null)
                    .replaceChild(XUI.Xml.SelectSingleNode(_oXmlDoc, "calendarrule", null), oOldRule);
            } else {

                XUI.Xml.SelectSingleNode(oRestrictionXml, "/calendarrules", null)
                    .appendChild(XUI.Xml.SelectSingleNode(_oXmlDoc, "calendarrule", null));
            }
    ...
    function DeleteServiceRestriction(rgCalRuleId) {
        ...
        for (var i = 0; i < rgCalRuleId.length; i++) {
            ...
            var oOldRule = XUI.Xml.SelectSingleNode(oRestrictionXml,
                "/calendarrules/calendarrule[calendarruleid='" + rgCalRuleId[i] + "']",
                null);
    ...
    function applychanges() {
        if (window.frames["TimeSheet"].CommitForm()) {
            ...
            command.SetParameter("exceptionDate",
                <%= (ExceptionMode ? "FormatUtcDate(Mscrm.DateTimeUtility.parseDate(Mscrm.FormControlInputBehavior.GetBehavior('dateControl').get_innerText()))" : "\"\"") %>);
            ...
            <div id="tab1" class="ms-crm-Tab ms-crm-absolutePosition" style="padding: 10px; height: auto;">
                ...
                <div style="display: <%= (ExceptionMode ? "block" : "none") %>; white-space: nowrap; position: absolute; top: 24px; left: 0px; right: 0px; height: 24px;" valign="middle">
                    <table style="width: 100%; height: 24px;" cellspacing="0" cellpadding="0">
                        <tr>
                            <td style="width: 21%;">
                                <label for="dateControl">
                                    <loc:Text ResourceId="SM_Date" runat="server"/>
                                </label>
                            </td>
                            <td style="width: 29%;">
                                <ui:DateTimeUI id="dateControl" runat="server" AllowBlankDate="False"/>
                            </td>
                ...
                <div style="position: absolute; top: <%= (ExceptionMode ? "48px" : "0px") %>; left: 0px; right: 0px; height: 24px;">
                    <table style="width: 100%;" cellspacing="0" cellpadding="0">
                        <tr>
                            <td style="width: 60%;" class="ms-crm-Form-Section ms-crm-Form-SectionBar" valign="bottom">
                                <loc:Text ResourceId="SM_Enter_Working_Hours_And_Breaks" runat="server"/>
                            </td>
                            <td style="width: 40%;" class="ms-crm-Form-Section ms-crm-Form-SectionBar dlgs_timesheet_td_BtnCap">
                                <ui:Button ID="capacitybutton" CssClass="ms-crm-RefreshDialog-Button RefreshDialog-Button_Override" OnClick="toggleCapacity();" ResourceId="SM_Unknown_Capacity" runat="server"></ui:Button>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div style="display: <%= (ExceptionMode ? "block" : "none") %>; white-space: nowrap; position: absolute; bottom: 0px; left: 0px; right: 0px; height: 24px;" valign="middle">
                    <table width="100%" cellspacing="0" cellpadding="0">
                        <tr>
                            <td style="width: 21%;">
                                <loc:Text ResourceId="SM_Timezone" runat="server"/>
                            </td>
                            <td style="width: 79%;">
