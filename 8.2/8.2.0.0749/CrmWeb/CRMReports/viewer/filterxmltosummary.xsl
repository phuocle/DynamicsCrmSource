...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
                xmlns:user="http://www.microsoft.com/crm" version="1.0">
  <xsl:output method="text" />

  <xsl:template match="/">
    <xsl:for-each select="/ReportFilter/ReportEntity">
      <xsl:choose>
        <xsl:when test=".//condition[(not(@uihidden) or @uihidden='0')]">
  ...
  <xsl:template match="condition">
    <xsl:choose>
      <xsl:when test="(not(@uihidden) or @uihidden='0')">
        <xsl:text>&#160;&#160;&#160;&#160;&#160;</xsl:text>
        <xsl:value-of select="@uiattributename" />
        <xsl:text>: </xsl:text>
        <em>
          <xsl:value-of select="@uioperatorname" />
        </em>
        <xsl:text> </xsl:text>
        <xsl:value-of select="@uiname" />
        <xsl:text>&#xA;</xsl:text>
