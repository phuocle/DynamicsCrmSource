...
<script language="javascript" type="text/javascript">
    ...
    function applychanges() {
        ...
        oEntity.Alias = (entityComponent.get_dataValue() == oSelectedEntity.getAttribute("entity"))
            ? ""
            : entityComponent.get_dataValue();
        oEntity.Name = oSelectedEntity.getAttribute("entity");
        if (oSelectedEntity.parentNode.tagName == "OPTGROUP") {


            oEntity.ParentEntity = (oSelectedEntity.parentNode.getAttribute("alias") ==
                    oSelectedEntity.parentNode.getAttribute("entity"))
                ? ""
                : oSelectedEntity.parentNode.getAttribute("alias");
            ...
            if (!IsNull(oEntity.Alias) && oEntity.Alias.length > 0) {
                ...
                if (localStorage.getItem(oEntity.EntityAlias) == oEntity.EntityAlias) {
                    oEntity.Alias = localStorage.getItem(oEntity.EntityAlias + '_Alias');
                } else {
                    oEntity.Alias = "LE_" + GetUniqueId();
                    localStorage.setItem(oEntity.EntityAlias, oEntity.EntityAlias);
                    localStorage.setItem(oEntity.EntityAlias + '_Alias', oEntity.Alias);
                }
            }
        }
        ...
        if (!IsNull(oSelectedEntity.getAttribute("attributedisplayname"))) {
            sAttributeDisplayName = formatString(LOCID_ATTRIBUTE_TITLE_FORMAT,
                sAttributeDisplayName,
                oSelectedEntity.getAttribute("attributedisplayname"));
        }
        ...
        oReturnValue.SummaryValue = (summaryValueComponent.get_disabled())
            ? null
            : summaryValueComponent.get_dataValue();
    ...
    function updateColumn() {
        ...
        if (!IsNull(oColumnOption)) {
            ...
            if (_iObjectType == PROPERTY_PAGE_TYPE_GROUP) {
                timeIntervalComponent.set_disabled(sDataType != "datetime" && sDataType != "date");
            }
    ...
    function updateColumns() {
        ...
        var sAttributeName;
        var sEntityAlias = (entityComponent.get_dataValue() ==
                entityComponent.get_selectedOption().getAttribute("entity"))
            ? ""
            : entityComponent.get_dataValue();
...
<frm:DialogForm id="crmForm" runat="server">
    <table cellpadding="0" cellspacing="5" width="100%" style="padding: 5px; table-layout: fixed;">
        ...
        <col/>
        <tr>
            <frm:FormCell runat="server" id="entityCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.RecordType" ShowLabel="True" RequiredLevel="Required">
                <ui:Select id="entity" runat="server" OnChange="updateColumns()"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="columnCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.Column" ShowLabel="True" RequiredLevel="Required">
                <ui:Select id="column" runat="server" OnChange="updateColumn()"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="timeIntervalCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.Interval">
                <ui:Select id="timeInterval" runat="server"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="dataTypeCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.DataType" ShowLabel="True">
                <ui:LabelUI id="dataType" runat="server"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="schemaNameCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.SchemaName" ShowLabel="True">
                <ui:LabelUI id="schemaName" runat="server"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="sortOrderCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.SortOrder" ShowLabel="True">
                <ui:Select id="sortOrder" runat="server"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="widthCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.Width" ShowLabel="True" RequiredLevel="Required">
                <ui:Select id="width" runat="server"/>
            </frm:FormCell>
        </tr>
        <tr>
            <frm:FormCell runat="server" id="summaryValueCell" LabelResourceId="CustomReporting.AdHocWizard.PropertyPageDialog.SummaryValue" ShowLabel="True">
                <ui:Select id="summaryValue" runat="server"/>
            </frm:FormCell>
        </tr>
