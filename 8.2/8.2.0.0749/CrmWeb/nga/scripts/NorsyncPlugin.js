...
(function() {
        ...
        for (var x in resultColumns) {
            ...
            for (var y in resultColumns[x].column) {
                ...
                if (typeof resultColumns[x].column[y].Value == 'string')
                    newResults += '"' +
                        resultColumns[x].column[y].Key.replace("\"", "\\\"") +
                        '":"' +
                        SQLitePluginTransaction.escapeJSONVal(resultColumns[x].column[y].Value) +
                        '"';
                else
                    newResults += '"' +
                        resultColumns[x].column[y].Key.replace("\"", "\\\"") +
                        '":' +
                        resultColumns[x].column[y].Value +
                        '';
    ...
    SQLitePluginTransaction.txCompleteCallback = function(result, success) {
        ...
        for (var x in result.results) {
            ...
            if (queryId) {
                SQLitePluginTransaction
                    .queryCompleteCallback(transId, queryId, SQLitePluginTransaction.fixQueryResult(queryResult));
            }
        }
    ...
    */
    SQLitePlugin.prototype.login = function(name,
        user,
        pass,
        host,
        port,
        ...
        if (!(name in this.openDBs)) {
            cdv.exec(
                ...
                "login",
                [
                    name, user, pass, host, port, dbpath, isssl, serverDBname, modelName, modelVersion, accessToken,
                    sessionId
                ]
            );
        } else {
    ...
    * @param syncProperties    the sync properties
     */
    SQLitePlugin.prototype.synchronize = function(message,
        progress,
        error,
        modelVersion,
        beforeModelChange,
            ...
            if (e.code == -19) {
                if (!!beforeModelChange) {
                    ...
                    if (typeof json.model == 'undefined') {
                        if (!!error) {
                            var errorObject =
                                checkErrorCallback("cannot call beforeModelChange: model is undefined", -19);
                            return error(errorObject);
                        }
                    }
                ...
                if (_this.syncRecall > 0) {
                    ...
                    setTimeout(function() {
                            _this
                                .synchronize(message, progress, error, modelVersion, beforeModelChange, syncProperties);
                        },
                        _this.syncTimeout);
                } else {
                    _this.getDefaultSyncRecallCount(function(recallCount) { _this.syncRecall = recallCount; });
        ...
        if (this.dbPath in this.openDBs) {
            cdv.exec(mysuccess,
                ...
                "SQLiteSyncDatabasePlugin",
                "synchronizeDatabase",
                [this.dbPath, modelVersion, JSON.stringify(syncProperties)]);
        } else {
            if (!!error) {
                return error(checkErrorCallback("Database does not open!", databaseDoesNotOpenErrorCode));
            }
