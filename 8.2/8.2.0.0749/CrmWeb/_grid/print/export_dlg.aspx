...
<title>
    <loc:Text ResourceId="Title_MultiPage_ExportXL" runat="server"/>
</title>
...
<script language="JavaScript" type="text/javascript">
    ...
    function windowOnLoad() {
        ...
        if (bEditableColumns) {
            ...
            for (var i = 0; i < iLen; i++) {
                ...
                oGridField = XUI.Xml.SelectSingleNode(_oFieldXml,
                    "/entities/entity/fields/field[@name = '" + CrmEncodeDecode.CrmXmlEncode(sFieldName) + "']",
                    null);
                if (IsNull(oGridField)) {
                    ...
                    if (!IsNull(aData) && aData.length == 2) {
                        ...
                        var linkEntity = XUI.Xml
                            .SelectSingleNode(_oFetchXml,
    ...
    function Ok(e) {
        ...
        if ($get("printCurrent") && $get("printCurrent").checked) {
            ...
            <% }
               ...
               { %>
            window.returnValue = new Result(RESULT_CURRENTPAGE,
                $get("chkReimport") ? $get("chkReimport").checked : false);
            <% } %>
            ...
            window.returnValue = new Result(RESULT_LIVELIST,
                false,
                _bUseSqlQueryForLiveExport,
                XUI.Xml.XMLSerializer.serializeToString(_oFetchXml),
                XUI.Xml.XMLSerializer.serializeToString(_oLayoutXml));
        } else if ($get("exportLivePivot").checked) {
            window.returnValue = new Result(RESULT_LIVEPIVOT,
                false,
                _bUseSqlQueryForLiveExport,
                XUI.Xml.XMLSerializer.serializeToString(_oFetchXml),
                XUI.Xml.XMLSerializer.serializeToString(_oLayoutXml));
    ...
    function editColumns(e) {
        ...
        var oURL = Mscrm.CrmUri.create("/AdvancedFind/dialogs/dlg_editview.aspx?EntityName=" +
            CrmEncodeDecode.CrmUrlEncode(_sEntityName) +
            "&FromExportExcel=1");
    ...
    function loadFieldAndPropertiesXml() {
        ...
        if (oResult.Success) {
            ...
            _oPropertiesXml = XUI.Xml.LoadXml(XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(oResultXml, "/fieldpropertiesxml/propertiesxml", null)));
            _oFieldXml = XUI.Xml.LoadXml(XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(oResultXml, "/fieldpropertiesxml/fieldxml", null)));
        }
    }
...
<style type="text/css">
    ...
    #crmDialogFooter .ms-crm-Button { background-color: white; }
...
<form name="crmDialog" style="height: 100%">
    <div class="ms-crm-export-outerdiv">
        <div class="ms-crm-RefreshDialog-Header">
            <a href="#" class="ms-crm-RefreshDialog-FirstTopButton" id="btnCross" tabindex="1">
            ...
            <div class="ms-crm-RefreshDialog-Header-Desc">
                <loc:Text ResourceId="Header_ExportXL" runat="server"/>
            </div>
        </div>
        <div style="vertical-align: middle; margin-left: 25px; margin-right: 25px;" class="ms-crm-RefreshDialog-Main">
            <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed; position: absolute;">
                ...
                <tr>
                    <td colspan="3">
                        <b>
                            <loc:Text ResourceId="Instructions_ExportXL" runat="server"/>
                        </b>
                    </td>
                </tr>
                <tr>
                ...
                <% if (showStatic)
                   { %>
                    <tr style="vertical-align: middle">
                        <td>
                            <input type="radio" name="exportType" id="printCurrent" tabindex="1" class="radioButton" checked="checked" onclick="setUI()">
                        </td>
                        <td colspan="2">
                            <label for="printCurrent" style="cursor: pointer;">
                                <loc:Text ResourceId="Ribbon.HomepageGrid.Data.Export.StaticExcelExport" runat="server"/>
                            </label>
                        </td>
                    </tr>
                            ...
                            <td colspan="3">
                                <table width="90%">
                                    ...
                                    <tr id="trReimport" disabled="true">
                                        <td align="right">
                                            <input type="checkbox" id="chkReimport" tabindex="1" class="checkbox" onclick="setUI();"/>
                                        </td>
                                        <td>
                                            <label for="chkReimport" style="cursor: pointer;">
                                                <loc:Text ResourceId="Label_PrintDlg_ExportForReimport" runat="server"/>
                                            </label>
                                        </td>
                                    </tr>
                                </table>
                    ...
                    <% if (showMultiplePage)
                       { %>
                        ...
                        <tr style="vertical-align: middle">
                            <td>
                                <input type="radio" name="exportType" id="printAll" tabindex="1" class="radioButton" onclick="setUI()">
                            </td>
                            <td colspan="2">
                                <label for="printAll" style="cursor: pointer;">
                                    <loc:Text ResourceId="Ribbon.HomepageGrid.Data.Export.StaticExcelExportAll" runat="server"/>
                                </label>
                            </td>
                        </tr>
                    <% } %>
                <% } %>
                <tr id="trExportLivePivot" style="vertical-align: middle" <% if (!enableLive)
                                                                             { %> disabled="true"<% } %>>
                    <td>
                        <input type="radio" name="exportType" id="exportLivePivot" tabindex="1" onclick="setUI()" class="radioButton">
                    </td>
                    <td>
                        <label for="exportLivePivot" id="lblExportLivePivot" style="cursor: pointer;">
                            <loc:Text ResourceId="Label_PrintDlg_LivePivot" runat="server"/>
                        </label>
                    </td>
                </tr>
                <tr id="trExportLiveList" style="vertical-align: middle"<% if (!enableLive)
                                                                           { %> disabled="true"<% } %>>
                    <td>
                        <input type="radio" name="exportType" id="exportLiveList" tabindex="1" onclick="setUI()" class="radioButton">
                    </td>
                    <td>
                        <label for="exportLiveList" id="lblExportLiveList" style="cursor: pointer;">
                            <loc:Text ResourceId="Label_PrintDlg_LiveList" runat="server"/>
                        </label>
                    </td>
                </tr>
            </table>
                    ...
                    <tr>
                        <td colspan="2">
                            ...
                            <script language="javascript">
                                crmCreate(Mscrm.NotificationList, {}, null, null, $get('updateViaImportNotification'));
                            </script>
                        </td>
                    </tr>
                </table>
