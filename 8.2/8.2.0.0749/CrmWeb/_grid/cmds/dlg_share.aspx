...
<script type="text/javascript">
    ...
    function LoadShareInfo() {
        ...
        XUI.Xml.Load(Mscrm.CrmUri.create("/_grid/cmds/getshareaccess.xsl").toString(),
            false,
            function(xmlDoc) { oXslDoc = xmlDoc; },
            function(statusCode, xmlDoc) { oXslDoc = null; });
    ...
    function addUserToShareCallback(retval) {
        ...
        for (i = 0; i < retval.items.length; i++) {
            ...
            if (!bExisting) {
                ...
                oTD.innerHTML = "<input type='checkbox' class='checkbox' chkType='se' onclick='selectUser()' id='" +
                    item.id +
                    "_selectUser' title='" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(LOCID_TITLE_SELECTUSERTEAM) +
                    "'/>";
                if (!bShare) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;

                oTD = oTR.insertCell(-1);
                oTD.innerHTML = "<img alt='' src='/_imgs/ico_18_" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(item.type) +
                    ".gif'>";

                oTD = oTR.insertCell(-1);
                oTD.innerHTML = "<nobr class='textalign' title='" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(item.name) +
                    "'>" +
                    CrmEncodeDecode.CrmHtmlEncode(item.name) +
                    "</nobr>";

                oTD = oTR.insertCell(-1);
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='r' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_read' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_READ, item.name)) + "'/>";
                if (!(bRead && bShare)) {
                ...
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='w' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_write' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_WRITE, item.name)) + "'/>";
                if (!(bWrite && bShare)) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;

                oTD = oTR.insertCell(-1);
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='d' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_delete' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_DELETE, item.name)) + "'/>";
                if (!(bDelete && bShare)) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;

                oTD = oTR.insertCell(-1);
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='ap' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_append' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_APPEND, item.name)) + "'/>";
                if (!(bAppend && bShare)) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;

                oTD = oTR.insertCell(-1);
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='as' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_assign' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_ASSIGN, item.name)) + "'/>";
                if (!(bAssign && bShare)) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;

                oTD = oTR.insertCell(-1);
                oTD
                    .innerHTML =
                    "<input type='checkbox' class='checkbox' chkType='sh' onclick='requireRead(new Sys.UI.DomEvent(event))' id='" + item.id + "_share' title='" + CrmEncodeDecode.CrmHtmlAttributeEncode(formatString(LOCID_FMT_SHARECHECKBOX, LOCID_PRIVILEGE_SHARE, item.name)) + "'/>";
                if (!bShare) XUI.Html.DomUtils.GetFirstChild(oTD).disabled = true;
    ...
    function RemoveUsersFromShare() {
        ...
        var tableUsersFirstChildNextSibling = XUI.Html.DomUtils
            .GetNextSibling(XUI.Html.DomUtils.GetFirstChild(tableUsers));
    ...
    function applychanges() {
        ...
        for (var i = 0; i < rowLen; i++) {
            ...
            if ((accessrights == 0) && (tableUsers.rows[i].getAttribute("rights") == -1)) {
                ...
                tempVal += "<accessrights>" +
                    accessrights +
                    "</accessrights><originalrights>" +
                    tableUsers.rows[i].getAttribute("rights") +
                    "</originalrights></assignee>"
                retval += tempVal;
            }
        ...
        for (i = 0; i < _aOriginalGuids.length; i++) {
            if (_aOriginalGuids[i]) {
                retval += "<assignee><assigneeid>" +
                    _aOriginalGuids[i][0] +
                    "</assigneeid><type>" +
                    _aOriginalGuids[i][1] +
                    "</type><accessrights>0</accessrights></assignee>";
            }
        }
        retval += "</root>";
            ...
            <div style="position: absolute; height: 21px; top: 0px; width: 100%;">
                <table id="tableUsersHeader" cellspacing="0" cellpadding="0" class="ms-crm-Share-Header">
                    ...
                    <tr onmouseover="RenderToolTip(new Sys.UI.DomEvent(event));">
                        <td>
                            <input type="checkbox" class="checkbox" id="chkSelectAll" name="chkSelectAll" onclick="SelectAll(new Sys.UI.DomEvent(event))" title="<loc:Text Encoding='HtmlAttribute' ResourceId='Grid.CheckAll.UsersTeams.Title' runat='server'/>"/>
                        </td>
                        <td>
                        ...
                        <th class="user-Header">
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_448" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_449" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_450" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_451" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_452" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_453" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif">
                        ...
                        <th>
                            <nobr>
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_454" runat="server"/>
                            </nobr>
                        </th>
                        <td>
                            <img alt="" src="/_imgs/grid/bar_line.gif"/>
            ...
            <div style="position: absolute; width: 100%; top: 21px; bottom: 2px;">
                <div id="divUsers" style="background-color: #ffffff; height: 100%; width: 100%; overflow-y: auto; border: 1px solid #999999; position: absolute; top: 0px; left: 0px;">
                    <table width="100%" height="100%" style="cursor: wait;">
                        <tr>
                            <td align="center">
                                <loc:Text ResourceId="Web._grid.cmds.dlg_share.aspx_468" runat="server"/>
                            </td>
                        </tr>
                    </table>
                </div>
