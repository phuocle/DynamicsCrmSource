...
<script type="text/javascript" language="javascript">
    ...
    function localizeActivityFeedsRole(roleXml) {
        ...
        if (role) {
            ...
            XUI.Xml.SetText(nameNode,
                <%= CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("Activity_Feeds_Role")) %>);
        }
    }
    ...
    function PageLoad() {
        ...
        XUI.Xml.Load(Mscrm.CrmUri.create("/_grid/cmds/role.xsl").toString(),
            false,
        ...
        if (tableRoles.rows.length == 0) {
            ...
            document.getElementById('divRoles')
                .innerHTML =
                "<table class='ms-crm-ListArea' style='text-align:center'><tr><td class='ms-crm-List-Disabled' style='valign:middle'>" + CrmEncodeDecode.CrmHtmlEncode(LOCID_NO_ROLES_FOUND) + "</td></tr></table>";
        }
        ...
        for (var i = 0; i < nLength; i++) {
            ...
            if (XUI.Html.DomUtils.GetFirstChild(oRow.cells[0]).checked) {
                ...
                if (!UserHadRole(oRow.getAttribute("oid")))
                    rolesToAdd += "<roleid bid='" +
                        oRow.getAttribute("bid") +
                        "'>" +
                        oRow.getAttribute("oid") +
                        "</roleid>";
            } else {
                if (UserHadRole(oRow.getAttribute("oid")))
                    rolesToRemove += "<roleid bid='" +
                        oRow.getAttribute("bid") +
                        "'>" +
                        oRow.getAttribute("oid") +
                        "</roleid>";
