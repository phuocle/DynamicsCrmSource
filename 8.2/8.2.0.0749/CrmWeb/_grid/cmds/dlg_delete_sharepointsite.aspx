...
<head>
    ...
    <script language="JavaScript">
        ...
        if (typeof dialogArguments === 'undefined') {
            dialogArguments = getDialogArguments();
        }

        for (i = 0; i < _iTotal; i++) {
            if (dialogArguments != null &&
                dialogArguments[i] != null &&
                dialogArguments[i].toUpperCase() == _defaultSiteId.toUpperCase()) {

                _descriptionDelete = (_iTotal == 1)
                    ? <%= CrmEncodeDecode.CrmJavaScriptEncode(BodyDescription_DeleteDefault) %>
                    : <%= CrmEncodeDecode.CrmJavaScriptEncode(BodyDescription_DeleteIncludesDefault) %>;
            }
        }
