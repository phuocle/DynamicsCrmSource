...
<head>
    <title>
        <loc:Text ResourceId="MenuItem_Label_WebMailMerge" runat="server"/>
    </title>
    <cnt:AppHeader id="crmHeader" runat="server"/>
    <style type="text/css">
        body { background-color: #E3EFFF; }

        .header { background-color: #6693CF; }
    ...
    <script type="text/javascript" language="Javascript">
        ...
        Sys.Application.add_load(function() {
            ...
            <% if (objectTypeCode == Util.Fax)
               { %>
            activityForm
                .setAttribute("action", Mscrm.CrmUri.create("/Tools/MailMerge/iframes/faxForm.aspx").toString());
            <% }
               else if (objectTypeCode == Util.Task)
               { %>
            activityForm.setAttribute("action",
                Mscrm.CrmUri.create("/Tools/MailMerge/iframes/taskForm.aspx").toString());
            <% }
               else
               { %>
            activityForm.setAttribute("action",
                Mscrm.CrmUri.create("/Tools/MailMerge/iframes/letterForm.aspx").toString());
            <% } %>
