    ...
    function GetLookupFieldsXml(oPropertiesXml, oFieldsXml) {
        ...
        if (!IsNull(oFields)) {
            ...
            for (i = 0; i < iLen; i++) {
                ...
                if (XUI.Xml.SelectSingleNode(checkField,
                        "/fields/field[@name = '" + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + "']",
                        null) ==
                    null) {
                    oEntityFields = XUI.Xml.SelectNodes(oFieldsXml,
                        "/entity/fields/field[@name = '" + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + "']",
                        null);
                    ...
                    if (null != oEntityFields && oEntityFields.length > 0) {
                        ...
                        if (dataType == "lookup" ||
                            ...
                            if (XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oEntityFields[0], "./@validforform", null)) ==
                                "true") {
                                ...
                                for (j = 0; j < jFieldLen; j++) {
                                    otc.push(XUI.Xml
                                        .GetText(XUI.Xml
                                            .SelectSingleNode(oEntityFields[j], "./@objecttypecode", null)));
                                }
                                ...
                                if (!(otc.length == 1 && otc[0] == '0')) {
                                    ...
                                    sFieldsXml += "<field name=\"" +
                                        ...
                                        "\" displayname=\"" +
                                        CrmEncodeDecode
                                        .CrmXmlAttributeEncode(XUI.Xml.GetText(XUI.Xml
...
**************************************************************/
    ...
    function populateParentAttribute() {
        ...
        if (_lookupFieldsXml != null) {
            ...
            for (i = 0; i < len; i++) {
                ...
                var j, jlen, otcNodes;
                var otcNodes = XUI.Xml.SelectNodes(oNodes[i],
                    "/fields/field[@name = '" +
                    CrmEncodeDecode.CrmXmlAttributeEncode(entityInfo) +
                    "']/objecttypecode[@value]",
                    null);
                jlen = otcNodes.length;
                for (j = 0; j < jlen; j++) {
                    var otc = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(otcNodes[j], "./@value", null));
                    var entityNodes = XUI.Xml
                        .SelectSingleNode(quickFormXmlDoc,
                            "/entities/entity[@objecttypecode = '" + CrmEncodeDecode.CrmXmlAttributeEncode(otc) + "']",
                            null);
                    var isQuickViewLookupEntity = false;
                    var isInteractionCentricEnabledentities = XUI.Xml
                        .GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@isinteractioncentricenabled", null));
                    var isVisibleInMobileClient = XUI.Xml
                        .GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@isvisibleinmobileclient", null));
                    var canCreateForms = XUI.Xml.GetText(XUI.Xml
                        .SelectSingleNode(entityNodes, "./@cancreateforms", null));
                    if (_oFormType == "mainInteractionCentric") {
                        if (canCreateForms == "true" &&
                            (isInteractionCentricEnabledentities == "true" || isVisibleInMobileClient == "true")) {
                            isQuickViewLookupEntity = true;
                        }
                    } else {
                        if (canCreateForms == "true") {
                            isQuickViewLookupEntity = true;
                    ...
                    if (isQuickViewLookupEntity) {
                        var entityName = CrmEncodeDecode
                            .CrmXmlDecode(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@name", null)));
                        var entityLogicalName = XUI.Xml
                            .GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@logicalname", null));
    ...
    function UpdateQuickFormViewer() {
        ...
        quickFormViewerData.options[relatedEntity.selectedIndex]
            .value = relatedEntity.options[relatedEntity.selectedIndex].value +
            ":" +
            embedFormData.options[embedFormData.selectedIndex].value;
        quickFormViewerData.options[relatedEntity.selectedIndex].innerHTML = quickFormViewerData.options[relatedEntity
            .selectedIndex].title;
    }
        ...
        if (_quickFormCollectionControlId != txtDesc.value) {
            ...
            if (_isReferencePanelQuickFormCollection) {
                oNodes = XUI.Xml.SelectNodes(_oFormXml,
                    ...
                    _ControlClassIds.referencepanelquickformcollection +
                    "']",
                    null);
            } else {
                oNodes = XUI.Xml.SelectNodes(_oFormXml,
                    "/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell/control[@classid = '" +
                    _ControlClassIds.quickformcollection +
                    "']",
                    null);
            }
    ...
    function createQuickForm() {
        ...
        var entityNodes = XUI.Xml.SelectSingleNode(quickFormXml,
            "/entities/entity[@logicalname = '" + CrmEncodeDecode.CrmXmlAttributeEncode(entityLogicalName) + "']",
            null);
        var otc = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@objecttypecode", null));

        crmUrl.get_query()["etc"] = otc;
        crmUrl.get_query()["extraqs"] = "formtype=" +
            CrmEncodeDecode.CrmNameValueEncode("quick") +
            "&action=" +
            CrmEncodeDecode.CrmNameValueEncode("0");
        _formWindow = true;
    ...
    function editQuickForm() {
        ...
        crmUrl.get_query()["extraqs"] = "formtype=" +
            ...
            "&action=" +
            CrmEncodeDecode.CrmNameValueEncode("-1");
        _formWindow = true;
        openStdWin(crmUrl, buildWinName(), _oConst.iAttributeEditorWidth, _oConst.iAttributeEditorHeight);
    }
    ...
    function RefreshComboIfRequired() {
        if (_formWindow) {
            ...
            if (_quickFormXml != null) {
                ...
                var entityNodes = XUI.Xml.SelectSingleNode(oldFormXml,
                    "/entities/entity[@logicalname = '" +
                    CrmEncodeDecode.CrmXmlAttributeEncode(entityLogicalName) +
                    "']",
                    null);
                var otc = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(entityNodes, "./@objecttypecode", null));
                ...
                if (xmlStr != null) {
                    ...
                    if (formUpdated) {
                        var xmlDoc = XUI.Xml.LoadXml(_quickFormXml);
                        var node = XUI.Xml.SelectSingleNode(xmlDoc,
                            "/entities/entity[@objecttypecode = '" + CrmEncodeDecode.CrmXmlAttributeEncode(otc) + "']",
                            null);
                        node.parentNode.replaceChild(XUI.Xml.SelectSingleNode(newFormXml, "/entities/entity", null),
                            node);
    ...
    function GetQuickForms(xmlDoc, entity) {
        ...
        var formNodes = XUI.Xml.SelectNodes(xmlDoc,
            "/entities/entity[@logicalname = '" + CrmEncodeDecode.CrmXmlAttributeEncode(entity) + "']/form[@Id]",
            null);
        var i = 0;
        var len = formNodes.length;

        for (i = 0; i < len; i++) {
            var formName = CrmEncodeDecode.CrmXmlDecode(XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(formNodes[i], "./@Name")));
            ...
            <div id="tab0" class="ms-crm-Tab" style="display: inline;">
                ...
                <fieldset>
                    ...
                    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
                        ...
                        <tr>
                            <td class="ms-crm-Field-Required" id="label_c">
                                <label for="txtLabel">
                                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subform.aspx.Label" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                                </label>
                            </td>
                            <td>
                                <ui:TextBox id="txtLabel" MaxLength="255" runat="server"/>
                            </td>
                        </tr>
                    </table>
                    ...
                    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
                        ...
                        <tr>
                            ...
                            <td >
                                <label for="chkDispOption" id="checkDisplayOptionLabel" runat="server"></label>
                            </td>
                        </tr>
                    </table>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
                        ...
                        <tr>
                            <td >
                                <label >
                                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subform.aspx.LookupLabel" runat="server"/>
                                </label>
                            </td>
                        ...
                        <tr>
                            <td >
                                <label >
                                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subform.aspx.RelatedEntity" runat="server"/>
                                </label>
                            </td>
                        ...
                        <tr>
                            <td>
                                <label >
                                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subform.aspx.SubForm" runat="server"/>
                                </label>
                            </td>
                            <td id="EmbedForm"></td>
                        </tr>
                        <tr>
                            <td colspan='2'>
                                <label id="quickFormViewerLabel"><loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subform.aspx.SelectedSubForm" runat="server"/>&nbsp;</label>
                            </td>
                        </tr>
                        <tr>
                    ...
                    <table id="newFormTable" cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
                        ...
                        <tr>
                            <td/>
                            <td>
                                <ui:Button id="editFormId" CssClass="ms-crm-RefreshDialog-Button" onclick="editQuickForm();" ResourceId="Edit_QuickForm_From_SubForm" runat="server"/></ui:Button>
                            </td>
                            <td>
                                <ui:Button id="newFormId" CssClass="ms-crm-RefreshDialog-Button" onclick="createQuickForm();" ResourceId="New_QuickForm_From_SubForm" runat="server"/></ui:Button>
                            </td>
                            <td/>
                        </tr>
                    </table>
                ...
                <fieldset id="AdditionalProperties">
                    ...
                    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
                        ...
                        <tr>
                            <td>
                                <ui:CheckBox id="displayAsCustomer360Option" runat="server"/>
                            </td>
                            <td >
                                <label for="displayAsCustomer360Option" id="displayAsCustomer360Tile">
                                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.SubForm.aspx.DisplayAsCustomer360Label" runat="server"/>
                                </label>
                            </td>
                        </tr>
                    </table>
