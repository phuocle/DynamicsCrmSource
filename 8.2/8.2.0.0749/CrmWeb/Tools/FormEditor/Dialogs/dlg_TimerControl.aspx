...
<script type="text/javascript" language="javascript">
    ...
    function OnTimerCtrlWindowLoad() {
        ...
        setPickListValues(_successConditionName,
            _successConditionValue,
            'selSuccessCondition',
            'selSuccessConditionValues');

        setPickListValues(_warningConditionName,
            _warningConditionValue,
            'selWarningCondition',
            'selWarningConditionValues');

        setPickListValues(_cancelConditionName,
            _cancelConditionValue,
            'selCancelCondition',
            'selCancelConditionValues');

        setPickListValues(_failureConditionName,
            _failureConditionValue,
            'selFailureCondition',
            'selFailureConditionValues');
    ...
    function setPickListValues(conditionName, conditionValue, parentpicklistname, childpicklistname) {
        ...
        Mscrm.FormControlInputBehavior.GetBehavior(parentpicklistname)
            .set_dataValue((temp && temp.length > 0 && temp[0].nodeValue != null) ? temp[0].nodeValue : null);
        onSelectOptionChange(parentpicklistname);
        if (childpicklistname != null) {
            temp = conditionValue ? conditionValue.childNodes : null;
            temp && temp[0].nodeValue != null && temp.length > 0
                ? selectValue($get(childpicklistname), temp[0].nodeValue)
                : $get(childpicklistname).selectedIndex = -1;
        }
    ...
    function applychanges() {
        ...
        _sSuccessConditionValue = $get('selSuccessConditionValues').selectedIndex != -1
            ? $get('selSuccessConditionValues').options[$get('selSuccessConditionValues').selectedIndex].title
            : null;
        if ($get('selSuccessConditionValues').selectedIndex == -1) {
            alert(LOCID_ENTER_SUCCESSVALUE);
            return false;
        }
        _sWarningConditionValue = $get('selWarningConditionValues').selectedIndex != -1
            ? $get('selWarningConditionValues').options[$get('selWarningConditionValues').selectedIndex].title
            : null;
        _sCancelConditionValue = $get('selCancelConditionValues').selectedIndex != -1
            ? $get('selCancelConditionValues').options[$get('selCancelConditionValues').selectedIndex].title
            : null;
        _sFailureConditionValue = $get('selFailureConditionValues').selectedIndex != -1
            ? $get('selFailureConditionValues').options[$get('selFailureConditionValues').selectedIndex].title
            : null;
        _sPauseConditionValue = $get('selPauseConditionValues').selectedIndex != -1
            ? $get('selPauseConditionValues').options[$get('selPauseConditionValues').selectedIndex].title
            : null;
                                    ...
                                    <table width="100%" cellspacing="0" cellpadding="0">
                                        ...
                                        <tr>
                                            <td style="vertical-align: top">
                                                <fieldset>
                                                    <legend>
                                                        <loc:text resourceid="Timer_Control_Dlg_Section_Name" runat="server"/>
                                                    </legend>
                                                            ...
                                                            <td id="tdDisplayNameLabel" class="ms-crm-Field-Required" style="width: 150px">
                                                                <label for="txtDisplayName">
                                                                    <loc:text resourceid="Timer_Control_Dlg_DisplayName" runat="server"/><img src="/_imgs/frm_required.gif"
                                                                                                                                              alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                                                                </label>
                                                            </td>
                                                            <td>
                                        ...
                                        <tr>
                                            <td style="vertical-align: top; padding-top: 20px">
                                                <fieldset>
                                                    <legend>
                                                        <loc:text resourceid="Timer_Control_Dlg_Section_DataSource" runat="server"/>
                                                    </legend>
                                                    <table width="100%" cellspacing="5" cellpadding="0" style="table-layout: fixed;">
                                                        <tr>
