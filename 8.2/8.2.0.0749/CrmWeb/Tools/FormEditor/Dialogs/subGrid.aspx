    ...
    function OnSubGridWindowLoad() {
        ...
        if (!IsNull(oLabels) && !IsNull(oLabels.length) && oLabels.length > 0) {
            for (var i = 0; i < oLabels.length; i++) {
                if (!IsNull(oLabels[i]) &&
                    !IsNull(oLabels[i].LanguageCode) &&
                    !IsNull(USER_LANGUAGE_CODE) &&
                    oLabels[i].LanguageCode == USER_LANGUAGE_CODE &&
                    !IsNull(oLabels[i].Description)) {
        ...
        if (containsEditableGrid && isDashboardEditor !== true) {
            ...
            if (!_isEventsTabInitialized) {
                Mscrm.FormLibraryAndEventHandlerUtils
                    .loadEventsTab(_oFormXml, _oFieldsXml, _oFieldPropertiesXml, _cellId, _objectCode);
...
*********************************************************************/
                ...
                if (chartsEnabled || isChartListEmpty) {
                    ...
                    try {
                        ...
                        if (_sDashboardType == Mscrm.EntityTypeCode.InteractionCentricSystemForm) {
                            command = new RemoteCommand("FormEditorWebService",
                                "GetVisualizationSelectorHtmlForInteractionCentric");
                        } else {
            ...
            if (value.length > 0) {
                ...
                try {
                    ...
                    if (result.Success) {
                        ...
                        ConstructViewList(retVal,
                            'ViewListSelector',
                            $get('viewSelection'),
                            $get('ViewsAvailable'),
                            handleViewListChange,
                            $get('ViewComboSelector'));
                    }
                } catch (e) {
                }
            }
    ...
    function populateEntityData(sRecordType) {
        ...
        if (_allEntities != null) {
            ...
            for (i = 0; i < len; i++) {
                ...
                var isInteractionCentricEnabledEntities = XUI.Xml
                    .GetText(XUI.Xml.SelectSingleNode(xmlDoc, "EntityInfo/IsInteractionCentricEnabled", null));
                var isVisibleInMobileClient = XUI.Xml
                    .GetText(XUI.Xml.SelectSingleNode(xmlDoc, "EntityInfo/IsVisibleInMobileClient", null));
                if (XUI.Xml.GetText(XUI.Xml.SelectSingleNode(xmlDoc, "EntityInfo/LogicalName", null)) ===
                    Mscrm.InternalUtilities.EntityNames.SharePointDocument) {
                    continue;
                }
    ...
    function IsUserViewType() {
        ...
        if (!IsNull(viewSelector) && viewSelector.selectedIndex != -1) {
            return parseInt(viewSelector.options[viewSelector.selectedIndex].getAttribute("Type"), 10) ==
                Mscrm.EntityTypeCode.UserQuery;
        }
        return false;
    }
    ...
    function IsUserVizType() {
        ...
        if (!IsNull(visualizationList) && visualizationList.selectedIndex != -1) {
            return parseInt(visualizationList.options[visualizationList.selectedIndex].getAttribute("type"), 10) ==
    ...
    function applychanges() {
        ...
        if (_subGridId != _txtDesc.value) {
            var oNodes = null;
            if (_isReferencePanelSubGrid) {
                oNodes = XUI.Xml.SelectNodes(_oFormXml,
                    "/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell/control[@classid = '" +
        ...
        if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) &&
            !IsNull(Mscrm.CustomControls) &&
            !IsNull(Mscrm.CustomControls.CustomControlManager.get_instance())) {
            var validationCheck = Mscrm.CustomControls.CustomControlManager.get_instance()
                .validateFieldCustomControls();
            if (!validationCheck) {
                return;
            }
            ...
            var currentEntityTypeCode = Mscrm.CustomControls.CustomControlManager.get_instance()
                .get_currentEntityTypeCode();
            var isNoCustomControlAdded = Mscrm.CustomControls.CustomControlManager.get_instance()
                .get_existingCustomControlsList().length -
                Mscrm.CustomControls.CustomControlManager.get_instance().get_existingEntityCustomControlsList()
                .length <=
                1;
        ...
        if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) &&
            ...
            Mscrm.CustomControls.CustomControlManager.get_instance()
                .setSubgridParameters(_oCustomControlSnippet,
                    viewID,
                    _bIsUserView,
                    sRelationShipName,
    ...
    function getGutter() {
        var gutter = GetGutterValue(_showChartsOnly.checked,
            $get('viewSelection').selectedIndex,
            _searchBox.checked,
            _chkDispOption.checked,
            _indexBox.checked,
            (_sEditorType === 'dashboardEditor'));
        return gutter;
    }
        ...
        if (entityId != null && objectCode != null) {
            var url = Mscrm.CrmUri
                .create(formatString("/tools/vieweditor/viewManager.aspx?objectTypeCode={0}&mode=new&entityId={1}",
                    CrmEncodeDecode.CrmUrlEncode(objectCode),
                    CrmEncodeDecode.CrmUrlEncode(entityId)));
            _viewWindow = openStdWin(url,
                buildWinName(),
                _oConst.iAttributeEditorWidth,
                _oConst
                .iAttributeEditorHeight);
        }
    }
    ...
    function editView() {
        ...
        if (entityId != null && objectCode != null && viewId != null) {
            var url = Mscrm.CrmUri.create(formatString("/tools/vieweditor/viewManager.aspx?id={0}&entityId={1}",
...
<div id="divRefPanDetails" style="display: none;">
    <fieldset>
        ...
        <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
            ...
            <tr>
                <td class="ms-crm-Field-Required" id="iconresource_c">
                    <label>
                        <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.section.aspx_350" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                    </label>
                </td>
                <td>
                    <sdk:LookupControl id="iconResources" DefaultViewId="DCAC5DFC-2EFF-42b3-9A0D-AFF1577533EB" DisableViewPicker="true" LookupBrowse="false" LookupStyle="single" runat="server"/>
                </td>
            </tr>
        </table>
    </fieldset>
        ...
        <tr>
            <td class="ms-crm-Field-Required" id="name_c">
                <label for="txtDescription">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.section.aspx_200" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                </label>
            </td>
            <td>
                <ui:TextBox id="txtDescription" MaxLength="255" runat="server"/>
            </td>
        </tr>
    </table>
</fieldset>
...
<fieldset>
    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        <col width="15"/><col width="70"/>
        <tr>
            <td class="ms-crm-Field-Required" id="label_c">
                <label for="txtLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.Label" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                </label>
            </td>
            <td>
                <input type="text" id="txtLabel" onkeypress="_labelDirty = true;" maxlength="255" class="ms-crm-Text"/>
            </td>
        </tr>
    </table>
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td>
                <ui:CheckBox id="chkDispOption" runat="server"/>
            </td>
            <td >
                <label for="chkDispOption" id="chkDispOptionLabel" runat="server"></label>
            </td>
        </tr>
...
<fieldset>
    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td >
                <label >
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.RecordsLabel" runat="server"/>
                </label>
            </td>
            <td id="Records"></td>
        </tr>
        <tr>
            <td >
                <label >
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.DataSource" runat="server"/>
                </label>
            </td>
            <td id="DataSource"></td>
        </tr>
        ...
        <tr id="TeamTemplateRow" style="display: none">
            <td>
                <label>
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.TeamTemplateView" runat="server"/>
                </label>
            </td>
            <td id="TeamTemplateCell"></td>
            <td id="TeamTemplateErrorCell" style="display: none">
                <label>
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.NoTeamTemplateErrorMessage" runat="server"/>
                </label>
            </td>
        </tr>
    </table>
    ...
    <table id="newViewTable" cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td/>
            <td>
                <ui:Button id="editViewId" class="ms-crm-RefreshDialog-Button" onclick="editView();" ResourceId="Edit_View_From_SubGrid" runat="server"/></ui:Button>
            </td>
            <td>
                <ui:Button id="newViewId" class="ms-crm-RefreshDialog-Button" onclick="createView();" ResourceId="New_View_From_SubGrid" runat="server"/></ui:Button>
            </td>
            <td/>
        </tr>
    </table>
...
<fieldset>
    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td>
                <ui:CheckBox id="searchBox" onclick="EnableDisableShowLabel();" runat="server"/>
            </td>
            <td colspan='2'>
                <label for="searchBox" id="searchBoxLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.SearchBox" runat="server"/>
                </label>
            </td>
        </tr>
        ...
        <tr>
            <td>
                <ui:CheckBox id="indexBox" runat="server"/>
            </td>
            <td colspan='2'>
                <label for="indexBox" id="indexBoxLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.Index" runat="server"/>
        ...
        <tr>
            <td colspan='2'>
                <label id="viewSelectionLabel"><loc:Text ResourceId="Web.DashboardEditor.ViewSelectorLabel" runat="server"/>&nbsp;</label>
            </td>
...
<fieldset>
    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td colspan='2'>
                <label for="DefaultViz" id="DefaultVizLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.SubGrid.aspx.DefaultChart" runat="server"/>
                </label>
            </td>
            <td id="DefaultViz"></td>
        </tr>
        ...
        <tr>
            <td>
                <ui:CheckBox id="showChartsOnly" onclick="handleChartSelection();" runat="server"/>
            </td>
            <td colspan='2'>
                <label for="showChartsOnly" id="showChartsOnlyLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.SubGrid.aspx.DisplayChartOnly" runat="server"/>
                </label>
            </td>
        </tr>
        ...
        <tr>
            <td>
                <ui:CheckBox id="visualizationBox" onclick="handleChartSelection();" runat="server"/>
            </td>
            <td colspan='2'>
                <label for="visualizationBox" id="visualizationBoxLabel">
                    <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.aspx.DisplayVisualization" runat="server"/>
                </label>
            </td>
        </tr>
    </table>
...
<div id="divAvailabilitySection">
    ...
    <fieldset>
        ...
        <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
            ...
            <tr>
                <td colspan="2">
                    <loc:Text ResourceId="FormEditor_Dialogs_Chart_Available_Section_Heading" runat="server"/>
                </td>
            </tr>
...
<fieldset>
    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            <td colspan="3">
                <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.SubGrid.aspx.RowsLegend.Description" runat="server"/>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <ui:Number id="RowCount" runat="server"/>
            </td>
        </tr>
    ...
    <table id="disableAutoExpandingMessage" style="display: none" cellpadding="0" width="100%" style="table-layout: fixed;">
        ...
        <tr>
            ...
            <td>
                <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.subgrid.disabledAutoExpanding" runat="server"/>
            </td>
        </tr>
    </table>
