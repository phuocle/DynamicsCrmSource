...
<script type="text/javascript" language="javascript">
    ...
    var invalidFormTypeSelectedText =
        <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(invalidFormTypeSelectedCaption) %>;
    var invalidFormPresentationSelectedText =
        <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(invalidFormPresentationSelectedCaption) %>;
    Mscrm.FormEditorVariables.webResourceRootUrl = '<%= (webResourceRootUrl) %>';
    Mscrm.FormEditorVariables
        .canAssignRoles = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(canAssignRoles.ToString().ToLower()) %>;
    Mscrm.FormEditorVariables
        .isCustomizabe = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(canCustomize.ToString().ToLower()) %>;
    Mscrm.FormEditorVariables
        .canCreateForms = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(canCreateForms.ToString().ToLower()) %>;
    Mscrm.FormEditorVariables
    ...
    var _dropItem = null;
    Mscrm.FieldExplorerUtils
        .fieldExplorerCaption = '<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlEncode(fieldExplorerCaption).ToString() %>';
    Mscrm.FieldExplorerUtils
        .relationshipExplorerCaption =
        '<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlEncode(relationshipExplorerCaption).ToString() %>';
    Mscrm.FieldExplorerUtils
        .businessRulesExplorerCaption =
        '<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlEncode(businessRulesExplorerCaption).ToString() %>';
    Mscrm.FormEditorVariables
        .currentFormId = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(formId.ToString("B")) %>;
    Mscrm.FormEditorVariables.parentFormId = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(parentFormId) %>;

    Mscrm.FormEditorVariables
        .entityUnpublishedLabel = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(unpublishedEntityName) %>;
    Mscrm.FormEditorVariables
        .entityLogicalName = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(unpublishedLogicalName) %>;
    ...
    function hideProcessControl() {
        var processControlNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,
            "//control[@classid='{06E9F7AF-1F54-4681-8EEC-1E21A1CEB465}']",
            null);
    ...
    function MergeEvents(selectedFormXml) {
        ...
        for (var i = 0; i < eventsList.length; i++) {
            ...
            if (IsNull(existingEvent)) {
                ...
                AppendEventHandlersIfNotDuplicate(eventName,
                    eventPath,
                    "InternalHandlers",
                    eventsList[i],
                    existingEvent);
    ...
    function AppendEventHandlersIfNotDuplicate(eventName, eventPath, handlerNode, event, existingEvent) {
        ...
        for (var j = 0; j < handlersList.length; j++) {
            ...
            var existingEventHandler = XUI.Xml
                .SelectSingleNode(existingEvent,
                    eventPath +
                    "/" +
                    handlerNode +
    ...
    function MergeResources(selectedFormXml) {
        ...
        var currentXmlClientResources = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form/clientresources", null);
        var currentXmlInternalResources = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables
                .formXml,
                "/entity/form/clientresources/internalresources",
                null);


        var selectedXmlClientIncludes = XUI.Xml
            .SelectSingleNode(selectedFormXml, "/entity/form/clientresources/internalresources/clientincludes", null);
        var currentXmlClientIncludes = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables.formXml,
                "/entity/form/clientresources/internalresources/clientincludes",
                null);
        ...
        var selectedXmlIsvClientIncludes = XUI.Xml
            .SelectSingleNode(selectedFormXml, "/entity/form/clientresources/isvresources/clientincludes", null)
        var currentXmlIsvClientIncludes = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables.formXml,
                "/entity/form/clientresources/isvresources/clientincludes",
                null);
        if (!(IsNull(currentXmlIsvClientIncludes) || IsNull(selectedXmlIsvClientIncludes)))
            currentXmlIsvClientIncludes = CheckDuplicates(currentXmlIsvClientIncludes,
                selectedXmlIsvClientIncludes,
                'path');
            ...
            if (IsNull(currentXmlClientResources)) {
                ...
                XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form", null)
                    .appendChild(newIsvClientIncludes);
            } else
                currentXmlClientResources.appendChild(selectedXmlIsvClientIncludes.parentNode);
        }


        var selectedXmlClientVariables = XUI.Xml
            .SelectSingleNode(selectedFormXml, "/entity/form/clientresources/internalresources/clientvariables", null)
        var currentXmlClientVariables = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables.formXml,
                "/entity/form/clientresources/internalresources/clientvariables",
                null);
        if (!(IsNull(currentXmlClientVariables) || IsNull(selectedXmlClientVariables)))
            ...
            if (IsNull(currentXmlClientResources)) {
                ...
                XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form", null)
                    .appendChild(newClientvariables);
            } else if (IsNull(currentXmlInternalResources)) {
                var newInternalResourcesNode = Mscrm.FormEditorVariables.formXml.createElement('internalresources');
        ...
        var currentXmlFormLibraries = XUI.Xml
            .SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form/formLibraries", null);
        if (!(IsNull(currentXmlFormLibraries) || IsNull(selectedXmlFormLibraries)))
            ...
            XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form", null)
                .appendChild(selectedXmlFormLibraries);
        }
    }


    function EnableMergeButton() {
        if (Mscrm.FormEditorVariables.FormType == 'main' &&
            _isRefreshForm == '1' &&
            _isFormMergeFcbEnabled == "True" &&
    ...
    function window_onbeforeunload(oEvent) {
        if (!_bSaving) {
            ...
            if (!IsNull(_previewer)) {
                SetPreviewXml();
                if ((_previewer.fieldXml.getAttribute("defaultValue") != _previewer.fieldXml.value) ||
                    (_previewer.propertiesXml.getAttribute("defaultValue") != _previewer.propertiesXml.value) ||
                    (_previewer.formXml.getAttribute("defaultValue") != _previewer.formXml.value)) {
    ...
    function SetPreviewXml() {
        ...
        var cells = XUI.Xml.SelectNodes(previewXml,
        ...
        if (Mscrm.FormUtils.isFormDirty() || bClose) {
            if (!_bSaving) {
                ...
                if (bClose) {
                    ...
                    var retValues = Mscrm.PageManager.get_instance()
                        .raiseEvent(Mscrm.ScriptEvents.InsertCacheData, parameters);
                                ...
                                <tr>
                                    <td>
                                        <a class="ms-crm-Menu-Label">
                                            <SPAN id="expandedExplorer"" class="ms-crm-MenuItem-TextRTL" style="font-size: 12px; font-weight: bold"><%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlEncode(fieldExplorerCaption) %></SPAN>
                                        </a>
                                    </td>
                                    <td id="fldExplorerImgContainer">
                                        <a class="ms-crm-Menu-Label" onclick="hideAttrExp()">
                                            <IMG id='shownExplorerImage' class="Field-Explorer" alt="" src="/_imgs/Tools/ico_form_assistexpanded.png"/>
                                        </a>
                                    </td>
                                </tr>
                                </tbody>
                    ...
                    <tr>
                        <td>
                            <table width="100%" height="100%" style="table-layout: fixed; border-bottom-style: solid; border-color: #a5acb5; border-width: 1px;">
                                    ...
                                    <td colspan="2">
                                        <b>
                                            <label for="txtDescription">
                                                <loc:Text ResourceId="Form_Editor_Filter_Caption" runat="server"/>
                                            </label>
                                        </b>
                                    </td>
                                    <td id="fieldFilterTD">
                                ...
                                <tr>
                                    <td>
                                        <ui:CheckBox id="unusedFilter" onclick="Mscrm.FieldExplorerUtils.handlerFilterCheckBoxClick();" TabIndex="10" runat="server"/>
                                    </td>
                                    <td colspan="2">
                                        <span id="unusedFieldLabelText" style="display: none;">
                                            <loc:Text ResourceId="Formeditor_FieldExplorer_CheckBox_Text" runat="server"/>
                                        </span>
                                        <span id="unusedRelationshipLabelText" style="display: none;">
                                            <loc:Text ResourceId="Formeditor_RelationshipExplorer_CheckBox_Text" runat="server"/>
                                        </span>
                                        <span id="fieldExplorerImageAltText" style="display: none;">
                                            <loc:Text ResourceId="Formeditor_FieldExplorer_Image_AltText" runat="server"/>
                                        </span>
                                        <span id="relationshipExplorerImageAltText" style="display: none;">
                                            <loc:Text ResourceId="Formeditor_RelationshipExplorer_Image_AltText" runat="server"/>
                                        </span>
                                        <label for="unusedFilter" id="unusedFilterLabel">
                                            <loc:Text ResourceId="Formeditor_FieldExplorer_CheckBox_Text" runat="server"/>
                                        </label>
                                    </td>
                                </tr>
            ...
            <tr height="20px">
                <td>
                    <center style="width: 100%">
                        <IMG id='hiddenExplorerImage' class="Field-Explorer" alt="" src="/_imgs/Tools/ico_form_assistcollapsed.png"/>
                    </center>
                </td>
            </tr>
