...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="RenderForm">
    <xsl:if test="$editorType='formEditor' and $formType!='quick' and $formType!='quickCreate'">
      <div id="formNavigationRoot" class="form-editor-nav-style"
           style="position:absolute; height:100%; width:180px; top:0px;">
        <!--FormName-->
        <br />
                ...
                <xsl:for-each select="tabs/tab">
                  <li class="ms-crm-FormSelector">
                    ...
                    <a class="ms-crm-FormSelector-SubItem">
                      <img align="absMiddle" class="ms-crm-ImageStrip-formNavTreeLine ms-crm-FormSelector-SubItem"
                           alt="" src="/_imgs/imagestrips/transparent_spacer.gif" complete="complete">
                        <xsl:if test="$RTL='true'">
              ...
              <tr>
                <td id="tabs">
                  ...
                  <xsl:for-each select="tabs/tab">
                    ...
                    <div>
                      <table class="tab" valign="top" width="100%"
                             style="table-layout: fixed;border:1px dashed #6d6e70;">
                        <xsl:if test="$isCustomizable='true'">
                          <xsl:attribute name="tabindex">10</xsl:attribute>
                          <xsl:attribute name="hidefocus">true</xsl:attribute>
                          <xsl:attribute name="onfocus">SetFocusedElementActive(Mscrm.Utilities.eventToDomEvent(event), this);</xsl:attribute>
                        ...
                        <tr>
                          <td name="tabData">
                            <a name="tabHeader" class="ms-crm-Menu-Label">
                              ...
                              <SPAN name="tabText" class="ms-crm-InlineTabHeaderText"
  ...
  <xsl:template name="RenderNavigation">
    <xsl:for-each select="Navigation/NavBarAreas/NavBarArea">
      <li tabindex="10" hidefocus="true"
          onfocus="SetFocusedElementActive(Mscrm.Utilities.eventToDomEvent(event), this);" class="navGroup">
        <xsl:if test="$isCustomizable='true'">
          <xsl:attribute name="onmouseover">
        ...
        <ul>
          ...
          <xsl:for-each select="../../NavBar/child::*[@Area=$areaId]">
            <xsl:if
              test="(name()='NavBarByRelationshipItem' and (not(@Show) or @Show != 'false')) or (name()='NavBarItem')">
              <li class="navItem">
                <xsl:if test="$isCustomizable='true'">
                  <xsl:attribute name="tabindex">10</xsl:attribute>
                  <xsl:attribute name="hidefocus">true</xsl:attribute>
      ...
      <tr>
        <td class="section">
          ...
          <xsl:if test="@showlabel = 'false' and @showbar = 'false'">
            <xsl:attribute name="style">
              color: #cccccc;<xsl:value-of select="$specialColumnSectionPadding" />
            </xsl:attribute>
          </xsl:if>
          <xsl:if test="@showlabel = 'true' and @showbar = 'true'">
            <xsl:attribute name="style">
              border-bottom: 1px solid #606050;
              <xsl:value-of select="$specialColumnSectionPadding" />
            </xsl:attribute>
          </xsl:if>
          <xsl:if test="@showlabel = 'false' and @showbar = 'true'">
            <xsl:attribute name="style">
              color: #cccccc;border-bottom: 1px solid #606050;
              <xsl:value-of select="$specialColumnSectionPadding" />
            </xsl:attribute>
          </xsl:if>
          <xsl:if test="@locklevel and @locklevel = '1'">
  ...
  <xsl:template match="footer/rows/row/cell">
    ...
    <xsl:variable name="isScriptRequired"
                  select="/entity/formeditor/form/tabs/tab/columns/column/sections/section/rows/row/cell/events/event/dependencies/dependency[@id = $fieldName] or
            ...
            <xsl:choose>
              <xsl:when test="$cellLabelPosLeft">
                ...
                <td valign="top" nowrap="" class="field">
                  ...
                  <xsl:if test="$fieldName">
                    <xsl:value-of
                      select="/entity/properties/fields/field[@name = $fieldName]/displaynames/displayname[@languagecode = $languageCode]/@description" />
                  </xsl:if>
                </td>
  ...
  <xsl:template match="header/rows/row/cell">
    ...
    <xsl:variable name="isLocked"
                  select="(@locklevel and @locklevel = '1') or (not($isPreviewMode) and ((/entity/metadata/fields/field[@name = $fieldName and @requiredforform = 'true']) or (/entity/properties/fields/field[@name = $fieldName and @requiredlevel = 'systemrequired'])))" />
    <xsl:variable name="isScriptRequired"
                  select="/entity/formeditor/form/tabs/tab/columns/column/sections/section/rows/row/cell/events/event/dependencies/dependency[@id = $fieldName] or
								/entity/form/header/rows/row/cell/events/event/dependencies/dependency[@id = $fieldName] or
								/entity/form/footer/rows/row/cell/events/event/dependencies/dependency[@id = $fieldName] or
              ...
              <xsl:when test="$cellLabelPosLeft">
                <xsl:if test="not(@showlabel) or @showlabel='true'">
                  <td style="vertical-align: top;height: 100%" class="label">
                    ...
                    <xsl:value-of select="labels/label[@languagecode = $languageCode]/@description" />
                    <xsl:if
                      test="/entity/metadata/fields/field[@name = $fieldName and @requiredforplatform = 'true'] or /entity/properties/fields/field[@name = $fieldName and @requiredlevel = 'required']">
                    ...
                    <xsl:if
                      test="/entity/properties/fields/field[@name = $fieldName and @requiredlevel = 'recommended']">
  ...
  <xsl:template match="section/rows/row/cell">
    ...
    <xsl:variable name="isWebResource"
                  select="control[starts-with(@classid,'{9FDF5F91-88B1-47f4-AD53-C11EFC01A01D}')] or control[starts-with(@classid,'{587CDF98-C1D5-4bde-8473-14A0BC7644A7}')] or control[starts-with(@classid,'{080677DB-86EC-4544-AC42-F927E74B491F}')]" />
    <xsl:variable name="isSubGrid"
                  select="control[(starts-with(@classid,'{E7A81278-8635-4d9e-8D4D-59480B391C5B}')) or (@indicationOfSubgrid = 'true')]" />
    ...
    <xsl:variable name="isNotes"
                  select="control[starts-with(@classid,'{06375649-c143-495e-a496-c962e5b4488e}')] or control[starts-with(@classid,'{06375649-C143-495E-A496-C962E5B4488E}')]" />
    <xsl:variable name="isUserSpacer" select="control[starts-with(@id,'spacer_User')]" />
    <xsl:variable name="isSystemSpacer" select="control[starts-with(@id,'spacer_System')]" />
    ...
    <xsl:variable name="isScriptRequired"
                  select="/entity/formeditor/form/tabs/tab/columns/column/sections/section/rows/row/cell/events/event/dependencies/dependency[@id = $fieldName] or
								...
								/entity/form/externaldependencies/dependency[@id = $fieldName]" />
    <xsl:variable name="isQuickFormCollection"
                  select="control[starts-with(@classid,'{5C5600E0-1D6E-4205-A272-BE80DA87FD42}')]" />
    <xsl:variable name="isReferencePanelQuickFormCollection"
                  select="control[starts-with(@classid,'{B68B05F0-A46D-43F8-843B-917920AF806A}')]" />
    <xsl:variable name="isReferencePanelSubGrid"
                  select="control[starts-with(@classid,'{02D4264B-47E2-4B4C-AA95-F439F3F4D458}')]" />
    <xsl:variable name="isInteractionWall"
                  select="control[starts-with(@classid,'{1F179106-FA28-4495-961E-F6BD93C21974}')]" />
    <xsl:variable name="isBingMap" select="control[starts-with(@classid,'{62B0DF79-0464-470F-8AF7-4483CFEA0C7D}')]" />
    <xsl:variable name="isSocialInsight"
                  select="control[starts-with(@classid,'{86B9E25E-695E-4FEF-AC69-F05CFA96739C}')]" />
    <xsl:variable name="isOrgInsights" select="control[starts-with(@classid,'{76B9E25E-695E-4FEF-AC69-F05CFA96739C}')]" />
    <xsl:variable name="isTimerControl"
                  select="control[starts-with(@classid,'{9C5CA0A1-AB4D-4781-BE7E-8DFBE867B87E}')]" />
    <xsl:variable name="isSearchWidget"
                  select="control[starts-with(@classid,'{E616A57F-20E0-4534-8662-A101B5DDF4E0}')]" />
    <xsl:variable name="isReferencePanelSearchWidget"
                  select="control[starts-with(@classid,'{7CCD1494-1F7A-4E3A-8BDE-F32069DAEB9F}')]" />
      ...
      <xsl:if test="$fieldName">
        <xsl:choose>
          ...
          <xsl:when test="$isSocialInsight">
            ...
            <xsl:attribute name="style">
              <xsl:text>border:1px dashed rgb(109, 110, 112);</xsl:text>
            </xsl:attribute>
          </xsl:when>
  ...
  <xsl:template name="RenderDashboardEditorCellContent">
    ...
    <xsl:variable name="desiredcontainerheight">
      <xsl:value-of select="($rowspan*$rowheight)+($rowspan*$cellpadding)-$cellpadding" />
      <!-- the height of the container will be equal to the rowspan*height of each row + the padding occupied by each row-->
    </xsl:variable>
    <xsl:if test="$isCustomizable='true'">
      <xsl:attribute name="onfocus">SetFocusedElementActive(Mscrm.Utilities.eventToDomEvent(event), this);</xsl:attribute>
      <xsl:attribute name="hidefocus">true</xsl:attribute>
      <xsl:attribute name="onmouseover">Mscrm.DashboardEditorUtils.registerDragObjectForCell(Mscrm.Utilities.eventToDomEvent(event), this);</xsl:attribute>
      ...
      <div>
        ...
        <table cellspacing="0" cellpadding="0" class="cell">
          ...
          <xsl:choose>
            ...
            <xsl:when test="$isSubGrid">
              <xsl:choose>
                <xsl:when test="$chartGridMode='Chart'">
                  <xsl:if
                    test="$isSubGrid/parameters/EnableViewPicker and $isSubGrid/parameters/EnableViewPicker='true'">
                  ...
                  <xsl:variable name="headerLabel">
                    <xsl:choose>
                      <xsl:when
                        test="$isSubGrid/parameters/EnableChartPicker and $isSubGrid/parameters/EnableChartPicker='true'">
                  ...
                  <xsl:call-template name="ComponentHeader">
                    ...
                    <xsl:with-param name="Selector"
                                    select="$isSubGrid/parameters/EnableChartPicker and $isSubGrid/parameters/EnableChartPicker='true'" />
                  </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
                  ...
                  <xsl:variable name="headerLabel">
                    <xsl:if
                      test="$isSubGrid/parameters/TargetEntityType != $officeGraphDocument and $isSubGrid/parameters/GridUIMode != $card">
                  ...
                  <xsl:call-template name="ComponentHeader">
                    <xsl:with-param name="Label" select="$headerLabel" />
                    <xsl:with-param name="Selector"
                                    select="$isSubGrid/parameters/EnableViewPicker and $isSubGrid/parameters/EnableViewPicker='true'" />
                  </xsl:call-template>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:when>
            ...
            <xsl:when test="$isIframe or $isWebResource or $isSocialInsight or $isPowerBITile">
              <xsl:variable name="headerFormat">
                <xsl:choose>
                  <xsl:when test="$isIframe">
          ...
          <tr>
            <xsl:variable name="toprowheight">24</xsl:variable>
            <!-- there is currently only one row that we are concerened here. Chart only can have multiple row-->
            <!--Get the no of selector in the cell. Only chart with a view picker has 2 cell-->
            <xsl:variable name="selectorcount">
              <xsl:choose>
                <xsl:when
                  test="$chartGridMode='Chart' and $isSubGrid/parameters/EnableViewPicker and $isSubGrid/parameters/EnableViewPicker='true'">
                  2
                </xsl:when>
            ...
            <xsl:variable name="previewcellheight">
              <xsl:value-of select="(($desiredcontainerheight)-($selectorcount*$toprowheight)-2)" />
              <!-- 2 is for border -->
            </xsl:variable>
            ...
            <td nowrap="" class="dashboardfield">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="$chartGridMode='Chart'">
                    ...
                    <xsl:variable name="previewFrameId">
                      previewFrame_<xsl:value-of select="$fieldName" />
                    </xsl:variable>
                    <xsl:variable name="loadingDivId">
                      previewLoadingDiv_<xsl:value-of select="$fieldName" />
                    </xsl:variable>
                    ...
                    <xsl:element name="iframe">
                      ...
                      <xsl:attribute name="style">
                        ...
                        <xsl:value-of select="$previewcellheight" />px
                      </xsl:attribute>
              ...
              <xsl:if
                test="$isSubGrid/parameters/TargetEntityType = $officeGraphDocument and $isSubGrid/parameters/GridUIMode = $card">
                <xsl:attribute name="style">
                  width:100%;text-align:center;height:
                  <xsl:value-of select="$previewcellheight" />px;margin-top:1px;
                </xsl:attribute>
                <xsl:value-of select="//resources/resource[@id = 'DelvePlaceholder']" />
              </xsl:if>
              <xsl:if test="$isPowerBITile">
                <xsl:variable name="previewFrameId">
                  powerBITilePreviewFrame_<xsl:value-of select="$fieldName" />
                </xsl:variable>
                <xsl:element name="iframe">
                  ...
                  <xsl:attribute name="tabIndex">-1</xsl:attribute>
                  <xsl:attribute name="style">
                    width:100%;height:<xsl:value-of select="$previewcellheight" />
                    px;margin-top:1px;
                  </xsl:attribute>
                  <xsl:attribute name="onload">
              ...
              <xsl:if test="$isPreviewCell">
                ...
                <xsl:if test="$supportInteractionCentric='false'">
                  <a id="chartLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addSubgridFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                  ...
                  <a id="gridLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addSubgridFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), false, true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                  ...
                  <br />
                  <a id="IframeLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addIframeFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), false, true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                  ...
                  <a id="WebResourceLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent()" tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addIframeFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                ...
                <xsl:if test="$supportInteractionCentric='true'">
                  ...
                  <xsl:if test="@istilecell='true'">
                    <a id="TilesLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                       onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                       onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                       onmousemove="Mscrm.PreviewCellUtils.cancelEvent()" tabIndex="10"
                       onclick="Mscrm.PreviewCellUtils.addTilesFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true);"
                       href="javascript:;" type="placeholder">
                      <xsl:element name="img">
                        <xsl:attribute name="src">
                  ...
                  <xsl:if test="@isstreamcell='true'">
                    <a id="QueuesLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                       onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                       onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                       onmousemove="Mscrm.PreviewCellUtils.cancelEvent()" tabIndex="10"
                       ondblclick="Mscrm.PreviewCellUtils.stopPropagationFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event));"
                       onclick="Mscrm.PreviewCellUtils.addQueuesFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true);"
                       href="javascript:;" type="placeholder">
                      <xsl:element name="img">
                        <xsl:attribute name="src">
                ...
                <xsl:if test="$supportSocialInsights='true'">
                  <a id="SocialInsightLink" class="PlaceholderAnchor"
                     onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addSocialInsightFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                ...
                <xsl:if test="$supportPowerBITile='true'">
                  <a id="PowerBITileLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addPowerBITileFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                ...
                <xsl:if test="$supportOrgInsights='true'">
                  <a id="OrgInsightsLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addOrgInsightsFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                ...
                <xsl:if test="$supportDelve='true'">
                  <a id="DelveLink" class="PlaceholderAnchor" onmouseover="Mscrm.PreviewCellUtils.onHover(this)"
                     onmousedown="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     onmouseout="Mscrm.PreviewCellUtils.offHover(this)"
                     onmousemove="Mscrm.PreviewCellUtils.cancelEvent(Mscrm.Utilities.eventToDomEvent(event))"
                     tabIndex="10"
                     onclick="Mscrm.PreviewCellUtils.addDelveFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true);"
                     href="javascript:;" type="placeholder">
                    <xsl:element name="img">
                      <xsl:attribute name="src">
    ...
    <table class="GridPreviewTable" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
      ...
      <tr class="ms-crm-List-Header">
        <td style="width:42px; text-align: center">
          <input type="checkbox" onclick="return false;" tabIndex="-1"></input>
        </td>
        <td class="ms-crm-List-Spacer">
          <img alt="" src="/_imgs/grid/bar_line.gif"></img>
        </td>
