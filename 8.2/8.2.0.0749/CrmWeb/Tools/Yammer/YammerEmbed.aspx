...
<div id="noFollowNoPermission" class="message" style="display: none;">
    ...
    <div class="warningText" tabindex="0">
        <loc:text resourceid="Web.Tools.Yammer.NoFollowNoPermission" runat="server"/>
    </div>
</div>
<div id="followNotInYammer" class="message" style="display: none;">
    <img class="warningImage" src="/_imgs/imagestrips/transparent_spacer.gif"/>
    <div class="warningText" tabindex="0">
        <loc:text resourceid="Web.Tools.Yammer.FollowNotInYammer" runat="server"/>
    </div>
</div>
...
<div id="loginBox">
    <% if (IsOnPremise)
       { %>
        <p tabindex="0">
            <loc:text resourceid="Web.Tools.Yammer.Embed.OnPremiseVerificationInvite" runat="server"/>
        </p>
        <p>
...
<script type="text/javascript" language="javascript">
    var activityFeedsService = window.ActivityFeeds && window.ActivityFeeds.UI
        ? ActivityFeeds.UI.IActivityFeedService
        : window.parent.ActivityFeeds.UI.IActivityFeedService;
    ...
    var neverConfigured = "<%= UserNeverConfigured %>" == "True";
    var noConversations = "<%= NoConversations %>" == "True";
    ...
    function checkYammerSecurity() {
        var noSecurityCheckRequired = IsNull(window.parent) ||
            IsNull(Mscrm.PageManager.getParentWindowXrmInternal().getServiceDirectory()) ||
            window.parent.YAMMER_POST_METHOD != 1;

        if (noSecurityCheckRequired)
            return true;

        var serviceDirectory = Mscrm.PageManager.getParentWindowXrmInternal().getServiceDirectory();
        var activityFeedService = serviceDirectory.find(activityFeedsService);
        if (!activityFeedService) {
            Xrm.Internal.trace.warning("YammerEmbed",
                "Yammer security check skipped: IActivityFeedService was not found");
            return true;
        }

        var isFollowed = activityFeedService.get_isFollowed();
        var isFollowedInYammer = activityFeedService.get_isFollowedInYammer();
    ...
    function embed(e) {
        ...
        if (checkYammerSecurity()) {
            ...
            Xrm.Internal.trace
                .log("YammerEmbed", "embedding Yammer with {0}", JSON.stringify(<%= FeedOptionsString %>));
            pageState = "embedded";
        }
    }
    ...
    if ((neverConfigured || noConversations) && userIdPresent) {
        showYammerDiv("#singleError");
    } else {
        $P_CRM(window).load(start);
    }
</script>
</body>
</html>