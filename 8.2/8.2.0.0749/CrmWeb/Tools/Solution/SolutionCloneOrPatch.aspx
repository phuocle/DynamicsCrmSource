...
<head>
    ...
    <style type="text/css">
        ...
        #txtPatchDisplayName_container { width: 100%; }
    ...
    <script type="text/javascript" language="JavaScript">
        ...
        function applychanges() {
            ...
            if (cloneType == 'CloneAsPatch') {
                patchOrSolutionVersion = document.getElementById("majorVersion").innerHTML +
                    "." +
                    $get("minorVersion").innerHTML +
                    "." +
                    $get("txtBuildVersion").value +
                    "." +
                    $get("txtRevisionVersion").value;
            }
            if (cloneType == 'CloneAsSolution') {
                patchOrSolutionVersion = $get("txtMajorVersion").value +
                    "." +
                    $get("txtMinorVersion").value +
                    "." +
                    $get("buildVersion").innerHTML +
                    "." +
                    $get("revisionVersion").innerHTML;
            }

            var solutionName = "<%= parentSolutionName %>";
            var displayName = $get("txtPatchDisplayName").value;
            Mscrm.SolutionHandler.cloneToPatchOrSolution(cloneType,
                solutionName,
                displayName,
                patchOrSolutionVersion,
                disableButton,
                postSave);
        }
        ...
        <table id="detailsTbl" cellpadding="0" cellspacing="2" style="width: 90%">
            <tr>
                <td style="width: 50%; display: inline">
                    <label id="baseSolutionName" title="<%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Base_Solution_Name")) %>"><%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Base_Solution_Name")) %></label>
                </td>
                <td style="width: 50%">
                    <%= parentSolutionName %>
                </td>
            ...
            <tr>
                <td style="width: 50%; display: inline">
                    <label id="patchDisplayName" for="txtPatchDisplayName" title="<%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Display_Name")) %>"><%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Display_Name")) %></label>
                </td>
                <td style="width: 50%">
            ...
            <tr>
                <td valign="top" style="width: 50%; display: inline">
                    <label id="patchVersion" title="<%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Version_Number")) %>"
                           ...
                           for="txtMajorVersion"
                           <% } %>>
                        <%= CrmEncodeDecode.CrmHtmlEncode(CurrentResourceManager.GetString("CloneSolution_Version_Number")) %>
                    </label>
                </td>
                <td style="width: 50%">
                    <div id="cloneDetails">
