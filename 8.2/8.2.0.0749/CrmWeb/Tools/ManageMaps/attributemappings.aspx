...
<script language=javascript>
    ...
    var entityName = "";
    var ignoreAtributeLabel =
        <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("DataManagement.ImportMap.AttributeMap.Ignored.Label")) %>;
    var unmappedAttributeLabel =
        <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("DataManagement.ImportMap.AttributeMap.Unmapped.Label")) %>;
    ...
    onload()
    {
        ...
        entityName =
            <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(Microsoft.Crm.Metadata.MetadataCache.GetInstance(new OrganizationContext(UserInformation.Current.OrganizationId)).GetEntity(entityTypeCode).LogicalName) %>;
        ...
        mapXml = "<Map Name=\"\"><Description></Description><EntityMaps><EntityMap TargetEntityName=\"" +
            entityName +
            "\" SourceEntityName=\"\"><AttributeMaps></AttributeMaps></EntityMap></EntityMaps></Map>";
    ...
    function UpdateCounts() {
        ...
        XUI.Html.SetText(document.getElementById("mappedHeadersCountLabel"),
            formatString(LOCID_MAPPED_HEADER_COUNT, headerMapCount, headerTotalCount));
        ...
        XUI.Html.SetText(document.getElementById("bizReqdMapCount"),
            formatString(LOCID_MAPPED_BIZATTRI_COUNT, bizreqMapCount, attriNodeList.length));
    ...
    function PopulateSourceAndTargetList() {
        ...
        for (var i = 0; i < sourceAttributelist.length; i++) {
            sourceAttributelistItemsHtml[i] =
                "<li class='ms-crm-Dialog-ListItem-ImportMap' onmouseover='OnSourceItem(this)' onmouseout='OffSourceItem(this)' onclick='SelectSourceItem(this)' title=\"" + CrmEncodeDecode.CrmHtmlAttributeEncode(sourceAttributelist[i]) + "\" id='sourceattributeitem" + i + "' style='width:196'>" + CrmEncodeDecode.CrmHtmlEncode(sourceAttributelist[i]) + "</li>";
            var concatenatedLogicalName = "";
            var displayHtml = "";
            var toolTipText = unmappedAttributeLabel;


            var xpath = "//AttributeMap[SourceAttributeName=\"" +
                escapeSingleQuotesAndBackSlashes(sourceAttributelist[i]) +
                "\" ]";
    ...
    function GetMappedPicklistAttributes() {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            ...
            if (!IsNull(targetNode)) {
                var xpath = "//Attribute[@logicalname=\"" +
                    XUI.Xml.GetText(targetNode) +
                    "\" and (@attributetype=\"picklist\" or @attributetype=\"boolean\" or @attributetype=\"bit\")]";
                ...
                if (!IsNull(nodeFound)) {
                    ...
                    if (!IsNull(sourceNode)) {
                        var xpath = "//Header[@name=\"" +
                            escapeSingleQuotesAndBackSlashes(XUI.Xml.GetText(sourceNode)) +
                            "\"]";
                        ...
                        if (!IsNull(node)) {
                            ...
                            for (var j = 0; j < dataNodes.length; j++) {
                                var xpathExp = "//PicklistMap[SourceValue=\"" +
                                    escapeSingleQuotesAndBackSlashes(XUI.Xml.GetText(dataNodes[j])) +
                                    "\"]";
    ...
    function SelectedAttributesChange() {
        ...
        case 0:
            ...
            for (var index = 0; index < businessRequiredArray.length; index++) {
                var displayHtml = GetHtmlWithImages(displayToLogicalNameArray[businessRequiredArray[index]],
                    businessRequiredArray[index]);
                var textIndentStyle = IsIndentationRequired(displayToLogicalNameArray[businessRequiredArray[index]])
                    ? "style='text-indent:19px'"
                    : "";

                businessRequiredlistItemsHtml[index] = "<li " +
                    textIndentStyle +
                    " class='ms-crm-Dialog-ListItem-ImportMap' style='width:280' onmouseover='On(this)' onmouseout='Off(this)' onclick='SelectItem(this)' title=\"" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(businessRequiredArray[index]) +
                    "\" ondblclick='btnMap()' id='entityattributeitem" +
                    index +
                    "'>" +
                    displayHtml +
                    "<input type='hidden' id='entityattributehiddenitem" +
                    index +
                    "' value='" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(displayToLogicalNameArray[businessRequiredArray[index]]) +
                    "' /></li>";
            }
        ...
        case 1:
            ...
            for (var index = 0; index < allAttributesArray.length; index++) {
                var displayHtml = GetHtmlWithImages(displayToLogicalNameArray[allAttributesArray[index]],
                    allAttributesArray[index]);
                var textIndentStyle = IsIndentationRequired(displayToLogicalNameArray[allAttributesArray[index]])
                    ? "style='text-indent:19px'"
                    : "";

                allAttributelistItemsHtml[index] = "<li " +
                    textIndentStyle +
                    " style='width:280' class='ms-crm-Dialog-ListItem-ImportMap' onmouseover='On(this)' onmouseout='Off(this)' onclick='SelectItem(this)' ondblclick='btnMap()' title=\"" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(allAttributesArray[index]) +
                    "\" id='entityattributeitem" +
                    index +
                    "'>" +
                    displayHtml +
                    "<input type='hidden' id='entityattributehiddenitem" +
                    index +
                    "' value='" +
                    CrmEncodeDecode.CrmHtmlAttributeEncode(displayToLogicalNameArray[allAttributesArray[index]]) +
                    "'/></li>";
            }
    ...
    function GetHtmlWithImages(logicalName, displayName) {
        ...
        if (!IsNull(node)) {
            ...
            case "lookup":
                htmlObj
                    .imgHtml =
                    "<img alt='' align='absmiddle' CLASS='dlg_create_RenderListItem_img' src='/_imgs/lookup.gif'>";
                break;
            }
            ...
            if (businessrequired == "true") {
                htmlObj
                    .businessrequiredImgHtml =
                    "<img alt='' align='absmiddle' CLASS='dlg_create_RenderListItem_img' src='/_imgs/frm_required.gif'>";
            }
        }
    ...
    function btnMap() {
        ...
        if (XUI.Html
            .GetText(_selectedTargetItem) ==
            "" ||
            XUI.Html.GetText(_selectedTargetItem) == ignoreAtributeLabel) {
            var str = GetHtmlWithImages(logicalName, XUI.Html.GetText(_selectedItem));

            logicalNameHiddenInput =
                "<input type='hidden' id='" + hiddenInputItemId + "' value='" + logicalName + "'/>";
            ...
            _selectedTargetItem.innerHTML = str;
            document.getElementById(hiddenInputItemId).value = document.getElementById(hiddenInputItemId).value
                .concat(";" + logicalName);
        }
    ...
    function btnUnmap() {
        ...
        if (XUI.Html.GetText(_selectedTargetItem) == ignoreAtributeLabel) {
            ...
            for (var i = 0; i < targetAttributes.length; i++) {
                var xpath = "//AttributeMap[TargetAttributeName=\"" +
                    targetAttributes[i] +
                    "\" and SourceAttributeName=\"" +
                    escapeSingleQuotesAndBackSlashes(XUI.Html.GetText(_selectedSourceItem)) +
                    "\"]";
    ...
    function RemoveIgnoredColumnMappingForSelectItem() {
        ...
        xpath = "//AttributeMap[ProcessCode=\"Ignore\" and SourceAttributeName=\"" +
            escapeSingleQuotesAndBackSlashes(XUI.Html.GetText(_selectedSourceItem)) +
            "\"]";
        var attributeNode = XUI.Xml.SelectSingleNode(mapXmlDoc, xpath, null);
        var attriMapsNode = XUI.Xml.SelectSingleNode(mapXmlDoc, "//AttributeMaps", null);
    ...
    function btnIgnore() {
        ...
        if (XUI.Html.GetText(_selectedTargetItem) != "") {
            ...
            for (var i = 0; i < targetAttributes.length; i++) {
                xpath = "//AttributeMap[TargetAttributeName=\"" +
                    targetAttributes[i] +
                    "\" and SourceAttributeName=\"" +
                    escapeSingleQuotesAndBackSlashes(XUI.Html.GetText(_selectedSourceItem)) +
                    "\"]";
    ...
    function btnLoadDataFile() {
        var dataXml = openStdDlg(Mscrm.CrmUri.create("/_grid/cmds/dlg_uploadsampledata.aspx"),
            null,
            400,
            300,
            true,
            false,
            "maximize:no;minimize:no");
        if (dataXml != "") {
    ...
    <table class="attrMap_table_header2">
        <tr style="padding-top: 20px">
            <td class="attrMap_td_headerLabel">
                <b>
                    <loc:Text ResourceId="DataManagement.ImportMap.AttributeMap.SourceHeader.Label" runat="server"/>
                </b>
            </td>
            <td class="attrMap_td_headerLabel">
                <b>
                    <loc:Text ResourceId="DataManagement.ImportMap.AttributeMap.TargetHeader.Label" runat="server"/>
                </b>
            </td>
        </tr>
        <tr>
            <td>
                <div style="padding: 5px; background-color: #C4DDFF; width: 450px; margin-left: 5px; margin-right: 5px; ; border: 1px solid #8C8E94;">
                    <table id="tblGridBar" width="400" cellpadding="0">
                        ...
                        <tr>
                            <td>
                                <label id="mappedHeadersCountLabel"></label>
                            </td>
                        </tr>
                ...
                <div style="padding: 5px; background-color: #C4DDFF; width: 210px; height: 360px; margin-left: 5px; margin-right: 5px; border: 1px solid #8C8E94;">
                    <table id="targetentityattributes" width="310" cellpadding="0" style="table-layout: fixed">
                        ...
                        <tr class="gridHeader" valign="bottom" onclick="onAttributeGridColumnClick();">
                            <td class="gridheader attrMap_td_gridHeader" field="cn" width="30">
                                <label style="width: 194; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlAttributeEncode(Microsoft.Crm.Application.Utility.WebUtility.GetFmtObjName(entityTypeCode, NameFormatStyle.Singular)) %>">
                                    <b><%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlEncode(Microsoft.Crm.Application.Utility.WebUtility.GetFmtObjName(entityTypeCode, NameFormatStyle.Singular)) %></b><img alt="" src="/_imgs/grid/bar_up.gif" class="addFields_img_barUp">
                                </label>
                            </td>
                        </tr>
                        <tr style="height: 300">
                        ...
                        <tr style="height: 30">
                            <td>
                                <label id="bizReqdMapCount"></label>
                            </td>
                        </tr>
                        <tr style="height: 40">
                            <td valign="bottom">
                    ...
                    <tr>
                        <td class="attrMap_td_MapFile">
                            <label class="attrMap_td_LegendText" title="<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("DataManagement.ImportMap.AttributeMap.Legend.Text")) %>">
                                <b>
                                    <loc:Text ResourceId="DataManagement.ImportMap.AttributeMap.Legend.Text" runat="server"/>
                                </b>
                            </label>
                        </td>
                        <td>
                            ...
                            &nbsp;<label class="attrMap_label_LegendLabel" title="<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("DataManagement.ImportMap.AttributeMap.PickListLegend.Text")) %>">
                                <loc:Text ResourceId="DataManagement.ImportMap.AttributeMap.PickListLegend.Text" runat="server"/>
                            </label>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            &nbsp;<label class="attrMap_label_LegendLabel" title="<%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("DataManagement.ImportMap.AttributeMap.LookupLegend.Text")) %>">
                                <loc:Text ResourceId="DataManagement.ImportMap.AttributeMap.LookupLegend.Text" runat="server"/>
                            </label>
                        </td>
                    </tr>
                </table>
