...
<script language="Javascript" type="text/javascript">
    ...
    function autoMappingForUpdateModeDone(oResult) {
        if (oResult.Success == false) {
            ...
            var unmappedEntityNodes = XUI.Xml.SelectNodes(mapXmlDoc,
                "/Map/EntityMaps/EntityMap[(@ProcessCode='" + ImportEntityProcessCode_Unmapped + "')]",
                null);
            var unmappedAttributeMapNodes = XUI.Xml
                .SelectNodes(mapXmlDoc,
                    "/Map/EntityMaps/EntityMap/AttributeMaps/AttributeMap[(ProcessCode='" +
                    ImportEntityProcessCode_Unmapped +
                    "')]",
                    null);
            var unmappedPicklistMapNodes = XUI.Xml
                .SelectNodes(mapXmlDoc,
                    "/Map/EntityMaps/EntityMap/AttributeMaps/AttributeMap/PicklistMaps/PicklistMap[ProcessCode='" +
                    ImportEntityProcessCode_Unmapped +
                    "']",
                    null);
            ...
            if (unmappedEntityNodes.length > 0) {
                ...
                for (i = 0; i < unmappedAttributeMapNodes.length; i++) {
                    var attributeName = XUI.Xml.GetText(XUI.Xml
                        .SelectSingleNode(unmappedAttributeMapNodes[i], "SourceAttributeName", null));
                ...
                if (unmappedPicklistMapNodes.length > 0) {
                    ...
                    for (i = 0; i < unmappedPicklistMapNodes.length; i++) {
                        var picklistValue = XUI.Xml.GetText(XUI.Xml
                            .SelectSingleNode(unmappedPicklistMapNodes[i], "SourceValue", null));
    ...
    function moveNext() {
        ...
        if (hasUserChangedUploadFile || WizardGetProperty("RemoveUploadedFile") == true) {
            if (!IsNull(importFileFullName) || WizardGetProperty("RemoveUploadedFile") == true) {
                ...
                XUI.Html.SetText(document.getElementById("progressMessage"),
                    formatString(LOCID_IW_FILEUPLOAD_PROGRESS, shortName))
    ...
    function SubmitIframeForm() {
        ...
        try {
            ...
            frame.contentWindow.document.getElementById("uploadFileform")
                .action = "ImportFileUpload.aspx?action=fileUpload";
    ...
    function PageOnLoad() {
        ...
        if (!IsNull(WizardGetProperty("ImportFileName"))) {
            ...
            frameContentDoc.getElementById("enclosedDivText")
                .title =
                formatString(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("ImportWizard.UploadFilePage.EnclosedDiv.EnclosedDivText")) %>, shortName);
            XUI.Html.SetText(frameContentDoc.getElementById("enclosedDivText"),
                formatString(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("ImportWizard.UploadFilePage.EnclosedDiv.EnclosedDivText")) %>, shortName));

            XUI.Html.SetText($get("infoBalloonText"),
                formatString(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("ImportWizard.UploadFilePage.InfoBalloonText")) %>, shortName));
        }

        hasUserChangedUploadFile = false;
        XUI.Html.SetText(document.getElementById("warningBalloonText"),
            <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("ImportWizard.UploadFilePage.WarningBalloonText")) %>);
    }
