<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/*">
    <table style="height: 100%; width: 100%; border: 0px; margin: 0px; table-layout:fixed;" cellspacing="0px"
           cellpadding="0px">
      <col width="70%" />
      <col width="30%" />
      <tr style="height: 100%;">
                ...
                <tr class="queryConditionRow">
                  ...
                  <xsl:choose>
                    <xsl:when test="$dataTypeValue= 'Picklist'">
                      <td>
                        <select req="0" style="width: 100%;" defaultselected="1" class="ms-crm-SelectBox "
                                onfocus="modifyPreview(new Sys.UI.DomEvent(event));">
                    ...
                    <xsl:otherwise>
                      <td>
                        <input tabindex="130" type="text" class="ms-crm-Text"
                               onfocus="modifyPreview(new Sys.UI.DomEvent(event));">
                          <xsl:attribute name="id">
                            <xsl:value-of select="@textboxId" />
                          </xsl:attribute>
