...
<style type="text/css">
    ...
    div.ms-crm-Tab { padding: 0px !important; }

</style>
<script language="javascript" type="text/javascript">
    var mode = "advancedfind";
    ...
    function PageLoad() {
        ...
        resultRender.action = Mscrm.CrmUri.create("/AdvancedFind/fetchData.aspx?dispActions=false&dispMenuBar=false")
            .toString();
                ...
                for (var i = 0; i < nLength; i++) {
                    ...
                    if (slugPosition >= 0) {
                        ...
                        if (primitiveSlugPosition >= 0) {
                            ctrlValue.set_dataValue(CrmEncodeDecode
                                .CrmXmlDecode(CrmEncodeDecode
                                    .CrmXmlDecode(CrmEncodeDecode
                                        .CrmHtmlDecode(infoList[1]
                                            .substring(primitiveSlugPosition + primitiveSlugTemplate.length)
                                            .split("\"")[0]))));
                        }
                    }
                    if (readOnlyForm == 'true') {
    ...
    function GenerateModifiedFetchQueryXml(onInitialLoad) {
        ...
        if ("none" == advancedFind.style.display.toString()) {
            ...
            sXml += "<modifiedFetch>" +
                CrmEncodeDecode.CrmXmlEncode(fetchQuery.getAttribute("defaultValue")) +
                "</modifiedFetch>";
            var tableAttributes = $get("tableAttributes");
            var nLength = !IsNull(tableAttributes) ? tableAttributes.rows.length : 0;
            if (nLength > 0) {
    ...
    function setupConditionTable(modifiedFetch) {
        ...
        var oXmlConditionDoc = XUI.Xml.LoadXml(XUI.Xml.XMLSerializer
            .serializeToString(XUI.Xml.SelectSingleNode(oXmlRootDoc.documentElement, "conditions", null)));
        var oXsl = null;
        XUI.Xml.Load(Mscrm.CrmUri.create("/SFA/workflow/queryConditions.xsl").toString(),
            false,
        ...
        var fetchQuery = document.getElementById("fetchQuery");
        if (null != fetchQuery) {
            fetchQuery.value = CrmEncodeDecode.CrmXmlDecode(XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(oXmlRootDoc.documentElement, "modifiedDisplayFetch", null)));
            fetchQuery.setAttribute("defaultValue",
                XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oXmlRootDoc.documentElement, "modifiedFetch", null)));
        }
    }
    ...
    function ExecuteQuery() {
        ...
        resultFrame.document.body
            .innerHTML =
            "<div style='height:100%;width:100%;position:absolute;'><table height='100%' width='100%' style='cursor:wait'><tr><td valign='middle' align='center'><img alt='' src='/_imgs/AdvFind/progress.gif'/><br>" + LOCID_AF_EXECUTINGSEARCH + "</td></tr></table></div>";
        switchTo("SEARCH");
                                                                ...
                                                                <div class="ms-crm-absolutePosition" style="left: 10px; right: 10px; top: 10px; bottom: 10px">
                                                                    <div id="dummyDiv4" class="ms-crm-IE7-Height-Fix-Dummy-Container">
                                                                        ...
                                                                        <div class="picklistSection" style="height: 190px; width: 100%;">
                                                                            <div id="divAttributes" style="background-color: #F3F4F8; height: 100%; width: 100%; border: 1px solid #A5AEB5; overflow: hidden;">
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
