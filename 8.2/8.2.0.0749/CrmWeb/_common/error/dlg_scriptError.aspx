...
<head>
    <script language="javascript" type="text/javascript">
        ...
        function PageOnLoad() {
            ...
            try {
                ...
                var serverData = "  <ServerInformation>\r\n" +
                    "    <OrgLanguage>" +
                    CrmEncodeDecode.CrmXmlEncode(OrgLanguage) +
                    "</OrgLanguage>\r\n" +
                    "    <OrgCulture>" +
                    CrmEncodeDecode.CrmXmlEncode(OrgCulture) +
                    "</OrgCulture>\r\n" +
                    "    <UserLanguage>" +
                    CrmEncodeDecode.CrmXmlEncode(UserLanguage) +
                    "</UserLanguage>\r\n" +
                    "    <UserCulture>" +
                    CrmEncodeDecode.CrmXmlEncode(UserCulture) +
                    "</UserCulture>\r\n" +
                    "    <OrgID>" +
                    CrmEncodeDecode.CrmXmlEncode(OrgID) +
                    "</OrgID>\r\n" +
                    <%
                    %>
                    "    <UserID>" +
                    CrmEncodeDecode.CrmXmlEncode(UserID) +
                    "</UserID>\r\n" +
                    "    <CRMVersion>" +
                    CrmEncodeDecode.CrmXmlEncode(CRMVersion) +
                    "</CRMVersion>\r\n" +
                    "  </ServerInformation>\r\n";
                ...
                if (!IsNull(oArgs)) {
                    for (var i = 0; i < oArgs.length; i++) {
                        ...
                        if (insertPos != -1) {
                            oArgs[i].report = oArgs[i].report.substring(0, insertPos) +
                                serverData +
                                oArgs[i].report.substring(insertPos);
                        }
                    }
                }
            } catch (e) {
        ...
        function OpenErrorDetails(e) {
            ...
            openStdDlg(Mscrm.CrmUri.create("/_common/error/dlg_scriptErrorDetails.aspx"),
                getDialogArguments(),
                oWindowInfo.Width,
                oWindowInfo.Height,
                true);
        ...
        function ChangeNotificationSettings(e) {
            ...
            var callbackFunction = Mscrm.Utilities
                .createCallbackFunctionForXrmDialog(changeNotificationSettingsCallback, parameters);

            var retValue = openStdDlgWithCallback(Mscrm.CrmUri
                .create("/tools/personalsettings/dialogs/personalsettings.aspx?privacy=1"),
                null,
                900,
                600,
                callbackFunction,
...
<table cellpadding="8" cellspacing="0" width="100%" height="100%" style="table-layout: fixed;">
    ...
    <tr>
        ...
        <td valign="top">
            ...
            <br/>
            <br/>
            <a id='privacyLink' class='ms-crm-Dialog-Error-Link' target='_blank' href='http://go.microsoft.com/fwlink/?LinkId=103482'>
