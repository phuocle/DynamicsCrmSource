...
<script language=javascript>
    ...
    function initialize() {
        ...
        var oPageDefinitionNode = XUI.Xml.SelectSingleNode(xmlDoc,
            "/Pages/Page[@pagesequencenumber='" + iWizardPageSequenceNumber + "']",
            null);
        ...
        oPageDefinition["Url"] = Mscrm.CrmUri.create(oPageDefinitionNode.attributes.getNamedItem("pageurl").nodeValue)
            .toString();
    ...
    function staticNavigate() {
        ...
        var sBackUrlId = "Url" + _iBackButtonCurrentIndex;
        var oPageDefinition = _aoBbackButtonPages[sBackUrlId];
        var oPageDefinitionNode = XUI.Xml.SelectSingleNode(pageDefinitions,
            "/Pages/Page[@pagesequencenumber='" + oPageDefinition["PageNumber"] + "']",
            null);
        ...
        oPageDefinition["Url"] = Mscrm.CrmUri.create(oNextPageDefinitionNode.attributes.getNamedItem("pageurl")
            .nodeValue).toString();
    ...
    function onWizardPageLoad(element) {
        if (_loadWizardPage) {
            ...
            if (bIsStaticPageSequence && !IsNull(oFrame.$get("spanPageNumber"))) {
                ...
                XUI.Html.SetText(oFrame.$get("spanPageNumber"),
                    formatString(LOCID_STATIC_PAGE_NUMBER, iPageNumber, iPageCount));
            }
...
<body onload='initialize();' <% if (_dialogType == 1)
                                { %>style="overflow: hidden"<% } %> <% if (Request.Browser.Browser == "IE")
                                                                       { %> class="ms-crm-absolutePosition" <% } %>>


<a id="reloader" href="" tabindex="-1"></a>
...
<div style="width: 100%; height: 100%; position: absolute; display: none;">
    ...
    <iframe onload='onWizardPageLoad(this);' name='wizardpageframe' id='wizardpageframe'
            <% if (_dialogType == 1)
               { %> height="100%" <% }
               else
               { %> height="99%" <% } %> width="100%" scrolling="no" src='/_static/blank.htm' frameborder="0">
    </iframe>
    <% if (Request.Browser.Browser == "IE")
       { %>
    </div>
