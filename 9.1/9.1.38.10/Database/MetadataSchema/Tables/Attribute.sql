CREATE TABLE [MetadataSchema].[Attribute] (
    ...
    [ManagedPropertyLogicalName]         NVARCHAR (128)   NULL,
    [ManagedPropertyParentComponentType] INT              NULL,
    [ManagedPropertyParentAttributeName] NVARCHAR (128)   NULL,
    ...
    [FormulaDefinition]                  NVARCHAR (MAX)   NULL,
    [SourceTypeMask]                     INT              DEFAULT ((0)) NOT NULL,
    [AutoNumberFormat]                   NVARCHAR (100)   NULL,
    [Behavior]                           INT              NULL,
    [CanModifyBehavior]                  BIT              NULL,
    [CanModifyGlobalFilterSettings]      BIT              DEFAULT ((1)) NOT NULL,
    [CanModifyIsSortableSettings]        BIT              DEFAULT ((1)) NOT NULL,
    [ParentEnumAttributeId]              UNIQUEIDENTIFIER NULL,
    [ExternalName]                       NVARCHAR (128)   NULL,
    [IsActive]                           BIT              DEFAULT ((1)) NOT NULL,
    [IsDataSourceSecret]                 BIT              DEFAULT ((0)) NULL,
    [IsGlobalFilterEnabled]              BIT              DEFAULT ((0)) NOT NULL,
    [IsSearchable]                       BIT              DEFAULT ((0)) NOT NULL,
    [IsFilterable]                       BIT              DEFAULT ((0)) NOT NULL,
    [IsRetrievable]                      BIT              DEFAULT ((0)) NOT NULL,
    [IsFilterableSetBySystem]            BIT              DEFAULT ((0)) NOT NULL,
    [IsRetrievableSetBySystem]           BIT              DEFAULT ((0)) NOT NULL,
    [IsSortableEnabled]                  BIT              DEFAULT ((0)) NOT NULL,
    [RecordId]                           BIGINT           IDENTITY (1, 1) NOT NULL,
    [CreatedOn]                          DATETIME         DEFAULT ((0)) NOT NULL,
    [ModifiedOn]                         DATETIME         DEFAULT ((0)) NOT NULL,
    CONSTRAINT [XPKAttribute] PRIMARY KEY NONCLUSTERED ([AttributeId] ASC, [SolutionId] ASC, [ComponentState] ASC, [OverwriteTime] ASC),
    ...
    CONSTRAINT [attribute_parent_enumattribute] FOREIGN KEY ([ParentEnumAttributeId]) REFERENCES [dbo].[AttributeIds] ([AttributeId]),
...
GO
EXECUTE sp_tableoption @TableNamePattern = N'[MetadataSchema].[Attribute]', @OptionName = N'large value types out of row', @OptionValue = N'1';


GO
ALTER TABLE [MetadataSchema].[Attribute] SET (LOCK_ESCALATION = DISABLE);


GO
CREATE UNIQUE CLUSTERED INDEX [ndx_Attribute_RecordId]
    ON [MetadataSchema].[Attribute]([RecordId] ASC);


GO
CREATE NONCLUSTERED INDEX [attr_column_name]
    ON [MetadataSchema].[Attribute]([EntityId] ASC, [Name] ASC)
    INCLUDE([AttributeId]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);
...
GO
CREATE NONCLUSTERED INDEX [ndx_Attribute_SolutionId]
    ON [MetadataSchema].[Attribute]([SolutionId] ASC, [OverwriteTime] ASC)
    INCLUDE([AttributeId], [VersionNumber]);


GO
CREATE UNIQUE NONCLUSTERED INDEX [ndx_Attribute_EntityId_ColumnNumber]
    ON [MetadataSchema].[Attribute]([EntityId] ASC, [ColumnNumber] ASC) WHERE ([OverwriteTime]=(0) AND [ComponentState]=(0));
...
CREATE NONCLUSTERED INDEX [ndx_OptionSetId]
    ON [MetadataSchema].[Attribute]([OptionSetId] ASC, [EntityId] ASC)
    INCLUDE([AttributeId]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);
...
GO
CREATE NONCLUSTERED INDEX [Attribute_ComponentStateOverwriteTime]
    ON [MetadataSchema].[Attribute]([ComponentState] ASC, [OverwriteTime] ASC)
    INCLUDE([AttributeId], [VersionNumber]);


GO
CREATE NONCLUSTERED INDEX [ndx_attribute_fegetformxml]
    ON [MetadataSchema].[Attribute]([EntityId] ASC, [OverwriteTime] ASC, [ComponentState] ASC, [AttributeOf] ASC, [AggregateOf] ASC, [ValidForCreateAPI] ASC, [ValidForUpdateAPI] ASC, [ValidForReadAPI] ASC, [AttributeTypeId] ASC, [AttributeId] ASC, [SolutionId] ASC, [AttributeRowId] ASC)
    INCLUDE([Length], [IsNullable], [LogicalName], [IsCustomField], [DisplayMask], [SourceTypeMask]);


GO
CREATE NONCLUSTERED INDEX [ndx_Attribute_SolutionImport]
    ON [MetadataSchema].[Attribute]([ComponentState] ASC, [OverwriteTime] ASC, [InheritsFrom] ASC, [CanBeSecuredForRead] ASC, [IsSecured] ASC)
    INCLUDE([AttributeId], [AttributeTypeId], [Name], [PhysicalName], [XmlAbbreviation], [EntityId], [ColumnNumber], [LogicalName], [ValidForReadAPI], [IsPKAttribute], [IsCustomField], [IsLogical], [AttributeOf], [HasMultipleLabels], [IsRowGuidAttribute], [CalculationOf], [IsStoredOnPrimaryTable], [IsInheritanceTypeAttribute], [TableColumnName], [IsEncrypted]);


GO
CREATE NONCLUSTERED INDEX [ndx_AttributeTypeAndEntity]
    ON [MetadataSchema].[Attribute]([AttributeTypeId] ASC, [EntityId] ASC, [LogicalName] ASC)
    INCLUDE([DisplayMask], [IsSearchable], [IsFilterable], [IsRetrievable], [IsPKAttribute]);


GO
CREATE NONCLUSTERED INDEX [ndx_ComponentStateOverwriteTimeAttributeTypeId]
    ON [MetadataSchema].[Attribute]([ComponentState] ASC, [OverwriteTime] ASC, [AttributeTypeId] ASC)
    INCLUDE([AttributeId], [Accuracy], [AccuracySource], [AggregateOf], [AppDefaultValue], [AttributeImeModeId], [AttributeLogicalTypeId], [AttributeOf], [AttributeRequiredLevelId], [AttributeRowId], [AutoNumberFormat], [Behavior], [CalculationOf], [CanBeSecuredForCreate], [CanBeSecuredForRead], [CanBeSecuredForUpdate], [CanModifyAdditionalSettings], [CanModifyAuditSettings], [CanModifyBehavior], [CanModifyGlobalFilterSettings], [CanModifyIsSortableSettings], [CanModifyRequirementLevelSettings], [CanModifySearchSettings], [ColumnNumber], [DefaultValue], [DeprecatedVersion], [DisplayMask], [EntityId], [ExternalName], [FormulaDefinition], [HasMultipleLabels], [ImeMode], [InheritsFrom], [IntroducedVersion], [IsActive], [IsAuditEnabled], [IsBaseCurrency], [IsCustomField], [IsCustomizable], [IsDataSourceSecret], [IsEncrypted], [IsFilterable], [IsFilterableSetBySystem], [IsGlobalFilterEnabled], [IsIdentity], [IsInheritanceTypeAttribute], [IsLogical], [IsManaged], [IsNullable], [IsOneWayBooleanAttribute], [IsPKAttribute], [IsRenameable], [IsReplicated], [IsRetrievable], [IsRetrievableSetBySystem], [IsRowGuidAttribute], [IsSearchable], [IsSecured], [IsSortableEnabled], [IsSortAttribute], [IsStoredOnPrimaryTable], [IsUnmanagedAttribute], [Length], [LinkedAttributeId], [Locked], [LogicalName], [LookupBrowse], [LookupStyle], [ManagedPropertyLogicalName], [ManagedPropertyParentAttributeName], [ManagedPropertyParentComponentType], [MaxLength], [MaxValue], [MinValue], [Name], [OptionSetId], [ParentEnumAttributeId], [PhysicalName], [PrecisionSource], [PrecisionValue], [RecordId], [ReferencedEntityObjectTypeCode], [SolutionId], [SourceType], [SourceTypeMask], [SupportingSolutionId], [TableColumnName], [UpgradeDefaultValue], [ValidForCreateAPI], [ValidForReadAPI], [ValidForUpdateAPI], [ValuesFromRelationshipAttribute], [VersionNumber], [VisibleToPlatform], [XmlAbbreviation], [YomiOf]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);


GO
CREATE NONCLUSTERED INDEX [ndx_EntityIdAttributeId_Included_ForRename]
    ON [MetadataSchema].[Attribute]([EntityId] ASC, [AttributeId] ASC)
    INCLUDE([Name], [LogicalName], [PhysicalName], [TableColumnName], [VersionNumber], [DisplayMask], [ComponentState], [OverwriteTime]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);


GO
CREATE UNIQUE NONCLUSTERED INDEX [ndx_Attribute_VersionNumber]
    ON [MetadataSchema].[Attribute]([VersionNumber] ASC);
