CREATE TABLE [MetadataSchema].[EntityRelationshipRelationships] (
    ...
    CONSTRAINT [XPKEntityRelationshipRelationships] PRIMARY KEY NONCLUSTERED ([EntityRelationshipRelationshipsId] ASC, [SolutionId] ASC, [ComponentState] ASC, [OverwriteTime] ASC),
...
GO
ALTER TABLE [MetadataSchema].[EntityRelationshipRelationships] SET (LOCK_ESCALATION = DISABLE);


GO
CREATE UNIQUE CLUSTERED INDEX [ndx_EntityRelationshipRelationships_RecordId]
    ON [MetadataSchema].[EntityRelationshipRelationships]([RecordId] ASC);
...
GO
CREATE UNIQUE NONCLUSTERED INDEX [ndx_EntityRelationshipRelationships_RowId]
    ON [MetadataSchema].[EntityRelationshipRelationships]([EntityRelationshipRelationshipsRowId] ASC, [EntityRelationshipRelationshipsId] ASC);
...
CREATE NONCLUSTERED INDEX [ndx_RelationshipIds]
    ...
    INCLUDE([EntityRelationshipId], [ComponentState]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);
