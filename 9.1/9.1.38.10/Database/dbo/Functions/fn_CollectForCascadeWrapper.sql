...
-----------------------------------------------------------------------
-- THIS IS A REAL FUNCTION WRAPPER TO COLLECT CASCADE AND            --
-- REMOVE LINK ENTITIES FOR CASCADE DELETE OPERATION                 --
-----------------------------------------------------------------------
create function [dbo].[fn_CollectForCascadeWrapper]
(
...
if ((select count(*) from @root_ids) = 0)begin
...
insert into @collect_entities_to_delete_all_levels(o,t,c,s)
	select distinct o,t,d,s from fn_CollectForCascadeDelete(@collect_entities_to_delete_first_levels,@isoffline,@managed_solution_operation)

-- Stop collection and exit if a Restrict relationship was hit (an invalid otc was collected).
if(exists(select 1 from @collect_entities_to_delete_all_levels where t=@invalid_otc))
begin
...
insert into @collect_by_cascade_removelink(o,t,c,s)
	select o,t,c,0 from fn_CollectForCascadeRemoveLink(@collect_entities_to_delete_all_levels,@is_delete_entity_schema,@root_otc,@isoffline)

-- exclude any duplicates; otherwise we can get PK contraint violation
insert into @entities_to_removelink(o,t,c,s)
	select distinct cr.o,cr.t,cr.c,cr.s from @collect_by_cascade_removelink cr
