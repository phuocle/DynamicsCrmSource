...
SET @maxRecord = ISNULL(@maxRecord,0)
		 ...
		 BEGIN 
			 ...
			 SET [a].[OpenRevenue] = ROUND(ROUND([b].[OutputValue],10) * [b].[ExchangeRate],10), [OpenRevenue_Base] = ROUND([b].[OutputValue],10)
...
, [OpenRevenue_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
			 ...
			 AND (([a].[OpenRevenue] IS NULL AND [b].[OutputValue] IS NOT NULL) OR 
			 ([a].[OpenRevenue] IS NOT NULL AND [b].[OutputValue] IS NULL) OR 
			 ([a].[OpenRevenue] != ROUND(ROUND([b].[OutputValue],10) * [b].[ExchangeRate],10)))
		 ...
		 BEGIN 
				 ...
				 BEGIN 
					 ...
					 INSERT INTO [dbo].[Rollup_Bootstrap_Markers_306845ef446a42e58df811c31bafaded]([Id],[Start],[End]) VALUES (@index +1, @start,@end) 
				 END 
		 END 
 ...
 BEGIN 
		 ...
		 SELECT @start = [Start], @end = [End] FROM [dbo].[Rollup_Bootstrap_Markers_306845ef446a42e58df811c31bafaded] WHERE [Id] = @index
...
, [OpenRevenue_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
		 ...
		 DELETE [dbo].[Rollup_Bootstrap_Markers_306845ef446a42e58df811c31bafaded] WHERE [Id] = @index
