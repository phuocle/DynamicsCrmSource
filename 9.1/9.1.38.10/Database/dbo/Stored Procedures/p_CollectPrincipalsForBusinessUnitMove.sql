...
-- There is at least one user in each bu : u0, u1, ..., u5
...
-- Subscenario a1: There is no custom roles defined in b1. 
--		  Expected - @countDeletedRoles will be 0, collected principals will have {u1,u2}.
-- Subscenario a2: There is a custom role defined in b1. 
--		  Expected - @countDeletedRoles will be 1, collected principals will have {u1,u2,u3,u4}.
...
--  
--  Principals in moved BU subtree are collected only if passed in @countDeletedRoles>0 (removed inherited roles) from oldParent.

create procedure dbo.p_CollectPrincipalsForBusinessUnitMove(@movedBusinessId uniqueidentifier, @oldParentId uniqueidentifier, @newParentId uniqueidentifier, @countDeletedRoles int) as
begin
SET NOCOUNT ON
...
-- Only if inherited roles were deleted from moved BU, add moved subtree
if (@countDeletedRoles > 0)
insert into @affectedBuIds
...
select TeamId as principalid, @Team as type from TeamBase 
		where BusinessUnitId in (select BusinessUnitId from @affectedBuIds)
		 and TeamBase.TeamType <> 1 -- skip access teams: could be thousands of them - one per entity instance		
