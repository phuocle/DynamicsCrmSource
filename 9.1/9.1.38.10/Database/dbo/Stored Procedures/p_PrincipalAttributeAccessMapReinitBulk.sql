...
insert into #New(
	  ...
	  select i.PrincipalId,
		...
		join AttributeView a on a.EntityId = e.EntityId and a.LogicalName = fp.AttributeLogicalName collate database_default
		group by a.AttributeId, i.PrincipalId option (recompile)
...
insert PrincipalAttributeAccessMap (PrincipalId,
	...
	select  #New.PrincipalId, 
		...
		on #New.PrincipalId = added.PrincipalId and #New.AttributeId = added.AttributeId option (recompile)
...
from PrincipalAttributeAccessMap paam
		...
		on paam.PrincipalId = deletedPaam.PrincipalId and paam.AttributeId = deletedPaam.AttributeId option (recompile)
...
if (@resetSyncSubscriptions = 1) 
	begin
		...
		IF OBJECT_ID('dbo.Subscription', 'U') IS NOT NULL 
			BEGIN
				create table #PrincipalIdsToAdd (PrincipalUserId uniqueidentifier) (select id from @principalUserIds);
				DECLARE @query NVARCHAR(MAX) = 'update Subscription WITH (READPAST, READCOMMITTEDLOCK) set ReInitialize = 1 where SystemUserId in (select PrincipalUserId from #PrincipalIdsToAdd)';
				EXECUTE(@query)
				drop table #PrincipalIdsToAdd;
			END
	end
