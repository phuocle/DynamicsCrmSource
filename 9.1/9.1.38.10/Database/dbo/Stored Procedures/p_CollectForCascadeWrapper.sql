...
(
	...
	@isoffline int,							-- is Offline or not; if the call is coming from Outlook Offline or not
...
create table #CascadeRestrictDetails(
	restrictingObjectTypeCode int NOT NULL,
	restrictingEntityRecordCount int NOT NULL,
	
	PRIMARY KEY CLUSTERED (	restrictingObjectTypeCode ASC )
)
...
begin;
	...
	select restrictingObjectTypeCode, restrictingEntityRecordCount from #CascadeRestrictDetails;
...
-- We pass the table variable @entities_to_delete instead of @collect_entities_to_delete_all_levels (old code), since @collect_entities_to_delete_all_levels can contain 
--	duplicate entries for same objectType & objectId with different depth level (column c). This can cause PK violation in stored proc p_CollectForCascadeRemoveLink which
--	uses objecttypecode & objectid from the stored proc input table parameters while inserting to table variable @collectedRecords which is of type CascadeRemoveLinkCollection. (Bug #1344118)
exec p_CollectForCascadeRemoveLink @entities_to_delete,@is_delete_entity_schema,@root_otc,@isoffline;
...
-- SQL will automatically drop the out of scope temp table in async mode. We dont need to explicitly drop & can improve performance. 
--	In some cases, we have seen drop table # taking 3 - 5 seconds due to metadata contention.
