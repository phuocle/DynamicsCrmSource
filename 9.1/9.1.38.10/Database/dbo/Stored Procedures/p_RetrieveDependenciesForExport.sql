...
(
	...
	-- When this is set to true, required components from other managed solutions are kipped. As we can not include
	-- the managed components from a different solution into a given solution, returning them will not be of much use
	@SkipManagedDependencies bit = false
)
as
begin
	...
	exec dbo.p_CollectSolutionComponents @SolutionId, @DoNotIncludeNonCustomRelationships

	declare @DependencyFeature int
	set @DependencyFeature = 160 -- constant for dependency feature type, which we can ignore for the purposes of export
	...
	begin
		...
		begin
			...
			where 
				-- select only required and 'soft' (can be soft deleted but must exist in db) dependencies where the dependent component has been customized and the required component 
				-- is not part of the active solution (and hence is being exported as part of this solution)
				((d.DependencyType = @Required or d.DependencyType = @Soft) and dc.TopSolutionId = @ActiveSolutionId and rc.TopSolutionId <> @ActiveSolutionId)
	...
	insert into @Results
		...
		where (d.DependencyType = @Required or d.DependencyType = @SolutionInternal or d.DependencyType = @Soft)		
			and d.RequiredComponentNodeId not in (
				...
				union
				--exclude dependencies on relationships where the associated entity relationship is in the solution
				select dnb.DependencyNodeId
					from DependencyNodeBase dnb
					join MetadataSchema.Relationship r on (dnb.ObjectId = r.RelationshipId)
					join EntityRelationshipRelationships err on (r.RelationshipId = err.RelationshipId)
					where err.EntityRelationshipId in (select ObjectId from SolutionComponentBase where SolutionId=@SolutionId))
			-- exclude dependencies between all relationship roles and relationships, they are automatically included
			and (d.RequiredComponentType != 11 or d.DependentComponentType != 10)

	--Add virtual dependencies from relationship roles in the component set to referencing entities outside of the solution from custom/customized relationships
	insert into @Results
		(RequiredComponentObjectId, RequiredComponentType, RequiredComponentParentId, RequiredComponentBaseSolutionId, DependencyType, 
		DependentComponentObjectId, DependentComponentType, DependentComponentParentId, DependentComponentBaseSolutionId, RequiredComponentIntroducedVersion)
		select 
			dnb1.ObjectId, dnb1.ComponentType, dnb1.ParentId, dnb1.BaseSolutionId, 2,
			dnb2.ObjectId, dnb2.ComponentType, dnb2.ParentId, dnb2.BaseSolutionId, dnb1.IntroducedVersion
		from DependencyNodeBase dnb2
			...
			join DependencyNodeBase dnb1 on (r.ReferencingEntityId = dnb1.ObjectId)
			and (r.referencingEntityId not in (select ObjectId from SolutionComponentBase where SolutionId=@SolutionId))
			join DependencyNodeBase dnb3 on (err.EntityRelationshipId = dnb3.ObjectId)
		where r.SolutionId = @ActiveSolutionId
			and dnb3.BaseSolutionId <> @ActiveSolutionId or r.IsCustomRelationship = 1

	-- include placeholder components as internal dependencies
	insert into @Results
	(RequiredComponentObjectId, RequiredComponentType, RequiredComponentParentId, RequiredComponentBaseSolutionId, DependencyType, 
	DependentComponentObjectId, DependentComponentType, DependentComponentParentId, DependentComponentBaseSolutionId, RequiredComponentIntroducedVersion)
	select dn.ObjectId, dn.ComponentType, ParentId, BaseSolutionId, @SolutionInternal,
			dn.ObjectId, dn.ComponentType, ParentId, BaseSolutionId, dn.IntroducedVersion
	from DependencyNode dn
		join SolutionComponent sc on (dn.ObjectId = sc.ObjectId and dn.ComponentType = sc.ComponentType and sc.RootComponentBehavior <> 0)
	where sc.SolutionId = @SolutionId 
		and (dn.ObjectId not in (select RequiredComponentObjectId from @Results))

	if (@SkipManagedDependencies = 1)
	begin
		-- exclude the dependencies that belong to SystemSolution and any other managed solution, except if it is one of the virtual referencing entity dependencies
		delete from @Results where RequiredComponentBaseSolutionId <> @ActiveSolutionId and (RequiredComponentType <> 1 or DependentComponentType <> 11)
	end

	-- return the results as a dataset
	select * from @Results where RequiredComponentType <> @DependencyFeature order by RequiredComponentType asc
