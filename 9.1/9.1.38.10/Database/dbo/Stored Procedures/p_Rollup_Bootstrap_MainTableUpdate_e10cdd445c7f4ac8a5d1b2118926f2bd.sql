...
, [OpenDeals_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
			 ...
			 AND (([a].[OpenDeals] IS NULL AND [b].[OutputValue] IS NOT NULL) OR 
			 ([a].[OpenDeals] IS NOT NULL AND [b].[OutputValue] IS NULL) OR 
			 ([a].[OpenDeals] != [b].[OutputValue]))
		 ...
		 BEGIN 
				 ...
				 BEGIN 
					 ...
					 INSERT INTO [dbo].[Rollup_Bootstrap_Markers_e10cdd445c7f4ac8a5d1b2118926f2bd]([Id],[Start],[End]) VALUES (@index +1, @start,@end) 
				 END 
		 END 
 ...
 BEGIN 
		 ...
		 SELECT @start = [Start], @end = [End] FROM [dbo].[Rollup_Bootstrap_Markers_e10cdd445c7f4ac8a5d1b2118926f2bd] WHERE [Id] = @index
...
, [OpenDeals_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
		 ...
		 DELETE [dbo].[Rollup_Bootstrap_Markers_e10cdd445c7f4ac8a5d1b2118926f2bd] WHERE [Id] = @index
