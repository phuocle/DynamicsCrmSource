...
-- If parameter @RoleId is not null, then recomputation is done only for read privileges of given role  
CREATE     procedure [dbo].[p_SystemUserBuEntityMapReinit](@UserIds nvarchar(max)
							, @DeletedRoleId uniqueidentifier = null
							, @ReadPrivilegesIdsPopulated bit = null
...
-- All activities share same set of privileges with activityPointer. Only activityPointer entity (not an activity) privileges will be processed.
...
begin
	...
	if (@RoleId is null)
		...
		insert into  #ReadPrivileges(PrivilegeId) 
			select PrivilegeId from Privilege where Privilege.AccessRight = @ReadAccessRight 	
	else
		--  reinit for read privileges of given role
		insert into  #ReadPrivileges(PrivilegeId) 
			select p.PrivilegeId from Privilege p
				join RolePrivileges rp on rp.PrivilegeId = p.PrivilegeId
				join Role r on r.RoleId = @RoleId and r.ParentRootRoleId = rp.RoleId -- only ParentRootRoleId privileges are in RolePrivileges
...
insert into #Old(SystemUserBusinessUnitEntityMapId, BusinessUnitId, SystemUserId, ObjectTypeCode, ReadPrivilegeDepth ) 
	...
	join PrivilegeObjectTypeCodeView potc on potc.ObjectTypeCode = SuBuMap.ObjectTypeCode
	join #ReadPrivileges pRecomp on pRecomp.PrivilegeId = potc.PrivilegeId option (recompile)
...
-- populate user's teams roles
	...
	from Role r
		...
		join #SystemUsers su on sup.SystemUserId = su.SystemUserId option (recompile)

-- Exclude deleted role from computation of #New set
if (@DeletedRoleId is not null)
  delete @roles where RoleId = @DeletedRoleId

-- Optimization for roles from Root BU with deep depth == Global depth
...
insert into @globalOtc (ObjectTypeCode, SystemUserId)
	select distinct potc.ObjectTypeCode, r.SystemUserId from PrivilegeObjectTypeCodeView potc
			...
			where 
			  ...
			  and e.IsActivity = 0
	union -- add OTCs with deep depth == global depth from root Bu roles
	select distinct potc.ObjectTypeCode, r.SystemUserId from PrivilegeObjectTypeCodeView potc
				join  #ReadPrivileges p on p.PrivilegeId = potc.PrivilegeId
				join  RolePrivileges rp on rp.PrivilegeId = p.PrivilegeId
				join  @roles r on r.ParentRootRoleId = rp.RoleId
			...
			where 
			  ...
			  and e.OwnershipTypeMask in (@UserOwnedMask, @BusinessOwnedMask)
			  and e.IsActivity = 0
			  and r.BusinessUnitId = @rootBU
	-- note: order by applies to all results of union  
	order by potc.ObjectTypeCode, r.SystemUserId option (recompile)
...
insert into #New( BusinessUnitId, SystemUserId, ObjectTypeCode, ReadPrivilegeDepth ) 
	select distinct r.BusinessUnitId, r.SystemUserId, potc.ObjectTypeCode, @LOCAL_MASK  
			from PrivilegeObjectTypeCodeView potc
				join  #ReadPrivileges p on p.PrivilegeId = potc.PrivilegeId
				join  RolePrivileges rp on rp.PrivilegeId = p.PrivilegeId
				join  @roles r on r.ParentRootRoleId = rp.RoleId
				join  EntityView e on e.ObjectTypeCode = potc.ObjectTypeCode
			where 
			  ...
			  and e.OwnershipTypeMask in (@UserOwnedMask, @BusinessOwnedMask)
			  and e.IsActivity = 0
			  and NOT EXISTS (select 1 from @globalOtc g where g.ObjectTypeCode = potc.ObjectTypeCode AND g.SystemUserId = r.SystemUserId) option (recompile)	
...
from #Old o
	join #New n on (n.BusinessUnitId = o.BusinessUnitId and 
					...
					n.ReadPrivilegeDepth = o.ReadPrivilegeDepth) option (recompile)

if (@@rowcount>0) -- if identical rows were found
begin
	-- Remove identical rows from #New
	delete from #New 
	where NewIdentityId in (select NewIdentityId from @ids) option (recompile) 
end
...
delete SystemUserBusinessUnitEntityMap
	...
	where SystemUserBusinessUnitEntityMapId in 
		(select SystemUserBusinessUnitEntityMapId from #Old) option (recompile)
...
insert into SystemUserBusinessUnitEntityMap
	...
	select SystemUserId,
			BusinessUnitId, ObjectTypeCode, ReadPrivilegeDepth from #New option (recompile) 
