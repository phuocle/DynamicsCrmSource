...
--		which doesn't exist as inherited custom roles already.
...
begin;
	...
	-- custom roles from parent BU which don't exist in given BU @bizId
    create table #roles (id uniqueidentifier);
    insert into #roles
    select RoleId
    from Role
    where BusinessUnitId = @parentBizId
          and RoleTemplateId is null  -- custom role condition
		  -- exclude custom inherited roles already existing in given BU
		  and ParentRootRoleId not in 
				(select ParentRootRoleId from Role 
						where BusinessUnitId = @bizId 
							and RoleTemplateId is null -- custom role
							and ParentRoleId is not null -- inherited role
				)
