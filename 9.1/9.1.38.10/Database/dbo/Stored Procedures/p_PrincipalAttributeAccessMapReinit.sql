...
insert into @New(
	  ...
	  select 	a.AttributeId,
		...
		group by a.AttributeId option (recompile)
...
insert PrincipalAttributeAccessMap (PrincipalId,
    ...
    select @principalId, 
			...
			UpdateAccess
		from @New 
	where AttributeId in (select AttributeId from @New Except select AttributeId from @Old) option (recompile) 
...
delete PrincipalAttributeAccessMap 
	...
	where PrincipalId = @principalId
	  and AttributeId in (select AttributeId from @Old Except select AttributeId from @New) option (recompile)
...
if (@type = @User AND @resetSyncSubscriptions = 1) 
	begin
		...
		BEGIN
			DECLARE @query NVARCHAR(MAX) = 'update Subscription WITH (READPAST, READCOMMITTEDLOCK) set ReInitialize = 1 where SystemUserId = ''' + cast(@principalId as nvarchar(36)) + '''';
			EXECUTE(@query)
		END
	end
