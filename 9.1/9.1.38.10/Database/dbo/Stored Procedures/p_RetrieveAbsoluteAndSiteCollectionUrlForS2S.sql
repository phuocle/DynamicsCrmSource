...
begin
	select @parentId = ParentSiteOrLocation, @objectTypeCode = ParentSiteOrLocationTypeCode , @relativeUrl = RelativeUrl , @absoluteLocationUrl = AbsoluteURL,
		@siteCollectionId = SiteCollectionId, @servicetype = ServiceType, @Name = Name
		from SharePointDocumentLocationBase	where SharePointDocumentLocationId = @parentId

		if(@locationName = '')
		begin
			set @locationName = @Name --Holding the root location name value of a SharePointDocumentLocation that would be used in SharePointSite loop below.
		end
	...
	begin
		...
		if(@isGridPresent = 1)
			begin
				select @absoluteUrl as 'AbsoluteURL', @siteCollectionUrl as 'SiteCollectionUrl' , @siteRelativeUrl as 'SiteRelativeUrl' , @servicetype as 'ServiceType', @locationName as 'LocationName'
				return
			end
		else
			begin
				select @absoluteUrl as 'AbsoluteURL', '' as 'SiteCollectionUrl' ,  @siteRelativeUrl as 'SiteRelativeUrl' , @servicetype as 'ServiceType', @locationName as 'LocationName'
				return
			end
	end
	...
	begin
		...
		begin
			select '' as 'AbsoluteURL', '' as 'SiteCollectionUrl' , '' as 'SiteRelativeUrl' , @servicetype as 'ServiceType', @locationName as 'LocationName'
		return
		end		
	end
...
begin
	select @parentId = ParentSite, @objectTypeCode = ParentSiteObjectTypeCode , @relativeUrl = RelativeUrl , @absoluteSiteUrl = AbsoluteURL, @isGridPresent = IsGridPresent, @siteCollectionId = SiteCollectionId, @servicetype = ServiceType
	...
	begin
		...
		begin
			...
			if(@isGridPresent = 1)
			begin
				select @absoluteUrl as 'AbsoluteURL', @absoluteSiteUrl as 'SiteCollectionUrl',@siteRelativeUrl as 'SiteRelativeUrl' , @servicetype as 'ServiceType', @locationName as 'LocationName'
				return
			end
		...
		begin
			select '' as 'AbsoluteUrl', '' as 'SiteCollectionUrl',@siteRelativeUrl as 'SiteRelativeUrl' , @servicetype as 'ServiceType', '' as 'LocationName'
			return
		end
	end
	else
