...
-- Checking for Exchange Rate Is Null or Not
		 ...
		 BEGIN
				...
				SELECT @result=SUM(ROUND([IQ].[EstimatedValue]/NULLIF([IQ].[ExchangeRate],0),10)), @maxLimitAggregate=MAX(__AggLimitExceededFlag__)
 FROM ( SELECT TOP (@aggregateQueryRecordLimit + 1) [aggregateEntity].[EstimatedValue], [aggregateEntity].[ExchangeRate], CASE WHEN ROW_NUMBER() OVER(ORDER BY (SELECT 1)) > @aggregateQueryRecordLimit THEN 1 ELSE 0 END AS __AggLimitExceededFlag__
...
-- Leaf Value calculation
				IF (@state = 1) BEGIN 
						 SELECT @leafValue=SUM(ROUND([aggregateEntity].[EstimatedValue]/NULLIF([aggregateEntity].[ExchangeRate],0),10))
						 FROM [Opportunity] [aggregateEntity] 
...
-- Sum of Hierarchy and Leaf Values
	...
	SET @result = ROUND(@result,10)
		IF (@state = 1) BEGIN 
			 SET @result_money = ROUND(@result * @exchRate,10)
		END
	 END TRY
	 BEGIN CATCH
...
-- Update Statement
		 ...
		 UPDATE [AccountBase]
		 SET @previousResult=[OpenRevenue_Base], [OpenRevenue_Base]=@result, [OpenRevenue]=@result_money, [OpenRevenue_Date]=@startDate, [OpenRevenue_State]=@state, @id=[ParentAccountId]
		 WHERE ([AccountId]=@id)
