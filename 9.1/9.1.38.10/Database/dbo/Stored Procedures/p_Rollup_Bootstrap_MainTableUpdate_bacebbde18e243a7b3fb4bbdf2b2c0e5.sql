...
SET NOCOUNT ON;
 ...
 DECLARE @rollupPropertyId  uniqueidentifier 
		 ...
		 SET @rollupPropertyId = 'bacebbde-18e2-43a7-b3fb-4bbdf2b2c0e5'; 
...
, [KnowledgeArticleViews_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
			 ...
			 AND (([a].[KnowledgeArticleViews] IS NULL AND [b].[OutputValue] IS NOT NULL) OR 
			 ([a].[KnowledgeArticleViews] IS NOT NULL AND [b].[OutputValue] IS NULL) OR 
			 ([a].[KnowledgeArticleViews] != [b].[OutputValue]))
		 ...
		 BEGIN 
				 ...
				 BEGIN 
					 ...
					 INSERT INTO [dbo].[Rollup_Bootstrap_Markers_bacebbde18e243a7b3fb4bbdf2b2c0e5]([Id],[Start],[End]) VALUES (@index +1, @start,@end) 
				 END 
		 END 
 ...
 BEGIN 
		 ...
		 SELECT @start = [Start], @end = [End] FROM [dbo].[Rollup_Bootstrap_Markers_bacebbde18e243a7b3fb4bbdf2b2c0e5] WHERE [Id] = @index
...
, [KnowledgeArticleViews_State]= CASE WHEN ([b].[Status] IS NULL OR [b].[Status] = 6) THEN 1 ELSE [b].[Status] END
		 ...
		 DELETE [dbo].[Rollup_Bootstrap_Markers_bacebbde18e243a7b3fb4bbdf2b2c0e5] WHERE [Id] = @index
