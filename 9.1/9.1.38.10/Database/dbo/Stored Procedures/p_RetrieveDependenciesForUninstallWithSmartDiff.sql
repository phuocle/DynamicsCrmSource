create   procedure dbo.p_RetrieveDependenciesForUninstallWithSmartDiff
(
...
-- This stored procedure is much like the p_RetrieveDependenciesForDelete stored
	...
	-- Algorithm:
	-- 1. Find all of the root components for the solution being uninstalled which are not enabled for smartdiff (and all relationships)
	-- 2. Based on the diffInfo, add in smart-diff enabled components which have been deleted or patched.
	...
	create statistics mpcstat on #ComponentSet(dependencynodeid, processinglevel, componentType, objectid)
	...
	declare @UpgradeSolutionId uniqueidentifier
	Select @UpgradeSolutionId=solutionId from SolutionBase where uniqueName in (Select uniqueName+'_Upgrade' from SolutionBase where solutionId = @solutionid)

	If @@ROWCOUNT = 0
	 THROW 51000, 'Cannot find the upgrade solution', 1
	
	Select @json=upgradeInfo from SolutionBase where solutionId = @UpgradeSolutionId

	-- Populate the EnabledComponentTypes table
	Select componentType=t.[value] INTO EnabledComponents from
	(Select [value] from OpenJson(@json, '$.EnabledComponentTypes'))  as t

	-- Populate the Deleted Components Table
	Select componentType=t.[key] ,objectid=cast(D.[value] as uniqueidentifier) INTO DeletedComponents 
	from (Select [key], [value] from OpenJson(@json, '$.DeletedComponents')) t
	CROSS APPLY OpenJson(t.[value]) D
	OPTION (MAXDOP 1);

    -- Populate the Patched Components Table
	Select componentType = t.[key], objectid = cast(D.value as uniqueidentifier) INTO PatchedComponents
	from (Select [key], [value]  from OpenJson(@json, '$.PatchedComponents')) t
	CROSS APPLY OpenJson(t.[value]) D
	OPTION (MAXDOP 1);

	-- insert all of the root components from the given solution which are not enabled for smartdiff
	insert into #ComponentSet
		(dependencynodeid, processinglevel, previousdependencynodeid, 	componentType , objectid)
		select dn.DependencyNodeId, 0, null, dn.ComponentType, dn.ObjectId
		from DependencyNode dn
		join SolutionComponent sc on (dn.ObjectId = sc.ObjectId and dn.ComponentType = sc.ComponentType)
		where sc.SolutionId = @SolutionId and dn.ComponentType not in (Select componentType from EnabledComponents)

	-- For the enabled components, insert all of the root components which have been deleted from smartdiff
	insert into #ComponentSet
		(dependencynodeid, processinglevel, previousdependencynodeid, 	componentType , objectid)
		select dn.DependencyNodeId, 0, null, dn.ComponentType, dn.ObjectId
		from DependencyNode dn
		join DeletedComponents sc on (dn.ObjectId = sc.objectid and dn.ComponentType = sc.ComponentType)

	-- For the enabled components, insert all the root components which have been patched in smartdiff.
	-- This is needed because if a child component of a root has been deleted, the root would've been patched only.
	insert into #ComponentSet
	(dependencynodeid, processinglevel, previousdependencynodeid, 	componentType , objectid)
	select dn.DependencyNodeId, 0, null, dn.ComponentType, dn.ObjectId
	from DependencyNode dn
	join PatchedComponents sc on (dn.ObjectId = sc.objectid and dn.ComponentType = sc.ComponentType)
	...
	exec p_CollectSolutionInternalDescendentsForSmartDiff @IncludeNonCustomRelationships
	...
	insert into #FilteredComponentSet
		(dependencynodeid, processinglevel, previousdependencynodeid, componentType, objectid)
		select cs.dependencynodeid, 0, cs.previousdependencynodeid, dn.ComponentType, dn.ObjectId
	...
	declare @BasicSolutionId uniqueidentifier
	set @BasicSolutionId = '{25A01723-9F63-4449-A3E0-046CC23A2902}'
	...
	declare @Soft int
	set @Soft = 8 -- constant representing all soft dependencies
	...
	insert into @Results (ResultId, 
		...
		select 
			...
			where (d.DependencyType = @Required or d.DependencyType = @Unpublished or d.DependencyType = @Soft)
	...
	insert into @Results (ResultId, 
		...
		select 
			...
			where d.RequiredComponentBaseSolutionId = @SolutionId 
				...
				and d.DependentComponentBaseSolutionId <> @BasicSolutionId
	...
	IF OBJECT_ID(N'DeletedComponents') IS NOT NULL
	DROP TABLE DeletedComponents; 
	IF OBJECT_ID(N'PatchedComponents') IS NOT NULL
	DROP TABLE PatchedComponents; 
	IF OBJECT_ID(N'EnabledComponents') IS NOT NULL
	DROP TABLE EnabledComponents;
end