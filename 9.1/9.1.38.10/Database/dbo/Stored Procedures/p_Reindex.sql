...
create procedure dbo.p_Reindex
as;
begin;
	set nocount on;
...
select @sqlstr = N'alter index @index_name_value ON @table_name_value rebuild with (fillfactor = @fill_factor_value)';
...
open indexCursor;
	...
	begin;
		...
		from AttributeView 
		where IsStoredOnPrimaryTable = 1 
		...
		from AttributeView 
		where IsStoredOnPrimaryTable = 0
			  and AttributeId in (select AttributeId
								  from IndexAttributes
								  where IndexId = @indexid);
		...
		begin;
			print 'ALTER INDEX @indexname ON @baseTableName REBUILD WITH (FILLFACTOR = 80)'
			fetch next from indexCursor
			into @indexid,
				 @indexname,
				 @entityid,
		...
		if (@hasBaseIndex <> 0)
		begin;
			print 'alter index @indexname ON @baseTableName rebuild with (fillfactor = convert(nvarchar(128), @fillfactor))';
			exec dbo.sp_executesql @sqlstr, @parameters, @table_name_value = @baseTableName, @index_name_value = @indexname, @fill_factor_value = @fillfactor;
		end;
	
		if (@hasExtensionIndex <> 0)
		begin;
			print 'alter index @indexname ON @extensionTableName rebuild with (fillfactor = convert(nvarchar(128), @fillfactor))';
			exec dbo.sp_executesql @sqlstr, @parameters, @table_name_value = @extensionTableName, @index_name_value = @indexname, @fill_factor_value = @fillfactor;
		end;
...
declare entityCursor cursor local FORWARD_ONLY READ_ONLY for
	select ExtensionTableName
	from EntityView
	where IsCustomizable <> 0
		  and ExtensionTableName is not null;

open entityCursor
	fetch next from entityCursor
	into @extensionTableName;
