...
create procedure [dbo].[p_RetrievePosts](
    ...
    @sortDirection bit = 0,             -- based on sort direction post will return asc, desc
...
begin;
    ...
    begin;
        ...
        (
            ...
            from [PostBase] post
                where exists
                (
                 Select 1 from [PostRoleBase] prb
        ...
        (
            ...
            where exists
                ( 
                    ...
                    where  post.PostId = prb.PostId 
                      ...
                      and  prb.RegardingObjectId = @entityId and prb.RegardingObjectTypeCode = @entityTypeCode
            ...
            where exists
                ( 
                  Select 1 from [PostRoleBase] prb
                  where  post.PostId = prb.PostId
                )  and post.CreatedBy = @entityId -- created by user
                  and (@type = -1 or post.Type = @type)
                  and (@source = -1 or post.Source = @source)
                  and post.ModifiedOn between @startDate and @endDate
                  and (@keyword is null or Text like @keyword)
    ...
    begin;
        ...
        from (
            ...
            where prb.RegardingObjectId = @entityId
                  ...
                  and (@source = -1 or post.Source = @source)
                  and post.ModifiedOn between @startDate and @endDate
                  and (@keyword is null or Text like @keyword)) PagedResults
        where rowNumber > @pageSize * (@pageNumber - 1)
    ...
    execute dbo.p_RetrievePostsAndComments @currentUserId, @commentsPerPost, @postId, @sortDirection;
end;