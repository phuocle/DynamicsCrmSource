...
SET @maxLimitAggregate=0
	 ...
	 BEGIN TRY
		 ...
		 BEGIN
				...
				SELECT @leafValue=SUM(ROUND([IQ].[NormalizedRating],2)), @maxLimitAggregate=MAX(__AggLimitExceededFlag__), @leafCount=COUNT([IQ].FeedbackId)
...
-- Leaf Value calculation
				IF (@state = 1) BEGIN 
						 SELECT @leafValue=SUM(ROUND([aggregateEntity].[NormalizedRating],2)), @leafCount=COUNT([aggregateEntity].[FeedbackId])
						 FROM [Feedback] [aggregateEntity] 
