...
-- Audit records of metadata change always have empty GUID as ObjectId
declare @result int
if @@ROWCOUNT != 1
begin
	select top 1 @result = 1 from AuditBase
	 where
	...
	and ((CreatedOn = @oldValueCreatedOn and AuditId > @oldValueAuditId) or (CreatedOn > @oldValueCreatedOn))
	OPTION (USE HINT('ENABLE_HIST_AMENDMENT_FOR_ASC_KEYS'))
	if (@result = 1)
	begin
