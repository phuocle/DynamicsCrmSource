...
create procedure dbo.p_PublishLabelsByObjectIdFromTable
	(@metadataPublishCollectionTable dbo.MetadataPublishCollection READONLY, @labelTypeCode int, 
	 @countRowsPublished int OUTPUT )
as
begin
	SET @countRowsPublished = 0
	...
	-- Copy the contents of the passed in table valued variable into the table variable
	IF EXISTS (SELECT 1 FROM @metadataPublishCollectionTable)
	begin
		insert into @MetadataPublishCollectInternal(ObjectId, ObjectTypeCode, ComponentState) select ObjectId, ObjectTypeCode, ComponentState from @metadataPublishCollectionTable
	end

	if @labelTypeCode is NOT NULL
	begin
		insert into @MetadataPublishCollectInternal(ObjectId, ObjectTypeCode, ComponentState) select distinct ObjectId, 0, 0 from LocalizedLabelView where LabelTypeCode = @labelTypeCode
	end
	...
	SET @countRowsPublished = @countRowsPublished + @@ROWCOUNT
