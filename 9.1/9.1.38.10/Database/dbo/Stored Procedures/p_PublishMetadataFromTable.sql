...
create procedure dbo.p_PublishMetadataFromTable
(
	@publishDeep bit,
	@metadataPublishCollectionTable dbo.MetadataPublishCollection READONLY
)
as
begin
	...
	begin
		...
		begin
			...
			--just insert entitykey labels without inserting entitykey as it's not publishable to publish entity key labels during upgrade
			insert into @MetadataPublishCollectInternal(ObjectId, ObjectTypeCode, ComponentState)  
				select l.LocalizedLabelId, 7, l.ComponentState FROM LocalizedLabelAsIfPublishedLogicalView l  
				join EntityKeyView E on E.EntityKeyId = l.ObjectId
				join @MetadataPublishCollectInternal mpc on E.EntityId = mpc.ObjectId  
				where mpc.ObjectTypeCode = 1  
		end
