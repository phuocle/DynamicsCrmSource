CREATE TABLE [dbo].[msdyn_PostRuleConfigBase] (
    ...
    CONSTRAINT [msdyn_postconfig_msdyn_postruleconfig] FOREIGN KEY ([msdyn_PostConfigId]) REFERENCES [dbo].[msdyn_PostConfigBase] ([msdyn_PostConfigId])
);


GO
ALTER TABLE [dbo].[msdyn_PostRuleConfigBase] SET (LOCK_ESCALATION = DISABLE);
...
GO
CREATE NONCLUSTERED INDEX [ndx_QF_Seek_3725C38EB78744979840C25D177713EF]
    ON [dbo].[msdyn_PostRuleConfigBase]([msdyn_PostRuleConfigId] ASC)
    INCLUDE([msdyn_name], [CreatedOn], [statecode], [VersionNumber]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);


GO
CREATE NONCLUSTERED INDEX [ndx_QF_SortedScan_3725C38EB78744979840C25D177713EF]
    ON [dbo].[msdyn_PostRuleConfigBase]([msdyn_name] ASC, [msdyn_PostRuleConfigId] ASC)
    INCLUDE([CreatedOn], [statecode], [VersionNumber]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = ROW);


GO
CREATE NONCLUSTERED INDEX [ndx_QF_Scan_3725C38EB78744979840C25D177713EF]
    ON [dbo].[msdyn_PostRuleConfigBase]([msdyn_PostRuleConfigId] ASC)
