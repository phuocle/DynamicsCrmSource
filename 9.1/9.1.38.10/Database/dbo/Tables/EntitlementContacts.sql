CREATE TABLE [dbo].[EntitlementContacts] (
    ...
    CONSTRAINT [UQ_EntitlementContacts] UNIQUE NONCLUSTERED ([ContactId] ASC, [EntitlementId] ASC) WITH (FILLFACTOR = 80, DATA_COMPRESSION = ROW)
);


GO
ALTER TABLE [dbo].[EntitlementContacts] SET (LOCK_ESCALATION = DISABLE);


GO
CREATE UNIQUE NONCLUSTERED INDEX [fndx_Sync_VersionNumber]
    ON [dbo].[EntitlementContacts]([VersionNumber] ASC) WITH (FILLFACTOR = 80, DATA_COMPRESSION = ROW);


GO
CREATE NONCLUSTERED INDEX [ndx_for_cascaderelationship_entitlement_contacts]
    ON [dbo].[EntitlementContacts]([EntitlementId] ASC) WITH (FILLFACTOR = 80, DATA_COMPRESSION = ROW);
