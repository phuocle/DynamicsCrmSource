...
from Appointment
    ...
    left outer join StringMap [IsDraftPLTable] on 
		([IsDraftPLTable].AttributeName = 'isdraft'
		and [IsDraftPLTable].ObjectTypeCode = 4201
		and [IsDraftPLTable].AttributeValue = [Appointment].[IsDraft]
		and [IsDraftPLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
...
(
	...
	(
		...
		[Appointment].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 4200)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredAppointment] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];
