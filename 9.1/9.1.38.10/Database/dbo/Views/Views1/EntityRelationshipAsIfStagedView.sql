...
create view dbo.[EntityRelationshipAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [EntityRelationship] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [EntityRelationship] WHERE OverwriteTime = 0) A2 ON
