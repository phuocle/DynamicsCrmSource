...
select
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([CampaignResponse].[DeliveryLastAttemptedOn],
			us.TimeZoneCode),
        [CampaignResponse].[DeliveryLastAttemptedOn],
    [CampaignResponse].[DeliveryPriorityCode],
    DeliveryPriorityCodePLTable.Value,
    [CampaignResponse].[Description],
    [CampaignResponse].[EMailAddress],
    [CampaignResponse].[ExchangeItemId],
    [CampaignResponse].[ExchangeRate],
    [CampaignResponse].[ExchangeWebLink],
    [CampaignResponse].[Fax],
    [CampaignResponse].[FirstName],
    [CampaignResponse].[ImportSequenceNumber],
    [CampaignResponse].[InstanceTypeCode],
    InstanceTypeCodePLTable.Value,
    [CampaignResponse].[IsBilled],
    IsBilledPLTable.Value,
    [CampaignResponse].[IsMapiPrivate],
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([CampaignResponse].[LastOnHoldTime],
			us.TimeZoneCode),
        [CampaignResponse].[LastOnHoldTime],
    [CampaignResponse].[LeftVoiceMail],
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([CampaignResponse].[PostponeActivityProcessingUntil],
			us.TimeZoneCode),
        [CampaignResponse].[PostponeActivityProcessingUntil],
    ...
    [CampaignResponse].[SenderMailboxId],
    [CampaignResponse].[SenderMailboxIdName],
    dbo.fn_UTCToTzCodeSpecificLocalTime([CampaignResponse].[SentOn],
			us.TimeZoneCode),
        [CampaignResponse].[SentOn],
    [CampaignResponse].[SeriesId],
    [CampaignResponse].[ServiceId],
    [CampaignResponse].[SLAId],
    [CampaignResponse].[SLAInvokedId],
    [CampaignResponse].[SLAInvokedIdName],
    [CampaignResponse].[SLAName],
    dbo.fn_UTCToTzCodeSpecificLocalTime([CampaignResponse].[SortDate],
			us.TimeZoneCode),
        [CampaignResponse].[SortDate],
...
from CampaignResponse
    ...
    left outer join StringMap [CommunityPLTable] on 
		...
		and [CommunityPLTable].ObjectTypeCode = 4401
		and [CommunityPLTable].AttributeValue = [CampaignResponse].[Community]
    ...
    left outer join StringMap [DeliveryPriorityCodePLTable] on 
		...
		and [DeliveryPriorityCodePLTable].ObjectTypeCode = 4401
		and [DeliveryPriorityCodePLTable].AttributeValue = [CampaignResponse].[DeliveryPriorityCode]
    ...
    left outer join StringMap [InstanceTypeCodePLTable] on 
		...
		and [InstanceTypeCodePLTable].ObjectTypeCode = 4401
		and [InstanceTypeCodePLTable].AttributeValue = [CampaignResponse].[InstanceTypeCode]
    ...
    left outer join StringMap [IsMapiPrivatePLTable] on 
		...
		and [IsMapiPrivatePLTable].ObjectTypeCode = 4401
		and [IsMapiPrivatePLTable].AttributeValue = [CampaignResponse].[IsMapiPrivate]
    ...
    left outer join StringMap [LeftVoiceMailPLTable] on 
		...
		and [LeftVoiceMailPLTable].ObjectTypeCode = 4401
		and [LeftVoiceMailPLTable].AttributeValue = [CampaignResponse].[LeftVoiceMail]
...
(
	...
	(
		...
		[CampaignResponse].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 4200)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredCampaignResponse] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredCampaignResponse] TO [CRMReaderRole]
    AS [dbo];
