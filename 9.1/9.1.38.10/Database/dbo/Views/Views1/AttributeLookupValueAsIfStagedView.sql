...
create view dbo.[AttributeLookupValueAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [AttributeLookupValue] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [AttributeLookupValue] WHERE OverwriteTime = 0) A2 ON
