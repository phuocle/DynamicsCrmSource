...
create view dbo.[LocalizedLabelAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [LocalizedLabel] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [LocalizedLabel] WHERE OverwriteTime = 0) A2 ON
