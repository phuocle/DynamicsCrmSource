...
select
    ...
    [Opportunity].[SkipPriceCalculation],
...
from Opportunity
    ...
    left outer join StringMap [SkipPriceCalculationPLTable] on 
		...
		and [SkipPriceCalculationPLTable].ObjectTypeCode = 3
		and [SkipPriceCalculationPLTable].AttributeValue = [Opportunity].[SkipPriceCalculation]
...
(
	...
	(
		...
		[Opportunity].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 3)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredOpportunity] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredOpportunity] TO [CRMReaderRole]
    AS [dbo];
