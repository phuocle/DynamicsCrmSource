...
select
    ...
    [Contact].[BusinessCardAttributes],
    [Contact].[Callback],
    ...
    [Contact].[msdyn_gdproptout],
    msdyn_gdproptoutPLTable.Value,
    [Contact].[msdyn_orgchangestatus],
    msdyn_orgchangestatusPLTable.Value,
    [Contact].[NickName],
...
from Contact
    ...
    left outer join StringMap [msdyn_orgchangestatusPLTable] on 
		([msdyn_orgchangestatusPLTable].AttributeName = 'msdyn_orgchangestatus'
		and [msdyn_orgchangestatusPLTable].ObjectTypeCode = 2
		and [msdyn_orgchangestatusPLTable].AttributeValue = [Contact].[msdyn_orgchangestatus]
		and [msdyn_orgchangestatusPLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
...
(
	...
	(
		...
		[Contact].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 2)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredContact] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];
