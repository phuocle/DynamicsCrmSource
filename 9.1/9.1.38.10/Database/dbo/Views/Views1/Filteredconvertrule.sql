...
select
    ...
    [ConvertRule].[ConvertRuleType],
    ConvertRuleTypePLTable.Value,
    ...
    [ConvertRule].[SenderResolutionOption],
    SenderResolutionOptionPLTable.Value,
...
from ConvertRule
    ...
    left outer join StringMap [ConvertRuleTypePLTable] on 
		([ConvertRuleTypePLTable].AttributeName = 'convertruletype'
		and [ConvertRuleTypePLTable].ObjectTypeCode = 9300
		and [ConvertRuleTypePLTable].AttributeValue = [ConvertRule].[ConvertRuleType]
		and [ConvertRuleTypePLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
    ...
    left outer join StringMap [SenderResolutionOptionPLTable] on 
		([SenderResolutionOptionPLTable].AttributeName = 'senderresolutionoption'
		and [SenderResolutionOptionPLTable].ObjectTypeCode = 9300
		and [SenderResolutionOptionPLTable].AttributeValue = [ConvertRule].[SenderResolutionOption]
		and [SenderResolutionOptionPLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
...
(
	...
	(
		...
		[ConvertRule].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 9300)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[Filteredconvertrule] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];
