...
create view dbo.[EntityRelationshipRoleAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [EntityRelationshipRole] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [EntityRelationshipRole] WHERE OverwriteTime = 0) A2 ON
