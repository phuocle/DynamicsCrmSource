...
from SystemUser
    ...
    left outer join StringMap [IsLicensedPLTable] on 
		([IsLicensedPLTable].AttributeName = 'islicensed'
		and [IsLicensedPLTable].ObjectTypeCode = 8
		and [IsLicensedPLTable].AttributeValue = [SystemUser].[IsLicensed]
		and [IsLicensedPLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
    left outer join StringMap [msdyn_gdproptoutPLTable] on 
		...
		and [msdyn_gdproptoutPLTable].ObjectTypeCode = 8
		and [msdyn_gdproptoutPLTable].AttributeValue = [SystemUser].[msdyn_gdproptout]
...
(
	...
	(
		...
		[SystemUser].[BusinessUnitId] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 8)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredSystemUser] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredSystemUser] TO [CRMReaderRole]
    AS [dbo];
