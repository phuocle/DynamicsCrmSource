...
create view dbo.[PrivilegeObjectTypeCodeAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [PrivilegeObjectTypeCodes] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [PrivilegeObjectTypeCodes] WHERE OverwriteTime = 0) A2 ON
