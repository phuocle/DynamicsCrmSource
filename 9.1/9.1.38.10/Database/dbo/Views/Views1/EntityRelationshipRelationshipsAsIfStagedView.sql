...
create view dbo.[EntityRelationshipRelationshipsAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [EntityRelationshipRelationships] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [EntityRelationshipRelationships] WHERE OverwriteTime = 0) A2 ON
