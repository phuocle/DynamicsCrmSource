...
select
    ...
    [Workflow].[IsCustomProcessingStepAllowedForOtherPublishers],
    [Workflow].[IsManaged],
    ...
    [Workflow].[ProcessTriggerFormId],
    [Workflow].[ProcessTriggerScope],
    ProcessTriggerScopePLTable.Value,
    [Workflow].[Rank],
...
from Workflow
    ...
    left outer join StringMap [ProcessTriggerScopePLTable] on 
		([ProcessTriggerScopePLTable].AttributeName = 'processtriggerscope'
		and [ProcessTriggerScopePLTable].ObjectTypeCode = 4703
		and [ProcessTriggerScopePLTable].AttributeValue = [Workflow].[ProcessTriggerScope]
		and [ProcessTriggerScopePLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
    ...
    left outer join StringMap [UIFlowTypePLTable] on 
		([UIFlowTypePLTable].AttributeName = 'uiflowtype'
		and [UIFlowTypePLTable].ObjectTypeCode = 4703
		and [UIFlowTypePLTable].AttributeValue = [Workflow].[UIFlowType]
		and [UIFlowTypePLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
...
(
	...
	(
		...
		[Workflow].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 4703)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredWorkflow] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredWorkflow] TO [CRMReaderRole]
    AS [dbo];
