...
create view dbo.[EntityKeyAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [EntityKey] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [EntityKey] WHERE OverwriteTime = 0) A2 ON
