...
select
    ...
    CustomerContactedPLTable.Value,
    [Incident].[CustomerId],
...
from Incident
    ...
    left outer join StringMap [CustomerContactedPLTable] on 
		([CustomerContactedPLTable].AttributeName = 'customercontacted'
		and [CustomerContactedPLTable].ObjectTypeCode = 112
		and [CustomerContactedPLTable].AttributeValue = [Incident].[CustomerContacted]
		and [CustomerContactedPLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
    ...
    left outer join StringMap [RouteCasePLTable] on 
		([RouteCasePLTable].AttributeName = 'routecase'
		and [RouteCasePLTable].ObjectTypeCode = 112
		and [RouteCasePLTable].AttributeValue = [Incident].[RouteCase]
		and [RouteCasePLTable].LangId = 
			case us.UILanguageId 
				when 0 then o.LanguageCode
				else us.UILanguageId
			end)
...
(
	...
	(
		...
		[Incident].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap where SystemUserId = u.SystemUserId and ObjectTypeCode = 112)
	) 
	or
	(
		-- global security
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FilteredIncident] TO [D365\ReportingGroup {17e68c54-332d-46c1-9c02-8ffa9543cd64}]
    AS [dbo];
