...
create view dbo.[ManagedPropertyAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [ManagedProperty] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [ManagedProperty] WHERE OverwriteTime = 0) A2 ON
