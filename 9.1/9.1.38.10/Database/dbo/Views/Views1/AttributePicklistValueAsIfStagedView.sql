...
create view dbo.[AttributePicklistValueAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [AttributePicklistValue] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [AttributePicklistValue] WHERE OverwriteTime = 0) A2 ON
