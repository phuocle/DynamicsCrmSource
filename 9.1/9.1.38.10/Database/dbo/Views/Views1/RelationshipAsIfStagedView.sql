...
create view dbo.[RelationshipAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [Relationship] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [Relationship] WHERE OverwriteTime = 0) A2 ON
