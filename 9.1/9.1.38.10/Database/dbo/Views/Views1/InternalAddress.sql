...
from [InternalAddressBase] 
	...
	left join BusinessUnitBase XXbusinessunit  on ([InternalAddressBase].ParentId = XXbusinessunit.BusinessUnitId and [InternalAddressBase].ObjectTypeCode = 10)
	left join SystemUserBase XXsystemuser  on ([InternalAddressBase].ParentId = XXsystemuser.SystemUserId and [InternalAddressBase].ObjectTypeCode = 8)

	left join SiteBase XXsite on ([InternalAddressBase].ParentId = XXsite.SiteId and[InternalAddressBase].ObjectTypeCode = 4009)
    left join [SystemUserBase] [lk_internaladdressbase_createdby]  on ([InternalAddressBase].[CreatedBy] = [lk_internaladdressbase_createdby].[SystemUserId])
    left join [SystemUserBase] [lk_internaladdressbase_createdonbehalfby]  on ([InternalAddressBase].[CreatedOnBehalfBy] = [lk_internaladdressbase_createdonbehalfby].[SystemUserId])
    left join [SystemUserBase] [lk_internaladdressbase_modifiedby]  on ([InternalAddressBase].[ModifiedBy] = [lk_internaladdressbase_modifiedby].[SystemUserId])
    left join [SystemUserBase] [lk_internaladdressbase_modifiedonbehalfby]  on ([InternalAddressBase].[ModifiedOnBehalfBy] = [lk_internaladdressbase_modifiedonbehalfby].[SystemUserId])
