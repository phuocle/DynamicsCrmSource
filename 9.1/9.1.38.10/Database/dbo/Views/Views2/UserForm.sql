...
select
    ...
    [UserFormBase].[IsTabletEnabled],
    [UserFormBase].[FormJson]
from [UserFormBase] 
    left join [SystemUserBase] [lk_userform_createdby] on ([UserFormBase].[CreatedBy] = [lk_userform_createdby].[SystemUserId])
    left join [SystemUserBase] [lk_userform_modifiedby] on ([UserFormBase].[ModifiedBy] = [lk_userform_modifiedby].[SystemUserId])
    left join [SystemUserBase] [lk_userformbase_createdonbehalfby] on ([UserFormBase].[CreatedOnBehalfBy] = [lk_userformbase_createdonbehalfby].[SystemUserId])
    left join [SystemUserBase] [lk_userformbase_modifiedonbehalfby] on ([UserFormBase].[ModifiedOnBehalfBy] = [lk_userformbase_modifiedonbehalfby].[SystemUserId])
    left join OwnerBase XXowner on ([UserFormBase].OwnerId = XXowner.OwnerId)
