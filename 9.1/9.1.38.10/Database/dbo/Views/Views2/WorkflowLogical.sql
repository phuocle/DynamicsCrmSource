...
select
    ...
    [T1].[UIFlowType],
    [T1].[ProcessTriggerFormId],
    [T1].[ProcessTriggerScope],
    [T1].[IsCustomProcessingStepAllowedForOtherPublishers]
from [WorkflowBase] [T1]
    ...
    left join [SystemUserBase] [workflow_createdby] on ([T1].[CreatedBy] = [workflow_createdby].[SystemUserId])
    left join [SystemUserBase] [workflow_createdonbehalfby] on ([T1].[CreatedOnBehalfBy] = [workflow_createdonbehalfby].[SystemUserId])
    left join [ImageDescriptor] [workflow_entityimage] on ([T1].[EntityImageId] = [workflow_entityimage].[ImageDescriptorId])
    left join [SystemUserBase] [workflow_modifiedby] on ([T1].[ModifiedBy] = [workflow_modifiedby].[SystemUserId])
    left join [SystemUserBase] [workflow_modifiedonbehalfby] on ([T1].[ModifiedOnBehalfBy] = [workflow_modifiedonbehalfby].[SystemUserId])
