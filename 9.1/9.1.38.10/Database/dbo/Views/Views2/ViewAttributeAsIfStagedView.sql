...
create view dbo.[ViewAttributeAsIfStagedView] as SELECT A1.* FROM (SELECT * FROM [ViewAttribute] WHERE OverwriteTime = 0) A1 LEFT OUTER JOIN (SELECT * FROM [ViewAttribute] WHERE OverwriteTime = 0) A2 ON
