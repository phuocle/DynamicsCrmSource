...
create view dbo.v_attributeobjecttypes (EntityId, 
									...
									ValidForGrid, ValidForForm, ReadOnlyInUI, ValidForRead, ValidForCreate, ValidForUpdate,
									IsNonSortableCalculatedField, Datatype, MaxLength, IsCustomField) as
select Attribute.EntityId, 
	   ...
	   -- readonlyinui
	   case cast(coalesce(Attribute.DisplayMask & 4194304, 0) as bit)
		 when 1 then 'true'
		 else 'false'
	   end,
	   -- validforread
	   ...
	   case when cast(coalesce(Attribute.SourceTypeMask & 2, 0) as bit) <> 0 OR 
		 cast(coalesce(Attribute.SourceTypeMask & 4, 0) as bit) <> 0 OR 
		 cast(coalesce(Attribute.SourceTypeMask & 8, 0) as bit) <> 0 
	   ...
	   case AttributeTypes.Description
		...
		when 'multiselectpicklist' then 'multiselectpicklist'
