...
-- Create p_CreateRefreshPSqlTables stored procedure, to dynamically create p_RefreshPSqlTables.
CREATE PROC [XrmTds].[p_CreateRefreshPSqlTables]
As
BEGIN;

	IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[XrmTds].[p_RefreshPSqlTables]') AND type IN (N'P', N'SP')) 
		DROP PROCEDURE [XrmTds].[p_RefreshPSqlTables]

	-- Create p_RefreshPSqlTables stored procedure.
	EXEC (N'CREATE PROC [XrmTds].[p_RefreshPSqlTables]
AS
BEGIN
