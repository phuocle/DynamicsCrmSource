...
-- Create p_CreateRefreshPSqlTables_Diff stored procedure, to dynamically create p_RefreshPSqlTables_Diff.
CREATE PROC [XrmTds].[p_CreateRefreshPSqlTables_Diff]
AS
BEGIN

	IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[XrmTds].[p_RefreshPSqlTables_Diff]') AND type IN (N'P', N'SP')) 
		DROP PROCEDURE [XrmTds].[p_RefreshPSqlTables_Diff]

	-- Create p_RefreshPSqlTables_Diff stored procedure.
	EXEC (N'CREATE PROC [XrmTds].[p_RefreshPSqlTables_Diff]
AS
BEGIN
	SET NOCOUNT ON;
