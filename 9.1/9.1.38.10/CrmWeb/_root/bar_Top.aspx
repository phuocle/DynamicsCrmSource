...
<cnt:AppHeader id="crmHeader" runat="server" />
    ...
    function DownloadClient()
    {
	    ...
	    if(IsNull(oFrame))
		{
	    	...
	    	oFrame = window.document.createElement("<iframe style='display:none' id='frmDownloadOlk' name='frmDownloadOlk' src='" + CrmEncodeDecode.CrmHtmlAttributeEncode(srcValue) + "'></iframe>");
		    oFrame = window.document.body.insertBefore(oFrame);
		    //give the frame a chance to get into the DOM use a timeout to set the url
        ...
        function CheckAddinAsync() {
            if (!IsNull(document.getElementById("btn_download_olk")))
            {
                ...
                // this error can happens when the addin is present but the call
                // we're trying to use fails. We know the addin is there, so
                // just hide it anyway
...
<table id="barTopTable" cellpadding="0" cellspacing="0">
	...
	<tr>
		<!-- We need to use extra inner tables because otherwise the gradient doesn't work correctly (it works best when defined on the table tag) -->		
