...
var _defaultTab = <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(defaultTab) %>;
var _formDirty = false;
var _isCustomizabe = '<%= canCustomize.ToString().ToLower() %>';
var _crmFormSubmit;
var _xmlDoc = null;
var dependenciesTab = null, generalTab = null;
var _dependenciesTabLoadedOnce = true;

Sys.Application.add_load(function () {
dependenciesTab = $get("tab1");
generalTab = $get("tab0");

$get("tab1Tab").style.display = "none";
dependenciesTab.style.display = "none";
generalTab.style.display = "none";

$addHandler($get("tab1Tab"), "click", ShowDependenciesTabs);
$addHandler($get("tab0Tab"), "click", ShowGeneralTab);
...
var webResourceName = _crmFormSubmit.displayName.value;
if(isNullOrEmptyString(_crmFormSubmit.id.value)){
webResourceName = window.top.LOCID_FORM_TITLE_NEW;
}

Mscrm.Utilities.updatePowerAppsNavBarComponentName(webResourceName);

_crmFormSubmit.tabSelected.value = _defaultTab;
$find('crmTabBar').down($get(_defaultTab+"Tab"), true);
$get(_defaultTab).style.display = "block";

if (isNullOrEmptyString(_dependencyXml))
{
initializeWebResourceDependencyXml();
}
else
{
...
Mscrm.FormLibraryAndEventHandlerUtils.renderWebResourceAttributeDependencyControl();
}
...
function ShowControlBarIngridIfHidden()
{
try
{
var libraryGrid = $get('libraryGrid');
var attributeGrid = $get('attributeGrid');

if (libraryGrid.style && libraryGrid.style.display != "none" && $get("libraryMenuBar"))
{
var libraryLinks = $get("addLibrary");
var attributeLinks = $get("addAttribute");
var removeLibrary = $get("removeLibrary");
var removeAttribute = $get("removeAttribute");

libraryLinks.style.display = "";
attributeLinks.style.display = "";
removeLibrary.style.display = "";
removeAttribute.style.display = "";

var elems = [libraryLinks, attributeLinks, removeLibrary, removeAttribute];
for (var y = 0; y < elems.length; y++)
{
menubarLinks = elems[y].getElementsByTagName("span");
for (var x = 0; x < menubarLinks.length; x++)
{
if (menubarLinks[x].style)
{
menubarLinks[x].style.display = "";
}
}
}
}
...
{
Xrm.Internal.trace.warning("edit.aspx", "ignoring commandbar showing in dependencies grid : {0}", err);
}
}

function ShowDependenciesTabs()
{
$find('crmTabBar').down(dependenciesTab, true);
_crmFormSubmit.tabSelected.value = "tab1";
ShowControlBarIngridIfHidden();
}


function ShowGeneralTab()
{
$find('crmTabBar').down(generalTab, true);
_crmFormSubmit.tabSelected.value = "tab0";
}

function initializeWebResourceDependencyXml()
{
_xmlDoc = createEmptyWebResourceDependencyXml();
...
{
_crmFormSubmit.webResourceDependencyXML.value = serializeWebResourceDependencyXML(_dependencyXml);
editWR();
}
...
<input name="tabSelected"	id="tabSelected"	type="hidden" />
...
<td>
<input id="pathAndFileName" name="pathAndFileName" type="text" class="ms-crm-Text" maxlength="100" onfocus="Focus();" onkeypress="setFormDirty();" runat="server" />
</td>
</tr>
</table>
</td>
...
<label id="IsEnabledForMobileClientLabel" for="IsEnabledForMobileClient">
...
<label id="IsAvailableForMobileOfflineLabel" for="IsAvailableForMobileOffline">
<loc:Text ResourceId="SystemCustomization_Webresource_field_Label" runat="server"/>
</label>
</td>
</tr>
</table>
...
<div id="tab1" class="ms-crm-Tab" style="display:block;">
<table cellspacing="0" cellpadding="0"  style="width:99%;">
<tr>
<td />
<td colspan="7" class="ms-crm-Form-Section ms-crm-Form-SectionBar">
<A id="dependencySectionTitle" class="ms-crm-InlineTabHeaderText" >
<H3 class="ms-crm-Form">
<loc:Text ResourceId="WebResource_Localization_MobileOffline_Dependency_Section_Label" runat="server"/>
</H3>
</A>
</td>
</tr>
...
<A id="attributedependencysectionmessage" class="ms-crm-InlineTabHeaderText" >
<H3 class="ms-crm-Form">
</H3>
</A>
</td>
...
<tr>
<td>
<cnt:AttributeDependencyControl id="attributeDependencyControl" runat="server"/>
</td>
</tr>
<tr />
</table>
