...
if (columnLable)
{
XUI.Html.SetText(columnLable, formatString(LOCID_COLUMNS_SELECTED_SAVE, IsNull(numColumns)? LOCID_COLUMNS_DEFAULT : numColumns));
}
}
...
var fetchXml	=	"<fetch><entity name=\"" + <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(objectTypeName) %> + "\"><filter></filter></entity></fetch>";
...
var oURL = Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/AddColumns.aspx?mode=select&maxcolumns=" + CrmEncodeDecode.CrmUrlEncode(maxSelectedColumns));
...
var callbackFunction = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterPublishTemplate", this);
var ret = openStdDlgWithCallback(Mscrm.CrmUri.create("/_grid/cmds/dlg_publishmailmergetemplate.aspx"), oArgs.Ids,650, 200, callbackFunction, true, false, "maximize:yes;minimize:yes");
...
try
{
window.opener.auto(<%=Microsoft.Crm.Util.MailMergeTemplate.ToString("D", System.Globalization.CultureInfo.InvariantCulture) %>);
}
catch(e)
{}
...
ruleIdArray[0]= <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(this.CurrentEntity.Id) %>;
oArgs.Ids = ruleIdArray;
var callbackFunction = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterUnpublishTemplate", this);
var ret = openStdDlgWithCallback(Mscrm.CrmUri.create("/_grid/cmds/dlg_unpublishmailmergetemplate.aspx"), oArgs.Ids, 650, 200, callbackFunction, true, false, "maximize:yes;minimize:yes");
if (isOutlookHostedWindow())
{
...
<div class="ms-crm-absPosition" style="position:fixed;top:48px;height:calc(100% - 48px);">
<%
}
else
{
