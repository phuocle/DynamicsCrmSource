...
AddMouseOverAsEventListener();
InitializeMouseOverObserver();
}

function AddMouseOverAsEventListener()
{


var linkIDs = ["chartLink","gridLink","IframeLink","WebResourceLink"];
for(var i = 0; i < linkIDs.length; i++)
{
var linkElements = $("[id='" + linkIDs[i] + "']");
if(linkElements && linkElements.length > 0)
{
linkElements.mouseover(Mscrm.PreviewCellUtils.onHover);
}
}

var tdElems = $("td[data-addmouseover='true']");
if(Mscrm.FormEditorVariables.isCustomizabe.toLowerCase() == "true" && tdElems && tdElems.length > 0)
{
tdElems.mouseover(function(event) {
Mscrm.DashboardEditorUtils.registerDragObjectForCell(Mscrm.Utilities.eventToDomEvent(event), this);
});
tdElems.mousedown(function(event) {



if (this && this.classList && this.classList.contains("keyboardFocusClass")) {
this.classList.remove("keyboardFocusClass");
}
});
}
}
...
/**
* Initializes the observer that will reapply the mouse over event listener.
*/
function InitializeMouseOverObserver()
{
if (!_mouseOverObserver)
{
var editorDiv = document.getElementById("editorDiv");
if (editorDiv)
{
_mouseOverObserver = new MutationObserver(AddMouseOverAsEventListener);
_mouseOverObserver.observe(editorDiv, {
childList: true,
subtree: true
});
}
...
/**
* Removes the mouse over observer
*/
function RemoveMouseOverObserver()
{
if (_mouseOverObserver)
{
_mouseOverObserver.disconnect();
...
var success = Mscrm.FormUtils.saveForm(0, 0, dashName.get_dataValue(), dashDesc.get_dataValue(),Mscrm.FormEditorVariables.isTabletEnabled);
if(success)
{
...
var resultobj = {"id":id, "displayName": dashName.get_dataValue(), "description": dashDesc.get_value(), "formType": 0}
...
if(Mscrm.FormEditorVariables.formId!='00000000-0000-0000-0000-000000000000')
{
originalOpener.postMessage({"formId":Mscrm.FormEditorVariables.formId}, '*');
}
