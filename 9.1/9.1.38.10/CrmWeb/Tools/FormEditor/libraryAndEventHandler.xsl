...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="Label_LanguageCode" />
	...
	<xsl:template name="renderHandler">
		<tr height="20px" onclick="Mscrm.FormLibraryAndEventHandlerUtils.setHandlerActive(Mscrm.Utilities.eventToDomEvent(event));" ondblclick="Mscrm.FormLibraryAndEventHandlerUtils.editHandler();">
			...
			<td class="ms-crm-library-cell">
				<a title="" onkeydown="Mscrm.FormLibraryAndEventHandlerUtils.handlerKeyPress(Mscrm.Utilities.eventToDomEvent(event));" onclick="return false;" href="javascript:onclick();" target="_self" >
					<xsl:value-of select="@libraryName" />
				</a>
			</td>
			<td class="ms-crm-library-cell">
	...
	<xsl:template match="/formLibraries | /Dependency[@componentType='WebResource']">
		<table cellspacing="0" cellpadding="0" width="100%" style="table-layout:fixed;">
			...
			<tr height="20px" class="ms-crm-library-header-row">
				...
				<th class="ms-crm-library-header-cell">
					<xsl:attribute name="title">
						<xsl:value-of select="$Label_LanguageCode" />
					</xsl:attribute>
					<xsl:value-of select="$Label_LanguageCode" />
				</th>
				<th class="ms-crm-library-header-cell">
	...
	<xsl:template name="renderLibrary">
		<tr onclick="Mscrm.FormLibraryAndEventHandlerUtils.setLibraryActive(Mscrm.Utilities.eventToDomEvent(event)); " ondblclick="Mscrm.FormLibraryAndEventHandlerUtils.editLibrary();" height="20px">
			<xsl:attribute name="id">
			...
			<xsl:attribute name="libraryDisplayName">
				<xsl:value-of select="@displayName" />
			</xsl:attribute>
			<xsl:attribute name="libraryLanguageCode">
				<xsl:value-of select="@languagecode" />
			</xsl:attribute>
			<xsl:attribute name="libraryDescription">
				<xsl:value-of select="@description" />
			</xsl:attribute>
			<td class="ms-crm-library-cell">
				<a onkeydown="Mscrm.FormLibraryAndEventHandlerUtils.handleLibraryKeyPress(Mscrm.Utilities.eventToDomEvent(event));" onclick="return false;" href="javascript:onclick();" target="_self" >
					<xsl:attribute name="title">
						<xsl:value-of select="@name" />
					</xsl:attribute>
					<xsl:value-of select="@name" />
