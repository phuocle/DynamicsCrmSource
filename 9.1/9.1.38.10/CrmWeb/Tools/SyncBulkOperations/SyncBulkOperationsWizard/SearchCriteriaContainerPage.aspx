<%@ Page language="c#" Inherits="Microsoft.Crm.Web.Tools.SyncBulkOperations.SearchCriteriaContainerPage" %>
...
{
var url = Mscrm.CrmUri.create("/Tools/SyncBulkOperations/SyncBulkOperationsWizard/SearchCriteriaPage.aspx");
var entityName;
var fetchXml;
...
{
var oUrl = Mscrm.CrmUri.create("/Tools/SyncBulkOperations/SyncBulkOperationsWizard/AdditionalOptionsPage.aspx");
...
var fetchXmlDoc = parser.parseFromString(fetchXml, "text/xml");
var fetchXmlConditions = fetchXmlDoc.getElementsByTagName("condition");
var containsWorkflowJobType = false;

for (i = 0; i < fetchXmlConditions.length; i++) {
if ((fetchXmlConditions[i].getAttribute('attribute') == 'operationtype') && (fetchXmlConditions[i].getAttribute('operator') == 'eq') &&
(fetchXmlConditions[i].getAttribute('value') == '10')) {
containsWorkflowJobType = true;
break;
}
}


if (!containsWorkflowJobType) {
var msg = "<% = Microsoft.Crm.Utility.AppResourceManager.Default.GetString("SyncBulkOperationsWizard.SelectCriteriaPage.SystemJobTypeNotIncluded") %>";
alert(msg);
return;
}
