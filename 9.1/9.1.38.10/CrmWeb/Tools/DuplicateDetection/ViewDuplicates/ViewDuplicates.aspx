...
.ms-crm-RefreshDialog-Warning { left: 4px; right: 4px;}
.ViewDuplicates_td_MyRecRsrcStr { padding-top: 11px; padding-bottom: 15px;}
...
var url = Mscrm.CrmUri.create("/Tools/DuplicateDetection/ViewDuplicates/DuplicateRecordsGrid.aspx?source=" + <% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(source)) %>  + "&dType=" + <% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(dialogType.ToString(CultureInfo.InvariantCulture))) %> + "&matchingentityname=" + matchingEntityName).toString();

<% if (duplicatesSource == Microsoft.Crm.Application.Utility.Util.DuplicatesSource.SystemWide)
{%>
url = url + "&jobid=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(asyncJobId)) %>" + "&oid=" + oid + "&baseentityname=" + baseEntityName;
<% } %>
<% else if (!string.IsNullOrEmpty(Request.QueryString["oid"]))
{%>
url = url + "&oid=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(Request.QueryString["oid"])) %>";
<%} %>
...
<% if (duplicatesSource == Microsoft.Crm.Application.Utility.Util.DuplicatesSource.SystemWide)
{%>
url = url + "&jobid=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(asyncJobId)) %>";
<% } %>
...
var url = Mscrm.CrmUri.create("/Tools/DuplicateDetection/ViewDuplicates/DuplicatesList.aspx?source=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(source)) %>").toString();

<% if (duplicatesSource == Microsoft.Crm.Application.Utility.Util.DuplicatesSource.SystemWide)
{%>
url = url + "&jobid=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(asyncJobId)) %>" + "&oid=" + oid + "&entityname=" + baseEntityName;
<% }%>
<% else if (!string.IsNullOrEmpty(Request.QueryString["oid"]))
{%>
url = url + "&oid=<% =Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(Microsoft.Crm.CrmEncodeDecode.CrmUrlEncode(Request.QueryString["oid"])) %>";
...
var _submitUrl = Mscrm.CrmUri.create("/tools/duplicatedetection/viewduplicates/viewduplicates.aspx");
...
var returnValue = "<OpenRecord><id>" + selectedRecords[0].Id + "</id><type>" + selectedRecords[0].TypeName + "</type></OpenRecord>";
