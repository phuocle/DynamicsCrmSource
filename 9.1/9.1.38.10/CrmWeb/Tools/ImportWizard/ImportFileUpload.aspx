...
{
$get("errorFrame").src = Mscrm.CrmUri.create("/_common/error/popuperror.aspx?hr=" + errorCode).toString();
}
}
...
if (!isValidFilename(currentFile.name)) {
window.alert(LOCID_FILEUPLOAD_INVALIDFILENAME);
return false;
}
...
div.innerHTML='<% =CurrentWrpcContext.GetWrpcTokenAsHiddenInputString(Microsoft.Crm.Application.Utility.CrmUri.Create("/Tools/ImportWizard/ImportFileUpload.aspx", Microsoft.Crm.Application.Security.UserInformation.Current)) %>';
...
document.getElementById("uploadFileNameId").setAttribute("aria-labelledby","");
}
}
...
<span class="mscrm-iw-PageWidth" id="dataFileNameLabel">
<loc:Text ResourceId="ImportWizard.UploadFilePage.UploadIFrame.Header" runat="server" />
<label id="hiddenFileName" style="opacity:0" role="alert"></label>
</span>
<br />
<br />
<input id='uploadFileNameId' type='file' name='uploadFileName' class="mscrm-iw-PageWidth ms-crm-WhiteBackground" onkeypress="handleKeyPress(new Sys.UI.DomEvent(event))" onclick="handleKeyPress(new Sys.UI.DomEvent(event))" aria-labelledby="" />
<br />
...
<span><loc:Text ResourceId="Web.UploadDocumentTemplatePage.DropFileHere" runat="server" /></span>
</div>
