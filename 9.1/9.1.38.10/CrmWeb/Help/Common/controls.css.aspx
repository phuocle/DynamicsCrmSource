...
<%-- CRM SE 35766: Indenting/Outdenting should not add spacing between lines. --%>
...
<%-- We need this here specifically because some control(phone, email) want to show the value always in LTR and for that we need to make complete chrome element as LTR because:
	1) The tooltip in chrome element should be in LTR.
	2) The read value should be LTR
	3) The edit value should be LTR (This is already taken care by correct style in edit mode).
Adding css property "direction:ltr" at chrome element thus become necessary because of reason#1. However, we need to take care of:
	1) If the value is empty, we should not be showing the read value as LTR always(Fixed by setting correct direction if the class is empty)
	2) The edit div width is rendered with width 100%, so if in RTL mode we render it as LTR the input box will start truncating. (Fixed by setting correct direction specifically for div)
The scope of the fix is limited since we just set these styles if the base element is chrome and has class ms-crm-ltrcontrol
 --%>
...
// CRM SE 26117
// IE10 displays radio controls with height less than default 20px incorrectly
INPUT.radio
{
	height:				20px;
}
...
.ms-crm-Input
{
	...
	padding-left: 0px;
	padding-bottom: 0px;
	/* padding-right:0 is hiding the cursor sign in RTL locales */
	<% if (!CrmStyles.IsRightToLeft) { %>
	padding-right: 0px;
	<%} %>
}
...
INPUT.ms-crm-Ticker
{
 ...
 /* CRM SE 4454: V4: Cannot assign value to a bit field, updating an entity in WF */
 /* Donaldla 05/06/2008*/
...
BUTTON.ms-crm-Button
{
	...
	/* This makes the button at least 84px wide.  The width will grow to accommodate the text that's in it. */	
	min-width: 84px;
	width: auto;	
	white-space:nowrap;
}
...
//Bug 304891 : Changing the margin-top to 0px to force IE11 to repaint the span. IE11 fails to apply the background color if there no layout changes.
...
td.ms-crm-Money-CurrencySymbol
{
	...
	/* even on RTL, the currencySymbol is always on the left of the value input box */
...
{
	/* This is to match the currencySymbol and input controls */
	border-left-width: 0;
	padding-left: 0px;
}
...
<%-- CRM SE 35766: When we insert a list, make sure the text container is displayed inline (ie- not underneath the list bullet/number) --%>
...
<%-- Begin styles for InlineEdit controls --%>
...
#lost-competitor .ms-crm-Inline-Edit.ms-crm-Inline-Lookup
{
	<% if (CrmStyles.IsRightToLeft) { %>
		margin-left: 5px;
		margin-left: 6px \0/;/*for ie8 & 9*/
	<% } else { %>
		margin-right: 5px;
		margin-right: 6px \0/;/*for ie8 & 9*/
	<% } %>
}
...
{
	<% if (CrmStyles.IsRightToLeft) { %>
	/*23px is for warning/lock icon. 6px is for 4px (padding-right of .ms-crm-Inline-Edit input.ms-crm-InlineInput) + 2 (1 for each border) */
	margin-right: 23px;
	margin-left: 6px;
	margin-left: 7px \0/;/*for ie8 & 9*/
	<% } else { %>
	/*must be (margin-left of .ms-crm-Inline-Value) */
	margin-left: 23px;
	/*must be (padding-left of .ms-crm-Inline-Edit input.ms-crm-InlineInput) + 2 (1 for each border) */
	margin-right: 6px;
	margin-right: 7px \0/;/*for ie8 & 9*/
	<% } %>
}
...
.ms-crm-Inline-Edit input.ms-crm-InlineInput
{
	<% if (CrmStyles.IsRightToLeft) { %>
	padding-right: 4px;
	padding-right: 5px \0/;/*for ie8 & 9*/
	<% } else { %>
	padding-left: 4px;
	padding-left: 5px \0/;/*for ie8 & 9*/
	<% } %>
}

.ms-crm-Inline-Edit.ms-crm-Inline-OptionSet
{
	/* Height on InlineEdit ensure that the control expands in overlay mode and doesn't shift other control down*/
	<% if (this.IsVisualRefreshEnabled) { %>
	height:23px;
	<% } else { %>
	height:22px;
	<% } %>
	height:23px \0/; /*only for ie8 & 9*/
...
:root .ms-crm-Inline-Edit.ms-crm-Inline-OptionSet
{
	...
	<% } else { %>
		margin-right: 1px \0/IE9; /*only for ie9*/
	<% } %>
}
...
.ms-crm-Inline-Edit.ms-crm-HeaderTile
{
	...
	margin: 2px 7px 0 2px\9; /*only for ie8*/
	*margin: 2px 6px 0 2px\9; /*only for ie7*/
}

:root .ms-crm-Inline-Edit.ms-crm-HeaderTile
{
	margin: 1px 7px 0 2px \0/IE9; /*only for ie9*/
}
...
{
	padding-top: 2px;
	padding-top: 1px \0/; /*only for ie8 & 9*/
	padding-bottom: 2px;
	border: none;
}
...
.ms-crm-Inline-Edit select.ms-crm-SelectBox
{
	...
	/*prevent 1px jitter when transitioning from Read to Edit and back.*/
	<% if (CrmStyles.IsRightToLeft) { %>
	margin-right: 1px;
	margin-left: 2px \0/;/*only for ie8 & 9*/
	<% } else { %>
	margin-left: 1px;
	margin-left: 2px \0/;/*only for ie8 & 9*/
	<% } %>
...
{
	border-top: 0.6px dashed #828181; /* as per new redline */
	position: relative;
	top: 10px;
	border-width: 1px;
}
...
<%-- begin styles for tablet header --%>
...
<%-- This class is used to hide an element without display:none. It is here for workaround IE10 <select> element animation issue. --%>
...
<%-- This class is used to force Height 0 to the decendents of .ms-crm-Inline-HideByZeroHeight for OptionSet--%>
select.ms-crm-Inline-HideByZeroHeight-Ie7 {
	*height: 0; /* ie6 ie7 */
}

<%-- When there is no business required/recommended image. --%>
...
<%-- Offset equals width of business required/recommended image. --%>
...
<%-- Display requiredIcon with Offset of image without fading--%>
...
.ms-crm-HeaderTileElement .money .ms-crm-Inline-Value.ms-crm-HeaderTile.ms-crm-Inline-EmptyValue div.ms-crm-Inline-GradientMask {
	background: rgba(255, 255, 255, 0.15); /* for non-css3 browsers */
...
.ms-crm-HeaderTileElement.ms-crm-Inline-EditHintState .money .ms-crm-Inline-Value.ms-crm-HeaderTile.ms-crm-Inline-EmptyValue div.ms-crm-Inline-GradientMask {
	background: rgba(243, 241, 241, 0.15); /* for non-css3 browsers */
...
<%-- End styles for InlineEdit controls --%>

<%-- RecordSetControl styles (start) --%>
...
<%-- Start styles to support High contrast mode EmailEngagementActions control --%>
...
<%-- End styles to support High contrast mode EmailEngagementActions control --%>

<%-- Start styles to support High contrast mode EmailRecipientActivity control --%>
...
<%-- End styles to support High contrast mode EmailRecipientActivity control --%>
...
<%--------------------------------------------------------------------------------
	Redline changes for edit form  controls related to recommendation UI
--------------------------------------------------------------------------------%>
...
.ms-crm-HeaderTileElement
{
	...
	padding-right:10px;
	<% if (CrmStyles.IsTextWrapEnabled) { %>
		/* As we incresed the clipboard height by 10px */
		height: 71px;
	<% } else { %>
		height: 42px;
	<% } %>
