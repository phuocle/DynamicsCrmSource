...
<% string pageNumber = string.IsNullOrEmpty(Request.QueryString["PageNumber"]) ? "1" : Request.QueryString["PageNumber"]; %>
createDialogControl = Mscrm.DocumentTemplate.CreateDialogControl.create(document.getElementsByTagName("body")[0], <%=TemplateTypeCode %>, <%=pageNumber%>);
checkDivWarnigCSS();
if($P_CRM('#DlgHdBodyContainer').length >0)
{
$P_CRM('#DlgHdBodyContainer').attr("tabindex","-1");
}
}
...
<a target="_blank" rel="noopener noreferrer" href="http://go.microsoft.com/fwlink/?LinkID=624118" class="helpContainer" title="<loc:Text ResourceId='Button_Label_Help' runat='server'/>">
<%= HelpIconTag %>
</a>
</frm:DialogForm>
