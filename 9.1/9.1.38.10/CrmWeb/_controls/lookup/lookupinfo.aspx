...
<%@ Import Namespace="Microsoft.Crm.Application.Controls" %>

<% if (!ProcessSearchTypes()) { %>

<script language="javascript">
alert("<% = Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncodeNoQuotes(CurrentResourceManager.GetString("Dialog_Lookup_Security_Error")) %>");
closeWindow();
</script>
...
var butBegin, btnAdd, btnNew, tdViewSelector, btnRemoveValue, btnCross, btnClick;
var btnFind, btnBack, tblFind, searchArea, crmDialogFooter, btnSelect, findCriteriaButton, btnCancel, btnAddMultiSelect, multiLookupControlsDiv;

function applychanges()
{
var entityTypeName = getEntityTypeName();
if(!IsNull(entityTypeName) && entityTypeName=="productpricelevel")
{
var btnClickId = !IsNull(butBegin) && butBegin.innerText !== "" ? butBegin.innerText : "Add";
Mscrm.Utilities.gatherAspxButtonClickTelemetryParams(btnClickId, (new Date()).getTime());
}
if (_isMulti)
{
if (isResultsMode())
...
if(!IsNull(crmGrid))
{
entityTypeName = crmGrid.get_entityTypeName();
}
...
var name = !isNullOrEmptyString(XUI.Html.GetText(item)) ? XUI.Html.GetText(item) : item.getAttribute("title");
...
document.body.className = document.body.className.replace('resultsMode' + suffix, '').replace('searchMode' + suffix, '') + ' searchMode' + suffix;
}

function setResultsMode() {
var suffix = _isMulti ? 'MultiSelect' : '';
document.body.className = document.body.className.replace('resultsMode' + suffix, '').replace('searchMode' + suffix, '') + ' resultsMode' + suffix;
}
...
btnCancel = $get('cmdDialogCancel');
btnCross = $get('btnCross');
...
if (!IsNull(oArgs))
{
if ((!IsNull(oArgs.items)) && (oArgs.items.length > 0))
{
btnRemoveValue.disabled = isNullOrEmptyString(oArgs.items[0].innerText);
}
else
{
...
multiLookupControlsDiv.getElementsByTagName('ul')[0].parentNode.style.height = multiLookupControlsDiv.style.height;
}
...
$addHandler(btnFind, 'click', function () {
...
$addHandler(btnCancel, 'click', function () {
cancel();
});


if (findCriteriaButton) {
$addHandler(findCriteriaButton, 'click', function () {

setResultsMode();
});
}

$addHandler(btnAddMultiSelect, 'click', function () {
addItems();
setSearchMode();
});

$addHandler(btnBack, 'click', function () {
setSearchMode();
});

$addHandler(btnSelect, 'click', function () {
butBegin.click();
});

$addHandler(btnMobileRemoveValue, 'click', function () {
...
$addHandler(btnCancel, 'keydown', keepFocusInDialog);
if (!IsNull(btnCross)) {
$addHandler(btnCross, 'keydown', keepFocusInDialogFromCross);
}

Mscrm.Utilities.reportAspxDialogLoadTelemetry();
setDialogControlsPositionAndHeight();
});
...
var allowedSavedQuerylist = ["{F2A910A4-0E0D-43AB-AF62-1399F1A53EDC}", "{125A31CE-0F5E-4957-8AA3-7A10C0713886}", "{B6C5FA2E-84F5-4F20-A239-B310D6AD62F4}", "{0096783C-7AB7-4E7A-A8E3-2ED1A33095C7}"];
if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.WebresourceLocalizationAndDependency))
{
allowedSavedQuerylist.push("{BF5BDAFC-9FD4-4466-87CA-434D6BF5F31F}");
}
...
var dialogHeaderContainerHeight = parseInt($get("DlgHdTitle").offsetHeight) + parseInt($get("DlgHdDesc").offsetHeight);
...
function setDialogControlsPositionAndHeight()
{
var dlgHdContainer = $get("DlgHdContainer");
var searchArea = $get("searchArea");
var gridArea = $get("tblFind");
var gridAreaTable = $get("tblFindGridContainer");
var dlgFooterContainer = $get("crmDialogFooter");
if (!IsNull(dlgHdContainer) && !IsNull(searchArea) && !IsNull(gridArea) && !IsNull(gridAreaTable) && !IsNull(dlgFooterContainer))
{
var searchAreaPadding = 8;
var dlgBodyContainerPaddingfromFooter = 5;
var searchAreaHeight = parseInt(searchArea.offsetHeight);
var searchAreaChildrenHeight = 0;
var gridAreaTableHeight = parseInt(gridAreaTable.offsetHeight);

searchAreaChildrenHeight = getChildsHeight(searchArea);
if (searchAreaHeight < searchAreaChildrenHeight)
{
searchArea.style.height = searchAreaChildrenHeight + "px";
searchAreaChildrenHeight = searchAreaChildrenHeight + searchAreaPadding;
}
else
{
searchAreaChildrenHeight = searchAreaHeight;
}

gridArea.style.top = searchAreaChildrenHeight + "px";

var multiLookupControls = $get("multiLookupControls");
var multiLookupControlsHeight = 0;
if (!IsNull(multiLookupControls))
{
multiLookupControls.style.top = searchAreaChildrenHeight + gridAreaTableHeight + "px";
multiLookupControlsHeight = getChildsHeight(multiLookupControls);
if (multiLookupControlsHeight > 0)
{
multiLookupControls.style.height = multiLookupControlsHeight + "px";
}
}

var dlgHdContainerHeight = parseInt(dlgHdContainer.offsetHeight);
var dlgBodyContainerHeight = searchAreaChildrenHeight + gridAreaTableHeight + multiLookupControlsHeight + dlgBodyContainerPaddingfromFooter;
var dlgFooterContainerHeight = parseInt(dlgFooterContainer.offsetHeight);
var dialogHeightAsPerContent = dlgHdContainerHeight + dlgBodyContainerHeight + dlgFooterContainerHeight;

var inlineDialog = null;
if (!IsNull(window.frameElement) && !IsNull(window.frameElement.parentElement) && window.frameElement.src.indexOf("lookupinfo.aspx") > 0)
{
inlineDialog = window.frameElement.parentElement;
var inlineDialogHeight = parseInt(inlineDialog.offsetHeight);
if (dialogHeightAsPerContent > inlineDialogHeight)
{
window.setTimeout(function ()
{
inlineDialog.style.height = dialogHeightAsPerContent + "px";
}, 100);
}
}
}
...
function getChildsHeight(element)
{
var childsHeight = 0;
...
{
childsHeight = childsHeight + parseInt(element.children[i].offsetHeight);
}
}
return childsHeight;
}
...
if (!IsNull(this.activeElement) && !IsNull(btnCancel) && ((this.activeElement.id != btnCancel.getAttribute("id") || btnRemoveValue.hasAttribute("disabled")))) {
lastFocusedElement = $get(this.activeElement.id);
}
...
if (_isMulti && (lastfocus == "selectedItems") && (this.activeElement != $get("crmGrid_findCriteria")) && (e.keyCode == 8 || e.keyCode == 127)) {
removeItems();
handled = true;
}
}
...
Mscrm.Utilities.setReturnValue(buildReturnValue(crmGrid.get_innerGrid().get_selectedRecords(), true, entityTypeName));
...
function addItems()
{
_selectedItems.AddItems(buildReturnValue($find("crmGrid").get_innerGrid().get_selectedRecords(), false, $find("crmGrid").get_entityTypeName()).items);
setNavigationState();
}
...
if (!IsNull(viewSelector.attributes.getNamedItem("disabled")) && viewSelector.attributes.getNamedItem("disabled").value !="false")
{
...
{
if (XUI.Html.DomUtils.GetFirstChild(tdViewSelector).options[i].parentNode.id == "AppSystemViews")
{
...
{
resultsTitleDiv.innerHTML = String.format(localizedMessageFormat, '<b>' +  CrmEncodeDecode.CrmHtmlEncode(sFindCriteria) + '</b>');
}
}
...
if (keyCode == 9) {
if(!IsNull(btnRemoveValue) && !IsNull(btnCross))
{
e.preventDefault();
(e.shiftKey && !IsNull(lastFocusedElement)) ? lastFocusedElement.focus() : btnRemoveValue.disabled ? btnCross.focus() : btnRemoveValue.focus();
}
if (e.shiftKey && !IsNull(butBegin) && !IsNull(btnNew)) {
e.preventDefault();
butBegin.disabled ? btnNew.focus() : butBegin.focus();
}
}
}

function keepFocusInDialogFromCross(e) {
var keyCode = e.keyCode || e.which;

if (keyCode == 9 && e.shiftKey && !IsNull(btnRemoveValue) && !IsNull(btnCancel)) {
e.preventDefault();
btnRemoveValue.disabled ? btnCancel.focus() : btnRemoveValue.focus();
}
}
</script>
<style type="text/css">
...
{
bottom: <% =LookupGridBottomPosition %>px;
}

#multiLookupControls .ms-crm-MultipleLookup-ButtonSection
{
width: 90px;
}

<% if(Microsoft.Crm.Application.Utility.Util.IsWebClientVisualRefreshFeatureAvailable(null)) { %>
.ms-crm-Dialog-Lookup-SearchArea#searchArea
{
top: 0px !important;
}
.ms-crm-ListControl#crmGrid
{
...
#tblFindGridContainer.tblFindGridContainer
{
min-height:220px;
}

div#DlgHdBodyContainer
{
height:auto;
}

td.AppQuickFind_Render_td A.ms-crm-FindButton
{
width: 22px;
height: 30px;
margin-bottom:0px;
...
.ms-crm-FindButton span img.ms-crm-ImageStrip-search,
.ms-crm-FindButton span img.ms-crm-ImageStrip-searchhover
{
border :1px solid #C6C6C6;
}

table tr td a.ms-crm-FindButton span img.ms-crm-ImageStrip-searchclear,
table tr td a.ms-crm-FindButton span img.ms-crm-ImageStrip-searchclearhover
{
border :1px solid #C6C6C6;
}
...
<tr>
<td class='searchMode searchModeMultiSelect'><button id='btnFind'><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.Find")) %></button></td>
<td class='resultsMode resultsModeMultiSelect'><button id='btnBack'><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.Back")) %></button></td>
</tr>
<tr>
<td class='searchMode'><button id='btnCancel' ><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.Cancel")) %></button></td>
<td class='resultsMode searchModeMultiSelect'><button id='btnSelect'><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.Done")) %></button></td>
<td class='resultsModeMultiSelect'><button id='btnAddMultiSelect'><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.Add")) %></button></td>
</tr>
</tr>
<tr>
<td class='searchMode'><button id='btnMobileRemoveValue' ><%= CrmEncodeDecode.CrmHtmlEncode(AppResourceManager.Default.GetString("MobileRefresh.LookupControl.Buttons.RemoveValue")) %></button></td>
</tr>
</table>
</div>
</div>
