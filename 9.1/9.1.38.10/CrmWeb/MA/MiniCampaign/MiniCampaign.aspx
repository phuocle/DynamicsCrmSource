...
{
if (e.className.indexOf(" ms-crm-QCActivityHover") < 0)
e.className += " ms-crm-QCActivityHover";
}
}
...
if(_selectedItem && e != _selectedItem)
{
e.className = e.className.replace(" ms-crm-QCActivityHover", "");
}
}
...
e.className = e.className.replace(" ms-crm-QCActivityHover", "");
if (e.className.indexOf(" ms-crm-QCActivitySelect") < 0)
e.className += " ms-crm-QCActivitySelect";

if( _selectedItem )
{
_selectedItem.className = _selectedItem.className.replace(" ms-crm-QCActivitySelect", "");
}
_selectedItem = e;
...
if (isOutlookHostedWindow() && getOutlookHostedWindow().RequiresWrappedArguments)
{
oEvent.rawEvent.returnValue = answer;
}
} catch (e) { }
...
if(mc_ActivityXml_str.indexOf("Appointment") == -1)
{
var parser = new DOMParser();
xmlDoc = parser.parseFromString(mc_ActivityXml_str,"text/xml");
statecodeNode= xmlDoc.getElementsByTagName("statecode")[0];
if (statecodeNode != undefined)
{
statecodeNode.parentNode.removeChild(statecodeNode);
}
statuscodeNode= xmlDoc.getElementsByTagName("statuscode")[0];
if (statuscodeNode != undefined)
{
statuscodeNode.parentNode.removeChild(statuscodeNode);
}
mc_ActivityXml_str = (new XMLSerializer()).serializeToString(xmlDoc);
}
}
...
{
if(_selectedText == <% =Util.Fax %> || _selectedText == <% =Util.Letter %> || _selectedText == <% =Util.PhoneCall %>)
{
...
if(_selectedText != <% =Util.Email %> && _activityFormFrame.Mscrm)
{
...
|| _selectedText == <% =Util.Letter %>
|| _selectedText == <% =Util.Fax %>) && _activityFormFrame.Mscrm)
{
tryClearFormField('from');
tryClearFormField('to');
trySetDirectionCode(true);
}
...
{
field = _activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior(fieldName);
if (!IsNull(field))
{
...
function trySetDirectionCode(fieldBoolValue)
{
field = _activityFormFrame.Mscrm.FormControlInputBehavior.GetBehavior('directioncode');
if (!IsNull(field))
{
field.set_dataValue(fieldBoolValue);
}
}
...
activityIframe.style.top = "41px";
containerIframe.style.top = "41px";
...
{
if (event.target.tagName == "UL" && isActivityFocus == true) {
try {
...
class="ms-crm-Dialog-List" style="border: 1px solid #CCCCCC; -webkit-margin-after: 1em;" tabindex="3">
<%
...
<div id="mcHeader" style="height:40x;">
...
<div id="iframeContainer" style="width:100%; top:41px; bottom:0px; position:absolute;">
<!--[if IE 7]>
<div class="ms-crm-IE7-Height-Fix-Dummy-Container">
<![endif]-->
