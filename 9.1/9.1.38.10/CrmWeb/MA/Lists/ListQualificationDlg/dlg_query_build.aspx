...
var rootEntity = fetchXmlDom.getElementsByTagName("entity")[0];
var childNodes = rootEntity.childNodes;
var attributes = [];
var linkEntities = [];
var attributeTagName = 'attribute';
var linkEntityTagName = 'link-entity';
var elementNodeTypeNumber = 1;


for (var i = 0; i < childNodes.length; i++) {
var child = childNodes[i];

if (child.nodeName === attributeTagName && child.nodeType === elementNodeTypeNumber) {
attributes.push(child.getAttribute("name"));
} else if (child.nodeName === linkEntityTagName && child.nodeType === elementNodeTypeNumber) {
linkEntities.push(child);
}
}


var linkEntityAliasName = "alias";
for (var i = 0; i < linkEntities.length; i++) {
var linkEntity = linkEntities[i];
var linkEntityAlias = linkEntity.getAttribute(linkEntityAliasName);

var linkEntityChildNodes = linkEntity.childNodes;
for (var j = 0; j < linkEntityChildNodes.length; j++) {
var linkEntityChildNode = linkEntityChildNodes[j];

if (linkEntityChildNode.nodeName === attributeTagName && linkEntityChildNode.nodeType === elementNodeTypeNumber) {

attributes.push(linkEntityAlias + "." + linkEntityChildNode.getAttribute("name"));
} else if (linkEntityChildNode.nodeName === linkEntityTagName && linkEntityChildNode.nodeType === elementNodeTypeNumber) {

linkEntities.push(linkEntityChildNode);
}
}
}
...
Marketing.List.Actions.processLQAdd(lqAdvFind.get_fetchXml(), oParams, a, iOpt);
...
Marketing.List.Actions.processLQRemove(lqAdvFind.get_fetchXml(), oParams, a, iOpt);
...
Marketing.List.Actions.processLQKeep(lqAdvFind.get_fetchXml(), oParams, a, iOpt);
...
Marketing.List.Actions.processLQUseQuery(XUI.Xml.XMLSerializer.serializeToString(fetchXmlDom), oParams);
break;
