...
_fieldsAlreadyInUseList = FieldsAlreadyInUseList;
}
...
var exclusionListForEntity = getExclusionListForEntity(sEntityAlias, columnComponent);
for (var i = 0; i < exclusionListForEntity.length; i++)
{
columnComponent.RemoveOption(exclusionListForEntity[i]);
}
...
function getExclusionListForEntity(sEntityAlias, columnComponent)
{
var exclusionList = [];


for (sAttributeName in _fieldsAlreadyInUseList[sEntityAlias]) {
if (!IsNull(_fieldsAlreadyInUseList[sEntityAlias][sAttributeName])) {
exclusionList.push(sAttributeName);
}
}



var attributeTypesToBeExcluded = ["multiselectpicklist"];


var colOptionsXml = columnComponent.get_element();
if (!IsNull(colOptionsXml)) {
var colNodes = colOptionsXml.options;
if (!IsNull(colNodes)) {
for (var i = 0; i < colNodes.length; i++) {
if (attributeTypesToBeExcluded.indexOf(colNodes[i].getAttribute("datatype").toLowerCase()) != -1) {
exclusionList.push(colNodes[i].getAttribute("value"));
}
}
}
}
