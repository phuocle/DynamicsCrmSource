...
function handleView(o,oGrid,promptIfChartDirty)
{
    ...
    if(o != null)
    {
        ...
        sViewId = selectBehavior.get_dataValue() == LOCID_SEARCH_RESULTS ? SavedQuerySelector.quickFindQuery : selectBehavior.get_dataValue();
        if(!IsNull(o.options[o.selectedIndex]) && !IsNull(o.options[o.selectedIndex].parentNode) && o.options[o.selectedIndex].parentNode.id == "AppUserViews")
            sviewType = 4230;
        else
    ...
    if(IsNull(promptIfChartDirty) || promptIfChartDirty === "" || promptIfChartDirty === true)
    {
        ...
        if(!IsNull(visualizationPaneControl) && !visualizationPaneControl.confirmViewChange(sViewId))
        {
            ...
            newVSControl != null && newVSControl.showNewVSControl && 
    ...
    var gridtype = oGrid.getGridType(),
        entitytype = oGrid.getEntityName();
    gridtype == 3 && (entitytype === "quotedetail" || entitytype === "salesorderdetail" || entitytype === "invoicedetail" || entitytype === "opportunityproduct") && 
        oGrid.SetParameter("isTurboForm",window.IsTurboForm ? 1 : 0);
    var entityName = "";
    if(newVSControl != null)
        entityName = newVSControl.viewEntityName;
    if(entityName == "sharepointdocument")
        oGrid.SetParameter("relativeLocation","");
    else
        if(entityName == "kbarticle")
            typeof Mscrm.ArticleSearchRibbon != "undefined" && 
                Mscrm.ArticleSearchRibbon.suppressSearchByType();
        else
            if(entityName == "systemform")
                typeof Mscrm.UIElementsGrid != "undefined" && typeof Mscrm.UIElementsGrid.handleSystemFormMenuButtons != "undefined" && 
    ...
    o != null && o.hasAttribute("returnedtypecode") && o.getAttribute("returnedtypecode") == "9997" && 
    ...
    function _handleComplete(oResult)
    {
        ...
        else
        {
            ...
            if(!IsNull(newVSControl) && newVSControl.isActivitiesViewSelector)
            {
                ...
                updateStickyViewUrl(oGrid.GetParameter("viewtype"),oGrid.GetParameter("viewid"),IsNull($get("crmTypeSelector")) ? null : crmTypeSelector.value)
        }
    ...
    if(crmSelectElement != null)
    {
        for(var i = 0; i < crmSelectElement.options.length; i++)
            with(crmSelectElement[i])
                !isNullOrEmptyString(attributes["isdefault"]) && Mscrm.Utilities.parseBoolean(attributes["isdefault"].value) && 
...
function clearFind(oGrid)
{
    ...
    if(!IsNull(oControl))
        ...
        {
            oControl.value = "All";
            oGrid.SetParameter("scheduledend",Mscrm.FormControlInputBehavior.GetBehavior("crmDateSelector").get_dataValue())
        }
...
function quickFind(oGrid)
{
    try
    {
        ...
        if(SavedQuerySelector != null)
        {
            ...
            oGrid.set_pageNumber(1);
        var oJumpBar = $find(oGrid.get_id() + "_JumpBar");
        typeof oJumpBar != "undefined" && !IsNull(oJumpBar) && 
            oJumpBar.Reset();
        SetSetDefaultViewButtonState()
    }
    catch(e)
...
function updateViewSelectorForKnowledgeArticlesSearch(gridControl)
{
    var newViewSelector = $find(gridControl.get_id() + "_SavedNewQuerySelector");
    if(newViewSelector != null)
    {
        var quickFindQueryId = newViewSelector.quickFindQuery;
        newViewSelector.setViewForNewSavedQuerySelector(LOCID_SEARCH_RESULTS,quickFindQueryId,SavedQuery)
    }
}
...
function openAdvFind(gridId,bOpenInNewMode)
{
    ...
    var appUrl = Mscrm.CrmUri.create(win.location.href);
    oMainUri.get_query()["appid"] = "appid"  in  appUrl.get_query() ? appUrl.get_query()["appid"] : null;
