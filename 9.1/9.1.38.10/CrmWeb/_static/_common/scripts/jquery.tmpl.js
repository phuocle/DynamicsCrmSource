...
(function(jQuery,undefined)
{
    ...
    {
        var newItem = {data:data || (parentItem ? parentItem.data : {}),tmpl:null,parent:parentItem || null,nodes:[],nest:nest};
        if(options)
        {
    ...
    {
        var frag,
            ret = jQuery.map(content,function(item)
            {
                return typeof item === "string" ? item.replace(/(<\w+)([^>]*)/g,"$1 " + tmplItmAtt + '="' + tmplItem.key + '" $2') : build(item,tmplItem,item.content)
            });
        if(parent)
            return ret;
        ret = ret.join("");
    ...
    {
        return new Function("jQuery","$item","var $=jQuery,_=[],$data=$item.data;with($data){_.push('" + $.trim(markup).replace(/([\\'])/g,"\\$1").replace(/[\r\t\n]/g," ").replace(/\${([^}]*)}/g,"{{= $1}}").replace(/{{(\/?)(\w+|.)(?:\(((?:.(?!}}))*?)?\))?(?:\s+(.*?)?)?(\((.*?)\))?\s*}}/g,function(all,slash,type,fnargs,target,parens,args)
        {
            ...
            return "');" + cmd[slash ? "suffix" : "prefix"].split("$notnull_1").join("typeof(" + target + ")!=='undefined' && (" + target + ")!=null").split("$1a").join(exprAutoFnDetect).split("$1").join(expr).split("$2").join(fnargs ? fnargs.replace(/\s*([^\(]+)\s*(\((.*?)\))?/g,function(all,name,parens,params)
            {
