...
(function()
{
    var e = {version:"0.7.1.0",isUndefined:function(a)
        {
            return a === undefined
        },isNull:function(a)
        {
    ...
    if(document.addEventListener)
        document.readyState == "complete" ? b() : document.addEventListener("DOMContentLoaded",b,false);
    else
...
if(!Array.prototype.every)
    Array.prototype.every = function(b,c)
    {
        for(var d = this.length,
            a = 0; a < d; a++)
            if(a  in  this && !b.call(c,this[a],a,this))
                return false;
        return true
    };
Array.prototype.extract = function(a,b)
{
...
if(!Array.prototype.forEach)
    Array.prototype.forEach = function(b,c)
    {
        for(var d = this.length,
            a = 0; a < d; a++)
            a  in  this && b.call(c,this[a],a,this)
    };
Array.prototype.getEnumerator = function()
{
    return new ss.ArrayEnumerator(this)
};
Array.prototype.groupBy = function(f,g)
{
    for(var h = this.length,
        d = [],
        e = {},
        b = 0; b < h; b++)
        if(b  in  this)
...
Array.prototype.index = function(d,e)
{
    for(var f = this.length,
        b = {},
        a = 0; a < f; a++)
...
Array.prototype.insertRange = function(c,b)
{
    if(c === 0)
        this.unshift.apply(this,b);
    else
        for(var a = 0; a < b.length; a++)
            this.splice(c + a,0,b[a])
};
...
Date.prototype.localFormat = function(a)
{
...
Date.prototype._netFormat = function(d,j)
{
    var c = j ? ss.CultureInfo.CurrentCulture.dateFormat : ss.CultureInfo.InvariantCulture.dateFormat,
        f = false;
    if(d.length == 1)
        switch(d)
        {
    ...
    var h = Date._formatRE,
        i = new ss.StringBuilder,
        b = this;
    if(f)
        b = new Date(Date.UTC(b.getUTCFullYear(),b.getUTCMonth(),b.getUTCDate(),b.getUTCHours(),b.getUTCMinutes(),b.getUTCSeconds(),b.getUTCMilliseconds()));
    h.lastIndex = 0;
    while(true)
    {
        var k = h.lastIndex,
            g = h.exec(d);
        i.append(d.slice(k,g ? g.index : d.length));
        if(!g)
            break;
        var e = g[0],
            a = e;
...
__Namespace.prototype = {__namespace:true,getName:function()
{
...
ss.Enum._enumToString = function(d)
{
    ...
    var b = function()
        {
            ...
            else
            {
                for(var c = a.clone(),
                    b = 0; b < c.length; b += 2)
                    ss.Delegate._contains(a,c[b],c[b + 1]) && c[b + 1].apply(c[b],arguments);
                return null
            }
        };
    b._targets = a;
...
{
    if(!a)
        return !b._targets ? ss.Delegate.create(null,b) : b;
    if(!b)
        return !a._targets ? ss.Delegate.create(null,a) : a;
...
{
    this._parts = !ss.isNullOrUndefined(a) ? [a] : [];
    this.isEmpty = this._parts.length == 0
};
ss.StringBuilder.prototype = {append:function(a)
{
    if(!ss.isNullOrUndefined(a))
    {
        this._parts.add(a);
...
if(!window.XMLHttpRequest)
    ...
    {
        for(var b = ["Msxml2.XMLHTTP","Microsoft.XMLHTTP"],
            a = 0; a < b.length; a++)
            try
            {
...
ss.Observable.prototype = {getValue:function()
{
...
ss.Observable._observerRegistration = {dispose:function()
{
...
ss.ObservableCollection.prototype = {get_item:function(a)
{
    this._observers = ss.Observable._captureObservers(this._observers);
