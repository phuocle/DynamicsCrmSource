...
function GenerateForEditForm()
{
    ...
    if(BeforeWindowOpenTimestamp && mainWnd.FinishInlineEditInitializerTimestamp)
    {
        ...
        if(typeof mainWnd.FinishMobileFirstControlTimestamp != "undefined")
        {
            ...
            addToPerfContent("&nbsp;&nbsp;&nbsp; Time to load first control (First control edit):\t" + FirstControlLoadTime + " ms<br/>")
        }
...
{
    event.keyCode == 81 && event.ctrlKey && event.shiftKey && 
        OpenPerformanceUI()
}
...
function ShowPerfReport(show)
{
    ...
    if(show)
    {
        ...
        perfUpperActions.innerHTML = "<font color='blue'><a href='#' onclick='window.RemovePerfReportDiv()'><u>close</u></a></font>";
