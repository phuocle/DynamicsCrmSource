...
function performActionAfterRecurringWorkShiftChange(resultValue,frmGrid,clickDate,rawUrl,iX,iY)
{
    if(resultValue)
    {
        ...
        if(selectedValue == 0)
        {
            var dialogUrl = Mscrm.CrmUri.create("/SM/workplans/Dialogs/timesheet.aspx?id=&calendarId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.calendarId.value) + "&resourceId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oId.value) + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&selecteddates=" + CrmEncodeDecode.CrmUrlEncode(clickDate)),
                callbackFunc = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performRefreshCalendarAndGridAction,[this]);
...
function openCrmObj(sUrl,sName,oSpan)
{
    ...
    switch(Number(oSpan.getAttribute("itemType")))
    {
        case ServiceRestrictionCalendarRule:
            openStdDlg(Mscrm.CrmUri.create(url + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&calendarId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.calendarId.value)),top.window,iX,iY);
            break;
        case TimeOffCalendarRule:
            oUrl = Mscrm.CrmUri.create(url + "&calendarId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.calendarId.value) + "&resourceId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oId.value) + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&name=Time off&mode=Edit");
            inlineDialogHelper(oUrl,iX,iY,window.self,null,null);
            break;
        case OccurrenceCalendarRule:
        case RecurrenceCalendarRule:
            openStdDlg(Mscrm.CrmUri.create(url + "&calendarId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.calendarId.value) + "&innerCalendarId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oId.value) + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&name=Working Hours&mode=Edit"),top.window,iX,iY);
            break;
        case HolidayCalendarRule:
            callbackFunction = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performRefreshCalendarAndGridAction,[this]);
        ...
        case OccurringWorkShift:
            callbackFunction = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performRefreshCalendarAndGridAction,[this]);
            oUrl = Mscrm.CrmUri.create(url + "&calendarId=" + CrmEncodeDecode.CrmUrlEncode(oSpan.getAttribute("itemId")) + "&resourceId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oId.value) + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&selecteddates=" + CrmEncodeDecode.CrmUrlEncode(clickDate) + "&mode=Edit");
        ...
        case RecurringWorkShift:
            ...
            callbackFunction = Mscrm.Utilities.createCallbackFunctionForXrmDialog(performActionAfterRecurringWorkShiftChange,parameters);
            oUrl = Mscrm.CrmUri.create("/SM/workplans/Dialogs/changeschedule.aspx?id=" + CrmEncodeDecode.CrmUrlEncode(oSpan.getAttribute("subId")) + "&calendarid=" + CrmEncodeDecode.CrmUrlEncode(oSpan.getAttribute("itemId")) + "&resourceId=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oId.value) + "&oType=" + CrmEncodeDecode.CrmUrlEncode(frmGrid.oType.value) + "&date=" + CrmEncodeDecode.CrmUrlEncode(clickDate));
            inlineDialogHelper(oUrl,iX,iY,null,null,callbackFunction);
            break;
        case Appointment:
...
function Delete(sCalendar,sNotSelectedError,iCalendarType)
{
    ...
    var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_delete.aspx?iObjType=" + iObjType + "&iTotal=" + itemIds.length + sCustParams),itemIds,450,250,null);
