...
function parseTime(sTime,iFormat)
{
    ...
    if(-1 < sTime.indexOf(sPM.toLowerCase()) && sPM.length >= 1)
    {
        ...
        if(-1 < sTime.indexOf(sAM.toLowerCase()) && sAM.length >= 1)
        {
...
function timeToUTCString(oTime)
{
    if(isNaN(oTime))
        return null;
    var sRVal = makeTwoDigitString(oTime.getHours()) + ":" + makeTwoDigitString(oTime.getMinutes()) + ":" + makeTwoDigitString(oTime.getSeconds());
    return sRVal
}
