...
Mscrm.DocumentTemplate.CreateDialogControl = function($p0, $p1, $p2) {
    ...
    this.$I_0 = this.$0_0.find('#updateTemplateDialogLink');
    this.$7_0 = this.$0_0.find('#wordTemplate');
    this.$D_0 = this.$0_0.find('#createTemplateLabelQuery');
    this.$4_0 = this.$0_0.find('#excelTemplate');
    this.$2_0 = this.$0_0.find('#buttonClose');
    this.templateTypeCode = $p1;
    this.$S_0 = $p2;
    this.$F_0 = this.$0_0.find('#editColumnsLink');
    this.$Q_0();
    this.$R_0();
}
Mscrm.DocumentTemplate.CreateDialogControl.create = function(element, templateTypeCode, pageNumber) {
    return new Mscrm.DocumentTemplate.CreateDialogControl($P_CRM(element), templateTypeCode, pageNumber);
}
Mscrm.DocumentTemplate.CreateDialogControl.prototype = {
    ...
    $Q_0: function() {
        ...
        this.$F_0.on('click', this.$$d_$13_0);
    ...
    $Z_0: function($p0) {
        ...
        $v_1.setParameter('pageNumber', this.$S_0);
    ...
    $b_0: function($p0) {
        if (this.$4_0.hasClass('selected')) {
            ...
            this.$g_0($v_1.ViewId, $v_1.ViewTypeCode, $v_1.PageRecordCount, $v_1.PageNumber, $v_1.FetchXml);
        }
        else {
    ...
    $g_0: function($p0, $p1, $p2, $p3, $p4) {
        ...
        if (!$p3 || $p3 === '0') {
            ...
            $v_4.attr('value', '<grid><pageNum>' + $p3 + '</pageNum><recsPerPage>50</recsPerPage></grid>');
        }
        $v_0.append($v_4);
        var $v_5 = $P_CRM('<input></input>');
        $v_5.attr('type', 'hidden');
        $v_5.attr('name', 'fetchXml');
        $p4 = (this.$8_0) ? this.$8_0 : $p4;
        $v_5.attr('value', $p4);
        $v_0.append($v_5);
        ...
        $v_0.append($v_6);
        var $v_7 = $P_CRM('<input></input>');
        $v_7.attr('type', 'number');
        $v_7.attr('name', 'page');
        $v_7.attr('value', $p3);
        var $v_8 = $P_CRM('<input></input>');
        $v_7.attr('type', 'number');
        $v_7.attr('name', 'count');
        $v_7.attr('value', $p2);
        document.cookie = 'excelDownloadToken=1; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/';
        var $v_9 = new XMLHttpRequest();
        $v_9.open('GET', Mscrm.CrmUri.create('/userdefined/checkExcelCookie.aspx').toString());
        var $$t_F = this;
        $v_9.onreadystatechange = function() {
            if ($v_9.readyState === 4) {
                if ($v_9.status === 200) {
                    $$t_F.$0_0.append($v_0);
                    $v_0.submit();
    ...
    $M_0: function() {
        ...
        $v_0.open('GET', Mscrm.CrmUri.create('/userdefined/checkExcelCookie.aspx').toString());
        var $$t_1 = this;
        $v_0.onreadystatechange = function() {
            if ($v_0.readyState === 4) {
                if ($v_0.status === 200) {
                    if ($v_0.responseText === '-1') {
                        $$t_1.$0_0.find('#exportForm').remove();
                        if (Mscrm.SessionInfo.isOutlookClient()) {
    ...
    dispose: function() {
        ...
        this.$I_0.off('click', this.$$d_$x_0);
        ...
        this.$F_0.off('click', this.$$d_$13_0);
    ...
    $10_0: function($p0) {
        ...
        this.$D_0.addClass('hideDropDown');
    ...
    $w_0: function($p0) {
        ...
        this.$D_0.removeClass('hideDropDown');
...
Mscrm.DocumentTemplate.SelectDialogControl.prototype = {
    ...
    $b_0: function($p0) {
        this.entityTypeCode = Number.parseInvariant(this.$3_0.children(':selected').val());
        var $v_0 = this.getRelationshipData();
        this.$h_0(this.entityTypeCode.toString(), $v_0);
    },
    ...
    $h_0: function($p0, $p1) {
        ...
        $v_4.open('GET', Mscrm.CrmUri.create('/userdefined/checkExcelCookie.aspx').toString());
        var $$t_7 = this;
    ...
    $M_0: function() {
        ...
        $v_0.onreadystatechange = function() {
            if ($v_0.readyState === 4) {
                if ($v_0.status === 200) {
                    if ($v_0.responseText === '-1' || $v_0.responseText === '-2') {
                        $$t_1.$0_0.find('#exportForm').remove();
                        if (Mscrm.SessionInfo.isOutlookClient()) {
                            $P_CRM('#loadingContainer').removeClass('indicatorOn');
                            $P_CRM('#downloadedTitle').css('display', 'block');
...
Mscrm.DocumentTemplate.UploadDialogControl = function($p0, $p1, $p2) {
    ...
    this.$0_0 = $p0;
    this.$N_0 = this.$0_0.find('#upload_link');
    this.$H_0 = this.$0_0.find('#fileUpload');
    this.$A_0 = this.$0_0.find('#drop_zone');
    this.$E_0 = this.$0_0.find('#displayFileName');
    this.$1_0 = this.$0_0.find('#editTemplateInformation');
    this.$6_0 = this.$0_0.find('.errorDescription');
    this.$L_0 = this.$0_0.find('.specificError');
    this.$2_0 = this.$0_0.find('#buttonClose');
    this.$C_0 = this.$0_0.find('#btnCancel');
    this.$T_0 = $p1;
    this.$1_0.addClass('hideUploadButton');
    this.$K_0 = (IsNull($p2)) ? 32 : $p2;
    this.$e_0();
    this.$Q_0();
    this.$R_0();
}
...
Mscrm.DocumentTemplate.UploadDialogControl.$B = function($p0) {
    if (IsNull($p0)) {
        return 'null';
    }
...
Mscrm.DocumentTemplate.UploadDialogControl.prototype = {
    ...
    $Q_0: function() {
        this.$N_0.on('click', this.$$d_$j_0);
        this.$H_0.on('change', this.$$d_$s_0);
        this.$A_0.on('dragover', this.$$d_$p_0);
        this.$A_0.on('dragenter', this.$$d_$l_0);
        this.$A_0.on('dragleave', this.$$d_$n_0);
        this.$A_0.on('drop', this.$$d_$q_0);
        this.$1_0.on('click', this.$$d_$u_0);
        this.$2_0.on('keydown', this.$$d_focusOnTab);
        this.$C_0.on('keydown', this.$$d_focusOnTab);
    },
    ...
    $c_0: function($p0) {
        ...
        this.$G_0 = $p0[0];
        this.$E_0.text($p0[0].name);
        this.$1_0.removeClass('hideUploadButton');
    },
    
    $16_0: function($p0) {
        var $v_0 = this.$K_0 * 1024 * 1024;
        if ($v_0 < $p0[0].size) {
            alert(String.format(Xrm.Internal.getResourceString('ImportWizard.UploadFilePage.UploadIFrame.Error.FileSize'), this.$K_0));
            return false;
        }
        return true;
    },
    ...
    $m_0: function($p0) {
        ...
        this.$J_0++;
        Xrm.Internal.trace.logT(Mscrm.DocumentTemplate.UploadDialogControl, 'Enter:\ntarget: {0}\ncurr: {1}\nrel: {2}', Mscrm.DocumentTemplate.UploadDialogControl.$B($p0.target), Mscrm.DocumentTemplate.UploadDialogControl.$B($p0.currentTarget), Mscrm.DocumentTemplate.UploadDialogControl.$B($p0.relatedTarget));
        $p0.stopPropagation();
        $p0.preventDefault();
    ...
    $i_0: function($p0, $p1, $p2) {
        ...
        if ($p0.status === 500 && !isNullOrEmptyString($p0.responseText)) {
            ...
            if ($v_0['Code'] === '0x80040265' && $v_0['Description']) {
                this.$L_0.text($v_0['Description']);
            }
            else {
                this.$L_0.text($v_0['DisplayText']);
            }
