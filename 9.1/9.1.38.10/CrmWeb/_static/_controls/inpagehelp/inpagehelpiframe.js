...
function ExecuteAction(actionName,etc,event)
{
    ...
    switch(actionName)
    {
        case "importdata":
            ...
            {
                var windowInf = parentFrame.Mscrm.WindowInformation.getWindowInformation(parentFrame.Mscrm.EntityTypeCode.ImportWizardDialog),
                    windowUrl = parentFrame.Mscrm.CrmUri.updateCrmUriHostAndScheme(parentFrame.Mscrm.CrmUri.create("/WebWizard/WizardContainer.aspx?WizardId=A0E9DC09-8730-4C97-A8B9-6F585C0C16F7")),
                    dialogOptions = new Xrm.DialogOptions;
                dialogOptions.height = windowInf.Height;
                dialogOptions.width = windowInf.Width;
                Xrm.Internal.openDialog(windowUrl.toString(),dialogOptions,null,null,function(result)
                {
                    result && 
        ...
        case "customize":
            if(parentFrame.IsOnline())
                if(UserAndOrgLanguagesMatch(parentFrame))
                {
                    ...
                    var windowInfo = parentFrame.Mscrm.WindowInformation.getWindowInformation(parentFrame.Mscrm.EntityTypeCode.Entity),
        ...
        case "personalsettings":
            ...
            var parameters = new Array(e),
                ...
                windowUrl = parentFrame.Mscrm.CrmUri.updateCrmUriHostAndScheme(parentFrame.Mscrm.CrmUri.create("/tools/personalsettings/dialogs/personalsettings.aspx"));
        ...
        case "systemsettings":
            if(parentFrame.IsOnline())
            {
                ...
                var parameters = new Array(e),
                    callbackFunction = Mscrm.Utilities.createCallbackFunctionForXrmDialog(executeActionCallback,parameters),
                    windowUrl = parentFrame.Mscrm.CrmUri.updateCrmUriHostAndScheme(parentFrame.Mscrm.CrmUri.create("/tools/systemsettings/dialogs/systemsettings.aspx"));
                Xrm.Internal.openDialog(windowUrl.toString(),dialogOptions,null,null,callbackFunction)
            }
            else
                alert(parentFrame.LOCID_NOT_AVAILABLE_OFFLINE);
        ...
        case "addusers":
            if(parentFrame.IsOnline())
            {
                var windowUrl = parentFrame.Mscrm.CrmUri.updateCrmUriHostAndScheme(parentFrame.Mscrm.CrmUri.create("/WebWizard/WizardContainer.aspx?WizardId=2631659F-A668-4A48-833B-D20E187B5A89")),
                    dialogOptions = new Xrm.DialogOptions;
        ...
        case "setupduplicatedetection":
            if(parentFrame.IsOnline())
            {
                var oUrl = parentFrame.Mscrm.CrmUri.create("/tools/duplicatedetection/duplicatedetectionsettings/duplicatedetectionsettings.aspx"),
                    windowUrl = parentFrame.Mscrm.CrmUri.updateCrmUriHostAndScheme(oUrl);
