...
function LookupObjects(lookupField,lookupStyle,lookupTypes,lookupBrowse,bindingColumns,additionalParams,showNew,showProp,bPopulateLookup,defaultType,searchString,dataProviderOverride,defaultViewId,customViews,filterRelationshipId,rId,rType,rDependAttr,allowFilterOff,disableQuickFind,disableViewPicker,viewsIds,crmAttributeId,callbackReference,additionalFetchFilter,additionalFetchFilterTypes)
{
    ...
    var bIsSubjectLookup = lookupStyle.toUpperCase() === "SUBJECT",
        dUrl = bIsSubjectLookup ? "/_controls/lookup/lookupsubject.aspx" : "/_controls/lookup/lookupinfo.aspx",
        oUrl = Mscrm.CrmUri.create(dUrl);
    if(!IsNull(lookupField))
    {
        if(Mscrm.Utilities.isIE())
            var appId = GetParam(window.location.href,"appid");
        else
            var appId = GetParam(lookupField.baseURI,"appid");
        if(!IsNull(appId))
            oUrl.get_query()["appid"] = appId
    }
    ...
    if(!bIsSubjectLookup && args && args.items)
        try
        {
            Mscrm.Utilities.gatherAspxDialogTelemetryParams(dUrl,Mscrm.Utilities.safeJsonStringify(oFeatures),Mscrm.Utilities.safeJsonStringify(args.items.length))
        }
        catch(e)
        {
            Mscrm.Utilities.reportScriptErrorInTelemetryCode("LookupInfo.LookupObjects OpenDialog",e,true)
        }
...
function getLookupControlFromItemSpan(oItemSpan)
{
    for(var lookupTable = oItemSpan.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode,
        ...
        if(tableCells[i].className == "Lookup_RenderButton_td" || tableCells[i].className == "Lookup_RenderButton_td keyboardFocusClass")
