...
(function (Activities) {
    var Task = (function () {
        ...
        Task.Form_onload = function (executionContext) {
            var telemetryItem = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.Task, Activities.Constants.TelemetryConstant.EventTaskOnLoad);
            ...
            // TODO: Evaluate the usage of Refresh and
            // Added Service Desk check in order to short Circuit the check for refresh form which is not valid for
            // Service Desk/MoCA. This was causing onload issues with Email form on service desk for IE. 
            // Adding check for Mobile as in case of windows 10 MOCA dialog script frees the Utilities script and throws an error
            //if (Xrm.Internal.IsEnabledForInteractionCentric() || xrmPage.Context.Client.GetClient() == Xrm.ClientName.Mobile || Utilities.IsRefreshForm())
            // The client context is removed in new API
            new Activities.ActivityPageHandler().setDefaultValues(xrmPage, telemetryItem);
            //Xrm.Internal.isEnabledForInteractionCentric()
            if (Activities.Common.Util.IsNewEntityForm(xrmPage)) {
                if (xrmPage.ui.getFormType() === 1 /* Create */) {
                    var durationCtrl = xrmPage.getAttribute("actualdurationminutes");
                    if (!Activities.Common.Util.IsNull(durationCtrl)) {
                        var durationValue = durationCtrl.getValue();
                        if (Activities.Common.Util.IsNull(durationValue) || Activities.Common.Util.IsNullOrWhiteSpace(durationValue.toString())) {
