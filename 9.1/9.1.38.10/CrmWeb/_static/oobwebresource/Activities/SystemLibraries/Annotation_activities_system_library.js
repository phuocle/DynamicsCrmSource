...
(function (Activities) {
    var Annotation = (function () {
        ...
        Annotation.Form_onload = function (executionContext) {
            if (Xrm.Internal.isUci()) {
                var formContext = executionContext && executionContext.getFormContext();
                var dummyFileAttr = formContext.data.entity.attributes.get("dummyfilename");
                var dummyRegardingAttr = formContext.data.entity.attributes.get("dummyregarding");
                if (!Activities.Common.Util.IsNull(dummyFileAttr))
                    dummyFileAttr.setSubmitMode(Activities.Constants.AttributeSubmitModes[Activities.Constants.AttributeSubmitModes.never]);
                if (!Activities.Common.Util.IsNull(dummyRegardingAttr))
                    dummyRegardingAttr.setSubmitMode(Activities.Constants.AttributeSubmitModes[Activities.Constants.AttributeSubmitModes.never]);
            }
        };
        return Annotation;
    }());
    Activities.Annotation = Annotation;
})(Activities || (Activities = {}));
