...
(function (Activities) {
    var Fax = (function () {
        ...
        Fax.Form_onload = function (executionContext) {
            var telemetryItem = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.Fax, Activities.Constants.TelemetryConstant.EventFaxOnLoad);
            var faxHandler = new Activities.FaxPageHandler();
            Activities.Common.Util.isExecutionContextMissingAndReport(executionContext, telemetryItem);
            faxHandler.setDefaultValues(executionContext.getFormContext(), telemetryItem);
            telemetryItem.report();
        };
        Fax.directionCodeOnChange = function (executionContext) {
            var telemetryItem = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.Fax, Activities.Constants.TelemetryConstant.EventDirectionCodeOnChange);
            Activities.Common.Util.isExecutionContextMissingAndReport(executionContext, telemetryItem);
            ...
            Fax.swapLookups(executionContext, oLookUpA, oLookUpB, telemetryItem);
        };
        Fax.swapLookups = function (executionContext, oLookupA, oLookupB, telemetryItem) {
            ...
            if (swapStyle == "single" && (!Activities.Common.Util.IsNull(aoLookupAItems) && aoLookupAItems.length > 1 || !Activities.Common.Util.IsNull(aoLookupBItems) && aoLookupBItems.length > 1)) {
                ...
                Activities.ClientApi.openConfirmDialog(swapDialogStrings, dialogOptions).then(function (response) {
                    if (response.confirmed) {
                        ...
                        Fax.swapLookupValues(true, isTrimRequired, oLookupA, oLookupB);
                    }
                    else {
                        telemetryItem.traceEventInformation("Trimming lookups not confirmed. Resetting directioncode value.");
                        Fax.resetDirectionValues(executionContext);
                    }
                }, null);
                return;
            }
            Fax.swapLookupValues(result, isTrimRequired, oLookupA, oLookupB);
        };
        Fax.swapLookupValues = function (result, isTrimRequired, lookupA, lookupB) {
            ...
            if (isTrimRequired) {
                lookupAItems = Fax.trimLookup(lookupAItems);
                lookupBItems = Fax.trimLookup(lookupBItems);
            }
            lookupA.setValue(lookupBItems);
            lookupB.setValue(lookupAItems);
        };
        Fax.trimLookup = function (lookupItems) {
        ...
        Fax.resetDirectionValues = function (executionContext) {
...
(function (Activities) {
    ...
    /// Fax Page Handler
    /// </summary>
    var FaxPageHandler = (function (_super) {
        __extends(FaxPageHandler, _super);
        function FaxPageHandler() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this._actualDuration = 30;
            return _this;
        }
        /// <summary>
        /// Sets the default value for a new fax form
        /// </summary>
        FaxPageHandler.prototype.setDefaultValues = function (xrmPage, telemetryItem) {
        ...
        FaxPageHandler.prototype.setParty = function (xrmPage, attributeName, currentUser, telemetryItem) {
            try {
        ...
        /// </summary>
        FaxPageHandler.prototype.setActualDurationIfNull = function (xrmPage, telemetryItem) {
        ...
        return FaxPageHandler;
    }(Activities.ActivityPageHandler));
    Activities.FaxPageHandler = FaxPageHandler;
})(Activities || (Activities = {}));
