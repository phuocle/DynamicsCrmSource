...
function Close(oEvent)
{
    ...
    if(CurrentFormXml != BuildFormXML(false) || currentFormName != FORM_NAME || currentFormDescription != FORM_DESCRIPTION)
    {
...
function SelectionChanged()
{
    ...
    if(!IsNull(RemoveAllButton))
        RemoveAllButton.disabled = selectedAttributesFirstChild.childNodes.length == RequiredRowsCount(selectedAttributesFirstChild);
        ...
        if(GetRowCheckbox(oRow).checked)
        {
            ...
            if(req != SYSTEM_REQUIRED && req != BUSINESS_REQUIRED || haveDuplicates)
            {
                ...
                else
                {
                    ...
                    !IsNull(readonlyAttr) && readonlyAttr.value == "true" && 
...
function MoveLeftAll()
{
    ...
    UnselectLeft();
    SelectionChanged();
    movedCount > 0 && XUI.DomUtilities.HasChildElements(oList) && 
        alert(REMOVE_REQUIRED_ATTRIBUTES_WARNING)
}
...
function SetReadonlyAttributes(oRow,shouldBeReadonly)
{
    ...
    if(shouldBeReadonly && !haveReadonlyMark)
    {
        ...
        !shouldBeReadonly && haveReadonlyMark && 
...
function BuildFormXML(includeDisplayConditions)
{
    ...
    sXml += "<form>";
    sXml += "<tabs>";
    sXml += formatString('<tab name="general" verticallayout="true" id="{0}" {1} IsUserDefined="0">',CrmEncodeDecode.CrmXmlAttributeEncode(CURRENT_TAB_ID),GetEncodedLabelIdAttribute(CURRENT_TAB_LABEL_ID));
    sXml += "<labels></labels>";
    sXml += "<columns>";
    sXml += '<column width="100%">';
    sXml += "<sections>";
    sXml += formatString('<section name="account information" showlabel="false" showbar="false" id="{0}" {1} IsUserDefined="0">',CrmEncodeDecode.CrmXmlAttributeEncode(CURRENT_SECTION_ID),GetEncodedLabelIdAttribute(CURRENT_SECTION_LABEL_ID));
    sXml += "<labels></labels>";
    sXml += "<rows>";
    sXml += "<row>";
    for(var oList = GetList("SelectedAttributes"),
        ...
        i = 0; i < oRowCount; i++)
    {
        ...
        sXml += formatString('<cell id="{0}" {1}>',CrmEncodeDecode.CrmXmlAttributeEncode(oList.childNodes[i].attributes.getNamedItem("cellid").value),GetEncodedLabelIdAttribute(oList.childNodes[i].attributes.getNamedItem("labelid").value));
        sXml += "<labels></labels>";
        sXml += formatString('<control id="{0}" classid="{1}" datafieldname="{0}" disabled="{2}" />',CrmEncodeDecode.CrmXmlAttributeEncode(oList.childNodes[i].attributes.getNamedItem("name").value),CrmEncodeDecode.CrmXmlAttributeEncode(oList.childNodes[i].attributes.getNamedItem("classid").value),CrmEncodeDecode.CrmXmlAttributeEncode(isDisabled));
        sXml += "</cell>"
    }
    sXml += "</row>";
    sXml += "</rows>";
