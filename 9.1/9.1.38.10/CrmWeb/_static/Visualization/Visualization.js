...
Mscrm.Visualization.showDrillDown = function(domEvent, seriesName, seriesIndex, yColumnName, formattedXValue, x1Value, lookupAttributeString, primaryGroupByStartDate, primaryGroupByEndDate, primaryGroupByAlias, x2Value, secondaryXLookupAttributeString, secondaryGroupByStartDate, secondaryGroupByEndDate, secondaryGroupByAlias) {
    var $v_0 = Mscrm.Visualization.$3();
    var $v_1 = $get('CrmChart');
    Mscrm.Visualization.cancelDrillDown(false, false);
    var $v_2 = seriesIndex;
    if (!IsNull(domEvent) && !IsNull(domEvent.target) && !IsNull(domEvent.target.nodeName) && domEvent.target.nodeName.toUpperCase() === 'AREA' && !IsNull(domEvent.target.getAttribute('href')) && !isNullOrEmptyString(domEvent.target.getAttribute('href').toString())) {
    ...
    if (!IsNull($v_1)) {
        ...
        if (!IsNull($v_0)) {
            ...
            Mscrm.Visualization.$0.set_isSecondaryGroupByRelatedEntityColumn(Mscrm.Visualization.$1('issecondarygroupbyrelatedentitycolumn'));
            Mscrm.Visualization.$0.set_secondaryGroupByRelatedEntityDetail(Mscrm.Visualization.$1('secondarygroupbyrelatedentitydetail'));
...
Mscrm.Visualization.$M = function($p0) {
    ...
    if ($p0.get_multiLevelDrillDownParametersBackup().get_count() > 0) {
        var $v_0 = $p0.get_multiLevelDrillDownParametersBackup().getAtIndex($p0.get_multiLevelDrillDownParametersBackup().get_count() - 1);
        var $v_1 = $p0.get_multiLevelDrillDownParametersBackup().getAtIndex($p0.get_multiLevelDrillDownParametersBackup().get_count());
        ...
        $p0.get_drillDownParamObj().set_isSecondaryGroupByRelatedEntityColumn($v_1.get_isSecondaryGroupByRelatedEntityColumn());
        $p0.get_drillDownParamObj().set_secondaryGroupByRelatedEntityDetail($v_1.get_secondaryGroupByRelatedEntityDetail());
        return true;
    }
    return false;
}
...
Mscrm.Visualization.drillChart = function() {
    ...
    if (!IsNull($v_2)) {
        var $v_3 = $get('btnBack');
        var $v_4 = $get('btnHome');
        Mscrm.Visualization.registerChildWithXrmUIControl($v_3, 'CrmChart', 'Back', Mscrm.Visualization.$4 + '_paneControl');
        Mscrm.Visualization.registerChildWithXrmUIControl($v_4, 'CrmChart', 'Home', Mscrm.Visualization.$4 + '_paneControl');
...
Mscrm.Visualization.$L = function() {
    return (document.implementation.hasFeature('org.w3c.svg', null) || (document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#BasicStructure', null) && document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#Shape', null)));
}
...
Mscrm.Visualization.hoverChartType = function(domEvent, hover, chartType) {
    ...
    if (hover) {
        var $v_2 = $v_0.id;
        $v_1.style.background = String.format('url(\'/_imgs/visualization/designer/{0}chart/{0}chart_sel.png\')', $v_2);
        ...
        if (Mscrm.Visualization.$5 !== Mscrm.Visualization.$2 && Mscrm.Visualization.$5.length) {
            var $v_3 = $v_0.id;
            $v_1.style.background = String.format('url(\'/_imgs/visualization/designer/{0}chart/{0}chart.png\')', $v_3);
            $v_1.className = '';
...
Mscrm.Visualization.hoverOKButton = function(srcElement, hover) {
    if (!IsNull(srcElement)) {
        ...
        if (IsNull($v_1) || $v_1 === false) {
            $v_0.style.background = (hover) ? 'url(\'/_imgs/visualization/ok_hover.png\')' : 'url(\'/_imgs/visualization/ok.png\')';
        }
    }
}
        ...
        if (!$v_0.style.display.length || $v_0.style.display === 'none') {
            ...
            for (var $v_3 = 0; $v_3 < $v_2.length; $v_3++) {
                if (!IsNull($v_2[$v_3].getAttribute('id'))) {
                    Mscrm.Visualization.registerChildWithXrmUIControl($v_2[$v_3], $v_2[$v_3].getAttribute('id').toString(), 'ChartDrillDownType', 'DrillDownContainer');
                }
            }
...
Mscrm.Visualization.$4 = ((!IsNull(Mscrm.Visualization.$7)) && (!IsNull(Mscrm.Visualization.$7.getAttribute('gridId')))) ? Mscrm.Visualization.$7.getAttribute('gridId').toString() : '';
