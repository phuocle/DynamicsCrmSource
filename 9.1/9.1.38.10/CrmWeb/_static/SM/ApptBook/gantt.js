...
{
    var cls = "ganttBlockOverlapped";
    return oBlock.classList.contains(cls)
}
...
function swapGradientColors(o)
{
    ...
    if(index != -1)
    {
        var endIndex = o.className.indexOf(" ",index),
            classNamesLength = o.className.length,
            oldClassName;
        if(endIndex != -1)
            oldClassName = o.className.substr(index,classNamesLength - index);
        else
...
function ResizeRowInTable(index,height)
{
    ...
    oRow.style.setProperty("height",height,"important")
}
