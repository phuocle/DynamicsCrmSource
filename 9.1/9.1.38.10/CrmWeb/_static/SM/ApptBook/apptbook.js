...
function Print()
{
    ...
    !IsNull(oGanttFrame) && !IsNull(oGanttFrame.window.$find("crmGrid")) && 
...
function Reschedule()
{
    ...
    var rescheduleQueryString = "?rof=false&loadDialog=true&autoSearch=true&id=" + CrmEncodeDecode.CrmUrlEncode(SelectedBlockId());
...
{
    return !IsNull(oGanttFrame) && !IsNull(oGanttFrame.selectedBlock) && oGanttFrame.selectedBlock.getAttribute("oid") != "" && oGanttFrame.selectedBlock.getAttribute("otype") != ""
}
        ...
        if(!IsNull(oRow))
            if(!IsNull(oRow.getAttribute("oid")) && !IsNull(oRow.getAttribute("otype")))
            {
                ...
                if(IsExpandedRow(oRow))
                    sDetailsUrlCache += "&rowType=" + CrmEncodeDecode.CrmUrlEncode(oRow.parentNode.parentNode.getAttribute("otype")) + "&rowId=" + CrmEncodeDecode.CrmUrlEncode(oRow.parentNode.parentNode.getAttribute("oid"));
                else
                    sDetailsUrlCache += "&rowType=" + CrmEncodeDecode.CrmUrlEncode(oRow.getAttribute("otype")) + "&rowId=" + CrmEncodeDecode.CrmUrlEncode(oRow.getAttribute("oid"));
                if(!IsNull(oBlock))
                    if(!IsNull(oBlock.getAttribute("oid")) && !IsNull(oBlock.getAttribute("otype")))
                        sDetailsUrlCache += "&blockType=" + CrmEncodeDecode.CrmUrlEncode(oBlock.getAttribute("otype")) + "&blockId=" + CrmEncodeDecode.CrmUrlEncode(oBlock.getAttribute("oid"))
            }
        bIsExpanded && 
            LoadDetailsPane()
    }
...
{
    !IsNull(oGanttFrame) && !IsNull(oGanttFrame.ClearSelection) && 
    ...
    !IsNull(sViewId) && sViewId != _sSearchResults && 
        updateStickyViewUrl(getViewType($get("crmGrid_SavedQuerySelector")),sViewId,$get("crmTypeSelector").value,null,extraAttributeDictionary)
}
function RefreshGantt()
{
    ...
    if(sLastViewId.length > 0 && sLastViewId != viewId)
        ...
        if(oGanttFrame.document.readyState == "complete" && !IsNull(oGanttFrame.window.$find("crmGrid")) && !IsNull(oGanttFrame.window.$find("crmGrid").GetProperty("pagingCookie")))
    ...
    $get("ganttPageBodyTable").summary = formatString(LOCID_OUTER_SUMMARY_STRING,CrmEncodeDecode.CrmHtmlEncode(outerSummaryArg))
}
function Today()
{
...
{
    _oCalendarTimer = null;
    ChangeRange(ParseUtcDate(oStartDate.getAttribute("returnValue")),ParseUtcDate(oEndDate.getAttribute("returnValue")))
}
...
function ExpandCalendar(expand)
{
    ...
    if(expand)
    {
        ...
        LaunchInlineCalendar(oStartDate,_startDate,1,true,oEndDate,null,$get("monthCalendar"),HILITE_DAY,_startDate);
...
function PopupCalendar(calbutton)
{
    ...
    LaunchCalendar(oStartDate.parentNode,oStartDate,_startDate,1,true,oEndDate,"parent.ReturnDate(this)",HILITE_DAY,_startDate,false,calbutton)
}
