...
function ComputeTotalMatchCodeLength()
{
                ...
                if(operator)
                    if(operator.isUnary())
                    {
                        ...
                        if(baseAttributeControl)
                        {
                            ...
                            if(baseAttributeMetadataList[baseAttributeName] && baseAttributeMetadataList[baseAttributeName] > 0)
                        ...
                        if(!window._isSingleEntityRule)
                        {
                            ...
                            if(matchingAttributeControl)
                            {
                                ...
                                if(matchingAttributeMetadataList[matchingAttributeName] && matchingAttributeMetadataList[matchingAttributeName] > 0)
                                    if(operator.get_dataValue() == "dedupe-equalsdateonly")
                                        currentRowMatchCodeLength = aspxVars_iDateOnlyMatchCodeLength;
                                    else
                                        currentRowMatchCodeLength = currentRowMatchCodeLength <= matchingAttributeMetadataList[matchingAttributeName] ? currentRowMatchCodeLength : matchingAttributeMetadataList[matchingAttributeName]
                            }
                        }
                    }
                    else
    ...
    XUI.Html.SetText(document.getElementById("status"),formatString(LOCID_DEDUP_MATCHCODE_STATUS,iTotalCurrentMatchCodeLength,aspxVars_iMaxMatchCodeLength));
...
function GetDeDupeAttributeOperatorList(args)
{
    ...
    {
        var valueControl = XUI.Html.DomUtils.GetFirstChild(appCondition.get_clientCache().GetElement("ValueControl","number"));
        if(valueControl)
        {
            ...
            var conditionParentControl = $find(regardingObject.id).get_conditionParentControl(),
                ...
                controlDiv = !IsNull(autoShowContainer) ? XUI.Html.DomUtils.GetChildElementAt(autoShowContainer,1) : null;
            if(controlDiv)
            ...
            if(appCondition.get_secondaryEntityCode() && appCondition.get_primaryEntityCode() != appCondition.get_secondaryEntityCode())
            {
