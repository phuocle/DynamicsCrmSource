...
function EditQueryHint()
{
    var url = Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/EditQueryHint.aspx"),
        setQueryHintCallback = Mscrm.Utilities.createCallbackFunctionForXrmDialog(callBackForSetQueryHint,[]),
        currentQueryHints = XUI.Xml.SelectSingleNode(oFetchXml,"fetch",null).getAttribute("options"),
        dialogOptions = new Xrm.DialogOptions;
    dialogOptions.height = 300;
    dialogOptions.width = 500;
    Xrm.Internal.openDialog(url.toString(),dialogOptions,currentQueryHints,null,setQueryHintCallback)
}
function callBackForSetQueryHint(updatedQueryHints)
{
    XUI.Xml.SelectSingleNode(oFetchXml,"fetch",null).setAttribute("options",updatedQueryHints);
    UpdateFormData()
}
function ViewCustomControlProperties(mode,name,description)
{
    var _viewCustomControlXml = null,
        customControlDescriptionsDoc = XUI.Xml.CreateDocument(),
        existingControlDescriptionsNode = XUI.Xml.SelectSingleNode(oGridXml,"grid/controlDescriptions",null);
    if(!IsNull(existingControlDescriptionsNode))
    {
        customControlDescriptionsDoc.appendChild(existingControlDescriptionsNode.cloneNode(true));
        _viewCustomControlXml = customControlDescriptionsDoc
    }
    if(!_bInitialized)
        return;
    var aViewInfo,
        crmForm = $get("crmForm");
    if(!IsNull(crmForm.iscustomizable))
        aViewInfo = [crmForm.name.value,crmForm.description.value,crmForm.iscustomizable.value,crmForm.querytype.value,$get("crmFormSubmit").crmFormSubmitObjectTypeCode.value,_viewCustomControlXml];
    else
        aViewInfo = [crmForm.name.value,crmForm.description.value,"1",crmForm.querytype.value,$get("crmFormSubmit").crmFormSubmitObjectTypeCode.value,_viewCustomControlXml];
    var oUrl = Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/ViewCustomControls.aspx?mode=" + mode),
        dialogOptions = new Xrm.DialogOptions;
    dialogOptions.height = 660;
    dialogOptions.width = 500;
    var parameters = [crmForm,mode,name,description],
        viewCustomControlPropertiesCallback = Mscrm.Utilities.createCallbackFunctionForXrmDialog(callBackForviewCustomControlProperties,parameters);
    Xrm.Internal.openDialog(oUrl.toString(),dialogOptions,aViewInfo,null,viewCustomControlPropertiesCallback);
    AddTelemetryLog("CustomControlProperties")
}
function callBackForviewCustomControlProperties(oRet,crmForm,mode,name,description)
{
    if(oRet)
    {
        var newControlDescriptionsNode = XUI.Xml.SelectSingleNode(oRet[2],"controlDescriptions",null).cloneNode(true);
        updateCustomControlDescriptions(newControlDescriptionsNode);
        crmForm.name.value = oRet[0];
        crmForm.description.value = oRet[1];
        if(crmForm.name.value != "" && mode != "saveas")
        {
    ...
    {
        crmForm.name.value = "";
        crmForm.description.value = ""
    }
}
function updateCustomControlDescriptions(newControlDescriptionsNode)
{
    var oldControlDescriptionsNode = XUI.Xml.SelectSingleNode(oGridXml,"grid/controlDescriptions",null),
        oControlDescriptionsNode = oGridXml.createElement("controlDescriptions");
    !IsNull(XUI.Xml.SelectSingleNode(newControlDescriptionsNode,"controlDescription",null)) && 
        oControlDescriptionsNode.appendChild(XUI.Xml.SelectSingleNode(newControlDescriptionsNode,"controlDescription",null));
    if(!IsNull(oldControlDescriptionsNode))
        oldControlDescriptionsNode.parentNode.replaceChild(oControlDescriptionsNode,oldControlDescriptionsNode);
    else
        oGridXml.childNodes[0].appendChild(oControlDescriptionsNode);
    UpdateFormData()
}
...
function performActionAfterAddingColumns(oFieldObjs,oFetchXml)
{
    if(oFieldObjs)
    {
            ...
            i = oFieldObjs.length - 1; i >= 0; i--)
        {
            ...
            if(IsMultiValueTypeOperator(sOperator))
                sFilterXml += '<condition attribute="' + oFieldObjs[i].FieldName + '" operator="' + sOperator + '"><value>' + sValue + "</value></condition>";
            else
...
function IsMultiValueTypeOperator(sOperator)
{
    if(sOperator == "contain-values")
        return true;
    else
        return false
}
