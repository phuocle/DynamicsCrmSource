...
{
    var oFieldNode = XUI.Xml.SelectSingleNode(oPropertiesXml,'entities/entity[@name="' + sEntityName + "\"]/fields/field[@name = '" + sFieldName + "']",null);
    return oFieldNode != null ? GetDescByLangCode(GetLocalizedObjsArray(XUI.Xml.SelectNodes(oFieldNode,"displaynames/displayname",null))) : null
}
...
function GetFetchAttributeNode(oFetchXml,sFieldName)
{
    ...
    if(oFieldName.Alias.length == 0)
        ...
        return XUI.Xml.SelectSingleNode(oFetchXml,'/fetch/entity/link-entity[@alias="' + oFieldName.Alias + "\"]/attribute[@name = '" + oFieldName.FieldName + "']",null)
}
function isSortableField(oFieldsXml,sEntityName,sFieldName)
{
    if(sFieldName.indexOf(".") != -1)
        return false;
    var oFieldNode = XUI.Xml.SelectSingleNode(oFieldsXml,'entities/entity[@name="' + sEntityName + '"]/fields/field[@name = "' + CrmEncodeDecode.CrmXmlEncode(sFieldName) + '"]',null);
    if(oFieldNode == null)
        return false;
...
function GetFieldInfo(sFieldName)
{
    ...
    {
        var oFieldNode = XUI.Xml.SelectSingleNode(oFieldsXml,'entities/entity[@name="' + sEntityName + "\"]/fields/field[@name = '" + sFieldName + "']",null);
        if(oFieldNode != null)
        {
    ...
    return {Alias:sAlias,FieldName:sFieldName,EntityName:sEntityName,EntityDisplayName:sEntityDisplayName,Type:sType,DisplayName:sDisplayName,SchemaName:sSchemaName,GridFieldDisplayMask:sGridFieldDisplayMask,InvalidField:oFieldNode == null,RelationshipId:sRelationshipId}
}
function GetDisableMetaDataBindingFieldInfo(oCell)
{
    ...
    return {Alias:sAlias,FieldName:sFieldName,EntityName:sEntityName,EntityDisplayName:sEntityDisplayName,Type:sType,DisplayName:sDisplayName,SchemaName:sSchemaName,GridFieldDisplayMask:sGridFieldDisplayMask,InvalidField:false,RelationshipId:sRelationshipId}
}
...
function GetFieldsXml(sEntityName,sAlias,bRetrieveLatest)
{
    ...
    return GetFieldsXmlInternal(sEntityName,sAlias,_oGridXml,_bQFAddSearch,_bExportSelect,_bUpdateOnAttr,_bSelectMode,bRetrieveLatest)
}
function GetFieldsXmlInlcudingHiddenFields(sEntityName,sAlias,bRetrieveLatest)
{
    if(typeof sEntityName == "undefined")
        var sEntityName = _sPrimaryEntityName;
    return GetFieldsXmlInternal(sEntityName,sAlias,_oGridXml,_bQFAddSearch,_bExportSelect,_bUpdateOnAttr,_bSelectMode,bRetrieveLatest,true)
}
function GetFieldsXmlInternal(sEntityName,sAlias,oGridXml,bQFAddSearch,bExportSelect,bUpdateOnAttr,bSelectMode,bRetrieveLatest,includeHiddenFields)
{
    ...
    oPropertiesXml = XUI.Xml.SelectSingleNode(_oAllFieldPropertiesXml,'/entities/entity[@name="' + sEntityName + '"]',null);
    ...
    if(!(bQFAddSearch || bExportSelect || bUpdateOnAttr || bSelectMode))
        if(includeHiddenFields)
            sFilter = BuildUsedFieldFilter(XUI.Xml.SelectNodes(oGridXml,"/grid/row/cell[not(@ishidden=1)]",null),sEntityName,sAlias);
        else
            sFilter = BuildUsedFieldFilter(XUI.Xml.SelectNodes(oGridXml,"/grid/row/cell",null),sEntityName,sAlias);
    for(var oFields = XUI.Xml.SelectNodes(oPropertiesXml,"fields/field" + (sFilter.length > 0 ? "[" + sFilter + "]" : ""),null),
        sFieldsXml = "<fields>",
        iLen = oFields.length,
        i = 0; i < iLen; i++)
    {
        sFieldName = oFields[i].getAttribute("name");
        var oEntityFieldNode = XUI.Xml.SelectSingleNode(oFieldsXml,"fields/field[@name = '" + CrmEncodeDecode.CrmXmlEncode(sFieldName) + "']",null);
        if(oEntityFieldNode != null)
        {
            var bValidForAdvancedFind = XUI.Xml.GetText(oEntityFieldNode.attributes.getNamedItem("validforadvancedfind")) == "true",
            ...
            if(sFieldName != "" && (bQFAddSearch && bValidForAdvancedFind && IsValidForQuickFind(sDataType) || !bQFAddSearch && bValidForGrid && IsValidForGrid(sDataType)))
            {
                var displayNameObject = XUI.Xml.SelectSingleNode(oFields[i],"displaynames/displayname[@languagecode = " + USER_LANGUAGE_CODE + "]/@description",null),
                ...
                if(!IsNull(displayNameObject) && !isNullOrEmptyString(displayNameObjectText))
                    sFieldsXml += '<field name="' + CrmEncodeDecode.CrmXmlEncode(sFieldName) + '" displayname="' + CrmEncodeDecode.CrmXmlEncode(displayNameObjectText) + '" datatype="' + sDataType + '" required="false" orgrequired="' + XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oPropertiesXml,'fields/field[@name = "' + CrmEncodeDecode.CrmXmlEncode(sFieldName) + '"]/@requiredlevel',null)) + '"/>'
            }
        }
    }
...
function BuildUsedFieldFilter(oNodes,sEntityName,sAlias)
{
        ...
        i = 0; i < iLen; i++)
    {
        ...
        if((iPos = sFieldName.indexOf(".")) != -1)
        {
            ...
            {
                var oEntity = XUI.Xml.SelectSingleNode(_oEntitiesXml,'entities/entity[@alias="' + sFieldAlias + '"]',null);
                if(oEntity != null)
...
function IsValidForGrid(dataType)
{
    if(dataType === "file" || dataType === "unknown-data-type: file")
        return false;
    return true
}
