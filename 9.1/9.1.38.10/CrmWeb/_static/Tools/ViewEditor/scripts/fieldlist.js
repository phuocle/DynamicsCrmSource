...
function BuildFieldList(includeHiddenFields)
{
    ...
    XUI.Xml.Load(Mscrm.CrmUri.create("/Tools/FormEditor/fields.xsl").toString(),false,function(xmlDoc)
    {
        ...
        var _oFriendlyTypeNames = XUI.Xml.LoadXml(_sFriendlyTypeNames),
            documentElementNode = Mscrm.Utilities.getAdoptedNode(_oFieldXslDoc,_oFriendlyTypeNames.documentElement);
        XUI.Xml.SelectSingleNode(_oFieldXslDoc,"*/xsl:variable[@name='FriendlyTypeNames']",XUI.Xml.CreateNSDictionary(_oFieldXslDoc.documentElement)).appendChild(documentElementNode)
...
{
    for(var oNonPersistentProperties = XUI.Xml.SelectNodes(_oAllFieldPropertiesXml,"/entity/entity[@name='" + _sPrimaryEntityName + "']/fields/field[@sourcetype!='0']",null),
        len = oNonPersistentProperties.length,
        parentNode = XUI.Xml.SelectSingleNode(_oFieldXmlDoc,"/fields",null),
        i = 0; i < len; i++)
    {
        var nodeToRemove = XUI.Xml.SelectSingleNode(_oFieldXmlDoc,"/fields/field[@name='" + oNonPersistentProperties[i].getAttribute("name") + "']",null);
        !IsNull(nodeToRemove) && 
...
function RefreshFieldExplorer(oFormXml,oPropertiesXml,oFieldsXml)
{
    ...
    (!_oFieldXslDoc || _oFieldXslDoc.xml == "") && 
        XUI.Xml.Load(Mscrm.CrmUri.create("/Tools/FormEditor/fieldExplorer.xsl").toString(),false,function(xmlDoc)
        {
...
function SkipField(sectionName,fieldDataType)
{
    if(Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.ExternalStorage) && Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.FileType))
        if(fieldDataType === "file" || fieldDataType === "unknown-data-type: file")
            return true;
    return SkipFieldForSection(sectionName,fieldDataType)
}
function SkipFieldForSection(sectionName,fieldDataType)
{
...
function GetFilteredFieldsXml(oFormXml,oPropertiesXml,oFieldsXml)
{
    ...
    {
        case "section":
            formXmlPath = "/entity/form/tabs/tab/columns/column/sections/" + Mscrm.FormEditorVariables.fieldExpIsFor + "/rows/row/cell[control/@id]";
            break;
        case "header":
        case "footer":
    ...
    if(Mscrm.FormEditorVariables.currentFieldFilter != Mscrm.FieldFilter.custom)
        oFields = XUI.Xml.SelectNodes(oPropertiesXml,"/entity/fields/field" + (sFilter.length > 0 ? "[" + sFilter + "]" : ""),null);
    else
        if(sFilter.length > 0)
            oFields = XUI.Xml.SelectNodes(oPropertiesXml,"/entity/fields/field" + (sFilter.length > 0 ? "[" + sFilter + "]" : ""),null);
    ...
    if(!IsNull(oFields))
            ...
            i = 0; i < iLen; i++)
        {
            ...
            oEntityField = XUI.Xml.SelectSingleNode(oFieldsXml,"/entity/fields/field[@name = '" + sFieldName + "']",null);
            if(null != oEntityField)
                switch(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oEntityField,"./@datatype",null)))
                {
                    ...
                    case "timezone":
                        continue;
                    default:
                        if(SkipField(Mscrm.FormEditorVariables.fieldExpIsFor,XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oEntityField,"./@datatype",null))))
                            continue;
                        if(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oEntityField,"./@validforform",null)) == "true")
                            if(Mscrm.FormEditorVariables.FormType != "quickCreate" || Mscrm.FormEditorVariables.FormType == "quickCreate" && XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oEntityField,"./@validforcreateapi",null)) == "true")
                            {
                                ...
                                if(oFormat == null)
                                    ...
                                    sFormat = oFormat;
                                sFieldsXml += '<field name="' + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + '" displayname="' + CrmEncodeDecode.CrmXmlAttributeEncode(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oFields[i],'displaynames/displayname[@languagecode = "' + CrmEncodeDecode.CrmXmlAttributeEncode(USER_LANGUAGE_CODE) + '"]/@description',null))) + '" datatype="' + CrmEncodeDecode.CrmXmlAttributeEncode(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oFieldsXml,'/entity/fields/field[@name = "' + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + '"]/@datatype',null))) + '" format="' + CrmEncodeDecode.CrmXmlAttributeEncode(sFormat) + '" required="' + CrmEncodeDecode.CrmXmlAttributeEncode(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oFieldsXml,'/entity/fields/field[@name = "' + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + '"]/@requiredforplatform',null))) + '" orgrequired="' + CrmEncodeDecode.CrmXmlAttributeEncode(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oPropertiesXml,'/entity/fields/field[@name = "' + CrmEncodeDecode.CrmXmlAttributeEncode(sFieldName) + '"]/@requiredlevel',null))) + '"/>'
                            }
                        break
                }
        }
    ...
    for(var rowIndex = 0; rowIndex < _tblGridBar.rows.length; rowIndex++)
            ...
            i = 0; i < aCells.length; i++)
        {
            ...
            if(oCell.className == "ViewForm_AddFields_th_gridheader")
            {
                ...
                var imgTitle = sSortColumnName == oCell.getAttribute("field") ? bSortOrderAscend ? LOCID_ALT_COLUMNSORTORDER_UP : LOCID_ALT_COLUMNSORTORDER_DOWN : "";
                oImg.alt = imgTitle;
                oImg.title = imgTitle;
