...
{
    var oSections = XUI.Xml.SelectNodes(_oFormXml,"/entity/form/tabs/tab[@id = '" + crmDialog.TabId.value + "']/columns/column/sections/section[not(@locklevel) or @locklevel = '0']",null),
    ...
    for(var iLen = oSections.length,
        i = 0; i < iLen; i++)
        SelectSections.AddOption(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oSections[i],"labels/label[@languagecode = " + _langCode + "]/@description",null)),oSections[i].getAttribute("name"));
    SelectSections.AddToControl(Sections);
    UpdateLayout()
}
function BuildLocations(sTabName,sSectionName)
{
    XUI.Xml.Load(Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/fieldlocations.xsl").toString(),false,function(xmlDoc)
    {
        _oXslDoc = xmlDoc
...
{
    var oNodes = XUI.Xml.SelectNodes(_oFormXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell/control",null),
        iLen = oNodes.length,
        sNodeName = "",
        sFieldsAry = [];
    sFieldsAry[0] = [];
...
{
    return sDataType == "memo" || sDataType == "iframe" || sDataType == "unknown" || sDataType == "text" && sFormat == "textarea"
}
...
function GetDisplayName(sFieldName)
{
    ...
    if(oFieldNode)
    {
        var oPropertiesNode = XUI.Xml.SelectSingleNode(_oPropertiesXml,"/entity/fields/field[@name='" + sFieldName + "']",null);
        if(oPropertiesNode != null)
            sDisplayName = XUI.Xml.SelectSingleNode(oPropertiesNode,"displaynames/displayname[@languagecode = " + _langCode + "]",null).getAttribute("description");
        else
        {
            var sFieldLocName = XUI.Xml.SelectSingleNode(_oFieldsXml,"/entity/fields/field[@name='" + sFieldName + "']",null).getAttribute("localizedname");
            sDisplayName = sFieldLocName ? sFieldLocName : sFieldName
        }
    }
    else
    {
        var labelNode = XUI.Xml.SelectSingleNode(_oFormXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell[control/@id = '" + sFieldName + "']/labels/label[@languagecode = " + _langCode + "]",null);
        if(labelNode === null)
            sDisplayName = "";
        else
...
function UpdateLayout()
{
    ...
    var oField = getDialogArguments().Field,
        oSection = XUI.Xml.SelectSingleNode(_oFormXml,"/entity/form/tabs/tab/columns/column/sections/section[@id = '" + crmDialog.SectionId.value + "']",null),
...
function GetActualRowSpan()
{
    ...
    if(auto.checked)
    {
        var fieldName = $get("FieldName"),
            oTab = XUI.Xml.SelectSingleNode(_oFormXml,"/entity/form/tabs/tab[@id = '" + crmDialog.TabId.value + "']",null),
            oAutoCell = XUI.Xml.SelectSingleNode(oTab,"columns/column/sections/section/rows/row/cell[@auto = 'true' and control/@id != '" + fieldName.value + "' ]",null);
        if(oAutoCell != null && XUI.Xml.SelectSingleNode(oAutoCell,"control",null).getAttribute("id") != fieldName.value)
        ...
        if(teamTemplateNodes.length > 0)
        {
            ...
            for(var i = 0; i < teamTemplateNodes.length; i++)
            {
                var teamTemplateId = teamTemplateNodes[i].getAttribute("teamtemplateid"),
                    teamTemplateName = CrmEncodeDecode.CrmXmlDecode(teamTemplateNodes[i].getAttribute("teamtemplatename"));
...
function ToggleSelection(e,args)
{
    if(args.length < 2)
        return;
    var selection = $get(args[0]),
        listSelector = $get(args[1]);
    if(selection == null || listSelector == null)
        return;
    switch(selection.selectedIndex)
    {
        case 0:
            listSelector.disabled = true;
            EnableAll(listSelector);
            break;
        case 1:
...
function EnableAll(listSelector)
{
    for(i = 0; i < listSelector.length; i++)
        listSelector.options[i].selected = true
}
...
function CreateModuleSelector(onchangeHandler)
{
    var ModuleSelector = new Select;
    ModuleSelector.ID = "moduleSelection";
    var optGrpViewSel = new OptionGroup("",false,true,false);
    optGrpViewSel.AddOption(LOCID_SHOW_ALL,"Show All");
    optGrpViewSel.AddOption(LOCID_SHOW_SELECTED,"Show Selected");
    ModuleSelector.AddOptionGroup(optGrpViewSel);
    ModuleSelector.AddToControl($get("tdModuleSelector"));
    $get("moduleSelection").onchange = onchangeHandler;
    var moduleListSelector = $get("modules");
    if(moduleListSelector)
        moduleListSelector.parentElement.innerHTML = moduleListSelector.parentElement.innerHTML.replace("<select","<select MULTIPLE SIZE=4 ")
}
function CreateActivitySelector(onchangeHandler)
{
    var ActivitySelector = new Select;
    ActivitySelector.ID = "activitySelection";
    var optGrpViewSel = new OptionGroup("",false,true,false);
    optGrpViewSel.AddOption(LOCID_SHOW_ALL,"Show All");
    optGrpViewSel.AddOption(LOCID_SHOW_SELECTED,"Show Selected");
    ActivitySelector.AddOptionGroup(optGrpViewSel);
    ActivitySelector.AddToControl($get("tdActivitySelector"));
    $get("activitySelection").onchange = onchangeHandler;
    var activityListSelector = $get("activities");
    if(activityListSelector)
        activityListSelector.parentElement.innerHTML = activityListSelector.parentElement.innerHTML.replace("<select","<select MULTIPLE SIZE=4")
}
