...
{
    if(!IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.fieldExpIsFor) && Mscrm.FormEditorVariables.fieldExpIsFor == "section")
        return true;
    else
        return false
}
...
{
    if(!IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.isTabletEnabled) && Mscrm.FormEditorVariables.isTabletEnabled.toUpperCase() === "TRUE")
        return true;
    else
        return false
}
...
{
    if(!IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.fieldExpIsFor) && Mscrm.FormEditorVariables.fieldExpIsFor == "navigation")
        return true;
    else
        return false
}
...
{
    if(_oActive.className == Mscrm.DragDropUtils.TAB && _oActive.childNodes[0].childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].getAttribute("name").startsWith("ref_pan_"))
        return false;
    return Mscrm && Mscrm.FormEditorRibbonRules && !Mscrm.FormEditorRibbonRules.isReferencePanelSectionOrControlSelected(_oActive)
}
...
function isCellHealthWidget()
{
    if(_oActive && _oActive.className == "cell")
    {
        var oFormCell = Mscrm.DragDropUtils.getCellNodeFromId(_oActive.id),
            oControl = XUI.Xml.SelectSingleNode(oFormCell,"control",null);
        if(!isNullOrEmptyString(oControl) && oControl.getAttribute("uniqueid") == Mscrm.ControlClassIds.healthwidget)
            return true
    }
    else
        return false
...
{
    if(!IsNull(Mscrm.FormEditorVariables) && Mscrm.FormEditorVariables.editorType === Mscrm.EditorType.dashboardEditor || gutterValue > rowSpan)
...
function GetGutterValue(showChartOnly,showViewSelection,showSearch,showDispOption,showIndex,isDashboard)
{
    ...
    if(!showChartOnly)
        if(isDashboard)
            if(!showViewSelection && !showSearch && !showDispOption && !showIndex)
                gutter = 2;
            else
            {
                gutter = showViewSelection || showDispOption ? gutter + 26 : gutter + 0;
                gutter = showSearch ? gutter + 26 : gutter + 0;
                gutter = showIndex ? gutter + 22 : gutter + 0
            }
        else
        {
            gutter = 2;
...
function AllowRegisterNewObject()
{
    ...
    return !(isObjectBeingDragged && (isMouseDown || Mscrm.FormEditorVariables.editorType != Mscrm.EditorType.formEditor))
}
function GetLanguageCode()
{
    if(!IsNull(Mscrm.FormEditorVariables.formAccessType) && Mscrm.FormEditorVariables.formAccessType == Mscrm.EntityTypeCode.UserForm)
        return ORG_LANGUAGE_CODE;
    return USER_LANGUAGE_CODE
}
function SetActiveObject(domEvent,passedObj)
{
    if(!IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.selectedStreamDiv) && Mscrm.FormEditorVariables.setActiveStreamCalled != true)
    {
...
{
    AddTelemetryLog("RegUnRegDropTargets");
    switch(oObj.className)
    {
        case "ms-crm-Tab":
...
function GetObject(domEvent,oObj)
{
    ...
    if(oObj.tagName == "IMG" || oObj.tagName == "A")
        if(oObj.className == Mscrm.DashboardControls.placeholderAnchorClass || oObj.className == Mscrm.DashboardControls.PlaceholderAnchorForRACardsClass)
            oObj = oObj.parentNode;
        else
            if(oObj.className == Mscrm.DashboardControls.placeholderImgClass)
...
function GetEventObjsArray(oNodes)
{
        ...
        i = 0; i < iLen; i++)
    {
        ...
        oObjs[i] = new EventObj(oNode.getAttribute("name"),oNode.getAttribute("active") == "true",oNode.getAttribute("application") == "true",XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oNode,"script",null)),oDependsAry)
    }
    if(oObjs.length == 0)
        oObjs = null;
    return oObjs
...
function GetCurrentTabName(oActive)
{
    ...
    switch(oActive.className)
    {
        case "ms-crm-Tab":
            ...
            break;
        case "section":
            return oActive.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.id;
            break;
        case "cell":
        case "iframe":
        case "aci":
        ...
        case "orgInsights":
        case "ricards":
            return oActive.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.id;
            break
    }
}
function GetCurrentSectionName(oActive)
{
    if(oActive.parentNode.className == "ms-crm-Tab ms-crm-Tab-Selected")
        oActive = oActive.parentNode;
    switch(oActive.className.split(" ")[0])
    {
        case "ms-crm-Tab":
            return oActive.childNodes[0].childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].id;
            break;
        case "section":
        case "header":
        case "footer":
...
{
    var notesControlNode = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"//control[@classid='{06375649-C143-495E-A496-C962E5B4488E}' or @classid='{06375649-c143-495e-a496-c962e5b4488e}']",null);
...
function UpdateLabelsForNodes(tabNode)
{
    ...
    for(var labelSectionNodesCollection = tabNode.getElementsByTagName("section"),
        i = 0; i < labelSectionNodesCollection.length; i++)
    ...
    for(var labelCellNodesCollection = tabNode.getElementsByTagName("cell"),
        i = 0; i < labelCellNodesCollection.length; i++)
...
function SwitchToModernLayout()
{
    ...
    if(oUrl.indexOf("?") != -1)
        ...
        oUrl += "?";
    oUrl += "AllowFilterOff=0&DisableViewPicker=1&DisableQuickFind=0&LookupStyle=single&ShowNewButton=0&ShowPropButton=1&browse=false&dType=1&objecttypes=1030&objectTypeCode=" + _objectTypeCode;
    var executeCallBack = function(lookupItems)
        {
            ...
            var selectedFormXml = XUI.Xml.LoadXml(_formXmlString),
                currentXmlEventsCount = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/events/event",null).length,
            ...
            var selectedXmlHiddenControls = XUI.Xml.SelectSingleNode(selectedFormXml,"/entity/form/hiddencontrols",null),
                currentXmlHiddenControls = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/hiddencontrols",null);
            if(!(IsNull(currentXmlHiddenControls) || IsNull(selectedXmlHiddenControls)))
                ...
                IsNull(currentXmlHiddenControls) && !IsNull(selectedXmlHiddenControls) && 
                    XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form",null).appendChild(selectedXmlHiddenControls);
            var selectedXmlFormParameters = XUI.Xml.SelectSingleNode(selectedFormXml,"/entity/form/formparameters ",null),
                currentXmlFormParameters = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/formparameters ",null);
            if(!(IsNull(currentXmlFormParameters) || IsNull(selectedXmlFormParameters)))
                ...
                IsNull(currentXmlFormParameters) && !IsNull(selectedXmlFormParameters) && 
                    XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form",null).appendChild(selectedXmlFormParameters);
...
function SwitchFormSections(className,oObj)
{
    ...
    if(className == Mscrm.FormNavigationUtils.NAVIGATION)
        ...
        if(className == Mscrm.BusinessRulesExplorer.businessRuleExplorer)
...
{
    AddTelemetryLog("EditDashboardProperties");
    var args = {},
    ...
    Xrm.Internal.openDialog(Mscrm.CrmUri.create("/AdvancedFind/QueryProperties.aspx?feature=dashboard&istabletenabled=" + Mscrm.FormEditorVariables.isTabletEnabled).toString(),dialogOptions,args,null,callback);
    return
}
function updateDashboardProperties(updatedSectionObject,dashName,dashDesc)
{
    AddTelemetryLog("updateDashboardProperties");
            ...
            else
            {
                ...
                switch(oActive.className)
                {
                    ...
                    case "footer":
                        ...
                        break;
                    case "cell":
                        !oActive.getAttribute("name").startsWith("spacer_") && (className != "header" || className != "footer") && !isStreamOrTile(oActive) && 
                            UpdateField(oActive);
                        break;
                    case "iframe":
                    case "aci":
                    ...
                    case "interactionwall":
                        break;
                    case "unboundcustomcontrol":
                        UpdateUnboundCustomControl(oActive);
                        break;
                    case "subgrid":
                    case "ricards":
                        UpdateSubGrid(oActive);
...
function getAnchorElement(cellType)
{
    ...
    if(cellType === "queues")
    {
        ...
        newAnchor.setAttribute("onclick","Mscrm.PreviewCellUtils.addQueuesFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true)");
        newAnchor.setAttribute("ondblclick","Mscrm.PreviewCellUtils.stopPropagationFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event))");
        ...
        if(cellType === "tiles")
        {
            ...
            newAnchor.setAttribute("onclick","Mscrm.PreviewCellUtils.addTilesFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true)");
            ...
            if(cellType === "charts")
            {
                ...
                newAnchor.setAttribute("onclick","Mscrm.PreviewCellUtils.addSubgridFromPlaceHolder(Mscrm.Utilities.eventToDomEvent(event), true, true)");
...
function removeFormXml(cellType,id,streamCellId)
{
    if(cellType === "queues")
    {
        if(!IsNull(id))
        {
            var streamObjectXml = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab[@name = 'StreamsContainer']/columns/column/sections/section[@name = 'Streams']/rows/row/cell/control[@classid = '{EE9078C8-6946-4E2C-B8F8-35E65F4BE6A8}']/parameters/StreamObjects/StreamObject[@id = '" + id + "']",null);
        ...
        if(Mscrm.FormEditorVariables.streamsCount == 0)
        {
            ...
            var streamCell = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab[@name = 'StreamsContainer']/columns/column/sections/section[@name = 'Streams']/rows/row/cell",null),
                streamXml = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab[@name = 'StreamsContainer']/columns/column/sections/section[@name = 'Streams']/rows/row/cell/control",null);
        ...
        if(cellType === "tiles")
        {
            var tileCell = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab[@name = 'StreamsContainer']/columns/column/sections/section[@name = 'Tiles']/rows/row/cell[@id = '" + id + "']",null),
                tileXml = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab[@name = 'StreamsContainer']/columns/column/sections/section[@name = 'Tiles']/rows/row/cell[@id = '" + id + "']/control",null);
            ...
            if(cellType === "charts")
            {
                var chartCell = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab/columns/column/sections/section[@name = 'VisualFilters']/rows/row/cell[@id = '" + id + "']",null),
                    chartXml = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab/columns/column/sections/section[@name = 'VisualFilters']/rows/row/cell[@id = '" + id + "']/control",null);
...
{
    for(var chartCells = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"//entity/form/tabs/tab/columns/column/sections/section[@name = 'VisualFilters']/rows/row/cell",null),
        i = 0; i < chartCells.length; i++)
    {
...
function Delete()
{
    if(typeof Mscrm.FormEditorVariables.supportInteractionCentric != "undefined" && Mscrm.FormEditorVariables.supportInteractionCentric === "True")
    {
...
function RemoveControlDescriptorsForNodes(nodes)
{
    ...
    {
        var controlDescriptionsTagXmlNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"entity/form/controlDescriptions",null);
        if(!controlDescriptionsTagXmlNode)
            return;
        for(i = 0; i < nodes.length; i++)
        {
            ...
            {
                var nodeUniqueId = nodes[i].getAttribute("uniqueid"),
                    controlDescriptor = XUI.Xml.SelectSingleNode(controlDescriptionsTagXmlNode,"controlDescription[@forControl='" + nodeUniqueId + "']",null);
                if(controlDescriptor != null)
                {
                    var numberOfControlsForSameUniqueId = Mscrm.DragDropUtils.getNumberOfControlsForGivenUniqueId(Mscrm.FormEditorVariables.formXml,nodeUniqueId);
                    numberOfControlsForSameUniqueId <= 1 && 
...
{
    return oFieldNode.getAttribute("requiredforplatform") == "true" && (oFieldNode.getAttribute("validforcreateapi") == "true" || oFieldNode.getAttribute("validforupdateapi") == "true")
}
...
{
    return oPropertiesXml && XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oPropertiesXml,"/entity/fields/field[@name = '" + sFieldName + "']/@requiredlevel",null)) == "required"
}
function IsCompositeFieldAndAllowedForRemoval(sFieldName)
{
    ...
    if(FieldIsCompositeControl(sFieldName))
    {
        ...
        {
            var childNode = XUI.Xml.SelectSingleNode(oFieldsXml,"/entity/fields/field[@name = '" + constituentfields[j] + "']",null);
...
function IsDependent(oNodes)
{
        ...
        i = 0; i < iLen; i++)
    {
        ...
        if(sFieldName)
        {
            if(XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell/events/event/dependencies/dependency[@id = '" + sFieldName + "']",null).length > 0)
                return true;
            if(XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/events/event/dependencies/dependency[@id = '" + sFieldName + "']",null).length > 0)
                return true;
            if(XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/externaldependencies/dependency[@id = '" + sFieldName + "']",null).length > 0)
                return true
        }
    }
    return false
...
function GetDependentList(oNodes,bIncludeFieldName)
{
        ...
        i = 0; i < iLen; i++)
    {
        ...
        if(sFieldName)
        {
            var oDependentNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell/events/event/dependencies/dependency[@id = '" + sFieldName + "']",null),
            ...
            oDependentNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/events/event/dependencies/dependency[@id = '" + sFieldName + "']",null);
            iDepLen = oDependentNodes.length;
            for(j = 0; j < iDepLen; j++)
            {
                var bSystem = oDependentNodes[j].parentNode.parentNode.getAttribute("application") == "true";
                oDependents[iIndex++] = new DependencyObj(oDependentNodes[j].parentNode.parentNode.parentNode.parentNode.getAttribute("name"),"tab",oDependentNodes[j].parentNode.parentNode.getAttribute("name"),bSystem)
            }
            oDependentNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/events/event/dependencies/dependency[@id = '" + sFieldName + "']",null);
            iDepLen = oDependentNodes.length;
            for(j = 0; j < iDepLen; j++)
            {
                var bSystem = oDependentNodes[j].parentNode.parentNode.getAttribute("application") == "true",
            ...
            oDependentNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/externaldependencies/dependency[@id = '" + sFieldName + "']",null);
            iDepLen = oDependentNodes.length;
            for(j = 0; j < iDepLen; j++)
    ...
    for(i = 0; i < iDepLen; i++)
    {
        ...
        switch(oDependents[i].Type)
        {
            ...
            case "external":
                ...
                break;
            case "tab":
                sfieldList += formatString(LOCID_FORMED_TABDEP,"",GetEventLocName(oDependents[i].Name),LOCID_TAB_LABEL + ":" + oDependents[i].FieldName,sDependencyType) + "\n";
                break
        }
    }
    return sfieldList
...
{
    return XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + sTabId + "']",null)
}
function GetSectionsNode(sTabId,iTabColumn)
{
    var columnNode = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + sTabId + "']/columns/column",null)[iTabColumn];
...
{
    return XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section[@id = '" + sSectionName + "']/rows",null)
}
function GetRowNode(sFieldName)
{
    return XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row[cell/control/@id = '" + sFieldName + "']",null)
}
...
{
    return XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell",null)
}
...
function ViewFormProperties()
{
    ...
    var oDependsNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,"/entity/form/externaldependencies/dependency",null),
        iDependsLen = oDependsNodes.length,
        oDependsAry = new Array(iDependsLen);
    for(j = 0; j < iDependsLen; j++)
        oDependsAry[j] = oDependsNodes[j].getAttribute("id");
    var formName = "",
        formNameNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/resources/resource[@id='formName']",null);
    if(!IsNull(formNameNode))
        formName = XUI.Xml.GetText(formNameNode);
    var formDescription = "",
        formDescriptionNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/resources/resource[@id='formDescription']",null);
    if(!IsNull(formDescriptionNode))
        formDescription = XUI.Xml.GetText(formDescriptionNode);
    var oFormObj = new FormObj(Mscrm.FormEditorVariables.formXml,oDependsAry,!bFormAssistantCollapsed,bShowNavBar,formName,formDescription,bEnableFormAssistant,oFormNode.getAttribute("maxWidth"),bshowImage);
    oArgs.FormProperties = oFormObj;
    oArgs.objectTypeCode = _objectTypeCode;
    var url = Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/formProperties.aspx?formType=" + CrmEncodeDecode.CrmUrlEncode(Mscrm.FormEditorVariables.FormType) + "&objectTypeCode=" + _objectTypeCode),
        dialogOptions = new Xrm.DialogOptions;
    dialogOptions.height = 700;
    dialogOptions.width = 650;
    var callbkFunction = Mscrm.Utilities.createCallbackFunctionForXrmDialog(BindFormProperties,[formName,formDescription,oDependsAry]);
...
function BindFormProperties(oUpdatedFormObj,formName,formDescription,oDependsAry)
{
    if(oUpdatedFormObj)
    {
        ...
        if(!IsNull(oUpdatedFormObj.FormXml) && XUI.Xml.XMLSerializer.serializeToString(oUpdatedFormObj.FormXml) != XUI.Xml.XMLSerializer.serializeToString(Mscrm.FormEditorVariables.formXml))
        {
        ...
        if(Mscrm.FormEditorVariables.FormType != "quick" && Mscrm.FormEditorVariables.FormType != "quickCreate" && Mscrm.FormEditorVariables.FormType != "card")
        {
        ...
        {
            var nameNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/resources/resource[@id='formName']",null);
            !IsNull(nameNode) && 
        ...
        {
            var descriptionNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/resources/resource[@id='formDescription']",null);
        ...
        if(oUpdatedFormObj.Dependencies)
        {
            var iNewLen = oUpdatedFormObj.Dependencies.length,
                oParentNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/externaldependencies",null);
            if(!oParentNode)
            {
...
function PersistFormEventXml(oEventNode,oUpdatedEventObj)
{
    ...
    if(!oEventNode)
    {
        ...
        if(!oEvents)
        {
            ...
            var oNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form",null),
                oRefNode = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/externaldependencies",null);
    ...
    XUI.Html.DomUtils.GetFirstChild(oScriptNode).replaceData(0,XUI.Html.DomUtils.GetFirstChild(oScriptNode).length,oUpdatedEventObj.Script)
}
function GetFieldDisplayName(sFieldName)
{
    ...
    if(oFieldNode)
    {
        var oPropertiesNode = XUI.Xml.SelectSingleNode(oPropertiesXml,"/entity/fields/field[@name='" + sFieldName + "']",null);
        if(oPropertiesNode != null)
            sDisplayName = XUI.Xml.SelectSingleNode(oPropertiesNode,"displaynames/displayname[@languagecode = " + _langCode + "]").getAttribute("description");
        else
        {
            var sFieldLocName = XUI.Xml.SelectSingleNode(oFieldsXml,"/entity/fields/field[@name='" + sFieldName + "']",null).getAttribute("localizedname");
            sDisplayName = sFieldLocName ? sFieldLocName : sFieldName
        }
    }
    else
        sDisplayName = Mscrm.FormEditorVariables.formXml.selectSingleNode("/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell[control/@id = '" + sFieldName + "']/labels/label[@languagecode = " + _langCode + "]").getAttribute("description");
    return sDisplayName
}
...
var _ControlClassIds = {iframe:"{FD2A7985-3187-444e-908D-6624B21F69C0}",lookup:"{270BD3DB-D9AF-4782-9025-509E298DEC0A}",regarding:"{F3015350-44A2-4aa0-97B5-00166532B5E9}",partylist:"{CBFB742C-14E7-4a17-96BB-1A13F7F64AA2}",duration:"{AA987274-CE4E-4271-A803-66164311A958}",integer:"{C6D124CA-7EDA-4a60-AEA9-7FB8D318B68F}","float":"{0D2C745A-E5A8-4c8f-BA63-C6D3BB604660}",money:"{533B9E00-756B-4312-95A0-DC888637AC78}",decimal:"{C3EFE0C3-0EC6-42be-8349-CBD9079DFD8E}",email:"{ADA2203E-B4CD-49be-9DDF-234642B43B52}",emailbody:"{6F3FB987-393B-4d2d-859F-9D0F0349B6AD}",text:"{4273EDBD-AC1D-40d3-9FB2-095C621B552D}",textarea:"{E0DECE4B-6FC8-4a8f-A065-082708572369}",url:"{71716B6C-711E-476c-8AB8-5D11542BFB47}",tickersymbol:"{1E1FC551-F7A8-43af-AC34-A8DC35C7B6D4}",memo:"{E0DECE4B-6FC8-4a8f-A065-082708572369}",picklist:"{3EF39988-22BB-4f0b-BBBE-64B5A3748AEE}",multiselectpicklist:"{4AA28AB7-9C13-4F57-A73D-AD894D048B5F}",status:"{5D68B988-0661-4db2-BC3E-17598AD3BE6C}",datetime:"{5B773807-9FB2-42db-97C3-7A91EFF8ADFF}",boolean_radio:"{67FAC785-CD58-4f9f-ABB3-4B7DDC6ED5ED}",boolean_checkbox:"{B0C6723A-8503-4fd7-BB28-C8A06AC933C2}",language:"{671A9387-CA5A-4d1e-8AB7-06E39DDCF6B5}",subgrid:"{E7A81278-8635-4d9e-8D4D-59480B391C5B}",timezone:"{7C624A0B-F59E-493d-9583-638D34759266}",connection:"{3246F906-1F71-45F7-B11F-D7BE0F9D04C9}",connectionroleojbjecttypecodelist:"{821ACF1A-7E46-4A0C-965D-FE14A57D78C7}",webResourceHtml:"{9FDF5F91-88B1-47f4-AD53-C11EFC01A01D}",webResourceImage:"{587CDF98-C1D5-4bde-8473-14A0BC7644A7}",webResourceSilverlight:"{080677DB-86EC-4544-AC42-F927E74B491F}",powerBITile:"{8C54228C-1B25-4909-A12A-F2B968BB0D62}",notes:"{06375649-c143-495e-a496-c962e5b4488e}",quickformcollection:"{5C5600E0-1D6E-4205-A272-BE80DA87FD42}",referencepanelquickformcollection:"{B68B05F0-A46D-43F8-843B-917920AF806A}",referencepanelsubgrid:"{02D4264B-47E2-4B4C-AA95-F439F3F4D458}",interactionwall:"{1F179106-FA28-4495-961E-F6BD93C21974}",bingmap:"{62B0DF79-0464-470F-8AF7-4483CFEA0C7D}",socialInsight:"{86B9E25E-695E-4FEF-AC69-F05CFA96739C}",orgInsights:"{76B9E25E-695E-4FEF-AC69-F05CFA96739C}",timercontrol:"{9C5CA0A1-AB4D-4781-BE7E-8DFBE867B87E}",searchwidget:"{E616A57F-20E0-4534-8662-A101B5DDF4E0}",referencepanelsearchwidget:"{7CCD1494-1F7A-4E3A-8BDE-F32069DAEB9F}",queuecontainer:"{EE9078C8-6946-4E2C-B8F8-35E65F4BE6A8}",multiplepiechart:"{F130D8AE-CE5B-43C5-BED1-1A6A5856CF3D}",tagcontrol:"{6FAE836F-FC01-48DE-9B63-9B68A8FD86B8}",customControl:"{F9A8A302-114E-466A-B582-6771B2AE0D92}",aci:"{C8BFBBEF-6851-4401-A0CC-7450062FE085}"},
    _oConst = {iAttributeEditorWidth:900,iAttributeEditorHeight:660},
    _oDataTypes = {unknown:"unknown",customer:"customer",owner:"owner",lookup:"lookup",partylist:"partylist",integer:"integer","float":"float",money:"money",decimal:"decimal",text:"text",memo:"memo","boolean":"boolean",picklist:"picklist",multiselectpicklist:"multiselectpicklist",status:"status",state:"state",datetime:"datetime",iframe:"iframe"},
    WebResourceTypes = {html:1,css:2,jscript:3,xml:4,png:5,jpg:6,gif:7,xap:8,xls:9,ico:10,svg:11,resx:12},
    _oDataTypeFormats = {regarding:"regarding",connection:"connection",duration:"duration",timezone:"timezone",language:"language",email:"email",emailbody:"emailbody",text:"text",textarea:"textarea",url:"url",tickersymbol:"tickersymbol"};
function GetControlClassId(sDataType,sFormat)
{
    switch(sDataType)
    {
        ...
        case _oDataTypes.multiselectpicklist:
            return _ControlClassIds.multiselectpicklist;
    ...
    switch(Mscrm.FormEditorVariables.editorType)
    {
        case Mscrm.EditorType.formEditor:
            ...
            if(Mscrm.FormUtils.isFormDirty() || Mscrm.FormEditorVariables.currentFormId == Mscrm.FormXmlUtils.emptyGuid)
            {
                ...
                formDetails.description = Mscrm.FormXmlUtils.getResourceValue("formDescription",Mscrm.FormEditorVariables.formXml);
                ...
                if(Mscrm.FormEditorVariables.currentFormId != Mscrm.FormXmlUtils.emptyGuid)
                    actionResult = Mscrm.FormCrudUtils.executeAction(Mscrm.FormCrudAction.update,formDetails,isMergeButtonClicked);
                else
                {
                ...
                if(!IsNull(actionResult) && actionResult.success)
                {
                    ...
                    if(!designer)
                    {
                        !IsNull(window.top) && !IsNull(window.top.opener) && !window.top.opener.closed && 
                            window.top.opener.auto(Mscrm.EntityTypeCode.SystemForm);
                        !IsNull(window.top) && !IsNull(window.top.opener) && window.top.opener.name == "hierarchySettingspage" && 
                    ...
                    if(!bClose)
                    {
                        if(actionResult.id != Mscrm.FormEditorVariables.currentFormId)
                        {
                            ...
                            window.parent.location.href = Mscrm.FormUrlUtils.getUpdatedUrl(window.parent.location.href,actionResult.id)
                        }
                    }
                    else
                        closeWindow()
...
function callBackForSaveAs(returnvalue)
{
    if(!IsNull(returnvalue) && returnvalue.success)
    {
        ...
        if(!designer)
        {
            ...
            Mscrm.FormUndoRedo.dispose();
            !IsNull(window.top) && !IsNull(window.top.opener) && 
                window.top.opener.auto(Mscrm.EntityTypeCode.SystemForm)
        }
        else
        {
...
function IsRequestFromAppDesigner()
{
    ...
    if(designer)
        designer = Mscrm.FeatureControl.get_Current().areFeaturesEnabled([Mscrm.FeatureNames.AppModuleForOrganization,Mscrm.FeatureNames.AppDesigner]) && !IsNull(window.parent) && !IsNull(window.parent.opener) && !IsNull(window.parent.opener.AppDesignerCallback);
    return designer
}
function SendResponseToAppDesigner(result)
{
    ...
    var resultobj = {id:id.toLowerCase(),displayName:result.name,formType:GetFormTypeCode(Mscrm.FormEditorVariables.FormType),description:result.description};
...
function DisplayActionMsg(sMsg,iWidth,iHeight)
{
    ...
    if(IsNull(_oActMsgPopup))
    {
        ...
        _oActMsgDom.innerHTML = "<table width=" + iWidth + "px height=" + iHeight + "px><tr><td style='font-weight: bold; font-size: 15px;text-align: center; vertical-align: middle; cursor: wait; color: #000099'> </td></tr></table>";
...
function PreviewMoca(sType)
{
    ...
    {
        var formAccessType = IsNull(Mscrm.FormEditorVariables.formAccessType) ? Mscrm.EntityTypeCode.SystemForm : Mscrm.FormEditorVariables.formAccessType,
            formId = Mscrm.FormEditorVariables.currentFormId === "" ? Mscrm.FormEditorVariables.formId : Mscrm.FormEditorVariables.currentFormId,
    ...
    function OpenInNewWindow(url,previewData)
    {
        ...
        {
            mocaPreviewFormAdded && mocaPreviewForm && 
...
function LoadXmlForEditorTransform(renderingPart)
{
    ...
    switch(renderingPart)
    {
        case Mscrm.Render.wholeCanvas:
            _oXmlDocEditor = XUI.Xml.LoadXml("<entity><formeditor>" + formXMLString + "</formeditor><metadata>" + fieldXml + "</metadata><properties>" + propertiesXml + "</properties><strings><lockedsection>" + CrmEncodeDecode.CrmXmlEncode(LOCID_FORMEDITOR_SECTION_LOCKED) + "</lockedsection></strings></entity>");
            break;
...
function IsFCBAndEnableDisableRICardsActionControl()
{
    return Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.ActionCard) && EnableDisableRICardsActionsControl()
}
function RefreshFormEditor(renderingPart)
{
    ...
    _oXslProcEditor.addParameter("supportSocialInsights",Mscrm.FormEditorVariables.supportSocialInsights == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("supportPowerBITile",Mscrm.FormEditorVariables.supportPowerBITile == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("supportOrgInsights",Mscrm.FormEditorVariables.supportOrgInsights == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("supportDelve",Mscrm.FormEditorVariables.supportDelve == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("supportInteractionCentric",Mscrm.FormEditorVariables.supportInteractionCentric == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("supportACIControl",Mscrm.FormEditorVariables.supportACIControl == "True" ? "true" : "false");
    _oXslProcEditor.addParameter("isRiCardsCell",IsFCBAndEnableDisableRICardsActionControl());
    ...
    typeof SetElementsPosition != "undefined" && 
        SetElementsPosition();
    SetPowerAppsNavBarComponentName()
}
function SetPowerAppsNavBarComponentName()
{
    var name = "";
    if(!IsNull(Mscrm.FormEditorVariables))
    {
        if(Mscrm.FormEditorVariables.editorType === Mscrm.EditorType.formEditor)
            if(Mscrm.FormEditorVariables.currentFormId == Mscrm.FormXmlUtils.emptyGuid)
                name = window.top.LOCID_FORM_TITLE_NEW;
            else
                name = Mscrm.FormXmlUtils.getResourceValue("formName",Mscrm.FormEditorVariables.formXml);
        else
            if(Mscrm.FormEditorVariables.editorType === Mscrm.EditorType.dashboardEditor)
            {
                var dashName = Mscrm.FormControlInputBehavior.GetBehavior("dashboardNameInput");
                if(!IsNull(dashName) && dashName.get_element().value != "")
                    name = dashName.get_element().value;
                else
                    name = window.top.LOCID_FORM_TITLE_NEW
            }
        Mscrm.Utilities.updatePowerAppsNavBarComponentName(name)
    }
}
function SetPreviewXml()
{
    ...
    var previewXml = XUI.Xml.LoadXml(Mscrm.FormEditorVariables.formXml.xml),
        cells = XUI.Xml.SelectNodes(previewXml,"/entity/form/tabs/tab/columns/column/sections/section/rows/row/cell",null);
...
{
    if(!IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.fieldExpIsFor) && Mscrm.FormEditorVariables.fieldExpIsFor == name)
...
function AutoExpandingRequired(tabId,controlId,formXml)
{
    var bAutoExpand = true,
        ...
        oAutoCell = XUI.Xml.SelectSingleNode(oTab,"columns/column/sections/section/rows/row/cell[@auto = 'true' and control/@id != '" + controlId + "' ]",null);
...
function ModifyOtherAutoExpandingFieldIfRequired(oCell)
{
    ...
    if(!IsNull(tabId))
    {
        var oTab = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,"/entity/form/tabs/tab[@id = '" + tabId + "']",null),
            oAutoCell = XUI.Xml.SelectSingleNode(oTab,"columns/column/sections/section/rows/row/cell[@auto = 'true' and control/@id != '" + XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oCell,"control/@id",null)) + "' ]",null);
        if(oAutoCell != null)
        {
...
{
    !IsNull(Mscrm.FormEditorVariables.currentFormId) && Mscrm.FormEditorVariables.currentFormId != Mscrm.FormXmlUtils.emptyGuid && 
...
{
    return !IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.isCustomizabe) && Mscrm.FormEditorVariables.isCustomizabe == "true"
}
function CanSaveAsForm()
{
    return !IsNull(Mscrm) && !IsNull(Mscrm.FormEditorVariables) && !IsNull(Mscrm.FormEditorVariables.canCreateForms) && Mscrm.FormEditorVariables.canCreateForms == "true"
}
...
{
    if(!IsNull(Mscrm.FormEditorVariables.currentFormId) && Mscrm.FormEditorVariables.currentFormId != Mscrm.FormXmlUtils.emptyGuid)
    {
        var oArgs = {};
        oArgs.formId = Mscrm.FormEditorVariables.currentFormId;
...
function OpenManagedProperties()
{
    if(!IsNull(Mscrm.FormEditorVariables.currentFormId) && Mscrm.FormEditorVariables.currentFormId != Mscrm.FormXmlUtils.emptyGuid)
    {
        Mscrm.ManagedPropertyUtil.launchManagedPropertyDlg(Mscrm.EntityTypeCode.SystemForm,Mscrm.FormEditorVariables.currentFormId);
        !IsNull(window.top) && !IsNull(window.top.opener) && 
            window.top.opener.auto(Mscrm.EntityTypeCode.SystemForm)
    }
}
...
{
    if(IsNull(XUI.Xml.SelectSingleNode(formXml,"/entity/form/tabs/tab[@name='" + sTabName + "' and @id !='" + tabId + "']",null)))
        return true;
    else
        return false
}
...
{
    if(IsNull(XUI.Xml.SelectSingleNode(formXml,"/entity/form/tabs/tab/columns/column/sections/section [@name='" + sSectionName + "' and @id !='" + sectionId + "']",null)) && !(sSectionName === "Header" || sSectionName === "Footer"))
        return true;
    else
        return false
}
...
function GetFormTypeCode(formTypeName)
{
    if(formTypeName == "dashboard")
        ...
        if(formTypeName == "appointmentBook")
            return 1;
        else
            if(formTypeName == "main")
                return 2;
            else
                if(formTypeName == "miniCampaignBO")
                    return 3;
                else
                    if(formTypeName == "preview")
                        return 4;
                    else
                        if(formTypeName == "mobile")
                            return 5;
                        else
                            if(formTypeName == "quick")
                                return 6;
                            else
                                if(formTypeName == "quickCreate")
                                    return 7;
                                else
                                    if(formTypeName == "card")
                                        return 11;
                                    else
                                        if(formTypeName == "mainInteractionCentric")
                                            return 12
}
function GetControlClassIdForFormFactor(formXml,customControlUniqueId)
{
    var classId = null;
    if(Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) && !IsNull(customControlUniqueId))
    {
        var controlDescriptionsTagXmlNode = XUI.Xml.SelectSingleNode(formXml,"entity/form/controlDescriptions",null);
        controlDescriptor = XUI.Xml.SelectSingleNode(controlDescriptionsTagXmlNode,"controlDescription[@forControl='" + customControlUniqueId + "']",null);
        if(!IsNull(controlDescriptor))
            for(var customControlNodes = XUI.Xml.SelectNodes(controlDescriptor,"customControl[@id and string(@id)]",null),
                count = 0; count < customControlNodes.length; count++)
            {
                var customControlNode = customControlNodes[count];
                if(!IsNull(customControlNode) && IsClassIdInFormControlsList(customControlNode.getAttribute("id")))
                {
...
function IsClassIdInFormControlsList(classId)
{
    for(var key in _ControlClassIds)
    {
        var formControlClassId = _ControlClassIds[key];
        if(formControlClassId.substring(0,1) == "{" && formControlClassId.substring(formControlClassId.length - 1,1) == "}" && classId.length > 2)
            formControlClassId = formControlClassId.substring(1,formControlClassId.length - 2);
        if(classId.substring(0,1) == "{" && classId.substring(formControlClassId.length - 1,1) == "}" && classId.length > 2)
            classId = classId.substring(1,classId.length - 2);
        if(classId.toUpperCase() == formControlClassId.toUpperCase())
            return true
    }
    return false
}
function AddTelemetryLog(operationName)
{
    try
    {
        ...
        Mscrm.MetricsReporting.instance().addMetric("clickactionutil",arguments)
    }
    catch(e)
    {
    }
