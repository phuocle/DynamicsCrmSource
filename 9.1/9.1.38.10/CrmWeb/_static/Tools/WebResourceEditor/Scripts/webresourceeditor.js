...
function SaveForm(fclose)
{
    if(FormFieldsValid(false))
        ...
        else
        {
            ...
            var arguments = {};
            if(_crmFormSubmit.id.value != "")
            {
                arguments["webresourceId"] = _crmFormSubmit.id.value;
                arguments["iswebResourcedependent"] = false;
                var table = document.getElementById("libraryGrid");
                if(table != undefined)
                {
                    var rows = table.getElementsByTagName("tr");
                    if(rows.length > 1)
                        arguments["iswebResourcedependent"] = true
                }
                Mscrm.MetricsReporting.instance().addMetric("webresourceDependency",arguments)
            }
...
function PickListValueChanged()
{
    switch(_crmFormSubmit.webResourceType.value)
    {
        case typeMapping["html"]:
            ...
            HideOrShowEnableForTablet("inline");
            if(isWebResourceLocalizationFCBEnabled())
            {
                HideorShowDependenciesTab("block");
                HideOrShowAddDependencyRelatedControls();
                HideOrShowAttributeRelatedControls("none");
                ...
                else
                {
                    HideOrShowMobileRelatedControls("none");
                    HideOrShowAttributeRelatedControls("none");
                    HideorShowDependenciesTab("none")
                }
            break;
        ...
        case typeMapping["xml"]:
            ...
            HideOrShowEnableForTablet("none");
            HideOrShowMobileRelatedControls("none");
            HideOrShowAttributeRelatedControls("none");
            HideorShowDependenciesTab("none");
            break;
        case typeMapping["js"]:
            ...
            {
                HideOrShowAddDependencyRelatedControls();
                HideOrShowAttributeRelatedControls("inline");
                HideorShowDependenciesTab("block")
            }
            else
            {
...
function HideorShowDependenciesTab(display)
{
    var tab = $get("tab1Tab");
...
function HideOrShowMobileRelatedControls(display)
{
    ...
    if(display == "inline")
    {
        ...
        if(!isWebResourceLocalizationFCBEnabled())
        {
            ...
            $get("librarySectionMessage").style.display = "";
            $get("dependencySectionTitle").style.display = display
        }
    }
    else
    {
...
function HideOrShowAttributeRelatedControls(display)
{
    var attributeGrid = $get("attributeGrid"),
        attributeGridMenubar = $get("attributeMenuBar");
    if(display == "inline")
    {
        attributeGridMenubar.style.display = "";
        attributeGrid.style.display = "";
        attributeGrid.style.borderColor = "#A9A9A9"
    }
    else
    {
        attributeGridMenubar.style.display = display;
        attributeGrid.style.display = display;
        attributeGrid.style.borderColor = "#ffffff"
    }
    $get("attributedependencysectionmessage").style.display = display;
    $get("attributeSectionMessage").style.display = display
}
function HideOrShowAddDependencyRelatedControls()
{
    ...
    libraryGrid.style.borderColor = "#A9A9A9";
    $get("librarySectionMessage").style.display = "";
    $get("dependencySectionTitle").style.display = "none"
}
...
function isWebResourceLocalizationFCBEnabled()
{
...
function HideOrShowEnableForTablet(display)
{
    $get("mobiledependencynavigationanchorid").style.display = display;
    _crmFormSubmit.IsEnabledForMobileClient.style.display = display;
...
function createEmptyWebResourceDependencyXml()
{
    _xmlDoc = XUI.Xml.CreateDocument();
    var dependenciesElementNode = _xmlDoc.createElement("Dependencies"),
        dependencyElementNode = _xmlDoc.createElement("Dependency"),
        attrib = _xmlDoc.createAttribute("componentType");
...
function serializeWebResourceDependencyXML(dependencyXmlDoc)
{
    if(dependencyXmlDoc)
        return XUI.Xml.XMLSerializer.serializeToString(XUI.Xml.SelectSingleNode(dependencyXmlDoc,"/Dependencies"));
    return ""
}
...
var typeMapping = {html:"1",htm:"1",css:"2",jscript:"3",js:"3",xml:"4",png:"5",jpg:"6",jpeg:"6",gif:"7",xap:"8",xsl:"9",xslt:"9",ico:"10",svg:"11",resx:"12"},
