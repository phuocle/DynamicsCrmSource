...
var _oMessages = {sCloseAlert:LOCID_FORMS_SAVE_CONFIRM_TITLE},
    _oConst = {text:"text",$none:"$none",none:"none",block:"block",sInvalidSchemaNameChars:"[^A-Za-z0-9_]",sAttributeSchemaNameFormat:"{0}Id",sRelationshipEditorUrlFormat:"/tools/systemcustomization/relationships/manageRelationship.aspx?entityRole={0}&entityRelationshipId={1}&entityId={2}",sInformationPageId:"divInformation",sMappingsPageId:"divMappings"},
    _bSaving = false,
    _sInputXml = "",
    _txtSchemaName,
    _selType;
...
function ManageRelationshipJsWindowOnLoad()
{
    ...
    try
    {
        if(_bManyToMany)
        {
            ...
            if(oCEDisplayOption != null && !oCEDisplayOption.get_disabled() && oCEDisplayOption.getElement().style.display != "none")
                oCEDisplayOption.setFocus();
            else
                if(oCEName != null && !oCEName.get_disabled() && oCEName.getElement().style.display != "none")
                    oCEName.setFocus();
                else
                    if(oOEDisplayOption != null && !oOEDisplayOption.get_disabled() && oOEDisplayOption.getElement().style.display != "none")
                        oOEDisplayOption.setFocus();
                    else
                        if(_txtSchemaName != null && !_txtSchemaName.get_disabled() && _txtSchemaName.get_element().style.display != "none")
                            _txtSchemaName.setFocus();
                        else
                            oIntersectEntityName != null && !oIntersectEntityName.get_disabled() && oIntersectEntityName.getElement().style.display != "none" && 
        ...
        else
        {
            var oReferencedEntityName = Mscrm.FormControlInputBehavior.GetBehavior("selReferencedEntityName"),
                ...
                oAttributeSchemaName = Mscrm.FormControlInputBehavior.GetBehavior("txtAttributeSchemaName");
            if(oReferencedEntityName != null && !oReferencedEntityName.get_disabled() && oReferencedEntityName.getElement().style.display != "none")
                oReferencedEntityName.setFocus();
            else
                if(oReferencingEntityName != null && !oReferencingEntityName.get_disabled() && oReferencingEntityName.getElement().style.display != "none")
                    oReferencingEntityName.setFocus();
                else
                    if(_txtSchemaName != null && !_txtSchemaName.get_disabled() && _txtSchemaName.getElement().style.display != "none")
                        _txtSchemaName.setFocus();
                    else
                        if(oReferencedInstanceName != null && !oReferencedInstanceName.get_disabled() && oReferencedInstanceName.getElement().style.display != "none")
                            oReferencedInstanceName.setFocus();
                        else
                            if(oAttributeSchemaName != null && !oAttributeSchemaName.get_disabled() && oAttributeSchemaName.getElement().style.display != "none")
                                oAttributeSchemaName.setFocus();
                            else
                                if(oReqLevel != null && !oReqLevel.get_disabled() && oReqLevel.getElement().style.display != "none")
                                    oReqLevel.setFocus();
                                else
                                    if(oDisplayOption != null && !oDisplayOption.get_disabled() && oDisplayOption.getElement().style.display != "none")
                                        oDisplayOption.setFocus();
                                    else
                                        _selType != null && !_selType.get_disabled() && _selType.getElement().style.display != "none" && 
                                            _selType.setFocus()
        }
    }
    catch(e)
...
function remoteRelationshipCommandCallback(oResult,oCallbackParams)
{
    if(oResult.Success)
    {
        if(oResult.RemoteCommand.Command == _oRelUtlConst.sCreateRelationshipCommand)
            ...
            if(oResult.RemoteCommand.Command == _oRelUtlConst.sDeleteRelationshipCommand)
        ...
        if(oCallbackParams.bReload)
        {
            ...
            frmReload.action = Mscrm.CrmUri.create(formatString(_oConst.sRelationshipEditorUrlFormat,_sEntityRole,_sEntityRelationshipId,_sEntityId)).toString();
            frmReload.submit();
            return false
        }
    }
...
function getRelationshipXml()
{
    ...
    var isSelectedreferencingEntityVE = "0";
    if(Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName").get_selectedOption() != null)
        isSelectedreferencingEntityVE = Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName").get_selectedOption().attributes["isdataprovidernotnull"].value;
    if(!IsNull(_dataProviderId) && _dataProviderId !== "" && _entityRole == "referencing" || isSelectedreferencingEntityVE == "1")
    {
        var externalName = $get("txtAttributeExternalName").value;
        addTextXmlNode(oDataXml,_oTags.externalname,externalName)
    }
    if(_bManyToMany)
    {
        ...
        addTextXmlNode(oDataXml,_oTags.currententityname,Mscrm.FormControlInputBehavior.GetBehavior("selCEName").get_dataValue());
        ...
        addTextXmlNode(oDataXml,_oTags.otherentityname,Mscrm.FormControlInputBehavior.GetBehavior("selOEName").get_dataValue());
    ...
    else
    {
        ...
        {
            addTextXmlNode(oDataXml,_oTags.relationshipbehavior,_selType.get_dataValue());
...
function validateParameters()
{
    ...
    else
    {
        ...
        if(Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName").get_selectedOption() != null)
            isSelectedreferencingEntityVE = Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName").get_selectedOption().attributes["isdataprovidernotnull"].value;
        if(!IsNull(_dataProviderId) && _dataProviderId !== "" && _entityRole == "referencing" || isSelectedreferencingEntityVE == "1")
            bValid = bValid && validateRequiredValue($get("txtAttributeExternalName"));
...
function confirmHierarchicalRelationship()
{
    ...
    if(_bManyToMany || (_sExistingHierarchicalRelationship == null || _sExistingHierarchicalRelationship.length == 0) || isControlDisabled)
        return true;
...
function onReferencedInstanceNameChange()
{
    ...
    if(_bCreate && (oReferencedInstanceName.get_dataValue() != null && oReferencedInstanceName.get_dataValue().length > 0) && (oAttributeSchemaName.get_dataValue() == null || oAttributeSchemaName.get_dataValue().length == 0))
    {
        var iNameMaxLen = oAttributeSchemaName.get_element().maxLength - formatString(_oConst.sAttributeSchemaNameFormat,"").length,
...
function onReferencingEntityChange()
{
    onEntityChange();
    var isSelectedvalueVE = Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName").get_selectedOption().attributes["isdataprovidernotnull"].value;
    if(isSelectedvalueVE == "1")
...
var _bSaveCustomCascadeParams = false,
    _oCustomCascadeParams = {sAssign:_oCascadeLinkTypes.sCascade,sShare:_oCascadeLinkTypes.sCascade,sUnshare:_oCascadeLinkTypes.sCascade,sReparent:_oCascadeLinkTypes.sCascade,sMerge:_oCascadeLinkTypes.sCascade,sDelete:_oCascadeLinkTypes.sCascade};
function onTypeChange()
{
    var oReferencedEntityName = Mscrm.FormControlInputBehavior.GetBehavior("selReferencedEntityName"),
        sReferencedEntityName = oReferencedEntityName.get_dataValue() == null ? _oConst.$none : oReferencedEntityName.get_dataValue(),
        oReferencingEntityName = Mscrm.FormControlInputBehavior.GetBehavior("selReferencingEntityName"),
        sReferencingEntityName = oReferencingEntityName.get_dataValue() == null ? _oConst.$none : oReferencingEntityName.get_dataValue(),
        isSelectedreferencingEntityVE = "0";
    if(oReferencingEntityName.get_selectedOption() != null)
        isSelectedreferencingEntityVE = oReferencingEntityName.get_selectedOption().attributes["isdataprovidernotnull"].value;
    var selCascadeAssign = Mscrm.FormControlInputBehavior.GetBehavior("selCascadeAssign"),
    ...
    switch(_selType.get_dataValue())
    {
        ...
        case _oCascadeTypes.sParentChild:
            ...
            selCascadeMerge.set_dataValue(typeof _oMergeableEntities[sReferencedEntityName] != "undefined" && isSelectedreferencingEntityVE == "0" ? _oCascadeLinkTypes.sCascade : _oCascadeLinkTypes.sNoCascade);
            _bCustom && 
        ...
        case _oCascadeTypes.sReferential:
            ...
            selCascadeMerge.set_dataValue(typeof _oMergeableEntities[sReferencedEntityName] != "undefined" && isSelectedreferencingEntityVE == "0" ? _oCascadeLinkTypes.sCascade : _oCascadeLinkTypes.sNoCascade);
            selCascadeDelete.set_dataValue(isSelectedreferencingEntityVE == "1" ? _oCascadeLinkTypes.sNoCascade : _oCascadeLinkTypes.sRemoveLink);
            break;
...
function setSelCascadeRollupDisableState(selCascadeRollupView,sReferencedEntityName,sReferencingEntityName)
{
    if(!IsNull(selCascadeRollupView))
        if(!_bManyToMany && _bCustom && _bMode != 2 && isValidRollupViewEntity(sReferencedEntityName) && !IsNull(_oRollupableEntities) && !IsNull(_oRollupableEntities[sReferencingEntityName]) && _oRollupableEntities[sReferencingEntityName] == true)
...
{
    return entityName == Mscrm.InternalUtilities.EntityNames.Account || entityName == Mscrm.InternalUtilities.EntityNames.Contact || entityName == Mscrm.InternalUtilities.EntityNames.Opportunity
}
...
{
    if(_bCreate && (sEntity1 != null && sEntity1.length > 0) && (sEntity2 != null && sEntity2.length > 0) && (_txtSchemaName.get_dataValue() == null || _txtSchemaName.get_dataValue().length == 0 || _txtSchemaName.get_dataValue() == _schemaNamePreviousValue))
    {
...
function createIntersectEntityName()
{
    ...
    if(_bCreate && (selCEName.get_dataValue() != null && selCEName.get_dataValue().length > 0) && (selOEName.get_dataValue() != null && selOEName.get_dataValue().length > 0) && (txtIntersectEntityName.get_dataValue() == null || txtIntersectEntityName.get_dataValue().length == 0 || txtIntersectEntityName.get_dataValue() == _intersectEntityNamePreviousValue))
    {
        txtIntersectEntityName.set_dataValue((selCEName.get_dataValue() + "_" + selOEName.get_dataValue()).substr(0,txtIntersectEntityName.get_element().maxLength));
