...
function ExternalPartyEnabledConfigOnloadHandler()
{
    isChanged = false;
    $addHandler(window,"resize",LayoutElements);
    attachWindowOnBeforeUnload(Close);
    attachWindowOnUnload(ExternalPartyEnabledConfigUnloadHandler);
...
Sys.Application.add_load(ExternalPartyEnabledConfigOnloadHandler);
function ExternalPartyEnabledConfigUnloadHandler()
{
    $removeHandler(window,"load",ExternalPartyEnabledConfigOnloadHandler);
    $removeHandler(window,"resize",LayoutElements);
    detachWindowOnUnload(ExternalPartyEnabledConfigUnloadHandler)
}
function LayoutElements()
{
    var formMenu = document.getElementById("ExternalPartyEnabledConfigMenu"),
        formHeader = document.getElementById("ExternalPartyEnabledConfigHeader"),
        formBody = document.getElementById("ExternalPartyEnabledConfigBody"),
        bodyPaddingPx = 5;
    formBody.style.top = formHeader.offsetHeight + formMenu.offsetHeight + "px";
    formBody.style.bottom = 0;
    var availableEntitiesCol = document.getElementById("ExternalPartyEnabledAvailableEntitiesCol"),
        buttonsCol = document.getElementById("ExternalPartyEnabledBodyButtonsCol"),
        selectedAttributesCol = document.getElementById("ExternalPartyEnabledSelectedEntitiesCol"),
        colBorderPx = 1,
        columnSpacingPx = 20,
    ...
    var buttonsPanel = document.getElementById("ExternalPartyEnabledConfigButtonList"),
...
function SelectionChanged()
{
    ...
    if(!IsNull(RemoveButton))
    {
        ...
        RemoveButton.disabled = SelectedRowsCount(selectedAttributesFirstChild) != 1 || _externalPartyEnabledEntityDisplayNames.indexOf(selectedRow.attributes.getNamedItem("name").value) > -1
    }
}
function SaveForm(closePage)
{
    ...
    else
    {
        isChanged = false;
        var retval = {};
        retval["selectedentities"] = GetList("SelectedEntities");
...
{
    if(IsNull(_oRow) || _externalPartyEnabledEntityDisplayNames.indexOf(_oRow.attributes.getNamedItem("name").value) > -1)
        return;
    var oList = GetList(GetListName());
    UnselectRows(oList);
    _oRowWithoutShift = _oRow;
