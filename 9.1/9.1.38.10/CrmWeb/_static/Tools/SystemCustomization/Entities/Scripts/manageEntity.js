...
function ManageEntityJsWindowOnLoad()
{
    ...
    _txtExternalCollectionName = Mscrm.FormControlInputBehavior.GetBehavior("txtExternalCollectionName");
...
function remoteEntityCommandCallback(oResult,oCallbackParams)
{
    if(oResult.Success || oResult.get_success && oResult.get_success())
    {
        ...
        if(!IsNull(oResult.RemoteCommand) && (oResult.RemoteCommand.Command == _oEntUtlConst.sCreateEntityCommand || oResult.RemoteCommand.Command == _oEntUtlConst.sUpdateEntityCommand))
        {
            SaveCustomControlsAndEventsForNonSafari(oResult,oCallbackParams);
            return false
        }
        else
            RefreshPageCallBack(oResult,oCallbackParams);
...
function RefreshPageCallBack(oResult,oCallbackParams)
{
...
function IsRequestFromAppDesigner()
{
    ...
    if(Mscrm.FeatureControl.get_Current().areFeaturesEnabled([Mscrm.FeatureNames.AppModuleForOrganization,Mscrm.FeatureNames.AppDesigner]))
        if(!IsNull(window.parent) && !IsNull(window.parent.opener))
            try
            {
                returnValue = !IsNull(window.parent.opener.AppDesignerCallback)
            }
            catch(e)
            {
            }
    ...
    if(IsRequestFromAppDesigner())
    {
        ...
        if(isNewEntity || isPublish)
        {
            var entityDisplayName = Mscrm.FormControlInputBehavior.GetBehavior($get("txtSingularName").id).get_dataValue(),
                ...
                entityExternalName = Mscrm.FormControlInputBehavior.GetBehavior($get("txtExternalName").id).get_dataValue(),
                entityExternalCollectionName = Mscrm.FormControlInputBehavior.GetBehavior($get("txtExternalCollectionName").id).get_dataValue();
            if(!isNullOrEmptyString(entityId))
                entityId = entityId.replace(/{|}/g,"").toLowerCase();
            var resultobj = {id:entityId,displayName:entityDisplayName,params:{isNewEntity:isNewEntity,pluralname:entityPluralName,logicalName:entityLogicalName,externalName:entityExternalName,externalCollectionName:entityExternalCollectionName}};
...
function saveEntityAction(bClose)
{
    if(!_bSaving && validateParameters())
    {
        ...
        if(_bCreate)
            ...
            {
                if(Mscrm.Utilities.isSafari())
                    SaveCustomControlsAndEventsForSafari();
                else
                    SaveCustomControlsAndEventsForNonSafari(null,oCallbackParams);
                if(bClose)
                {
                    CloseEntityEditor();
                    return
...
function SaveCustomControlsAndEventsForSafari()
{
    var _isCCFSaving = false;
    _isCCFSaving = createorUpdateCustomControlDefaultConfigXml(_isCCFSaving,null);
    !_isCCFSaving && scheduleUpdateEntityHomeGridEventsForSafari()
}
function createorUpdateCustomControlDefaultConfigXml(_isCCFSaving,oCallbackParams)
{
            ...
            if(!IsNull(entityCustomControlXmlDoc))
            {
                ...
                if(!IsNull(sCustomControlXml))
                {
                    ...
                    if(Mscrm.Utilities.isSafari())
                        ...
                        {
                            createOrupdateCustomControlDefaultConfig(sCustomControlXml,scheduleUpdateEntityHomeGridEventsForSafari)
                        },100);
                    else
                        createOrupdateCustomControlDefaultConfig(sCustomControlXml,scheduleUpdateEntityHomeGridEventsForNonSafari,oCallbackParams)
                }
            }
        }
    return _isCCFSaving
}
function scheduleUpdateEntityHomeGridEventsForSafari()
{
...
function scheduleUpdateEntityHomeGridEventsForNonSafari(oResult,oCallbackParams)
{
    if(Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.EditableGridControlJsEvents) && isEventsControlsTabDirty())
    {
        Mscrm.FormLibraryAndEventHandlerUtils.processLibraryXmlBeforeSave();
        var newFormXml = convertXmlDocToString(Mscrm.FormLibraryAndEventHandlerUtils.formXml);
        if(!IsNull(newFormXml))
            updateEntityHomeGridEvents(newFormXml,_sEntityId,RefreshPageCallBack,oCallbackParams);
        else
            RefreshPageCallBack(oResult,oCallbackParams)
    }
    else
...
function SaveCustomControlsAndEventsForNonSafari(oResult,oCallbackParams)
{
    var _isCCFSaving = false;
    _isCCFSaving = createorUpdateCustomControlDefaultConfigXml(_isCCFSaving,oCallbackParams);
    !_isCCFSaving && scheduleUpdateEntityHomeGridEventsForNonSafari(oResult,oCallbackParams)
}
...
function showComponentHistoryAction()
{
    Mscrm.SolutionComponentList.launchComponentHistoryDlg("Entity",_sEntityId)
}
...
function getEntityXml()
{
    var oDataXml = createXmlDoc(_oTags.entity),
        ...
        addForVirtualEntity = false;
    if($get("chkIsVirtualEntity") != null && $get("chkIsVirtualEntity").checked)
        addForVirtualEntity = true;
    ...
    addActivityXml(oDataXml,addForVirtualEntity);
    addRichClientXml(oDataXml,addEvenWhenDisabled,addForVirtualEntity);
    addMailMergeXml(oDataXml,addEvenWhenDisabled);
    addAccessTeamXml(oDataXml,addForVirtualEntity);
    addDuplicateDetectionXml(oDataXml);
    addAssociatedEntityXml(oDataXml,addEvenWhenDisabled);
    addConnectionsXml(oDataXml,true);
    addAuditXml(oDataXml,addForVirtualEntity);
    addChangeTrackingXml(oDataXml);
    addDocumentManagementXml(oDataXml);
    addKnowledgeManagementXml(oDataXml);
    addBusinessProcessXml(oDataXml,addForVirtualEntity);
    addCollaborationXml(oDataXml);
    addVisibleInMobileXml(oDataXml);
    addReadingPaneEnabledXml(oDataXml,addForVirtualEntity);
    addPrimaryAttributeXml(oDataXml);
    addQuickCreateXml(oDataXml,addForVirtualEntity);
    addSolutionAwareXml(oDataXml,addForVirtualEntity);
    addPrimaryImageXml(oDataXml);
    addEntityColorXml(oDataXml);
    addOneNoteIntegrationXml(oDataXml);
    addExternalChannelsXml(oDataXml);
    addSlaXml(oDataXml);
    addDataSourceXml(oDataXml,addForVirtualEntity);
    addVirtualEntityXml(oDataXml);
    addExternalNameXml(oDataXml);
    addExternalCollectionNameXml(oDataXml);
    addAttributeExternalNameXml(oDataXml);
...
function addSchemaXml(oDataXml)
{
    ...
    addControlDataValue(oDataXml,_oTags.externalname,$get("txtexternalName"),true);
    addControlDataValue(oDataXml,_oTags.externalcollectionname,$get("txtExternalCollectionName"),true);
...
function addAuditXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.auditenabled,$get("chkIsAuditingEnabled"),addForVirtualEntity);
    addControlDataValue(oDataXml,_oTags.retrieveauditenabled,$get("chkRetrieveAuditing"),addForVirtualEntity);
    addControlDataValue(oDataXml,_oTags.retrievemultipleauditenabled,$get("chkRetrieveMultipleAuditing"),addForVirtualEntity)
}
...
function addAccessTeamXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.autocreateaccessteams,$get("enableAutoCreateAccessTeams"),addForVirtualEntity)
}
function addBusinessProcessXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.businessprocessenabled,$get("chkIsBusinessProcessEnabled"),addForVirtualEntity)
}
function addVirtualEntityXml(oDataXml)
{
    addControlDataValue(oDataXml,_oTags.isvirtualentity,$get("chkIsVirtualEntity"),true)
}
function addExternalNameXml(oDataXml)
{
    addControlDataValue(oDataXml,_oTags.externalname,$get("txtExternalName"))
}
function addExternalCollectionNameXml(oDataXml)
{
    addControlDataValue(oDataXml,_oTags.externalcollectionname,$get("txtExternalCollectionName"))
}
function addAttributeExternalNameXml(oDataXml)
{
    addControlDataValue(oDataXml,_oTags.attributeexternalname,$get("txtAttributeExternalName"))
}
...
function addActivityXml(oDataXml,addForVirtualEntity)
{
    ...
    addForVirtualEntity && 
        addControlDataValue(oAcXml,_oTags.entityisactivity,checkIsActivity,true)
}
function addRichClientXml(oDataXml,addEvenWhenDisabled,addForVirtualEntity)
{
    ...
    addForVirtualEntity && 
        addControlDataValue(oRCXml,_oTags.offline,$get("chkRCOffline"),true)
}
...
function addReadingPaneEnabledXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.readingpaneenabled,$get("chkIsReadingPaneEnabled"),addForVirtualEntity)
}
function addQuickCreateXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.isquickcreateenabled,$get("chkIsQuickCreateEnabled"),addForVirtualEntity)
}
function addSolutionAwareXml(oDataXml,addForVirtualEntity)
{
    addControlDataValue(oDataXml,_oTags.issolutionawareenabled,$get("chkIsSolutionAwareEnabled"),addForVirtualEntity)
}
function addPrimaryAttributeXml(oDataXml)
{
    if(_bCreate)
    {
        ...
        addControlDataValue(oAttributeXml,_oTags.externalname,$get("txtAttributeExternalName"));
...
function addDataSourceXml(oDataXml,addForVirtualEntity)
{
    var _dataSourceType = $get("selDataSourceType");
    if(addForVirtualEntity !== false && !IsNull(_dataSourceType))
    {
        var _oDataSourceType = Mscrm.FormControlInputBehavior.GetBehavior("selDataSourceType").get_dataValue().split(",");
        if(!IsNull(_oDataSourceType))
        {
            addTextXmlNode(oDataXml,_oTags.datasourceid,_oDataSourceType[0]);
            addTextXmlNode(oDataXml,_oTags.dataproviderid,_oDataSourceType[1])
        }
    }
}
...
function validateParameters()
{
    ...
    if(_chkIsVirtualEntity != null && _chkIsVirtualEntity.checked)
    {
        bValid = bValid && validateRequiredValue($get("txtExternalName"));
        bValid = bValid && validateRequiredValue($get("txtExternalCollectionName"))
    }
    if($get("entityColor"))
...
function onSingularNameChange()
{
    var oSchemaName = Mscrm.FormControlInputBehavior.GetBehavior("txtSchemaName"),
        oExternalName = Mscrm.FormControlInputBehavior.GetBehavior("txtExternalName");
    ...
    var _chkIsVirtualEntity = $get("chkIsVirtualEntity");
    if(_chkIsVirtualEntity != null && _chkIsVirtualEntity.checked)
        if(_bCreate && (_txtSingularName.get_dataValue() != null && _txtSingularName.get_dataValue().length > 0) && (oExternalName.get_dataValue() == null || oExternalName.get_dataValue().length == 0))
        {
            var regExp = new RegExp(_oConst.sInvalidExternalNameChars,"g");
            oExternalName.set_dataValue(_txtSingularName.get_dataValue().replace(regExp,"").substr(0,oExternalName.get_element().maxLength));
            oExternalName.get_dataValue() != null && oExternalName.get_dataValue().length > 0 && 
                oExternalName.set_dataValue(oExternalName.get_dataValue().toLowerCase())
        }
}
...
function onAttributeSchemaNameChange()
{
    var oAttributeDisplayName = Mscrm.FormControlInputBehavior.GetBehavior("txtAttributeDisplayName"),
        ...
        oAttributeExternalName = Mscrm.FormControlInputBehavior.GetBehavior("txtAttributeExternalName");
    ...
    var _chkIsVirtualEntity = $get("chkIsVirtualEntity");
    if(_chkIsVirtualEntity != null && _chkIsVirtualEntity.checked)
        if(_bCreate && (oAttributeDisplayName.get_dataValue() != null && oAttributeDisplayName.get_dataValue().length > 0) && (oAttributeExternalName.get_dataValue() == null || oAttributeExternalName.get_dataValue().length == 0))
        {
            var regExp = new RegExp(_oConst.sInvalidExternalNameChars,"g");
            oAttributeExternalName.set_dataValue(oAttributeDisplayName.get_dataValue().replace(regExp,"").substr(0,oAttributeDisplayName.get_element().maxLength));
            oAttributeExternalName.set_dataValue(oAttributeExternalName.get_dataValue().toLowerCase())
        }
}
