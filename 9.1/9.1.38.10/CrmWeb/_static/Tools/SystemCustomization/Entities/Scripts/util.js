...
var _oEntUtlMessages = {sCannotUpdateSystemEntityIcons:LOCID_ENTUTL_CANTUPDSYSENTICN,sCannotDeleteSystemEntity:LOCID_ENTUTL_CANTDELSYSENT,sCannotPublishEntity:LOCID_ENTUTL_CANTPUBLISHENT,sCreatingEntity:LOCID_ENTUTL_CREATINGENT,sUpdatingEntity:LOCID_ENTUTL_UPDATINGENT,sDeletingEntity:LOCID_ENTUTL_DELETINGENT,sPublishingEntity:LOCID_ENTUTL_PUBLISHINGENT},
    _oEntUtlConst = {entityId:"entityId",sCustomizationCommandObject:"SystemCustomization",sCreateEntityCommand:"CreateEntity",sUpdateEntityCommand:"UpdateEntity",sCreateOrUpdateEntityConfigCommand:"CreateOrUpdateCustomControlDefaultConfig",sPublishCustomizationsCommand:"PublishCustomizations",sUpdateEntityHomeGridEvents:"UpdateEntityHomeGridEvents",sPublishAllCustomizationsCommand:"PublishAllCustomizations",sCommandParameterName:"data",sUpdateEntityIconsUrlFormat:"/tools/systemcustomization/entities/updateIcons.aspx?entityId={0}",iIconsDialogWidth:670,iIconsDialogHeight:550};
function createEntity(sDataXml,fCallback,oCallbackParams)
{
    var oCommand;
    if(Mscrm.Utilities.isSafari())
    {
        oCommand = createRemoteCommandXmlWithCallbackParams(_oEntUtlConst.sCreateEntityCommand,sDataXml,oCallbackParams);
...
function updateEntity(sDataXml,fCallback,oCallbackParams)
{
    ...
    if(Mscrm.Utilities.isSafari())
    {
        oCommand = createRemoteCommandXmlWithCallbackParams(_oEntUtlConst.sUpdateEntityCommand,sDataXml,oCallbackParams);
...
{
    var oCommand = new RemoteCommand(_oEntUtlConst.sCustomizationCommandObject,_oEntUtlConst.sCreateOrUpdateEntityConfigCommand);
...
{
    var oCommand = new RemoteCommand(_oEntUtlConst.sCustomizationCommandObject,_oEntUtlConst.sUpdateEntityHomeGridEvents);
...
function publishEntities(sDataXml,bPublishableEntity,fCallback,oCallbackParams)
{
    ...
    if(Mscrm.Utilities.isSafari())
    {
        oCommand = createRemoteCommandXmlWithCallbackParams(_oEntUtlConst.sPublishCustomizationsCommand,sDataXml,oCallbackParams);
        executeRemoteCommandXmlAsync(oCommand,_oEntUtlMessages.sPublishingEntity,fCallback,oCallbackParams)
    }
    else
    {
        oCommand = createRemoteCommandXml(_oEntUtlConst.sPublishCustomizationsCommand,sDataXml);
...
{
    var oCommand = new Mscrm.RemoteCommandXml(_oEntUtlConst.sCustomizationCommandObject,_oEntUtlConst.sPublishAllCustomizationsCommand);
    executeRemoteCommandXml(oCommand,_oEntUtlMessages.sPublishingEntity,fCallback,oCallbackParams)
}
