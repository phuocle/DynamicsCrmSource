...
var _oMessages = {sCloseAlert:LOCID_FORMS_SAVE_CONFIRM_TITLE,sConfirmEntityKeyDelete:LOCID_KEYSUTL_CONFIRMDELKEYS,sCreatingEntityKey:LOCID_KEYSUTL_CREATINGKEYS,sDeletingEntityKey:LOCID_KEYSUTL_DELETINGKEYS,sEntityKeyId:_sAlternateKeyId,sEntityId:_sEntityId},
    _oTypes = {sString:"nvarchar"},
    _oConst = {sInvalidSchemaNameChars:"[^A-Za-z0-9_]",maxSelectableColumns:_maxAttributesAllowedPerEntityKey,zeroSelectableColumns:0,sCustomizationCommandObject:"SystemCustomization",sCreateEntityKeyCommand:"CreateEntityKey",sDeleteEntityKeyCommand:"DeleteEntityKey",sCommandParameterName:"data"},
    _bSaving = false,
    _txtKeyDisplayName,
    _txtKeyName;
...
function remoteEntityKeyCommandCallback(oResult,oCallbackParams)
{
    if(oResult.Success)
    {
        if(oResult.RemoteCommand.Command == _oConst.sCreateEntityKeyCommand)
            ...
            if(oResult.RemoteCommand.Command == _oConst.sDeleteEntityKeyCommand);
        ...
        if(oCallbackParams.bClose)
        {
            ...
            window.location.href = String.format(Mscrm.AlternateKeysGrid.viewAlternateKeysSettingUrl,_oMessages.sEntityId,_sAlternateKeyId,true)
    }
    _bSaving = false;
    return true
}
...
function getEntityKeyXml()
{
    ...
    typeof selectExportKeyEnabled != "undefined" && 
        addControlDataValue(oDataXml,_oTags.isexportkey,$get("selectExportKeyEnabled"),true);
...
function GetKeyAttributes()
{
    for(var _oResult = {},
        ...
        rowId = 0; rowId < oRows.length; rowId++)
        alternateKeyAttributes = alternateKeyAttributes + (rowId == 0 ? "" : ";") + XUI.Html.DomUtils.GetFirstChild(oRows[rowId]).id;
...
{
    if(_bCreate)
        if(_txtKeyDisplayName.get_dataValue() != null && _txtKeyDisplayName.get_dataValue().length > 0 && (_txtKeyName.get_dataValue() == null || _txtKeyName.get_dataValue().length == 0))
        {
            var regExp = new RegExp(_oConst.sInvalidSchemaNameChars,"g");
            _txtKeyName.set_dataValue(_txtKeyDisplayName.get_dataValue().replace(regExp,"").substr(0,_txtKeyName.get_maxLength()))
        }
}
