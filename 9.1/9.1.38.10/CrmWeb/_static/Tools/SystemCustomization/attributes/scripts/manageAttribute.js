...
var _oMessages = {sCloseAlert:LOCID_FORMS_SAVE_CONFIRM_TITLE},
    ...
    _txtSchemaName,
    _txtExternalName,
    _selDataSourceSecret,
    _selType,
    _ledtStatusValues,
    _selectParentOptionSet,
    _selectOptionSet,
    _txtOptionSetExternalName,
    _ledtPicklistValues,
    _ledtPicklistValuesParent,
    _selectTarget,
    ...
    _bDependentOptionsetFCB = false,
    _bMultipleImagesFCB = false;
...
function ManageAttributeJsWindowOnLoad()
{
    ...
    _txtExternalName = Mscrm.FormControlInputBehavior.GetBehavior("txtExternalName");
    _txtOptionSetExternalName = Mscrm.FormControlInputBehavior.GetBehavior("txtOptionSetExternalName");
    _selDataSourceSecret = Mscrm.FormControlInputBehavior.GetBehavior("selectDataSourceSecret");
    _selType = Mscrm.FormControlInputBehavior.GetBehavior("selType");
    _ledtStatusValues = $find("ledtStatusValues");
    _selectParentOptionSet = Mscrm.FormControlInputBehavior.GetBehavior("selectParentOptionSet");
    _selectOptionSet = Mscrm.FormControlInputBehavior.GetBehavior("selectOptionSet");
    _ledtPicklistValues = $find("ledtPicklistValues");
    _ledtPicklistValuesParent = $find("ledtPicklistValuesParent");
    _ledtPicklistValuesChild = $find("ledtPicklistValuesChild");
    ...
    _bDependentOptionsetFCB = Mscrm.FeatureControl.get_Current().isFeatureEnabled(_oConst.sFCBDependentOptionSet);
    _bMultipleImagesFCB = Mscrm.FeatureControl.get_Current().isFeatureEnabled(_oConst.sFCBMultipleImages);
    loadStates();
    initControls();
    ...
    $addHandler($get("rdIsFieldSecurityEnabled"),"change",onFieldSecurityChangeToHandleTabbing);
    ...
    _bDependentOptionsetFCB && 
        initCascadingOptionsetControls()
}
function initCascadingOptionsetControls()
{
    tabControl = $find("crmTabBar");
    !IsNull(tabControl) && 
        tabControl.add_change(onTabSwitched);
    !IsNull(_ledtPicklistValuesParent) && !IsNull(_ledtPicklistValuesChild) && 
        _ledtPicklistValuesChild.bindToParentValueControl(_ledtPicklistValuesParent)
}
...
function addSchemaXml(oDataXml)
{
    ...
    if(!!_txtExternalName)
        if(_txtExternalName.get_dataValue())
            addControlDataValue(oDataXml,_oTags.externalname,_txtExternalName.get_element());
        else
            addExternalNameEmptyValue(oDataXml,_oTags.externalname);
    _selDataSourceSecret != null && 
        addControlDataValue(oDataXml,_oTags.isdatasourcesecret,$get("selectDataSourceSecret"));
    ...
    typeof selectLocalizableEnabled != "undefined" && 
        addControlDataValue(oDataXml,_oTags.islocalizable,$get("selectLocalizableEnabled"),bAddEvenIfDisabled());
    typeof selectExportableEnabled != "undefined" && 
        addControlDataValue(oDataXml,_oTags.isexportable,$get("selectExportableEnabled"),bAddEvenIfDisabled());
    addControlDataValue(oDataXml,_oTags.searchable,$get("selectSearchable"),bAddEvenIfDisabled())
}
function addExternalNameEmptyValue(oParentNode,sChildNodeName)
{
    var oChildNode = null;
    oChildNode = addXmlNode(oParentNode,sChildNodeName);
    XUI.Xml.SetText(oChildNode,"");
    return oChildNode
}
function addTypeXml(oDataXml)
{
    ...
    switch(_selType.get_dataValue())
    {
        ...
        case _oTypes.sMemo:
            addMemoXml(oTypeXml);
            break;
        case _oTypes.sMultiSelectPicklist:
            addPicklistXml(oTypeXml);
            break
    }
}
...
{
    addControlDataValue(oTypeXml,_oTags.sourcetype,$get("selDataSource"),true);
    if(_bDependentOptionsetFCB)
        if(!isNullOrEmptyString(_selectParentOptionSet.get_dataValue()))
            addTextXmlNode(oTypeXml,_oTags.parentenumattributeid,_selectParentOptionSet.get_dataValue().split("|")[0]);
        else
        {
            _ledtPicklistValuesChild.resetParentValues();
            !_bCreate && 
                addTextXmlNode(oTypeXml,_oTags.parentenumattributeid,"")
        }
    !_ledtPicklistValues.get_disabled() && 
        addXmlString(oTypeXml,_ledtPicklistValues.get_xmlData());
    !!_txtOptionSetExternalName && 
        addControlDataValue(oTypeXml,_oTags.optionsetexternalname,_txtOptionSetExternalName.get_element());
...
function addLookupXml(oTypeXml)
{
    if(!_selectTarget.get_element().disabled && _selectTarget.get_element().selectedIndex >= 0)
    {
        ...
        if(!!_txtExternalName)
            if(_txtExternalName.get_dataValue())
                addControlDataValue(oRelationshipXml,_oTags.externalname,_txtExternalName.get_element());
            else
                addExternalNameEmptyValue(oRelationshipXml,_oTags.externalname);
        ...
        if(_isVirtualEntity)
            addTextXmlNode(oCascadingXml,_oTags.Delete,_sNoCascade);
        else
...
function onDisplayNameChange()
{
    if(_bCreate)
    {
        ...
        if(_isVirtualEntity && !!_txtExternalName)
            if(_txtDisplayName.get_dataValue() != null && _txtDisplayName.get_dataValue().length > 0 && (_txtExternalName != null && (_txtExternalName.get_dataValue() == null || _txtExternalName.get_dataValue().length == 0)))
            {
                var regExp = new RegExp(_oConst.sInvalidSchemaNameChars,"g");
                _txtExternalName.set_dataValue(_txtDisplayName.get_dataValue().replace(regExp,"").substr(0,_txtExternalName.get_maxLength()))
            }
        if(!!_txtExternalName && _txtDisplayName.get_dataValue() != null && _txtDisplayName.get_dataValue().length > 0 && (_txtExternalName.get_dataValue() == null || _txtExternalName.get_dataValue().length == 0))
        {
            var regExp = new RegExp(_oConst.sInvalidSchemaNameChars,"g");
            _txtExternalName.set_dataValue(_txtDisplayName.get_dataValue().replace(regExp,"").substr(0,_txtExternalName.get_maxLength()))
        }
...
function onFieldSecurityChangeToHandleTabbing()
{
    if($get("rad_rdIsFieldSecurityEnabled1").checked)
    {
        $($get("rad_rdIsFieldSecurityEnabled1")).attr("tabindex","0");
        $($get("rad_rdIsFieldSecurityEnabled2")).attr("tabindex","-1")
    }
    else
    {
        $($get("rad_rdIsFieldSecurityEnabled1")).attr("tabindex","-1");
        $($get("rad_rdIsFieldSecurityEnabled2")).attr("tabindex","0")
    }
}
...
function onTypeChange()
{
    var selDataValue = _selType.get_dataValue(),
        _selDataSource = Mscrm.FormControlInputBehavior.GetBehavior("selDataSource");
    setDependencyTabVisibility(selDataValue,_selDataSource.get_dataValue());
    if(selDataValue == _oTypes.sBoolean || selDataValue == _oTypes.sPicklist || selDataValue == _oTypes.sLookup || selDataValue == _oTypes.sOwner || selDataValue == _oTypes.sCustomer || selDataValue == _oTypes.sPrimaryKey || selDataValue == _oTypes.sStatus || selDataValue == _oTypes.sImage || selDataValue == _oTypes.sMultiSelectPicklist || selDataValue == _oTypes.sFile)
    {
    ...
    if(Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CalculatedFields) || Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.Rollups))
        if(selDataValue == _oTypes.sMemo || selDataValue == _oTypes.sFloat || selDataValue == _oTypes.sLookup || selDataValue == _oTypes.sOwner || selDataValue == _oTypes.sCustomer || selDataValue == _oTypes.sPrimaryKey || selDataValue == _oTypes.sStatus || selDataValue == _oTypes.sImage || selDataValue == _oTypes.sFile)
        {
    ...
    if(selDataValue == _oTypes.sMultiSelectPicklist)
    {
        $get("picklistDefault_control").style.display = "none";
        _oVisibleTypeParams = $get(_oTypes.sPicklist + "Params")
    }
    if(!IsNull(_oVisibleTypeParams))
    ...
    if(selDataValue == _oTypes.sImage)
    {
        ...
        if(!IsNull(txtSchemaName) && !_bMultipleImagesFCB)
        {
    ...
    {
        picklist_section.style.display = (selDataValue == _oTypes.sPicklist || selDataValue == _oTypes.sMultiSelectPicklist) && Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 0 ? "" : "none";
        picklist_control.style.display = (selDataValue == _oTypes.sPicklist || selDataValue == _oTypes.sMultiSelectPicklist) && Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 0 ? "" : "none";
        if(selDataValue == _oTypes.sPicklist || selDataValue == _oTypes.sMultiSelectPicklist)
    ...
    if(selDataValue == _oTypes.sString || selDataValue == _oTypes.sMemo || selDataValue == _oTypes.sImage || selDataValue == _oTypes.sFile)
...
function onDataSourceChange()
{
    ...
    else
    {
        ...
        if(selDataValue == _oTypes.sImage || selDataValue == _oTypes.sFile)
            selReqLevel.disabled = true;
        else
            selReqLevel.disabled = false;
...
function filterDataSourceOnTypeChange()
{
    ...
    if(!isNull(_selDataSource))
    {
        ...
        {
            !_selType.get_disabled() && _selDataSource.get_disabled() && 
                _selDataSource.set_disabled(false);
            if(selDataValue == _oTypes.sInteger || selDataValue == _oTypes.sDecimal || selDataValue == _oTypes.sMoney || selDataValue == _oTypes.sDateTime)
                ...
                else
                {
                    ...
                    selDataValue == _oTypes.sMultiSelectPicklist && 
                        _selDataSource.set_disabled(true)
                }
            if(!isNull(prevSelectedDataSource))
                for(var availableDataSources = _selDataSource.get_options(),
                    i = 0; i < availableDataSources.length; ++i)
                    if(availableDataSources[i].DataValue == prevSelectedDataSource)
                    {
...
function OnUseExisitingChanged()
{
    var picklist_section = document.getElementById("picklistParams_section"),
        ...
        tr_OptionSetExternalName = document.getElementById("tr_OptionSetExternalName");
    if(_bDependentOptionsetFCB)
    {
        _ledtPicklistValuesParent.resetControl();
        _ledtPicklistValuesParent.set_xmlData(GetOptionSetDataXml(""))
    }
    if(Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 1)
    {
        ...
        if(tr_OptionSetExternalName)
            tr_OptionSetExternalName.style.display = "none";
        OnOptionSetChange()
    }
    else
    {
        ...
        if(tr_OptionSetExternalName)
            tr_OptionSetExternalName.style.display = "";
        OnOptionSetChange()
    }
}
...
function RefreshParentPickListSet(ret)
{
    var i,
        selectParentOptionSet = $get("selectParentOptionSet");
    while(selectParentOptionSet.options.length > 0)
        selectParentOptionSet.options.remove(0);
    if(ret.indexOf("SELECTED") > 0 && ret.indexOf("SELECTED=") < 0 && ret.indexOf("SELECTED =") < 0)
        ret = ret.replace("SELECTED","");
    var oXml = XUI.Xml.LoadXml("<select>" + ret + "</select>"),
        path = "/select/option",
    ...
    {
        var oOption = document.createElement("OPTION");
        _selectParentOptionSet.get_element().options.add(oOption);
        XUI.Html.SetText(oOption,XUI.Xml.GetText(oNodes[i]));
        oOption.value = oNodes[i].getAttribute("value")
    }
}
function OnOptionSetChange()
{
    if(Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() == 1)
    {
        if(!IsNull(_selectOptionSet.get_dataValue()) && _selectOptionSet.get_dataValue().length > 0)
        {
            var optionSetId = _selectOptionSet.get_dataValue(),
                retVal = GetOptionSetDataXml(optionSetId);
            if(retVal != null)
            {
            ...
            ReloadParentPicklists(_sEntityId,optionSetId,_sAttributeId)
        }
        else
            if(!IsNull(_selectParentOptionSet))
                _selectParentOptionSet.get_element().disabled = true
    }
    else
    {
        ...
        ReloadDefault();
        ReloadParentPicklists(_sEntityId,"",_sAttributeId)
    }
}
function ReloadParentPicklists(entityId,optionsetId,attributeId)
{
    if(_bDependentOptionsetFCB)
    {
        _selectParentOptionSet.get_element().disabled = false;
        if(!(isNull(attributeId) && Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() != 1))
        {
            var ret = GetParentPicklistAttributeList(entityId,optionsetId,attributeId);
            if(!isNullOrEmptyString(ret))
            {
                RefreshParentPickListSet(ret);
                setDefaultValueControlVisibility()
            }
        }
    }
}
function OnParentOptionSetChange()
{
    var selectedVal = _selectParentOptionSet.get_dataValue();
    setDefaultValueControlVisibility(selectedVal);
    if(!isNullOrEmptyString(selectedVal))
        var retVal = GetOptionSetDataXml(selectedVal.split("|")[1],true);
    else
        var retVal = GetOptionSetDataXml(selectedVal);
    if(Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue() != 1)
    {
        _ledtPicklistValuesParent.resetControl();
        _ledtPicklistValuesChild.resetParentValues()
    }
    _ledtPicklistValuesParent.set_xmlData(retVal)
}
function onTabSwitched(sender,eventArg)
{
    var tabId = eventArg.get_tab().id;
    if(tabId == "tabCascadingTab")
    {
        var values = _ledtPicklistValues.get_values();
        _ledtPicklistValuesChild.set_isReadOnlyMode(Mscrm.FormControlInputBehavior.GetBehavior("rdUseExisting").get_dataValue());
        _ledtPicklistValuesChild.set_values(values)
    }
}
...
function setDependencyTabVisibility(selDataValue,selDataSource)
{
    if(_bDependentOptionsetFCB)
    {
        if(selDataValue == _oTypes.sPicklist || selDataValue == _oTypes.sMultiSelectPicklist)
        {
            resetParentOptionSet();
            if(isNull(selDataSource) || selDataSource != _oDataSources.sCalculated)
            {
                $get("tabCascadingTab").style.display = "";
                OnParentOptionSetChange();
                return
            }
        }
        $get("tabCascadingTab").style.display = "none"
    }
}
function setDefaultValueControlVisibility(selectedParentVal)
{
    if(_bDependentOptionsetFCB)
    {
        var defaultControl = Mscrm.FormControlInputBehavior.GetBehavior("ledtPicklistDefault");
        if(!isNull(selectedParentVal))
        {
            if(defaultControl.get_element().selectedIndex != 0)
                defaultControl.get_element().selectedIndex = 0;
            defaultControl.set_disabled(true)
        }
        else
            defaultControl.set_disabled(false)
    }
}
function resetParentOptionSet()
{
    if(!isNull(_selectParentOptionSet.get_dataValue()) && $get("tabCascadingTab").style.display == "none")
    {
        _selectParentOptionSet.set_dataValue(null);
        OnParentOptionSetChange()
    }
}
