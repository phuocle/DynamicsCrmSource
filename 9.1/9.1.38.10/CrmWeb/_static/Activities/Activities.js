...
function OnActivityTypeChange(oSelect)
{
    ...
    if(!IsNull(vizPaneControl))
        if(vizPaneControl.isVisualizationPaneVisible() && vizPaneControl.isPaneInDesignerMode() && vizPaneControl.isDesignerPaneDirty())
        {
    ...
    if(oResult.Success == ERROR_NONE)
    {
        ...
        if(newVSControl != null && newVSControl.showNewVSControl == true)
        {
            ...
            if(!IsNull(retValues) && retValues.length > 0 && !IsNull(retValues[0]))
                !IsNull(retValues[0]["viewid"]) && !IsNull(retValues[0]["viewtype"]) && 
...
function ChangeView(val,promptIfChartDirty)
{
    ...
    {
        newVSControl != null && newVSControl.showNewVSControl == true && 
...
function updateVisualizationList(crmGrid,entityName)
{
    ...
    !IsNull(compositeControl) && compositeControl.isChartEnabled() && 
...
function GetStickyVisualizationId(crmGrid,entityName)
{
    ...
    if(!IsNull(retValues) && retValues.length > 0 && !IsNull(retValues[0]))
        if(!isNullOrEmptyString(retValues[0]["visualizationId"]) && !isNullOrEmptyString(retValues[0]["visualizationType"]))
