...
function CustomFormSubmit(sender,args)
{
    ...
    if(IsNull(args.get_isCalledFromSubmitCrmForm()) && (entityType == "appointment" || entityType == "serviceappointment"))
        return;
    ...
    if(bUseEngine)
        if(mode == 1 || mode == 2 || mode == 59)
        {
            ...
            if(iStatus == 1)
                    ...
                    if(oResult.Success)
                        if(oResult.ReturnValue.OperationSuccess)
                        {
                            ...
                            else
                            {
                                ...
                                if(typeof _redirectNewUrlOnUpdate != "undefined" && !IsNull(_redirectNewUrlOnUpdate) && _redirectNewUrlOnUpdate)
                                {
                                    url.clearQuery();
                                    url.get_query()["id"] = oResult.ReturnValue.Id
                                }
                        ...
                        else
                        {
                            ...
                            if(!isArray(notification))
                                ...
                                {
                                    var oaNotifications = new Array(crmNotifications.CreateNotification("guid",1,"strategy",LOCID_GENERIC_SERVER_ERROR));
                                    crmNotifications.SetNotifications(oaNotifications,"strategy")
                                }
                                else
                                {
                                    arguments = notification;
                                    oNotification = crmNotifications.CreateNotification(arguments.Id,arguments.Severity,arguments.Source,arguments.Text);
                            ...
                            callbackReference = Mscrm.Utilities.createCallbackFunctionObject("callBackFunction",this,parameters,false);
                ...
                {
                    var oaNotifications = new Array(crmNotifications.CreateNotification("Cannot_Validate_Activity",1,"PrivilegeCheck",LOCID_CANNOT_VALIDATE_APPT));
...
function ShowNotificationDialog(callbackReference)
{
    ...
    {
        var oUrl = Mscrm.CrmUri.create("/SM/ActivityScheduling/SchedulingNotificationsDialog.aspx?activityType=" + _iActivityTypeCode);
...
function performActionAfterSND(sResult,callbackReference,isPreventDefault)
{
    switch(sResult)
    {
        ...
        case "Schedule":
            isPreventDefault = false;
            ScheduleActivity($get("crmFormSubmit").crmFormSubmitId.value,_iActivityTypeCode,callbackReference,isPreventDefault);
            break
    }
    return isPreventDefault
}
...
function SeveritySetNotifications()
{
    for(var crmNotifications = $find("crmNotifications"),
        ...
        i = 0; i < aNotifications.length; i++)
    {
        var aNote = aNotifications[i];
        (aNote.Severity == Mscrm.Severity.SEVERITYERROR || aNote.Severity == Mscrm.Severity.SEVERITYWARNING || aNote.Severity == Mscrm.Severity.SEVERITYINFORMATION) && 
...
function RefreshRelatedGrids(activityTypeCode,entityId)
{
    if(isOutlookHostedWindow())
        ...
        if(typeof customGridRefreshHandler == "function")
