    ...
    if(!IsNull(oForm))
    {
        ...
        if(!IsNull(crmFormSubmitId) && !IsNull(crmFormSubmitObjectType))
        {
            ...
            if(!IsNull(crmFormSubmitId.getAttribute("value")))
                sParams = sParams + "&_CreateFromId=" + CrmEncodeDecode.CrmUrlEncode(crmFormSubmitId.getAttribute("value"))
        }
    }
    return sParams
}
function getNotSelected(sGridName)
{
    ...
    for(var o = innerGrid.get_dataTableBody(),
        ...
        i = 0; i < iTotal; i++)
    {
        var rowSelected = o.rows[i].getAttribute("selected"),
            bResult = rowSelected == "true";
        if(!bResult)
        {
...
function doAction(sGrid,iObjType,sAction,sObjId)
{
    ...
    if(gridControl)
    {
        ...
        switch(sAction)
        {
            ...
            case "syncBulkOperationPause":
                return Mscrm.GridRibbonActions.syncBulkOperations(gridControl,0);
            case "syncBulkOperationResume":
                return Mscrm.GridRibbonActions.syncBulkOperations(gridControl,1);
            case "syncBulkOperationCancel":
                return Mscrm.GridRibbonActions.syncBulkOperations(gridControl,2);
            case "connect":
            ...
            case "deleteaudithistory":
                return Mscrm.GridCommandActions.deleteAuditHistory(gridControl,selectedRecords,etc);
            ...
            case "editreport":
                return Mscrm.ReportActions.editReport(gridControl,selectedRecords,etc);
            ...
            case "queueitempick":
                IsNull(Mscrm.QueueItemGridCommandActions) && 
                    Mscrm.CrmHeader.setScriptFile(Mscrm.CrmUri.create("$webresource:QueueItem_main_system_library.js"),true);
        ...
        if(sAction == "resetdatafilters")
        {
            ...
            if(sAction == "mergeduplicates")
            {
                ...
                else
                {
                    sIds = baseRecord[0];
                    sIds += ";" + a[0];
                    var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_merge.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),"",iX,iY,null);
                    oResult = crmDialog.show()
                }
            }
            else
                if(sAction == "automergeduplicates")
                {
                    for(var c = $get("baseRecordsIframe",parent.document),
                        doc = c.contentWindow.document || c.contentDocument,
                        b = doc.getElementById("baseRecordsGrid").control.get_innerGrid().get_selectedRecords(),
                    ...
                    if(sAction == "showvisualization")
                    {
                        ...
                        if(sAction == "publishduplicaterule")
                        {
                            var oArgs = {};
                            ...
                            if(sAction == "modifyrecurrence")
                            {
                                ...
                                else
                                {
                                    ...
                                    var oUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),
                                        parameters = [iObjType],
                                        modifyRecurrenceCallBackObject = Mscrm.Utilities.createCallbackFunctionForXrmDialog(actionRefreshCallBack,parameters);
                                    Xrm.Internal.openDialog(oUrl.toString(),dialogOptions,a,null,modifyRecurrenceCallBackObject);
                                    return
                                }
                            }
                            else
                                if(sAction == "uploadimportmap")
                                {
                                    ...
                                    if(sAction == "deleteimportedrecords" || sAction == "deleteallimportedrecords")
                                    {
                                        var aIds = [];
                                        aIds.type = "guid";
                                        for(var i = 0; i < a.length; i++)
                                        ...
                                        if(sAction == "unpublishduplicaterule")
                                        {
                                            var oArgs = {};
                                            oArgs.Ids = getSelected(sGrid);
                                            var oUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx"),
                                            ...
                                            if(sAction == "publishmailmergetemplate" || sAction == "unpublishmailmergetemplate")
                                            {
                                                var oArgs = {};
                                                oArgs.Ids = getSelected(sGrid);
                                                ...
                                                if(sAction == "copyto")
                                                {
                                                    ...
                                                    LookupObjectsWithCallback(callbackRef,null,"single","4300",0,null,null,"1","1","0","","","","577EA96E-B1F6-499b-98A7-ABB5BE8233F9")
                                                }
                                                else
                                                    if(sAction == "copylistmember")
                                                    {
                                                        ...
                                                        var sMemberType = "",
                                                            windowContext = this;
                                                        Xrm.Internal.messages.retrieve(Mscrm.InternalUtilities.EntityNames.List,Xrm.Page.data.entity.getId(),["createdfromcode"]).then(function(response)
                                                        {
                                                            if(!Mscrm.InternalUtilities.JSTypes.isNull(response))
                                                            {
                                                                ...
                                                                if(Mscrm.ListAction._entityDoc.hasValue("createdfromcode"))
                                                                    sMemberType = parseInt(Mscrm.ListAction._entityDoc.getValue("createdfromcode").getValue("value").toString())
                                                            }
                                                            var parameters = [iObjType,a,sIds,iX,iY,sCustParams],
                                                                callbackRef = Mscrm.Utilities.createCallbackFunctionObject("copyToListAction",windowContext,parameters,false);
                                                            LookupObjectsWithCallback(callbackRef,null,"single","4300",0,null,"currentid=" + CrmEncodeDecode.CrmUrlEncode(_sListId) + "&membertypecode=" + CrmEncodeDecode.CrmUrlEncode(sMemberType) + "&listType=" + CrmEncodeDecode.CrmUrlEncode("static"),"1","1","0","","","","577EA96E-B1F6-499b-98A7-ABB5BE8233F9")
                                                        ...
                                                        if(sAction == "addtolist")
                                                        {
                                                            var parameters = [iObjType,a,sIds,iX,iY,sCustParams],
                                                                callbackRef = Mscrm.Utilities.createCallbackFunctionObject("addToListAction",this,parameters,false);
                                                            LookupObjectsWithCallback(callbackRef,null,"single","4300",0,null,"membertypecode=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&listType=" + CrmEncodeDecode.CrmUrlEncode("static"),1,1,false,"","","","577EA96E-B1F6-499b-98A7-ABB5BE8233F9",null,null,null,null)
                                                        }
                                                        else
                                                            if(sAction == "addtocampaign")
                                                            {
                                                                var parameters = [iObjType,a,sIds,iX,iY,sCustParams],
                                                                    callbackRef = Mscrm.Utilities.createCallbackFunctionObject("addToCampaignAction",this,parameters,false);
                                                                LookupObjectsWithCallback(callbackRef,null,"single","4400","0")
                                                            }
                                                            else
                                                                if(sAction == "emailviamailmergeoff")
                                                                    alert(LOCID_PROPAGATE_MM_EXE_OFF);
                                                                else
                                                                    if(sAction == "letterviamailmerge" || sAction == "faxviamailmerge" || sAction == "emailviamailmerge")
                                                                    {
                                                                            ...
                                                                            if(iObjType == CampaignActivity && Xrm.Page.data.entity.getIsDirty())
                                                                                ...
                                                                                if(a.length != 0)
                                                                                {
                                                                                    ...
                                                                                    if(sAction == "faxviamailmerge")
                                                                                        ...
                                                                                        if(sAction == "emailviamailmerge")
                                                                                            mergetype = 4;
                                                                                    var dialogOptions = new Xrm.DialogOptions;
                                                                                    dialogOptions.height = 500;
                                                                                    dialogOptions.width = 600;
                                                                                    Xrm.Internal.openDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_webmailmerge.aspx?objectTypeCode=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&objectId=" + CrmEncodeDecode.CrmUrlEncode(a[0]) + "&mergetype=" + CrmEncodeDecode.CrmUrlEncode(mergetype)).toString(),dialogOptions,null,null,null)
                                                                                }
                                                                    }
                                                                    else
                                                                        if(sAction == "listappointment" || sAction == "listletter" || sAction == "listfax" || sAction == "listphone" || sAction == "listemail" || sAction == "createopportunity")
                                                                        {
                                                                            sIds = a[0];
                                                                            if(iObjType == "4402" && statusCode != "1")
                                                                                alert(LOCID_PROPAGATE_ERROR);
                                                                            ...
                                                                            if(sAction == "qualifylist")
                                                                            {
                                                                                ...
                                                                                if(sAction == "changeTaskstate")
                                                                                {
                                                                                    sIds = a;
                                                                                    var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_deactivate.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + "&bulkChangeStateMode=1" + sCustParams),a,iX,iY,null);
                                                                                    oResult = crmDialog.show()
                                                                                }
                                                                                else
                                                                                    if(sAction == "reactivateCR")
                                                                                    {
                                                                                        var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_activate.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + "&bulkChangeStateMode=1" + sCustParams),a,iX,iY,null);
                                                                                        oResult = crmDialog.show()
                                                                                    }
                                                                                    else
                                                                                        if(sAction == "updatecustomstring")
                                                                                        {
                                                                                            if(a.length > 1)
                                                                                            {
                                                                                            ...
                                                                                            if(sAction == "applyrule")
                                                                                            {
                                                                                                sRet = openStdDlg(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),a,iX,iY);
                                                                                                oResult = sRet == null;
                                                                                                if(!oResult)
                                                                                                {
                                                                                                ...
                                                                                                if(sAction == "nochannel")
                                                                                                {
                                                                                                    ...
                                                                                                    if(sAction == "invalidchannel")
                                                                                                    {
                                                                                                        ...
                                                                                                        if(sAction == "invalidmailmergechannel")
                                                                                                        {
                                                                                                            if(iObjType == CampaignActivity && Xrm.Page.data.entity.getIsDirty())
                                                                                                            {
                                                                                                                alert(LOCID_PROPAGATE_DIRTY);
                                                                                                            ...
                                                                                                            if(sAction == "nochannelfield")
                                                                                                            {
                                                                                                                ...
                                                                                                                if(sAction == "nolistassociated")
                                                                                                                {
                                                                                                                    ...
                                                                                                                    if(sAction == "inviteusernotavailable")
                                                                                                                    {
                                                                                                                        ...
                                                                                                                        if(sAction == "addusers")
                                                                                                                        {
                                                                                                                            ...
                                                                                                                            Xrm.Internal.openDialog(Mscrm.CrmUri.create("/WebWizard/WizardContainer.aspx?WizardId=2631659F-A668-4A48-833B-D20E187B5A89").toString(),dialogOptions,null,null,AddAutoFunction(iObjType))
                                                                                                                        }
                                                                                                                        else
                                                                                                                            if(sAction == "setstate_bulkdelete")
                                                                                                                            {
                                                                                                                                ...
                                                                                                                                if(result.ReturnValue.string != null)
                                                                                                                                {
                                                                                                                                    ...
                                                                                                                                    {
                                                                                                                                        var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),aId,450,250,null);
                                                                                                                                        crmDialog.show();
                                                                                                                                        isOutlookHostedWindow() && 
                                                                                                                                            $find("crmGrid").Refresh()
                                                                                                                                    }
                                                                                                                                    else
                                                                                                                                        oResult = openStdDlg(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),aId,iX,iY)
                                                                                                                                }
                                                                                                                            }
                                                                                                                            else
                                                                                                                                if(sAction == "enabledisablefilters")
                                                                                                                                {
                                                                                                                                    ...
                                                                                                                                    if(sAction == "savefilterstocurrentview")
                                                                                                                                    {
                                                                                                                                        var filterSet = $find(sGrid + "_filterSet");
                                                                                                                                        ...
                                                                                                                                        if(sAction == "setstate_asyncoperation")
                                                                                                                                        {
                                                                                                                                            var crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams),a,450,250,null);
...
{
    oResult && Mscrm.Utilities.isModalDialogSupported() && 
        auto(iObjType)
}
...
function addToListAction(lookupItems,iObjType,a,sIds,iX,iY,sCustParams)
{
    ...
    if(lookupItems)
        if(lookupItems.items.length > 0)
        {
            var itemObjectId = lookupItems.items[0].id,
                itemObjectTypeCode = lookupItems.items[0].type,
                crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_addtolist.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&autoTrigger=1&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams + "&itemObjectId=" + CrmEncodeDecode.CrmUrlEncode(itemObjectId) + "&itemObjectTypeCode=" + CrmEncodeDecode.CrmUrlEncode(itemObjectTypeCode)),a,iX,iY,null);
            return crmDialog.show()
        }
    return oResult
}
function copyToAction(lookupItems,iObjType,a,sIds,iX,iY,sCustParams)
{
    ...
    if(lookupItems)
        if(lookupItems.items.length > 0)
        {
            var itemObjectId = lookupItems.items[0].id,
                itemObjectTypeCode = lookupItems.items[0].type;
            oResult = openStdDlg(Mscrm.CrmUri.create("/_grid/cmds/dlg_copyto.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams + "&itemObjectId=" + CrmEncodeDecode.CrmUrlEncode(itemObjectId) + "&itemObjectTypeCode=" + CrmEncodeDecode.CrmUrlEncode(itemObjectTypeCode)),a,iX,iY)
        }
    return oResult
}
function addToCampaignAction(lookupItems,iObjType,a,sIds,iX,iY,sCustParams)
{
    ...
    if(lookupItems)
        if(lookupItems.items.length > 0)
        {
            ...
            itemObjectTypeCode = lookupItems.items[0].type;
            var oUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_addtocampaign.aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + "&sIds=" + CrmEncodeDecode.CrmUrlEncode(sIds) + sCustParams + "&itemObjectId=" + CrmEncodeDecode.CrmUrlEncode(itemObjectId) + "&itemObjectTypeCode=" + CrmEncodeDecode.CrmUrlEncode(itemObjectTypeCode)),
                crmDialog = new Mscrm.CrmDialog(oUrl,a,iX,iY,null);
            crmDialog.show();
            oResult = false
        }
...
function doActionExArray(sGrid,iObjType,sParentId,sAction,iParentType,sParams,a)
{
    ...
    else
    {
        ...
        if(sAction == "createopportunity")
        {
            var oUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&sParentId=" + CrmEncodeDecode.CrmUrlEncode(sParentId) + "&iParentType=" + CrmEncodeDecode.CrmUrlEncode(iParentType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + sParams),
                crmDialog = new Mscrm.CrmDialog(oUrl,a,800,550,"maximize:yes;minimize:yes");
            crmDialog.show()
        }
        else
            if(sAction == "disassociate")
            {
                var parameters = new Array(sGrid),
                    crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&sParentId=" + CrmEncodeDecode.CrmUrlEncode(sParentId) + "&iParentType=" + CrmEncodeDecode.CrmUrlEncode(iParentType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + sParams),a,iX,iY,null);
                crmDialog.setCallbackInfo("PerformActionGridRefresh",this,parameters);
                crmDialog.show()
            }
            else
                if(sAction == "delete" && !Mscrm.Utilities.isModalDialogSupported())
                {
                    var gridControl = $find(sGrid),
                        callbackFunction = "PerformActionAfterDeleteFromGrid",
                        parameters = [gridControl,a],
                        callBackRef = Mscrm.Utilities.createCallbackFunctionObject(callbackFunction,Mscrm.GridCommandActions,parameters,false);
                    openStdDlgWithCallback(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&sParentId=" + CrmEncodeDecode.CrmUrlEncode(sParentId) + "&iParentType=" + CrmEncodeDecode.CrmUrlEncode(iParentType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + sParams),a,iX,iY,callBackRef)
                }
                else
                    openStdDlg(Mscrm.CrmUri.create("/_grid/cmds/dlg_" + CrmEncodeDecode.CrmUrlEncode(sAction) + ".aspx?iObjType=" + CrmEncodeDecode.CrmUrlEncode(iObjType) + "&sParentId=" + CrmEncodeDecode.CrmUrlEncode(sParentId) + "&iParentType=" + CrmEncodeDecode.CrmUrlEncode(iParentType) + "&iTotal=" + CrmEncodeDecode.CrmUrlEncode(a.length) + sParams),a,iX,iY) && 
...
{
    if(typeof parentId == "undefined" || IsNull(parentId) || typeof parentObjectTypeCode == "undefined" || IsNull(parentObjectTypeCode))
...
function locAssocOneToMany(iType,sRelationshipName,parentId,parentObjectTypeCode)
{
    ...
    LookupObjectsWithCallback(callbackRef,null,"multi",iType,0,null,additionalParams,showNewButton,showPropButton)
}
...
function locAssocObj(iType,sSubType,sAssociationName,iRoleOrdinal,additionalParams,parentId,parentObjectTypeCode)
{
    var parent = GetParentObject(parentId,parentObjectTypeCode),
...
function locAssocObjAction(lookupItems,iType,sSubType,sAssociationName,iRoleOrdinal,parent)
{
    if(lookupItems)
        lookupItems.items.length > 0 && 
            AssociateObjects(parent.objectTypeCode,parent.id,iType,lookupItems,iRoleOrdinal == 2,sSubType,sAssociationName)
}
function CreateNewTemplate()
{
    ...
    options.width = 400;
    options.height = 200;
    Xrm.Internal.openDialog(Mscrm.CrmUri.create("/Tools/EmailTemplateEditor/Dialogs/emailtemplateproperties.aspx?isPersonal=0").toString(),options,window,null,null)
}
...
function SendBulkEmail(sGridName,iObjectTypeCode)
{
    ...
    IsNull(Mscrm.GridRibbonActions) && 
        Mscrm.CrmHeader.setScriptFile(Mscrm.CrmUri.create("/_static/_common/scripts/ribbonactions.js"),true);
    Mscrm.GridRibbonActions.sendBulkEmail(gridControl,ids,iObjectTypeCode,gridControl.get_innerGrid().get_numberOfRecords())
}
function WebMailMerge(sGridName,iObjectTypeCode)
{
    ...
    IsNull(Mscrm.GridRibbonActions) && 
        Mscrm.CrmHeader.setScriptFile(Mscrm.CrmUri.create("/_static/_common/scripts/ribbonactions.js"),true);
    Mscrm.GridRibbonActions.webMailMerge(gridControl,ids,iObjectTypeCode,gridControl.get_innerGrid().get_numberOfRecords())
}
...
{
    var gridControl = $find(sGrid);
    Mscrm.Utilities.sendSelectedRecordsUrl(bUsingEmail,gridControl.get_selectedRecords(),gridControl.get_entityTypeCode())
}
function RunFunctionFromParentOrSelf(sFunctionName,param1,param2,param3,param4,param5)
{
    ...
    Mscrm.CrmDebug.assert(!IsNull(func) && typeof func === "function","Function '" + sFunctionName + "' is not found in window.parent nor in window.self");
...
function openUrlFromGrid(e)
{
    try
    {
        ...
        if(!IsNull(o))
        {
            while(o.tagName != "A")
                o = o.parentNode;
            var url = Trim(CrmEncodeDecode.CrmUrlDecode(o.getAttribute("targetUrl") || o.href));
            if(!isNullOrEmptyString(url))
            {
                ...
                var newWindowUri = Mscrm.CrmUri.createForOrganization(url,null);
                newWindowUri.checkParamsNoEqual = true;
                var windowHandle = safeWindowOpen(newWindowUri,"","height=" + screen.availHeight * .75 + ",width=" + screen.availWidth * .75 + ",scrollbars=1,resizable=1,status=1,toolbar=1,menubar=1,location=1",false);
                !IsNull(windowHandle) && 
                    windowHandle.focus()
            }
        }
...
function DownLoadMap(importMapId,gridRecordsNumber)
{
    ...
    if(gridRecordsNumber == 1)
    {
        ...
        exportMap.action = Mscrm.CrmUri.create("/tools/managemaps/downloadimportmap.aspx?id=" + CrmEncodeDecode.CrmUrlEncode(importMapId)).toString();
        exportMap.submit()
    }
    else
        gridRecordsNumber > 1 && 
