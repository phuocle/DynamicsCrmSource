...
function go()
{
    ...
    if(_sAction == "routecase")
    {
        ...
        if(index != -1)
        {
            ...
            if(stateCode == 0)
                window.parent.frames[index].Xrm.Page.data.save().then(callPerformAction,Mscrm.InternalUtilities.ClientApiUtility.operationFailedCallback);
            else
                callPerformAction()
        }
        else
...
function PrepareFillBar()
{
    ...
    if(_renderFillBar)
    {
        var oHeader = document.getElementById("tdDialogHeader"),
            ...
            nVerticalPosition = (nDialogHeight - (nHeaderHeight + nFooterHeight)) / 2 + nHeaderHeight - nFillBarHeight / 2;
...
function wait()
{
    ...
    if(status == ERROR_CONTINUE)
    {
        if(_errorCount == 0)
            _errorCode = status;
        if(_sAction == "mergemultiple" || _sAction == "queueitemrouting" || Mscrm.Utilities.enforceStateTransitions() || _sAction == "routecase")
        {
    ...
    if(_i < getDialogArgs().length - 1 && (_bContinueOnError || status == ERROR_NONE))
    {
        ...
        if(_errorCode == ERROR_CONTINUE)
        {
            ...
            {
                (_sAction == "activate" || _sAction == "deactivate") && window.parent.GenericWorkflowDesigner && window.parent.GenericWorkflowDesigner.designerType == window.parent.GenericWorkflowDesigner.DesignerType.BpfDesigner && 
                    window.parent.GenericWorkflowDesigner.StaticStopwatch.stop();
...
function doPostProcessing()
{
    if(IsOutlookClient())
    {
        ...
        if((IsActivityTypeCode(_iObjType) || Contact == _iObjType || QueueItem == _iObjType) && typeof oArgs.opener != "undefined")
            try
            {
        ...
        if(!Mscrm.Utilities.isModalDialogSupported() || isInlinedDialog())
            if(!IsNull(window.opener))
            {
                try
                {
                    !IsNull(window.parent) && !IsNull(window.parent.opener) && window.parent.opener.Xrm.Internal.isUci() && 
    ...
    if(_sAction == "mergemultiple")
        ...
        if((_sAction == "activate" || _sAction == "deactivate") && window.parent.GenericWorkflowDesigner && window.parent.GenericWorkflowDesigner.designerType == window.parent.GenericWorkflowDesigner.DesignerType.BpfDesigner)
        {
            var errInfo = Mscrm.ErrorInformation.createFromDoc(_oXml),
                returnObj = {value:window.returnValue,errorCode:errInfo.get_code(),error:errInfo.get_serializedException()};
...
function LaunchErrorDlg(_oXml,callback)
{
    ...
    openErrorDlg(IsNull(errInfo.get_code()) ? _errorNumber : errInfo.get_code(),errInfo.get_description(),errInfo,null,null,callback)
}
function performAction()
{
    ...
    _submitUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_" + _sAction + ".aspx?iObjType=" + _iObjType + "&iTotal=1&iIndex=" + _i + _custParams + sCustParamsForItem);
    if(getDialogArgs())
    ...
    if(_isAsynch)
        _xmlhttp.onreadystatechange = function()
        {
            if(_xmlhttp.readyState == 4)
            {
                ...
                {
                    if(_isCancelPressed && window.confirm(formatString(LOCID_CONFIRM_MULTI_CANCEL,getDialogArgs().length - _i)))
                    {
                        window.returnValue = true;
                        _isCancelPressed = false;
                        closeWindow()
                    ...
                    if(!(_xmlhttp.status == 0 && window.UseTabletExperience) && !_isCancelPressed)
                    {
...
{
    if(IsNull(_a))
        _a = _overrideDialogArgs && typeof getCustomDialogArgs != "undefined" ? getCustomDialogArgs() : getDialogArguments();
    return _a
}
