...
function LoadDynamicValuesForAttribute(valueControl,attribute)
{
    ...
    attribute.id != "DisplayText" && attribute.id != "staticUrlTextbox" && 
...
function OnEntityChangeCore(selectBox,dataSelectBox,entityValue,entityHasDot,attribute)
{
    ...
    if(!IsNull(attribute))
        if(!IsNull(attribute.getAttribute("references")))
            ...
            if(!IsNull(attribute.getAttribute("lookuptypes")))
                references = attribute.getAttribute("lookuptypes");
            else
                if(_currentDataType == "Picklist")
                {
                    var entityName = null;
    ...
    for(var index = 0,
        index = 0; index < dataSelectBox.options.length; index++)
    {
            ...
            if(!IsNull(references) && references.length > 0)
            {
                ...
                if(option.getAttribute("references") != undefined && !IsNull(option.getAttribute("references")))
                    for(var searchInString = "," + option.getAttribute("references") + ",",
                        ...
                        luIndex = 0; !fAdd && luIndex < referenceTypes.length; luIndex++)
                        if(-1 != searchInString.indexOf("," + referenceTypes[luIndex] + ","))
                            fAdd = true;
                if(!fAdd && _currentDataType == "Picklist")
                    if(option.getAttribute("references") != undefined && !IsNull(option.getAttribute("references")) && FindOptionSetName(option.getAttribute("references")) == references)
                        fAdd = true;
                    else
                    {
...
function AddCompatibleEntityAttributes(selectBox,entityValue,entityHasDot,attribute,dataType)
{
    ...
    if(compatibleTypes != undefined)
        for(var typeNames = compatibleTypes.split(","),
            index = 0,
            index = 0; index < typeNames.length; index++)
...
function addOptionSortedByText(selectBox,optionText,optionValue)
{
    try
    {
        if(IsOptionFiltered && IsOptionFiltered(optionValue))
            return
    }
    catch(e)
    {
    ...
    else
    {
        for(var index = 0,
            ...
            if(XUI.Html.GetText(selectBox[index]) > optionText)
            {
                ...
                if(selectBox.options[index].value == optionValue)
                    return;
        index == length && 
            Mscrm.Utilities.addOption(anOption,selectBox,index)
    }
    ...
    if(!IsNull(_currentValueControl))
    {
        ...
        if(Mscrm.FormUtility.getSlugBehavior(_slugControl))
        {
            ...
            if(!IsTimeoutCondition(_currentValueConditionControl.get_conditionParentControl()))
                slugText = XUI.Html.GetText(entityNameControl.options[entityNameControl.selectedIndex]) + "-" + slugText;
...
function UpdateOperator(control,value,text)
{
    ...
    slugBehavior.get_isDataSlug() && !IsNull(value) && !IsNull(text) && 
        slugBehavior.UpdateOperatorControl(value,text,getOperatorTextForValue(text))
}
function InsertDataSlugCore(control,value,text,isMultiSlug,attributeName,operatorValue,operatorText,isReadOnly,isDefaultValue,isInsertHyperLink)
{
    var slugBehavior = Mscrm.FormUtility.getSlugBehavior(control);
    Mscrm.CrmDebug.assert(!IsNull(slugBehavior),"Can't get SlugBehavior.");
    ...
    else
    {
        ...
        if(_currentDataType == "DateTime")
        {
            ...
            !IsNull(beforeAfter) && beforeAfter.value != DurationOption && 
                ResetDurationValue()
        }
    }
}
function InsertDataSlugInEmailRTE(slugBehavior,value,text)
{
    ...
    html += '<SPAN id="' + id + '" class="ms-crm-DataSlug" contentEditable="true" style="DISPLAY: inline" tabIndex="-1" value=\'';
...
function AddToDynamicValueList()
{
    ...
    if(index == length)
    {
        ...
        var entityNameControl = document.getElementById("selObjects");
        XUI.Html.SetText(anOption,XUI.Html.GetText(selectedOption) + "(" + XUI.Html.GetText(entityNameControl.options[entityNameControl.selectedIndex]) + ")");
...
function setOperatorAsClearOperator()
{
    for(var operatorControl = document.getElementById("operatorSelector"),
        index = 0,
        index = 0; index < operatorControl.options.length; index++)
...
function OperatorChange()
{
    ...
    if(operator[1] == "clear")
    {
        ...
        var expression = GetExpressionForClearValueSlug(),
            ...
            isMultiSlug = false,
            isReadOnly = true;
        InsertDataSlugCore(_currentValueControl,expression[0],expression[1],isMultiSlug,attributeName,operator[0],operator[1],isReadOnly,false,false)
    }
    else
    {
        EnableOkAddButtons();
        ...
        slugBehavior && slugBehavior.get_isDataSlug() && 
    ...
    switch(_currentDataType)
    {
        ...
        case "Money":
            for(var defaultValueControl = document.getElementById("defaultValueControlSpan"),
                ...
                index = 0; index < inputControls.length; index++)
            {
...
{
    var listControl = document.getElementById("dynamicValueSelector");
    if(listControl.options.length > 0 && listControl.selectedIndex >= 0 && listControl.selectedIndex < listControl.options.length - 1)
    {
        var currentOption = listControl.options[listControl.selectedIndex],
...
function SetAttributeTypeMapping(entityName,attributeMapping)
{
    if(!IsNull(attributeMapping))
    {
        ...
        for(var attrList = _attributeTypeMapping.split("&"),
            index = 0,
            index = 0; index < attrList.length; index++)
        {
            ...
            if(IsNull(attrControl))
                ...
                if(!IsNull(attrControl) && Sys.UI.DomElement.containsCssClass(attrControl,"ms-crm-Duration"))
                    attrControl = document.getElementById(attrName + "Select");
            if(!IsNull(attrControl))
            {
                var className = attrControl.className;
                if(!IsNull(className))
                ...
                switch(className)
                {
                    ...
                    case "ms-crm-SelectBox":
                        if(attrControl.tagName.toUpperCase() == "SPAN")
                        {
                            ...
                            attrControl.tagName.toUpperCase() == "SELECT" && attrControl.contentEditable != "false" && 
                    ...
                    case "ms-crm-CheckBox":
                        attrControl.contentEditable != "false" && (Mscrm.Utilities.isChrome() || Sys.Browser.agent == Sys.Browser.Safari) && 
...
function CallCustomOnFocusEventHandler(srcElement)
{
    try
    {
        if(srcElement.id.startsWith("Variable") && srcElement.getAttribute("attrOperator") != undefined && srcElement.getAttribute("attrOperator") != null && this.location.pathname.endsWith("fetchquery.aspx"))
        {
    ...
    if(!IsNull(dataType))
    {
        ...
        switch(_currentDataType)
        {
            ...
            case "Money":
                ...
                {
                    if(Sys.UI.DomElement.containsCssClass(currentControl,"ms-crm-Money") && currentControl.tagName == "TABLE")
                        break;
        ...
        if(!IsNull(_firstChild) && (_firstChild.tagName == "INPUT" || _firstChild.tagName == "TEXTAREA") && Sys.UI.DomElement.containsCssClass(_firstChild,"ms-crm-Input"))
        {
        ...
        do
        {
            ...
            {
                if(Sys.UI.DomElement.containsCssClass(_currentValueControl,"ms-crm-Email-Address") || Sys.UI.DomElement.containsCssClass(_currentValueControl,"ms-crm-LiveId-Email-Address"))
                {
                    crmCreate(Mscrm.FormInputControl.EmailAddressInputBehavior,{},null,null,defaultValueControlOwner);
                    break
                }
                    ...
                    if(moneyBehavior)
                    {
                        ...
                        if(moneyBehavior.style.display == "none")
                        {
                            ...
                            moneyBehavior.parentNode.childNodes.length > 1 && 
                                moneyBehavior.parentNode.removeChild(XUI.Html.DomUtils.GetChildElementAt(moneyBehavior.parentNode,1))
                        }
        ...
        switch(_currentDataType)
        {
            ...
            case "Integer":
                ...
                if(_currentDataType == "Integer" && (Sys.UI.DomElement.containsCssClass(control,"ms-crm-SelectBox") && durationInputControl != null))
                {
                    defaultValueControl.innerHTML = XUI.Html.GetOuterHTML(durationInputControl) + defaultValueControl.innerHTML;
                ...
                if(shouldCreateDurationControl)
                {
                    ...
                    _currentDataType == "DateTime" && 
                        Mscrm.FormInputControl.DateTimeBehavior.registerDateTimeComponents(defaultValueControlOwner.parentNode);
                break;
            case "Boolean":
                ...
                switch(className)
                {
                    ...
                    case "ms-crm-RadioButton":
                        ...
                        for(var index = 0,
                            length = inputControls.length; index < length; index++)
                        {
            ...
            case "PartyList":
                ...
                if(slugBehavior.get_isTextSlugInTextBox())
                {
                    ...
                    defaultValueControl.innerHTML = "<table cellspacing='0' cellpadding='0' style='width:100%;'><tbody><tr><td id=" + newParentId + ">" + defaultValueControl.innerHTML + "</td></tr></tbody></table>";
...
function InsertCustomizedDataSlug(slugValue,slugText)
{
    ...
    if(IsNull(slugText))
        slugText = "";
    if(_currentValueControl != null && _currentValueControl.id != "DisplayText" && _currentValueControl.id != "staticUrlTextbox")
    {
...
function ConstructSlugCoreParameters(tagName,hyperlinkXmlDoc)
{
    ...
    if(tagName == "DisplayText")
    {
        ...
        if(hyperlinkXmlDoc.getElementsByTagName("slug").length == 2)
        {
            var xmlSlugValues = [];
    ...
    if(slugElements.length != 0)
        if(slugElements[index].attributes.length == 2)
        {
            ...
            if(slugTextElement.length != 0)
                slugValues[1] = XUI.Xml.GetText(slugTextElement[index]) ? XUI.Xml.GetText(slugTextElement[index]) : slugTextElement[index].text
        }
    return slugValues
}
...
function InsertXMLTags(_currentValueControl,XMLTag)
{
    ...
    if(slugBehavior.get_isDataSlug() == true)
        ...
        if(IsEmailBody(_currentValueControl))
        ...
        else
        {
            ...
            if(document.selection && document.selection.createRange)
            {
                ...
                if(_currentValueControl.selectionStart || _currentValueControl.selectionStart == "0")
                {
                    ...
                    _currentValueControl.value = oldValue.substring(0,startPos) + CrmEncodeDecode.CrmXmlDecode(XMLTag) + oldValue.substring(endPos,oldValue.length)
                }
                else
...
function CallInsertDataSlugCore(hyperlinkXml,_currentValueControl,tagName,beginTag)
{
    ...
    if(!IsNull(_currentValueControl) && slugBehavior)
        if(elementNodes.length == 0)
        {
            ...
            InsertDataSlugCore(_currentValueControl,expressionAndDefaultValueSlug[0],expressionAndDefaultValueSlug[1],isMultiSlug,attributeName,operator[0],operator[1],isReadOnly,false,true)
        }
        else
            if(slugBehavior.get_isDataSlug() == true)
...
function InsertHyperLinkSlug(hyperlinkXml,elementId)
{
    ...
    if(IsNull(oNode))
    {
        InsertXMLTags(_currentValueControl,"<hyperlink><name>" + XUI.Xml.GetText(XUI.Xml.SelectSingleNode(xmlDoc,"//DisplayText",null)) + "</name><value>" + XUI.Xml.GetText(XUI.Xml.SelectSingleNode(xmlDoc,"//staticUrlTextbox",null)) + "</value></hyperlink>");
        return
    }
...
function ResetTabIndexRecursive(control)
{
    ...
    {
        control.tabIndex = -1;
        XUI.Html.DomUtils.ForEachChild(control,function(childNode)
        {
...
function AddOperatorForDataType(additionalOperators,dataType)
{
    ...
    for(var length = dataType.length + 1,
        ...
        index = 0,
        index = 0; index < operatorList.length; index++)
    {
...
function GetSelfPropertyExpression(operator)
{
    ...
    var attributeName = GetEntityAttributeName(_currentValueControl),
        expression = '<slugelement type="slug"><slug type="dynamic" value="' + entityNameValue + "." + attributeName + '"/></slugelement>';
    return expression
}
...
function GetExpressionForSlug()
{
    ...
    for(var durationExpression = GetDurationExpression(),
        index = 0,
        index = 0; index < listControl.options.length; index++)
    {
...
function GetDefaultValueForSlug()
{
    ...
    if(defaultValueControl.tagName == "TABLE" && (IsNull(ctrl) || !("get_dataValue"  in  ctrl)) && XUI.DomUtilities.HasChildElements(defaultValueControl))
    {
    ...
    if(ctrl && !IsNull(ctrl.get_dataValue()))
    {
        ...
        if(_currentDataType == "Boolean")
        {
            ...
            switch(className)
            {
                ...
                case "ms-crm-RadioButton":
                    ...
                    for(var index = 0; index < inputControls.length; index++)
                        if(Sys.UI.DomElement.containsCssClass(inputControls[index],"ms-crm-RadioButton") && inputControls[index].value == valueToMatch)
                        {
            ...
            if(_currentDataType == "Picklist" || _currentDataType == "Status" || _currentDataType == "State")
            {
                ...
                if(_currentDataType == "DateTime")
                {
                    ...
                    if(_currentDataType == "PartyList")
                        for(var innerParentControl = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(defaultValueControl))))),
                            index = 0,
                            index = 0; index < innerParentControl.childNodes.length; index++)
                            if(index == 0)
                                returnValue[1] = XUI.Html.GetText(XUI.Html.DomUtils.GetFirstChild(innerParentControl));
                            else
                                returnValue[1] += "; " + XUI.Html.GetText(XUI.Html.DomUtils.GetChildElementAt(innerParentControl,index));
                    else
                        if(_currentDataType == "Integer")
                        {
                                    ...
                                    case "INPUT":
                                        if(Sys.UI.DomElement.containsCssClass(defaultValueControl,"ms-crm-Duration"))
                                        {
                                            var controlId = defaultValueControl.id,
                                                sanitizedControlId = controlId.substring(0,controlId.indexOf("DefaultValueControl")) + "SelectDefaultValueControl",
                                                selectControl = $find(sanitizedControlId);
                                            if(!IsNull(selectControl))
                                                returnValue[1] = selectControl.get_returnValueProperty()
                                        }
                                        else
                                            if(XUI.Html.GetText(defaultValueControl) != "")
                            ...
                            if(_currentDataType == "Money")
                            {
                                ...
                                if(defaultValueControl.nodeName.toUpperCase() != "TEXTAREA" && XUI.Html.GetText(defaultValueControl) != "")
                                    returnValue[1] = XUI.Html.GetText(defaultValueControl);
                                else
                                    returnValue[1] = defaultValueControl.value
    }
...
function ResetDefaultValue(defaultValueControl)
{
    ...
    if(!IsNull(ctrl) && !IsNull(ctrl.get_dataValue()))
        ...
        if(Sys.UI.DomElement.containsCssClass(defaultValueControl,"ms-crm-DateTime"))
        {
...
function GetDurationExpression()
{
    ...
    if(XUI.Html.GetText(month) == "0" && XUI.Html.GetText(day) == "0" && XUI.Html.GetText(hour) == "0" && XUI.Html.GetText(minute) == "0")
        return returnValue;
    returnValue[0] = '<slug type="duration" value="' + XUI.Html.GetText(month) + ";" + XUI.Html.GetText(day) + ";" + XUI.Html.GetText(hour) + ";" + XUI.Html.GetText(minute) + ";" + beforeAfter.value + '"/>';
    returnValue[1] = null;
...
function PopulateSlugControlsWithCallback(slugInfo,eventName,eventHandler)
{
    for(var slugInfoList = slugInfo.split("<;>"),
        index = 0,
        index = 0; index < slugInfoList.length; index++)
    {
...
function PopulateControl(controlId,valueXml,eventName,eventHandler)
{
    ...
    if(!IsNull(control) && slugBehavior)
    {
        ...
        XUI.Html.DomUtils.ForEachChild(xmlDocFirstChild,function(childNode)
        {
            ...
            else
            {
                ...
                _currentDataType = dataType;
                InsertDataSlugCore(control,slugInfo[0],slugInfo[1],isMultiSlug,controlId,parsedInfo[0],parsedInfo[1],isReadOnly,true,false)
            }
        })
    }
}
function PopulateEmailBody(control,xmlDoc,dataType)
{
    ...
    XUI.Html.DomUtils.ForEachChild(xmlDocFirstChild,function(childNode)
    {
        ...
        {
            html += '<SPAN class="ms-crm-DataSlug" style="DISPLAY: inline;background-color: #FFFF33; border: 1px solid #808080" tabIndex="-1" value=\'';
            html += slugInfo[0] + "'";
...
function ParseOperatorAndSlugInfo(valueXml)
{
    for(var dataXml = "<data>" + valueXml + "</data>",
        ...
        index = 0,
        childNode,
        index = 0; index < slugBody.childNodes.length; index++)
    {
    ...
    for(var newIndex = 0,
        newIndex = index + 1; newIndex < slugBody.childNodes.length; newIndex++)
    {
...
function GetTextForNode(xmlNode,dataType)
{
    ...
    for(var index = 0,
        durationText = null,
        index = 0; index < xmlNode.childNodes.length; index++)
    {
        ...
        if(nodeType.nodeName == "type" && nodeType.nodeValue == "dynamic")
        {
            ...
            if(nodeType.nodeName == "type" && nodeType.nodeValue == "defaultvalue")
            {
                ...
                if(nodeType.nodeName == "type" && nodeType.nodeValue == "duration")
    ...
    switch(dataType)
    {
        ...
        case "Boolean":
            ...
            switch(className)
            {
                ...
                case "ms-crm-SelectBox":
                    ...
                    break;
                case "ms-crm-RadioButton":
                    for(var firstChild = XUI.Html.DomUtils.GetFirstChild(control),
                        inputControls = firstChild.getElementsByTagName("INPUT"),
                        inputLabels = firstChild.getElementsByTagName("LABEL"),
                        index = 0; index < inputControls.length; index++)
                        if(Sys.UI.DomElement.containsCssClass(inputControls[index],"ms-crm-RadioButton") && inputControls[index].value == XUI.Xml.GetText(defaultValueNode))
        ...
        case "PartyList":
            for(var returnValue = null,
                index = 0,
                index = 0; index < defaultValueNode.childNodes.length; index++)
            {
...
function GetOptionText(controlId,compareText,isTitle)
{
    for(var control = document.getElementById(controlId),
        index = 0,
        index = 0; index < control.options.length; index++)
    {
...
function IsMultiSlug(dataType,control)
{
    ...
    if(dataType == "PartyList")
    {
        for(var imgControls = control.getElementsByTagName("IMG"),
            index = 0; index < imgControls.length; index++)
            if(Sys.UI.DomElement.containsCssClass(imgControls[index],"ms-crm-Lookup-Party") && Mscrm.FormControlInputBehavior.GetBehavior(imgControls[index].id).get_lookupStyle() == "single")
                return false;
        return true
    }
    return false
...
function DurationValueChanged(eventObj)
{
    ...
    if(!IsNull(beforeAfter) && beforeAfter.value == DurationOption)
    {
        ...
        {
            for(var i = 0,
                i = 0; i < entityNameSelect.options.length; i++)
        ...
        if(IsDurationSelected())
            for(var hiddenSelectBox = document.getElementById("valueSelectorHiddenDateTime"),
                index = 0; index < hiddenSelectBox.options.length; index++)
            {
                var option = hiddenSelectBox.options[index];
