...
{
    XUI.Html.DomUtils.ForEachChild(o.parentNode,Function.createDelegate(this,function(child)
    {
        child.className = ""
    }));
...
function getHomeCalendarElements()
{
    var parentContext = window.frameElement.ownerDocument.parentWindow || window.frameElement.ownerDocument.defaultView,
        ...
        rVal = {};
    rVal.oTitle = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(oMainTable)));
...
function FixCalendarFrameHeightForIE7()
{
    if(Sys.Browser.agent == Sys.Browser.InternetExplorer && Sys.Browser.version < 8)
    {
        ...
        if(!IsNull(frameWrapper))
        {
            ...
            if(wrapperHeight > 0)
                XUI.Html.DomUtils.GetFirstChild(frameWrapper).style.height = wrapperHeight - Mscrm.Utilities.getVerticalBoxPadding(frameWrapper) + "px"
        }
    }
}
