...
function FetchBuilder()
{
    ...
    function addAttribute(sEntityAlias,sAttributeName)
    {
        var oEntityNode = getEntityNode(sEntityAlias),
            oAttributeNode = XUI.Xml.SelectSingleNode(oEntityNode,formatString("attribute[@name = '{0}']",sAttributeName),null),
            sAttributeAlias;
    ...
    function addLinkEntity(sName,sAlias,sFrom,sTo,sParentAlias,sEntityAlias)
    {
        var oParentNode = getEntityNode(sParentAlias),
            oLinkEntityNode = XUI.Xml.SelectSingleNode(oParentNode,formatString("link-entity[@alias = '{0}']",sAlias),null);
    ...
    {
        return sAlias.length == 0 ? getPrimaryEntityNode() : XUI.Xml.SelectSingleNode(_oFetchXml,formatString("//link-entity[@alias = '{0}']",sAlias),null)
    }
    function getLinkEntityString(oLinkEntityNode)
    {
        return formatString("{0};{1};{2}",oLinkEntityNode.getAttribute("name"),oLinkEntityNode.getAttribute("from"),oLinkEntityNode.getAttribute("to"))
    }
    ...
    function getSecondaryEntityString()
    {
        ...
        if(isLinkEntityNode(oSecondaryEntityNode.parentNode))
        {
            ...
            return formatString("{0};;;true;{1};{2};{3};{4};{5};",oSecondaryEntityNode.getAttribute("name"),oIntersectEntityNode.getAttribute("name"),oIntersectEntityNode.getAttribute("to"),oSecondaryEntityNode.getAttribute("from"),oIntersectEntityNode.getAttribute("from"),oSecondaryEntityNode.getAttribute("to"))
        }
        else
    ...
    function getXml()
    {
        for(var oReturnFetchXml = XUI.Xml.LoadXml(XUI.Xml.XMLSerializer.serializeToString(_oFetchXml)),
            ...
            i = 0; i < oaAttributeNodes.length; i++)
            oaAttributeNodes[i].removeAttribute("alias");
        return XUI.Xml.XMLSerializer.serializeToString(oReturnFetchXml)
    }
    ...
    {
        _oFetchXml = IsNull(oFetchXml) ? XUI.Xml.LoadXml('<fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false" />') : oFetchXml;
        if(!IsNull(oFetchXml))
            for(var oaAttributeNodes = XUI.Xml.SelectNodes(_oFetchXml,"//attribute",null),
                i = 0; i < oaAttributeNodes.length; i++)
            {
                var oAttributeNode = oaAttributeNodes[i],
                    sAttributeAlias = getAttributeAlias(oAttributeNode.parentNode.getAttribute("alias"),oAttributeNode.getAttribute("name"));
                oAttributeNode.setAttribute("alias",sAttributeAlias)
            }
    }
    function removeAttribute(sAttributeAlias)
    {
        ...
        oEntityNode.childNodes.length == 0 && isLinkEntityNode(oEntityNode) && IsNull(oEntityNode.getAttribute("secondary")) && 
            oEntityNode.parentNode.removeChild(oEntityNode);
        var oSortNode = getSortNode();
        !IsNull(oSortNode) && oSortNode.getAttribute("attribute") == sAttributeAlias && 
        ...
        if(!IsNull(sEntityName) && sEntityName.length > 0 && bSecondaryEntityChanged)
        {
            ...
            if(bIsIntersect)
            {
                var sIntersectEntityName = saEntityStringElements[4],
                    ...
                    oIntersectEntityNode = createLinkEntityNode(sIntersectEntityName,sIntersectFromAttributeName,sIntersectToAttributeName,null);
                ...
                oNewSecondaryEntityNode = createLinkEntityNode(sEntityName,sSecondaryEntityFromAttribute,sSecondaryEntityToAttribute,sSecondaryEntityAlias);
