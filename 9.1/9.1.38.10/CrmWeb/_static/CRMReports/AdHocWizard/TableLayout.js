...
function initializePage()
{
    ...
    if(_oWizardXmlBuilder.GetColumnCount() > 0 || _oWizardXmlBuilder.GetGroupingCount() > 0)
    {
        var oAttributeInfoCache = new AttributeInfoCache,
            oFetchXml;
        oFetchXml = _oFetchBuilder.GetXml();
        var bDisplayNamesFound = oAttributeInfoCache.Initialize(oFetchXml,_iLocaleId);
...
function openPropertyPage(iMode,iType,oArgs,oContextNode)
{
    ...
    var fieldsAlreadyInUseList = _oFetchBuilder.GetAttributeInfos(_oWizardXmlBuilder.GetAttributeAliases(iType));
    if(iMode == EDIT_MODE_UPDATE)
        fieldsAlreadyInUseList[IsNull(oArgs.Entity) ? "" : oArgs.Entity][oArgs.Attribute] = null;
    oArgs.FieldsAlreadyInUseList = fieldsAlreadyInUseList;
...
function removeColumnOrGrouping()
{
    ...
    if(IsNull(iObjectType))
        return;
    if(iObjectType == PROPERTY_PAGE_TYPE_COLUMN && WizardHasProperty("ColumnXml") && !confirm(LOCID_REMOVE_COLUMN_WARNING))
        return;
