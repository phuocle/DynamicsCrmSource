...
function chartFormatPage_onload()
{
    ...
    $get("previewImage").src = formatString("/_imgs/ReportWizard/chart_{0}.gif",_oChartLayoutXmlBuilder.GetChartType().toLowerCase());
    ...
    if(_iColumnAxisType == PROPERTY_PAGE_TYPE_GROUP)
            ...
            i = 0; i < oaGroupings.length; i++)
        {
            var oGrouping = oaGroupings[i],
                ...
                sDisplayName = IsNull(sTimeInterval) ? sDisplayNameBase : formatString(_saGroupingIntervalFormats[sTimeInterval],sDisplayNameBase);
...
function initializeChartingCommon()
{
    ...
    _oWizardXmlBuilder.SetGroupingXml(getXmlDocumentFromString(WizardGetProperty("GroupingXml")));
    _oWizardXmlBuilder.SetColumnXml(getXmlDocumentFromString(WizardGetProperty("ColumnXml")));
    _oWizardXmlBuilder.SetTableLayoutXml(getXmlDocumentFromString(WizardGetProperty("TableLayoutXml")));
    ...
    _oAttributeInfoCache.Initialize(WizardGetProperty("FetchXml"),WizardGetProperty("Language"));
    _oChartLayoutXmlBuilder = new ChartLayoutXmlBuilder;
    _oChartLayoutXmlBuilder.SetLayoutXml(getXmlDocumentFromString(WizardGetProperty("ChartLayoutXml")))
}
function initializeValueAxisSelects()
{
    ...
    for(var oaValueAxes = getValueAxes(),
        iValueAxisCount = _oChartLayoutXmlBuilder.GetChartType() == CHART_TYPE_PIE ? PIE_VALUE_AXIS_COUNT : VALUE_AXIS_COUNT,
        i = 0; i < iValueAxisCount; i++)
    {
        ...
        if(i == 0)
            ...
            _valueAxis2FieldCtrl = Mscrm.FormControlInputBehavior.GetBehavior("valueAxis2Field");
        var oFieldControl = Mscrm.FormControlInputBehavior.GetBehavior(oValueAxisSelect.ID);
        if(IsNull(oValueAxisNode) || bColumnAxisTypeChanged || oFieldControl.get_dataValue() != oValueAxisSelect.Selected)
        ...
        else
        {
            ...
            {
                var oLabelControl = Mscrm.FormControlInputBehavior.GetBehavior(formatString("valueAxis{0}Label",i + 1));
...
function updateNextButton()
{
    var bIsPieChart = _oChartLayoutXmlBuilder.GetChartType() == CHART_TYPE_PIE,
        bEnableNextButton = !IsNull(_columnAxisFieldCtrl.get_dataValue()) && !IsNull(_valueAxis1FieldCtrl.get_dataValue()) && (bIsPieChart || !IsNull(_columnAxisLabelCtrl.get_dataValue()) && !IsNull(_valueAxis1LabelCtrl.get_dataValue())) && (bIsPieChart || IsNull(_valueAxis2FieldCtrl.get_dataValue()) || !IsNull(_valueAxis2LabelCtrl.get_dataValue()));
...
function updatePreviewAxes()
{
    ...
    if(sChartType == CHART_TYPE_PIE)
        XUI.Html.SetText(xAxisElement,formatString(LOCID_PIE_CHART_PREVIEW,_valueAxis1FieldCtrl.get_selectedText(),_columnAxisFieldCtrl.get_selectedText()));
    else
    {
