...
runAsCurrentValue = parseInt(($("[name='rad_runas']") && $("[name='rad_runas']").length > 0 && $("[name='rad_runas']:checked").length > 0) ? $("[name='rad_runas']:checked").val() : 1);
...
_oldRunOnDemand = $get("chkOnDemand") != null ? $get("chkOnDemand").checked : false;
}
...
var runOnDemand = $get("chkOnDemand") != null ? $get("chkOnDemand").checked : false;
if(LOCID_IS_ASYNC_WORKFLOW || LOCID_IS_SYNC_WORKFLOW)
{
var currScope = $get("scope").selectedIndex;

if(	_oldIAttrs != _iAttrs ||
_oldRunOnDemand != runOnDemand ||
_oldRunSubprocess != $get("chkSubprocess").checked ||
_oldOnCreate != $get("chkOnCreate").checked ||
_oldOnDelete != $get("chkOnDelete").checked ||
_oldScope != currScope ||
...
_oldBAllAttrSelected != _bAllAttrSelected ||
(LOCID_IS_ASYNC_WORKFLOW && _oldAsyncAutoDelete != $get("asyncautodelete").checked) || SyncWorkflowAttributesChanged())
{
return true;
}
}
...
if (pos == controlId.length - expectedSuffix.length && controlId.substr(0, expectedPrefix.length)== expectedPrefix)
{
AddAttributeTypeMapping(controlId, "Owner");
}
}

PopulateSlugControlsWithCallback(CrmEncodeDecode.CrmXmlDecode($get("hidSlugInfo").value), "deleteslugbody", OnAssigneeSlugDelete);
}
...
{
ToggleView("PublicationParameterArea", "optionsText", "optionsImg", LOCID_PUBLICATION_TITLE_SHOW, LOCID_PUBLICATION_TITLE_HIDE);
}

function ToggleArgumentView()
{
ToggleView("customOperationArguments", "argumentsText", "argumentsImg", LOCID_ARGUMENTS_TITLE_SHOW, LOCID_ARGUMENTS_TITLE_HIDE);
}
...
{
ShowViewTitle(bVisible, "PublicationParameterArea", "optionsText", "optionsImg", LOCID_PUBLICATION_TITLE_SHOW, LOCID_PUBLICATION_TITLE_HIDE);
}
...
stages = $get('createstage').value + "," + $get('updatestage').value + "," + $get('deletestage').value;
}
}
else if(LOCID_IS_CUSTOMOPERATION)
{
...
dialogOptions.height = <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/InsertVariable.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>;
dialogOptions.width = <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/InsertVariable.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>
...
{
var runOneDemand = $get("chkOnDemand") != null ? $get("chkOnDemand").checked : false;
if (isInputArg && (runOneDemand == true || $get("chkSubprocess").checked == false))
{
var parameters = new Array(entityId, variableId,isInputArg);
var callBackRef = Mscrm.Utilities.createCallbackFunctionObject("confirmInputArgumentCallback", this, parameters, false);
var result = openStdDlgWithCallback(Mscrm.CrmUri.create("/sfa/workflow/dlg_confirm_inputargument.aspx"), null, 400, 230, callBackRef);
...
{
oUrl.get_query()["StepDescription"] = Mscrm.FormControlInputBehavior.GetBehavior(oid + "_wfdescwfc").get_dataValue();
}
else
{
...
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterAssignValue", this, parameters, false);

var sNewXml = openStdDlgWithCallback(oUrl, oDialogArgs, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/assignValue.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/assignValue.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>, callbackFunc);
...
oUrl.get_query()["StepDescription"] = CrmEncodeDecode.CrmUrlEncode(Mscrm.FormControlInputBehavior.GetBehavior(oid + "_wfdescwfc").get_dataValue());
}
else
{

oUrl.get_query()["StepDescription"] = CrmEncodeDecode.CrmUrlEncode(XUI.Html.GetText($get(oid + "_wfdescwfc")));
}

var parameters = new Array(oid);
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterSetAssignArgument", this, parameters, false);
var sNewXml = openStdDlgWithCallback(oUrl, oCustomOperationArgs, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/assignValue.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/assignValue.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>, callbackFunc);
...
var parameters = new Array(oid);
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterSetPromptResponse", this, parameters, false);
var sNewXml = openStdDlgWithCallback(oUrl, oDialogArgs, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/PromptResponse.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/PromptResponse.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>, callbackFunc);
...
var parameters = new Array(oid);
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterMapChildArguments", this, parameters, false);
...
if(readOnlyMode == "false")
{
oUrl.get_query()["StepDescription"] = (Mscrm.FormControlInputBehavior.GetBehavior(stepId + "_wfdescwfc").get_dataValue());
}
else
{
...
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterFetchQuery", this, parameters, false);

var sFetchXml = openStdDlgWithCallback(oUrl, oDialogArgs, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/FetchQuery.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>, <%=Convert.ToInt32(CurrentResourceManager.GetString("SFA/workflow/FetchQuery.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>, callbackFunc);
...
var parameters = new Array(oid);
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterSetCondition", this, parameters, false);

var sNewXml = openStdDlgWithCallback(oUrl, oDialogArgs, <%=Convert.ToInt32(CurrentResourceManager.GetString("condition/condition.aspx_WINDOW_WIDTH"), System.Globalization.CultureInfo.InvariantCulture)%>, <%=Convert.ToInt32(CurrentResourceManager.GetString("condition/condition.aspx_WINDOW_HEIGHT"), System.Globalization.CultureInfo.InvariantCulture)%>, callbackFunc);
...
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterSetWaitBranchCondition", this, parameters, false);
...
var parameters = new Array(stepId);
var callbackFunc = Mscrm.Utilities.createCallbackFunctionObject("performActionAfterSetWaitTimeoutExpression", this, parameters, false);
...
popupControl.setItemProperty('mnu_InsertDirection_Before', Microsoft.Crm.CommandBar.MenuControlConstants.iconPathAttributeName, "/_imgs/ico_16_insertOff.gif");

popupControl.setItemProperty('mnu_InsertDirection_After', Microsoft.Crm.CommandBar.MenuControlConstants.iconPathAttributeName, "/_imgs/ico_16_insertCheck.gif");
}
else
{

popupControl.setItemProperty('mnu_InsertDirection_Before', Microsoft.Crm.CommandBar.MenuControlConstants.iconPathAttributeName, "/_imgs/ico_16_insertCheck.gif");


popupControl.setItemProperty('mnu_InsertDirection_After', Microsoft.Crm.CommandBar.MenuControlConstants.iconPathAttributeName, "/_imgs/ico_16_insertOff.gif");
}
}
...
RegisterClientControl("INPUT", Mscrm.FormInputControl.TextBoxInputBehavior, element, function(node) {
...
RegisterClientControl("IMG", Mscrm.FormInputControl.PresenceLookupUIBehavior, element, function(node) {
...
addStepMenuCtrl.setItemProperty('mnu_AddStep_StopWorkflowStep', 'disabled', isStopWorkflowMenuDisabled.toString());
addStepMenuCtrl.setItemProperty('mnu_AddStep_ChildWorkflowStep', 'disabled', isChildWorkflowMenuDisabled.toString());
...
addStepMenuCtrl.setItemProperty('mnu_AddStep_WaitBranchStep', 'disabled', isWaitBranchStepMenuDisabled.toString());
}
else if(LOCID_IS_DIALOG)
{
addStepMenuCtrl.setItemProperty('mnu_AddStep_InteractionPageStep', 'disabled', isAddPageMenuDisabled.toString());
addStepMenuCtrl.setItemProperty('mnu_AddStep_InteractionStep', 'disabled',  isPromptResponseMenuDisabled.toString());
addStepMenuCtrl.setItemProperty('mnu_AddStep_QueryDataStep', 'disabled', isQueryMenuDisabled.toString());
addStepMenuCtrl.setItemProperty('mnu_AddStep_ChildInteractiveWorkflowStep', 'disabled',  isChildInteractiveWorkflowMenuDisabled.toString());
addStepMenuCtrl.setItemProperty('mnu_AddStep_AssignVariableStep', 'disabled', isFormulaMenuDisabled.toString());
}
else if (LOCID_IS_CUSTOMOPERATION)
{
...
{%>
var fetchXml = "<fetch><entity name=\"" + CrmEncodeDecode.CrmXmlAttributeEncode(primaryEntityName) + "\"><filter></filter></entity></fetch>";
...
shouldDisable =  !($get("chkOnUpdateOwner").checked || $get("chkOnUpdateStatus").checked || $get("chkOnUpdateAttr").checked);
break;
case 2:
...
{%>
{
$find("WorkflowNotifications").AddNotification("invalidWorkflow", Mscrm.Severity.SEVERITYERROR, "", <%="\""+workflowErrorMessage+"\""%>);
}
<%}%>
}
...
<div id="editLargeDiv" class="ms-crm-Form-Background ms-crm-absolutePosition" style="top: 98px; bottom: 25px;">
<%if(!string.IsNullOrEmpty(workflowErrorMessage))
{%>
<div>
...
<nobr><A HREF="#" onclick="ToggleParameterView();" id="optionsTitle"><img name="optionsImg" align="middle" class="expandCollapseImg" src="/_imgs/ico_16_expanded.png" id="optionsImg"/>
<b style="padding-left:3px;padding-right:3px;vertical-align:middle" id="optionsText"><loc:Text ResourceId="WorkflowPublicationTitleHide" runat="server" /></b>
</A></nobr>
</div>
...
<nobr><A HREF="#" onclick="ToggleArgumentView();" id="argumentsTitle"><img name="argumentsImg" align="middle" class="expandCollapseImg" src="/_imgs/ico_16_expanded.png" id="argumentsImg"/>
<b style="padding-left:3px;padding-right:3px;vertical-align:middle" id="argumentsText"><loc:Text ResourceId="WorkflowArgumentsTitleHide" runat="server" /></b>
</A></nobr>
</div>
<div id="CO_edit"> <asp:PlaceHolder id="placeholderCustomOperationArguments" runat="server"/> </div>
<%}%>
</div>
...
<col width="100"/><col/>
<tr>
<td id="owner_c" class="ms-crm-Field-Required"><label for="ownerid"><loc:Text ResourceId="Web.SFA.Workflow.owner" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/></label></td>
<td><sdk:LookupControl id="ownerid" AccessibilityLabelResourceId="Web.SFA.Workflow.owner" LookupStyle="single" runat="server" TabIndex="2" /></td>
</tr>
<tr height="100%">
<td valign="top"><label for="description"><loc:Text ResourceId="Web.Tools.SubjectEditor.Dialogs.Edit.aspx_85" runat="server"/></label></td>
<td valign="top"><sdk:TextAreaControl id="description" runat="server" TabIndex="3" height="500px" maxlength="2000"/></td>
</tr>
</table>
</div>
