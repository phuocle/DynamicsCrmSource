	...
	<script language="Javascript" type="text/javascript">
		...
		function save(sender, args)
		{
			if (args)
			{
				...
				args.preventDefault();
			} // args can be null when triggered from file Menu
		...
		function IsLookupFieldEmpty() {
			//Function to check if the Lookup field is specifically empty
		...
		function PageOnLoad() {
			...
			//This function goes to slugsupport.js
		...
		/////////////////////////////////////////////////////////////
		//Function to generate XML returned by the dialog
		function GenerateXml(sVal) {
			var sXml = "";
			sXml += "<AssignStep>";
			sXml += GenerateStepLabelXml();
			//add variable specific XML only if some variable is specified
			var vSelector = document.getElementById("variableSelector");
			var aSelector = document.getElementById("argumentSelector");
			if ((($get("radioVariable").checked == 1) && (vSelector.length != 0)) || (($get("radioVariable").checked == 0) && (aSelector.length != 0))) {
		...
		function GenerateVariableNameXml() {
			...
			var vSelector = document.getElementById("variableSelector");
			var aSelector = document.getElementById("argumentSelector");
			sXml += "<VariableName>" + ($get("radioVariable").checked ? CrmEncodeDecode.CrmXmlEncode(vSelector[vSelector.selectedIndex].text) : CrmEncodeDecode.CrmXmlEncode(aSelector[aSelector.selectedIndex].text)) + "</VariableName>";
			return sXml;
		}
		...
		function GetLookupValueControlXml(sVal) {
			...
			if (IsDataSlugSpecified(currentLookup)) {
				...
				// Refresh Contents and get the necessary values for sending back
		...
		function GetCurrentKeySlugValue() {
			return ($get("radioVariable").checked ? $get("variableSelector").getAttribute("variableSlug" + $get("variableSelector").selectedIndex) : $get("argumentSelector").getAttribute("variableSlug" + $get("argumentSelector").selectedIndex));
		}

		//////////////////////////////////////////////////////////////
		////SECTION: Helper functions on this page/////////////////
		/////////////////////////////////////////////////////////////
		function GetCurrentVariableType() {
			var selector = ($get("radioVariable").checked ? Mscrm.FormControlInputBehavior.GetBehavior("variableSelector") : Mscrm.FormControlInputBehavior.GetBehavior("argumentSelector"));
			var value = selector.get_dataValue();
			if (value == null) {
				//textbox to be displayed by default
				return "0";
			}
		...
		function toggleVariableSelectorVisibility(variablesVisible) {
			//need to maintain 2 labels for accessibility
			$get("labelVarSelector").style.display = $get("variableSelector").style.display = (variablesVisible ? "inline" : "none");
			$get("labelArgSelector").style.display = $get("argumentSelector").style.display = (variablesVisible ? "none" : "inline");
			//bring the focus back to selector since showValueBox changes the focus to populate form assistant
		...
		function showValueBox(clearData) {
			...
			Mscrm.FormControlInputBehavior.GetBehavior("variableValueText").setDisplay(value == LOCID_VARIABLE_TEXT ? true : false);
			$get("labelValueText").style.display = (value == LOCID_VARIABLE_TEXT ? "inline" : "none");
			Mscrm.FormControlInputBehavior.GetBehavior("variableValueInteger").setDisplay(value == LOCID_VARIABLE_INTEGER ? true : false);
			$get("labelValueInteger").style.display = (value == LOCID_VARIABLE_INTEGER ? "inline" : "none");
			Mscrm.FormControlInputBehavior.GetBehavior("variableValueFloat").setDisplay(value == LOCID_VARIABLE_FLOAT ? true : false);
			$get("labelValueFloat").style.display = (value == LOCID_VARIABLE_FLOAT ? "inline" : "none");
			Mscrm.FormControlInputBehavior.GetBehavior("variableValueDateOnly").setDisplay(value == LOCID_VARIABLE_DATEONLY ? true : false);
			$get("labelValueDateOnly").style.display = (value == LOCID_VARIABLE_DATEONLY ? "inline" : "none");
			Mscrm.FormControlInputBehavior.GetBehavior("variableValueDateTime").setDisplay(value == LOCID_VARIABLE_DATETIME ? true : false);
			$get("labelValueDateTime").style.display = (value == LOCID_VARIABLE_DATETIME ? "inline" : "none");

			$get("labelValueLookup").style.display = $get("variableValueLookup_d").style.display = (value == LOCID_VARIABLE_LOOKUP ? "inline" : "none");
			//bring the focus to the visible text box
			if (value == LOCID_VARIABLE_LOOKUP) {
				showHideLookupControls();
			}
			// Setting the focus can be bypassed for disabled controls
			// Lookup control's setfocus is handled in setLookupAttributeMappings() function
		...
		function formulateLookupCollection() {
			// Create an array of all the Lookups generated in the hiddendiv
			var str = LOCID_LOOKUPS_CONTROL;
			lookupControls = str.split(";");
		}  

		function showHideLookupControls() {
			//function iterates through lookups in the hidden div and then replaces the div holder for Lookup with its innerhtml
			...
			for (var i = 0; i < lookupControls.length - 1; i++) {
				if ((vSelector.get_dataValue() == lookupControls[i] && $get("radioVariable").checked == "1") || (aSelector.get_dataValue() == lookupControls[i] && $get("radioVariable").checked == "0")) {
		...
		function clearLookupControl() {
			...
			//Special function to clear the Lookup Control
			if (!isNullOrEmptyString(_currentLookupId)) {
				...
				if (IsDataSlugSpecified(vLookup)) {
					...
					//Clear the value in the hidden div if the user changes the variable. this is keep consistent with other behavior
					// Using Lookups Clear function to clear the values.
					Mscrm.FormControlInputBehavior.GetBehavior(_currentLookupId).Clear(false);
				}
			}
			//Clear the inner Html to hide the Lookup Control
			var lookUpDivElement = $get("variableValueLookup_d");
		...
		function setLookupAttributeMappings() {
			...
			// Set the attribute type mapping to lookup specifically for lookup as the type of lookup can change every time the variable is changed
		...
		function replaceSlugXmlBindingsForLookup(slugXml, currentVariableType) {
			// replace the <_currentLookupId> with <variableValueLookup> to keep the Xml sent to the server consistent and clean.
			if (currentVariableType == LOCID_VARIABLE_LOOKUP) {
				slugXml = slugXml.replace("<" + _currentLookupId + ">", "<variableValueLookup>");
				slugXml = slugXml.replace("</" + _currentLookupId + ">", "</variableValueLookup>");
				slugXml = slugXml.replace(CrmEncodeDecode.CrmXmlEncode("<" + _currentLookupId + "DefaultValueControl"), CrmEncodeDecode.CrmXmlEncode("<variableValueLookupDefaultValueControl"));
				slugXml = slugXml.replace(CrmEncodeDecode.CrmXmlEncode("</" + _currentLookupId + "DefaultValueControl>"), CrmEncodeDecode.CrmXmlEncode("</variableValueLookupDefaultValueControl>"));
