...
<html>
<head>
<title><loc:Text ResourceId="Web._grid.cmds.dlg_addquota.aspx_9" runat="server"/></title>
...
aLkupCtrlItms.push(new LookupControlItem(grdQuota.oTransCurId, TransactionCurrency, grdQuota.oTransCurName));
...
if (_a.length == 1)
{
oXmlHttp.open("POST", Mscrm.CrmUri.create("/_grid/cmds/getquotas.aspx?id=" + _a[0] + "&y=" + oYear).toString(), false);
}
else
{
...
{
oInput = XUI.Html.DomUtils.GetFirstChild(XUI.Html.DomUtils.GetFirstChild(grdQuota.rows[i].cells[4]).rows[0].cells[1]);
...
{
xmlhttp.open("POST", Mscrm.CrmUri.create("/_grid/cmds/dlg_addquota.aspx?iTotal=1&id=" + _a[i]).toString(), false);
...
{
xmlhttp.open("POST", Mscrm.CrmUri.create("/_grid/cmds/dlg_addquota.aspx?iTotal=1&id=" + quotaForm.txtSPId.value).toString(), false);
Mscrm.Utilities.setResponseTypeToMSXml(xmlhttp);
SetTokenInHeader(xmlhttp, Mscrm.CrmUri.create("/_grid/cmds/dlg_addquota.aspx"));
boSuccess = Mscrm.Utilities.safeHttpSend(xmlhttp, xmlQuotas);
}
...
<tr height="30">
<td width="220"><label for="transCurId">
