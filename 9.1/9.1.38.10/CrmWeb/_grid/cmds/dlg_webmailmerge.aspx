...
<%if (!String.IsNullOrEmpty(objectId)) { %>
queryString += "&oid=" + CrmEncodeDecode.CrmUrlEncode(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(objectId) %>);
<%} %>
...
if (IsNull(languageid))		{ languageid = <%= defaultLanguage %>;	}
if (IsNull(layoutXml))		{ layoutXml = "";	}
if (IsNull(templateId))		{ templateId = "";	}
if (!radCurrentSelection.checked)	{ _aIds = "";	}
var quickCampaingName = "";

if (IsNull(_aIds))			{ _aIds = ""; }
if (IsNull(_oGridXml))		{ _oGridXml = ""; }
...
try
{
runMailMergeInCurrentProcess = getOutlookHostedWindow().DoMailMerge(<%= objectTypeCode %>,<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(objectId) %>, <%= lookupObjectTypeCode %>, languageid, templateId, currentPage, _aIds, _oGridXml, layoutXml, mergeType, quickCampaingName);
}
catch (e)
{
...
_oMailMerge.DoMailMerge(<%= objectTypeCode %>,<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(objectId) %>, <%= lookupObjectTypeCode %>, languageid, templateId, currentPage, _aIds, _oGridXml, layoutXml, mergeType, quickCampaingName);
}

<%} else {%>

$get('downloadMailMerge').action = Mscrm.CrmUri.create("/tools/mailmerge/download.aspx?" + queryString).toString();
...
var fetchXml	=	"<fetch><entity name=\"" + CrmEncodeDecode.CrmXmlAttributeEncode(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(lookupObjectTypeName) %>) + "\"><filter></filter></entity></fetch>";
...
var oURL = Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/AddColumns.aspx?mode=select&frommailmerge=1&maxcolumns=" + CrmEncodeDecode.CrmUrlEncode(maxSelectedColumns));
...
<loc:Text ResourceId="web._grid.cmds.dlg_webmailmerge.aspx_language" runat="server" /></td>
<td>
<sdk:LanguagePicker id="Language" AutoRegisterClientComponent="false" runat="server" />
<script language="javascript">crmCreate(Mscrm.FormInputControl.SelectInputBehavior, {}, null, null, $get('Language'));</script>
</td>
</tr>
</table>
</span>
...
<loc:Text ResourceId="web._grid.cmds.dlg_webmailmerge.aspx_mergetype" runat="server" /></label></td>
</tr>
<tr>
<td>
...
<loc:Text ResourceId="Web._grid.cmds.dlg_webmailmerge.aspx_personal" runat="server" /></label>
</td>
<td>
...
<loc:Text ResourceId="Web._grid.cmds.dlg_webmailmerge.aspx_Select_query" runat="server" /></label>
</td>
</tr>
</table>
</td>
...
<label class="ms-crm-Bold-Header">
<loc:Text ResourceId="Web._grid.cmds.dlg_webmailmerge.aspx_FieldSelect_heading" runat="server" /></label>
</td>
</tr>
<tr>
<td><loc:Text ResourceId="Web._grid.cmds.dlg_webmailmerge.aspx_FieldSelect_description" runat="server"/></td>
</tr>
<tr>
<td>
<table width="100%">
