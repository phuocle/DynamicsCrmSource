...
<%@ Import Namespace="Microsoft.Crm.Web.Tools.SystemCustomization.BusinessRules.ViewModels" %>
...
var isAsyncMergeApiFCBEnabled = false;

var radio1;
var radio2;
var firstLookupElementDiv;
var secondLookupElementDiv;
var notification;

Sys.Application.add_load(function () {
isAsyncMergeApiFCBEnabled = Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.EnableAsyncMergeAPI);
...
firstLookupElementDiv = document.querySelectorAll("#MergeRecordOne_d div.ms-crm-Lookup");
secondLookupElementDiv = document.querySelectorAll("#MergeRecordTwo_d div.ms-crm-Lookup");
notification = $get("Notification0");
var radioButton1 = document.getElementById("radio1");
if(radioButton1)
{
if(radioButton1.checked)
{
if(firstLookupElementDiv && firstLookupElementDiv.length > 0)
{
firstLookupElementDiv[0].setAttribute("tabindex","1");
}
}
}
...
{
if(secondLookupElementDiv && secondLookupElementDiv.length > 0)
{
secondLookupElementDiv[0].setAttribute("tabindex","1");
}
}
}
...
function LookupRadioOnChange()
{
var radioButton1 = document.getElementById("radio1");
var radioButton2 = document.getElementById("radio2");
if(firstLookupElementDiv && firstLookupElementDiv.length > 0 && secondLookupElementDiv && secondLookupElementDiv.length > 0)
{
if(radioButton1.checked)
{
firstLookupElementDiv[0].setAttribute("tabindex","1");
...
if (isAsyncMergeApiFCBEnabled) {
doMerge($get("parentingCheckBox").checked ? 1 : 0);
}
else
{
...
window.setTimeout("doMerge(1)", 100);
}
}


function doMerge(parentCheck)
{
var sMasterId;
var sSubordinateId;
...
var result = callMerge(sMasterId, sSubordinateId, parentCheck);
...
{
if (isAsyncMergeApiFCBEnabled) {
var url = Mscrm.CrmUri.create("/_grid/cmds/dlg_mergerecordsconfirmation.aspx");
var dialogOptions = new Xrm.DialogOptions;
dialogOptions.height = 200;
dialogOptions.width = 600;
Xrm.Internal.openDialog(url.toString(), dialogOptions, null, null, mergeSucessMessageDialogCallback);
}
else
{
alert(LOCID_MERGE_SUCCESS_MESSAGE);
mergeSucessMessageDialogCallback(true);
}
}
else
{
...
mergeSucessMessageDialogCallback = function (retValue) {
if (retValue) {
...
<% int timeout = (int)SettingsHelper.RetrieveDeploymentSettingWithOrgOverride("CascadeOperationsHttpTimeoutInSeconds", Constants.CascadeOperationsHttpTimeoutInSecondsDefault, new ExecutionContext(Microsoft.Crm.Application.Security.UserInformation.Current.OrganizationId));
if (timeout > Server.ScriptTimeout)
{
Server.ScriptTimeout = timeout;
}

using (Microsoft.Crm.Application.Utility.KeepAlive keepAlive = new Microsoft.Crm.Application.Utility.KeepAlive(Response))
{
keepAlive.KeepAliveData = Microsoft.Crm.Application.Utility.KeepAlive.GetKeepAliveData(Response);
keepAlive.Start();
try
{ %>
...
if (uri && uri.get_query() && uri.get_query()["appid"])
{
_commandGenerateForm.setParameter("appid", uri.get_query()["appid"]);
}
...
<input tabindex="1" class="radio" type="radio" name="type" onclick="changeRecord(this);" onchange="LookupRadioOnChange();" id="radio1" checked hidefocus="true" wasChecked="1" title="<loc:Text Encoding='HtmlAttribute' ResourceId='MergeRecords.SelectMaster.Title' runat='server'/>" aria-label="<loc:Text ResourceId='Merge_Master_Record_Field' runat='server'/> <loc:Text Encoding='HtmlAttribute' ResourceId='MergeRecords.SelectMaster.Title' runat='server'/>"/>
</td>
...
<input tabindex="1" class="radio" type="radio" name="type" onclick="changeRecord(this);" onchange="LookupRadioOnChange();" id="radio2" hidefocus="true" wasChecked="0" title="<loc:Text Encoding='HtmlAttribute' ResourceId='MergeRecords.SelectSubordinate.Title' runat='server'/>" aria-label="<loc:Text ResourceId='Merge_Master_Record_Field' runat='server'/> <loc:Text Encoding='HtmlAttribute' ResourceId='MergeRecords.SelectSubordinate.Title' runat='server'/>"/>
</td>
...
<% if (Microsoft.Crm.Caching.FeatureEnabledHelper.IsFeatureEnabled(Microsoft.Crm.FeatureControl.Current.Features.EnableAsyncMergeAPI, Microsoft.Crm.Application.Security.UserInformation.Current)) { %>
<tr>
<td></td>
<td>
<input type="checkbox" checked="checked" tabindex="1" id="parentingCheckBox" class="checkbox" />
</td>
<td class="dlg_merge_td_SelectData">
<label for="parentingCheckBox">
<loc:Text ResourceId="Merge_AsyncOperation_ParentingCheck_Label" runat="server" />
</label>
</td>
</tr>
<% } %>
