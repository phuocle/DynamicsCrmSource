...
<head>
    ...
    <script language="JavaScript" type="text/javascript">
        ...
        function applychanges() {
            if ($find('crmGridControl').get_innerGrid().get_numberOfRecords() == 0) {
                closeWindow();
                return;
            }
            ...
            if (selectedItem.length == 0) {
                alert(LOCID_MERGE_ATLEASETONE_ERROR);
                return;
            }
        ...
        function processMergeMultiple() {
            ...
            case "merge":
                if (total > 0) {
                    success = String.format(LOCID_MERGE_SUCCESS, total, title);
                }
                if (_errorCount > 0) {
                    fail = String.format(LOCID_MERGE_FAIL, _errorCount);
                }
                break;

            case "associatechild":
                if (total > 0) {
                    success = String.format(LOCID_PARENT_CHILD_ASSCO_SUCCESS, total + 1, title);
                }
                if (_errorCount > 0) {
                    fail = String.format(LOCID_PARENTCHILD_NOSUCC_FAILS, _errorCount);
                }
                if (total == 0 && _errorCount > 0) {
                    fail = String.format(LOCID_PARENTCHILD_NOSUCC_FAILS, _errorCount);
                }
                break;
            }
                ...
                for (var prop in _errors) {
                    try {
                        if (_errors.hasOwnProperty(prop)) {
                            ...
                            err_summary = String.format(LOCID_ERR_FAIL1_FAIL2,
                                err_summary,
                                LOCID_ERR_MSG,
                                var_LOCID_ERR_MSG);
        ...
        function setFilters() {
            ...
            if (!IsNull(oArgs)) {
                ...
                if (!IsNull(XUI.Xml.SelectSingleNode(fetchXmlDoc, "fetch/entity/filter", null))) {
                    XUI.Xml.SelectSingleNode(fetchXmlDoc, "fetch/entity", null)
                        .removeChild(XUI.Xml.SelectSingleNode(fetchXmlDoc, "fetch/entity/filter", null));
                }

                XUI.Xml.SelectSingleNode(fetchXmlDoc, "fetch/entity", null)
                    .appendChild(XUI.Xml.SelectSingleNode(filterXml, "filter", null));
        ...
        function getFilterCondition() {
            ...
            case "merge":
                filterXml =
                    "<filter><condition attribute='statecode' operator='eq' value='0' /><condition attribute='incidentid' operator='in'>";
                break;
            case "associatechild":
                filterXml =
                    "<filter><condition attribute='statecode' operator='eq' value='0' /><condition attribute='parentcaseid' operator='null' /><condition attribute='incidentid' operator='in'>";
                break;
            }
            var oArgs = getDialogArguments();
