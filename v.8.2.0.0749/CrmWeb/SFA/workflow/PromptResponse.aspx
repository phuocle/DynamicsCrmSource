...
<script language="Javascript" type="text/javascript">
    ...
    function compareReturnXmlInitialReturnXml(returnXML) {
        ...
        (decodeInitialReturnXml.replace("</slugelement><slugelement type=\"primitive\" value=\"&#13;&#10;\"/>",
                "</slugelement>") !=
            decodeReturnXML);
        returnValue = returnValue &&
        (decodeInitialReturnXml
            .replace("</slugelement><slugelement type=\"primitive\" value=\"&#13;&#10;&#13;&#10;\"/>",
                "</slugelement>") !=
            decodeReturnXML);
        return returnValue;
    }
    ...
    function save(sender, args) {
        ...
        if ((Mscrm.FormControlInputBehavior.GetBehavior("promptText").get_dataValue() == null) &&
            !IsDataSlugSpecified($get("promptText"))) {
            alert(LOCID_REQUIRED_FIELD);
            return;
        }
        if (isCrmType) {
        ...
        if (IsDataSlugSpecified(control)) {
            ...
            if (IsDateControl(control))
                sXml += "<" +
                    controlId +
                    ">" +
                    (dataValue != null ? CrmEncodeDecode.CrmXmlEncode(FormatDateTime(dataValue)) : "") +
                    "</" +
                    controlId +
                    ">";
            else
                sXml += "<" +
                    controlId +
                    ">" +
                    (dataValue != null ? CrmEncodeDecode.CrmXmlEncode(dataValue) : "") +
                    "</" +
                    controlId +
                    ">";
        }
    ...
    function GenerateResponseXml(sVal) {
        ...
        case LOCID_CONTAINER_NONE:
            sXml += "<responsecontainer name=\"None\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseMetadataBound();
            break;
        case LOCID_CONTAINER_TEXT:
            sXml += "<responsecontainer name=\"Textbox\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
        ...
        case LOCID_CONTAINER_RADIO:
            sXml += "<responsecontainer name=\"Radio\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseDetailsXml();
        ...
        case LOCID_CONTAINER_PICKLIST:
            sXml += "<responsecontainer name=\"Picklist\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseDetailsXml();
            sXml += GenerateOptionSetXml();
        ...
        case LOCID_CONTAINER_MULTILINETEXT:
            sXml += "<responsecontainer name=\"MultilineText\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseDetailsXml();
            sXml += GenerateDefaultValueXml(sVal);
        ...
        case LOCID_CONTAINER_DATEONLY:
            sXml += "<responsecontainer name=\"DateOnly\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseDetailsXml();
            sXml += GenerateDefaultValueXml(sVal);
        ...
        case LOCID_CONTAINER_DATETIME:
            sXml += "<responsecontainer name=\"DateTime\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += GenerateResponseDetailsXml();
            sXml += GenerateDefaultValueXml(sVal);
        ...
        case LOCID_CONTAINER_LOOKUP:
            sXml += "<responsecontainer name=\"Lookup\">" +
                CrmEncodeDecode.CrmXmlEncode(responseType) +
                "</responsecontainer>";
            sXml += "<responsetype name=\"Lookup\">" +
                CrmEncodeDecode.CrmXmlEncode(LOCID_RESPONSE_TYPE_LOOKUP) +
                "</responsetype>";
    ...
    function GenerateResponseLoggingXml() {
        return "<responselogging>" +
            CrmEncodeDecode.CrmXmlEncode($get('radioLogResponseYes').checked ? 1 : 0) +
            "</responselogging>";
    }
    ...
    function handleResponseTypeChangeForDateTime(responseType) {
        ...
        populateDateAndLookupOptions();
        var oDataTypeCtrl = Mscrm.FormControlInputBehavior.GetBehavior("dataTypeSelector");
        oDataTypeCtrl.set_dataValue((responseType == LOCID_CONTAINER_DATETIME)
            ? LOCID_RESPONSE_TYPE_DATETIME
            : LOCID_RESPONSE_TYPE_DATEONLY);
    ...
    function hideDateAndLookupOptions() {
        ...
        if (!IsNull(dataTypeSelector)) {
            ...
            for (var i = 0; i < dataTypeMap.length; i++) {
                var key = dataTypeMap[i].key;
                if (key == LOCID_RESPONSE_TYPE_DATEONLY ||
                    key == LOCID_RESPONSE_TYPE_DATETIME ||
                    key == LOCID_RESPONSE_TYPE_LOOKUP) {
                    continue;
                } else {
                    var value = dataTypeMap[i].value;
                    if (!IsNull(value) && value.length > 0) {
    ...
    function queryVariableChanged() {
        ...
        if (oQuerySelectCtrl.get_dataValue() != null) {
            ...
            var oXsl = null;
            XUI.Xml.Load(Mscrm.CrmUri.create("/SFA/workflow/queryAttribute.xsl").toString(),
                false,
                function(xmlDoc) { oXsl = xmlDoc },
                function(statusCode, xmlDoc) { oXsl = null; });
            ...
            if ($get('tableAttributes').rows.length == 0) {
                ...
                $get('divAttributes')
                    .innerHTML =
                    "<table class='ms-crm-ListArea' style='text-align:center'><tr><td class='ms-crm-List-Disabled' style='valign:middle'></td></tr></table>";
            }
        }
        modifyPreview();
    }
    ...
    function toggleOptionSetControlVisibility(staticOption) {
        $get('rowResponsePicklistValues').className = (staticOption
                        ...
                        <td class="picklistSection">
                            <table width="100%" cellspacing="0" cellpadding="5">
                                ...
                                <tr>
                                    ...
                                    <td class="picklistSection">
                                        <div id="divAttributes" style="background-color: #ffffff; height: 100%; width: 100%; overflow-y: scroll; border: 1px solid #999999;">
                                        </div>
                                    </td>
                                </tr>
                            </table>
