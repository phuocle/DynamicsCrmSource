    ...
    <script language="Javascript">
        ...
        function windowOnLoad() {
            <% if (mapValidationErrorOccurred)
               { %>
            window.parent
                .MapValidationFailureCallBackFromUpload(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(validationErrors) %>);

            <% }
               else if (!String.IsNullOrEmpty(mapXml))
               { %>
            window.parent
                .CheckForDuplicateMapResultCallBackFromUpload(CrmEncodeDecode
                    .CrmXmlEncode(<%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(mapXml) %>),
                    <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(mapName) %>,
                    <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(duplicateFound.ToString(CultureInfo.InvariantCulture)) %>,
...
<label for="fileUpload">
    <b>
        <loc:text resourceid="UploadMapFile_Label" runat="server"/>
    </b>
</label>
<br>
<br>
