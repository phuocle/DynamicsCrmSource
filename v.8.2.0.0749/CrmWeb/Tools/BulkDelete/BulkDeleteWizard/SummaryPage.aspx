<%@ Page language="c#" Inherits="Microsoft.Crm.Web.Tools.BulkDelete.SummaryPage" %>
...
<script language="javascript">
    ...
    function SummaryPageOnLoad() {
        ...
        if (WizardHasProperty("emailCc")) {
            ...
            for (var index = 0; index < ccReceipients.length; index++) {
                if (XUI.Html.GetText($get("emailNotification")) == "") {
                    ...
                    XUI.Html.SetText($get("emailNotification"),
                        XUI.Html
                        .GetText($get("emailNotification")) +
                        LOCID_EMAIL_SEPARATOR +
                        ccReceipients[index].name);
                }
            }
        }
        ...
        if (WizardHasProperty("isRecurring")) {
            ...
            if (_isRecurring == true) {
                XUI.Html.SetText($get("schedulingOption"),
                    formatString(LOCID_JOB_RECURRING, startDate, _iNumDays, startTime));
            } else {
        ...
        if (WizardHasProperty("runNow") &&
            WizardGetProperty("entityName") != null &&
            WizardGetProperty("entityName") == "asyncoperation") {
    ...
    function moveNext() {
        ...
        var startDateTime = FormatUtcDate(new Date(WizardGetProperty("startDateTime")));

        var command = new RemoteCommand("BulkDelete", "SubmitBulkDeletionJob");
        ...
        command.SetParameter("jobName", _jobName);
        command.SetParameter("entityName", WizardGetProperty("entityName"));
        command.SetParameter("sendEmailNotification", _sendEmail);
        command.SetParameter("recipients", _sCCRecipients);
        command.SetParameter("fetchXml", fetchXml);
        command.SetParameter("startDateTime", startDateTime);
        command.SetParameter("isRecurring", _isRecurring);
        command.SetParameter("numberOfDays", _iNumDays);
        command.SetParameter("runNow", _runNow);
...
<frm:WizardForm id="crmForm" runat="server">
    ...
    <table width="100%" cellpadding=0 cellspacing=0>
        ...
        <tr>
            <td>
                <loc:Text ResourceId="BulkDeleteWizard.SummaryPage.JobName" runat="server"/>
            </td>
            <td align="left">
    ...
    <table width="100%" cellpadding=0 cellspacing=0>
        ...
        <tr id="notificationArea">
            <td>
                <loc:Text ResourceId="BulkDeleteWizard.SummaryPage.EmailNotofication" runat="server"/>
            </td>
            <td align="left">
    ...
    <table width="100%" cellpadding=0 cellspacing=0>
        <tr>
            <td>
                <loc:Text ResourceId="BulkDeleteWizard.SummaryPage.Note" runat="server"/>
            </td>
        </tr>
    </table>
</frm:WizardForm>
