...
<script language="javascript" type="text/javascript">
    ...
    function UpdateEntityDropDowns() {
        ...
        for (var i = 0; i < processNodes.length; i++) {
            ...
            if (processCode == ImportEntityProcessCode_Ignore) {
                ...
                var newTargetEntityName = processNode
                    .getAttribute((processCode == ImportEntityProcessCode_Create)
                        ? "TargetEntityRef"
                        : "TargetEntityName");
                ...
                if (processCode == ImportEntityProcessCode_Create) {
                    ...
                    if (ReturnOptionObject(oSelectElement, newTargetEntityName) == null) {
                        var newTargetEntityDisplayName = XUI.Xml
                            .GetText(XUI.Xml
                                .SelectSingleNode(mapXmlDoc,
                                    "/Map/Customizations/Entities/Entity[@Id='" +
                                    newTargetEntityName +
                                    "']/CustomizationXml/DisplayName",
                                    null));
                        AddOptionToAllSelectElements(newTargetEntityName, newTargetEntityDisplayName);
                    }
                }

                if (IndexOf(oSelectElement, newTargetEntityName) == -1) {
    ...
    function WantToAvoidEntityRemapping() {
        var importType = WizardGetProperty("ImportType");
        if (WizardHasProperty("EntityPageLoaded") && importType == ImportType_XlsxCreateUpdate)
            return true;

        WizardSetProperty("EntityPageLoaded", true);
    ...
    function handlePageChange(isNext, bWaitForCompletion) {
        ...
        var ignoreEntities = XUI.Xml.SelectNodes(mapXmlDoc,
            "/Map/EntityMaps/EntityMap[(@ProcessCode='" + ImportEntityProcessCode_Ignore + "') and not(@Unused)]",
            null);
        var allEntities = XUI.Xml.SelectNodes(mapXmlDoc, "/Map/EntityMaps/EntityMap[not(@Unused)]", null);
        var entitiesMappedToUser = XUI.Xml.SelectNodes(mapXmlDoc,
            "/Map/EntityMaps/EntityMap[@TargetEntityName='systemuser' and not(@Unused)]",
            null);
        ...
        if (allEntities.length == (ignoreEntities.length + entitiesMappedToUser.length) &&
            isNext == true &&
            !hasSystemUserCreatePrivilege) {
        ...
        if (hasMapChanged || WizardGetProperty("EntityMapPageWSError") == true) {
            ...
            if (bWaitForCompletion == true) {
                ...
                CallImportWebService("GetImportMapXml",
                    propertiesToPost,
                    function(oResult) { GetImportMapXmlCallback(oResult, iNavigateEnum); });
                return false;
            }
        } else {
            return true;
    ...
    function RemoveAttributeNodesFromMapXml() {
        ...
        for (var i = 0; i < allSelectElements.length; i++) {
            ...
            var sourceEntityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/EntityMaps/EntityMap[@InputFileId='" + element.id + "']",
                null);
            ...
            var customizationNode = XUI.Xml
                .SelectSingleNode(mapXmlDoc,
                    "/Map/Customizations/Entities/Entity[LogicalName='" + elementComponent.get_defaultValue() + "']",
                    null);

            if (!IsNull(customizationNode)) {

                var mappedEntity = XUI.Xml.SelectNodes(mapXmlDoc,
                    "/Map/EntityMaps/EntityMap[@TargetEntityName = '" +
                    elementComponent.get_defaultValue() +
                    "' and not(@Unused)]",
                    null);
    ...
    function RemoveRedundantNewEntityCustomizations() {
        ...
        var customEntities = XUI.Xml.SelectNodes(mapXmlDoc,
        ...
        for (var i = 0; i < customEntities.length; i++) {
            ...
            var totalSourceToNewEntityMappings = XUI.Xml
                .SelectNodes(mapXmlDoc, "/Map/EntityMaps/EntityMap[@TargetEntityRef='" + entityId + "']", null);
    ...
    function removeLookupsToNewEntity(entityId, mapXmlDoc) {
        var attributeCusotmizationToRemove = XUI.Xml
            .SelectNodes(mapXmlDoc,
                "/Map/Customizations/Entities/Entity/Attributes/Attribute[CustomizationXml/attribute/type/entity='" +
                entityId +
                "']",
                null);
        for (var i = 0; i < attributeCusotmizationToRemove.length; i++) {
            ...
            var entityLogicalName = XUI.Xml
                .SelectSingleNode(attributeCusotmizationToRemove[i].parentNode.parentNode, "LogicalName", null);
            var attributeMappingToModify;

            if (IsNull(entityId) == false) {

                attributeMappingToModify = XUI.Xml
                    .SelectSingleNode(mapXmlDoc,
                        "/Map/EntityMaps/EntityMap[@TargetEntityRef='" +
                        entityId +
                        "']/AttributeMaps/AttributeMap[SourceAttributeId='" +
                        attributeId +
                        "']",
                        null);
            } else {
                ...
                attributeMappingToModify = XUI.Xml
                    .SelectSingleNode(mapXmlDoc,
                        "/Map/EntityMaps/EntityMap[@TargetEntityName='" +
                        XUI.Xml.GetText(entityLogicalName) +
                        "']/AttributeMaps/AttributeMap[SourceAttributeId='" +
                        attributeId +
                        "']",
                        null);
            ...
            if (IsNull(attributeMappingToModify) == false) {
                ...
                XUI.Xml.SetText(XUI.Xml.SelectSingleNode(attributeMappingToModify, "ProcessCode", null),
                    ImportEntityProcessCode_Unmapped);

                var targetAttributeRefNode = XUI.Xml
                    .SelectSingleNode(attributeMappingToModify, "TargetAttributeRef", null);
    ...
    function EntityMappingChanged(fileId) {
        ...
        if (selectElement.value == ENTITY_MAPPING_ACTION_CREATENEW) {
            ...
            var sourceEntityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/EntityMaps/EntityMap[@InputFileId='" + fileId + "']",
                null);

            XUI.Html.SetText(document.getElementById(fileId + "_OldValue"), selectElement.value);

            if (selectElement.value != ENTITY_MAPPING_ACTION_SELECT &&
                selectElement.value != ENTITY_MAPPING_ACTION_IGNORE) {
            ...
            } else if (selectElement.value != ENTITY_MAPPING_ACTION_IGNORE) {
    ...
    function FillCreateEntityDiv(selectElement, isReopen) {
        ...
        if (isReopen == true) {
            ...
            var customEntityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "Map/Customizations/Entities/Entity[@Id='" + targetEntityRef + "']",
                null);


            document.getElementById("inputInternalName").value = XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/DisplayName", null));
            document.getElementById("inputInternalNamePlural").value = XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/CollectionName", null));
            document.getElementById("inputPrimaryField").value = XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/PrimaryKeyDisplayName", null));
            document.getElementById("buttonCreateOK").disabled = false;
        } else {
            document.getElementById("buttonCreateOK").onclick = CreateNewEntity;

            var sourceEntityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/EntityMaps/EntityMap[@InputFileId='" + _entityDivFileId + "']",
                null);
            var inputFileName = sourceEntityNode.getAttribute("InputFileName");
            document.getElementById("inputInternalName")
                .value = Trim(inputFileName.slice(0, inputFileName.lastIndexOf(".")).substring(0, 40));
    ...
    function ShowCreateEntityDiv() {
        ...
        if (document.getElementById("createEntityDiv").parentNode.offsetHeight <= height)
            ...
            if ((rowPosRelativeToMapArea + height) >
                document.getElementById("createEntityDiv").parentNode
                .offsetHeight &&
                (rowPosRelativeToMapArea - height - oRow.offsetHeight) > 0)
    ...
    function ModifyNewEntity() {
        ...
        var customEntityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
            "Map/Customizations/Entities/Entity[@Id='" + oldTargetEntityName + "']",
            null);
        var referredNodes = XUI.Xml.SelectNodes(mapXmlDoc,
            "/Map/EntityMaps/EntityMap[@TargetEntityRef='" + oldTargetEntityName + "']",
            null);
        var referredLookupNodes = XUI.Xml.SelectNodes(mapXmlDoc,
            "/Map/Customizations/Entities/Entity/Attributes/Attribute/CustomizationXml/attribute/type[name='lookup' and entity='" + oldTargetEntityName + "']",
            null);
        ...
        if (newTargetEntityDisplayName != oldTargetEntityDisplayName) {
            ...
            for (var i = 0; i < allSelectElements.length; i++) {
                ...
                oSelectElementComponent.AddOption(newTargetEntityDisplayName,
                    newTargetEntityName,
                    LOCID_OPTGROUP_NEWENTITIES);
        ...
        XUI.Xml.SetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/CollectionName", null),
            document.getElementById("inputInternalNamePlural").value);
        XUI.Xml.SetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/PrimaryKeyDisplayName", null),
            document.getElementById("inputPrimaryField").value);
        XUI.Xml.SetText(XUI.Xml.SelectSingleNode(customEntityNode, "CustomizationXml/DisplayName", null),
            newTargetEntityDisplayName);
    ...
    function ValidateCreateEntityContents(newDisplayName, newPluralName, oldTargetEntityId) {
        ...
        var customEntitiesNode = XUI.Xml.SelectNodes(mapXmlDoc,
            "/Map/Customizations/Entities/Entity/CustomizationXml",
            null);
    ...
    function CreateNewEntity() {
        ...
        var entityNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
            "/Map/EntityMaps/EntityMap[@InputFileId='" + _entityDivFileId + "']",
            null);
    ...
    function AddOptionToAllSelectElements(newTargetEntityName, displayName) {
        ...
        for (var i = 0; i < allSelectElements.length; i++) {
            ...
            if (oSelectActions.length == 2) {
                ...
                Mscrm.FormControlInputBehavior.GetBehavior(oSelectElement.id)
                    .AddOption(displayName, newTargetEntityName, LOCID_OPTGROUP_NEWENTITIES);
            }
        }
    }
    ...
    function OKbuttonDisabledCheck() {
        if (Trim(document.getElementById("inputInternalNamePlural").value).length > 0 &&
            Trim(document.getElementById("inputPrimaryField").value).length > 0 &&
            Trim(document.getElementById("inputInternalName").value).length > 0) {
            document.getElementById("buttonCreateOK").disabled = false;
        } else {
            document.getElementById("buttonCreateOK").disabled = true;
        }
    ...
    function InternalNamePluralChanged() {
        document.getElementById("inputInternalNamePlural")
            .value = Trim(document.getElementById("inputInternalNamePlural").value);
    }
            ...
            <div class="mscrm-iw-Border mscrm-iw-DivGeneric  ms-crm-absolutePosition">
                <div class="ms-crm-EntityMapPageContentHeader">
                    <div class="ms-crm-EntityMapPageContentHeader_Cell1" tabindex=0>
                        <div class="ms-crm-EntityMapPageContentHeader_Cell1Text">
                            <nobr>
                            <nobr>
                                                ...
                                                <tr class="mscrm-iw-AMP-PopupHeader">
                                                    ...
                                                    <td>
                                                        <span class="mscrm-iw-AMP-CrossImage">
                                                            <a id="closeCreateDivLink" href="javascript:CancelCreateNewEntity();">
                                                                <img src="/_imgs/importwizard_cross.gif"/>
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                ...
                <tr>
                    <td valign='middle' align='center'>
                        ...
                        <label id="progressMessage">
                            <loc:Text ResourceId="ImportWizard.EntityMapPage.ProgressMessage" Encoding="None" runat="server"/>
                        </label>
                    </td>
                </tr>
            </table>
