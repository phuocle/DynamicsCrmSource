...
<script language="javascript" type="text/javascript">
    ...
    function SubmitImportJob() {
        var propertiesToPost = new Array("ImportId",
            ...
            "DefaultOwnerType",
            "ImportType",
            "HeaderColumnIndexesToBeIgnored");
    ...
    function customizationsDone(oResult) {
        if (oResult.Success == true) {
            ...
            if (cusotomizationsLeft == NoErrorCustomization) {
                ...
                if (cusotomizationsLeft == ErrorAttributeCustomization ||
                    cusotomizationsLeft == ErrorPicklistCustomization) {
    ...
    function checkForCustomizationErrors(mapXml) {
        ...
        var publishErrorNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
            "/Map/Customizations/PublishError/ErrorMessage",
            null);
        ...
        var picklistCustomizationErrorNode = XUI.Xml
            .SelectSingleNode(mapXmlDoc, "/Map/Customizations/PicklistCustomizationError/ErrorMessage", null);
        if (!IsNull(picklistCustomizationErrorNode)) {
            var entityNameNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/Customizations/PicklistCustomizationError/EntityDisplayName",
                null);
            var attribNameNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/Customizations/PicklistCustomizationError/AttributeDisplayName",
                null);
            var errorMessage = formatString(LOCID_ERROR_PICKLIST_CREATE,
                XUI.Xml.GetText(entityNameNode),
                XUI.Xml.GetText(attribNameNode),
                XUI.Xml.GetText(picklistCustomizationErrorNode));
        ...
        var attributeErrorNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
            "/Map/Customizations/Entities/Entity/Attributes/Attribute/CustomizationError/ErrorMessage",
            null);
        if (!IsNull(attributeErrorNode)) {
            var attributeCustomizationNode = attributeErrorNode.parentNode.parentNode;
            var attributeName = XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(attributeCustomizationNode, "CustomizationXml/attribute/displayname", null));
            var errorMessage = formatString(LOCID_ERROR_ATTRIBUTE_CREATION,
                attributeName,
                XUI.Xml.GetText(attributeErrorNode));

            alert(errorMessage);
            return ErrorAttributeCustomization;
        } else {
            var entityErrorNode = XUI.Xml.SelectSingleNode(mapXmlDoc,
                "/Map/Customizations/Entities/Entity/CustomizationError/ErrorMessage",
                null);
            if (!IsNull(entityErrorNode)) {
                var entityCustomizationNode = entityErrorNode.parentNode.parentNode;
                var entityName = XUI.Xml.GetText(XUI.Xml
                    .SelectSingleNode(entityCustomizationNode, "CustomizationXml/DisplayName", null));
                var errorMessage = formatString(LOCID_ERROR_ENTITY_CREATION,
                    entityName,
                    XUI.Xml.GetText(entityErrorNode));
    ...
    function customizationsPresent() {
        ...
        if (IsNull(customizationNode)) {
            ...
            var picklistCreateNode = XUI.Xml
                .SelectSingleNode(mapXmlDoc,
                    "/Map/EntityMaps/EntityMap[not(@Unused)]/AttributeMaps/AttributeMap[not(@Unused)]/PicklistMaps/PicklistMap[ProcessCode='" + ImportEntityProcessCode_Create + "']",
                    null);
            ...
            var entityCustomizationNode = XUI.Xml
                .SelectNodes(customizationNode, "Entities/Entity/CustomizationXml", null);
    ...
    function UpdateImportSettings() {
        ...
        if (!IsNull(Mscrm.FormControlInputBehavior.GetBehavior("assignTo").get_dataValue()) &&
            !IsNull(Mscrm.FormControlInputBehavior.GetBehavior("assignTo").get_dataValue()[0])) {
            WizardSetProperty("DefaultOwner",
                Mscrm.FormControlInputBehavior.GetBehavior("assignTo").get_dataValue()[0].id);
            WizardSetProperty("DefaultOwnerType",
                Mscrm.FormControlInputBehavior.GetBehavior("assignTo").get_dataValue()[0].type);
            WizardSetProperty("OwnerDisplayName",
                Mscrm.FormControlInputBehavior.GetBehavior("assignTo").get_dataValue()[0].name);
    ...
    function PageOnLoad() {
        ...
        if (importType == ImportType_Update) {
            ...
            document.getElementById("infoBalloonImage")
                .alt =
                '<% = CurrentResourceManager.GetString("ImportWizard.CustomizeImportPage.UpdateViaImportWarning") %>';
        } else {
        ...
        if (!IsNull(defaultOwner) && !IsNull(defaultOwnerType) && !IsNull(ownerName)) {
...
<frm:WizardForm id="crmForm" runat="server">
    <div id="mainBody">
        <div id="infoBalloon" class="mscrm-iw-InfoBarYellow" style="display: none">
            <img id="infoBalloonImage" class="mscrm-iw-InfoBarBalloon" src="/_imgs/importwizard_yellowwarning.gif">&nbsp;&nbsp;<span class="mscrm-iw-InfoBarText">
                <loc:Text ResourceId="ImportWizard.CustomizeImportPage.UpdateViaImportWarning" runat="server"/>
            </span>
            <br/>
        </div>
        <br/>
        ...
        <div id="duplicateDetectionSection" class="mscrm-iw-BorderedDivBlueBackground">
            <table style="width: 100%">
                ...
                <tr>
                    ...
                    <td>
                        <label for="deDupEnabled">
                            <nobr>
                                <loc:Text ResourceId="ImportWizard.CustomizeImportPage.DeDupOptionNo" runat="server"/>
                            </nobr>
                        </label>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <label for="deDupDisabled">
                            <nobr>
                                <loc:Text ResourceId="ImportWizard.CustomizeImportPage.DeDupOptionYes" runat="server"/>
                            </nobr>
                        </label>
                    </td>
                </tr>
        ...
        <div id="duplicateDetectionDisabled" style="display: none" class="mscrm-iw-BorderedDivBlueBackground">
            <img alt='' src='/_imgs/importwizardinfo.gif'/>&nbsp;&nbsp;<span class="mscrm-iw-InfoBarText">
                <loc:Text ResourceId="IW_MSG_Label_DeDup_Disabled" runat="server"/>
            </span>
        </div>
        <br/>
