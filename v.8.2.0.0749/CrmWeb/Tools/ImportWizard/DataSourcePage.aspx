...
<script language="javascript" type="text/javascript">
    ...
    function WantToSkip() {
        ...
        if ((importType == ImportType_CreateWithTemplate && WizardGetProperty("WizardMode") == 0) ||
            importType == ImportType_XlsxCreateUpdate) {
        ...
        if (dataSourceCtrl.get_isDirty() ||
            ...
            if (bWaitForCompletion) {
                ...
                CallImportWebService("GetImportMapXml",
                    propertiesToPost,
                    function(oResult) { autoMappingDone(oResult, true); });
                disableAllButtons();
                return true;
            }
        } else {
    ...
    function OnDataSourcePageResize() {
        ...
        while (mainBody.offsetHeight < wizardFormMain.offsetHeight && crmDataSource.size > 0) {
            resizingDone = true;
            crmDataSource.size = crmDataSource.size + 1;
        }

        while (mainBody.offsetHeight > wizardFormMain.offsetHeight && crmDataSource.size > 1) {
            resizingDone = true;
            crmDataSource.size = crmDataSource.size - 1;
        }
