...
*/
    ...
    div.editPage[contentEditable=true] DIV, td.ms-crm-Email-Body[contentEditable=true] DIV, div.editor[contentEditable=true] DIV { margin: 12px 0px; }
    ...
    span.dataslug.selected { border-width: 2px; }
...
<script type="text/javascript">
    ...
    function ResizeTemplateEditorAndAttacmentsGrid() {
        ...
        if (!IsNull(contentWrapper) &&
            !IsNull(templatePropertiesTable) &&
            !IsNull(templateEditor) &&
            !IsNull(gridWrapper)) {


            var templateEditorHeight = (contentWrapper.clientHeight -
                templatePropertiesTable.clientHeight -
                (!IsNull(htmlBarWrapper) ? htmlBarWrapper.clientHeight : 0));
    ...
    function showSubjectNotification() {
        ...
        var oaNot = new Array(oNotification
            .CreateNotification("SubjectDataSlugs", 3, "SubjectEditor", LOCID_ETMPLSUBJECTDATAFIELDWARN));
    ...
    function GenHtml() {
        Mscrm.FormControlInputBehavior.GetBehavior("html")
            .set_dataValue(removeSpanTags($get("TemplateEditor").innerHTML));
        Mscrm.FormControlInputBehavior.GetBehavior("subjecthtml")
            .set_dataValue(removeSpanTags($get("SubjectEditor").innerHTML));
    }
        ...
        if (!IsNull(_savedRange) && navigator.appName != 'Microsoft Internet Explorer') {
            ...
            case "paste":
                ...
                if (!IsNull(fragment)) {
                    ...
                    for (var j = 0, len = !IsNull(insertedFragment.childNodes) ? insertedFragment.childNodes.length : 0;
                        j < len;
                        j++) {
    ...
    function moveCursor(slug, isLeft, eventData) {
        ...
        if (isSlug) {
            ...
            } else if (next.nodeType !== 3 || (next.nodeType === 3 && !next.nodeValue.length)) {
                textNode = window.document.createTextNode(ThinSpace);
...
<div class="ms-crm-Form-Layout">
    <frm:HardcodedForm id="crmForm" runat="server">
        <span style="display: none">
            <sdk:LookupControl id="ownerid" runat="server"/>
        </span>
                    ...
                    <div id="tab0" class="ms-crm-Tab ms-crm-TabContainer" style="top: 0px; bottom: 0px; left: 10px; right: 10px; height: auto; width: auto">
                        <div id="contentWrapper" style="position: absolute; top: 10px; left: 10px; right: 10px; bottom: 10px; min-width: 550px">
                            <table id="templatePropertiesTable" width="100%" cellspacing="4" cellpadding="0" style="table-layout: fixed;">
                                ...
                                <tr>
                                    <td class="ms-crm-Field-Required" id="type_c">
                                        <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.addfields.aspx_269" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                                    </td>
                                    <td id="TemplateType"><%= _templateTypeHtml %></td>
                                    <td></td>
                                    <td id="languagecode_c">
                                        <label for="languagecode">
                                            <loc:MetadataText EntityType="template" AttributeName="languagecode" runat="server"/>
                                        </label>
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <td colspan="4">
                                        <sdk:TextBoxControl id="title" runat="server"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td valign="top">
                                        <label for="description">
                                            <loc:Text ResourceId="Web.Tools.FormEditor.Dialogs.field-text.aspx_289" runat="server"/>
                                        </label>
                                    </td>
                                    <td colspan="4">
                                        <sdk:TextAreaControl id="description" runat="server" Height="70px"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="5">
                                ...
                                <tr>
                                    <td class="ms-crm-Field-Required" id="subject_c">
                                        <loc:Text ResourceId="Web.CS.cases.closecase.aspx_40" runat="server"/><img src="/_imgs/frm_required.gif" alt="<loc:Text Encoding='HtmlAttribute' ResourceId='Forms.Required.AltTag' runat='server'/>"/>
                                    </td>
                                    <td colspan="4">
                                        <nobr>
                                            <div id="SubjectEditor" class="editSubject" applyformat=false contenteditable="<%= (crmForm.Disabled || crmForm.ReadOnly) ? "false" : "true" %>" style="background-color: <%= Microsoft.Crm.CrmEncodeDecode.CrmHtmlAttributeEncode(AppResourceManager.Default.GetString("Required_Fields_Background_Color")) %>" onselectstart="new Sys.UI.DomEvent(event).stopPropagation();return true;" onfocus="HandleFocus(new Sys.UI.DomEvent(event))" onblur="stripFormatting(new Sys.UI.DomEvent(event));checkForSubjectDataSlugs();" onkeyup="handleKey(new Sys.UI.DomEvent(event))" onkeypress="handleKey(new Sys.UI.DomEvent(event))" onpaste="handlePaste(new Sys.UI.DomEvent(event))"><%= _subjectHtml %></div>
                                        </nobr>
                                    </td>
                                </tr>
                            </table>
                            ...
                            <div id="tab1">
                                <table cellspacing="0" cellpadding="0" class="ms-crm-InlineTabHeader ms-crm-InlineTabBorder" id="attachmentId_header">
                                    ...
                                    <tr>
                                        <td class="ms-crm-InlineTabHeaderText" name="attachmentId" tabindex="1281">
