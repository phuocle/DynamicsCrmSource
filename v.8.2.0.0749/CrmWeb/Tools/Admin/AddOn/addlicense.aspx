...
<script language="javascript" type="text/javascript">
    ...
    function moveNext() {
        ...
        if (PageId == "PurchasePage") {
            ...
            XUI.Html.SetText($get('LicenseToAddTableHeader'),
                <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.LicenseInputPage.LicenseToAddHeaderAfterChange")) %>);
            ...
            setUserLicenseUsageInfoText(LICENSES_TO_ADD_USAGE_INFO_CONFIRM_PAGE,
                CURRENT_MONTHLY_CHARGES_USER_LICENSES_QUANTITY * 1 + quantity * 1,
                LICENSES_AVAILABLE * 1 + quantity * 1);
        } else if (PageId == "ConfirmPage") {

            WizardSetProperty("ProcessingUrl", "/tools/admin/addon/addlicensecomplete.aspx");
            var postData = CrmEncodeDecode.CrmXmlEncode($get('NumberOfLicensesToAdd').value) +
                ";" +
    ...
    function moveBack() {
        ...
        XUI.Html.SetText($get('LicenseToAddTableHeader'),
            <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.LicenseInputPage.LicenseToAddHeader")) %>);
        $get('UserInputTable').setAttribute('summary',
            <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(CurrentResourceManager.GetString("AddOnWizard.LicenseInputPage.LicenseToAddHeader")) %>);
        ...
        setUserLicenseUsageInfoText(LICENSES_TO_ADD_USAGE_INFO_PURCHASE_PAGE,
            CURRENT_MONTHLY_CHARGES_USER_LICENSES_QUANTITY,
            LICENSES_AVAILABLE);
    }
        ...
        if (!valid) {
            ...
            if (!ValidateRange(quantity)) {
                $get('InvalidLicenseCountErrorMessage')
                    .innerHTML =
                    <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(Microsoft.Crm.Application.Utility.Util.HtmlEncodeResource("AddOnWizard.LicenseInputPage.OutOfRangeAddLicenseCountErrorMessage", Microsoft.Crm.Application.Utility.Util.CreateContactSupportHtml())) %>;
            } else {
                $get('InvalidLicenseCountErrorMessage')
                    .innerHTML =
                    <%= Microsoft.Crm.CrmEncodeDecode.CrmJavaScriptEncode(Microsoft.Crm.Application.Utility.Util.HtmlEncodeResource("AddOnWizard.LicenseInputPage.InvalidAddLicenseCountErrorMessage", Microsoft.Crm.Application.Utility.Util.CreateContactSupportHtml())) %>;
            }
        } else {
            ...
            FormatToCurrencyAsync('LicensesToAddPrice',
                licensesToAddPrice.toFixed(NUMBER_OF_DIGITS_AFTER_DECIMAL_POINT));

            var updatedTotalMonthlyChargesPrice = (CURRENT_MONTHLY_CHARGES_TOTAL_PRICE * 1 +
                (CURRENT_MONTHLY_CHARGES_USER_LICENSES_UNIT_PRICE * quantity));
