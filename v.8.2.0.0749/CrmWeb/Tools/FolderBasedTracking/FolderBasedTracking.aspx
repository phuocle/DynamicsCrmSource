...
<script type="text/javascript">
    ...
    $P_CRM(document).ready(function() {
        $P_CRM('html').click(function(event) {
            ...
            if (objElement.id.indexOf('txtExchangefolder_') == -1 &&
                objElement.className.indexOf('ms-crm-folderbasedtracking-TreeView-parent-div') == -1 &&
                objElement.className.indexOf('ms-crm-navtree-node-icon') == -1 &&
                objElement.className.indexOf('tree_foldercontainer')) {
                $P_CRM('.ms-crm-folderbasedtracking-TreeView-parent-div').addClass('hidden');
            }

            if (saveStatus &&
                objElement.id.indexOf('row_') != -1 &&
                objElement.id.indexOf('_column_') != -1 &&
                objElement.id.indexOf('_lookup') != -1) {
    ...
    $P_CRM(function() {
        $P_CRM(window).bind('beforeunload',
            function() {
    ...
    window.onload = function() {
        ...
        $P_CRM('#FolderBasedTrackingDialogDescription')[0]
            .innerHTML = $P_CRM('#FolderBasedTrackingDialogDescriptionWithEmail')[0].defaultValue;
        ...
        if (tempEvent.nodeName != 'IMG' && !$P_CRM(obj).hasClass('ms-crm-folderbasedtracking-disableNode')) {
            ...
            for (var i = 0; i < objUnSelectCheckItem.length; i++) {
                if ($P_CRM(objUnSelectCheckItem[i]).hasClass('selectedItem')) {
                    ...
                    _prvSelectedId = objUnSelectCheckItem[i].childNodes[objUnSelectCheckItem[i].childNodes.length - 1]
                        .id;
            ...
            $P_CRM('#txtExchangefolder_' + treeId.split('_')[1] + '_container')
                .removeClass('ms-crm-ExchangeFolder-Error');
            $P_CRM('#' + treeId).addClass('hidden');
    ...
    function SaveMappings() {
        if (Validate() && !$P_CRM('#idSave div').hasClass('ms-crm-folderbasedtracking-newfoldermapping-disabledstate')
            ...
            for (var i = 0; i < trackedFolderIds.length; i++) {
                ...
                var regardingObject = Sys.Application
                    .findComponent('row_' + rowId + '_column_2_lookup$PresenceLookupUIBehavior')
                    .Items(false, false, false, false, true)

                var regardingObjectText = Sys.Application
                    .findComponent('row_' + rowId + '_column_2_lookup$PresenceLookupUIBehavior').getLookupSearchText();
                if ((regardingObject.length == 0 && regardingObjectText != '') ||
                    (regardingObject.length > 0 && regardingObjectText != regardingObject[0].name)) {
                ...
                if (regardingObject.length > 0) {
                    folderBasedTrackingMapping = new Mscrm.EmailConnector
                        .FolderBasedTrackingMapping(folderId,
                            FolderName,
                            regardingObject[0].type,
                            regardingObject[0].id,
                            regardingObject[0].name);
                } else {
                    folderBasedTrackingMapping = new Mscrm.EmailConnector
                        .FolderBasedTrackingMapping(folderId, FolderName, '', '', '');
                }
            ...
            window.setTimeout(Function.createDelegate(this,
                    function() {
                        var result = submitJob
                            .updateMappings($P_CRM('#MailboxId')[0].value, createUpdateMappingCollection);
    ...
    function Validate() {
        ...
        $P_CRM('#allFolderMappings tr:not(.hidden) input.ms-crm-Exchangefolder').each(function() {
            if (this.id.indexOf('txtExchangefolder_') != -1) {
                if (this.value == '') {
                    $P_CRM('#txtExchangefolder_' + this.id.split('_')[1] + '_container')
                        .addClass('ms-crm-ExchangeFolder-Error');
...
<style type="text/css">
    ...
    .ms-crm-folderbasedtracking-table-row-hover { background-color: #F6F6F6; }
    ...
    .ms-crm-folderbasedtracking-table-deleteimage-column { width: 4.167em; }
    ...
    .ms-crm-folderbasedtracking-base-color { color: #444444 !important; }

    .ms-crm-folderbasedtracking-newfoldermapping-disabledstate { color: #C0C0C0; }
    ...
    .ms-crm-folderbasedtracking-disableNode { color: Silver; }
    ...
    .ms-crm-ExchangeFolder-Error { border: solid red 0.09em !important; }

    .ms-crm-Image-VAlign { vertical-align: middle; }

    .ms-crm-SuccessMessage { color: #029002 !important; }
    ...
    a.NavigateLink { text-decoration: underline !important; }
    ...
    DIV.ms-crm-Lookup { height: 18px !important; }
    ...
    .ms-crm-FolderBasedTracking-Save_16 { vertical-align: middle; }
</style>
<title>
...
<div class="ms-crm-folderbasedtracking">
    <a class="ms-crm-folderbasedtracking-savebutton-hover ms-crm-folderbasedtracking-savebutton" href="#" id="idSave" title="<loc:Text Encoding='HtmlAttribute' ResourceId='FolderBasedTracking.Save.Button' runat='server'/>" onclick="SaveMappings()">
        ...
        <div class="ms-crm-folderbasedtracking-savebutton-savetext">
            <loc:Text resourceid="FolderBasedTracking.Save.Button" runat="server"/>
        </div>
    </a>
