    ...
    <table cellpadding="0" cellspacing="5" width="100%" style="table-layout: fixed;">
      ...
      <tr>
        <td class="ms-crm-Field-Normal">
          <xsl:value-of select="$Tools.FormEditor.Dialogs.fieldlocations.xsl_1" />
        </td>
        <td>
          <select id="SelectTab" class="ms-crm-SelectBox" name="TabName" onchange="UpdateSections(this)">
            <xsl:if
              test="tab[@name = $tabName and @locklevel and @locklevel != '0'] or tab[@name = $tabName]/sections/section[@name = $sectionName and @locklevel and @locklevel != '0']">
              <xsl:attribute name="disabled"></xsl:attribute>
            </xsl:if>
            <xsl:for-each
              select="tab[sections/section[not(@locklevel) or @locklevel = '0'] and (not(@locklevel) or @locklevel = '0')]">
              <option>
      ...
      <tr>
        <td class="ms-crm-Field-Normal">
          <xsl:value-of select="$Tools.FormEditor.Dialogs.fieldlocations.xsl_36" />
        </td>
        <td id="Sections">
                  ...
                  <xsl:when test="tab/sections/section[@name = $sectionName and @locklevel and @locklevel != '0']">
                    <xsl:for-each select="tab[@name = $tabName]/sections/section">
                      <option>
                        ...
                        <xsl:if test="@name=$sectionName or (''=$sectionName and position()=last())">
                          <xsl:attribute name="selected"></xsl:attribute>
                        </xsl:if>
                        <xsl:value-of select="labels/label[@languagecode = $languageCode]/@description" />
                      </option>
