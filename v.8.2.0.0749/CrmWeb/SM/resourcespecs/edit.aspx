...
<script type="text/javascript" language="javascript">
    ...
    function registerNodeTypes(oTree) {
        ...
        oTree.RegisterNodeTypeWithImageStrip("equipment",
            ico4000Info.source,
            ico4000Info.cssClass,
            null,
            null,
            "ua_label",
            "description");
        ...
        oTree.RegisterNodeTypeWithImageStrip("systemuser",
            ico8ImageInfo.source,
            ico8ImageInfo.cssClass,
            null,
            null,
            "ua_label",
            "description");
    ...
    function needNotification() {
        ...
        return (null !=
            XUI.Xml.SelectSingleNode(oXml,
                "//resourcespec[(ua_node or not(node()) or not(*[@fulltree_id and (not(@fulltree_status)  or (@fulltree_status and @fulltree_status != \"deleted\"))])) and (not(@fulltree_status) or (@fulltree_status and @fulltree_status != \"deleted\"))]",
                null));
    }
    ...
    function setUaLabels(oTree) {
        ...
        if (aoResourcespecNodes.length > 0) {
            ...
            bAddHelper = (XUI.Xml.SelectSingleNode(aoResourcespecNodes[0],
                    "equipment|resourcespec|constraintbasedgroup|systemuser|team",
                    null) ==
                null);
        }
        ...
        for (var i = 0; i < aoResourcespecNodes.length; i++) {
            ...
            var oCData = oXml.ownerDocument
                .createCDATASection(getRuleDisplayText(bHasParent, iQuantity, sObjective, bSameSite, bAddHelper));
        ...
        aoGroupNodes = XUI.Xml.SelectNodes(oXml,
            "//resourcespec[not(equipment|resourcespec|constraintbasedgroup|systemuser|team)]",
            null);
    ...
    function getStartTemplate() {
        ...
        var sRuleChildren =
            makeTag("ua_label",
                    "<![CDATA[" + getRuleDisplayText(false, 1, OBJECTIVE_RANDOM_EXPRESSION, 1, true) + "]]>") +
    ...
    function getHelperNodeLabel() {
        ...
        return formatString(CrmEncodeDecode.CrmHtmlEncodeForFormatString(LOCID_RESSPEC_HLPLINKFMT),
            sResourceLink,
            sResourceGroupLink,
            sRuleLink);
    }
    ...
    function getRuleDisplayText(bHasParent, iQuantity, sObjective, bSameSite, bAddHelper) {
        ...
        if (bAddHelper && !_bReadOnly) {
            var sQuantityLink = "<span><a onclick=\"editroot(event)\" class=\"rsHelperlink\">" +
                CrmEncodeDecode.CrmHtmlEncode(LOCID_RESSPEC_FMTQTY) +
                "</a></span>";
            var sLabel = formatString(CrmEncodeDecode
                .CrmHtmlEncodeForFormatString(LOCID_RESSPEC_FMTLBL),
                sQuantityLink);
        ...
        return formatString(CrmEncodeDecode.CrmHtmlEncodeForFormatString(sFormatName),
            sChoiceLink,
            sCriteria,
            sSite,
            sHelper);
    ...
    function AddRule() {
        ...
        var addRuleCallBackObj = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(addRuleCallBack, [oParent, _bDirty]);
    ...
    function addRuleCallBack(oResult, oParent) {
        if (oResult) {
            ...
            var sRuleChildren =
                makeTag("ua_label",
                        "<![CDATA[" +
                        getRuleDisplayText(true, oResult.iQuantity, oResult.sObjective, oResult.bSameSite, false) +
                        "]]>") +
                    makeTag("name", CrmEncodeDecode.CrmHtmlEncode(LOCID_RESSPEC_NAMEIMPL)) +
    ...
    function AddItems(sTypesToAdd) {
        ...
        var callbackFunctionObject = Mscrm.Utilities
            .createCallbackFunctionObject("performActionAfterLookUpAddAddItems", this, parameters, false);
    ...
    function refreshResourceGroupList(aoRetval, sTypesToAdd, oParent) {
        ...
        for (i = 0; i < aoRetval.items.length; i++) {
            ...
            if (sNodeTypeName == "constraintbasedgroup" || sNodeTypeName == "team") {
                sNodeChildren += makeTag("ua_label",
                    formatString(LOCID_RESSPEC_NAMEFMTGRP, CrmEncodeDecode.CrmHtmlEncode(aoRetval.items[i].name)));
        ...
        if (oTargetNode !== null) {
            ...
            if (sElementType == "ua_node") {
                ...
                if (oElement !== null) {
                    ...
                    sObjective =
                        convertIdToExpression(convertExpressionToId(XUI.Xml.XMLSerializer.serializeToString(oElement)));
                }
                ...
                dialogOptions.width = 400;
                var editSelectedItemCallBackObj = Mscrm.Utilities
                    .createCallbackFunctionForXrmDialog(editSelectedItemCallBack, [oTargetNode, sParent, _bDirty]);
    ...
    function editSelectedItemCallBack(oResult, oTargetNode, sParent) {
        if (oResult) {
            ...
            if (oElement !== null) {
                ...
                XUI.Xml.SetText(oElement,
                    getRuleDisplayText((sParent != "tree"),
                        oResult.iQuantity,
                        oResult.sObjective,
                        oResult.bSameSite,
                        false));
    ...
    function DeleteSelectedItem() {
        ...
        if (oItem.nodeName == "ua_node") {
            ...
            if (XUI.Xml.SelectSingleNode(oParent,
                    "*[@fulltree_id and (not(@fulltree_status) or (@fulltree_status and @fulltree_status !=\"deleted\"))]",
                    null) ==
                null) {
