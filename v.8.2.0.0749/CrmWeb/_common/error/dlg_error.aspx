...
<style type="text/css">
    ...
    #ErrorDetailsContainerToggleContainer, #ErrorDetailsContainer { padding-top: 10px; }

    #ErrorDetailsContainer { padding-bottom: 44px; }

    #ErrorDetailsContainerToggle:hover { text-decoration: underline; }
    ...
    .ms-crm-Error-Details-Label { font-weight: bold; }
    ...
    .ms-crm-Error-Details-Hide-Element { display: none; }
</style>
...
<script type="text/javascript">
    ...
    function Pageload() {
        ...
        if (!window.parent.UseTabletExperience &&
            (IsNull(errInfo) || isNullOrEmptyString(errInfo.get_serializedException())) &&
            errorCode != "0x8005E25C") {
        ...
        if (!IsNull(oArgs)) {
            ...
            } else if (ErrorCodeSupportsParameters &&
                !isNullOrEmptyString(localizedMessage) &&
                (!IsNull(errInfo)) &&
                errInfo != "") {
                var parameters = errInfo.get_parameters();
                if (errInfo.get_code() == "0x80061680" ||
                    errInfo.get_code() == "0x80044291" ||
                    errInfo.get_code() == "0x80044281" ||
                    errInfo.get_code() == "0x80044289" ||
                    errInfo.get_code() == "0x80044282" ||
                    errInfo.get_code() == "0x80044283" ||
                    errInfo.get_code() == "0x80044284" ||
                    errInfo.get_code() == "0x80044285" ||
                    errInfo.get_code() == "0x80044290" ||
                    errInfo.get_code() == "0x80044286") {
            ...
            if (!IsNull(errInfo)) {
                var msg = Mscrm.ErrorInformation
                    .formatMessageWithParameters(document.getElementById("ErrorMessage").innerHTML,
                        errInfo.get_parameters());
                if (errInfo.get_code() == "0x80061007") {

                    msg = Mscrm.ErrorInformation
                        .formatMessageWithParameters(document.getElementById("ErrorMessage").innerHTML,
                            getParameters(errInfo.get_serializedException()));
                }
                msg = msg.replace(/(\r)?\n/g, "<br/>");
                msg = msg.replace(/(\\r)?\\n/g, "<br/>");


                if (((errInfo.get_code() == "0x8004832B")) && !IsNull(errInfo.get_description())) {
                    var errorDetails = "";
                    <%
                    ...
                    errorDetails = "<br/>" +
                        CrmEncodeDecode.CrmHtmlEncode(
                            "<%= AppResourceManager.Default.GetString("Error_Details") %>") +
                        "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
                    ...
                    var principalTeam = CrmEncodeDecode
                        .CrmHtmlEncode("<%= AppResourceManager.Default.GetString("SecurityPrincipal.Team") %>");
                    var principalUser = CrmEncodeDecode
                        .CrmHtmlEncode("<%= AppResourceManager.Default.GetString("SecurityPrincipal.User") %>");
                ...
                if (!isNullOrEmptyString(activityId) && activityId != "00000000-0000-0000-0000-000000000000") {
                    document.getElementById("ErrorActivityIdLabel").innerHTML = CrmEncodeDecode
                        .CrmHtmlEncode("<%= AppResourceManager.Default.GetString("ErrorDialog.TelemetryActivityIdLabel") %>");
                    document.getElementById("ErrorActivityIdValue").innerHTML = CrmEncodeDecode
                        .CrmHtmlEncode(activityId);
                ...
                if (!isNullOrEmptyString(timeStampDateObject) &&
                    !isNaN(timeStampDateObject) &&
                    timeStampDateObject.getTime() != 0) {
                    document.getElementById("ErrorTimestampLabel").innerHTML = CrmEncodeDecode
                        .CrmHtmlEncode("<%= AppResourceManager.Default.GetString("ErrorDialog.TelemetryTimestampLabel") %>");
                    document.getElementById("ErrorTimestampValue").innerHTML = CrmEncodeDecode
                        .CrmHtmlEncode(new Date(timeStamp));
                    technicalDetailsAvailable = true;
                } else {
                ...
                if (technicalDetailsAvailable) {
                    ...
                    var provideTechnicalDetailsString = CrmEncodeDecode
                        .CrmHtmlEncode("<%= AppResourceManager.Default.GetString("ErrorDialog.ProvideTechnicalDetails") %>");
                ...
                document.getElementById("serializedException")
                    .value = "<Exception>" +
                    CrmEncodeDecode.CrmXmlEncode(errInfo.get_serializedException()) +
                    "</Exception>";
            }
        }
    }
    ...
    function retrieveNodeValueFromSerializedException(nodeName, errInfo) {
        if (isNullOrEmptyString(nodeName) ||
            isNullOrEmptyString(errInfo) ||
            isNullOrEmptyString(errInfo.get_serializedException())) {
    ...
    function useErrorDescription(errorCode) {
        if (errorCode == "0X8004F976" ||
            errorCode == "0X8003F452" ||
            errorCode == "0X8003F451" ||
            errorCode == "0X80044331" ||
            errorCode == "0X8008100D" ||
            errorCode == "0X8008100F" ||
            errorCode == "0X8008100E" ||
            errorCode == "0X8004F985" ||
            errorCode == "0X80060861" ||
            errorCode == "0X80060892" ||
            errorCode == "0X80060893" ||
            errorCode == "0X80060897" ||
            errorCode == "0X800609A0" ||
            errorCode == "0X8006098E" ||
            errorCode == "0X80071123" ||
            errorCode == "0X80071121" ||
            errorCode == "0X80071119") {
            return true;
        }
        return false;
    }
    ...
    function getParameters(serializedException) {
        ...
        appendToArray = function(childNode) {
            ...
            parameters[val] = XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(childNode, "descendant::*[local-name()='value']"));
        }
