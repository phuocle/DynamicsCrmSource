    ...
    try {
        if (!Mscrm.InternalUtilities._Script.isNullOrUndefined(method)) {
            ...
            if (!Mscrm.InternalUtilities._Script.isNullOrUndefined(scope) &&
                !Mscrm.InternalUtilities._Script.isNullOrUndefined(scope[scopeSplit[scopeSplit.length - 1]])) {
                return scope[scopeSplit[scopeSplit.length - 1]].apply(null, parameterArray);
            }

            Xrm.XrmInternalWrapper.logVerbose("Mscrm.RunHandlerWithHandlerName: Function scope not found for method: " +
                method);
            return;
        }
