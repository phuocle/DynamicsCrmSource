...
function RenderRow(RowType, iRangeIndex, DecorClassName, oForcedSplitTime, ForcedEffort) {
    ...
    if (RowType == "effort") {
        ...
        oTd.align = "center";
        oTd.innerHTML = RenderInput(inputType,
            Effort,
            "Effort" + (iRangeIndex + 1),
            "OnCapacityLostFocus(" + (iRangeIndex + 1) + ");",
            null,
            EffortRuleName);
        NewRow.appendChild(oTd)
    } else {
    ...
    if (RowType == "effort") {
        ...
        innerHtml += RenderButton("InsertButtonClickHandler(new Sys.UI.DomEvent(event));",
            "InsertButton" + (iRangeIndex + 1),
            InsertActionName,
            "ms-crm-RefreshDialog-Button RefreshDialog-Button_Override",
            "block",
            iRangeIndex + 1);
        innerHtml += "</td><td>";
        innerHtml += RenderImage("/_imgs/menuButton",
            LOCID_MENUBUTTON_ALTTAG,
            "onMenuButton(new Sys.UI.DomEvent(event));",
            "MenuButton" + (iRangeIndex + 1),
            "menuButton",
            "none",
            iRangeIndex + 1);
        innerHtml += "</td></tr></table>"
    } else
        innerHtml = RenderButton("DeleteBreakClickHandler(new Sys.UI.DomEvent(event));",
            "MergeButton" + (iRangeIndex + 1),
            MergeActionName,
            "ms-crm-RefreshDialog-Button RefreshDialog-Button_Override",
            "block",
            iRangeIndex + 1);
...
function IsNextRowValidForMerge(iRangeIndex) {
    ...
    if (document.getElementById("Type" + prevRowIndex).value == "effort")
        return !EnableEffort &&
            ...
            document.getElementById("Effort" + iRangeIndex).value ==
            document.getElementById("Effort" + prevRowIndex).value;
    else
        return !EnableEffort &&
            iRangeIndex < iMaxRange &&
...
function OnStartTimeChangeInternal(iRangeIndex) {
    ...
    if (sRangeOverlaps != "")
        if (confirm(sWarnMessage)) {
            iRangeIndex > 0 &&
                CheckEndNotCloseToMidnight(oNewDateTime) &&
            ...
            for (i = 0; i < iMaxRange; i++) {
                ...
                else if (i == iRangeIndex &&
                (oStartDateTime.toString() == oEndDateTime.toString() ||
...
function OnEndTimeChangeInternal(iRangeIndex) {
    ...
    if (sRangeOverlaps != "")
        if (confirm(sWarnMessage)) {
            iRangeIndex + 1 < iMaxRange &&
                CheckStartNotCloseToMidnight(oNewDateTime) &&
...
function ToggleCapacity(type, display) {
    ...
    for (i = 0; i < iMaxRange; i++) {
        ...
        if (oElement) {
            ...
            var elementHTML = RenderInput(type,
                oElement.value,
                "Effort" + i,
                "OnCapacityLostFocus(" + i + ");",
                null,
                EffortRuleName);
...
function RenderImage(sBaseUrl, altTag, sOnClick, sId, sClass, sDisplay, rangeIndex) {
    return '<img alt="' +
        ...
        '" src="' +
        sBaseUrl +
        'Off.gif" style="cursor:pointer; display:' +
        sDisplay +
        '" class="' +
        sClass +
        '" tabIndex="0" onclick="' +
...
function GetEndTimeSelect(iRangeIndex) {
    ...
    if (oEndTime.getHours() == 0 && oEndTime.getMinutes() == 0)
        ...
        oEndTime = newDate(_oBaseDate.getFullYear(),
            _oBaseDate.getMonth(),
            _oBaseDate.getDate(),
            oEndTime.getHours(),
            oEndTime.getMinutes(),
...
function LoadFromXml() {
    var oChildNode,
        ...
        i = 0;
    XUI.Xml.DomUtils.ForEachChild(XUI.Xml.DomUtils.GetFirstChild(oXml),
        Function.createDelegate(this,
            function(oChildNode) {
                type = XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oChildNode, "type", null));
                start = new Date(ParseClientDateTime(XUI.Xml
                    .GetText(XUI.Xml.SelectSingleNode(oChildNode, "start", null))));
                duration = ParseClientTimeSpan(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(oChildNode, "duration", null)));
                if (type == "effort" &&
                    XUI.Xml.DomUtils.GetNextSibling(XUI.Xml.DomUtils
                        .GetNextSibling(XUI.Xml.DomUtils.GetNextSibling(XUI.Xml.DomUtils.GetFirstChild(oChildNode))))) {
