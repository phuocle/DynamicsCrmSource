                    ...
                    elements: [
                        {
                            type: 'html',
                            id: 'securityMsg',
                            html: '<div>' + lang.securityMsg + '</div>'
                        },
                        {
                            type: 'html',
                            id: 'pasteMsg',
                            html: '<div>' + lang.pasteMsg + '</div>'
                        },
                        {
                            type: 'html',
                            id: 'editing_area',
                            className: 'cke_dialog_ui_vbox_child_html_editable',
                            html: '',
                            focus: function() {
                            ...
                            setup: function() {
                                ...
                                var parent = null;
                                if (element) {
                                    parent = new CKEDITOR.dom.element(element.$.parentNode);
                                }

                                var dialog = this.getDialog();
                                var htmlToLoad = '<html dir="' +
                                    editor.config.contentsLangDirection +
                                    '"' +
                                    ' lang="' +
                                    (editor.config.contentsLanguage || editor.langCode) +
                                    '">' +
                                    '<head><style>body{margin:3px;height:95%;word-break:break-all;font-family:"SegoeUI","Segoe UI";font-size:14px;}</style></head><body>' +
                                    '<script id="cke_actscrpt" type="text/javascript">' +
                                    'window.parent.CKEDITOR.tools.callFunction(' +
                                    CKEDITOR.tools.addFunction(onPasteFrameLoad, dialog) +
                                    ',this);' +
                                    '</script></body>' +
                                    '</html>';
                                ...
                                var src =
                                    CKEDITOR.env.air
                                        ...
                                        encodeURIComponent(// jshint ignore:line
                                            'document.open();' +
                                            '(' +
                                ...
                                var iframe = CKEDITOR.dom.element.createFromHtml('<iframe' +
                                    ...
                                    '"' +
                                    ' aria-describedby="' +
                                    dialog.getContentElement('general', 'pasteMsg').domId +
                                    '"' +
                                    '></iframe>');

                                var clickHandlerId = 'click.' + me.getElement().$.id;
                                var focusHandlerId = 'focus.' + me.getElement().$.id;
                                var doc = null;
                                iframe.on('load',
                                    function(e) {
                                        e.removeListener();
                                        ...
                                        if (parent) {
                                            // Trigger the focus event of the element on iframe document focus.
                                            $(doc.$).find('body')
                                                .on(clickHandlerId,
                                                    function() {
                                                        parent.fire('focus');
                                ...
                                dialog.once('hide',
                                    function() {
                                        if (doc) {
                                            $(doc.$).find('body').off(clickHandlerId);
                                            $(doc.$).find('body').off(focusHandlerId);
                                        }
                                    });
