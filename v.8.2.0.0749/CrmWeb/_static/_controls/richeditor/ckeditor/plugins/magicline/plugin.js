...
(function() {
    ...
    {
        lang:
            'ar,bg,ca,cs,da,de,el,en,es,et,eu,fi,fr,gl,he,hi,hr,hu,id,it,ja,kk,ko,lt,lv,ms,nb,nl,pl,pt-br,pt,ro,ru,sk,sl,sr-latn,sr,sv,th,tr,uk,vi,zh-cn,zh-HK,zh', // %REMOVE_LINE_CORE%
        init: initPlugin
    });
    ...
    function initPlugin(editor) {
        ...
        var config = editor.config,
            ...
            that = {
                ...
                triggers: config.magicline_everywhere
                    ? DTD_BLOCK
        ...
        function addListeners() {
            ...
            editable.attachListener(that.inInlineMode ? doc : doc.getWindow().getFrame(),
                'mouseout',
                function(event) {
                    ...
                    if (that.inInlineMode) {
                        ...
                        if (!inBetween(mouse.x, size.left - scroll.x, size.right - scroll.x) ||
                            !inBetween(mouse.y, size.top - scroll.y, size.bottom - scroll.y)) {
            ...
            editable.attachListener(editable,
                'keyup',
                function() {
                    that.hiddenMode = 0;
                });
            ...
            editable.attachListener(that.inInlineMode ? editable : doc,
                'mousemove',
                function(event) {
            ...
            editable.attachListener(env_ie8 ? doc : win,
                'mousedown',
                function() {
                    if (editor.mode != 'wysiwyg')
                        return;
    ...
    var extend = CKEDITOR.tools.extend,
        ...
        CSS_COMMON = 'width:0px;height:0px;padding:0px;margin:0px;display:block;' +
            'z-index:9999;color:#fff;position:absolute;font-size: 0px;line-height:0px;',
    ...
    function areSiblings(that, upper, lower) {
        return isHtml(upper) &&
            isHtml(lower) &&
            lower.equals(upper.getNext(function(node) {
    ...
    var elementFromMouse = (function() {
        function elementFromPoint(doc, mouse) {
            ...
            return pointedElement && pointedElement.nodeType ? new CKEDITOR.dom.element(pointedElement) : null;
        }
    ...
    function getMidpoint(that, upper, lower) {
        ...
        return upperSizeBottom && lowerSizeTop
            ? 0 | (upperSizeBottom + lowerSizeTop) / 2
            : upperSizeBottom || lowerSizeTop;
    }
    ...
    function initLine(that) {
        var doc = that.doc,
            ...
            line = newElementFromHtml('<span contenteditable="false" style="' +
                CSS_COMMON +
                'position:absolute;border-top:1px dashed ' +
                that.boxColor +
                '"></span>',
                doc),
            iconPath = CKEDITOR.getUrl(this.path +
                'images/' +
                (env.hidpi ? 'hidpi/' : '') +
                'icon' +
                (that.rtl ? '-rtl' : '') +
        ...
        extend(line,
        {
            ...
            lineChildren: [
                extend(
                    newElementFromHtml(
                        '<span title="' +
                        that.editor.lang.magicline.title +
            ...
            mouseNear: function() {
                ...
                if (size &&
                    inBetween(that.mouse.y, size.top - offset, size.bottom + offset) &&
                    inBetween(that.mouse.x, size.left - offset, size.right + offset)) {
                    return true;
                }
            ...
            place: function() {
                ...
                if (parent.equals(editable)) {
                    ...
                    styleSet.left = any.size.left -
                        any.size.margin.left +
                        view.scroll.x -
                        (that.inInlineMode ? view.editable.left + view.editable.border.left : 0);
                ...
                if (trigger.is(LOOK_TOP) || inBetween(styleSet.top, view.scroll.y - 15, view.scroll.y + 5)) {
                    ...
                    this.look(LOOK_TOP);
                } else if (trigger.is(LOOK_BOTTOM) ||
                    inBetween(styleSet.top, view.pane.bottom - 5, view.pane.bottom + 15)) {
        ...
        line.lineChildren[0].on('mouseup',
            function(event) {
                line.detach();

                accessFocusSpace(that,
                    function(accessNode) {
        ...
        return {
            ...
            exec: (function() {
                ...
                return function(editor) {
                    ...
                    if (
                        isHtml(neighbor) &&
                            neighbor.is(that.triggers) &&
                            neighbor.is(DTD_NONACCESSIBLE) &&
                            (
    ...
    function triggerEditable(that) {
        ...
        //	\-> Microsoft Edge node is text.
        //	\-> Microsoft Edge node is floated, etc.
        //
        // Microsoft Edge node *must be* a valid trigger at this stage as well.
        ...
        if (!bottomTrigger && // Top trigger case.
            ...
            inBetween(mouse
                .y,
                0,
                edgeNode.size.top + triggerOffset)) { // Check if mouse in [0, edgeNode.top + triggerOffset].

            // Determine trigger look.
            triggerLook = that.inInlineMode || view.scroll.y === 0 ? LOOK_TOP : LOOK_NORMAL;
        ...
        // \->	Bottom case.
            ...
            inBetween(mouse.y, // Check if mouse in...
                edgeNode.size.bottom - triggerOffset,
                view.pane.height)) { // [ edgeNode.bottom - triggerOffset, paneHeight ]
    ...
    function triggerEdge(that) {
        ...
        if (!elementSibling) {
            ...
            triggerSetup = [null, element][bottomTrigger ? 'reverse' : 'concat']().concat([
                ...
                ? (bottomTrigger ? LOOK_BOTTOM : LOOK_TOP)
                : LOOK_NORMAL
            ]);
    ...
    var triggerExpand = (function() {
        ...
        function expandFilter(that, trigger) {
            ...
            if (
                !upper ||
                    !lower || // NOT: EDGE_MIDDLE trigger ALWAYS has two elements.
                    isFlowBreaker(lower) ||
                    isFlowBreaker(upper) || // NOT: one of the elements is floated or positioned
                    lower.equals(upper) ||
                    upper.equals(lower) || // NOT: two trigger elements, one equals another.
    ...
    function getSize(that, element, ignoreScroll, force) {
        ...
        box.top = docPosition.y - (ignoreScroll ? 0 : that.view.scroll.y), box
                .left = docPosition.x - (ignoreScroll ? 0 : that.view.scroll.x),

            // w/ borders and paddings.
            box.outerWidth = element.$.offsetWidth, box.outerHeight = element.$.offsetHeight,

            // w/o borders and paddings.
            box.height = box.outerHeight - (padding.top + padding.bottom + border.top + border.bottom), box
                .width = box.outerWidth - (padding.left + padding.right + border.left + border.right),
    ...
    function updateSize(that, element, ignoreScroll) {
        ...
        return extend(element.size,
            getSize(that, element, ignoreScroll),
            {
                date: +new Date()
            },
            true);
