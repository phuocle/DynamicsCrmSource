    ...
    CKEDITOR.plugins.add('codemirror',
    {
        ...
        init: function(editor) {
            ...
            if (editor.elementMode === CKEDITOR.ELEMENT_MODE_INLINE || editor.plugins.sourcedialog) {
                ...
                CKEDITOR.dialog.add('sourcedialog',
                    function(editor) {
                        ...
                        function loadCodeMirrorInline(editor, textarea) {
                                ...
                                extraKeys: {
                                    "Ctrl-Q": function(codeMirror_Editor) {
                                        if (config.enableCodeFolding) {
                                            ...
                                            ](codeMirror_Editor, codeMirror_Editor.getCursor().line);
                                        }
                                    },
                            ...
                            if (config.autoFormatOnStart) {
                                if (config.useBeautify) {
                                    ...
                                    window["codemirror_" + editor.id]
                                        .setValue(html_beautify(source, indent_size, indent_char, 120, brace_style));
                                } else {
                            ...
                            window["codemirror_" + editor.id].on("change",
                                function() {
                            ...
                            // inherit blur event
                            window["codemirror_" + editor.id].on("blur",
                                function() {
                                            ...
                                            children: [
                                                {
                                                    ...
                                                    onClick: function() {
                                                        ...
                                                        window["codemirror_" + editor.id]
                                                            .commentRange(true, range.from, range.to);
                                                    }
                                                }, {
                                                    type: 'button',
                                                    ...
                                                    onClick: function() {
                                                        ...
                                                        window["codemirror_" + editor.id]
                                                            .commentRange(false, range.from, range.to);
                                                        if (window["codemirror_" + editor.id].config
            ...
            function loadCodeMirror(editor) {
                ...
                var holderHeight = holderElement.$.clientHeight == 0
            ...
            editor.on('beforeModeUnload',
                function(evt) {
                    if (editor.mode === 'source' && editor.plugins.textselection) {
                        ...
                        range
                            .startOffset =
                            LineChannelToOffSet(window["codemirror_" + editor.id],
            ...
            editor.on('mode',
                function() {
                    editor.getCommand('source').setState(editor.mode === 'source'
                        ? CKEDITOR.TRISTATE_ON
                        : CKEDITOR.TRISTATE_OFF);

                    if (editor.mode === 'source') {
                        editor.getCommand('autoCompleteToggle')
                            .setState(window["codemirror_" + editor.id].config.autoCloseTags
                                ? CKEDITOR.TRISTATE_ON
                                : CKEDITOR.TRISTATE_OFF);
