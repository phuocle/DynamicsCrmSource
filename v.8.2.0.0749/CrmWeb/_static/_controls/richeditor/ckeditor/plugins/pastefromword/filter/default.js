...
(function() {
    ...
    var plugin = (CKEDITOR.plugins.pastefromword = {
        utils: {
                        ...
                        attrs.style = plugin.filters.stylesFilter([
                                ...
                                [
                                    (/^margin(:?-left)?$/), null, function(margin) {
                                        ...
                                        attrs['cke:indent'] =
                                            listBaseIndent && (Math.ceil(margin / listBaseIndent) + 1) ||
                                            1;
                                    }
                                ],
            ...
            getStyleComponents: (function() {
                var calculator = CKEDITOR.dom.element
                    .createFromHtml('<div style="position:absolute;left:-9999px;top:-9999px;"></div>',
                        CKEDITOR.document);
        ...
        filters: {
            ...
            flattenList: function(element, level) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if (child.name in CKEDITOR.dtd.$listItem) {
                        ...
                        if (first.attributes &&
                            first.attributes.style &&
                            first.attributes.style.indexOf('mso-list') > -1) {
            ...
            // @param approvedlist {Boolean} Whether the {@param styles} will be considered as a approved-list.
            stylesFilter: function(styles, approvedlist) {
                return function(styleText, element) {
                    ...
                    (styleText || '').replace(/&quot;/g, '"').replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g,
                        function(match, name, value) {
                            ...
                            for (var i = 0; i < styles.length; i++) {
                                if (styles[i]) {
                                    ...
                                    if (name.match(namePattern) && (!valuePattern || value.match(valuePattern))) {
                                        name = newName || name;
                                        approvedlist && (newValue = newValue || value);
                            ...
                            !approvedlist && rules.push([name, value]);
            ...
            elementMigrateFilter: function(styleDefinition, variables) {
                return styleDefinition
                    ? function(element) {
                        var styleDef = variables
                            ? new CKEDITOR.style(styleDefinition, variables)._.definition
                            : styleDefinition;
                ...
                elements: {
                    ...
                    $: function(element) {
                        ...
                        if (tagName in blockLike && attrs.style)
                            attrs
                                .style = stylesFilter([[(/^(:?width|height)$/), null, convertToPx]])(attrs.style) || '';
                    ...
                    'p': function(element) {
                        ...
                        if (config.enterMode == CKEDITOR.ENTER_BR) {
                            ...
                            elementMigrateFilter(config['format_' + (config.enterMode == CKEDITOR.ENTER_P ? 'p' : 'div')
                            ])(element);
                        }
                    },
                    ...
                    'span': function(element) {
                        ...
                        if (styleText) {
                            attrs.style = stylesFilter([
                                    ...
                                    [
                                        (/^font-family$/), null,
                                        !removeFontStyles ? styleMigrateFilter(config.font_style, 'family') : null
                                    ],
                                    [
                                        (/^font-size$/), null, !removeFontStyles
                                        ? styleMigrateFilter(config.fontSize_style, 'size')
                                        : null
                                    ],
                                    [
                                        (/^color$/), null, !removeFontStyles
                                        ? styleMigrateFilter(config.colorButton_foreStyle, 'color')
                                        : null
                                    ],
                                    [
                                        (/^background-color$/), null,
                                        !removeFontStyles
                                        ? styleMigrateFilter(config.colorButton_backStyle, 'color')
                                        : null
                                    ]
                                ])(styleText, element) ||
                                '';
                            ...
                            [
                                (/^margin$|^margin-(?!bottom|top)/), null, function(value, element, name) {
                                    if (element.name in { p: 1, div: 1 }) {
                                        var indentStyleName =
                                            config.contentsLangDirection == 'ltr' ? 'margin-left' : 'margin-right';
                        ...
                        if (CKEDITOR.env.gecko && imageInfo) {
                            var img = CKEDITOR.htmlParser.fragment.fromHtml(imageInfo[0]).children[0],
                                ...
                                imgSrcInfo = previousComment &&
                                    previousComment.value.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/),
