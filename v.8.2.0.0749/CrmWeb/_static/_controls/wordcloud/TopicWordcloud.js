...
(function() {
    ...
    function _successCallback(data, selectedWord) {
        ...
        ReactDOM.render(React.createElement(WordCloudControl,
            {
                phrases: wordCloudWords,
                parentClass: className,
                onWordClick: !_isMobileClient() && _applyDrillDown,
    ...
    function _applyDrillDown(word) {
        ...
        if (!queryParams)
            return;
        var extraqs = "visualizationPaneMode=1&isWordCloudDrillDown=true&visualizationId=" +
                encodeURIComponent(queryParams["visid"]) +
                "&visualizationType=" +
                encodeURIComponent(queryParams["vistype"]) +
                "&parentVizPaneId=" +
                encodeURIComponent(word.tooltip),
            mainPageUrl = Xrm.Page.context.getClientUrl() +
                "/main.aspx?pagetype=ENTITYLIST&etc=112&viewid=" +
                encodeURIComponent(queryParams["viewid"]) +
                "&viewtype=" +
                encodeURIComponent(queryParams["viewtype"]) +
                "&extraqs=" +
