...
function drawFieldSelect(sAvailAry, sSelectedAry, isStateTransition, isEntityAttributes) {
    ...
    if ($get("divFieldSelect") != null)
        $get("divFieldSelect")
            .innerHTML =
            "<table cellspacing='5' cellpadding='0' style='table-layout:fixed;width:100%;-moz-user-select: none;'><col width='*'><col width='50px'><col width='*'><tr><td><b class='label'>" + CrmEncodeDecode.CrmHtmlEncode(_sFieldSelectLeftTitle) + "</b></td><td>&nbsp;</td><td><b class='label'>" + CrmEncodeDecode.CrmHtmlEncode(_sFieldSelectRightTitle) + "</b></td></tr><tr><td><div id='objList' class='divList' style='height:" + encodedHeight + "px;' tabIndex=0 onfocus='handleListFocus(new Sys.UI.DomEvent(event))' onkeydown='handleListKey(new Sys.UI.DomEvent(event))' hidefocus='true'></div></td><td align='center'><button id='btnRight' type='button' onclick='MoveRight();' aria-label='" + CrmEncodeDecode.CrmHtmlAttributeEncode(_sMoveRightTitle) + "' class='ms-crm-RefreshDialog-Button' style='min-width: 40px;'>" + CrmEncodeDecode.CrmHtmlEncode(">>") + "</button><p><button id='btnLeft' type='button' onclick='MoveLeft();' aria-label='" + CrmEncodeDecode.CrmHtmlAttributeEncode(_sMoveLeftTitle) + "' class='ms-crm-RefreshDialog-Button' style='min-width: 40px;'>" + CrmEncodeDecode.CrmHtmlEncode("<<") + "</button>" + (_bFieldSelectShowUp ? "<p><button id='btnUp' type='button' style='width: 40px;' onclick='MoveUp();'>Up</button>" : "") + (_bFieldSelectShowDown ? "<p><button id='btnDown' type='button' style='width: 40px;' onclick='MoveDown();'>Down</button>" : "") + "</td><td><div id='rtnObjList' class='divList' style='height:" + encodedHeight + "px;' tabIndex=0 onfocus='handleListFocus(new Sys.UI.DomEvent(event))' onkeydown='handleListKey(new Sys.UI.DomEvent(event))' hidefocus='true'></div></td></tr></table>";
    if ($get("divAlternateKeysFieldSelect") != null)
        $get("divAlternateKeysFieldSelect")
            .innerHTML =
            "<table cellspacing='5' cellpadding='0' style='table-layout:fixed;width:100%;-moz-user-select: none;'><col width='*'><col width='100px'><col width='*'><tr><td><b class='label'>" + CrmEncodeDecode.CrmHtmlEncode(_sFieldSelectLeftTitle) + "</b></td><td>&nbsp;</td><td><b class='label'>" + CrmEncodeDecode.CrmHtmlEncode(_sFieldSelectRightTitle) + "</b></td></tr><tr><td><div id='objList' class='divList' style='height:" + encodedAlternateKeysHeight + "px;' tabIndex=0 onfocus='handleListFocus(new Sys.UI.DomEvent(event))' onkeydown='handleListKey(new Sys.UI.DomEvent(event))' hidefocus='true'></div></td><td align='center'><button id='btnRight' type='button' accesskey='a' style='width: 80px;' onclick='MoveRight();'>" + CrmEncodeDecode.CrmHtmlEncode(LOCID_KEYSUTL_ADDKEYS + " > ") + "</button><p><button id='btnLeft' type='button' accesskey='r' style='width: 80px;' onclick='MoveLeft();'>" + CrmEncodeDecode.CrmHtmlEncode(" < " + LOCID_KEYSUTL_REMOVEKEYS) + "</button></td><td><div id='rtnObjList' class='divList' style='height:" + encodedAlternateKeysHeight + "px;' tabIndex=0 onfocus='handleListFocus(new Sys.UI.DomEvent(event))' onkeydown='handleListKey(new Sys.UI.DomEvent(event))' hidefocus='true'></div></td></tr></table>";
    ...
    for (var iAvailLen = sAvailAry[0].length,
        ...
        else if (_isEntityAttributes)
            CreateNewRow(i, sAvailAry[0][i], sAvailAry[0][i], sAvailAry[1][i], _objList, false, sAvailAry[2][i]);
        else
            CreateNewRow(i, sAvailAry[0][i], sAvailAry[0][i], sAvailAry[1][i], _objList, false);
    _rtnObjList
        .innerHTML = "<table cellpadding='2' cellspacing='0' style='width:100%;-moz-user-select: none;'></table>";
    var iSelectedLen;
    if (sSelectedAry)
...
function handleListFocus(e) {
    ...
    IsNull(oTable.lastSelected) &&
        oTable.rows.length > 0 &&
...
function CreateNewRow(iIndex, sId, sName, sDisplay, oDestList, bReinsert, sStatecode) {
    ...
    } else if (oDestList == _rtnObjList && _isEntityAttributes) {
    ...
    if (_isStateTransition) {
        ...
        if (window.LOCID_UI_DIR === "RTL")
            oCell.innerHTML = "<div><img alt='" +
                CrmEncodeDecode.CrmHtmlEncode(sDisplay) +
                "' src='" +
                CrmEncodeDecode.CrmHtmlAttributeEncode(Mscrm.SetStatusTransition.getStateIcon(sStatecode)) +
                "' style='float:right;' /><nobr class='sel-nobr'>" +
                CrmEncodeDecode.CrmHtmlEncode(sDisplay) +
                "</nobr></div>";
        else
            oCell.innerHTML = "<div><img alt='" +
                ...
                "' src='" +
                CrmEncodeDecode.CrmHtmlAttributeEncode(Mscrm.SetStatusTransition.getStateIcon(sStatecode)) +
                "' style='float:left;' /><nobr class='sel-nobr'>" +
                CrmEncodeDecode.CrmHtmlEncode(sDisplay) +
                "</nobr></div>"
    } else if (_isEntityAttributes) {
        oCell.title = CrmEncodeDecode.CrmHtmlDecode(sDisplay);
...
function MoveLeft() {
    for (var rtnObjListFirstChild = XUI.Html.DomUtils.GetFirstChild(_rtnObjList),
        ...
        if (1 == oRows[i].selected) {
            if (_isStateTransition)
                CreateNewRow(oRows[i].cells[0].getAttribute("index"),
                    oRows[i].cells[0].getAttribute("id"),
                    oRows[i].name,
                    XUI.Html.GetText(oRows[i]),
                    _objList,
                    true,
                    oRows[i].cells[0].getAttribute("state"));
            else if (_isEntityAttributes)
                CreateNewRow(oRows[i].cells[0].getAttribute("index"),
                    oRows[i].cells[0].getAttribute("id"),
                    oRows[i].name,
        ...
        if (i < j)
            if (x.localeCompare(A1[j]) == 0)
                ...
                if (typeof isStateTransition != "undefined" && isStateTransition) {
                    ...
                    A3[i] = A3[j];
                    A3[j] = tmp
                } else if (typeof isEntityAttributes != "undefined" && isEntityAttributes) {
                    tmp = A3[i];
                    A3[i] = A3[j];
                    A3[j] = tmp
                }
