...
function ConfigureToolbars(oCondition) {
    var oToolbar,
        sOp = "attachclick",
        hasDynamicId = oCondition.getAttribute("genDynamicId") &&
            oCondition.getAttribute("genDynamicId").toString() == "True",
        buttonAnd,
        buttonOr;
    ...
    if (!IsNull(oToolbar)) {
        ...
        PerformToolbarOperation(sOp,
            oToolbar,
            $get(hasDynamicId ? "btnClear_" + oCondition.id : "btnClear", obuttonContainer),
            "$find('" + oCondition.id + "').Clear()");
...
function CreateEntityUI(sColId, elem) {
    ...
    while (true) {
        ...
        if (sIdAttr.value == sPrimaryEntity) {
            sList += "<OPTION VALUE='" +
                CrmEncodeDecode.CrmHtmlAttributeEncode(sIdAttr.value) +
                "'>" +
                CrmEncodeDecode.CrmHtmlEncode(sNameAttr.value) +
                "</OPTION>";
            break
        }
        i++
...
function CreateConfigurationUI(sColId, elem) {
    ...
    if (sAction == "assign") {
        var oLookupControl = elem.Container.ClientCache.GetElement("ValueControl", "lookup"),
            oLUImg = Mscrm.FormControlInputBehavior
                .GetElementBehavior(XUI.Html.DomUtils.GetFirstChild(oLookupControl.rows[0].cells[1]));
        ...
        for (var i = 1; i < aiLookupTypes.length; i++)
            oLUImg.set_lookupTypeIcons(oLUImg.get_lookupTypeIcons() +
                ":" +
                elem.Container.ClientCache.LookupIcons[aiLookupTypes[i]]);
...
function IsTimeoutCondition(cdnCtrl) {
    ...
    while (i < iLen) {
        ...
        if (!IsNull(attrCtrl) &&
            XUI.DomUtilities.GetBaseName(cdnCtrl.GetDOMControl(i)).toUpperCase().indexOf("ATTRIBUTELIST") >= 0) {
