...
AutoNavBarPlugin.$C = function($p0, $p1) {
    ...
    AutoNavBarPlugin.$2 = {
        listSelector: null,
        scrollRegionSelector: null,
        listElementCssClass: null,
        insertSpacer: true,
        animateScroll: true,
...
AutoNavBarPlugin.updateLiElement = function() {
    ...
    var $v_0;
    if ((AutoNavBarPlugin.$1[0]
            .scrollHeight -
            AutoNavBarPlugin.$1[0].scrollTop) ===
        AutoNavBarPlugin.$1[0].clientHeight) {
        $v_0 = AutoNavBarPlugin.$0[AutoNavBarPlugin.$0.length - 1];
    } else {
        $v_0 = AutoNavBarPlugin.$B(AutoNavBarPlugin.$1.scrollTop(),
            AutoNavBarPlugin.$0,
            0,
            AutoNavBarPlugin.$0.length - 1);
    }
...
ChildFocusTracker.trackChildFocus = function(customOptions) {
    ...
    return this.each(function($p1_0, $p1_1) {
        ...
        if (Mscrm.CrmBrowser.get_currentBrowser().get_isTouchEnabled()) {
            ...
            $v_3.find('a, button, textarea, input[type=\'button\'], input[type=\'text\'], input[type=\'\']')
                .filter('[tabindex!=\'-1\']').each(function($p2_0, $p2_1) {
                    $P_CRM($p2_1).filter(function($p3_0) {
                        ...
                        if (Mscrm.CrmBrowser.get_currentBrowser().get_agent() === 1 ||
                            Mscrm.CrmBrowser.get_currentBrowser().get_agent() === 3) {
...
TextAreaAutoResizePlugin.textAreaAutoResize = function() {
    ...
    var $v_1 = this.filter('textarea').each(function($p1_0, $p1_1) {
        ...
        var $v_4 = {
            ...
            top: -10000,
            'z-index': -10000,
            width: ($v_3 <= 0) ? 'auto' : $v_3,
            fontSize: $v_2.css('fontSize'),
            fontFamily: $v_2.css('fontFamily'),
            lineHeight: $v_2.css('lineHeight'),
            letterSpacing: $v_2.css('letterSpacing'),
...
TextAreaAutoResizePlugin.removeTextAreaAutoResize = function() {
    return this.filter('textarea').each(function($p1_0, $p1_1) {
        var $v_0 = $P_CRM(this);
        $v_0.unbind('change', Mscrm.TextAreaAutoResizePluginHelper.$9)
            .unbind('keyup', Mscrm.TextAreaAutoResizePluginHelper.$9);
...
HTMLElementWatermarkPlugin.htmlElementSetWatermark = function(customOptionsObject, htmlElementTagName) {
    ...
    var $v_2 = this.filter(htmlElementTagName).each(function($p1_0, $p1_1) {
        ...
        var $v_4;
        if (!IsNull($v_1) &&
            !IsNull($v_1.watermarkText) &&
            !Mscrm.InternalUtilities._String.isNullOrEmpty($v_1.watermarkText)) {
        ...
        $v_3.focus(function($p2_0) {
            ...
            if ($v_6 === $v_4) {
                $v_3.val('');
            } else if (!Mscrm.InternalUtilities._String.isNullOrEmpty($v_6) &&
                $v_6.trim().length > 0 &&
                $v_3.hasClass('watermark')) {
                var $v_7 = 0;
        ...
        var $v_5 = function($p2_0) {
            if (Mscrm.InternalUtilities._String.isNullOrEmpty($v_3.val()) &&
                Mscrm.Utilities.getActiveElement() !== $p1_1) {
