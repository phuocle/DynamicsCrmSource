    ...
    $.fn.smartZoom = function(method) {
        ...
        var publicMethods = {
            ...
            init: function(options) {
                ...
                var zoomContainerId = "zoomCanvasWrapper";
                var containerDiv = $('<div id="' +
                    zoomContainerId +
                    '" class="' +
                    "canvas zoomCanvasWrapper" +
                    '"></div>');
                ...
                targetElement.data('smartZoomData',
                {
                    ...
                    originalSize: {
                        width: targetElement.get(0).scrollWidth,
                        height: targetElement.get(0)
                            .scrollHeight
...
// use to know if the browser is compatible with css transition
...
// use to save the adjust information in "adjustToContainer" method (so we can access the normal target size in plugin when we whant)
                ...
                if (settings.mouseEnabled == true) {
                    ...
                    //targetElement.bind('mousedown.smartZoom', mouseDownHandler);
                        if (settings.scrollEnabled == true) {
                            ...
                            containerDiv.bind('mousewheel.smartZoom DOMMouseScroll.smartZoom',
                                containerMouseWheelHander);
                        }
            ...
            zoom: function(scaleToAdd, globalRequestedPosition, duration) {
                ...
                if (!globalRequestedPosition) { // use target center if globalRequestedPosition is not set
                    ...
                    globaRequestedX = containerRect.x;
                    globaRequestedY = containerRect.y;
                } else {
                ...
                newScale = Math.min(smartData.settings
                    .maxScale,
                    newScale); // the scale couldn't be highest than the max setted by user
...
// the position difference between with the target position
                var positionGlobalYDiff = globaRequestedY - targetRect.y;
                var sizeRatio = newWidth / targetRect.width;

                var newGlobalX = targetRect
                    .x -
                    ((positionGlobalXDiff * sizeRatio) - positionGlobalXDiff);
// apply the ratio to positionGlobalDiff and then find the final target point
...
// return a valid position from the calculated position 
                ...
                animate(targetElement,
                    ...
                    newHeight,
                    duration,
                    function() { // set the new position and size via the animation function
                ...
                //To trigger recalculation of zoom wrapper element
                targetElement.parent().css("overflow", "");
                targetElement.parent().css("overflow", "scroll");
            },
            /**
            ...
            pan: function(xToAdd, yToAdd, duration) {
                ...
                var validPosition =
                    getValidTargetElementPosition(currentPosition.left + xToAdd,
                        currentPosition.top + yToAdd,
                        targetRect.width,
                        targetRect.height); // add the given numbers to the current coordinates and valid the result
            ...
            destroy: function() {
                ...
                animate(targetElement,
                    smartData.originalPosition.left,
                    smartData.originalPosition.top,
                    smartData.originalSize.width,
                    smartData.originalSize.height,
                    5); // reset initial position
        ...
        if (publicMethods[method]) { // if the parameter is an existing method, then we call this method
            ...
            if (targetElement[0].tagName
                .toLowerCase() ==
                "img" &&
                !targetElement[0]
                .complete) { // if the target is an image when wait for image loading before initialization
        ...
        function mouseWheelHandler(e, delta) {
            ...
            if (smartData
                .currentWheelDelta *
                delta <
                0) // if current and delta have != sign we set 0 to go to other direction
...
// multiply by 100 to resolve precision problem
        ...
        function mouseDownHandler(e) {
            ...
            $(document)
                .on('mousemove.smartZoom', mouseMoveHandler); // add mouse move and mouseup listeners to enable drag
        ...
        function mouseMoveHandler(e) {
            ...
            (!smartData.mouseMoveForPan &&
                smartData.moveCurrentPosition.x != e.pageX &&
                smartData.moveCurrentPosition.y != e.pageY)) {
        ...
        function mouseUpHandler(e) {
            ...
            if (smartData.mouseMoveForPan) {
                ...
                if (smartData.moveLastPosition.distance(smartData
                        .moveCurrentPosition) >
                    4) { // smooth the drag end when user move the mouse fast
        ...
        function touchStartHandler(e) {
            ...
            $(document)
                .bind('touchmove.smartZoom', touchMoveHandler);
// listen move and touch end events to manage drag on touch
            ...
            smartData.moveCurrentPosition = new Point(firstTouch
                .pageX,
                firstTouch.pageY); // save the finger position on screen on touch start
...
// if the user use only one finger and touchPinch==false => we manage drag
                if (!smartData.touch.touchMove) {
                    var downLastPoint = smartData.touch.lastTouchPositionArr[0];
                    if (downLastPoint.distance(new Point(currentFirstTouchEv
                            .pageX,
                            currentFirstTouchEv.pageY)) <
                        3) { // check if user really had moved
                        return;
                    } else
...
// if the user use two fingers and touchMove==false => we manage pinch 
                ...
                var currentP1P2Distance = currentP1
                    .distance(currentP2); // distance between current two fingers positions
                var lastP1P2Distance = lastP1
                    .distance(lastP2); // distance between the last two fingers positions registered 
                var currentDistance = currentP1P2Distance - lastP1P2Distance;
// distance between last move and current move

                if (Math
                    .abs(currentDistance) <
                    3) // finger pixel error (jump between in and out mode sometimes if i don't do this check)  
                    return;

                var middle = new Point((currentP1
                        .x +
                        currentP2.x) /
                    2,
                    (currentP1.y + currentP2.y) / 2); // get the point between the two fingers
...
// scale between current distance and last fingers distance
                var newZoomScale = ((targetRect.width * newZoomValueToAdd) / originSize.width); // the new zoom scale

                publicMethods
                    .zoom(newZoomScale - currentScale, middle, 0);
// call zoom fonction with the scale to add (newZoomScale - currentScale)

                smartData.touch
                    .lastTouchPositionArr[0] = currentP1;
//update last touch position points for next function iteration 
        ...
        function touchEndHandler(e) {
            ...
            if (smartData.touch.touchMove) { // smooth motion at end if we are in drag mode
                if (smartData.moveLastPosition.distance(smartData
                        .moveCurrentPosition) >
                    2) { // smooth only if the user drag fast
                    var interpolateP = smartData.moveLastPosition
                        .interpolate(smartData
                            .moveCurrentPosition,
                            -4); // the end smooth motion is calculate according to last finger motion 
                    moveOnMotion(interpolateP.x, interpolateP.y, 500, true);
                }
            } else {
                if (smartData.settings.dblTapEnabled == true &&
                    smartData.touch.lastTouchEndTime != 0 &&
                    new Date()
                    .getTime() -
                    smartData.touch.lastTouchEndTime <
                    400) {
// if the user double tap (double tap if there is less than 300 ms between first and second tap)
        ...
        function moveOnMotion(xPos, yPos, duration, motionEnd) {
            ...
            var smartData = targetElement.data('smartZoomData');
            smartData.moveLastPosition.x = smartData.moveCurrentPosition
                .x; // save the current position in "moveLastPosition" before moving the plugin target
            smartData.moveLastPosition.y = smartData.moveCurrentPosition
                .y; // (moveLastPosition will be use to smooth last motion in "touchEndHandler" and "mouseUpHandler")

            var currentPosition = targetElement.offset(); // the target current position
            var targetRect = getTargetRect(); // current target size

            var newMarginLeft = currentPosition
                .left +
                (xPos - smartData.moveCurrentPosition.x);
// add current mouseX (orTouchX) position difference to the target position
            var newMarginTop = currentPosition.top + (yPos - smartData.moveCurrentPosition.y);

            var validPosition =
                getValidTargetElementPosition(newMarginLeft, newMarginTop, targetRect.width, targetRect.height);
// check if the new position is valid
        ...
        function zoomOnDblClick(pageX, pageY) {
            ...
            var dblClickMaxScale = parseFloat(smartData.settings
                .dblClickMaxScale); // the doucble click or double tap max scale

            var scaleDiff;
// if the current scale is close from "dblClickMaxScale" go to "originalScale" else go to "dblClickMaxScale" 
            if (currentScale.toFixed(2) > dblClickMaxScale.toFixed(2) ||
                Math.abs(dblClickMaxScale - currentScale) > Math.abs(currentScale - originalScale)) {
...
// set the current current target size and the target scale to css transformation so it stop previous transformation
                ...
                if (smartData.transitionObject.css3dSupported) {
                    cssObject[smartData.transitionObject
                            .transform] = 'translate3d(' +
                        targetRect.x +
                        'px, ' +
                        targetRect.y +
                        'px, 0) scale3d(' +
                        targetRect.width / originalSize.width +
                        ',' +
                        targetRect.height / originalSize.height +
                        ', 1)';
                } else {
                    cssObject[smartData.transitionObject
        ...
        function getValidTargetElementPosition(xPosition, yPosition, width, height) {
            ...
            newMarginTop += Math.max(0,
                (smartData.adjustedPosInfos
                    .top +
                    smartData.adjustedPosInfos.height) -
                (newMarginTop + height)) // adjust in bottom
            var newMarginLeft = Math.min(smartData.adjustedPosInfos.left, xPosition); // adjust in left
            newMarginLeft += Math.max(0,
                (smartData.adjustedPosInfos
                    .left +
                    smartData.adjustedPosInfos.width) -
                (newMarginLeft + width)); // adjust in right
        ...
        function adjustToContainer() {
            ...
            var containerDivNewLeft =
                getContainerDivPositionFromSettings(smartData.settings.left,
                    parentOffset.left,
                    containerDiv.parent().width());
            var containerDivNewTop =
                getContainerDivPositionFromSettings(smartData.settings.top,
                    parentOffset.top,
                    containerDiv.parent().height());
            ...
            var scaleToFit = Math.min(Math.min(containerRect.width / originalSize.width,
                    containerRect.height / originalSize.height),
                1).toFixed(2); // scale to use to include the target into containerRect
            ...
            animate(targetElement,
                smartData.adjustedPosInfos.left,
                smartData.adjustedPosInfos.top,
                newWidth,
                newHeight,
                0,
                function() {
...
// set target visibility to visible if developper whant to hide it before the plugin resize
                });
        ...
        function animate(target, globalLeft, globalTop, width, height, duration, callback) {
            ...
            var left = 0; // get the relative position from parent 
            var top = 0;
            ...
            if (smartData.transitionObject != null) { // use css transition if supported
                ...
                if (GenericWorkflowDesigner.Settings.renderRTL)
                    cssObject[smartData.transitionObject.transform + '-origin'] = 'top right';
                else
                    cssObject[smartData.transitionObject.transform + '-origin'] = '0 0';

                cssObject[smartData.transitionObject
                    .transition] = 'all ' + 0 / 1000 + 's ease-out'; // set effect duration
...
// get the target css matrix tranform 
                    var scale;
                    var arrValues;
...
// if the action type has changed and that the function is called from "stopAnim" dispatch END event
                ...
                if (smartData
                    ...
                    ESmartZoomEvent
                    .END) {
// if the current action had changed (MOVE to ZOOM for exemple) and the last action step was END we whant to dispatch START
                    ...
                    .currentActionType ==
                    actionType &&
                    actionStep == ESmartZoomEvent.END) {
// dispatch END if actionstep ask is END and action type don't change
...
// all type all transitions to test
            var transitionArr = [
            ...
            for (var i = 0; i < length; i++) { // for all kind of css transition we make a test
                if (bodyStyle[transitionTestArr[i]] != null) {
                    ...
                    css3dSupported = (Math.round(div.offset()
                            .left -
                            $('body').offset().left) ===
                        20);
// if translate3d(20px,0,0) via transformArr[i] == 20px the transformation is valid for this browser
                    div.empty().remove();
        ...
        */
        function getContainerDivPositionFromSettings(settingsValue,
            zoomableContainerParentPosValue,
            zoomableContainerParentSizeValue) {
            if (settingsValue.search && settingsValue.search("%") != -1)
                return zoomableContainerParentPosValue +
                    zoomableContainerParentSizeValue * (parseInt(settingsValue) / 100);
            else
        ...
        */
        function getRect(jqObject) {
            ...
            return {
                'x': formX,
                'y': formY,
                'width': jqObject.get(0).scrollWidth,
                'height': jqObject.get(0).scrollHeight
            };
        }
