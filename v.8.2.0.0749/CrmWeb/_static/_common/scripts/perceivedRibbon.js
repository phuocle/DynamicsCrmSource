...
function cleanUpRibbon(ribbonElement) {
    var cleanedUpRibbon = ribbonElement.innerHTML.replace(/currentRibbonElement="ribbonContainer0"/,
        'currentRibbonElement="ribbonContainer0" class=ms-crm-TopBarContainerForm');
...
function addLightBox(ribbonElement, ribbonContent) {
    var lightBoxContent = '<div id="perceivedRibbonLightBox" class="ms-crm-lightbox" style="top:' +
        ribbonElement.offsetTop +
        "px; left:" +
        ribbonElement.offsetLeft +
        "px; height:" +
        ribbonElement.offsetHeight +
        "px; width:" +
        ribbonElement.offsetWidth +
        'px;"/>';
...
function AddInfoToTheKey(key) {
    var extraqs = queryStringParameter("extraqs"),
        ...
        newInfo = ":new-1";
    if (extraqs != null &&
        (extraqs.indexOf("&id=") >= 0 ||
            extraqs.indexOf("?id=") >= 0 ||
            extraqs.indexOf("%26id%3d") >= 0 ||
            extraqs.indexOf("%3fid%3d") >= 0 ||
            extraqs.indexOf("%26id%3D") >= 0 ||
            extraqs.indexOf("%3Fid%3D") >= 0) ||
        id != null)
        newInfo = ":new-0";
...
function processLoadedRibbon() {
    queryStringParameter("pagetype").toLowerCase() == "entityrecord" &&
...
if (queryStringParameter("pagetype").toLowerCase() == "entityrecord") {
    ...
    if (isLocalStorageAvailable() && typeof PERCEIVED_RIBBON_KEY != "undefined") {
        document.write("<div style='position:absolute;top:0;width:100%;display:block' id='" +
            perceivedRibbonId +
            "'></div>");
