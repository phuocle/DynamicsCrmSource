...
function assignSpecificActivity(iObjType, sSrcQueueId) {
    ...
    var sEntityId = new Array($get("crmFormSubmit").crmFormSubmitId.value),
        oUrl = Mscrm.CrmUri.create("/_grid/cmds/dlg_assignqueue.aspx?iObjType=" +
            CrmEncodeDecode.CrmUrlEncode(iObjType) +
            "&iTotal=1&sQType=1&srcQueueId=" +
            CrmEncodeDecode.CrmUrlEncode(sSrcQueueId)),
        dialogOptions = new Xrm.DialogOptions;
    dialogOptions.height = 310;
    dialogOptions.width = 500;
    var parameters = [iObjType, crmFormCtrl],
        refreshFormAndGridCallBack = Mscrm.InternalUtilities.GridUtilities
            .createCallbackFunctionFactory(refreshFormAndGrid, parameters);
...
function SwapLookups(oLookupA, oLookupB) {
    ...
    if (!IsNull(aoLookupAItems) && aoLookupAItems.length > 1 && (!IsNull(aoLookupBItems) && aoLookupBItems.length > 1))
        swapStyle = "multi";
    if (swapStyle == "single" &&
        (!IsNull(aoLookupAItems) && aoLookupAItems.length > 1 || !IsNull(aoLookupBItems) && aoLookupBItems.length > 1)
    ) {
        ...
        if (Xrm.Page.context.client.getClient() == Xrm.ClientName.mobile) {
            ...
            swapLookupValuesCallBack = Mscrm.InternalUtilities.GridUtilities
                .createCallbackFunctionFactory(swapLookupValuesForMobile, parameters)
        } else {
            parameters = [true, oLookupA, oLookupB];
            swapLookupValuesCallBack = Mscrm.InternalUtilities.GridUtilities
                .createCallbackFunctionFactory(swapLookupValues, parameters)
        }
...
function swapLookupValuesForMobile(result, isTrimRequired, oLookupAName, oLookupBName) {
    swapLookupValues(result,
        isTrimRequired,
        Xrm.Page.data.entity.attributes.get(oLookupAName),
        Xrm.Page.data.entity.attributes.get(oLookupBName))
}
...
function promoteToResponse(iObjType) {
    if (Xrm.Page.data.getIsValid()) {
        ...
        Xrm.Utility.openEntityForm(Mscrm.InternalUtilities.EntityNames.CampaignResponse,
...
function interactionCentricConvertToCase(records, iObjType) {
    ...
    else if (entityLogicalName == "phonecall")
        columns = ["from", "subject", "to"];
    else if (entityLogicalName == "appointment")
    ...
    Xrm.Internal.messages.retrieve(entityLogicalName, entityId, columns).then(function(response) {
        ...
        if (directionCodeVal == true) {
            ...
            if (!IsNull(from))
                customerItems = from;
        if (iObjType == Mscrm.InternalUtilities.EntityTypeCode.Appointment ||
            iObjType == Mscrm.InternalUtilities.EntityTypeCode.RecurringAppointmentMaster) {
        ...
        GetValidCustomerSuccess(sUrlElement,
            iObjType,
            customerItems,
            Mscrm.InternalUtilities.EntityNames.Incident,
            entityId)
    })
}
...
function GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo, customerType) {
    ...
    try {
        ...
        if (!IsNull(customerType) &&
            customerType == "lead" &&
            customerItems[customerNo].type.toString() != Mscrm.InternalUtilities.EntityTypeCode.Lead.toString())
            isValidCustomer = false;
        if (!IsNull(customerType) &&
            customerType != "lead" &&
            (customerItems[customerNo].type.toString() != Mscrm.InternalUtilities.EntityTypeCode.Account.toString() &&
                customerItems[customerNo].type.toString() != Mscrm.InternalUtilities.EntityTypeCode.Contact.toString()))
            isValidCustomer = false;
        if (!isValidCustomer)
        ...
        if (Xrm.Utility.isMocaOffline()) {
            ...
            var columns = [];
            columns[0] = "statecode";
            Xrm.Internal.messages.retrieve(Xrm.Internal
                .getEntityName(typeof customerItems[customerNo].type != "number"
                    ? Number.parseInvariant(customerItems[customerNo].type)
                    : customerItems[customerNo].type),
                customerItems[customerNo].id,
                columns).then(function(response) {
                    if (typeof response.entity != "undefined" &&
                        typeof response.entity.getObjectData != "undefined" &&
                        typeof response.entity.getObjectData().fields != "undefined" &&
                        ...
                        for (var id = response.entity.get_key().toString(),
                            k = 0;
                            k < customerItems.length;
                            k++)
                            if (!IsNull(customerItems[k].id) &&
                                customerItems[k].id.toLowerCase().indexOf(id.toLowerCase()) != -1) {
...
function AppendValidCustomerAttributes(customerItems, customerNo, customerType) {
    if (customerType != "lead")
        return "&customerId=" +
            CrmEncodeDecode.CrmUrlEncode(customerItems[customerNo].id) +
            "&customerType=" +
            CrmEncodeDecode.CrmUrlEncode(customerItems[customerNo].type) +
            "&customerName=" +
            CrmEncodeDecode.CrmUrlEncode(customerItems[customerNo].name);
    else
        return "&leadId=" +
            CrmEncodeDecode.CrmUrlEncode(customerItems[customerNo].id) +
            "&leadName=" +
...
function GetValidCustomerSuccess(sUrlElement, iObjType, customerItems, customerType, entityId) {
    ...
    var height = 320,
        parameters = null;
    parameters = [iObjType];
    var convertToCaseActionCallBack = Mscrm.InternalUtilities.GridUtilities
            .createCallbackFunctionFactory(convertToCaseAction, parameters),
    ...
    if (Mscrm.InternalUtilities.DialogUtility.isMDDEnabled() &&
        Mscrm.InternalUtilities.DialogUtility.isMDDConverted("converttocase", Xrm.Page.data.entity.getEntityName()))
...
function convertToCaseAction(objRet, iObjType) {
    if (objRet)
        if (Xrm.Page.context.client.getClient() == Xrm.ClientName.outlook &&
            ...
            .isMDDConverted("converttocase", Xrm.Page.data.entity.getEntityName())) {
            ...
            var activityId = Xrm.Page.data.entity.getId(),
                ...
                customerTypeCode = typeof objRet.CustomerType != "number"
                    ? Number.parseInvariant(objRet.CustomerType)
                    : objRet.CustomerType,
                subjectTypeCode = typeof objRet.SubjectType != "number"
                    ? Number.parseInvariant(objRet.SubjectType)
                    : objRet.SubjectType;
            try {
                Mscrm.InternalUtilities.Sdk.convertToCase(function(response) {
                    ...
                    function(response) {
                        if (Mscrm.CommandBarActions.isMobileCompanionApp())
                            ...
                            Xrm.Internal.openErrorDialog(response.get_organizationServiceFault().get_errorCode(),
                                response.get_error().toString(),
                                response.get_organizationServiceFault().get_responseOuterXml())
                    },
                    Xrm.Page.data.entity.attributes.get("subject").getValue(),
                    activityId,
                    objectTypeCode,
                    customerid,
                    customerTypeCode,
        ...
        if (saveActivity == false)
            ...
            if (openNewRecord && caseId != "") {
                ...
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(),
                ...
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(), Xrm.Page.data.entity.getEntityName(), true)
        }
        RefreshParentGrid(iObjType)
    } else
        convertToEntityLegacy(caseId,
            saveActivity,
            openNewRecord,
            iObjType,
            Mscrm.InternalUtilities.EntityNames.Incident)
...
function convertToLead(iObjType) {
    ...
    var oUrl = Mscrm.CrmUri.create("/Activities/act_dlgs/convert_to_lead.aspx?" + sUrlElement),
        convertToLeadActionCallBack = Mscrm.InternalUtilities.GridUtilities
            .createCallbackFunctionFactory(convertToLeadAction, parameters),
        dialogOptions = new Xrm.DialogOptions;
...
function ConvertToLeadSuccess(response, saveActivity, openNewRecord, iObjType) {
    ...
    if (typeof response.ReturnValue != "undefined")
        ...
        leadId = response.id;
    else if (typeof response.get_recordId != "undefined" && typeof response.get_recordId() != "undefined")
    ...
    if (Mscrm.CommandBarActions.isMobileCompanionApp() || Mscrm.InternalUtilities.Utilities.isRefreshForm()) {
            ...
            if (openNewRecord && leadId != "") {
                ...
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(),
                        Xrm.Page.data.entity.getEntityName(),
                        false,
                        Mscrm.InternalUtilities.EntityNames.Lead,
                        leadId)
            } else
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(), Xrm.Page.data.entity.getEntityName(), true)
        ...
        convertToEntityLegacy(leadId,
            objRet.SaveActivity,
            objRet.openNewRecord,
            iObjType,
            Mscrm.InternalUtilities.EntityNames.Lead)
}
...
function GetValidForConvertToOpportunityWithLead(sUrlElement, iObjType) {
    ...
    if (!IsNull(regardingCtrl)) {
        ...
        if (!IsNull(regardingItems) && regardingItems.length == 1)
            if (regardingItems[0].type
                .toString() ==
                Mscrm.InternalUtilities.EntityTypeCode.CampaignActivity.toString() ||
                regardingItems[0].type.toString() == Mscrm.InternalUtilities.EntityTypeCode.BulkOperation.toString())
                sUrlElement += "&campaignId=" +
                    CrmEncodeDecode.CrmUrlEncode(regardingItems[0].id) +
                    "&campaignType=" +
                    CrmEncodeDecode.CrmUrlEncode(regardingItems[0].type) +
                    "&campaignName=" +
                    CrmEncodeDecode.CrmUrlEncode(regardingItems[0].name)
    }
    var subjectField = "",
        _subject = Xrm.Page.data.entity.attributes.get("subject");
    ...
    var isMDDConverted = Mscrm.InternalUtilities.DialogUtility
        .isMDDConverted("converttoopportunity", Xrm.Page.data.entity.getEntityName());
    if (isMDDConverted)
        ...
        var oUrl = Mscrm.CrmUri.create("/Activities/act_dlgs/convert_activity.aspx?" + sUrlElement),
            convertToOppActionCallBack = Mscrm.InternalUtilities.GridUtilities
                .createCallbackFunctionFactory(convertToOppAction, parameters);
            ...
            try {
                var activityTypeCode = typeof iObjType != "number" ? Number.parseInvariant(iObjType) : iObjType,
                    customerTypeCode = typeof objRet.CustomerType != "number"
                        ? Number.parseInvariant(objRet.CustomerType)
                        : objRet.CustomerType,
                    campaignTypeCode = typeof objRet.CampaignType != "number"
                        ? Number.parseInvariant(objRet.CampaignType)
                        : objRet.CampaignType,
                    ownerTypeCode = typeof objRet.ownerType != "number"
                        ? Number.parseInvariant(objRet.ownerType)
                        : objRet.ownerType;
            ...
            if (openNewRecord && oppId != "") {
                ...
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(),
                        Xrm.Page.data.entity.getEntityName(),
                        false,
                        Mscrm.InternalUtilities.EntityNames.Opportunity,
                        oppId)
            } else
                Mscrm.CommandBarActions
                    .markActivityComplete(Xrm.Page.data.entity.getId(), Xrm.Page.data.entity.getEntityName(), true)
...
function okToSave() {
    if (Xrm.Page.data.entity.getIsDirty() &&
        ...
        (Xrm.Page.data.entity.attributes.get("statecode").getValue() == 1 ||
            Xrm.Page.data.entity.attributes.get("statecode").getValue() == 2))
