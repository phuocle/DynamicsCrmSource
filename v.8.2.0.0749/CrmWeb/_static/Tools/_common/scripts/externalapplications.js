...
function SelectExternalPartyEntitiesButtonClicked() {
    var oUrl = Mscrm.CrmUri.create("/Tools/SystemSettings/Dialogs/ExternalPartyEnabledConfiguration.aspx"),
        callbackFunctionObject = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(ExternalPartyEnabledConfigCallback, [this]),
...
function SelectAutoCreateExternalPartyEntitiesButtonClicked() {
    var oUrl = Mscrm.CrmUri.create("/Tools/SystemSettings/Dialogs/AutoCreateExternalPartyEnabledConfiguration.aspx"),
        callbackFunctionObject = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(AutoCreateExternalPartyEnabledConfigCallback, [this]),
        dialogOptions = new Xrm.DialogOptions;
    dialogOptions.height = 600;
    dialogOptions.width = 800;
    Xrm.Internal.openDialog(oUrl.toString(), dialogOptions, null, null, callbackFunctionObject)
...
function ExternalPartyEnabledConfigCallback(returnval) {
    if (returnval && returnval["selectedentities"]) {
        for (var strToAppend = "",
            ...
            i = 0;
            i < returnval["selectedentities"].childNodes.length;
            i++) {
            newEntityLogicalName = returnval["selectedentities"].childNodes[i].attributes.getNamedItem("name").value;
            strToSearch = "<EntityLogicalName>" +
                CrmEncodeDecode.CrmXmlEncode(newEntityLogicalName) +
                "</EntityLogicalName>";
            if (_externalPartyEnabledEntitiesSettingsXml.indexOf(strToSearch) == -1) {
                if (strToAppend == "")
                    strToAppend = "<ExternalPartyEntity><EntityLogicalName>" +
                        CrmEncodeDecode.CrmXmlEncode(newEntityLogicalName) +
                        "</EntityLogicalName><IsAutoCreateExternalPartyEnabled>0</IsAutoCreateExternalPartyEnabled></ExternalPartyEntity>";
                else
                    strToAppend += "<ExternalPartyEntity><EntityLogicalName>" +
                        CrmEncodeDecode.CrmXmlEncode(newEntityLogicalName) +
                        "</EntityLogicalName><IsAutoCreateExternalPartyEnabled>0</IsAutoCreateExternalPartyEnabled></ExternalPartyEntity>";
                _externalPartyEnabledEntities.push(newEntityLogicalName);
                addRow("recordassociation",
                    "recordAssociationEntitiesTable",
                    newEntityLogicalName,
                    returnval["logicalAndDisplayNamesMapping"][newEntityLogicalName]);
                addRow("parentassociation",
                    "parentAssociationEntitiesTable",
                    newEntityLogicalName,
                    returnval["logicalAndDisplayNamesMapping"][newEntityLogicalName]);
                addRow("correlation",
                    "corrEntitiesTable",
        ...
        _externalPartyEnabledEntitiesSettingsXml = [
            _externalPartyEnabledEntitiesSettingsXml.slice(0, position), strToAppend,
            _externalPartyEnabledEntitiesSettingsXml.slice(position)
        ].join("");
        loadExternalPartyEnabledEntitiesSettings(_externalPartyEnabledEntitiesSettingsXml);
        var retval = new StringBuilder;
        retval.Append("<organization>");
        retval.Append("<externalpartyentitysettings>" +
            CrmEncodeDecode.CrmXmlEncode(_externalPartyEnabledEntitiesSettingsXml) +
...
function AutoCreateExternalPartyEnabledConfigCallback(returnval) {
    if (returnval) {
        var strToReplace = "",
            strToSearch = "",
            newEntityLogicalName = "";
        if (_externalPartyEnabledEntitiesSettingsXml
            .indexOf("<IsAutoCreateExternalPartyEnabled>1</IsAutoCreateExternalPartyEnabled>") !=
            -1)
            _externalPartyEnabledEntitiesSettingsXml = _externalPartyEnabledEntitiesSettingsXml
                .split("<IsAutoCreateExternalPartyEnabled>1</IsAutoCreateExternalPartyEnabled>")
                .join("<IsAutoCreateExternalPartyEnabled>0</IsAutoCreateExternalPartyEnabled>");
        for (var i = 0; i < returnval["SelectedAutoCreateExternalPartyEntities"].childNodes.length; i++) {
            newEntityLogicalName = returnval["SelectedAutoCreateExternalPartyEntities"].childNodes[i].attributes
                .getNamedItem("name").value;
            strToSearch = "<EntityLogicalName>" +
                CrmEncodeDecode.CrmXmlEncode(newEntityLogicalName) +
                "</EntityLogicalName><IsAutoCreateExternalPartyEnabled>0</IsAutoCreateExternalPartyEnabled>";
            if (_externalPartyEnabledEntitiesSettingsXml.indexOf(strToSearch) != -1) {
                strToReplace = "<EntityLogicalName>" +
                    CrmEncodeDecode.CrmXmlEncode(newEntityLogicalName) +
                    "</EntityLogicalName><IsAutoCreateExternalPartyEnabled>1</IsAutoCreateExternalPartyEnabled>";
                _externalPartyEnabledEntitiesSettingsXml = _externalPartyEnabledEntitiesSettingsXml
                    .replace(strToSearch, strToReplace)
            }
        }
        loadExternalPartyEnabledEntitiesSettings(_externalPartyEnabledEntitiesSettingsXml);
        var retval = new StringBuilder;
...
function sortTable(tableID) {
    for (var tableData = document.getElementById(tableID).getElementsByTagName("tbody").item(0),
        ...
        for (var j = 0; j < rowData.length - (i + 1); j++)
            rowData.item(j).getElementsByTagName("td").item(0).innerHTML >
...
function addRow(context, tableID, entityLogicalName, newEntityDisplayName) {
    ...
    newRow.onclick = function() {
        ...
        else if (context == "recordassociation")
    ...
    else if (context == "parentassociation")
...
function fetchExternalPartyFieldsForCorrelation(entityLogicalName) {
    if (this._selectedCorrEntity != entityLogicalName) {
        for (var corrEntities = document.getElementById("corrEntitiesTable"),
            i = 0,
            corrEntitiesRow;
            corrEntitiesRow = corrEntities.rows[i];
            i++)
        ...
        if (this._correlationFieldChangeWarning.hasOwnProperty(this._selectedCorrEntity) &&
            this._correlationFieldChangeWarning[this._selectedCorrEntity])
...
function fetchExternalPartyFieldsForRecordAssociation(entityLogicalName, entityDisplayName) {
    if (this._selectedRecordAssocEntity != entityLogicalName) {
        for (var recordAssocEntities = document.getElementById("recordAssociationEntitiesTable"),
            i = 0,
            recordAssocEntitiesRow;
            recordAssocEntitiesRow = recordAssocEntities.rows[i];
            i++)
        ...
        var RecordAssociationExternalPartyEnabledEntity = document
            .getElementById("recordAssociationExternalPartyEnabled_" + entityLogicalName);
        ...
        document.getElementById("recordAssociationDesc").innerHTML = _recordAssocDescription
            .replace("{0}", entityDisplayName);
        for (i = 0; i < _externalChannelEnabledEntities.length; i++)
            this.entityAttributeSelect(document
                .getElementById(_externalChannelEnabledEntities[i] + "record_asso_ext_channel_fields"),
                _externalChannelEnabledEntities[i],
                "recordassociation")
    }
}

function toggleParentAssocExtChannelVisiblity() {
    if (this._parentAssociationMap.hasOwnProperty(this._selectedParentAssocEntity) &&
        Object.keys(this._parentAssociationMap[this._selectedParentAssocEntity]).length != 0) {
        ...
        document.getElementById("parentAssocDesc").innerHTML = _parentAssocDescription
            .replace("{0}", parentFieldSelect.options[parentFieldSelect.selectedIndex].text);
        for (i = 0; i < _externalChannelEnabledEntities.length; i++)
            this.entityAttributeSelect(document
                .getElementById(_externalChannelEnabledEntities[i] + "parent_asso_ext_channel_fields"),
                _externalChannelEnabledEntities[i],
                "ParentAssocExtChannelFields")
    } else {
...
function fetchExternalPartyFieldsForParentAssociation(entityLogicalName, entityDisplayName) {
    if (this._selectedParentAssocEntity != entityLogicalName) {
        for (var parentAssocEntities = document.getElementById("parentAssociationEntitiesTable"),
            i = 0,
            parentAssocEntitiesRow;
            parentAssocEntitiesRow = parentAssocEntities.rows[i];
            i++)
        ...
        var ParenAssociationExternalPartyEnabledEntity = document
            .getElementById("parentAssociationExternalPartyEnabled_" + entityLogicalName);
        ...
        this.entityAttributeSelect(document.getElementById("parentAttributeField"),
            this._selectedParentAssocEntity,
    ...
    if (entityNodes) {
        ...
        for (var i = 0; i < entityNodes.length; i++)
            if (XUI.Xml.GetText(XUI.Xml.SelectSingleNode(entityNodes[i], "name", null)) === entityName) {
                ...
                if (context == "correlation" &&
                    Object.keys(this._correlationKeyMap).length != 0 &&
                    this._correlationKeyMap.hasOwnProperty(entityName))
                    attributeSelect.value = this._correlationKeyMap[entityName];
                else if (context == "recordassociation" &&
                    Object.keys(this._recordAssociationMap).length != 0 &&
                    this._recordAssociationMap.hasOwnProperty(this._selectedRecordAssocEntity)) {
                    if (Object.keys(this._recordAssociationMap[this._selectedRecordAssocEntity]).length != 0 &&
                        this._recordAssociationMap[this._selectedRecordAssocEntity].hasOwnProperty(entityName))
                        attributeSelect.value = this._recordAssociationMap[this._selectedRecordAssocEntity][entityName]
                } else if (context == "ParentAssocExtChannelFields" &&
                    this._parentAssociationMap[this._selectedParentAssocEntity][Object
                        .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]]
                    .hasOwnProperty(entityName))
                    attributeSelect.value = this._parentAssociationMap[this._selectedParentAssocEntity][Object
                        .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]][entityName];
                else if (context == "parentAssociationField" &&
                    Object.keys(this._parentAssociationMap).length != 0 &&
                    this._parentAssociationMap.hasOwnProperty(this._selectedParentAssocEntity))
                    if (Object.keys(this._parentAssociationMap[this._selectedParentAssocEntity]).length != 0)
                        attributeSelect.value = Object.keys(this._parentAssociationMap[this
                            ._selectedParentAssocEntity])[0];
                return true
            }
    }
    return false
...
function getAttributesForEntity(entityName, context) {
    ...
    if (context == "correlation")
        ...
        context == "ParentAssocExtChannelFields" ||
        context == "parentAssociationField")
...
function loadCorrelationKeySettings(corrKeyMapSettingsXML) {
    ...
    if (externalPartyUniquekeys)
        for (var i = 0; i < externalPartyUniquekeys.length; i++)
            this._correlationKeyMap[XUI.Xml.GetText(XUI.Xml
                    .SelectSingleNode(externalPartyUniquekeys[i], "EntityLogicalName", null))] = XUI.Xml
                .GetText(XUI.Xml.SelectSingleNode(externalPartyUniquekeys[i], "CorrelationField", null))
}
...
function loadExternalPartyEnabledEntitiesSettings(ExternalPartyEnabledEntitiesSettingsXml) {
    ...
    if (externalPartyEnabledEntities)
        for (var i = 0; i < externalPartyEnabledEntities.length; i++) {
            this._autoCreateExternalPartyMap[XUI.Xml
                    .GetText(XUI.Xml
                        .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))] = XUI
                .Xml.GetText(XUI.Xml.SelectSingleNode(externalPartyEnabledEntities[i],
                    "IsAutoCreateExternalPartyEnabled",
                    null));
            this._recordAssociationMap[XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))] = {};
            externalChannelEntities = XUI.Xml.SelectNodes(externalPartyEnabledEntities[i],
                "ExternalChannelAccessEnabledEntity",
                null);
            for (var j = 0; j < externalChannelEntities.length; j++)
                this._recordAssociationMap[XUI.Xml.GetText(XUI.Xml
                        .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))][XUI.Xml
                        .GetText(XUI.Xml.SelectSingleNode(externalChannelEntities[j], "EntityLogicalName", null))] = XUI
                    .Xml
                    .GetText(XUI.Xml.SelectSingleNode(externalChannelEntities[j], "AssociatedField", null));
            this._parentAssociationMap[XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))] = {};
            externalPartyParentRecords = externalPartyEnabledEntities[i]
                .getElementsByTagName("ExternalPartyParentRecord");
            for (var externalPartyParentChannelAccessEntities,
                j = 0;
                j < externalPartyParentRecords.length;
                j++) {
                this._parentAssociationMap[XUI.Xml.GetText(XUI.Xml
                    .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))][XUI.Xml
                    .GetText(XUI.Xml.SelectSingleNode(externalPartyParentRecords[j], "FieldLogicalName", null))] = {};
                externalPartyParentChannelAccessEntities = externalPartyParentRecords[j]
                    .getElementsByTagName("ExternalChannelAccessEnabledEntity");
                for (var k = 0; k < externalPartyParentChannelAccessEntities.length; k++)
                    this._parentAssociationMap[XUI.Xml
                        .GetText(XUI.Xml
                            .SelectSingleNode(externalPartyEnabledEntities[i], "EntityLogicalName", null))][XUI.Xml
                        .GetText(XUI.Xml.SelectSingleNode(externalPartyParentRecords[j], "FieldLogicalName", null))][XUI
                        .Xml.GetText(XUI.Xml
                            .SelectSingleNode(externalPartyParentChannelAccessEntities[k], "EntityLogicalName", null))
                    ] = XUI.Xml.GetText(XUI.Xml
                        .SelectSingleNode(externalPartyParentChannelAccessEntities[k], "AssociatedField", null))
            }
        }
}

function onCorrelationKeyMapFieldChange(o) {
    if (this._correlationKeyMap.hasOwnProperty(this._selectedCorrEntity) &&
        !(this.corrFieldChangeInSameSession.hasOwnProperty(this._selectedCorrEntity) &&
            this.corrFieldChangeInSameSession[this._selectedCorrEntity])) {
        document.getElementById("corrFieldChangeWarning").className += " ms-crm-Visible";
    ...
    else if (this._correlationKeyMap[this._selectedCorrEntity])
...
function onRecordAssociationFieldChange(o) {
    ...
    else if (this._recordAssociationMap.hasOwnProperty(this._selectedRecordAssocEntity) &&
        this._recordAssociationMap[this._selectedRecordAssocEntity].hasOwnProperty(externalChannelEntityName))
...
function onParentFieldChange(o) {
    ...
    } else if (this._parentAssociationMap.hasOwnProperty(this._selectedParentAssocEntity) &&
        Object.keys(this._parentAssociationMap[this._selectedParentAssocEntity]).length != 0) {
        for (var i = 0;
            i <
                Object.keys(this._parentAssociationMap[this._selectedParentAssocEntity][Object
                    .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]]).length;
            i++)
            delete this._parentAssociationMap[this._selectedParentAssocEntity][Object
                .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]][Object
                .keys(this._parentAssociationMap[this._selectedParentAssocEntity][Object
                    .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]])[i]];
...
function onParentAssociationFieldChange(o) {
    ...
    if (o.value != "blank")
        this._parentAssociationMap[this._selectedParentAssocEntity][Object
            .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]][externalChannelEntityName] = o.value;
    else if (this._parentAssociationMap[this._selectedParentAssocEntity][Object
            .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]]
        .hasOwnProperty(externalChannelEntityName))
        delete this._parentAssociationMap[this._selectedParentAssocEntity][Object
            .keys(this._parentAssociationMap[this._selectedParentAssocEntity])[0]][externalChannelEntityName]
}
...
function getExternalPartyCorrKeyMapDataXml() {
    ...
    for (var i = 0; i < Object.keys(this._correlationKeyMap).length; i++) {
        ...
        correlationKeyMapSettingsXml += "<EntityLogicalName>" +
            Object.keys(this._correlationKeyMap)[i] +
            "</EntityLogicalName>";
        correlationKeyMapSettingsXml += "<CorrelationField>" +
            this._correlationKeyMap[Object.keys(this._correlationKeyMap)[i]] +
            "</CorrelationField>";
        correlationKeyMapSettingsXml += "</ExternalPartyEntity>"
    }
...
function getExternalPartyEnableEntitiesSettingsXml() {
    ...
    for (i = 0; i < _externalPartyEnabledEntities.length; i++) {
        ...
        externalPartyEnabledEntitiesSettingsXml += "<EntityLogicalName>" +
            _externalPartyEnabledEntities[i] +
            "</EntityLogicalName>";
        if (this._autoCreateExternalPartyMap.hasOwnProperty(_externalPartyEnabledEntities[i]))
            externalPartyEnabledEntitiesSettingsXml += "<IsAutoCreateExternalPartyEnabled>" +
                this._autoCreateExternalPartyMap[_externalPartyEnabledEntities[i]] +
                "</IsAutoCreateExternalPartyEnabled>";
        else
            externalPartyEnabledEntitiesSettingsXml +=
                "<IsAutoCreateExternalPartyEnabled>0</IsAutoCreateExternalPartyEnabled>";
        if (this._recordAssociationMap.hasOwnProperty(_externalPartyEnabledEntities[i]))
            for (var j = 0; j < Object.keys(this._recordAssociationMap[_externalPartyEnabledEntities[i]]).length; j++) {
                ...
                externalPartyEnabledEntitiesSettingsXml += "<EntityLogicalName>" +
                    Object.keys(this._recordAssociationMap[_externalPartyEnabledEntities[i]])[j] +
                    "</EntityLogicalName>";
                externalPartyEnabledEntitiesSettingsXml += "<AssociatedField>" +
                    this._recordAssociationMap[_externalPartyEnabledEntities[i]][Object
                        .keys(this._recordAssociationMap[_externalPartyEnabledEntities[i]])[j]] +
                    "</AssociatedField>";
                externalPartyEnabledEntitiesSettingsXml += "</ExternalChannelAccessEnabledEntity>"
            }
        if (this._parentAssociationMap.hasOwnProperty(_externalPartyEnabledEntities[i]))
            for (var j = 0; j < Object.keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]]).length; j++) {
                ...
                externalPartyEnabledEntitiesSettingsXml += "<FieldLogicalName>" +
                    Object.keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]])[j] +
                    "</FieldLogicalName>";
                for (var k = 0;
                    k <
                        Object.keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]][Object
                            .keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]])[j]]).length;
                    k++) {
                    externalPartyEnabledEntitiesSettingsXml += "<ExternalChannelAccessEnabledEntity>";
                    var externalChannelEntityKey = Object
                        .keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]][Object
                            .keys(this._parentAssociationMap[_externalPartyEnabledEntities[i]])[j]])[k];
                    externalPartyEnabledEntitiesSettingsXml +=
                        "<EntityLogicalName>" + externalChannelEntityKey + "</EntityLogicalName>";
                    externalPartyEnabledEntitiesSettingsXml +=
                        "<AssociatedField>" +
                        this._parentAssociationMap[_externalPartyEnabledEntities[i]][Object
                            .keys(this
                                ._parentAssociationMap[_externalPartyEnabledEntities[i]])[j]][externalChannelEntityKey
                        ] +
                        "</AssociatedField>";
                    externalPartyEnabledEntitiesSettingsXml += "</ExternalChannelAccessEnabledEntity>"
                }
