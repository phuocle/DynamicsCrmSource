...
function ForceSubmitRuleForm(sender, args) {
    ...
    if (aspxVars_IsDuplicateRulePublished == "true")
        if (crmFormCtrl.get_isDirty() == true && $get("crmFormSubmitXml").value.indexOf("<iscasesensitive>") != -1 ||
            isConditionsDirty == true)
...
function onBaseRecordTypeChange() {
    ...
    if (baseEntityCode != aspxVars_utilNoneEntityCode) {
        ...
        url = Mscrm.CrmUri
            .create("/tools/duplicatedetection/duplicatedetectionrules/duplicaterulecondition.aspx?baseentitycode=" +
                baseEntityCode +
                "&matchingentitycode=" +
