...
var _oConst = {
        ...
        sEntityEditorUrlFormat: "/tools/systemcustomization/Entities/manageEntity.aspx?entityId={0}",
        sInformationPageId: "divInformation",
        sFormsPageId: "divForms",
        sViewsPageId: "divViews",
        sUIElementsPageId: "divUIElements",
        sVisualizationsPageId: "divVisualizations",
        ...
        sBusinessRulesListPage: "divBusinessRules",
        sHierarchyRulesListPage: "divHierarchyRules",
        sSimilarityRulesListPage: "divSimilarityRules",
        sAlternateKeysListPage: "divAlternateKeys",
        sDashboardsPage: "divDashboards"
    },
    _oMessages = {
        sPublishAlert: LOCID_MANENT_PUBLISHALERT,
        sCloseAlert: LOCID_FORMS_SAVE_CONFIRM_TITLE,
        sPublishUnsavedAlert: LOCID_MESSAGE_PUBLISHUNSAVED
    },
    _bSaving = false,
    _bReadyState = false,
    _sInputXml = "",
...
function remoteEntityCommandCallback(oResult, oCallbackParams) {
    if (oResult.Success || oResult.get_success && oResult.get_success()) {
        ...
        } else if (oResult.get_remoteCommandXml().get_command() == _oEntUtlConst.sCreateEntityCommand) {
            var iEntityIdStart = oResult.get_returnValue().indexOf("<entityid>");
            if (iEntityIdStart != -1) {
                iEntityIdStart += 10;
                _sEntityId = CrmEncodeDecode.CrmXmlDecode(oResult.get_returnValue()
                    .substring(iEntityIdStart, oResult.get_returnValue().indexOf("</entityid>")));
                isNewEntity = true
            }
        }
...
function remoteEntityCommandCallbackAsync(oResult, oCallbackParams) {
    ...
    !IsNull(oResult) &&
        !oResult.get_success() &&
        ViewXml(XUI.Xml.XMLSerializer.serializeToString(oResult.get_rawResponse()))
}
...
function IsRequestFromAppDesigner() {
    ...
    if (Mscrm.FeatureControl.get_Current()
        ...
        returnValue = !IsNull(window.parent) &&
...
function SendResponseToAppDesigner(isNewEntity, entityId, oResult) {
    if (IsRequestFromAppDesigner()) {
        var command = oResult.Success ? oResult.RemoteCommand.Command : oResult.get_remoteCommandXml().get_command(),
            isPublish = command == _oEntUtlConst.sPublishAllCustomizationsCommand ||
                command == _oEntUtlConst.sPublishCustomizationsCommand;
        if (isNewEntity || isPublish) {
            var entityDisplayName = Mscrm.FormControlInputBehavior.GetBehavior($get("txtSingularName").id)
                    .get_dataValue(),
                entityPluralName = Mscrm.FormControlInputBehavior.GetBehavior($get("txtPluralName").id).get_dataValue(),
                entityLogicalName = "new_" +
                    Mscrm.FormControlInputBehavior.GetBehavior($get("txtSchemaName").id).get_dataValue();
            if (!isNullOrEmptyString(entityId))
                entityId = entityId.replace(/{|}/g, "").toLowerCase();
            var resultobj = {
                id: entityId,
                displayName: entityDisplayName,
                params: { isNewEntity: isNewEntity, pluralname: entityPluralName, logicalName: entityLogicalName }
            };
...
function saveEntityAction(bClose) {
    if (!_bSaving && validateParameters()) {
        ...
        if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) &&
            ...
            !IsNull(Mscrm.CustomControls.CustomControlManager))
            if (isCustomControlsTabDirty()) {
                var entityCustomControlXmlDoc = Mscrm.CustomControls.CustomControlManager.get_instance()
                    .generateGridSnippetForEntity();
                if (!IsNull(entityCustomControlXmlDoc)) {
                    ...
                    if (!IsNull(sCustomControlXml)) {
                        ...
                        window.setTimeout(function() {
                                createOrupdateCustomControlDefaultConfig(sCustomControlXml,
...
function addVisibleInMobileXml(oDataXml) {
    ...
    if (!IsNull($get("numDaysSinceRecordLastModified")))
        ...
        typeof _mobileOfflineFilters != "undefined" &&
            !IsNull(_mobileOfflineFilters) &&
...
function validateParameters() {
    ...
    if ($get("numDaysSinceRecordLastModified") &&
        !$get("numDaysSinceRecordLastModified").disabled &&
        entityXml.getElementsByTagName("dayssincerecordlastmodified").length > 0) {
        var daysSinceRecordLastModifiedNode = entityXml.getElementsByTagName("dayssincerecordlastmodified")[0]
                .childNodes[0],
        ...
        if (_bCreate ||
            oldValue != daysSinceRecordLastModified.get_dataValue() ||
            daysSinceRecordLastModified.get_dataValue() == 0) {
            ...
            bValid = bValid &&
                validateDaysSinceRecordLastModifiedValue(daysSinceRecordLastModified.get_dataValue(),
                    oldValue,
                    EnablingMobileOffline)
        }
    }
    var _chkMobileClientOffline = $get("chkIsOfflineForMobileClient");
    if (_chkMobileClientOffline != null &&
        _chkMobileClientOffline.checked &&
        typeof _isMobileOfflineFiltersFeatureAvailable != "undefined" &&
        _isMobileOfflineFiltersFeatureAvailable == true) {
        var EnablingMobileOffline = false;
        if (entityXml.getElementsByTagName("offlineinmobileclient").length > 0)
            if (entityXml.getElementsByTagName("offlineinmobileclient")[0].childNodes[0].nodeValue == "false")
                EnablingMobileOffline = true;
        if (typeof _mobileOfflineFilters != "undefined" &&
            !isNullOrEmptyString(_mobileOfflineFilters) &&
            entityXml.getElementsByTagName("mobileofflinefilters") != null &&
            entityXml.getElementsByTagName("mobileofflinefilters").length > 0 &&
            entityXml.getElementsByTagName("mobileofflinefilters")[0].childNodes.length > 0) {
            ...
            if (_bCreate || oldValue != currentValue || currentConditions == 0)
                bValid = bValid &&
                    validateMobileOfflineFiltersValue(currentConditions, currentValue, oldValue, EnablingMobileOffline)
        } else if (EnablingMobileOffline &&
            typeof _mobileOfflineFilters != "undefined" &&
            isNullOrEmptyString(_mobileOfflineFilters))
...
function onSingularNameChange() {
    ...
    if (_bCreate &&
        (_txtSingularName.get_dataValue() != null && _txtSingularName.get_dataValue().length > 0) &&
        (oSchemaName.get_dataValue() == null || oSchemaName.get_dataValue().length == 0)) {
        var regExp = new RegExp(_oConst.sInvalidSchemaNameChars, "g");
        oSchemaName.set_dataValue(_txtSingularName.get_dataValue().replace(regExp, "")
            .substr(0, oSchemaName.get_element().maxLength));
        oSchemaName.get_dataValue() != null &&
            oSchemaName.get_dataValue().length > 0 &&
...
function onAttributeSchemaNameChange() {
    ...
    if (_bCreate &&
        (oAttributeDisplayName.get_dataValue() != null && oAttributeDisplayName.get_dataValue().length > 0) &&
        (oAttributeSchemaName.get_dataValue() == null || oAttributeSchemaName.get_dataValue().length == 0)) {
        var regExp = new RegExp(_oConst.sInvalidSchemaNameChars, "g");
        oAttributeSchemaName.set_dataValue(oAttributeDisplayName.get_dataValue().replace(regExp, "")
            .substr(0, oAttributeDisplayName.get_element().maxLength));
...
function isCustomControlsTabDirty() {
    try {
        var entityCustomControlXmlDoc = Mscrm.CustomControls.CustomControlManager.get_instance()
                .generateGridSnippetForValidation(),
        ...
        if (!IsNull(entityCustomControlXmlDoc) && IsNull(existingCustomControlXml) ||
            IsNull(entityCustomControlXmlDoc) && !IsNull(existingCustomControlXml))
            return true;
        if (!IsNull(entityCustomControlXmlDoc) && !IsNull(existingCustomControlXml))
