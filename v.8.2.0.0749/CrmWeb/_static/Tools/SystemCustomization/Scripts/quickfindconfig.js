...
function SaveFormForRelevanceSearch(closePage) {
    ...
    saveCommand
        .SetParameter("solutionId", parent.APP_SOLUTION_ID == "" ? this.APP_SOLUTION_ID : parent.APP_SOLUTION_ID);
...
function SaveForm(closePage) {
    ...
    else if (oRowCount > MAX_ENTITIES_ALLOWED)
...
function UpdateProgressBar() {
    if (IS_RELEVANCE_SEARCH) {
        ...
        progressBarStatus.innerHTML = String.format(PROGRESS_BAR_STATUS_FORMAT,
            percentageUsed,
            totalFields,
            RELEVANCE_SEARCH_FIELD_LIMIT)
    }
}
