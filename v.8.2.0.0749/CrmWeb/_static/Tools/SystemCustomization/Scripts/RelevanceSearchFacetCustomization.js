...
function onFilterBoxChange() {
    ...
    DisplayRecords(entity);
    var bTypes = [
        { bType: "Add" }, { bType: "Remove" }, { bType: "MoveUp" }, { bType: "MoveDown" }, { bType: "DefaultButton" }
    ];
...
function DisplayRecords(entity) {
    ...
    if (entityRecord != null) {
        if (entityRecord.modifiedsequence != null &&
            entityRecord.modifiedsequence[0] != null &&
            entityRecord.modifiedsequence[0].availableAttributeList != null) {
            for (var html = "<ul>",
                j = 0;
                j < entityRecord.modifiedsequence[0].availableAttributeList.length;
                j++) {
                html += "<li title='" +
                    entityRecord.modifiedsequence[0].availableAttributeList[j].displayName +
                    "' class='ms-crm-Dialog-ListItem' name='" +
                    entityRecord.modifiedsequence[0].availableAttributeList[j].logicalName +
                    "' tabIndex='0'>";
                html +=
                    "<a style='display: block;' ondblclick='DoubleClick(new Sys.UI.DomEvent(event));' onmousedown='SelectRow(new Sys.UI.DomEvent(event));' onclick='return false;' onmouseover='OnFocus(new Sys.UI.DomEvent(event)); onmouseout=OnBlur(new Sys.UI.DomEvent(event));>";
                html += "<input title='" +
                    entityRecord.modifiedsequence[0].availableAttributeList[j].displayName +
                    "' class='ms-crm-Dialog-ListItemSelect' type='checkbox' id='chk" +
                    entityRecord.modifiedsequence[0].availableAttributeList[j].logicalName +
                    "' style='display:none'/>";
                html += "<label for='chk" +
                    entityRecord.modifiedsequence[0].availableAttributeList[j].logicalName +
        ...
        if (entityRecord.modifiedsequence != null &&
            entityRecord.modifiedsequence[0] != null &&
            entityRecord.modifiedsequence[0].selectedAttributeList != null) {
            for (var html = "<ul>",
                j = 0;
                j < entityRecord.modifiedsequence[0].selectedAttributeList.length;
                j++) {
                html += "<li title='" +
                    entityRecord.modifiedsequence[0].selectedAttributeList[j].displayName +
                    "' class='ms-crm-Dialog-ListItem' name='" +
                    entityRecord.modifiedsequence[0].selectedAttributeList[j].logicalName +
                    "' tabIndex='0'>";
                html +=
                    "<a style='display: block;' ondblclick='DoubleClick(new Sys.UI.DomEvent(event));' onmousedown='SelectRow(new Sys.UI.DomEvent(event));' onclick='return false;' onmouseover='OnFocus(new Sys.UI.DomEvent(event)); onmouseout=OnBlur(new Sys.UI.DomEvent(event));>";
                html += "<input title='" +
...
function IsMultiSelectCriteriaFails() {
    ...
    if (GetListName() == "AvailableEntities" &&
        selectedCounter > 0 &&
        availableEntityAttributeCollection[entity].modifiedsequence[0].selectedAttributeList.length + selectedCounter >
        4) {
        var bTypes = [
            { bType: "Add" }, { bType: "Remove" }, { bType: "MoveUp" }, { bType: "MoveDown" },
            { bType: "DefaultButton" }
        ];
        ChangeButtonVisibility(bTypes, true, selectedCounter, GetListName());
        DisableKeyboardEventWhenMultiSelectCriteriaFails(true);
        return
    } else if (GetListName() == "SelectedEntities" &&
        selectedCounter > 0 &&
        availableEntityAttributeCollection[entity].modifiedsequence[0].selectedAttributeList.length - selectedCounter <
        1) {
        var bTypes = [
    ...
    for (var parsedRecordType = [],
        ...
        if (d.ismodified && !d.isdefault) {
            for (var obj,
                attributeList = [],
                j = 0;
                j < d.modifiedsequence[0].selectedAttributeList.length;
                j++)
...
function DoubleClick(event) {
    ...
    if (_oRow.nextSibling == null && _oRow.previousSibling == null && oList.parentElement.id == "SelectedEntities" ||
        oList.parentElement.id == "AvailableEntities" &&
        availableEntityAttributeCollection[entity].modifiedsequence[0].selectedAttributeList.length == 4)
        return;
    UnselectRows(oList);
    _oRowWithoutShift = _oRow;
    GetRowCheckbox(_oRow).checked = true;
...
function AddRecordToJson(entity, column) {
    for (var d = availableEntityAttributeCollection[entity],
        i = 0;
        i < d.modifiedsequence[0].availableAttributeList.length;
        i++)
...
function RemoveRecordFromJson(entity, column) {
    for (var d = availableEntityAttributeCollection[entity],
        i = 0;
        i < d.modifiedsequence[0].selectedAttributeList.length;
        i++)
...
function MoveDownRecordInJson(entity, column) {
    for (var d = availableEntityAttributeCollection[entity],
        ...
        i++)
        if (d.modifiedsequence[0].selectedAttributeList[i].logicalName == column &&
            i != d.modifiedsequence[0].selectedAttributeList.length - 1) {
            var temp = d.modifiedsequence[0].selectedAttributeList[i + 1];
            d.modifiedsequence[0].selectedAttributeList[i + 1] = d.modifiedsequence[0].selectedAttributeList[i];
            d.modifiedsequence[0].selectedAttributeList[i] = temp;
            d.isdefault = false;
...
function IsButtonEligibleForThisAction(bType, isEnable, selectedliCount, listType, entity) {
    if (bType == "Add") {
        if (listType == "AvailableEntities")
            if (availableEntityAttributeCollection[entity].modifiedsequence[0].selectedAttributeList.length +
                selectedliCount <=
                4)
                return false;
            else
    ...
    if (bType == "Remove")
        if (listType == "AvailableEntities")
            return true;
        else if (listType == "SelectedEntities")
            if (availableEntityAttributeCollection[entity].modifiedsequence[0].selectedAttributeList.length >
                selectedliCount)
                return false;
            else
...
function SetDefault() {
    ...
    if (entityRecord != null) {
        ...
        if (entityRecord.defaultsequence != null &&
            entityRecord.defaultsequence[0] != null &&
            entityRecord.defaultsequence[0].availableAttributeList != null) {
            for (var html = "<ul>",
                j = 0;
                j < entityRecord.defaultsequence[0].availableAttributeList.length;
                j++) {
                html += "<li title='" +
                    entityRecord.defaultsequence[0].availableAttributeList[j].displayName +
                    "' class='ms-crm-Dialog-ListItem' name='" +
                    entityRecord.defaultsequence[0].availableAttributeList[j].logicalName +
                    "' tabIndex='0'>";
                html +=
                    "<a style='display: block;' ondblclick='DoubleClick(new Sys.UI.DomEvent(event));' onmousedown='SelectRow(new Sys.UI.DomEvent(event));' onclick='return false;' onmouseover='OnFocus(new Sys.UI.DomEvent(event)); onmouseout=OnBlur(new Sys.UI.DomEvent(event));>";
                html += "<input title='" +
                ...
                if (entityRecord.modifiedsequence[0].availableAttributeList[j] != null) {
                    entityRecord.modifiedsequence[0].availableAttributeList[j].logicalName = entityRecord
                        .defaultsequence[0].availableAttributeList[j].logicalName;
                    entityRecord.modifiedsequence[0].availableAttributeList[j].displayName = entityRecord
                        .defaultsequence[0].availableAttributeList[j].displayName
                } else {
                    var obj = {
                        logicalName: entityRecord.defaultsequence[0].availableAttributeList[j].logicalName,
                        displayName: entityRecord.defaultsequence[0].availableAttributeList[j].displayName
                    };
        ...
        if (entityRecord.defaultsequence != null &&
            entityRecord.defaultsequence[0] != null &&
            entityRecord.defaultsequence[0].selectedAttributeList != null) {
            for (var html = "<ul>",
                j = 0;
                j < entityRecord.defaultsequence[0].selectedAttributeList.length;
                j++) {
                html += "<li title='" +
                    entityRecord.defaultsequence[0].selectedAttributeList[j].displayName +
                    "' class='ms-crm-Dialog-ListItem' name='" +
                    entityRecord.defaultsequence[0].selectedAttributeList[j].logicalName +
                    "' tabIndex='0'>";
                html +=
                    "<a style='display: block;' ondblclick='DoubleClick(new Sys.UI.DomEvent(event));' onmousedown='SelectRow(new Sys.UI.DomEvent(event));' onclick='return false;' onmouseover='OnFocus(new Sys.UI.DomEvent(event)); onmouseout=OnBlur(new Sys.UI.DomEvent(event));>";
                html += "<input title='" +
                ...
                if (entityRecord.modifiedsequence[0].selectedAttributeList[j] != null) {
                    entityRecord.modifiedsequence[0].selectedAttributeList[j].logicalName = entityRecord
                        .defaultsequence[0].selectedAttributeList[j].logicalName;
                    entityRecord.modifiedsequence[0].selectedAttributeList[j].displayName = entityRecord
                        .defaultsequence[0].selectedAttributeList[j].displayName
                } else {
                    var obj = {
                        logicalName: entityRecord.defaultsequence[0].selectedAttributeList[j].logicalName,
                        displayName: entityRecord.defaultsequence[0].selectedAttributeList[j].displayName
                    };
