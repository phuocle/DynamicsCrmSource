...
function GetFieldCellHtml(oCell) {
    ...
    var sFieldDisplayName = formatString(oFieldInfo.GridFieldDisplayMask,
            oFieldInfo.InvalidField ? sFieldName : oFieldInfo.DisplayName),
        sTitle = formatString(LOCID_VIEWEDITOR_COLTOOLTIP, sFieldDisplayName),
        sErrTitle = LOCID_VIEWEDITOR_COLERR_TOOLTIP,
        sErrImg = "<IMG alt='" +
            CrmEncodeDecode.CrmHtmlAttributeEncode(sErrTitle) +
            "' src='/_imgs/error/notif_icn_warn16.png' onclick='alert(this.alt)'/>&nbsp;",
        sNewTdInnerHTML =
...
function ViewProperties(mode, name, description) {
    ...
    if (!IsNull(crmForm.iscustomizable))
        aViewInfo = [
            crmForm.name.value, crmForm.description.value, crmForm.iscustomizable.value, crmForm.querytype.value
        ];
    else
    ...
    var parameters = [crmForm, mode, name, description],
        viewPropertiesCallback = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(callBackForviewProperties, parameters);
...
function AddColumns(oActive, bQuickFindMode) {
    ...
    var parameters = [oArgs, oActive],
        callbackObject = Mscrm.Utilities
            .createCallbackFunctionObject("performActionAfterAddColumns", this, parameters, false),
...
function performActionAfterAddingColumns(oFieldObjs, oFetchXml) {
    if (oFieldObjs) {
        for (var sOperator,
            ...
            sFilterXml += '<condition attribute="' +
                oFieldObjs[i].FieldName +
                '" operator="' +
                sOperator +
                '" value="' +
                sValue +
        ...
        if (!IsNull(oOldFilter))
            ...
            XUI.Xml.SelectSingleNode(oFetchXml, "/fetch/entity", null)
                .appendChild(XUI.Xml.SelectSingleNode(_oXmlDoc, "filter", null));
        UpdateFormData()
    }
}
...
function ColumnProperties(oActive) {
    ...
    if (oActive == null)
        ...
        try {
            if (oActive.getAttribute("disableMetaDataBinding") != null &&
                oActive.getAttribute("disableMetaDataBinding") == "1")
            ...
            oArgs.ImageProviderFunctionName = GetGridCellNode(oGridXml, sFieldName)
            ...
            if (!IsNull(GetGridCellNode(oGridXml, sFieldName).getAttribute("imageproviderwebresource")))
                ImageProviderWebResource = GetGridCellNode(oGridXml, sFieldName)
        ...
        var parameters = [size, sFieldName, oActive, oFieldInfo, oArgs.bQuickFindMode],
            callbackFunctionObject = Mscrm.Utilities
                .createCallbackFunctionObject("ColumnPropertiesCallback", this, parameters, false),
            crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri
                .create("/Tools/ViewEditor/Dialogs/ColumnProperties.aspx?size=" +
                    CrmEncodeDecode.CrmUrlEncode(size) +
                    "&entityName=" +
                    CrmEncodeDecode.CrmUrlEncode(oFieldInfo.EntityName) +
                    "&attributeName=" +
                    CrmEncodeDecode.CrmUrlEncode(oFieldInfo.SchemaName) +
                    "&ImageProviderWebResource=" +
                    CrmEncodeDecode.CrmUrlEncode(ImageProviderWebResource) +
                    "&FromExportExcel=" +
                    CrmEncodeDecode.CrmUrlEncode(_bFromExportExcel) +
                    "&queryApiBasedView=" +
                    CrmEncodeDecode.CrmUrlEncode(bQueryAPIBasedView)),
                oArgs,
                500,
                500,
                null);
...
function ColumnPropertiesCallback(oReturnValue, size, sFieldName, oActive, oFieldInfo, bQuickFindMode) {
    if (!IsNull(oReturnValue)) {
        ...
        !IsNull(oReturnValue.ImageProviderFunctionName) &&
            GetGridCellNode(oGridXml, sFieldName)
            .setAttribute("imageproviderfunctionname", oReturnValue.ImageProviderFunctionName);
        !IsNull(oReturnValue.ImageProviderWebResource) &&
            GetGridCellNode(oGridXml, sFieldName)
            .setAttribute("imageproviderwebresource", "$webresource:" + oReturnValue.ImageProviderWebResource);
        if (oReturnValue.PresenceEnabled != null) {
            ...
            if (relationshipIdToUse == "")
                ...
                var sAlias = "",
                    oAlias = XUI.Xml.SelectSingleNode(oEntitiesXml,
                        'entities/entity[@relid="' + relationshipIdToUse + '"]/@alias',
                        null);
                if (oAlias != null)
                    sAlias = oAlias.value;
                if (sAlias !== "")
                    if (oReturnValue.PresenceEnabled) {
                        ...
                        _oAllFieldsXml = oFieldsXml;
                        _oAllFieldPropertiesXml = oPropertiesXml;
                        GetFieldsXmlInternal(oReturnValue.EntityName,
                            sAlias,
                            oGridXml,
                            bQuickFindMode,
                            false,
    ...
    if (!IsNull(oOrderNodes) && oOrderNodes.length > 0) {
        for (var sortColumnDictionary = {},
            ...
            if (IsNull(orderDirAttr))
                ...
                sortColumnDictionary[oOrderNodes[i]
                    .getAttribute("attribute")] = oOrderNodes[i].getAttribute("descending") == "true"
        }
...
function SetSort() {
    ...
    if (hasSortableColumns()) {
        ...
        var callbackFunctionObject = Mscrm.Utilities
                .createCallbackFunctionObject("SetSortCallback", this, parameters, false),
            crmDialog = new Mscrm.CrmDialog(Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/SetSort.aspx"),
                oArgs,
                420,
                345,
                null);
...
function SetSortCallback(aSetSort, oOrderNodes) {
    if (aSetSort)
        if (aSetSort[0]) {
            ...
            else if (aSetSort[4] != "undefinedselectattribute")
...
function Filter() {
    ...
    options.width = 600;
    options.height = 500;
    var url = Mscrm.CrmUri.create("/Tools/ViewEditor/Dialogs/SetFilters.aspx?entityCode=" +
        $get("crmFormSubmit").crmFormSubmitObjectTypeCode.value);
    ...
    if (oReturned)
        if (oReturned.bIsDirty) {
            ...
            if (oLinkEntityNodes.length > 0) {
                for (var i = 0; i < oLinkEntityNodes.length; i++)
                    for (var j = i + 1; j < oLinkEntityNodes.length; j++)
                        if (oLinkEntityNodes[i].getAttribute("name") == oLinkEntityNodes[j].getAttribute("name") &&
                            oLinkEntityNodes[i].getAttribute("from") == oLinkEntityNodes[j].getAttribute("from") &&
                            oLinkEntityNodes[i].getAttribute("to") == oLinkEntityNodes[j].getAttribute("to") &&
                            oLinkEntityNodes[i].getAttribute("alias") == oLinkEntityNodes[j].getAttribute("alias")) {
                            oLinkEntityNodes = oLinkEntityNodes.splice(j, 1);
                            j--
                        }
                for (var entityNode = oFetchXml.childNodes[0].childNodes[0],
                    i = 0;
                    i < entityNode.childNodes.length;
                    i++)
            ...
            for (var sEntity,
                ...
                var oEntityNode = XUI.Xml.SelectSingleNode(oEntitiesXml,
                    'entities/entity[@name="' + sEntity + '" and @fromattr="' + sFrom + '" and @toattr="' + sTo + '"]',
                    null);
                oEntityNode != null &&
...
function removeFieldFromFetch(oFetchXml, sFieldName) {
    ...
    oEntityNode.nodeName == "link-entity" &&
...
function AddField(oActive, oFieldObj, iWidth) {
    ...
    if (oActive)
        GetGridRowNode(oGridXml).insertBefore(oCell,
            GetGridCellNode(oGridXml, oActive.getAttribute("name")).nextSibling);
    else
...
function addFieldToFetch(oFetchXml, oFieldObj, sFieldName, oEntitiesXml) {
    ...
    if (IsNull(oFieldObj.RelationshipId) || oFieldObj.RelationshipId.length == 0) {
        ...
        var oFieldName = GetFieldAliasName(sFieldName),
            oLinkEntityNode = XUI.Xml.SelectSingleNode(oFetchXml,
                'fetch/entity/link-entity[@alias="' + oFieldName.Alias + '"]',
                null);
        if (oLinkEntityNode == null) {
            var oEntityNode = XUI.Xml.SelectSingleNode(oEntitiesXml,
                'entities/entity[@alias="' + oFieldName.Alias + '"]',
                null);
...
function Move(bLeft, e) {
    ...
    if (_oActive) {
        ...
        var activeElementPos = _oActive.clientWidth + _oActive.offsetLeft,
            containerWidthInclScroll = _oActive.offsetParent.offsetParent.scrollLeft +
                _oActive.offsetParent.offsetParent.clientWidth;
        if (activeElementPos > containerWidthInclScroll) {
            var viewEditorGridArea = document.getElementById("viewEditorGridArea");
            viewEditorGridArea.scrollLeft = _oActive.offsetParent.offsetParent.scrollLeft +
                (activeElementPos - containerWidthInclScroll)
        }
    } else
