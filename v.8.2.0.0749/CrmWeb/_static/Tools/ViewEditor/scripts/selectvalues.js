...
function SelectValuesJsWindowOnLoad() {
    ...
    var oNodes = XUI.Xml.SelectNodes(_oValuesXml, "/select/option", null),
        ...
        isEntityAttributes = XUI.Xml.SelectNodes(_oValuesXml, "/select/option/@attributes", null).length == 0
            ? false
            : true,
        isSelectedEntityAttributes = !Mscrm.InternalUtilities._Script.isNullOrUndefined(_okeyAttributesXml)
            ? XUI.Xml.SelectNodes(_okeyAttributesXml, "/select/option/@keyattribute", null).length == 0 ? false : true
            : false,
        iLen = oNodes.length;
    ...
    if (disablesorting == null || XUI.Xml.GetText(disablesorting) == "false")
        if (isStateTransition)
            Quicksort(sValuesAry[1],
                0,
                sValuesAry[0].length - 1,
                sValuesAry[0],
                sValuesAry[2],
                isStateTransition,
                isEntityAttributes);
