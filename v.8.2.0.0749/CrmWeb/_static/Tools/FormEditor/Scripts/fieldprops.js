...
function FieldPropsJsWindowOnLoad() {
    ...
    if (FORM_TYPE != "quick" && FORM_TYPE != "card") {
        ...
        if (!IsNull(_cellId) &&
            ...
            .canElementBeAdded(_oField.sAttributeSchemaName, controlClassId, _objTypeCode))
...
function getDialogParams() {
    ...
    Mscrm.FormLibraryAndEventHandlerUtils.formXml = XUI.Xml
        .LoadXml(XUI.Xml.XMLSerializer.serializeToString(_oField.FormXml));
...
function initControls() {
    ...
    if ((_oField.sLabel == null || _oField.sLabel == "") &&
    ...
    case _oDataTypes.partylist:
        ...
        _chkDisableMruControl.set_dataValue(_chkDisableAutoResolveControl.get_dataValue() ||
            GetParamValue("DisableMru", "false") == "true");
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull(_oParameters) && _oParameters["ShowArticleTab"] == "true") {
        ...
        setSelectIndexFromValue("defaultLanguage",
            _oParameters["SelectDefaultLanguage"] &&
...
function IsLookupBrowse() {
    var oLookupField = XUI.Xml.SelectSingleNode(_oPropertiesXml,
        "entity/fields/field[@name='" + _sActualDBName + "']",
        null);
...
function editSchemaProps(solutionId) {
    ...
    var oPropertiesField = XUI.Xml.SelectSingleNode(_oPropertiesXml,
        "/entity/fields/field[@name = '" + _sActualDBName + "']",
        null);
    sAttributeDisplayName = XUI.Xml.SelectSingleNode(oPropertiesField,
        "displaynames/displayname[@languagecode = " + USER_LANGUAGE_CODE + "]",
        null).getAttribute("description");
    var oEntityField = XUI.Xml.SelectSingleNode(_oFieldsXml,
        "/entity/fields/field[@name = '" + _sActualDBName + "']",
        null);
    ...
    if (validateParameters()) {
                ...
                if (_oField.sDataType == _oDataTypes.lookup) {
                    ...
                    if ($get("divRelationshipFilterSection").style.display != "none") {
                        ...
                        if (_chkEnableRelFilterControl.get_dataValue()) {
                            ...
                            _oParameters["AllowFilterOff"] =
                                _chkAllowFilterOffControl.get_dataValue() ? "true" : "false"
                        } else {
        ...
        if (_oField.sControlClassId.toUpperCase() == _ControlClassIds.notes.toUpperCase()) {
            ...
            if (Mscrm.Utilities.IsKnowledgeBaseFeatureAvailable() && showArticleTab()) {
                ...
                _oParameters["SelectPrimaryCustomer"] = oKmControl.SelectPrimaryCustomer == null
                    ? ""
                    : oKmControl.SelectPrimaryCustomer;
        ...
        var currentControlClassId = IsMultiControlField(_oField.sDataType, _oField.sDataTypeFormat)
            ? Mscrm.FormControlInputBehavior.GetBehavior("selControlClassId").get_dataValue()
            : _oField.sControlClassId;
        if (Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) &&
            ...
            var _updatedFormXML = Mscrm.CustomControls.CustomControlManager.get_instance()
                .getCustomControlDOMForformXML(Mscrm.FormLibraryAndEventHandlerUtils.formXml,
                    currentControlClassId,
                    _oParameters);
            Mscrm.FormLibraryAndEventHandlerUtils.formXml = _updatedFormXML
        }
        var oField = new FieldObj(_txtLabelControl.get_dataValue() == null ? "" : _txtLabelControl.get_dataValue(),
            _chkShowLabelControl.get_dataValue(),
            _chkDisabledControl.get_dataValue(),
            null,
            _oField.sSectionName,
            _chkLockedControl.get_dataValue(),
            colSpan,
            _numRowsControl.get_dataValue(),
            bAutoExpanding,
            Mscrm.FeatureControl.get_Current().isFeatureEnabled(Mscrm.FeatureNames.CustomControlMobile) &&
            !IsNull(Mscrm.CustomControls)
            ? Mscrm.CustomControls.CustomControlManager.get_instance().get_updatedFieldRowClassId()
            : currentControlClassId,
            _oField.sDataType,
...
function validateParameters() {
    ...
    CanSpanRows(controlClassId) &&
        IsNull(_numRowsControl.get_dataValue()) &&
...
function ShowTabSections(bPreviewMode) {
    ...
    if (controlClassId == _ControlClassIds.customControl)
        controlClassId = GetControlClassIdForFormFactor(_oField.FormXml, _oField.sCustomControlUniqueId, 2);
    Mscrm.Utilities.IsKnowledgeBaseFeatureAvailable() &&
        _supportKMControl === "True" &&
        !Mscrm.InternalUtilities.JSTypes.isNull(controlClassId) &&
        (controlClassId.toUpperCase() == _ControlClassIds.searchwidget.toUpperCase() ||
            controlClassId.toUpperCase() == _ControlClassIds.notes.toUpperCase()) &&
...
function populateTabSelector(sTabName, sSectionName) {
    var oTabs = XUI.Xml.SelectNodes(_oFormXml,
            "/entity/form/tabs/tab[(not(@locklevel) or @locklevel = '0') and columns/column/sections/section[not(@locklevel) or @locklevel = '0']]",
            null),
    ...
    for (var i = 0; i < oTabs.length; i++)
        oTabSelector.AddOption(XUI.Xml.GetText(XUI.Xml
                .SelectSingleNode(oTabs[i], "labels/label[@languagecode = " + _langCode + "]/@description", null)),
            oTabs[i].getAttribute("name"));
...
function populateSectionSelector(sTabName, sSectionName) {
    ...
    for (var i = 0; i < oSections.length; i++)
        oSectionSelector.AddOption(XUI.Xml.GetText(XUI.Xml
...
function populateColumnLayoutSection() {
    ...
    } else if (CanSpanColumns(_sControlClassId)) {
...
function populateDisplaySection() {
    try {
        ...
        Mscrm.FormControlInputBehavior.GetBehavior("DefaultViewCombo")
            .set_disabled(GetParamValue("DefaultViewReadOnly", "false") == "true");
...
function CreateViewFilters() {
    ...
    if (result.Success) {
        ...
        ConstructViewList(retVal,
            ...
            $get("DefaultViewCombo"));
        Mscrm.FormXmlUtils.setDomAttribute($get("ViewListSelector"), "title", LOCID_MULTISELECT_VIEW_TITLE);
        Mscrm.FormXmlUtils.setDomAttribute($get("DefaultViewCombo"),
            "title",
            XUI.Html.GetText($get("defaultViewLabelId")))
    }
}
...
function GetRelKey(depAttr, depAttrTyp, relName, isCurPl) {
    var curNode = XUI.Xml.SelectSingleNode(_xmlDocRels.documentElement,
        "RelationInfo[RelationKey='" + relName + "']",
        null);
    if (curNode != null)
...
function CreateCurRelFilters() {
    ...
    if (result.Success) {
        ...
        for (var i = 0; i < parRelNodes.length; i++)
            optGrpParRel.AddOption(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(parRelNodes[i], "DisplayName", null)),
                XUI.Xml.GetText(XUI.Xml.SelectSingleNode(parRelNodes[i], "RelationKey", null)));
        SelCurRel.AddOptionGroup(optGrpParRel);
        SelCurRel.AddToControl($get("tdParRels"));
        $get("selCurRels").onchange = CreateTarRelFilters;
        Mscrm.FormXmlUtils.setDomAttribute($get("selCurRels"),
            "title",
            XUI.Html.GetText($get("chkEnableRelFilterLabel")))
    }
}
...
function CreateTarRelFilters() {
    ...
    var tarRelNodes = XUI.Xml.SelectNodes(_xmlDocRels.documentElement,
            "RelationInfo[RelationKey='" + $get("selCurRels").value + "']/RelationInfoCollection/RelationInfo",
            null),
    ...
    for (var i = 0; i < tarRelNodes.length; i++)
        optGrpLkupRel.AddOption(XUI.Xml.GetText(XUI.Xml.SelectSingleNode(tarRelNodes[i], "DisplayName", null)),
            XUI.Xml.GetText(XUI.Xml.SelectSingleNode(tarRelNodes[i], "RelationKey", null)));
...
function ToggleDisableMru() {
    _chkDisableMruControl.set_dataValue(_chkDisableAutoResolveControl.get_dataValue() ||
        _chkDisableMruControl.get_dataValue());
    _chkDisableMruControl.set_disabled(_chkDisableAutoResolveControl.get_dataValue())
}
    ...
    if (IsMultiControlField(_oField.sDataType, _oField.sDataTypeFormat))
        ...
        case _oDataTypes.boolean:
            ...
            if (_sControlClassId == _ControlClassIds.customControl)
                oControlClassIdSelector
                    .Selected = GetControlClassIdForFormFactor(_oField.FormXml, _oField.sCustomControlUniqueId, 2);
            else
...
function onTabChange() {
    var sTabName = $find("selTab").get_dataValue(),
        sSectionName = XUI.Xml.SelectSingleNode(_oFormXml,
            "/entity/form/tabs/tab[@name = '" +
            sTabName +
            "']/columns/column/sections/section[not(@locklevel) or @locklevel = '0']",
            null).getAttribute("name");
...
function GetCurrentTab() {
    return XUI.Xml.SelectSingleNode(_oFormXml,
        "/entity/form/tabs/tab[@id = '" + Mscrm.FormControlInputBehavior.GetBehavior("selTab").get_dataValue() + "']",
        null)
}
...
function GetCurrentSection() {
    return XUI.Xml.SelectSingleNode(_oFormXml,
        ...
        Mscrm.FormControlInputBehavior.GetBehavior("selTab").get_dataValue() +
        "']/columns/column/sections/section[@id = '" +
        selSection.get_dataValue() +
        "']",
        null)
}
