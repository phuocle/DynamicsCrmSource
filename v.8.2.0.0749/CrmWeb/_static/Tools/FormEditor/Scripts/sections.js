...
function AddSectionFromRibbon(itabCol, sSecCols, sReferencePanelSec) {
    ...
    if (sReferencePanelSec == "true") {
        var oNodes = null;
        oNodes = XUI.Xml.SelectNodes(Mscrm.FormEditorVariables.formXml,
...
function AddSection(oActive, itabCol, sSecCols, sReferencePanelSec, fClickSection, fForceAdd) {
    ...
    var oSection = new SectionObj(currentTabName,
        createGuid(),
        false,
        false,
        false,
        new Array(new LocalizedObj(LOCID_FORMEDITOR_SECTION_LABEL, _langCode)),
        "varwidth",
        sSecCols,
        "115",
        "Left",
        ...
        isReferencePanelSec);
    oSection &&
        AddSectionXml(oSection, sSecCols) &&
...
function UpdateSection(oActive) {
    ...
    var oLabels = GetLocalizedObjsArray(XUI.Xml.SelectNodes(oSection,
            "/entity/form/tabs/tab/columns/column/sections/section[@id = '" + sSectionId + "']/labels/label",
            null)),
        oSectionObj = new SectionObj(sTabId,
            sSectionId,
            bShowLabel,
            bShowBar,
            bLockSection,
    ...
    if (Mscrm.FormEditorVariables.FormType != "card") {
        var crmUri = Mscrm.CrmUri.create("/Tools/FormEditor/Dialogs/section.aspx?type=" +
                type +
                "&editorType=" +
        ...
        var callbkFunction = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(BindSectionUpdates, [oSectionObj, oActive]);
...
function BindSectionUpdates(oUpdatedSectionObj, oSectionObj, oActive) {
    if (oUpdatedSectionObj)
        if (GetDescByLangCode(oSectionObj.Labels) != GetDescByLangCode(oUpdatedSectionObj.Labels) ||
            oSectionObj.Columns != oUpdatedSectionObj.Columns ||
            oSectionObj.ShowLabel != oUpdatedSectionObj.ShowLabel ||
            oSectionObj.ShowBar != oUpdatedSectionObj.ShowBar ||
            oSectionObj.LockSection != oUpdatedSectionObj.LockSection ||
            oSectionObj.LabelWidth != oUpdatedSectionObj.LabelWidth ||
            ...
            oSectionObj.Visible != oUpdatedSectionObj.Visible ||
            oSectionObj.AvailableForPhone != oUpdatedSectionObj.AvailableForPhone ||
            oSectionObj.SectionName != oUpdatedSectionObj.SectionName ||
            oSectionObj.RowHeight != oUpdatedSectionObj.RowHeight ||
            oSectionObj.AutoExpand != oUpdatedSectionObj.AutoExpand) {
...
function AddSectionXml(oSectionObj, sSectionCols) {
    ...
    PersistLocalizedNodes(Mscrm.FormEditorVariables.formXml,
        "/entity/form/tabs/tab/columns/column/sections/section[@id = '" + oSectionObj.SectionId + "']/labels/label",
        oSectionObj.Labels);
...
function UpdateSectionXml(oSectionObj, oUpdatedSectionObj) {
    ...
    default:
        ...
        PersistLocalizedNodes(Mscrm.FormEditorVariables.formXml,
            "/entity/form/tabs/tab/columns/column/sections/section[@id = '" +
            oUpdatedSectionObj.SectionId +
            "']/labels/label",
            oUpdatedSectionObj.Labels);
    ...
    setAttributeIfNotDefaultValue(oSection,
        "availableforphone",
        oUpdatedSectionObj.AvailableForPhone.toString(),
        "true");
    if (oSectionObj.Columns != oUpdatedSectionObj.Columns)
        if (oSectionObj.Columns.length < oUpdatedSectionObj.Columns.length)
            Mscrm.SectionUtils.addColumnsToSection(oUpdatedSectionObj.Columns.length - oSectionObj.Columns.length,
                oSection);
        else if (Mscrm.SectionUtils.canReduceSecColumns(oUpdatedSectionObj.Columns.length, oSection))
...
function AddSectionHtml(oSectionObj, fClickSection) {
    var sSectionHtml = "<table id='" +
        CrmEncodeDecode.CrmHtmlAttributeEncode(oSectionObj.SectionId) +
        "' name='" +
        CrmEncodeDecode.CrmHtmlAttributeEncode(oSectionObj.SectionName) +
        "' cellspacing='4' cellpadding='4' class='section' valign='top' height='1%' width = '100%' style='table-layout: fixed;'><col/><col/><tr><td colspan='2' class='section' style='";
    if (!oSectionObj.ShowLabel)
        sSectionHtml += "color: #cccccc;";
    if (oSectionObj.ShowBar)
        sSectionHtml += "border-bottom: 1px solid #606050;";
    sSectionHtml += "'>" +
        (oSectionObj.LockSection == true
            ? "<img alt='" +
            CrmEncodeDecode.CrmHtmlAttributeEncode(LOCID_FORMEDITOR_SECTION_LOCKED) +
            "' class='imgLock' src='/_imgs/tools/ico_lock.gif' />"
            : "") +
        CrmEncodeDecode.CrmHtmlEncode(GetDescByLangCode(oSectionObj.Labels)) +
        "</td></tr></table>";
    var oTab = $get(oSectionObj.TabName),
        sectionPositionInHTML = oTab.children[0].children[0].children[0].children[1].children[1].children[0]
            .children[oSectionObj.TabColumn].children[0].children[0],
...
function RemoveSection(oActive) {
    var oSection = XUI.Xml.SelectSingleNode(Mscrm.FormEditorVariables.formXml,
            "/entity/form/tabs/tab/columns/column/sections/section[@id = '" + oActive.id + "']",
            null),
    ...
    if (confirm(LOCID_FORMED_CONFIRMREMOVESEC)) {
        ...
        !IsNull(sectionNode) &&
            ...
            .removeHandlersFromFields(XUI.Xml.SelectNodes(sectionNode, "rows/row/cell", null));
...
function LoadXmlForSectionTransform(bCellTransform) {
    ...
    resourceXml = XUI.Xml.XMLSerializer.serializeToString(XUI.Xml
        .SelectSingleNode(Mscrm.FormEditorVariables.formXml, "/entity/form/resources", null));
    if (bCellTransform)
        _oXmlDoc = XUI.Xml.LoadXml("<entity><form><cellrefresh><" +
            Mscrm.FormEditorVariables.fieldExpIsFor +
            "><rows><row><cell></cell></row></rows></" +
            Mscrm.FormEditorVariables.fieldExpIsFor +
            "></cellrefresh>" +
            resourceXml +
            "</form><metadata>" +
        ...
        _oXmlDoc = XUI.Xml.LoadXml("<entity><form><" +
            Mscrm.FormEditorVariables.fieldExpIsFor +
            "></" +
            Mscrm.FormEditorVariables.fieldExpIsFor +
            ">" +
...
function RefreshSectionHtml(oSection, sClickCellName, fClick) {
    ...
    var sectionNodeForTransform = sectionNode.cloneNode(true),
        oldSectionNodeInTransform = XUI.Xml
            .SelectSingleNode(_oXmlDoc, "/entity/form/" + Mscrm.FormEditorVariables.fieldExpIsFor, null);
...
function RefreshOnlyCellHtml(oCellId) {
    ...
    var cellNodeForTransform = cellNode.cloneNode(true),
        oldCellNodeInTransform = XUI.Xml.SelectSingleNode(_oXmlDoc,
            "/entity/form/cellrefresh/" + Mscrm.FormEditorVariables.fieldExpIsFor + "/rows/row/cell",
            null);
...
function TransformFormXml() {
    if (oPropertiesXml) {
        var propertiesNodeForTransform = XUI.Xml.SelectSingleNode(oPropertiesXml, "/entity/fields", null)
                .cloneNode(true),
            oldPropertiesNodeInTransform = XUI.Xml.SelectSingleNode(_oXmlDoc, "/entity/properties/fields", null);
        XUI.Xml.SelectSingleNode(_oXmlDoc, "/entity/properties", null)
            .replaceChild(propertiesNodeForTransform, oldPropertiesNodeInTransform)
    }
    ...
    _oXslProc.addParameter("supportSocialInsights",
        Mscrm.FormEditorVariables.supportSocialInsights == "True" ? "true" : "false");
    _oXslProc.addParameter("supportPowerBITile",
        Mscrm.FormEditorVariables.supportPowerBITile == "True" ? "true" : "false");
    _oXslProc.addParameter("supportOrgInsights",
    ...
    _oXslProc.addParameter("supportInteractionCentric",
            ...
            if (_section[iColumn][iRow] != fieldNames[iColumn] || _section[iColumn][iRow] == "") {
                ...
                if (nextCell == null || GetFieldName(nextCell) != fieldNames[iColumn]) {
                    ...
                    if (fieldNames[iColumn] == "")
                        ...
                        newCell = XUI.Xml.SelectSingleNode(oRows,
                            "row/cell[control/@id = '" + fieldNames[iColumn] + "']",
                            null);
                        if (newCell == null)
                            newCell = XUI.Xml
                                .SelectSingleNode(tempRow, "cell[control/@id = '" + fieldNames[iColumn] + "']", null)
                    }
