...
function MoveUpCell(oActive) {
    ...
    if (XUI.Html.DomUtils.GetPrevSibling(oActive.parentNode).rowIndex > 0)
        if (oActive.parentNode.cells.length == 1 ||
            XUI.Html.DomUtils.GetPrevSibling(oActive.parentNode).cells.length != 2) {
            MoveUpRows(oActive);
            bRefresh = true
        } else {
            ...
            if (XUI.Html.DomUtils.GetNextSibling(oNode)) {
                oParentNode.insertBefore(oPrevParentNode.removeChild(oPrevParentNode.childNodes[0]),
                    XUI.Html.DomUtils.GetNextSibling(oNode));
                ...
                oParentNode.insertBefore(oPrevParentNode.removeChild(XUI.Html.DomUtils.GetLastChild(oPrevParentNode)),
                    null);
                oNode = oPrevParentNode.childNodes[0];
                oParentNode = oNode.parentNode;
                oNextParentNode = XUI.Html.DomUtils.GetNextSibling(oParentNode);
                oParentNode.insertBefore(oNextParentNode
                    .removeChild(XUI.Html.DomUtils.GetPrevSibling(XUI.Html.DomUtils.GetLastChild(oNextParentNode))),
                    null)
            }
...
function MoveDown(oActive) {
    if (oActive != null)
        ...
        case "section":
            var oSection = oActive.parentNode.parentNode;
            if (XUI.Html.DomUtils.GetNextSibling(oSection) &&
                XUI.DomUtilities.HasChildElements(XUI.Html.DomUtils.GetNextSibling(oSection).cells[0])) {
...
function MoveDownCell(oActive) {
    ...
    if (XUI.Html.DomUtils.GetNextSibling(oActive.parentNode))
        if (oActive.parentNode.cells.length == 1 ||
            XUI.Html.DomUtils.GetNextSibling(oActive.parentNode).cells.length != 2) {
            ...
            var oNode = GetCellNode(oActive.getAttribute("name")),
                ...
                oPrevParentNode;
            if (XUI.Html.DomUtils.GetNextSibling(oNode)) {
                oParentNode.insertBefore(oNextParentNode.removeChild(oNextParentNode.childNodes[0]),
                    XUI.Html.DomUtils.GetNextSibling(oNode));
                ...
                oParentNode.insertBefore(oNextParentNode.removeChild(XUI.Html.DomUtils.GetLastChild(oNextParentNode)),
                    oNode);
                oNode = oNextParentNode.childNodes[0];
                oParentNode = oNode.parentNode;
                oPrevParentNode = XUI.Html.DomUtils.GetPrevSibling(oParentNode);
...
function MoveLeftCell(oActive) {
    ...
    if (XUI.Html.DomUtils.GetPrevSibling(oActive) && oActive.parentNode.cells.length > 1) {
        if (oActive.getAttribute("name") != "")
            ...
            MoveUpAdjacentXml(XUI.Html.DomUtils
                .GetNextSibling(GetCellNode(XUI.Html.DomUtils.GetPrevSibling(oActive).getAttribute("name"))));
        oActive.swapNode(XUI.Html.DomUtils.GetPrevSibling(oActive))
    }
    bRefresh &&
        RefreshSectionHtml(oActive.parentNode.parentNode.parentNode, oActive.getAttribute("name"))
...
function MoveRightCell(oActive) {
    ...
    if (XUI.Html.DomUtils.GetNextSibling(oActive) && oActive.parentNode.cells.length > 1) {
        if (oActive.getAttribute("name") != "")
            ...
            MoveDownAdjacentXml(XUI.Html.DomUtils
                .GetPrevSibling(GetCellNode(XUI.Html.DomUtils.GetNextSibling(oActive).getAttribute("name"))));
        oActive.swapNode(XUI.Html.DomUtils.GetNextSibling(oActive))
    }
    bRefresh &&
        RefreshSectionHtml(oActive.parentNode.parentNode.parentNode, oActive.getAttribute("name"))
...
function MoveUpAdjacentXml(oNode) {
    ...
    oParentNode.insertBefore(oParentNode.removeChild(XUI.Html.DomUtils.GetPrevSibling(oNode)),
        XUI.Html.DomUtils.GetNextSibling(oNode))
}
