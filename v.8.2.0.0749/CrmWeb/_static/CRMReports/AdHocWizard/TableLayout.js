...
function createColumnOrGroupingSpan(iObjectType, sId, sDisplayNameBase, sDisplayName, sWidth) {
    var sObjectTypeName = iObjectType == PROPERTY_PAGE_TYPE_GROUP ? "Grouping" : "Column",
        sSpanHtml = formatString('<div class="ColumnOrGroupingHeader" style="width: {0}px;" title="{1}"/>',
            sWidth,
            getSpanTooltip(iObjectType, sDisplayName)),
...
function getColumnIndex(oColumnOption) {
    ...
    for (var iPosition = null,
        ...
        i = 0;
        i < $get("Columns").childNodes.length && IsNull(iPosition);
        i++)
...
function getSpanTooltip(iObjectType, sDisplayName) {
    return formatString(iObjectType == PROPERTY_PAGE_TYPE_GROUP ? LOCID_TOOLTIP_GROUPING : LOCID_TOOLTIP_COLUMN,
        sDisplayName)
}
...
function initializeColumns(oAttributeInfoCache) {
    for (var oaColumns = _oWizardXmlBuilder.GetColumns(),
        ...
        if (!IsNull(sDisplayName)) {
            var oNewSpan = createColumnOrGroupingSpan(PROPERTY_PAGE_TYPE_COLUMN,
                oColumn.getAttribute("ID"),
                sDisplayName,
                sDisplayName,
    ...
    for (var oaGroupings = _oWizardXmlBuilder.GetGroupings(),
        ...
        if (!IsNull(sDisplayNameBase)) {
            var sDisplayName = getDisplayName(sDisplayNameBase, oGrouping.getAttribute("Interval")),
                oNewSpan = createColumnOrGroupingSpan(PROPERTY_PAGE_TYPE_GROUP,
                    oGrouping.getAttribute("Field"),
                    sDisplayNameBase,
                    sDisplayName,
                    oGrouping.getAttribute("Width"));
...
function initializePage() {
    ...
    if (_oWizardXmlBuilder.GetColumnCount() > 0 || _oWizardXmlBuilder.GetGroupingCount() > 0) {
        var oAttributeInfoCache = new AttributeInfoCache,
            bDisplayNamesFound = oAttributeInfoCache
                .Initialize(XUI.Xml.XMLSerializer.serializeToString(_oFetchBuilder.GetXml()), _iLocaleId);
        if (bDisplayNamesFound) {
            ...
            (bGroupingsRemoved || bColumnsRemoved) &&
                $find("notifications").AddNotification("fieldsRemoved",
                    Mscrm.Severity.SEVERITYWARNING,
                    "",
                    LOCID_FIELDS_REMOVED);
            updateSortingUI()
        }
    }
    updateGhostGroup()
...
function insertGroupingSpan(oNewGroupingSpan, iPosition) {
    ...
    if (iPosition < iGroupingCount)
        for (var i = iGroupingCount - 1; i >= iPosition; i--)
            swap(XUI.Html.DomUtils.GetFirstChild(getGroupingListItem(i)),
                XUI.Html.DomUtils.GetFirstChild(getGroupingListItem(i + 1)));
...
function move(iDirection) {
    ...
    if (iObjectType == PROPERTY_PAGE_TYPE_GROUP && (iDirection == DIRECTION_LEFT || iDirection == DIRECTION_RIGHT) ||
        ...
        return;
    var iPosition = iObjectType == PROPERTY_PAGE_TYPE_GROUP
            ? getGroupingIndex(_oSelectedColumnOrGrouping)
            : getColumnIndex(_oSelectedColumnOrGrouping),
        sItemId = _oSelectedColumnOrGrouping.id;
    switch (iDirection) {
    case DIRECTION_UP:
        if (iObjectType == PROPERTY_PAGE_TYPE_COLUMN) {
            ...
            if (iGroupingCount < GROUPING_MAX) {
                var oColumn = _oWizardXmlBuilder.GetColumn(sItemId),
                    ...
                    sTimeInterval = _oCacheManager
                        .GetFieldDataType(oAttributeInfo.Attribute, oAttributeInfo.Entity, _iLocaleId) ==
                        "datetime"
                        ? GROUPING_DEFAULT_TIME_INTERVAL
                        : null;
                _oWizardXmlBuilder.AddGrouping(sAttributeAlias,
            ...
            swap(_oSelectedColumnOrGrouping.parentNode,
                XUI.Html.DomUtils.GetFirstChild(getGroupingListItem(iPosition - 1)));
    ...
    case DIRECTION_DOWN:
        if (iPosition < _oWizardXmlBuilder.GetGroupingCount() - 1) {
            ...
            var oGrouping = _oWizardXmlBuilder.GetGrouping(sItemId),
                sColumnId = _oWizardXmlBuilder.AddColumn(sItemId,
                    oGrouping.getAttribute("Width"),
                    COLUMN_DEFAULT_SUMMARY_VALUE,
                    0);
...
function openPropertyPage(iMode, iType, oArgs, oContextNode) {
    ...
    var params = [iMode, iType, oContextNode],
        openPropertyCallBackObj = Mscrm.Utilities
            .createCallbackFunctionForXrmDialog(callBackforOpenpropertyPage, params);
...
function callBackforOpenpropertyPage(oResult, iMode, iObjectType, oContextNode) {
    ...
    if (iMode == EDIT_MODE_UPDATE) {
        if (iObjectType == PROPERTY_PAGE_TYPE_GROUP)
            _oWizardXmlBuilder.UpdateGrouping(_oSelectedColumnOrGrouping.id,
                oResult.SortOrder,
                oResult.Width,
                oResult.TimeInterval,
                oResult.SummaryValue);
        else
            _oWizardXmlBuilder.UpdateColumn(_oSelectedColumnOrGrouping.id,
                oResult.Width,
                oResult.SummaryValue,
                oResult.AddRawValueColumn);
        updateColumnOrGroupingSpan(_oSelectedColumnOrGrouping.parentNode,
    ...
    if (iMode == EDIT_MODE_CREATE) {
        !IsNull(oResult.Entity.ParentEntity) &&
            _oFetchBuilder.AddLinkEntity(oResult.Entity.Name,
                oResult.Entity.Alias,
                oResult.Entity.From,
                oResult.Entity.To,
                oResult.Entity.ParentEntity,
                oResult.Entity.EntityAlias);
        var sAttributeAlias = _oFetchBuilder.AddAttribute(oResult.Entity.Alias, oResult.Attribute),
            sItemId = sAttributeAlias,
            iPosition = iObjectType == PROPERTY_PAGE_TYPE_GROUP
                ? getGroupingIndex(oContextNode)
                : getColumnIndex(oContextNode);
        if (!IsNull(iPosition))
            iPosition++;
        if (iObjectType == PROPERTY_PAGE_TYPE_GROUP)
            _oWizardXmlBuilder.AddGrouping(sAttributeAlias,
            ...
            sItemId = _oWizardXmlBuilder.AddColumn(sAttributeAlias,
                oResult.Width,
                oResult.SummaryValue,
                iPosition,
                oResult.AddRawValueColumn);
        var sDisplayName = getDisplayName(oResult.DisplayName, oResult.TimeInterval),
            oNewSpan = createColumnOrGroupingSpan(iObjectType,
                sItemId,
                oResult.DisplayName,
                sDisplayName,
                oResult.Width);
        if (iObjectType == PROPERTY_PAGE_TYPE_GROUP) {
            var iPosition = IsNull(oContextNode)
                ? _oWizardXmlBuilder.GetGroupingCount() - 1
                : getGroupingIndex(oContextNode) + 1;
...
function removeColumnOrGrouping() {
    ...
    if (IsNull(iObjectType))
        return;
    if (iObjectType == PROPERTY_PAGE_TYPE_COLUMN &&
        reportWizardHasXmlProperty("ColumnXml") &&
        !confirm(LOCID_REMOVE_COLUMN_WARNING))
        return;
    var sItemId = _oSelectedColumnOrGrouping.id,
        sAttributeAlias = iObjectType == PROPERTY_PAGE_TYPE_GROUP
            ? sItemId
            : _oWizardXmlBuilder.GetColumn(sItemId).getAttribute("Field"),
        bRemoveAttribute = iObjectType == PROPERTY_PAGE_TYPE_GROUP
            ? _oWizardXmlBuilder.RemoveGrouping(sItemId)
            : _oWizardXmlBuilder.RemoveColumn(sItemId);
    iObjectType == PROPERTY_PAGE_TYPE_COLUMN &&
        _oFetchBuilder.IsSortAttribute(sAttributeAlias) &&
...
function setFilter() {
    ...
    if (iObjectType == PROPERTY_PAGE_TYPE_GROUP) {
        for (var oaSummaryValueColumns = _oWizardXmlBuilder.GetColumnsWithSummaryValues(),
            i = 0;
            i < oaSummaryValueColumns.length;
            i++) {
        ...
        for (var oaColumns = _oWizardXmlBuilder.GetColumns(),
            ...
            if (isNumericDataType(_oCacheManager
                .GetFieldDataType(oAttributeInfo.Attribute, oAttributeInfo.Entity, _iLocaleId))) {
                var oFilterColumn = {};
    ...
    oArgs.ScopeDisplayName = iObjectType == PROPERTY_PAGE_TYPE_GROUP
        ? XUI.Html.GetText(XUI.Html.DomUtils
            .GetChildElementAt(XUI.Html.DomUtils.GetFirstChild(getGroupingListItem(0)), 1))
        : null;
    ...
    var oUrl = Mscrm.CrmUri
        .create(formatString("/CRMReports/AdHocWizard/FilterDialog.aspx?lcid={0}&primary={1}&type={2}",
            CrmEncodeDecode.CrmUrlEncode(_iLocaleId),
            CrmEncodeDecode.CrmUrlEncode(_oFetchBuilder.GetPrimaryEntityName()),
            CrmEncodeDecode.CrmUrlEncode(iObjectType)));
...
function updateGhostGroup() {
    ...
    if (iGroupingCount < GROUPING_MAX) {
        ...
        oGhostSpan
            .innerHTML =
            formatString('<a href="javascript:addColumnOrGrouping(PROPERTY_PAGE_TYPE_GROUP)" title="{0}" id="GhostGroup">{1}</a>', CrmEncodeDecode.CrmHtmlAttributeEncode(LOCID_GHOST_GROUPING), CrmEncodeDecode.CrmHtmlEncode(LOCID_GHOST_GROUPING));
...
function updateSortingUI() {
    ...
    if (!IsNull(oSortNode)) {
        ...
        sSortColumnId = _oWizardXmlBuilder.GetColumnByAttributeAlias(oSortNode.getAttribute("attribute"))
            .getAttribute("ID")
    }
    XUI.Html.DomUtils.ForEachChild($get("Columns"),
