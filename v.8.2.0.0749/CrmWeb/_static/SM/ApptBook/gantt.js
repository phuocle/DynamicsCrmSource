...
function LoadSubjectsAsynchronouslyCallback(oResult) {
    ...
    if (typeof oResult.ReturnValue.Subjects.string == "string")
        ...
        for (var overlappedAppt = [],
            i = 0;
            i < oResult.ReturnValue.ControlIds.string.length;
            i++)
            ...
            else if (IsNull(overlappedAppt[oResult.ReturnValue.ControlIds.string[i]]))
                overlappedAppt[oResult.ReturnValue.ControlIds.string[i]] = false;
        for (var indexes = [],
            subject,
            i = 0;
            i < oResult.ReturnValue.Subjects.string.length;
            i++) {
...
function InsertRowsToGantt(oOverlappedRow, numberToInsert, html) {
    ...
    for (var oContent = $get("ganttContent"),
        ...
        if (rowInner.className == "ganttGuide") {
            if (bSetRowsHeight) {
                rowInner.setAttribute("rowsheight",
                    parseInt(rowInner.getAttribute("rowsheight"), 10) + oLastOverlappedBlockHeight);
                bSetRowsHeight = false
            }
        } else
...
function GetFirstBlock(oRow) {
    ...
    XUI.Html.DomUtils.ForEachChild(oRow,
        function(oBlock) {
...
function ResizeGuides(oContent) {
    ...
    if (Sys.Browser.agent != Sys.Browser.InternetExplorer || Sys.Browser.version != 7)
        bottomPosition -= iScrollbarWidth;
    XUI.Html.DomUtils.ForEachChild(oContent,
        function(child) {
    ...
    if (!IsNull(oUrl.get_query()["quickfind"])) {
        ...
        if (SavedQuerySelector.value != LOCID_SEARCH_RESULTS) {
            ...
            if (i >= SavedQuerySelector.length) {
                var SavedQuerySelectorBehavior = window.parent.Mscrm.FormControlInputBehavior
                        .GetElementBehavior(SavedQuerySelector),
...
function GetNextChunk(iState, rootElement) {
    ...
    for (var bUpdateBlock = !IsNull(iState); _iNextBlock < _iLen && j <= _iChunkSize; _iNextBlock++) {
        o = a[_iNextBlock];
        if (o.getAttribute("otype") == Appointment ||
            o.getAttribute("otype") == ServiceAppointment ||
            o.className.indexOf("ganttBlockOverlapped") != -1)
            if (o.className.indexOf("ganttBlockBase") != -1 || o.className.indexOf("ganttBlockOverlapped") != -1) {
                if (j >= _iChunkSize)
                    break;
...
function ZoomChanged() {
    ...
    !IsNull(window.parent) &&
        !IsNull(window.parent.SetZoomLevel) &&
