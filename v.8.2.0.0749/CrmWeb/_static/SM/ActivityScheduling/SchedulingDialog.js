...
function SchedulingDialogJsWindowOnLoad() {
    ...
    _bAutoSearch &&
        !IsNull(oServiceid.get_dataValue()) &&
        Search();
    setFocus()
}
...
function BuildProposalInfo() {
    ...
    for (var dStart,
        ...
        case "ProposalParties":
            XUI.Html.DomUtils.ForEachChild(oCellData,
                function(element) {
                    if (IsNull(element) ||
                        IsNull(XUI.Html.DomUtils.GetFirstChild(element)) ||
                        XUI.Html.DomUtils.GetFirstChild(element).tagName != "SPAN")
                        return false;
                    oValueSpan = XUI.Html.DomUtils.GetFirstChild(element);
                    aoResources.push(new AttendeeObject(Trim(XUI.Html.GetText(oValueSpan)),
                        oValueSpan.getAttribute("oid"),
                        oValueSpan.getAttribute("otype"),
                        oValueSpan.getAttribute("resourcespecid"),
                        oValueSpan.getAttribute("effort")))
                });
            break
        }
    }
...
function ValidateSearchCriteria() {
    ...
    var SearchDurationMinutes = Mscrm.FormControlInputBehavior.GetBehavior("SearchDurationMinutes");
    if (!crmForm.UseDefaultDuration.checked &&
        (SearchDurationMinutes.get_dataValue() == null || SearchDurationMinutes.get_dataValue() == 0)) {
