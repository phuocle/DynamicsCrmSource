...
function getProducts() {
    ...
    var parameters = [],
        oURL = Mscrm.CrmUri.create("/sfa/quotes/dlg_products.aspx?opportunityid=" +
            CrmEncodeDecode.CrmUrlEncode(sOppID) +
            "&name=" +
            CrmEncodeDecode.CrmUrlEncode(sOppName) +
            "&transactioncurrencyid=" +
            CrmEncodeDecode.CrmUrlEncode(Xrm.Page.data.entity.attributes.get("transactioncurrencyid").getValue()[0]
                .id)),
...
function GetQuantityDecimal(objectTypeCode, objectId, productId, uomId) {
    ...
    oXmlHTTP.open("POST",
        Mscrm.CrmUri.create("/sfa/quotes/cmd_getquantitydecimal.aspx?objectTypeCode=" +
            CrmEncodeDecode.CrmUrlEncode(objectTypeCode) +
            "&objectId=" +
            CrmEncodeDecode.CrmUrlEncode(objectId) +
            "&productId=" +
            CrmEncodeDecode.CrmUrlEncode(productId) +
            "&uomId=" +
            CrmEncodeDecode.CrmUrlEncode(uomId)).toString(),
        false);
...
function locCustomAddRelatedToNonForm(iTypeToAdd, createFromType, createFromId, additionalAttributes) {
    ...
    if (pricelevelControl.getValue() == null)
    ...
    else if (pricelevelControl.getIsDirty()) {
        ...
        var sUrl = "?_CreateFromType=" +
            CrmEncodeDecode.CrmUrlEncode(createFromType) +
            "&_CreateFromId=" +
            CrmEncodeDecode.CrmUrlEncode(createFromId) +
        ...
        else if (typeof openFrmObj == "function")
...
function UpdatePriceOverriden(refreshPrice) {
    var ispriceoverridenAttributes = Xrm.Page.data.entity.attributes.get("ispriceoverridden"),
        bIsPriceOverride = !Mscrm.InternalUtilities.JSTypes.isNull(ispriceoverridenAttributes)
            ? ispriceoverridenAttributes.getValue() == 1
            : false,
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull(CAN_OVERRIDE_PRICE) &&
        (!CAN_OVERRIDE_PRICE || CAN_OVERRIDE_PRICE === "false")) {
    ...
    if (!Mscrm.InternalUtilities.JSTypes.isNull(shipToFaxControl)) {
        ...
        Xrm.Page.data.entity.attributes.get("priceperunit")
            .setRequiredLevel(bIsPriceOverride ? Xrm.RequiredLevel.required : Xrm.RequiredLevel.none)
    }
}
...
function ProductOverriddenHandler() {
    ...
    if (!bIsExistingProduct) {
        SetAttributeIfNotNull(oPriceOverriddenAttributes.getName(),
            "prevDataValue",
            oPriceOverriddenAttributes.getValue());
        ...
        oPriceOverriddenAttributes.setValue(!Mscrm.InternalUtilities.JSTypes.isNull(priceOverrideSavedValue)
            ? priceOverrideSavedValue
            : true);
...
function createNewLineItem(sLineItemEntity, sParentEntity, sParentId, oForm, bWriteInPOroduct) {
    ...
    if (typeof RunFunctionFromParentOrSelf == "function")
        RunFunctionFromParentOrSelf("locCustomAddRelatedToNonForm",
            sLineItemEntity,
            sParentEntity,
            sParentId,
            sAdditionalAttributes);
    else {
