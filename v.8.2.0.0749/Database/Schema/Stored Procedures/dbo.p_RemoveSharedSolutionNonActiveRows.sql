...
BEGIN
	...
	AND e.BaseTableName NOT IN ('SiteMapBase', 'SystemFormBase', 'WorkflowBase')) OR t.schema_id=schema_id('MetadataSchema'))
	...
	--WHERE ent.SolutionId=@SolutionId and ent.OverwriteTime !=0 and dnb.BaseSolutionId != @SolutionId and 
	...
	--;WITH cte AS
	--(
	--SELECT dnb.BaseSolutionId, ent.SolutionId, dnb.IsSharedComponent, dnb.ObjectId, ROW_NUMBER()  OVER (PARTITION BY ent.EntityId order by OverwriteTime asc) as RowNumber
	...
	--WHERE ent.SolutionId != dnb.TopSolutionId
	--)
	--UPDATE cte SET BaseSolutionId = SolutionId, IsSharedComponent=0 WHERE RowNumber=1
	...
	BEGIN
		...
		WHERE tbl.SolutionId=''' + @SolutionId +''' AND tbl.OverwriteTime !=0 AND dnb.BaseSolutionId != ''' + @SolutionId +''' AND 
		dnb.TopSolutionId!=''' + @SolutionId +'''')
		...
		EXEC(';WITH cte AS
		(
		SELECT dnb.BaseSolutionId, tbl.SolutionId, dnb.IsSharedComponent, dnb.ObjectId, ROW_NUMBER()  OVER (PARTITION BY tbl.' + @columnName + ' order by OverwriteTime asc) as RowNumber
		...
		WHERE tbl.SolutionId != dnb.TopSolutionId
		)
		UPDATE cte SET BaseSolutionId = SolutionId, IsSharedComponent=0 WHERE RowNumber=1')
