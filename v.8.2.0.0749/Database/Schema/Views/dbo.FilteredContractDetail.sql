...
(
	...
	[ContractDetail].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 1010))
...
(
	...
	(
		...
		[ContractDetail].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 1010)
	) 
	or
	(
		-- global security
	...
	[ContractDetail].[ContractId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 1010))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredContractDetail] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredContractDetail] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
