...
(
	...
	[Campaign].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 4400))
...
(
	...
	(
		...
		[Campaign].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 4400)
	) 
	or
	(
		-- global security
	...
	[Campaign].[CampaignId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 4400))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredCampaign] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredCampaign] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
