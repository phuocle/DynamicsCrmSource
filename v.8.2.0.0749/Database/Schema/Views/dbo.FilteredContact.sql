...
select
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([Contact].[Anniversary],
			us.TimeZoneCode),
        [Contact].[Anniversary],
    ...
    dbo.fn_UTCToTzCodeSpecificLocalTime([Contact].[BirthDate],
			us.TimeZoneCode),
        [Contact].[BirthDate],
    [Contact].[Business2],
    [Contact].[Callback],
...
(
	...
	[Contact].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 2))
...
(
	...
	[Contact].[ContactId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 2))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredContact] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredContact] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
