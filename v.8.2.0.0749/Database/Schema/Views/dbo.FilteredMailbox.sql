...
(
	...
	[Mailbox].OwnerId in 
		(select OwnerId from [dbo].[fn_GetOwnerIdsForFilteredView](u.SystemUserId, 9606))
...
(
	...
	(
		...
		[Mailbox].[OwningBusinessUnit] in (select BusinessUnitId from SystemUserBusinessUnitEntityMap WITH (NOLOCK) where SystemUserId = u.SystemUserId and ObjectTypeCode = 9606)
	) 
	or
	(
		-- global security
	...
	[Mailbox].[MailboxId] in 
		(select ObjectId from [dbo].[fn_GetSharedRecordIdsForFilteredView](u.SystemUserId, 9606))
	)
)
GO
GRANT SELECT ON  [dbo].[FilteredMailbox] TO [CRMReaderRole]
GO
GRANT SELECT ON  [dbo].[FilteredMailbox] TO [PHUOCLE\ReportingGroup {8ff092ff-6d16-41c8-aeb9-43e799050400}]
GO
